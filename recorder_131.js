// ==========================================
// RECORDER.JS - V70.8 EYE-CANDY OVERLOAD (VISUAL PEAK EDITION)
// ĐÃ FIX: Viền đen động học (Kinetic Letterbox) ép tim và bung tỏa.
// CẬP NHẬT: Tàn ảnh tốc độ cao (Afterimages), RGB Split Glitch, Parallax 3D Sparks.
// CẢM XÚC: Đỉnh cao kỹ xảo AMV TikTok, không thể rời mắt, nhịp độ 3.2 giây.
...
// ====================================================================
// ĐẠO DIỄN KỸ XẢO CAO CẤP: EYE-CANDY OVERLOAD (3.2s)
// ====================================================================
window.drawProceduralIntro = function(ctx, w, h, progress) {
    ...
    // HÀM VẼ NHÂN VẬT (BỔ SUNG CƠ CHẾ ISTRAIL ĐỂ KHÔNG VẼ BÓNG/ĐỔ TÊN KHI LÀM AFTERIMAGE)
    ...
    // --- CẬP NHẬT 1: RGB CHROMATIC SPLIT (ACT 2 - TENSION) ---
    if (act === 2) {
        let rumble = (Math.random() - 0.5) * 100; ctx.translate(rumble, rumble);
        ...
        // Kỹ thuật RGB Glitch bóng đỏ/lục cực gắt
        ctx.save();
    ...
    if (act === 0) {
        ...
        // CẬP NHẬT 2: AFTERIMAGE TRAILS (TÀN ẢNH) VÀ LAO VÀO
        let easeIn = Math.pow(localProg, 3);
        let pushP1 = -300 + (easeIn * 250); 
        let pushP2 = 300 - (easeIn * 250);

        // Vẽ tàn ảnh (Trail) tốc độ cao phía sau lưng
        for(let t=1; t<=3; t++) {
        ...
        // CẬP NHẬT 3: PARALLAX 3D SPARKS (Tia lửa đa tầng bay sát màn hình)
        ctx.translate(0, 150);
        for(let i=0; i<60; i++) {
            let ang = Math.random() * Math.PI * 2; 
            let speed = Math.random(); // 0 -> 1
            let dist = localProg * (500 + speed * 1200); // Hạt bay cực xa/nhanh
            let size = (1 - localProg) * (5 + speed * 35); // Hạt bay gần thì cực to tạo chiều sâu
            ...
            ctx.rotate(ang);
            if (speed > 0.8) ctx.filter = "blur(5px)"; // Hạt gần camera bị mờ (Bokeh DOF)
    ...
    // --- CẬP NHẬT 4: KINETIC LETTERBOX (VIỀN ĐEN ÉP TIM & BUNG TỎA) ---
    let barHeight = 0;
    if (act === 0 || act === 1) {
        barHeight = 180 + (localProg * 40); // Từ 180 ép dần xuống 220
    } else if (act === 2) {
        barHeight = 220 + (localProg * 80); // STROBE: Ép kịch liệt xuống 300
    } else if (act === 3) {
        barHeight = 300 - (localProg * 1200); // CLASH: Bật tung ra ngoài màn hình (0) cực lẹ
    }
    barHeight = Math.max(0, barHeight);

    if (barHeight > 0) {
        ctx.fillStyle = "#000000"; 
        ctx.fillRect(0, 0, w, barHeight); // Trên
        ctx.fillRect(0, h - barHeight, w, barHeight); // Dưới
    }
...
// AUDIO & RECORDER CORE (GIỮ NGUYÊN HOẠT ĐỘNG HOÀN HẢO)
