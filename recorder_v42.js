// ==========================================
// RECORDER.JS - BẢN HOÀN HẢO CHO CONTENT CREATOR (V58.0 - MANGA TRASH-TALK INTRO)
// [NEW] ANIMATED INTRO: Màn hình chẻ đôi kiểu Manga, 2 nhân vật xuất hiện cà khịa trước trận (Trash Talk).
// [OPTIMIZED] Rút ngắn Thumbnail tĩnh còn 0.5s để làm Cover, nhường chỗ cho 2.5s Intro siêu ngầu.
// [RETAINED] Chống lỗi 0-byte, Tiêu đề đùn khối 3D, Meme clickbait, Word-Wrap thông minh.
// ==========================================
...
// ==========================================
// 🧠 HỆ THỐNG STORYTELLING AI (CHUYÊN NGÁCH STICKMAN VIRAL)
// ==========================================
window.StoryModeAI = {
...
// ==========================================
// 🎨 THUMBNAIL GENERATOR (TẠO POSTER MEME & GẮN TIÊU ĐỀ AI LÀM 3D TEXT)
// ==========================================
window.bakeThumbnailsForVideo = function(titleText) {
    ...
    let hue2 = (hue1 + 150 + Math.floor(Math.random() * 60 - 30)) % 360; 
    let bgStyle = Math.random() > 0.5 ? 'sunburst' : 'comic_split';

    const drawMemeBg = (ctx, w, h, isVertical) => {
        let grad = ctx.createLinearGradient(0, 0, w, h);
        grad.addColorStop(0, `hsl(${hue1}, 80%, 20%)`); grad.addColorStop(1, `hsl(${hue2}, 80%, 15%)`);
    ...
    drawMemeBg(ctxH, 1920, 1080, false);
    drawMemeBg(ctxV, 1080, 1920, true);
    ...
    const drawCharSafe = (ctx, charObj, cx, cy, scale, isFacingRight, colorGlow) => {
        if(!charObj) return;
    ...
    window.ctx = ctxH;
    drawCharSafe(ctxH, window.p1, 350, 950, (window.p1.scale || 1) * 4.8, sP1, true, `hsl(${hue1}, 100%, 60%)`);
    drawCharSafe(ctxH, e1, 1570, 950, (e1.scale || 1) * 4.8, sP2, false, `hsl(${hue2}, 100%, 60%)`);

    window.ctx = ctxV;
    drawCharSafe(ctxV, window.p1, 540, 1750, (window.p1.scale || 1) * 5.5, sP1, true, `hsl(${hue1}, 100%, 60%)`);
    drawCharSafe(ctxV, e1, 540, 750, (e1.scale || 1) * 5.5, sP2, false, `hsl(${hue2}, 100%, 60%)`);
    window.ctx = originalCtx;

    const drawClickbaitProps = (ctx, w, h, isVertical) => {
        ctx.save();
        if (Math.random() > 0.2) {
            let cx = isVertical ? w/2 : w/2 + (Math.random() > 0.5 ? 300 : -300);
            let cy = isVertical ? h*0.5 + Math.random()*150 : h/2 + Math.random()*100;
            ctx.strokeStyle = "#ff0000"; ctx.lineWidth = 25; ctx.shadowColor = "#000"; ctx.shadowBlur = 15;
            ctx.beginPath(); ctx.ellipse(cx, cy, 120 + Math.random()*50, 180 + Math.random()*50, Math.random()*0.5, 0, Math.PI*2); ctx.stroke();
            ctx.font = "150px Arial"; ctx.fillText(cx > w/2 ? "⬅️" : "➡️", cx + (cx > w/2 ? -200 : 100), cy);
        }
        if (Math.random() > 0.4) {
            let cx = isVertical ? w*0.75 : w*0.8; let cy = isVertical ? h*0.65 : h*0.4;
            ctx.translate(cx, cy); ctx.rotate((Math.random()-0.5)*0.5);
            ctx.font = "italic 900 110px Impact"; ctx.textAlign = "center";
            ctx.lineWidth = 20; ctx.strokeStyle = "#000"; ctx.strokeText("-999,999 💢", 0,0);
            ctx.fillStyle = "#ff003c"; ctx.fillText("-999,999 💢", 0,0);
        ...
        if (Math.random() > 0.5) {
            ...
            let sx = isVertical ? w*0.3 : w*0.2; let sy = isVertical ? h*0.85 : h*0.75;
            ctx.translate(sx, sy); ctx.rotate(-0.2);
    ...
    drawClickbaitProps(ctxH, 1920, 1080, false);
    drawClickbaitProps(ctxV, 1080, 1920, true);

    const draw3DTitleWrap = (ctx, w, h, text, isVertical) => {
        ctx.save();
        let grad = ctx.createRadialGradient(w/2, isVertical ? h*0.25 : h*0.2, h*0.2, w/2, isVertical ? h*0.25 : h*0.2, Math.max(w,h));
        grad.addColorStop(0, "rgba(0,0,0,0)"); grad.addColorStop(1, "rgba(0,0,0,0.85)");
        ctx.fillStyle = grad; ctx.fillRect(0,0,w,h);

        ctx.translate(w/2, isVertical ? h*0.2 : h*0.2); 
        ctx.rotate(-0.06); 
        ctx.textAlign = "center"; ctx.textBaseline = "middle";
        
        let fontSize = isVertical ? 100 : 130;
        ctx.font = `italic 900 ${fontSize}px 'Arial Black', Impact`;
        
        let words = text.split(" ");
        let lines = [];
        let currentLine = words[0];
        let maxLineWidth = isVertical ? 1000 : 1800; 
        ...
        let depth = Math.floor(fontSize * 0.15); 
        let colors = [["#f1c40f", "#d35400"], ["#00f3ff", "#0055ff"], ["#ff4757", "#8b0000"]];
        let clr = colors[Math.floor(Math.random() * colors.length)];

        let startY = -(lines.length * (fontSize + 10)) / 2 + (fontSize/2); 
        
        lines.forEach((line, index) => {
            let yOffset = startY + index * (fontSize + 15);
            ctx.lineWidth = Math.floor(fontSize * 0.25); ctx.strokeStyle = "#000";
            for(let d=depth; d>0; d--) {
                ctx.strokeText(line, d, yOffset + d);
                ctx.fillStyle = clr[1]; ctx.fillText(line, d, yOffset + d);
            }
            ctx.strokeText(line, 0, yOffset);
            ctx.fillStyle = clr[0]; ctx.fillText(line, 0, yOffset);
            ctx.fillStyle = "#ffffff"; ctx.fillText(line, -4, yOffset - 4);
        });
        ctx.restore();
    };

    let generatedTitle = titleText || "EPIC STICKMAN FIGHT 💀🔥";
    draw3DTitleWrap(ctxH, 1920, 1080, generatedTitle, false);
    draw3DTitleWrap(ctxV, 1080, 1920, true);
};

// ==========================================
// MỚI: HỆ THỐNG VẼ ANIMATED INTRO (MANGA TRASH TALK)
// ==========================================
window.drawAnimatedIntro = function(ctx, w, h, isVertical, progress) {
    let originalCtx = window.ctx; window.ctx = ctx;
    ctx.save();
    
    // Background Speedlines di chuyển nhanh
    ctx.fillStyle = "#111"; ctx.fillRect(0, 0, w, h);
    ctx.save(); ctx.translate(w/2, h/2); ctx.rotate(Math.PI/12);
    let speedOffset = (progress * 3000) % 200;
    for (let i = -w; i < w*2; i+=40) {
        ctx.fillStyle = (i%80===0) ? "rgba(255,255,255,0.15)" : "rgba(255,0,50,0.15)";
        ctx.fillRect(i - speedOffset, -h*2, 10 + Math.random()*15, h*4);
    }
    ctx.restore();

    let e1 = window.enemies && window.enemies.length > 0 ? window.enemies[0] : window.p1;
    
    // Tính toán Animation Slide In (Vào cực nhanh, giật lại)
    let easeOut = 1 - Math.pow(1 - Math.min(1, progress * 4), 3); // 0.0 -> 0.25 là xong slide
    
    let p1TargetX = isVertical ? w*0.5 : w*0.3;
    let p1TargetY = isVertical ? h*0.25 : h*0.7;
    let p1StartX = isVertical ? -w : -w*0.5;
    let p1X = p1StartX + (p1TargetX - p1StartX) * easeOut;

    let p2TargetX = isVertical ? w*0.5 : w*0.7;
    let p2TargetY = isVertical ? h*0.75 : h*0.7;
    let p2StartX = isVertical ? w*2 : w*1.5;
    let p2X = p2StartX + (p2TargetX - p2StartX) * easeOut;

    // Chẻ đôi màn hình (Clipping)
    ctx.lineWidth = 20; ctx.strokeStyle = "#fff";
    
    // P1 Panel (Nửa trái hoặc Nửa trên)
    ctx.save(); ctx.beginPath();
    if (isVertical) { ctx.moveTo(0,0); ctx.lineTo(w,0); ctx.lineTo(w, h*0.48); ctx.lineTo(0, h*0.52); }
    else { ctx.moveTo(0,0); ctx.lineTo(w*0.55, 0); ctx.lineTo(w*0.45, h); ctx.lineTo(0, h); }
    ctx.closePath(); ctx.clip();
    ctx.fillStyle = "rgba(0, 100, 255, 0.2)"; ctx.fill(); // Phủ màu team Xanh
    
    let p1Clone = Object.assign({}, window.p1, {x:0, y:0, scale: isVertical ? 3.5 : 4.0, state: 'cast', isFacingRight: true});
    ctx.translate(p1X, p1TargetY); 
    ctx.shadowColor = "#00f3ff"; ctx.shadowBlur = 40;
    if(typeof window.drawStickman === 'function') window.drawStickman(ctx, p1Clone);
    ctx.restore();

    // P2 Panel (Nửa phải hoặc Nửa dưới)
    ctx.save(); ctx.beginPath();
    if (isVertical) { ctx.moveTo(0, h*0.52); ctx.lineTo(w, h*0.48); ctx.lineTo(w, h); ctx.lineTo(0, h); }
    else { ctx.moveTo(w*0.55, 0); ctx.lineTo(w, 0); ctx.lineTo(w, h); ctx.lineTo(w*0.45, h); }
    ctx.closePath(); ctx.clip();
    ctx.fillStyle = "rgba(255, 0, 50, 0.2)"; ctx.fill(); // Phủ màu team Đỏ
    
    let p2Clone = Object.assign({}, e1, {x:0, y:0, scale: isVertical ? 3.5 : 4.0, state: 'cast', isFacingRight: false});
    ctx.translate(p2X, p2TargetY); ctx.scale(-1, 1); // Flip
    ctx.shadowColor = "#ff003c"; ctx.shadowBlur = 40;
    if(typeof window.drawStickman === 'function') window.drawStickman(ctx, p2Clone);
    ctx.restore();

    // Vẽ đường chẻ đôi màu trắng vắt ngang
    ctx.beginPath();
    if (isVertical) { ctx.moveTo(-50, h*0.52); ctx.lineTo(w+50, h*0.48); }
    else { ctx.moveTo(w*0.55, -50); ctx.lineTo(w*0.45, h+50); }
    ctx.stroke();

    // VẼ KHUNG THOẠI TRASH TALK (Manga Speech Bubbles)
    const drawSpeech = (x, y, text, isP1) => {
        ctx.save(); ctx.translate(x, y);
        let tw = ctx.measureText(text).width + 80;
        let th = 100;
        ctx.transform(1, 0, -0.15, 1, 0, 0); // Nghiêng khung
        ctx.fillStyle = "#fff"; ctx.strokeStyle = "#000"; ctx.lineWidth = 8;
        ctx.shadowColor = "#000"; ctx.shadowOffsetX = 10; ctx.shadowOffsetY = 10;
        ctx.fillRect(-tw/2, -th/2, tw, th); ctx.strokeRect(-tw/2, -th/2, tw, th);
        
        ctx.beginPath(); // Đuôi thoại
        if(isP1) { ctx.moveTo(tw/2 - 30, 0); ctx.lineTo(tw/2 + 60, 60); ctx.lineTo(tw/2 - 50, 30); }
        else { ctx.moveTo(-tw/2 + 30, 0); ctx.lineTo(-tw/2 - 60, 60); ctx.lineTo(-tw/2 + 50, 30); }
        ctx.fill(); ctx.stroke();

        ctx.transform(1, 0, 0.15, 1, 0, 0); // Trả lại để viết chữ thẳng
        ctx.fillStyle = isP1 ? "#0055ff" : "#c0392b"; ctx.shadowOffsetX = 0; ctx.shadowOffsetY = 0;
        ctx.font = "italic 900 40px 'Arial Black', Impact"; ctx.textAlign = "center"; ctx.textBaseline = "middle";
        ctx.fillText(text, 0, 0);
        ctx.restore();
    };

    ctx.font = "italic 900 40px 'Arial Black', Impact"; // Setup font trước để đo độ rộng
    
    // Khung P1 nảy lên
    if (progress > 0.3) {
        let p1Pop = Math.min(1, (progress - 0.3) * 10); // Phóng to nhanh
        ctx.save(); ctx.translate(isVertical ? w*0.5 : w*0.25, isVertical ? h*0.1 : h*0.3); ctx.scale(p1Pop, p1Pop);
        drawSpeech(0, 0, window.trashTalkP1, true); ctx.restore();
    }
    // Khung P2 đáp trả
    if (progress > 0.5) {
        let p2Pop = Math.min(1, (progress - 0.5) * 10); 
        ctx.save(); ctx.translate(isVertical ? w*0.5 : w*0.75, isVertical ? h*0.9 : h*0.7); ctx.scale(p2Pop, p2Pop);
        drawSpeech(0, 0, window.trashTalkP2, false); ctx.restore();
    }

    // HIỆU ỨNG SNAP-CUT (Chớp sáng trắng ở cuối để bay thẳng vào game)
    if (progress > 0.9) {
        let flashAlpha = (progress - 0.9) * 10;
...
window.startRecording = function() {
    ...
    // TẠO LỜI THOẠI TRASH TALK NGẪU NHIÊN CHO INTRO
    const p1Talks = ["You're already dead.", "Let's make this quick.", "I'll show you true power 💢", "Is this a joke?", "Prepare yourself!"];
    const p2Talks = ["Try me! 💀", "You talk too much.", "I'm gonna delete you.", "Bold words for a dead man.", "Pathetic."];
    ...
    // BAKE THUMBNAIL (Sẽ chớp hiện 0.5s làm ảnh bìa)
    window.bakeThumbnailsForVideo(window.StoryModeAI.viralTitle);
    window.thumbnailHoldFrames = 30; // 0.5 giây
    window.introHoldFrames = window.totalIntroFrames; // Khởi động đếm ngược 2.5s Intro Manga
...
window.captureFrames = function() {
    ...
    // GIAI ĐOẠN 1: IN THUMBNAIL TĨNH (0.5 Giây đầu để TikTok/YouTube bắt bìa)
    ...
    // GIAI ĐOẠN 2: INTRO MANGA TRASH TALK (2.5 Giây tiếp theo)
    if (window.introHoldFrames > 0) {
        let progress = 1 - (window.introHoldFrames / window.totalIntroFrames);
        
        // Đánh tiếng pít pop lúc khung thoại hiện ra
        if (window.introHoldFrames === Math.floor(window.totalIntroFrames * 0.7)) window.playSound(300, 'square', 0.2, 0.6);
        if (window.introHoldFrames === Math.floor(window.totalIntroFrames * 0.5)) window.playSound(200, 'square', 0.2, 0.6);
        if (window.introHoldFrames === 5) window.playSound(100, 'sawtooth', 0.5, 0.8, true); // Âm thanh nổ trước khi bay vào game
        ...
        return; // Ép dừng không xuất hình gameplay
    }

    // GIAI ĐOẠN 3: CAPTURE GAMEPLAY BÌNH THƯỜNG
