// ==========================================
// RECORDER.JS - V70.4 THE OPTIMIZED MASTERPIECE (ZERO-LAG SAKUGA)
// ĐÃ FIX: Xóa lệnh blur() gây lag, thay bằng Hệ thống Sương mù Gradient siêu nhẹ.
// CẬP NHẬT: Camera cầm tay trôi nổi (Handheld), Tia lửa văng tung tóe (Sparks FX), Cinematic Bloom.
// CẢM XÚC: Mượt mà 60FPS, hình ảnh sáng rực rỡ chuẩn Trailer AAA.
...
window.introDuration = 3500; 
window._sfxCuts = [false, false, false, false, false]; 
window.introParams = null; 

// HỆ THỐNG CACHE ĐỂ TỐI ƯU HIỆU NĂNG (TRÁNH LAG GAME)
...
// ====================================================================
// HỆ THỐNG QUÉT TÊN THỰC TẾ TRONG GAME (DEEP SCANNER)
// ====================================================================
window.getRealCharName = function(obj, fallback) {
    if (!obj) return fallback.toUpperCase();
    let n = null;
    if (obj.classId !== undefined && window.classStats && window.classStats[obj.classId]) {
        n = window.classStats[obj.classId].className || window.classStats[obj.classId].name;
    }
...
// ====================================================================
// ĐẠO DIỄN: HOLLYWOOD SAKUGA INTRO (TỐI ƯU HIỆU NĂNG - ZERO LAG)
// ====================================================================
window.drawProceduralIntro = function(ctx, w, h, progress) {
    ...
    // Camera trôi nổi bồng bềnh (Handheld Camera FX)
    let floatX = Math.sin(Date.now() * 0.002) * 15;
    let floatY = Math.cos(Date.now() * 0.003) * 10;
    let panX = cutIndex < 2 ? (cutProg * 100) - 50 : 0; 
    
    ctx.translate(w/2 + cutShake + panX + floatX, h/2 + cutShake + floatY);
    ctx.rotate(cut.rot);
    ...
    // Cache nền Gradient tối ưu hóa
    let bgKey = "bg_" + bgTheme.id;
    ...
    ctx.fillStyle = window._cachedGradients[bgKey]; 
    ctx.fillRect(-w, -h, w*2, h*2);
    
    // TỐI ƯU 1: HỆ THỐNG SƯƠNG MÙ KHÔNG DÙNG BLUR() CỰC NHẸ
    let fogKey = "fog_" + bgTheme.id;
    if(!window._cachedGradients[fogKey]) {
        let fog = ctx.createRadialGradient(0, 0, 0, 0, 0, 400);
        // Chuyển màu Hex sang RGBA mờ
        let hx = bgTheme.aura; 
        let r = parseInt(hx.slice(1,3), 16), g = parseInt(hx.slice(3,5), 16), b = parseInt(hx.slice(5,7), 16);
        fog.addColorStop(0, `rgba(${r},${g},${b},0.4)`); fog.addColorStop(1, `rgba(${r},${g},${b},0)`);
        window._cachedGradients[fogKey] = fog;
    }

    ctx.globalCompositeOperation = "screen"; // Ánh sáng Bloom
    ctx.globalAlpha = 0.5 + Math.sin(Date.now() * 0.003) * 0.2; // Đập nhịp thở
    ctx.save(); ctx.translate(-100 + panX, 200); ctx.fillStyle = window._cachedGradients[fogKey]; ctx.fillRect(-400,-400, 800, 800); ctx.restore();
    ctx.save(); ctx.translate(100 - panX, 250); ctx.fillStyle = window._cachedGradients[fogKey]; ctx.fillRect(-400,-400, 800, 800); ctx.restore();
    ctx.globalCompositeOperation = "source-over";
    ctx.globalAlpha = 1.0;

    // Hạt điện ảnh bay mượt mà
    ctx.fillStyle = bgTheme.aura;
    for(let i=0; i<30; i++) {
        let px = (Math.sin(i*12 + Date.now()*0.0005)*w) - w/2;
        let py = h/2 - ((Date.now()*(0.1 + i*0.01)) % h);
        ctx.globalAlpha = Math.max(0, Math.sin(Date.now()*0.002 + i));
        ctx.beginPath(); ctx.arc(px, py, 1+Math.random()*3, 0, Math.PI*2); ctx.fill();
    }
    ctx.globalAlpha = 1.0;

    // HÀM VẼ NHÂN VẬT & NAME TAG THỰC TẾ
    const drawFighterArtistic = (charObj, cx, cy, forcedState, isFacingRight, theme, fx, charName) => {
        if(!charObj) return; 
        ...
        if (charName && fx !== 'silhouette') {
            ...
            ctx.shadowColor = theme.aura; ctx.shadowBlur = 15; ctx.shadowOffsetX = 0; ctx.shadowOffsetY = 0;
        ...
        if (fx === 'silhouette') {
            ...
            // Kỹ thuật Silhouette Không dùng Filter rườm rà
            ctx.shadowBlur = 20; ctx.shadowColor = theme.aura;
        }
        ...
        if (fx === 'silhouette') ctx.restore();
        ctx.restore();
    };

    if (cut.focus === 'p1') drawFighterArtistic(window.p1, 0, 100, cut.state, true, prm.themeP1, cut.fx, realP1Name);
    else if (cut.focus === 'p2') drawFighterArtistic(e1, 0, 100, cut.state, false, prm.themeP2, cut.fx, realP2Name);
    else { 
        ...
        if (cutIndex === 3) {
            ...
            for(let i=0; i<25; i++) {
                ...
                let dist = cutProg * 600 * Math.random();
                ctx.fillStyle = Math.random() > 0.5 ? "#fff" : prm.themeP1.aura;
                ctx.globalAlpha = 1 - cutProg;
                ctx.fillRect(Math.cos(ang)*dist, Math.sin(ang)*dist, 15*(1-cutProg), 3);
            }
            ctx.globalAlpha = 1.0;
            ctx.globalCompositeOperation = "source-over";
        }
    ...
    // TỐI ƯU 2: CACHE VIGNETTE (Viền mờ)
    ...
    // CINEMATIC LETTERBOX 
    let barHeight = 250; 
    if (cutIndex === 2) barHeight = 250 - (Math.pow(cutProg, 2) * 120); 
    ...
    if (cut.txt && cutIndex < 2) {
        ...
        ctx.textAlign = "center"; ctx.textBaseline = "bottom"; ctx.font = "bold 45px Arial";
    ...
    if (cutIndex === 2 && cut.txt) {
        ...
        let txtScale = 1 + Math.pow(cutProg, 0.5) * 0.5; ctx.scale(txtScale, txtScale);
        ctx.font = "italic 900 130px Impact"; ctx.textAlign = "center"; ctx.textBaseline = "middle";
        ctx.lineWidth = 15; ctx.strokeStyle = "#000"; ctx.strokeText(cut.txt, 0,0); ctx.fillStyle = "#fff"; ctx.fillText(cut.txt, 0,0);
        ctx.restore();
    }
...
window.startRecording = function() {
    ...
    // TRUY QUÉT TÊN THỰC TẾ TRONG GAME (DEEP EXTRACTION)
