// ==========================================
// RECORDER.JS - EVENT & GIVEAWAY EDITION
// - Gỡ bỏ: Sóng âm thanh & Fake Donate (Superchat).
// - Thêm mới: Banner kêu gọi Comment/Subscribe nhận $5-$100 ghim ngay trên Game.
// - Giữ nguyên: Nền Deep Space, Chat trôi mượt mà, Action Cam Zoom, Intro/Outro.
// ==========================================
...
window.startRecording = function() {
    ...
    window.introParams = window.generateIntroParams(); window.recordedChunksV = []; window._scrollingChats = [];
...
// ================= THE MAIN RENDER LOOP (EVENT EDITION) =================
window.captureFrames = function() {
    ...
    if (renderNormalV) {
        ...
        // --- LỚP 4: CALL TO ACTION BANNER (GIVEAWAY) ---
        ctxV.save();
        let ctaY = gameY - 60; // Nằm sát ngay trên đầu khung Game
        let ctaPulse = 1 + Math.sin(now / 150) * 0.03; // Nhấp nháy to nhỏ nhẹ nhàng
        
        ctxV.translate(540, ctaY);
        ctxV.scale(ctaPulse, ctaPulse);
        
        ctxV.font = "900 32px 'Arial Black', sans-serif";
        let ctaText = "🎁 ĐĂNG KÝ & COMMENT NHẬN $5, $10, $20, $50, $100! 🎁";
        ...
        // --- LỚP 5: MÀN HÌNH GAME (ACTION CAM ZOOM) ---
        ctxV.save();
...
window.downloadRawVideo = function(vidId) {
    ...
    let a = document.createElement("a"); a.href = vid.urlV; a.download = "[EVENT_TIKTOK]_" + vid.viralTitle.replace(/[^a-z0-9]/gi, '_') + "." + vid.ext; a.click();
};
