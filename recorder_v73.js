// ==========================================
// RECORDER.JS - V67.1 ULTRA SMOOTH (FULL VERSION)
// Tích hợp Đầy đủ HUD, Intro, Outro Grid + Lõi tối ưu chống giật lag
// ==========================================
...
window.startRecording = function() {
    ...
    // [FIX SPEED VIDEO] Giảm Bitrate xuống 12 Mbps
    let options = { videoBitsPerSecond: 12000000 }; 
    window.currentVideoExt = "webm";

    if (MediaRecorder.isTypeSupported('video/webm; codecs="vp8"')) {
        options.mimeType = 'video/webm; codecs="vp8"'; // Ưu tiên VP8 chống lag real-time
    ...
    // Thêm timeslice 1000ms giúp giảm tràn RAM khi quay lâu
...
function wrapTextTikTok(ctx, text, x, y, maxWidth, lineHeight) {
    ...
    for(let i=0; i<lines.length; i++) {
        ...
        for(let j=0; j<lineWords.length; j++) {
            ...
            // Tối ưu: Bỏ shadowBlur trong Text Loop gây lag, chỉ dùng strokeText
...
function drawAudioVisualizer(ctx, x, y, width, height) {
    ...
    // Tối ưu Caching Gradient Visualizer
...
window.captureFrames = function() {
    ...
    // Giới hạn FPS ghi hình (chỉ xử lý tối đa ~60 FPS) để bảo vệ Game Thread
    ...
    // Tối ưu Glitch: Kích hoạt ngắt quãng (Throttling) thay vì liên tục mỗi frame
    ...
    if (renderNormalH) {
        ...
        if (shouldGlitch) {
            let glitchStr = ((audioPeak - 0.75) * 40) | 0;
            ctxH.globalCompositeOperation = 'screen';
            ctxH.fillStyle = 'rgba(255, 0, 0, 0.4)'; 
            ctxH.drawImage(window.canvas, 0, 0, window.canvas.width, window.canvas.height, -glitchStr, shakeY | 0, 1920 + glitchStr, 1080);
    ...
    // ==========================================
    // 3. BACKGROUND LƯỚI KHÔNG GIAN 3D (TỐI ƯU CỰC MẠNH)
    // ==========================================
    ...
    // Cache Nền Gradient 3D
    if(!window._cachedGradients.bgGrid) {
        window._cachedGradients.bgGrid = ctxV.createLinearGradient(0, retainY, 0, 1920);
        window._cachedGradients.bgGrid.addColorStop(0, "#0b001a"); 
        window._cachedGradients.bgGrid.addColorStop(1, "#3c003c");
    }
    ...
    ctxV.lineWidth = 2; // Tối ưu nét vẽ
    ctxV.beginPath(); 
    
    for(let x = -20; x <= 20; x+=2) { // Giảm mật độ đường dọc
        ctxV.moveTo(540, retainY);
        ctxV.lineTo(540 + x * 200, 1920);
    }
    let zSpeed = (Date.now() / 15) % 20; 
    for(let y = 1; y < 30; y++) { // Giảm mật độ đường ngang
    ...
    // --- Particle System (Tối ưu dùng fillRect thay vì Arc + Bỏ bóng) ---
    if (!window.retentionParticles) window.retentionParticles = [];
    if (Math.random() < 0.3 && window.retentionParticles.length < 30) {
        window.retentionParticles.push({
            x: Math.random() * 1080, y: 1920 + 50,
            s: Math.random() * 6 + 3, v: Math.random() * 8 + 4,
    ...
    for (let i = window.retentionParticles.length - 1; i >= 0; i--) {
        ...
        ctxV.fillRect(p.x | 0, p.y | 0, p.s, p.s); // fillRect nhanh hơn Arc 3 lần
    ...
    if (audioPeak > 0.35 && Math.random() < 0.25 && !window.gameOver && window.retentionEmojis.length < 15) { 
        ...
        window.retentionEmojis.push({
            ...
            v: 10 + Math.random() * 8, e: emos[(Math.random() * emos.length) | 0],
            r: (Math.random() - 0.5) * 0.5, age: 0
        });
    }
