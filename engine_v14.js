// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 13.5 [ULTRA-CORE GRAPHICS]
// [FIXED 100%: TRIỆT ĐỂ LỖI DÍNH VỆT MÀU DO TRÀN BỘ ĐỆM ALPHA/COMPOSITE]
// [NÂNG CẤP ĐỒ HỌA: DYNAMIC HIT-SPARKS, GROUND REFLECTIONS, GOD RAYS]
// ==========================================
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce) {
    ...
    if (finalDmg > 0) {
        ...
        // [NEW 13.5] DYNAMIC HIT SPARKS
        if (isCrit) {
            for(let i=0; i<8; i++) {
                window.impactSparks.push({
                    x: target.x, y: target.y - 40,
                    vx: (Math.random()-0.5)*18, vy: (Math.random()-0.5)*18,
                    life: 20 + Math.random()*15, maxLife: 35, color: color || "#f1c40f"
                });
            }
        }
...
window.draw = function() {
    ...
    // [FIX 13.5] HARD RESET 100% MATRIX TRƯỚC KHI VẼ ĐỂ CHỐNG DÍNH HÌNH
    ...
    if (window.isLoading) {
        ...
        window.ctx.fillStyle = "#00f3ff"; window.ctx.font = "italic 900 40px Arial"; window.ctx.textAlign = "center"; window.ctx.shadowBlur = 15; window.ctx.shadowColor = "#00f3ff"; window.ctx.fillText("INITIALIZING ULTRA-CORE 13.5...", cx, cy - 50);
    ...
    try {
       ...
       if (window.envHazards && window.envHazards.length > 0) {
        ...
        // TẮT BỘ LỌC ĐEN TRẮNG TRƯỚC KHI VẼ NHÂN VẬT ĐỂ CHỐNG LỖI MÀU
        window.ctx.filter = 'none';

        // ĐỔ BÓNG PHẢN CHIẾU MẶT SÀN
            ...
            allFighters.forEach(p => { 
                if (p && p.hp > 0 && p.trailArr) { 
                    p.trailArr.forEach(t => { 
                        window.ctx.save(); // FIX QUAN TRỌNG CHỐNG DÍNH MÀU TRAIL
