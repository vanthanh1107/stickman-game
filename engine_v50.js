// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 38.3 [THUMBNAIL & ANTI-GLARE EDITION]
// [RESTORED] Giữ nguyên UI Emoji, Text Manga, và Hiệu ứng không gian Râu Trắng.
// [ANTI-GLARE] Giảm 50% độ chói lóa (Screen Flash) toàn màn hình để bảo vệ mắt.
// [NEW DYNAMIC THUMBNAIL] Màn hình Loading được biến thành Meme Splash Screen (Ngẫu nhiên 100% màu, text, emoji) để record video không bị trùng lặp Thumbnail.
// ==========================================
...
window.initGameEngine = function() {
    ...
    let loadInterval = setInterval(() => {
        // Tải chậm lại một chút (kéo dài ~1.5s) để phần mềm quay video kịp bắt được Frame hình Thumbnail độc lạ này
...
window.draw = function() {
    ...
    if (window.isLoading) {
        ...
        // 1. Nền siêu sặc sỡ và ngẫu nhiên
        window.ctx.fillStyle = `hsl(${tConf.hue}, 90%, 25%)`;
        window.ctx.fillRect(0, 0, window.canvas.width, window.canvas.height);
        
        // 2. Tia sáng Sunburst manga xoay tít
        window.ctx.save();
