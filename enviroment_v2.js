// ==========================================
// ENVIRONMENT.JS - TRẠM KẾT XUẤT HIỆU ỨNG & ĐA VŨ TRỤ (V2.0)
// [TỰ ĐỘNG KẾT NỐI] Tự động đọc config.js và nạp Map mỗi khi vào trận
// [TỐI ƯU BATCHING] Gộp nét vẽ giúp 10 loại thời tiết chạy mượt ở 60 FPS
// ==========================================

// ==========================================
// 1. BỘ KHỞI TẠO MÔI TRƯỜNG & THỜI TIẾT (AUTO-SETUP)
// ==========================================
window.setupEnvironment = function() {
    // 1. Bốc thăm ngẫu nhiên 1 Map từ config.js (nếu có)
    if (window.MAPS && window.MAPS.length > 0) {
        window.currentMap = window.MAPS[Math.floor(Math.random() * window.MAPS.length)];
    } else {
        // Map dự phòng nếu config.js bị lỗi
    ...
    // Lấy loại thời tiết từ Map vừa chọn
    window.currentWeather = window.currentMap.weather || 'none';
    window.weatherParticles = [];

    // 2. Tạo hạt thời tiết tùy theo loại (Số lượng vừa đủ chống lag)
    let ptCount = 0;
    ...
    for (let i = 0; i < ptCount; i++) {
        window.weatherParticles.push({
            x: Math.random() * 2000 - 500, // Rải rộng ra ngoài màn hình
            y: Math.random() * 1000 - 200,
            speed: Math.random() * 3 + (window.currentWeather.includes('rain') ? 8 : 1),
            size: Math.random() * 2.5 + 1.5,
            ang: Math.random() * Math.PI * 2, // Dành cho Sakura xoay
            char: Math.random() > 0.5 ? "1" : "0", // Dành cho Matrix
            alpha: Math.random() * 0.5 + 0.3
        });
    }
    
    // Xóa các bẫy, vết nứt cũ
    window.envDamage = [];
    window.envHazards = [];
};

// ==========================================
// 2. HỆ THỐNG VẼ HIỆU ỨNG (RENDERERS)
// ==========================================
...
window.drawWeatherEffects = function(ctx) {
    ...
    // GỘP LỆNH VẼ (BATCHING) CHỐNG LAG
...
// ==========================================
// 3. TỰ ĐỘNG GẮN VÀO ENGINE CŨ MÀ KHÔNG CẦN SỬA MAIN.JS
// ==========================================

// BẮT SỰ KIỆN: Khi hàm Reset Trận của Engine được gọi -> Chạy Auto-Setup
if (!window._hookedEnvSetup) {
    window._hookedEnvSetup = true;
    
    // Gắn móc vào resetMatchVariables của file main.js
...
// BẮT SỰ KIỆN: Vẽ môi trường vào Camera
