...
// [BẢN CẬP NHẬT CINEMATIC CAMERA - ZOOM CHUYÊN NGHIỆP KHI KẾT THÚC & MƯỢT 100%]
// ==========================================
...
window.spawnEnvDamage = function(x, y, type, scale, isBurning = false) {
    let cracks = []; 
    // Giảm số lượng vết nứt xuống 1 đến 2 vết cho chuyên nghiệp và Clean
...
window.update = function() {
    ...
    if (window.gameOver && window.p1) {
        ...
        // 1. Khóa mục tiêu vào P1 (Winner nếu thắng, hoặc Loser đang nằm nếu thua).
        // Tránh việc bị trượt cam ra khoảng không vô định.
        midPointX = window.p1.x;
    } else if (window.p1 && window.enemies.length > 0) {
        // TRONG TRẬN ĐẤU: Focus vào trung điểm giữa P1 và Kẻ địch
    ...
    if (!window.isSpinningCam) {
        // Tốc độ bắt cam: lúc game over có thể chậm lại tí cho ngầu
    ...
    // Điều chỉnh Y focus để không bị trôi đi
    if (window.gameOver) {
        // Giữ camera không bị giật Y
    ...
    if (window.gameOver) {
        // Zoom cận cảnh chuyên nghiệp khi hết trận
        window.targetZoom = 1.8; 
        window.targetTilt = 0; // Tắt nghiêng màn hình
