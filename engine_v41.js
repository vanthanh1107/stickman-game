// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 35.0 [THE 360 MATRIX REVOLUTION]
// [FIXED] PERFECT Y-FRAMING: Camera luôn lấy nét trọn vẹn cả thân dưới và mặt sàn phản chiếu.
// [FIXED] INFINITE 360 BACKGROUND: Phông nền trượt theo module động, xoay bao nhiêu vòng cũng không bị mất cảnh.
// [NEW] TRUE DUAL-ORBIT 3D: Máy quay tự động lấy tâm điểm giữa 2 nhân vật và xoay tròn 360 độ chuẩn Ma Trận.
// [BASE] Giữ nguyên toàn bộ tuyệt kỹ: Số nảy Lò xo, Lethal Void, RTX Floor, Tách màu quang học.
// ==========================================
...
// [VIRAL 35.0] 3D MATRIX CAMERA VARS
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce, attackerDirRight = true) {
    ...
    if (finalDmg > 0) {
        ...
        if (isCrit) {
            ...
            // [VIRAL 34.0] GÓC QUAY CHÉO KHI ĐÁNH CHÍ MẠNG (Tạo chiều sâu 3D)
        ...
        if (target.hp <= 0) {
            ...
            if (isNearWall && isCrit && typeof window.triggerStageTransition === 'function') { window.triggerStageTransition(target); } 
                ...
                if (isCrit) {
                    ...
                    // [VIRAL 34.0] MATRIX 360 DUAL-ORBIT LETHAL FINISHER
                    window.isSpinningCam = true; 
                    window.orbitFocusX = (target.x + window.p1.x) / 2; // Căn chính giữa 2 người
...
window.update = function() {
    ...
    // [VIRAL 35.0] THUẬT TOÁN 360 DUAL-ORBIT VÀ KHÓA TÂM Y-AXIS
    if (window.isSpinningCam) {
        window.camOrbitAngle += 0.06; // Quay đều khi đóng băng Ma trận
    } else {
    ...
    // Luôn bắt nét vào giữa trận chiến
    ...
    // SỬA LỖI MẤT GÓT CHÂN: Ép trọng tâm máy quay bám chặt vào mặt đất
    window.targetCamY = (window.canvas.height / 2) - window.orbitFocusY + window.actionCamOffsetY + 40;
...
window.draw = function() {
    ...
    let impactShift = 0;
    if (window.impactAberration > 0) { impactShift = window.impactAberration * (Math.random() > 0.5 ? 1 : -1); }

    // [VIRAL 35.0] MA TRẬN 3D - HOÀN CHỈNH CHIỀU SÂU Z-AXIS
    ...
    let project3D = (obj) => {
        ...
        // CHÂN BÁM CHẶT ĐẤT KHI QUAY
        let pY = obj.y - pZ * 0.05; 
    ...
    if (window.invertFrames > 0) {
        ...
        sortedFighters.forEach(p => { 
            if (p && p.hp > 0 && typeof window.drawStickman === 'function') { 
                ...
                else window.drawStickman(window.ctx, clone); window.ctx.restore(); 
            } 
    ...
    if (window.isLoading) {
        ...
        window.ctx.fillStyle = "#00f3ff"; window.ctx.font = "italic 900 40px Arial"; window.ctx.textAlign = "center"; window.ctx.shadowBlur = 15; window.ctx.shadowColor = "#00f3ff"; window.ctx.fillText("MATRIX 360 REVOLUTION...", cx, cy - 50); window.ctx.shadowBlur = 0; window.ctx.strokeStyle = "#333"; window.ctx.lineWidth = 4; window.ctx.strokeRect(cx - 250, cy, 500, 24); window.ctx.fillStyle = "#ff4757"; window.ctx.shadowBlur = 20; window.ctx.shadowColor = "#ff4757"; window.ctx.fillRect(cx - 247, cy + 3, (window.loadProgress / 100) * 494, 18); window.ctx.shadowBlur = 0; window.ctx.fillStyle = "#fff"; window.ctx.font = "bold 16px monospace"; window.ctx.fillText(`RENDERING PERFECT FRAME... ${Math.floor(window.loadProgress)}%`, cx, cy + 60); return; 
    }
    ...
    try {
        ...
        // [VIRAL 35.0] INFINITE BACKGROUND WRAPPING
        // Đảm bảo nền trượt vô tận không bị mất khi xoay 360 độ
...
window.gameLoop = function(timestamp) { 
    ...
    if (window.hitStopFrames > 0) {
        ...
        // [VIRAL 34.0] Xoay mượt ngay cả khi đóng băng thời gian
        if (window.isSpinningCam) { window.camOrbitAngle += 0.08 * (deltaTime / 16.666); } 
    ...
    if (window.isSpinningCam) { window.camOrbitAngle += 0.06 * (deltaTime / 16.666); } 
    else { window.camOrbitAngle += (window.targetCamOrbitAngle - window.camOrbitAngle) * 0.1 * (deltaTime / 16.666); }
