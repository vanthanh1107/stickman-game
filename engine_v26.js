// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 24.0 [THE ANIME COMPOSITOR]
// [NEW] SPIDER-VERSE MANGA IMPACT: Chuyển đổi style nét vẽ tay trong 3 frames khi Crit.
// [NEW] HIT-STOP STUTTER: Đóng băng khung hình tạo sức nặng vật lý chuẩn Anime (Studio Bones).
// [NEW] DESPERATION VHS: Màn hình nhiễu sóng sọc ngang khi HP dưới 20% tạo kịch tính.
// [NEW] BREATHING ASPECT RATIO: Khung hình co bóp theo tốc độ di chuyển.
// ==========================================
...
window.cinemaBarsHeight = 0; // Co bóp khung hình động
...
// ==========================================
// 1. HỆ THỐNG COLOR GRADING CHUẨN ĐIỆN ẢNH
// ==========================================
window.initGameEngine = function() {
    ...
    window.viralColorGrade = {
        contrast: 115 + Math.random() * 20,     // Tương phản cao hơn
        brightness: 85 + Math.random() * 15,    // Tối đi một chút để deep hơn
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce) {
    ...
    if (target.state === 'block') { 
        if (target.stamina < 15) {
            ...
            window.hitStopFrames = 25; // Hit-stop nặng hơn khi bể khiên
    ...
    if (finalDmg > 0) {
        ...
        if (isCrit) {
            // [VIRAL 24.0] SPIDER-VERSE MANGA IMPACT & HIT-STOP STUTTER
            window.impactFrameCount = 4; // Kéo dài frame giật nét vẽ tay
            window.targetZoom = 1.4; 
            window.dynamicBlur = 15; 
            window.actionCamOffsetX = (target.isFacingRight ? 80 : -80); 
            ...
            // Hit-Stop cực nặng (đóng băng game hoàn toàn trong tích tắc rồi xả)
            window.hitStopFrames = 15; 
            window.shakeScreen(20, 18);
            
            window.targetTimeScale = 0.1; setTimeout(()=>{ window.targetTimeScale = 1.3; setTimeout(()=>{ window.targetTimeScale = 1.0; }, 200); }, 350);
        ...
        if (target.hp <= 0) {
            ...
            if (isNearWall && isCrit && typeof window.triggerStageTransition === 'function') { window.triggerStageTransition(target); } 
                ...
                if (isCrit) {
                    ...
                    window.hitStopFrames = 30; // Kéo dài Hit-Stop khi kết liễu
                    window.shakeScreen(60, 45); // Rung nát màn hình
...
window.attack = function(attacker, targetGroup) {
    ...
    if (MathDist > reach && !isFinisher) { 
        ...
        // [VIRAL 24.0] BREATHING ASPECT RATIO KHI LƯỚT
        window.cinemaBarsHeight = 150; // Khép nhẹ viền tạo cảm giác xé gió
...
window.update = function() {
    ...
    // Đạo diễn trả góc quay & viền màn hình về mặc định
...
window.draw = function() {
    ...
    // [VIRAL 24.0] SPIDER-VERSE MANGA IMPACT (KHUNG HÌNH TRUYỆN TRANH KHI CRIT)
    if (window.impactFrameCount > 0) {
        window.ctx.save();
        // Nền đen trắng chớp giật
        window.ctx.fillStyle = window.impactFrameCount % 2 === 0 ? "#000000" : "#ffffff"; 
        window.ctx.fillRect(0, 0, window.canvas.width, window.canvas.height);
        
        let allFighters = [window.p1].concat(window.enemies);
        
        // Vẽ nhân vật nét phác thảo (Sketch)
    ...
    if (window.isLoading) {
        ...
        window.ctx.fillStyle = "#00f3ff"; window.ctx.font = "italic 900 40px Arial"; window.ctx.textAlign = "center"; window.ctx.shadowBlur = 15; window.ctx.shadowColor = "#00f3ff"; window.ctx.fillText("ANIME COMPOSITOR INITIALIZING...", cx, cy - 50);
    ...
    try {
        ...
        // [VIRAL 24.0] DESPERATION VHS SCANLINES (HP < 20%)
        if (window.p1 && window.p1.hp > 0 && window.p1.hp <= window.p1.maxHp * 0.2 && !window.gameOver) {
            ...
            window.ctx.fillStyle = bloodGrad; window.ctx.globalCompositeOperation = 'multiply'; 
            window.ctx.fillRect(0, 0, window.canvas.width, window.canvas.height); 
            window.ctx.globalCompositeOperation = 'source-over';

            // Scanlines
            window.ctx.fillStyle = "rgba(0,0,0,0.15)";
            for(let i=0; i<window.canvas.height; i+=4) {
                window.ctx.fillRect(0, i + (window.matchTimer%4), window.canvas.width, 1);
            }
        }

        // [VIRAL 24.0] BREATHING ASPECT RATIO (Viền đen co bóp)
...
// ==========================================
// 7. [VIRAL 24.0] AFTER EFFECTS TIME-RAMPING & UNLOCKED 120Hz LOOP
// ==========================================
