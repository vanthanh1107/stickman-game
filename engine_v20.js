// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 18.0 [APEX VISUALS]
// [FIXED] COMBO SSS GỌN GÀNG, CHỈ HIỂN THỊ RANK KHỔNG LỒ, KHÔNG CHE MÀN HÌNH
// [NEW FEATURE] SF6 INK SPLATTER: Vệt mực thư pháp siêu lực phong cách Street Fighter 6
// ==========================================
...
// [VIRAL 18.0] HỆ THỐNG VFX ĐỈNH CAO
window.mangaSfx = []; // Hiệu ứng chữ truyện tranh Anime
window.dimensionCracks = []; // Nứt vỡ không gian
window.inkSplatters = []; // Vệt mực thư pháp (SF6 Style)
...
// [VIRAL 18.0] SFX INK SPLATTER (Street Fighter 6 Style)
window.spawnInk = function(x, y, color, isRight) {
    window.inkSplatters.push({
        x: x, y: y, color: color, life: 35, maxLife: 35, 
        scale: Math.random() * 0.8 + 0.6, 
        ang: (Math.random() - 0.5)*0.5 + (isRight ? 0 : Math.PI)
    });
}
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce) {
    ...
    if (finalDmg > 0) {
        ...
        if (isCrit) {
            ...
            window.spawnInk(target.x, target.y - 40, color || "#ff003c", target.isFacingRight); // [VIRAL 18.0] Spawn mực SF6
...
window.draw = function() {
    ...
    if (window.isLoading) {
        ...
        window.ctx.fillStyle = "#00f3ff"; window.ctx.font = "italic 900 40px Arial"; window.ctx.textAlign = "center"; window.ctx.shadowBlur = 15; window.ctx.shadowColor = "#00f3ff"; window.ctx.fillText("INITIALIZING APEX VISUALS 18.0...", cx, cy - 50);
    ...
    try {
        ...
        // VẼ INK SPLATTER (Street Fighter 6) NẰM SAU NHÂN VẬT ĐỂ TẠO CHIỀU SÂU
        ...
        // [VIRAL 18.0] HỆ THỐNG DEVIL MAY CRY COMBO SSS RANKING (TỐI GIẢN & SIÊU LỚN)
        ...
        // [VIRAL 18.0] Kính vỡ 4th Wall (Screen Shatter) khi K.O
