// Khai báo biến toàn cục (Sử dụng chung cho toàn bộ Game)
var canvas = null, ctx = null, audioCtx = null, isMuted = false;
var selectedRedClass = null;
...
function toggleAudio(e) { e.stopPropagation(); isMuted = !isMuted; let btn = document.getElementById("btn-audio"); if(btn) btn.innerText = isMuted ? "🔇" : "🔊"; if (!isMuted && audioCtx && audioCtx.state === 'suspended') { audioCtx.resume(); } }
            ...
            if (closest && closest.hp > 0) {
                ...
                if (absDist > reach) {
                    ...
                    if (f.aiDelay <= 0) {
                        ...
                        let usedSkill = false;
                        if (f.skill && typeof f.skill.actionCode3 === 'function' && f.stamina >= 100 && Math.random() < 0.05) { f.stamina -= 100; usedSkill = true; triggerCinematic(f, () => { f.superArmor = 25; try { f.skill.actionCode3(f, closest, gameContext); if(f.state==='idle') { f.state = 'cast'; f.attackTimer = 15; } } catch (e) {} }); }
                        if (!usedSkill) {
                            ...
                            if (closest.attackTimer > 0 || closest.state === 'dash') {
                                ...
                                if (rand < 0.9) {
                                    ...
                                    f.comboTimer = 50; attack(f, targetGroup);
            ...
            allFighters.forEach(p => { 
                if (p.trailArr) { 
                    p.trailArr.forEach(t => { 
                        ...
                        if (trailP.isDragon && typeof drawDragon === 'function') drawDragon(ctx, trailP, true); 
