// ==========================================
// ENGINE.JS - LÕI VẬT LÝ VÀ CHUYỂN ĐỘNG (ĐÃ FIX LỖI TƯỜNG TÀNG HÌNH)
// ==========================================
...
window.update = function() {
    ...
    allFighters.forEach(f => {
        ...
        if (f.isGuardBroken) {
            f.shieldBreak += 0.4; // Tốc độ hồi giáp
        ...
        // FIX LỖI BỨC TƯỜNG TÀNG HÌNH Ở BIÊN CANVAS! (Thay 600 thành window.canvas.width)
        let bounds = 30 * (f.scale || 1);
        if (f.x < bounds) { 
            f.x = bounds; 
            if (f.hitStun > 0 && f.vx < -4) { f.vx = -f.vx * 0.4; f.hitStun = 5; window.shakeScreen(5, 3); if(typeof window.takeDamage === 'function') window.takeDamage(f, Math.floor(Math.random() * 4) + 4, "#fff", false, true); window.playSound(100, 'sine', 0.2, 0.3, true); window.spawnDust(f.x, f.y); } else if(f.state !== 'walk' && f.state !== 'dash_back') { f.vx = 0; } 
        }
    ...
    for (let i = 0; i < allFighters.length; i++) {
        for (let j = i + 1; j < allFighters.length; j++) {
            let f1 = allFighters[i], f2 = allFighters[j]; let overlapX = f2.x - f1.x; let pushDist = 30 * Math.max(f1.scale||1, f2.scale||1);
...
window.draw = function() {
    ...
    try {
        ...
        window.ctx.save(); window.ctx.fillStyle = "rgba(255, 255, 255, 0.5)"; window.ctx.strokeStyle = "rgba(255, 255, 255, 0.4)"; window.ctx.lineWidth = 1; window.weatherParticles.forEach(w => { if (window.currentWeather === 'snow') { window.ctx.beginPath(); window.ctx.arc(w.x + window.camX * 0.8, w.y, 2, 0, Math.PI*2); window.ctx.fill(); } else if (window.currentWeather === 'rain') { window.ctx.beginPath(); window.ctx.moveTo(w.x + window.camX * 0.8, w.y); window.ctx.lineTo(w.x - 5 + window.camX * 0.8, w.y + 15); window.ctx.stroke(); } }); window.ctx.restore();
        ...
        if (window.introTimer > 0 && !window.gameOver && window.p1) {
            ...
            if (window.introTimer > 60) {
                ...
                if (window.enemies && window.enemies.length > 0) { let repEnemy = window.enemies[0]; let p2Clone = Object.assign({}, repEnemy, {x: slideX2, y: window.canvas.height/2 + 30, state: 'idle', isFacingRight: false}); if (repEnemy.isDragon && typeof window.drawDragon === 'function') window.drawDragon(window.ctx, p2Clone); else if (typeof window.drawStickman === 'function') window.drawStickman(window.ctx, p2Clone); if (window.introTimer < 130 && window.introTimer > 70) { window.ctx.fillStyle = "rgba(255, 255, 255, 0.9)"; window.ctx.fillRect(slideX2 - 20, window.canvas.height/2 - 140, 40, 30); window.ctx.fillStyle = "#111"; window.ctx.font = "bold 20px Arial"; window.ctx.fillText(repEnemy.taunt || "🤖", slideX2, window.canvas.height/2 - 118); } }
