// ==========================================
// ENGINE.JS - TRÁI TIM CỦA GAME (ĐÃ FIX LỖI ĐEN MÀN HÌNH)
// ==========================================
...
// Gắn tất cả biến lên window để các file khác (combat, main, fighter) đọc được
...
window.currentZoom = 1; window.targetZoom = 1;

window.currentWeather = 'none'; window.weatherParticles = [];
window.GROUND_Y = 320; window.GRAVITY = 0.8;
window.lastFrameTime = 0; window.FRAME_MIN_TIME = 1000 / 60;
...
window.toggleAudio = function(e) { e.stopPropagation(); window.isMuted = !window.isMuted; let btn = document.getElementById("btn-audio"); if(btn) btn.innerText = window.isMuted ? "🔇" : "🔊"; if (!window.isMuted && audioCtx && audioCtx.state === 'suspended') { audioCtx.resume(); } }
...
window.update = function() {
    ...
    window.weatherParticles.forEach(w => { w.y += w.speed; w.x += (window.currentWeather === 'rain') ? -2 : Math.sin(w.y/50)*2; if(w.y > canvas.height + 20) { w.y = -20; w.x = Math.random() * 1200 - 300; } });
    ...
    if (Math.random() < 0.12) { window.particles.push({ x: Math.random() * canvas.width, y: window.GROUND_Y, vx: (Math.random() - 0.5) * 1, vy: -Math.random() * 2 - 0.5, life: 40, maxLife: 40, color: "rgba(255, 159, 67, 0.35)", size: Math.random() * 3 + 1 }); }
    ...
    window.enemies = window.enemies.filter(e => { 
        if(e.hp <= 0) { 
            ...
            if (window.p1 && window.p1.hp > 0) {
                ...
                let sT = ""; if(window.p1.killCount===2) sT="DOUBLE 💀💀"; else if(window.p1.killCount===3) sT="TRIPLE 💀💀💀"; else if(window.p1.killCount===5) sT="RAMPAGE 🔥"; else if(window.p1.killCount>=8) sT="GODLIKE 👑";
                if(sT) window.floatingTexts.push({ x: window.p1.x, y: window.p1.y - 120, text: sT, color: "#ff4757", alpha: 1, vx: 0, vy: -5, font: "italic 900 32px Arial", life: 50 });
            } return false; 
        } return true; 
    });
            ...
            if (closest && closest.hp > 0) {
                ...
                if (absDist > reach) {
                    ...
                    if (f.aiDelay <= 0) {
                        ...
                        let usedSkill = false;
                        if (f.skill) {
                             if (f.stamina >= 100 && typeof f.skill.actionCode3 === 'function' && Math.random() < 0.05) { f.stamina -= 100; usedSkill = true; window.triggerCinematic(f, () => { f.superArmor = 25; try { f.skill.actionCode3(f, closest, gameContext); if(f.state==='idle') { f.state = 'cast'; f.attackTimer = 15; } } catch (e) {} }); }
    ...
    if (window.p1) {
        ...
        if (b1 && b2 && b3 && bDodge) { b1.className = (window.p1.stamina >= 25) ? "skill-btn s1-ready" : "skill-btn"; b2.className = (window.p1.stamina >= 50) ? "skill-btn s2-ready" : "skill-btn"; b3.className = (window.p1.stamina >= 100) ? "skill-btn s3-ready" : "skill-btn"; bDodge.className = (window.p1.stamina >= 15) ? "skill-btn s-dodge-ready" : "skill-btn"; }
    }

    for (let i = window.projectiles.length - 1; i >= 0; i--) { let proj = window.projectiles[i]; proj.x += proj.vx; proj.y += proj.vy; let dx = proj.x - proj.target.x; let dy = proj.y - proj.target.y; if (Math.sqrt(dx*dx + dy*dy) < proj.radius + 20) { if(proj.onHit) proj.onHit(); if(typeof window.takeDamage === 'function') window.takeDamage(proj.target, proj.dmg, "#9b59b6", false, false); window.shakeScreen(8, 4); window.projectiles.splice(i, 1); } else if (proj.x < -100 || proj.x > canvas.width + 100 || proj.y < -100 || proj.y > canvas.height + 100) { window.projectiles.splice(i, 1); } }
...
// -----------------------------------------------------
// HÀM VẼ (RENDER): XUẤT HIỆN HÌNH ẢNH TRÊN MÀN HÌNH
// -----------------------------------------------------
window.draw = function() {
    ...
    try {
        ctx.translate(canvas.width/2, canvas.height/2); ctx.scale(window.currentZoom, window.currentZoom); ctx.translate(-canvas.width/2, -canvas.height/2);
        
        if (window.slowMoTimer > 0) { let loserX = (window.p1 && window.p1.hp <= 0) ? window.p1.x : (window.enemies.length > 0 ? window.enemies[0].x : 300); let targetCamX = (canvas.width / 2) - loserX; window.camX += (targetCamX - window.camX) * 0.1; ctx.translate(canvas.width/2, canvas.height/2); ctx.scale(1.2, 1.2); ctx.translate(-canvas.width/2 + window.camX, -canvas.height/2 + 20); } 
        else if (window.p1 && !window.gameOver && window.introTimer === 0) { let closest = window.getClosestEnemy(window.p1, window.enemies); let centerX = closest ? (window.p1.x + closest.x) / 2 : window.p1.x; let targetCamX = (canvas.width / 2) - centerX; targetCamX = Math.max(-100, Math.min(100, targetCamX)); window.camX += (targetCamX - window.camX) * 0.1; ctx.translate(window.camX, 0); }
        if (window.shakeTime > 0) ctx.translate((Math.random() - 0.5) * window.shakeMag, (Math.random() - 0.5) * window.shakeMag); 

        ctx.fillStyle = "#1e272e"; ctx.fillRect(-400, -100, canvas.width + 800, canvas.height + 100);
        ctx.save(); ctx.translate(window.camX * 0.2, 0); ctx.fillStyle = "#2f3640"; for(var i = -500; i < canvas.width + 1000; i += 120) { ctx.fillRect(i, window.GROUND_Y - 150 + Math.sin(i)*30, 80, 150); } ctx.restore();
        ctx.save(); ctx.translate(window.camX * 0.5, 0); ctx.fillStyle = "#353b48"; for(var i = -500; i < canvas.width + 1000; i += 90) { ctx.beginPath(); ctx.moveTo(i, window.GROUND_Y); ctx.lineTo(i + 45, window.GROUND_Y - 100); ctx.lineTo(i + 90, window.GROUND_Y); ctx.fill(); } ctx.restore();
        
        ctx.fillStyle = "#111"; ctx.fillRect(-400, window.GROUND_Y, canvas.width + 800, canvas.height - window.GROUND_Y); 
        ctx.strokeStyle = "#ff4757"; ctx.lineWidth = 4; ctx.beginPath(); ctx.moveTo(-400, window.GROUND_Y); ctx.lineTo(canvas.width + 400, window.GROUND_Y); ctx.stroke();
        ctx.strokeStyle = "#222"; ctx.lineWidth = 2; for(var i = -400; i < canvas.width + 400; i+=50) { ctx.beginPath(); ctx.moveTo(i, window.GROUND_Y); ctx.lineTo(i - 20, canvas.height); ctx.stroke(); }
        ...
        window.weatherParticles.forEach(w => { if (window.currentWeather === 'snow') { ctx.beginPath(); ctx.arc(w.x + window.camX * 0.8, w.y, 2, 0, Math.PI*2); ctx.fill(); } else if (window.currentWeather === 'rain') { ctx.beginPath(); ctx.moveTo(w.x + window.camX * 0.8, w.y); ctx.lineTo(w.x - 5 + window.camX * 0.8, w.y + 15); ctx.stroke(); } });
        ctx.restore();

        window.traps.forEach(t => { ctx.beginPath(); ctx.arc(t.x, t.y, t.radius, 0, Math.PI*2); ctx.fillStyle = t.color; ctx.globalAlpha = Math.max(0, Math.min(1, t.life / t.maxLife)) * 0.5; ctx.fill(); ctx.globalAlpha = 1.0; });
        window.projectiles.forEach(proj => { ctx.beginPath(); ctx.arc(proj.x, proj.y, proj.radius, 0, Math.PI * 2); ctx.fillStyle = proj.color; ctx.fill(); });
        
        ctx.globalCompositeOperation = 'lighter';
        window.shockwaves.forEach(sw => { ctx.beginPath(); ctx.arc(sw.x, sw.y, sw.r, 0, Math.PI*2); ctx.lineWidth = 5; ctx.strokeStyle = sw.color; ctx.globalAlpha = Math.max(0, Math.min(1, sw.alpha)); ctx.stroke(); });
        window.impactSparks.forEach(isp => { ctx.save(); ctx.translate(isp.x, isp.y); ctx.globalAlpha = Math.max(0, Math.min(1, isp.life / isp.maxLife)); ctx.fillStyle = isp.color; ctx.beginPath(); let len = Math.sqrt(isp.vx*isp.vx + isp.vy*isp.vy) * 2; let ang = Math.atan2(isp.vy, isp.vx); ctx.rotate(ang); ctx.ellipse(0, 0, len, 2, 0, 0, Math.PI*2); ctx.fill(); ctx.restore(); });
        ctx.globalCompositeOperation = 'source-over';

        if (window.p1) {
            let allFighters = [window.p1].concat(window.enemies); ctx.globalCompositeOperation = 'lighter';
            allFighters.forEach(p => { 
                if (p.trailArr) { 
                    p.trailArr.forEach(t => { 
                        ...
                        if (trailP.isDragon && typeof window.drawDragon === 'function') window.drawDragon(ctx, trailP, true); 
                        else if (typeof window.drawStickman === 'function') window.drawStickman(ctx, trailP, true); 
                    }); 
                } 
            });
            ctx.globalCompositeOperation = 'source-over';
            if (window.p1.stamina >= 100) { ctx.shadowBlur = 20; ctx.shadowColor = "#f1c40f"; } 
            
            window.enemies.forEach(e => {
                if(e.isDragon && typeof window.drawDragon === 'function') window.drawDragon(ctx, e);
                else if (typeof window.drawStickman === 'function') window.drawStickman(ctx, e);
            }); 
            if(typeof window.drawStickman === 'function') window.drawStickman(ctx, window.p1); ctx.shadowBlur = 0;
            ...
            if (window.p1.comboHits >= 2) { 
                ...
                ctx.fillText(`🔥 ${window.p1.comboHits}`, 30 - window.camX, 100 + Math.sin(Date.now() / 100) * 5); 
                ctx.restore(); 
            }
        }

        window.slashes.forEach(s => { ctx.save(); ctx.translate(s.x, s.y); if (!s.isRight) ctx.scale(-1, 1); ctx.scale(s.scale, s.scale); ctx.rotate(s.rotation || 0); let prog = 1 - (s.life / s.maxLife); ctx.globalAlpha = Math.max(0, 1 - Math.pow(prog, 2)); ctx.beginPath(); ctx.arc(0, 0, 40 + prog * 20, -Math.PI/2 + prog*1.2, Math.PI/2 - prog*1.2); ctx.lineWidth = 15 * (1 - prog); let grad = ctx.createRadialGradient(0, 0, 10, 0, 0, 60); grad.addColorStop(0, "white"); grad.addColorStop(1, s.color); ctx.strokeStyle = grad; ctx.lineCap = "round"; ctx.shadowBlur = 15; ctx.shadowColor = s.color; ctx.stroke(); ctx.restore(); });
        window.particles.forEach(pt => { ctx.beginPath(); ctx.arc(pt.x, pt.y, pt.size, 0, Math.PI*2); ctx.fillStyle = pt.color; ctx.globalAlpha = Math.max(0, Math.min(1, pt.life / pt.maxLife)); ctx.fill(); if (pt.isCoin) { ctx.strokeStyle = "#d35400"; ctx.lineWidth = 1; ctx.stroke(); } }); ctx.globalAlpha = 1.0;
        window.floatingTexts.forEach(t => { ctx.font = t.font || "900 22px Arial"; ctx.fillStyle = t.color; ctx.shadowBlur = 5; ctx.shadowColor = t.color; ctx.globalAlpha = Math.max(0, Math.min(1, t.alpha)); ctx.fillText(t.text, t.x, t.y); ctx.shadowBlur = 0; }); ctx.globalAlpha = 1.0;

        if (window.screenFlash > 0) { ctx.fillStyle = `rgba(255, 255, 255, ${window.screenFlash})`; ctx.fillRect(0, 0, canvas.width, canvas.height); }
        if (window.cinematicTimer > 0 && window.cinematicCaster) {
            ctx.fillStyle = "rgba(0, 0, 0, 0.82)"; ctx.fillRect(0, 0, canvas.width, canvas.height); let stripY = canvas.height / 2 - 50; ctx.fillStyle = window.cinematicCaster.color; ctx.fillRect(0, stripY, canvas.width, 100);
            ...
            let avaX = canvas.width - slideX; let casterClone = Object.assign({}, window.cinematicCaster, {x: avaX, y: stripY + 70, state: 'cast', isFacingRight: true}); 
            if(casterClone.isDragon && typeof window.drawDragon === 'function') window.drawDragon(ctx, casterClone); else if(typeof window.drawStickman === 'function') window.drawStickman(ctx, casterClone);
        }
        ...
        if (window.introTimer > 0 && !window.gameOver && window.p1) {
            ...
            if (window.introTimer > 60) {
                ...
                let p1Clone = Object.assign({}, window.p1, {x: slideX1, y: canvas.height/2 + 30, state: 'idle', isFacingRight: true}); 
                if(typeof window.drawStickman === 'function') window.drawStickman(ctx, p1Clone);
                
                if (window.enemies && window.enemies.length > 0) {
                    let repEnemy = window.enemies[0]; let p2Clone = Object.assign({}, repEnemy, {x: slideX2, y: canvas.height/2 + 30, state: 'idle', isFacingRight: false}); 
                    if (repEnemy.isDragon && typeof window.drawDragon === 'function') window.drawDragon(ctx, p2Clone);
                    else if (typeof window.drawStickman === 'function') window.drawStickman(ctx, p2Clone);
                    if (window.introTimer < 130 && window.introTimer > 70) { ctx.fillStyle = "rgba(255, 255, 255, 0.9)"; ctx.fillRect(slideX2 - 20, canvas.height/2 - 140, 40, 30); ctx.fillStyle = "#111"; ctx.font = "bold 20px Arial"; ctx.fillText(repEnemy.taunt || "🤖", slideX2, canvas.height/2 - 118); }
                }
                ...
                if (window.introTimer < 130 && window.introTimer > 70) { ctx.fillStyle = "rgba(255, 255, 255, 0.9)"; ctx.fillRect(slideX1 - 20, canvas.height/2 - 140, 40, 30); ctx.fillStyle = "#111"; ctx.font = "bold 20px Arial"; ctx.fillText(window.p1.taunt || "🔥", slideX1, canvas.height/2 - 118); }
                if (window.introTimer <= 120) { ctx.font = "italic 900 80px Arial"; ctx.fillStyle = "#f1c40f"; ctx.shadowBlur = 25; ctx.shadowColor = "#f1c40f"; ctx.fillText("🆚", canvas.width/2, canvas.height/2 - 10); ctx.shadowBlur = 0; }
            } else { let scale = 1 + (window.introTimer / 60); ctx.save(); ctx.translate(canvas.width/2, canvas.height/2); ctx.scale(scale, scale); ctx.font = "italic 900 90px Arial"; ctx.fillStyle = "#ff9f43"; ctx.shadowBlur = 30; ctx.shadowColor = "#ff9f43"; ctx.fillText("🥊", 0, 30); ctx.restore(); }
        }
