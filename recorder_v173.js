// ==========================================
// RECORDER.JS - V76.6 (UPDATE: WEBM YOUTUBE FIX + 2.5X SPEED + BULLET-TIME DOUBLE TAP)
// ĐÃ FIX & GIỮ NGUYÊN: Intro God-Tier, Vỡ kính 3D, Kill-Cam, Facecam Hậu Kỳ Không Giật Lag.
// NÂNG CẤP ĐẶC BIỆT: 
// 1. Tốc độ kết xuất X2.5 siêu cuốn.
// 2. Nhắc nhở thả tim (Double Tap) chỉ hiện 1 lần kết hợp hiệu ứng làm chậm thời gian (Slow-mo/Bullet-time) 
//    rồi vọt lên tốc độ 2.5x để tạo sự bùng nổ.
// ==========================================
...
window.captureFrames = function() {
    ...
    if (renderNormalV) {
        ...
        // ==========================================
        // DOUBLE TAP REMINDER (HIỆN DUY NHẤT 1 LẦN)
        // ==========================================
        if (!isKillCamActive && window._gamePhaseStarted) {
            let timeInGame = Date.now() - window.gamePhaseStartTime;
            // Chỉ hiện 1 lần duy nhất từ giây thứ 2 đến giây thứ 4.5
            if (timeInGame >= 2000 && timeInGame <= 4500) {
                ctxV.save();
                ctxV.translate(540, splitGameHeight * 0.25); 
                
                let dtPulse = 1 + Math.sin(Date.now() / 150) * 0.08;
                ctxV.scale(dtPulse, dtPulse);
                
                let boxW = 320; let boxH = 64;
                ctxV.fillStyle = "rgba(0, 0, 0, 0.65)";
                ctxV.strokeStyle = "#ff0050";
                ctxV.lineWidth = 3;
                
                ctxV.beginPath();
                if(ctxV.roundRect) ctxV.roundRect(-boxW/2, -boxH/2, boxW, boxH, 32); 
                else ctxV.rect(-boxW/2, -boxH/2, boxW, boxH);
                ctxV.fill(); ctxV.stroke();

                ctxV.fillStyle = "#ffffff";
                ctxV.font = "900 26px 'Arial Black', sans-serif";
                ctxV.textAlign = "center";
                ctxV.textBaseline = "middle";
                ctxV.fillText("DOUBLE TAP TO LIKE", 25, 2);
                
                // Tay chỉ nhấp nháy
                let handPulse = 1 + Math.sin(Date.now() / 80) * 0.15;
                ctxV.translate(-115, 0);
                ctxV.scale(handPulse, handPulse);
                ctxV.font = "30px Arial";
                ctxV.fillText("👆", 0, 0);
...
// ========================================================
// HÀM GHÉP FACECAM VÀO VIDEO (ĐÃ FIX YOUTUBE UPLOAD & TÍCH HỢP DYNAMIC SPEED)
// ========================================================
window.mergeAndDownloadVideo = async function(vidId) {
    ...
    try {
        ...
        if(btn) { btn.innerText = "🚀 THỬ LẠI"; btn.disabled = false; btn.style.background = "linear-gradient(90deg, #ff0050, #00f2fe)"; }
        return;
    }

    if(btn) btn.innerText = "⏳ ĐANG RENDER HIỆU ỨNG TỐC ĐỘ...";
    ...
    // Tăng capture fps lên 60 để quay bắt kịp khoảnh khắc x2.5 mượt mà nhất
    renderCanvas.captureStream(60).getVideoTracks().forEach(t => combinedStream.addTrack(t));
    ...
    // BẮT BUỘC DÙNG WEBM ĐỂ FIX LỖI UPLOAD YOUTUBE
    ...
    recorder.onstop = () => {
        let finalBlob = new Blob(chunks, { type: recorder.mimeType || "video/webm" });
        let finalUrl = URL.createObjectURL(finalBlob);
        
        let a = document.createElement("a");
        a.href = finalUrl;
        a.download = "[FINAL_TIKTOK]_" + vid.safeFileName + ".webm";
        a.click();
        
        window.hideRenderToast();
        if(btn) { btn.innerText = "✅ TẢI XUỐNG HOÀN TẤT"; btn.style.background = "#1ed760"; btn.disabled = false; }
    };

    recorder.start();

    // Khởi tạo tốc độ ban đầu 2.5X
    rawVid.playbackRate = 2.5;
    faceVid.playbackRate = 2.5;
    ...
    function drawLoop() {
        ...
        if (!rawVid.paused) {
            ...
            // =====================================
            // ĐIỀU CHỈNH TỐC ĐỘ (DYNAMIC BULLET TIME)
            // =====================================
            if (isDuringGameplay) {
                let gameTimeSec = currentSec - vid.gameStartSec;
                // Khớp chính xác với thời gian từ giây 2 đến 4.5 của bảng Double Tap 
                // Ép làm chậm thành Slow-mo (0.7x) để người xem có thời gian thả tim
                if (gameTimeSec >= 2.0 && gameTimeSec <= 4.5) {
                    if (rawVid.playbackRate !== 0.7) {
                        rawVid.playbackRate = 0.7;
                        faceVid.playbackRate = 0.7;
                    }
                } else {
                    // Sau khi tắt bảng, bứt tốc lại lên 2.5X
                    if (rawVid.playbackRate !== 2.5) {
                        rawVid.playbackRate = 2.5;
                        faceVid.playbackRate = 2.5;
                    }
                }
