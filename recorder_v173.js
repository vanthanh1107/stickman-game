// ==========================================
// RECORDER.JS - V76.6 VIRAL STREAMER PRO (MASTERPIECE)
// Nâng cấp: Action Cam Zoom, Hype Train Chat, Fake Superchat, Audio Visualizer.
// Nền Cinematic Deep Space + Chat Kính mờ siêu sang trọng.
// ==========================================
...
const LORES = [ { chapter: "ACT I: THE BETRAYAL", p2Sub: "I taught you everything...", p1Sub: "Now I'll show you my wrath." }, { chapter: "FINAL CHAPTER: ENDGAME", p2Sub: "Know your place, kid.", p1Sub: "Your era ends tonight." } ];
const WAGER_BADGES = ["🏆 GLOBAL TOP 1% RANKED DUEL", "💰 $10,000 WAGER MATCH", "💀 PERMA-BAN DEATHMATCH"];
window.generateIntroParams = function() { const r = (arr) => arr[Math.floor(Math.random() * arr.length)]; return { themeP1: r(THEMES), themeP2: r(THEMES), lore: { ...r(LORES) }, badge: r(WAGER_BADGES) }; };

// ================= HỆ THỐNG CHAT & SUPERCHAT =================
window.CELEB_LIST = [
...
window._superChats = []; // Hệ thống Donate
...
window.spawnScrollingChat = function(ctxV, isHype = false) {
    ...
    chat.speed = isHype ? (5 + Math.random() * 2) : 3.5; // Chat Hype trôi nhanh hơn
    window._scrollingChats.push(chat);
};

window.spawnSuperChat = function(ctxV) {
    const amounts = ["$5.00", "$10.00", "$49.99", "$100.00", "5000 Bits"];
    const colors = [ ["#1e88e5", "#0d47a1"], ["#fbc02d", "#f57f17"], ["#e53935", "#b71c1c"] ];
    const msgs = ["Nice script bro 😂", "Teach me how to play!", "Pay to win spotted 💀", "My grandma plays better"];
    let r = (arr) => arr[Math.floor(Math.random() * arr.length)];
    
    let sc = {
        name: r(window.CELEB_LIST).name, amount: r(amounts), msg: r(msgs), colorTheme: r(colors),
        y: 1950, speed: 2.5 + Math.random(), 
        nameWidth: 0, msgWidth: 0, amountWidth: 0
    };
    ctxV.font = "bold 28px Arial"; sc.nameWidth = ctxV.measureText(sc.name).width; sc.msgWidth = ctxV.measureText(sc.msg).width; sc.amountWidth = ctxV.measureText(sc.amount).width;
    window._superChats.push(sc);
};
...
// ================= INTRO & OUTRO THUMBNAILS (GIỮ NGUYÊN) =================
window.bakeThumbnailsForVideo = function(titleText) {
    ...
    try {
        ...
        const drawCharSafe = (ctx, charObj, cx, cy, scale, isFacingRight) => { if(!charObj) return; ctx.save(); ctx.translate(cx, cy); if(!isFacingRight) ctx.scale(-1, 1); let clone = Object.assign({}, charObj, {x:0, y:0, scale: scale, isFacingRight: true, state: 'cast'}); if (typeof window.drawStickman === 'function') window.drawStickman(ctx, clone); ctx.restore(); };
...
window.drawProceduralIntro = function(ctx, w, h, progress) {
    ...
    if (progress < 0.25) { act = 0; localProg = progress / 0.25; } else if (progress < 0.55) { act = 1; localProg = (progress - 0.25) / 0.30; } else if (progress < 0.80) { act = 2; localProg = (progress - 0.55) / 0.25; } else { act = 3; localProg = (progress - 0.80) / 0.20; } 
    if (!window._sfxCuts[act]) { if (act === 0 && typeof window.playSound === 'function') window.playSound(100, 'sine', 0.5, 1.0); if (act === 1 && typeof window.playSound === 'function') window.playSound(300, 'square', 0.1, 0.5); if (act === 2 && typeof window.playSound === 'function') window.playSound(200, 'triangle', 0.2, 0.5); if (act === 3 && typeof window.playSound === 'function') window.playSound(120, 'sawtooth', 0.8, 1.5, true); window._sfxCuts[act] = true; }
    ...
    const drawFighterArtistic = (charObj, cx, cy, forcedState, isFacingRight, theme) => { if(!charObj) return; ctx.save(); ctx.translate(cx, cy); if(!isFacingRight) ctx.scale(-1, 1); let clone = Object.assign({}, charObj, {x:0, y:0, scale: 3.2, isFacingRight: true, state: forcedState}); ctx.filter = "contrast(115%) saturate(120%) drop-shadow(0 20px 20px rgba(0,0,0,0.8))"; ctx.save(); ctx.globalAlpha = 0.5 + Math.sin(Date.now()*0.02)*0.1; ctx.shadowBlur = 50; ctx.shadowColor = theme.aura; ctx.scale(1.15, 1.15); if (typeof window.drawStickman === 'function') window.drawStickman(ctx, clone); ctx.restore(); if (typeof window.drawStickman === 'function') window.drawStickman(ctx, clone); ctx.restore(); };
    ctx.save(); let cutShake = 0; let panX = 0; let currentZoom = 2.0; 
    if (act === 0) { currentZoom = 2.5 - localProg * 0.3; panX = 40; cutShake = (Math.random()-0.5)*10; } else if (act === 1) { currentZoom = 2.5; panX = 60; cutShake = (Math.random()-0.5)*5; } else if (act === 2) { currentZoom = 2.2; panX = -60; cutShake = (Math.random()-0.5)*5; } else if (act === 3) { currentZoom = 1.2 + localProg * 0.5; cutShake = (Math.random() - 0.5) * 50; }
    ...
    if (act === 0) { drawFighterArtistic(window.p1, 0, 100, 'idle', true, prm.themeP1); } else if (act === 1) { drawFighterArtistic(window.p1, 0, 100, 'cast', true, prm.themeP1); } else if (act === 2) { drawFighterArtistic(e1, 0, 100, 'cast', false, prm.themeP2); } else if (act === 3) {
        let easeIn = Math.pow(localProg, 3); let pushP1 = -500 + (easeIn * 400); let pushP2 = 500 - (easeIn * 400);
        drawFighterArtistic(e1, pushP2, 100, 'attack', false, prm.themeP2); drawFighterArtistic(window.p1, pushP1, 100, 'attack', true, prm.themeP1);
        ...
        if (localProg > 0.8) { let shatterProg = (localProg - 0.8) * 5; ctx.save(); ctx.translate(0, -100); ctx.strokeStyle = "#00f3ff"; ctx.lineWidth = 8; ctx.shadowColor = "#00f3ff"; ctx.shadowBlur = 20; for(let i=0; i<12; i++) { let ang = (Math.PI * 2 / 12) * i; let crackLen = Math.min(w, h) * shatterProg * 1.5; ctx.beginPath(); ctx.moveTo(0, 0); ctx.lineTo(Math.cos(ang)*crackLen, Math.sin(ang)*crackLen); ctx.stroke(); }
            if (shatterProg > 0.5) { let whiteFlash = (shatterProg - 0.5) * 2; ctx.fillStyle = `rgba(255, 255, 255, ${whiteFlash})`; ctx.fillRect(-w*2, -h*2, w*4, h*4); } ctx.restore(); }
    }
...
window.startRecording = function() {
    if (window.isRecording) { window.stopRecording(); } window.initRecorder(); if (window.audioCtx.state === 'suspended') window.audioCtx.resume();
    
    window.introParams = window.generateIntroParams(); window.recordedChunksV = []; window._scrollingChats = []; window._superChats = [];
    ...
    if (MediaRecorder.isTypeSupported('video/mp4; codecs="avc1,mp4a.40.2"')) { options.mimeType = 'video/mp4; codecs="avc1,mp4a.40.2"'; } else { options.mimeType = 'video/webm; codecs="vp8"'; window.currentVideoExt = "webm"; }
    ...
    window.StoryModeAI.init(); window.bakeThumbnailsForVideo(window.StoryModeAI.viralTitle);
...
// ================= THE MAIN RENDER LOOP (MASTERPIECE) =================
window.captureFrames = function() {
    ...
    if (elapsed < 150) { if (window.bakedThumbV) ctxV.drawImage(window.bakedThumbV, 0, 0, 1080, 1920); renderNormalV = false; 
    } else if (elapsed < window.introDuration) { let introProgress = (elapsed - 150) / (window.introDuration - 150); window.drawProceduralIntro(ctxV, 1080, 1920, introProgress); renderNormalV = false; }

    // PHÂN TÍCH ÂM THANH (Bass Peak)
    ...
    if (shouldGlitch) window._impactZoom = 1.15; // Giật Bass -> Phóng to cam
    window._impactZoom += (1.0 - window._impactZoom) * 0.15; // Zoom out mượt mà về 1.0
    ...
    if (renderNormalV) {
        ...
        // --- LỚP 3: HYPE TRAIN CHAT & SUPERCHAT ---
        // Sinh Superchat ngẫu nhiên
        if (Math.random() < 0.002) window.spawnSuperChat(ctxV);
        
        let chatNow = Date.now();
        // HYPE TRAIN: Bơm lượng lớn chat "W", "OMG" nếu có đòn đánh to
        ...
        for (let i = window._scrollingChats.length - 1; i >= 0; i--) { 
            ...
            ctxV.fillStyle = c.isHype ? "rgba(220, 38, 38, 0.4)" : "rgba(15, 23, 42, 0.65)"; // Đỏ mờ nếu là Hype
        ...
        // Vẽ SuperChats (Donate)
        for (let i = window._superChats.length - 1; i >= 0; i--) { 
            let sc = window._superChats[i]; sc.y -= sc.speed; 
            if (sc.y < -200) { window._superChats.splice(i, 1); continue; }
            let alpha = 1; if (sc.y > 1750) alpha = Math.max(0, (1920 - sc.y) / 170); if (sc.y < gameY) alpha = Math.max(0, (sc.y + 100) / (gameY + 100)); 
            ctxV.save(); ctxV.globalAlpha = alpha; ctxV.font = "bold 26px Arial";
            let totalWidth = sc.nameWidth + sc.amountWidth + sc.msgWidth + 100; let chatX = 40; 
            
            let scGrad = ctxV.createLinearGradient(chatX, 0, chatX + totalWidth, 0);
            scGrad.addColorStop(0, sc.colorTheme[0]); scGrad.addColorStop(1, sc.colorTheme[1]);
            ctxV.fillStyle = scGrad; ctxV.shadowColor = sc.colorTheme[0]; ctxV.shadowBlur = 15;
            if(ctxV.roundRect) { ctxV.beginPath(); ctxV.roundRect(chatX, sc.y - 40, totalWidth, 80, 20); ctxV.fill(); } else { ctxV.fillRect(chatX, sc.y - 40, totalWidth, 80); }
            
            ctxV.shadowBlur = 0; ctxV.fillStyle = "rgba(0,0,0,0.3)";
            if(ctxV.roundRect) { ctxV.beginPath(); ctxV.roundRect(chatX, sc.y - 40, totalWidth, 40, {tl:20, tr:20, bl:0, br:0}); ctxV.fill(); }
            
            ctxV.fillStyle = "rgba(255,255,255,0.8)"; ctxV.fillText(sc.name, chatX + 20, sc.y - 20);
            ctxV.fillStyle = "#fff"; ctxV.font = "900 28px Arial"; ctxV.fillText(sc.amount, chatX + 30 + sc.nameWidth, sc.y - 20);
            ctxV.font = "bold 26px Arial"; ctxV.fillText(sc.msg, chatX + 20, sc.y + 20);
            ctxV.restore();
        }

        // --- LỚP 4: AUDIO VISUALIZER (SÓNG NHẠC STREAMER) ---
        ctxV.save();
        let visY = gameY + splitGameHeight + 20; // Ngay dưới khung game
        let barWidth = 10; let spacing = 6; let numBars = 60;
        let startX = (1080 - (numBars * (barWidth + spacing))) / 2;
        for(let i=0; i<numBars; i++) {
            let val = window.analyserData[i % 30] / 255.0; // Lấy dữ liệu phổ âm (phản chiếu 2 bên cho đẹp)
            let h = Math.max(5, val * 60); 
            ctxV.fillStyle = `rgba(0, 243, 255, ${0.4 + val})`;
            if(ctxV.roundRect) { ctxV.beginPath(); ctxV.roundRect(startX + i*(barWidth+spacing), visY, barWidth, h, 5); ctxV.fill(); }
        }
        ctxV.restore();

        // --- LỚP 5: MÀN HÌNH GAME (CÓ ACTION CAM ZOOM) ---
        ctxV.save();
        // Áp dụng Impact Zoom (Camera dội theo nhịp đòn đánh) vào chính giữa khung Game
        ...
        // Hiệu ứng Glitch (ách mẻ màn hình khi đánh mạnh)
        if (shouldGlitch) {
        ...
        ctxV.restore(); // Phục hồi context sau khi Zoom Game

        // --- LỚP 6: KINETIC SUBTITLES ---
        if (!isOutroActive) {
        ...
        // --- LỚP 7: OUTRO ANIME SLASH ---
        if (isOutroActive) {
            let outroAlpha = Math.min(1, (window.matchEndTimer - 90) / 80); ctxV.save(); ctxV.globalAlpha = outroAlpha;
            ...
            ctxV.lineWidth = 5; ctxV.strokeStyle = "rgba(255, 255, 255, 0.5)"; ctxV.stroke(); ctxV.fillStyle = "#ffffff"; ctxV.font = `900 60px 'Arial Black', sans-serif`; ctxV.fillText("✨ CREATE YOUR OWN", 0, 7); ctxV.restore();
            ...
            if (window.matchEndTimer > 230 && window.bakedThumbV) {
                ...
                if (loopProg > 0 && loopProg < 0.95) {
                    ...
                    ctxV.beginPath(); ctxV.moveTo(-500, 1920 - slashWidth); ctxV.lineTo(1580, 0 - slashWidth); ctxV.stroke(); ctxV.beginPath(); ctxV.moveTo(-500, 1920 + slashWidth); ctxV.lineTo(1580, 0 + slashWidth); ctxV.stroke(); 
...
window.downloadRawVideo = function(vidId) {
    ...
    let a = document.createElement("a"); a.href = vid.urlV; a.download = "[PRO_TIKTOK]_" + vid.viralTitle.replace(/[^a-z0-9]/gi, '_') + "." + vid.ext; a.click();
};
