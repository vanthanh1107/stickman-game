// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 25.0 [THE E-SPORTS SOUL]
// [NEW] ESPORTS FOOTSIES: Nhân vật liên tục nhấp nhả di chuyển mô phỏng người thật chơi.
// [NEW] RTX FLOOR REFLECTION: Sàn thi đấu phản chiếu quang học 3D siêu thực.
// [NEW] PHANTOM ECHO TRAILS: Bóng ma tốc độ cao sáng rực rỡ khi lướt.
// [NEW] DIRECTIONAL KINETIC SPARKS: Tia lửa văng theo quỹ đạo lực đấm chuẩn vật lý.
// ==========================================
...
// ==========================================
// 1. HỆ THỐNG ÂM THANH & COLOR GRADING ĐIỆN ẢNH
// ==========================================
...
// ==========================================
// 2. HỆ THỐNG SPAWN VFX VẬT LÝ CAO CẤP
// ==========================================
...
window.spawnSlash = function(x, y, isRight, color, isCrit, scale, rotation = 0) { window.slashes.push({ x: x, y: y, isRight: isRight, life: 12, maxLife: 12, color: color, scale: (isCrit ? 2.2 : 1.4) * scale, rotation: rotation }); }
...
window.spawnMangaSFX = function(x, y, isCrit) {
    ...
    let size = isCrit ? (45 + Math.random()*20) : (25 + Math.random()*10);
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce, attackerDirRight = true) {
    ...
    if (target.state === 'block') { 
        if (target.stamina < 15) {
            ...
            target.stamina = Math.min(100, target.stamina + 30); window.shakeScreen(15, 10); window.hitStopFrames = 15; window.chromaTimer = 10;
    ...
    if (finalDmg > 0) {
        ...
        // [VIRAL 25.0] DIRECTIONAL KINETIC SPARKS
        let sparkCount = isCrit ? 15 : 6;
        let baseDirX = attackerDirRight ? 1 : -1;
        for(let i=0; i<sparkCount; i++) { 
            let vx = baseDirX * (Math.random() * 20 + 5) + (Math.random()-0.5)*5; // Văng hình nón theo hướng đấm
        ...
        if (isCrit) {
            window.impactFrameCount = 3; window.targetZoom = 1.3; 
            window.targetCinemaBars = 60; 
            setTimeout(() => { if(!window.gameOver) window.targetCinemaBars = 0; }, 600); 
        ...
        if (target.hp <= 0) {
            ...
            if (isNearWall && isCrit && typeof window.triggerStageTransition === 'function') { window.triggerStageTransition(target); } 
                ...
                if (isCrit) {
                    window.invertFrames = 4; window.applyFilter('grayscale', 90); window.speedLinesAlpha = 1.0; window.spawnLensFlare(target.x, target.y - 40, "#ff003c", 5.0);
                ...
                } else { window.hitStopFrames = 15; window.shakeScreen(30, 25); window.targetZoom = 1.5; window.actionCamOffsetY = -40; window.targetTimeScale = 0.2; setTimeout(()=>{window.targetTimeScale = 1.0; }, 1000); }
                
                window.screenFlash = 0; window.impactFrameTimer = 0; window.chromaTimer = 40;
...
window.attack = function(attacker, targetGroup) {
    ...
    if (['uppercut', 'dragon_uppercut', 'high_kick'].includes(selectedMove)) {
        ...
        window.actionCamOffsetY = 80; 
    } else if (['axe_kick', 'elbow_strike'].includes(selectedMove)) {
        slashAngle = Math.PI / 5;
        window.actionCamOffsetY = -40; 
    ...
    if (isFinisher) {
        ...
        window.actionCamOffsetX = attacker.isFacingRight ? 100 : -100; 
    } else {
    ...
    window.spawnSlash(target.x, target.y - 35, attacker.isFacingRight, isCrit ? "#ff4757" : "#ecf0f1", isCrit, isFinisher ? 2.0 : 1.4, slashAngle);
};
...
window.update = function() {
    ...
    allFighters.forEach(f => {
        ...
        // [VIRAL 25.0] E-SPORTS FOOTSIES (Micro-movements giả lập tay cầm người chơi)
        ...
        // [VIRAL 25.0] Trượt dài hơn một chút khi lướt tạo cảm giác có ma sát thật
    ...
    allFighters.forEach(f => {
        ...
        // [VIRAL 25.0] GHI NHẬN LỊCH SỬ BÓNG MỜ ĐỂ VẼ HIỆU ỨNG PHANTOM ECHO
    ...
    if (window.p1 && window.introTimer === 0) {
        ...
        if (window.gameOver && closest) {
            ...
            window.targetCamX = Math.max(-maxPanX, Math.min(maxPanX, desiredCamX)) + window.actionCamOffsetX; 
...
window.draw = function() {
    ...
    if (window.isLoading) {
        ...
        window.ctx.fillStyle = "#00f3ff"; window.ctx.font = "italic 900 40px Arial"; window.ctx.textAlign = "center"; window.ctx.shadowBlur = 15; window.ctx.shadowColor = "#00f3ff"; window.ctx.fillText("ESPORTS ENGINE INITIALIZING...", cx, cy - 50);
        ...
        window.ctx.fillStyle = "#fff"; window.ctx.font = "bold 16px monospace"; window.ctx.fillText(`RENDERING HUMANIZED PHYSICS... ${Math.floor(window.loadProgress)}%`, cx, cy + 60);
        return; 
    }
    ...
    try {
        ...
        // [VIRAL 25.0] RTX FLOOR REFLECTION (PHẢN CHIẾU MẶT SÀN)
        // Tạo một gradient bán trong suốt để làm mặt kính
        window.ctx.save();
        let reflectionGrad = window.ctx.createLinearGradient(0, window.GROUND_Y, 0, window.GROUND_Y + 300);
        reflectionGrad.addColorStop(0, "rgba(255, 255, 255, 0.2)"); // Sáng bóng ở ngay chân
        reflectionGrad.addColorStop(1, "rgba(0, 0, 0, 0.8)"); // Đen dần xuống dưới
        
        window.ctx.globalAlpha = 0.35; // Độ trong suốt của hình phản chiếu
        window.ctx.translate(0, window.GROUND_Y); 
        window.ctx.scale(1, -0.6); // Lật ngược xuống dưới và ép dẹt nhẹ (Phối cảnh)
        window.ctx.translate(0, -window.GROUND_Y);
        
        let allFighters = [window.p1].concat(window.enemies); 
        
        // Vẽ nhân vật phản chiếu
        allFighters.forEach(p => { 
            if (p && p.hp > 0 && typeof window.drawStickman === 'function') { 
                window.ctx.save(); window.ctx.translate(p.x, p.y); if (!p.isFacingRight) window.ctx.scale(-1, 1); 
                let clone = Object.assign({}, p, { x: 0, y: 0 }); 
        ...
        // Phủ lớp gradient làm bóng kính
        window.ctx.restore();
        window.ctx.save();
        ...
        window.ctx.restore();
        // KẾT THÚC RTX REFLECTION

        if (window.globalIllumination) {
            window.ctx.save();
        ...
        // [VIRAL 25.0] PHANTOM ECHO TRAILS (HÒA TRỘN LIGHTER SIÊU ĐẸP)
        if (window.p1) {
            window.ctx.globalCompositeOperation = 'lighter'; // Giúp tia sáng rực hơn, không bị bết màu
            allFighters.forEach(p => { 
                if (p && p.hp > 0 && p.trailArr) { 
                    p.trailArr.forEach(t => { 
                        ...
                        // Scale nhỏ dần tàn ảnh để tạo cảm giác hút về tâm
                        let scaleDown = 1 - (t.timer * 0.02);
                        if (scaleDown < 0.1) scaleDown = 0.1;
                        
                        // Ám màu Neon cho tàn ảnh (Tùy nhân vật)
        ...
        window.particles.forEach(pt => { 
            ...
            if (pt.isGroundDust) {
                window.ctx.beginPath(); window.ctx.ellipse(pt.x, window.GROUND_Y - pt.size/2, pt.size * 1.5, pt.size * 0.4, 0, 0, Math.PI*2); window.ctx.fill();
            }
        ...
        // [VIRAL 25.0] SPARKS PHÁT SÁNG THEO VECTOR
        window.ctx.globalCompositeOperation = 'lighter';
...
// ==========================================
// 7. [VIRAL 25.0] AFTER EFFECTS TIME-RAMPING & UNLOCKED 120Hz LOOP
// ==========================================
