// ==========================================
// RECORDER.JS - V76.5 ULTIMATE LAYERED EDITION
// - Giữ NGUYÊN 100% Intro God-Tier, Outro Anime Slash, Kinetic Subtitles (Trash Talk)
// - Background Lưới Neon 3D + Particles cực đẹp.
// - Game đặt chính giữa, Chat chạy trôi từ dưới lên chui qua sau lưng game!
// ==========================================
...
function makeTypo(str) { if(Math.random() > 0.5) return str; let arr = str.split(''); let idx = Math.floor(Math.random() * (arr.length - 2)) + 1; if (arr[idx] !== ' ' && arr[idx+1] !== ' ') { let temp = arr[idx]; arr[idx] = arr[idx+1]; arr[idx+1] = temp; } return arr.join(''); }
...
window._lastChatUpdate = 0; window._nextChatDelay = 500;
...
window.generateLiveChatEvent = function() {
    ...
    return { name: celeb.name, color: celeb.color, msg: msg, x: 0, y: 0, speed: 0, nameWidth: 0, msgWidth: 0, lines: null };
};

window.spawnScrollingChat = function(ctxV) {
    let chat = window.generateLiveChatEvent();
    ctxV.font = "bold 30px Arial";
    chat.nameWidth = ctxV.measureText(chat.name + ": ").width;
    chat.msgWidth = ctxV.measureText(chat.msg).width;
    chat.x = 40 + Math.random() * (1080 - (chat.nameWidth + chat.msgWidth) - 120);
    chat.y = 1950; 
    chat.speed = 3.5 + Math.random() * 4; 
...
window.StoryModeAI = {
    ...
    generateViralStickmanTitle: function() {
        ...
        const acts = ["PSYCHOLOGICAL WARFARE", "LOBOTOMY COMBO", "SCHIZO MOVEMENT"];
...
function drawBaitSạn(ctx, w, h, floatY) {
    ...
    if (window._baitType === 0) {
        ...
        ctx.translate(w/2, slideY + floatY); ctx.fillStyle = "rgba(255, 255, 255, 0.98)";
...
window.drawProceduralIntro = function(ctx, w, h, progress) {
    ...
    if (act === 0) {
        drawFighterArtistic(window.p1, 0, 100, 'idle', true, prm.themeP1, realP1Name); ctx.restore(); ctx.save();
        if(!window._introEmojis || window._introEmojis.length === 0) { window._introEmojis = []; const emos = ["💀", "🤡", "🐔", "❓", "📉", "😂", "🔥", "🥶", "🤯"]; for(let c=0; c<25; c++) { window._introEmojis.push({ e: emos[Math.floor(Math.random()*emos.length)], x: (Math.random()-0.5)*900, y: (Math.random()-0.5)*900, s: 0.1, maxS: 1 + Math.random()*2.5, rot: (Math.random()-0.5)*1 }); } }
        for(let e of window._introEmojis) { if (e.s < e.maxS) e.s += 0.2; e.y -= 4; ctx.save(); ctx.translate(w/2 + e.x, h/2 + e.y); ctx.rotate(e.rot); ctx.scale(e.s, e.s); ctx.font = "60px Arial"; ctx.textAlign = "center"; ctx.textBaseline = "middle"; ctx.fillText(e.e, 0, 0); ctx.restore(); }
        ...
        ctx.fillStyle = "rgba(10, 15, 30, 0.85)"; ctx.strokeStyle = "rgba(0, 243, 255, 0.8)"; ctx.lineWidth = 3; ctx.beginPath(); if(ctx.roundRect) ctx.roundRect(-chatW/2, -chatH/2, chatW, chatH, 20); else ctx.fillRect(-chatW/2, -chatH/2, chatW, chatH); ctx.fill(); ctx.stroke();
        ...
        for(let i=0; i<numChatsToShow && i < window._introBottomChats.length; i++) { let c = window._introBottomChats[i]; ctx.fillStyle = c.color; ctx.fillText(c.name + ":", -chatW/2 + 30, cY); let nW = ctx.measureText(c.name + ":").width; ctx.fillStyle = "#fff"; let text = c.msg.length > 35 ? c.msg.substring(0,35)+"..." : c.msg; ctx.fillText(" " + text, -chatW/2 + 30 + nW, cY); cY += 55; }
        ctx.restore(); ctx.save(); ctx.translate(w/2 + cutShake + panX, h/2 + cutShake); ctx.scale(currentZoom, currentZoom); ctx.translate(0, 100); 
    }
    else if (act === 2) {
...
window.startRecording = function() {
    ...
    window._baitTriggered = false; window._baitType = -1; window._scrollingChats = [];
...
window.captureFrames = function() {
    ...
    if (renderNormalV && elapsed > 4000 && !window._baitTriggered && Math.random() < 0.008) { window._baitTriggered = true; window._baitType = Math.floor(Math.random() * 3); window._baitStartTime = Date.now(); }
    ...
    if (renderNormalV) {
        ...
        // ================= LỚP 1: BACKGROUND CỰC ĐẸP =================
        let bgGrad = ctxV.createLinearGradient(0, 0, 0, 1920);
        bgGrad.addColorStop(0, "#0b001a"); bgGrad.addColorStop(1, "#3c003c");
        ctxV.fillStyle = bgGrad; ctxV.fillRect(0, 0, 1080, 1920); 

        // Nền Grid Neon chéo
        ctxV.strokeStyle = "rgba(0, 255, 200, 0.15)"; ctxV.lineWidth = 2; 
        if(!window._gridVerticalPath) { window._gridVerticalPath = new Path2D(); for(let x = -20; x <= 20; x+=2) { window._gridVerticalPath.moveTo(540, 0); window._gridVerticalPath.lineTo(540 + x * 200, 1920); } }
        ctxV.stroke(window._gridVerticalPath); ctxV.beginPath(); 
        let zSpeed = (Date.now() / 15) % 40; for(let y = 1; y < 40; y++) { let actualY = Math.pow(y, 1.8) * 2.5 + zSpeed; if (actualY <= 1920) { ctxV.moveTo(0, actualY | 0); ctxV.lineTo(1080, actualY | 0); } }
        ctxV.stroke(); 

        // Particles sáng lấp lánh (Hạt bay từ dưới lên)
        if (Math.random() < 0.3) { let p = window.retentionParticles.find(p => !p.active); if (p) { p.active = true; p.x = Math.random() * 1080; p.y = 1950; p.s = Math.random() * 6 + 3; p.v = Math.random() * 8 + 4; p.age = 0; } }
        ctxV.fillStyle = "rgba(0, 255, 200, 0.8)"; ctxV.beginPath(); 
        for (let i = 0; i < window.retentionParticles.length; i++) { let p = window.retentionParticles[i]; if (!p.active) continue; p.age++; p.y -= p.v; p.x += Math.sin(p.age * 0.1) * 3; ctxV.rect(p.x | 0, p.y | 0, p.s | 0, p.s | 0); if (p.y < -50) p.active = false; }
        ctxV.fill(); 

        // Emojis 3D bay lên (Nổi lửa, đầu lâu)
        if (audioPeak > 0.35 && Math.random() < 0.25) { let re = window.retentionEmojis.find(e => !e.active); if (re) { const emos = ["🔥", "💀", "🤯", "🥶", "💯", "📈"]; re.active = true; re.x = 100 + Math.random() * 880; re.y = 1950; re.v = 10 + Math.random() * 8; re.e = emos[(Math.random() * emos.length) | 0]; re.r = (Math.random() - 0.5) * 0.5; re.age = 0; } }
        for (let i = 0; i < window.retentionEmojis.length; i++) {
            ...
            ctxV.font = "90px Arial"; ctxV.globalAlpha = Math.max(0, Math.min(1, (re.y + 100) / 400)); ctxV.textAlign = "center"; ctxV.textBaseline = "middle"; ctxV.fillText(re.e, 0, 0); ctxV.restore();
            if (re.y < -100) re.active = false;
        }

        // ================= LỚP 2: SCROLLING CHAT (BAY TỪ DƯỚI LÊN ĐẰNG SAU GAME) =================
        let chatNow = Date.now();
        if (chatNow - window._lastChatUpdate > window._nextChatDelay) { window._lastChatUpdate = chatNow; window._nextChatDelay = 400 + Math.random() * 800; window.spawnScrollingChat(ctxV); }
        ctxV.font = "bold 30px Arial"; ctxV.textAlign = "left"; ctxV.textBaseline = "middle";
        for (let i = window._scrollingChats.length - 1; i >= 0; i--) { 
            ...
            if(ctxV.roundRect) { ctxV.beginPath(); ctxV.roundRect(c.x - 20, c.y - 30, totalWidth, 60, 30); ctxV.fill(); } else { ctxV.fillRect(c.x - 20, c.y - 30, totalWidth, 60); }
            ctxV.fillStyle = c.color; ctxV.fillText(c.name + ":", c.x, c.y); 
            ctxV.fillStyle = "#ffffff"; ctxV.fillText(" " + c.msg, c.x + c.nameWidth, c.y); 
            if (c.y < -100) window._scrollingChats.splice(i, 1);
        }

        // ================= LỚP 3: MÀN HÌNH GAME (CÓ VIỀN NEON SÁNG) =================
        ctxV.shadowColor = "#00f3ff"; ctxV.shadowBlur = 35;
        ctxV.fillStyle = "#000"; ctxV.fillRect(shakeX | 0, (gameY + shakeY) | 0, 1080, splitGameHeight);
        ctxV.shadowBlur = 0;
        ...
        // ================= POP-UP TROLL (LỚP TRÊN CÙNG TRƯỚC HYPE TEXT) =================
        drawBaitSạn(ctxV, 1080, splitGameHeight, gameY);

        // ================= LỚP 4: KINETIC SUBTITLES (CHỮ NẢY MRBEAST) =================
        if (!isOutroActive) {
        ...
        // ================= LỚP 5: OUTRO ANIME SLASH (NHƯ V76.4 CŨ) =================
        if (isOutroActive) {
...
window.downloadRawVideo = function(vidId) {
    ...
    let a = document.createElement("a"); a.href = vid.urlV; a.download = "[ULTIMATE_TIKTOK]_" + vid.viralTitle.replace(/[^a-z0-9]/gi, '_') + "." + vid.ext; a.click();
};
