// ==========================================
// RECORDER.JS - V78 THE ULTIMATE ALGORITHM BREAKER (YOUTUBE SHORTS EDITION)
// ĐÃ FIX & GIỮ NGUYÊN: Intro God-Tier, Vỡ kính 3D, Kill-Cam, Facecam Hậu Kỳ Không Giật Lag.
// TÍCH HỢP MỚI: Tự động Upload Nhị Phân (Binary) lên YouTube Shorts qua Google Apps Script (Chống lỗi Connection Closed & Quá dài).
// ==========================================
...
window.sendVideoToWebhook = async function(videoBlob, title, description) {
    window.showRenderToast("☁️ ĐANG GỬI LÊN MÁY CHỦ YOUTUBE SHORTS...");

    try {
        // Đưa tiêu đề và mô tả lên thẳng đường link (Query Params) để chừa chỗ body cho file Video
        let requestUrl = window.WEBHOOK_URL + 
                         "?title=" + encodeURIComponent(title) + 
                         "&desc=" + encodeURIComponent(description);

        // Gửi body là nguyên cục videoBlob nhị phân (Siêu nhẹ, Chống lỗi ERR_CONNECTION_CLOSED)
        ...
        if (result.status === "success") {
            window.showRenderToast(`✅ TẢI LÊN YOUTUBE SHORTS THÀNH CÔNG!`);
...
window.videoTrackV = null; window.isRecording = false; window.currentVideoExt = "webm"; window.savedVideos = [];
...
window.startRecording = function() {
    ...
    // FIX LỖI THỜI LƯỢNG YOUTUBE: Dùng WebM chuẩn thay vì MP4 lỗi
    ...
    try { 
        ...
        window.mediaRecorderV = new MediaRecorder(combinedStreamV, { mimeType: 'video/webm' }); 
    }
    ...
    if (renderNormalV) {
        ...
        if (!isOutroActive) {
            ...
            if (window.matchEndTimer > 230 && window.bakedThumbV) {
                ...
                // 1. TẠO MẶT NẠ (MASK) VẾT CHÉM M mở RỘNG DẦN
    ...
    recorder.onstop = async () => {
        ...
        if (ytVideoId) {
            if(btn) { 
                ...
                btn.onclick = () => window.open(`https://youtube.com/shorts/${ytVideoId}`, '_blank');
                btn.disabled = false; 
            }
        } else {
