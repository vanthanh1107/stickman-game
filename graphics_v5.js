...
window.blendSkeleton = function(p, targetPts, speed = 0.35) {
    ...
    for (let key in targetPts) {
        // Bọc an toàn: Tránh lỗi NaN phá hủy khung xương
...
window.drawStickman = function(ctx, p, isTrail = false) {
    ...
    if (!customDrawSuccess) {
        ...
        // [FIX 1 ÁP DỤNG]: Bỏ qua Lerp hoàn toàn nếu đang vẽ cái bóng (Trail)
        ...
        if (!isTrail) {
            if (!p.scarfNodes) {
                // [FIX 2 ÁP DỤNG]: Lấy tọa độ cổ (neck) hiện tại làm điểm sinh ra chiếc khăn thay vì tọa độ ảo (0, -45)
            ...
            for (let i = 1; i < p.scarfNodes.length; i++) {
                ...
                if (dist < 0.001) dist = 0.001; // Chống lỗi chia cho 0 sinh ra NaN
