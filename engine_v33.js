// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 27.1 [KINETIC TYPOGRAPHY INTEGRATED]
// [BASE] Giữ nguyên 100% độ ổn định và các tính năng AAA của V27.0
// [NEW] KINETIC TYPOGRAPHY: Tích hợp sát thương bật nảy lò xo 3D (Spring Physics).
// [NEW] COMIC OUTLINES: Chữ bay lên có viền đen dày, nghiêng dồn dập chuẩn eSports.
// ==========================================
...
window.foregroundDebris = []; // Mảnh vỡ văng vào ống kính (Depth of Field 3D)
window.lethalVoid = 0; // Hư không khi kết liễu
window.impactAberration = 0; // Tách màu vật lý
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce, attackerDirRight = true) {
    ...
    if (target.state === 'dash' || target.state === 'dash_back') {
        if (target.dashTimer > 2 && target.dashTimer < 14) {
            ...
            // [KINETIC TYPOGRAPHY INTEGRATED] EVADE
    ...
    if (target.state === 'block') { 
        if (target.stamina < 15) {
            ...
            // [KINETIC TYPOGRAPHY INTEGRATED] GUARD CRUSH
            ...
            // [KINETIC TYPOGRAPHY INTEGRATED] PARRY
    ...
    if (finalDmg > 0) {
        ...
        // [KINETIC TYPOGRAPHY INTEGRATED] DAMAGE
        ...
        window.floatingTexts.push({ 
            x: target.x, y: target.y - 60, text: dmgText, color: color || (isCrit ? "#ff4757" : "#fff"), 
            alpha: 1.5, vx: textVx, vy: textVy, font: isCrit ? "italic 900 40px 'Arial Black', Impact" : "italic 900 30px 'Arial Black', Impact", 
        ...
        if (target.hp <= 0) {
            ...
            if (isNearWall && isCrit && typeof window.triggerStageTransition === 'function') { window.triggerStageTransition(target); } 
                ...
                if (isCrit) {
                    ...
                    // [KINETIC TYPOGRAPHY INTEGRATED] LETHAL
...
window.attack = function(attacker, targetGroup) {
    ...
    if (isFinisher) {
        ...
        // [KINETIC TYPOGRAPHY INTEGRATED] FINISHER K.O. EXPLOSION
        ...
        if (Math.random() < attacker.critChance) { isCrit = true; finalDmg = baseDmg * attacker.critMult; window.floatingTexts.push({ x: target.x, y: target.y - 60, text: "💢", color: "#f1c40f", alpha: 1.5, vx: 0, vy: -4, font: "italic 900 35px 'Arial Black', Impact", life: 30, scale: 2.0, targetScale: 1.0, scaleVel: 0, rot: 0 }); } else { window.playSound(350, 'sine', 0.1, 0.1, false); }
...
window.update = function() {
        ...
        window.enemies.forEach(e => { 
            if (e.hp <= 0 && !e.deathTriggered) {
                ...
                if (window.p1 && window.p1.hp > 0) { let heal = Math.floor(window.p1.maxHp * 0.08); window.p1.hp = Math.min(window.p1.maxHp, window.p1.hp + heal); 
                    // [KINETIC TYPOGRAPHY INTEGRATED] HEAL
    ...
    allFighters.forEach(f => {
            ...
            if (f.hitStun <= 0 && f.stunTimer <= 0 && f.state !== 'dash_back' && f.state !== 'block') {
                ...
                if (isCloseEnough) {
                    if (window.classStats && window.classStats[type] && typeof window.classStats[type].executeUltimate === 'function') {
                        f.stamina = 0; let baseDmg = 50 * (f.currentDmgMod || 1); if (!f.isPlayer) baseDmg = 35 * (f.currentDmgMod || 1); window.playSound(400, 'sine', 0.5, 0.6); window.shakeScreen(15, 10); window.spawnParticles(f.x, f.y, "#f1c40f", true); 
                        // [KINETIC TYPOGRAPHY INTEGRATED] ULTIMATE
                        window.floatingTexts.push({ x: f.x, y: f.y - 100, text: f.isPlayer ? "🔥 ULTIMATE!" : "⚠️ DANGER!", color: f.isPlayer ? "#ff4757" : "#ff0000", alpha: 1.5, vx: 0, vy: -6, font: "italic 900 45px 'Arial Black', Impact", life: 50, scale: 3.0, targetScale: 1.0, scaleVel: 0, rot: 0 }); 
                        f.isFacingRight = distToTarget > 0; window.classStats[type].executeUltimate(f, closestTarget, baseDmg); f.vx = 0; window.speedLinesAlpha = 1.0; 
        ...
        if (!launchedUltimate && f.attackTimer <= 0 && f.hitStun <= 0 && f.dashTimer <= 0 && f.stunTimer <= 0 && !window.gameOver && f.hp > 0) {
            if (f.isDragon) {
                if (f.hp > 0 && f.hp <= f.maxHp * 0.3 && !f.isEvolved) { f.isEvolved = true; window.slowMoTimer = 60; window.screenFlash = 0; window.shakeScreen(50, 15); window.playSound(50, 'sawtooth', 2.0, 1.0, true); f.color = "#8e44ad"; f.scale *= 1.25; 
                    // [KINETIC TYPOGRAPHY INTEGRATED] DRAGON
                    window.floatingTexts.push({ x: f.x, y: f.y - 150, text: "🐉🔥", color: "#8e44ad", alpha: 1.5, vx: 0, vy: -5, font: "italic 900 60px 'Arial Black', Impact", life: 100, scale: 3.0, targetScale: 1.0, scaleVel: 0, rot: 0 }); 
                ...
                if (targetFighter && targetFighter.hp > 0) {
                    ...
                    if (f.aiDelay <= 0) {
                        ...
                        if (absDist > 110) { f.state = 'walk'; f.vx = Math.sign(dist) * f.currentSpeed * 1.5; if (Math.random() < 0.3) { f.state = 'dash'; f.dashTimer = 8; f.dashDir = Math.sign(dist); window.spawnDust(f.x, f.y); } } 
                            ...
                            window.floatingTexts.push({ x: f.x, y: f.y - 100, text: "👟", color: "#f1c40f", alpha: 1.5, vx: 0, vy: -4, font: "900 40px Arial", life: 40, scale: 2.0, targetScale: 1.0, scaleVel: 0, rot: 0 }); 
                        } 
                ...
                if (targetFighter && targetFighter.hp > 0) {
                    ...
                    if (f.aiDelay <= 0) {
                        ...
                        if (absDist > 250 && Math.random() < 0.6) { f.state = 'cast'; f.attackTimer = 25; window.playSound(300, 'sine', 0.3, 0.6); window.spawnSlash(f.x + (f.isFacingRight? 50:-50), f.y - 40, f.isFacingRight, "#fff", true, 2.0, Math.PI/2); window.spawnProjectile(f.x, f.y - 40, f.isFacingRight ? 12 : -12, 0, 15, "#fff", Math.floor(25 * f.dmgMod), window.p1); 
                            window.floatingTexts.push({ x: f.x, y: f.y - 80, text: "🗡️", color: "#fff", alpha: 1.5, vx: 0, vy: -4, font: "900 40px Arial", life: 40, scale: 2.0, targetScale: 1.0, scaleVel: 0, rot: 0 }); 
                        } 
                        else { f.state = 'dash'; f.dashTimer = 15; f.dashDir = Math.sign(dist); f.currentSpeed *= 2.5; f.iFrames = 20; window.playSound(400, 'sawtooth', 0.4, 0.8, true); setTimeout(() => { if(window.gameOver || f.hp <= 0 || !window.p1) return; if(Math.abs(window.p1.x - f.x) < 180) { if(typeof window.takeDamage === 'function') window.takeDamage(window.p1, Math.floor(40 * f.dmgMod), "#e74c3c", true, false); window.shakeScreen(20, 15); window.p1.vx = Math.sign(dist) * 6; } window.spawnSlash(f.x, window.p1.y - 30, f.isFacingRight, "#e74c3c", true, 3.0, (Math.random()-0.5)); 
                            window.floatingTexts.push({ x: f.x, y: f.y - 80, text: "⚡", color: "#e74c3c", alpha: 1.5, vx: 0, vy: -5, font: "900 40px Arial", life: 40, scale: 2.5, targetScale: 1.0, scaleVel: 0, rot: 0 }); 
                        }, 200); }
                ...
                if (targetFighter && targetFighter.hp > 0) {
                    ...
                    if (f.aiDelay <= 0) {
                        ...
                        if (randAction < 0.4) { f.state = 'cast'; f.attackTimer = 20; window.playSound(400, 'sine', 0.2, 0.4); window.spawnProjectile(f.x, f.y - 50, f.isFacingRight ? 15 : -15, 0, 8, "#9b59b6", Math.floor(15 * f.dmgMod), window.p1); if(Math.random() < 0.5) window.spawnProjectile(f.x, f.y - 60, f.isFacingRight ? 14 : -14, -3, 8, "#9b59b6", Math.floor(15 * f.dmgMod), window.p1); 
                            window.floatingTexts.push({ x: f.x, y: f.y - 80, text: "🥷", color: "#9b59b6", alpha: 1.5, vx: 0, vy: -4, font: "900 40px Arial", life: 30, scale: 2.0, targetScale: 1.0, scaleVel: 0, rot: 0 }); 
                        } 
                        else if (randAction < 0.7) { window.spawnParticles(f.x, f.y, "#2c3e50"); f.x = window.p1.x + (window.p1.isFacingRight ? -80 : 80); f.y = window.p1.y; f.isFacingRight = window.p1.x > f.x; window.spawnParticles(f.x, f.y, "#9b59b6"); f.state = 'spinning_backfist'; f.attackTimer = 15; window.playSound(200, 'square', 0.2, 0.6); if(typeof window.takeDamage === 'function') window.takeDamage(window.p1, Math.floor(20 * f.dmgMod), "#9b59b6", false, false); 
                            window.floatingTexts.push({ x: f.x, y: f.y - 80, text: "💨", color: "#8e44ad", alpha: 1.5, vx: 0, vy: -4, font: "900 40px Arial", life: 30, scale: 2.0, targetScale: 1.0, scaleVel: 0, rot: 0 }); 
                        } 
            ...
            for (let j = i + 1; j < allFighters.length; j++) { 
                ...
                if (dist < 80 && f1.attackTimer > 5 && f1.attackTimer < 20 && f2.attackTimer > 5 && f2.attackTimer < 20) {
                    if (!f1.clashCooldown && !f2.clashCooldown && f1.isFacingRight !== f2.isFacingRight) {
                        ...
                        // [KINETIC TYPOGRAPHY INTEGRATED] CLASH
    ...
    allFighters.forEach(f => {
        ...
        if (f.x < wallBound) { 
            ...
            if (f.hitStun > 0 && f.vx < -8 && !f.wallBounced) {
                ...
                // [KINETIC TYPOGRAPHY INTEGRATED] WALL SPLAT
    ...
    // [VIRAL 27.1 - NEW] CẬP NHẬT KINETIC TYPOGRAPHY (SPRING PHYSICS LÒ XO)
    for (let i = window.floatingTexts.length - 1; i >= 0; i--) { 
        let t = window.floatingTexts[i]; 
        if (t.life !== undefined) { 
            t.vy += window.GRAVITY * 0.6; // Nặng hơn để rớt nhanh hơn có uy lực
            t.x += t.vx; 
            t.y += t.vy; 
            t.life--; 
...
window.draw = function() {
    ...
    if (window.isLoading) {
        ...
        window.ctx.fillStyle = "#00f3ff"; window.ctx.font = "italic 900 40px Arial"; window.ctx.textAlign = "center"; window.ctx.shadowBlur = 15; window.ctx.shadowColor = "#00f3ff"; window.ctx.fillText("ESPORTS STADIUM INITIALIZING...", cx, cy - 50);
        ...
        window.ctx.fillStyle = "#fff"; window.ctx.font = "bold 16px monospace"; window.ctx.fillText(`RENDERING AAA PHYSICS... ${Math.floor(window.loadProgress)}%`, cx, cy + 60);
        return; 
    }
    ...
    try {
        ...
        // [VIRAL 27.1 - KINETIC TYPOGRAPHY RENDER]
        ...
        if (typeof impactShift !== 'undefined' && impactShift !== 0) {
            window.ctx.save();
...
// ==========================================
// 7. [VIRAL 27.1] TIME-ENGINE VÀ PHYSICS LOOP CHUẨN XÁC
// ==========================================
