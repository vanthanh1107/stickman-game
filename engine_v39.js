// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 33.0 [THE GOLDEN RATIO FRAMING]
// [NEW] CINEMATIC SKY OFFSET: Đẩy mặt đất xuống dưới, ưu tiên 75% màn hình cho Bầu trời & Nhân vật.
// [FIXED] Mở rộng và định dạng lại cấu trúc code để không bị mất bất kỳ logic nào.
// [BASE] Đầy đủ 360 Orbit Camera, Kinetic Text, Lethal Void, RTX Floor, Z-Axis Parallax.
// ==========================================
...
// [VIRAL 33.0] HỆ THỐNG MÁY QUAY ĐA CHIỀU 360 ĐỘ
...
window.initGameEngine = function() {
    ...
    for (let i = 0; i < imgData.data.length; i += 4) { 
        ...
        imgData.data[i] = val; imgData.data[i+1] = val; imgData.data[i+2] = val; imgData.data[i+3] = Math.random() * 25; 
    }
...
window.spawnParticles = function(x, y, color, isCrit = false) { 
    ...
    for(let i=0; i<count; i++) { 
        ...
        let speed = Math.random() * (isCrit?18:8) + 2; 
        window.particles.push({ x: x, y: y - 30, vx: Math.cos(angle)*speed, vy: Math.sin(angle)*speed, life: 25, maxLife: 25, color: color, size: Math.random() * 5 + 2 }); 
    } 
}
window.spawnDust = function(x, y) { 
    if (window.particles.length > MAX_PARTICLES) return;
    for(let i=0; i<8; i++) { 
        window.particles.push({ x: x + (Math.random()*40-20), y: y, vx: (Math.random()-0.5)*8, vy: -Math.random()*2 - 0.2, life: 30, maxLife: 30, color: "rgba(189, 195, 199, 0.4)", size: Math.random() * 12 + 6, isGroundDust: true }); 
    } 
}
...
window.triggerDimensionShatter = function() { 
    ...
    let cx = window.canvas ? window.canvas.width/2 : 600; 
    let cy = window.canvas ? window.canvas.height/2 : 300; 
    for(let i=0; i<8; i++) { 
...
window.spawnEnvDamage = function(x, y, type, scale, isBurning = false) { 
    ...
    for(let i=0; i<numCracks; i++) { 
        let angle; if (type === 'crater') { let step = (Math.PI * 0.8) / Math.max(1, numCracks - 1); angle = Math.PI * 0.1 + step * i + (Math.random() - 0.5) * 0.2; } else if (type === 'wall_left') angle = -Math.PI/2 + (Math.random() * Math.PI * 0.8) + Math.PI*0.1; else angle = Math.PI/2 + (Math.random() * Math.PI * 0.8) - Math.PI*0.4; 
...
window.update = function() {
            ...
            for (let j = i + 1; j < allFighters.length; j++) { 
                ...
                if (dist < 80 && f1.attackTimer > 5 && f1.attackTimer < 20 && f2.attackTimer > 5 && f2.attackTimer < 20) {
                    if (!f1.clashCooldown && !f2.clashCooldown && f1.isFacingRight !== f2.isFacingRight) {
                        ...
                        // [VIRAL 32.0] GÓC QUAY CHÉO KHI ĐỤNG CHIÊU
    ...
    // [VIRAL 33.0] GÓC QUAY, ORBIT VÀ OFFSET ĐIỆN ẢNH
    ...
    // Tính toán lia máy (Pan) dựa trên tốc độ di chuyển
    ...
    // BÙ TRỪ TRỤC Y ĐỂ TẠO TỶ LỆ VÀNG 70% BẦU TRỜI / 30% MẶT ĐẤT
    let cinematicSkyOffset = 130; 
    
    window.targetCamX = (window.canvas.width / 2) - window.orbitFocusX + window.actionCamOffsetX + actionPan;
    window.targetCamY = (window.canvas.height / 2) - window.orbitFocusY + window.actionCamOffsetY + cinematicSkyOffset;
...
window.draw = function() {
    ...
    if (window.isLoading) {
        ...
        window.ctx.fillStyle = "#00f3ff"; window.ctx.font = "italic 900 40px Arial"; window.ctx.textAlign = "center"; window.ctx.shadowBlur = 15; window.ctx.shadowColor = "#00f3ff"; window.ctx.fillText("GOLDEN FRAMING INITIALIZING...", cx, cy - 50);
        ...
        window.ctx.fillStyle = "#fff"; window.ctx.font = "bold 16px monospace"; window.ctx.fillText(`RENDERING CINEMATIC SKY... ${Math.floor(window.loadProgress)}%`, cx, cy + 60);
        return; 
    }
