// ==========================================
// ENGINE_CORE_V7.JS - BIẾN TOÀN CỤC, LOGIC, COMBAT & UPDATE
// [BẢN HOÀN CHỈNH ULTIMATE: SỐ SÁT THƯƠNG RƠI NẢY - TÊN SKILL TRÔI NỔI PHÓNG TO (NO GRAVITY)]
// ==========================================
...
// 🟢 BIẾN TOÀN CỤC CHO HỆ THỐNG ĐẠO DIỄN (CINEMATIC DIRECTOR)
window.activeSequences = []; 
...
window.playSequence = function(sequenceEvents) {
    window.activeSequences.push({ currentFrame: 0, events: sequenceEvents });
};
...
window.spawnProjectile = function(x, y, vx, vy, radius, color, dmg, target, customOnHit, imageUrl = null, imgW = 100, imgH = 100) { 
    window.projectiles.push({ 
        x: x, y: y, vx: vx, vy: vy, radius: radius, color: color, 
        dmg: dmg, target: target, onHit: customOnHit, 
        imageUrl: imageUrl, imgW: imgW, imgH: imgH 
    }); 
}
...
window.triggerTimeStop = function(frames, caster) { 
    // ĐÃ XÓA SLOW-MOTION - MỌI THỨ CHẠY MƯỢT MÀ 100% THEO THỜI GIAN THỰC
...
// 🟢 MANG SFX BAY NGƯỢC & NHỎ GỌN NHƯ GAME ACTION RPG
window.spawnMangaSFX = function(x, y, isCrit) {
    let sfxList = ['ゴゴゴ', 'ドドド', 'ドム', 'ズバーン', 'バキッ']; let text = sfxList[Math.floor(Math.random() * sfxList.length)]; 
    let size = isCrit ? 35 : 20; 
    window.mangaSfx.push({ 
        x: x + (Math.random()*60-30), y: y + (Math.random()*40-20), 
        vx: (Math.random()-0.5)*8, vy: -5 - Math.random()*5, 
        life: 40, maxLife: 40, text: text, size: size, isCrit: isCrit, ang: (Math.random()-0.5)*0.5 
    });
}

window.triggerDimensionShatter = function(x, y) { window.dimensionCracks = []; }
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce, attackerDirRight = true) {
    ...
    let wasInAir = !target.onGround || target.y < window.GROUND_Y;
    let wasBlocking = target.state === 'block';

    // 🟢 Xóa Slow Motion khi Evade, chữ trôi nổi không rớt đất
    if (target.state === 'dash' || target.state === 'dash_back') {
        if (target.dashTimer > 2 && target.dashTimer < 14) {
            ...
            window.floatingTexts.push({ x: target.x, y: target.y - 80, text: "⚡ EVADED!", color: "#00f3ff", alpha: 1.5, vx: 0, vy: -2, font: "italic 900 30px 'Arial Black', Impact", life: 50, scale: 2.0, targetScale: 1.0, scaleVel: 0, rot: (Math.random()-0.5)*0.1, noGravity: true });
    ...
    if (target.state === 'block') { 
        if (target.stamina < 15) {
            ...
            window.spawnParticles(target.x, target.y - 40, "#ff003c", true); 
            window.floatingTexts.push({ x: target.x, y: target.y - 90, text: "💔 CRUSH!", color: "#ff003c", alpha: 1.5, vx: 0, vy: -2, font: "italic 900 35px 'Arial Black', Impact", life: 60, scale: 2.5, targetScale: 1.0, scaleVel: 0, rot: (Math.random()-0.5)*0.2, noGravity: true });
            ...
            window.targetZoom = 1.5; window.dynamicBlur = 10; window.impactAberration = 15; window.chromaTimer = 20; window.vhsGlitchTimer = 20; window.spawnForegroundDebris(target.x, target.y, attackerDirRight); 
            
            if (typeof window.spawnGlassShatter === 'function') window.spawnGlassShatter(target.x, target.y - 40);
        } 
            ...
            window.floatingTexts.push({ x: target.x, y: target.y - 80, text: "🛡️ PARRY!", color: "#00ffff", alpha: 1.5, vx: 0, vy: -2, font: "italic 900 30px 'Arial Black', Impact", life: 50, scale: 2.0, targetScale: 1.0, scaleVel: 0, rot: (Math.random()-0.5)*0.1, noGravity: true });
            target.stamina = Math.min(100, target.stamina + 30); window.shakeScreen(12, 8); window.chromaTimer = 10; window.impactAberration = 10;
    ...
    if (finalDmg > 0) {
        ...
        // 🟢 SỐ SÁT THƯƠNG NHỎ, BAY LÙI THEO CÚ ĐẤM VÀ RỚT XUỐNG ĐẤT
        let textVx = (attackerDirRight ? 1 : -1) * (Math.random() * 5 + 3); let textVy = -6 - Math.random() * 4; 
        let initialScale = isCrit ? 2.0 : 1.2; let targetScale = isCrit ? 1.0 : 0.8; 
        
        window.floatingTexts.push({ x: target.x, y: target.y - 60, text: dmgText, color: color || (isCrit ? "#ff4757" : "#fff"), alpha: 1.5, vx: textVx, vy: textVy, font: isCrit ? "italic 900 35px 'Arial Black', Impact" : "italic 900 25px 'Arial Black', Impact", life: 60, scale: initialScale, targetScale: targetScale, scaleVel: 0, rot: (Math.random()-0.5)*0.3 });
        ...
        if (isCrit) {
            ...
            // XÓA SLOW-MOTION CHÍ MẠNG
            window.spawnLightning(target.x, target.y-40, color || "#f1c40f", 30, 3);
            window.spawnSpaceRipple(target.x, target.y - 40, color || "#f1c40f");

            // 🟢 EMOJI CRIT NỔI LÊN, KHÔNG RỚT XUỐNG ĐẤT
            window.floatingTexts.push({ x: target.x + (Math.random()*40-20), y: target.y - 60, text: "💢", color: "#f1c40f", alpha: 1.5, vx: 0, vy: -2, font: "italic 900 35px 'Arial Black', Impact", life: 40, scale: 2.0, targetScale: 1.0, scaleVel: 0, rot: 0, noGravity: true }); 
        ...
        if (target.hp > 0 && wasInAir && !wasBlocking) {
            target.vy = isCrit ? -15 : -9; 
            target.onGround = false;
            if (typeof window.spawnMangaSFX === 'function') window.spawnMangaSFX(target.x, target.y - 50, true);
            window.speedLinesAlpha = 0.8;
        }
        ...
        if (target.hp <= 0) {
            ...
            if (isNearWall && isCrit && typeof window.triggerStageTransition === 'function') { window.triggerStageTransition(target); } 
                ...
                if (isCrit) {
                    window.screenFlash = 0; window.invertFrames = 0; window.speedLinesAlpha = 0.5; window.spawnLensFlare(target.x, target.y - 40, "#ff003c", 3.0);
                    // 🟢 K.O NỔI TO RA GIỮA MÀN HÌNH KHÔNG RỚT XUỐNG
                    window.floatingTexts.push({ x: target.x, y: target.y - 120, text: "💀 K.O!", color: "#ff003c", alpha: 1.5, vx: 0, vy: -1, font: "italic 900 65px 'Arial Black', Impact", life: 100, scale: 3.5, targetScale: 1.2, scaleVel: 0, rot: (Math.random()-0.5)*0.1, noGravity: true });
                    window.targetZoom = 1.6; window.actionCamOffsetX = 0; window.actionCamOffsetY = -20; window.dynamicBlur = 10; 
                    window.lethalVoid = 60; window.impactAberration = 10; window.spawnForegroundDebris(target.x, target.y, attackerDirRight); 
                    ...
                    window.isSpinningCam = true; window.orbitFocusX = target.x; 
                    setTimeout(()=>{ window.cameraTilt = 0; window.isSpinningCam = false; window.targetCamOrbitAngle = 0; }, 2500); 

                } else { window.shakeScreen(20, 15); window.targetZoom = 1.4; window.actionCamOffsetY = -30; window.impactAberration = 15; }
        ...
        } else if (isCrit) { window.shakeScreen(12, 10); window.chromaTimer = 12; window.playSound(180, 'square', 0.3, 0.6, true); } 
        else { window.shakeScreen(4, 3); window.playSound(250, 'sine', 0.15, 0.3, true); }
    }
};
...
window.attack = function(attacker, targetGroup) {
    ...
    let selectedMove = 'jab'; let isFinisher = false;
    ...
    attacker.pendingHit = {
        target: target,
        delay: isFinisher ? 12 : 6,
        isFinisher: isFinisher,
        baseDmg: 12 * attacker.currentDmgMod
    };
};
...
window.update = function() {
    ...
    // CHẠY CHỮ MANGA RƠI RỤNG (SFX)
    if (window.mangaSfx) { 
        window.mangaSfx.forEach(sfx => { 
            sfx.vy += window.GRAVITY * 0.4; 
            sfx.x += sfx.vx; 
            sfx.y += sfx.vy; 
            if (sfx.y >= window.GROUND_Y - 5) {
                sfx.y = window.GROUND_Y - 5;
                sfx.vy *= -0.5;
                sfx.vx *= 0.6;
            }
            sfx.life--; 
        }); 
        window.mangaSfx = window.mangaSfx.filter(sfx => sfx.life > 0); 
    }
    ...
    if (Math.random() < 0.15 && window.bokehs.length < 35 && window.lethalVoid <= 0) {
        window.bokehs.push({
            x: window.camX + Math.random()*1500 - 750, y: Math.random()*window.canvas.height + 100, 
    ...
    if (!window.gameOver) {
        ...
        let hazardChance = 0.002 + (window.matchTimer / 3600) * 0.01; 
        if (Math.random() < hazardChance && window.projectiles.length < 10) { window.projectiles.push({ x: Math.random() * window.canvas.width, y: -100, vx: (Math.random() - 0.5) * 4, vy: 8 + Math.random() * 6, radius: 12 + Math.random() * 8, color: "#e67e22", dmg: 45, target: null, isMeteor: true }); }
        if (Math.random() < hazardChance) { window.envHazards.push({ type: 'lightning', x: Math.random() * window.canvas.width, timer: 45, state: 'warning' }); }
        if (window.currentWeather === 'ash' && Math.random() < 0.003) { window.envHazards.push({ type: 'lava', x: Math.random() * window.canvas.width, timer: 60 }); }
    }
    ...
    // 🟢 CẬP NHẬT CINEMATIC MƯỢT MÀ SONG SONG 
    if (window.cinematicTimer > 0) { 
        window.cinematicTimer--; 
        if (window.cinematicTimer === 0 && window.cinematicCallback) { 
            try { window.cinematicCallback(); } catch(e) {} 
            window.cinematicCallback = null; 
        } 
    }
    ...
    for (let i = window.projectiles.length - 1; i >= 0; i--) {
        ...
        if (proj.isMeteor) {
            ...
            if (proj.y >= window.GROUND_Y) {
                ...
                if (window.particles.length < MAX_PARTICLES) { for(let p=0; p<20; p++) { window.particles.push({ x: proj.x + (Math.random()-0.5)*50, y: window.GROUND_Y, vx: (Math.random()-0.5)*8, vy: -Math.random()*8 - 6, life: 45 + Math.random()*20, maxLife: 65, color: Math.random() > 0.4 ? "#f1c40f" : "#ff4757", size: Math.random()*6 + 4, isAuraFlame: true }); } }
    ...
    for (let i = window.envHazards.length - 1; i >= 0; i--) {
        ...
        if (haz.type === 'lightning') {
            if (haz.state === 'warning' && haz.timer <= 0) { 
                haz.state = 'striking'; haz.timer = 12; window.playSound(300, 'sawtooth', 0.8, 0.8, true); window.shakeScreen(20, 15); 
                window.spawnEnergyPillar(haz.x, window.GROUND_Y, "rgba(0, 243, 255, 0.4)", 50, 12); 
                for(let l=0; l<3; l++) window.spawnLightning(haz.x, window.GROUND_Y, "#00f3ff", 80, 5); 
                if (window.shockwaves.length < MAX_SHOCKWAVES) window.shockwaves.push({x: haz.x, y: window.GROUND_Y, r: 10, maxR: 200, color: "#00f3ff", alpha: 0.8, speed: 15});
                if (window.particles.length < MAX_PARTICLES) { for(let p=0; p<20; p++) { window.particles.push({ x: haz.x + (Math.random()-0.5)*50, y: window.GROUND_Y, vx: (Math.random()-0.5)*8, vy: -Math.random()*8 - 6, life: 40 + Math.random()*20, maxLife: 60, color: Math.random() > 0.4 ? "#00f3ff" : "#ffffff", size: Math.random()*5+4, isAuraFlame: true }); } }
    ...
    window.enemies.forEach(e => { 
        if (e.hp <= 0 && !e.deathTriggered) {
            ...
            if (window.p1 && window.p1.hp > 0) { let heal = Math.floor(window.p1.maxHp * 0.08); window.p1.hp = Math.min(window.p1.maxHp, window.p1.hp + heal); window.floatingTexts.push({ x: window.p1.x, y: window.p1.y - 80, text: `+${heal} 💚`, color: "#2ed573", alpha: 1.5, vx: 0, vy: -2, font: "italic 900 25px 'Arial Black', Impact", life: 45, scale: 1.5, targetScale: 1.0, scaleVel: 0, rot: 0, noGravity: true }); }
        }
    });
    ...
    allFighters.forEach(f => {
        ...
        if (window.gameOver && f.hp > 0) {
            f.dashTimer = 0; 
            if (f.onGround) { f.vx *= 0.5; if (Math.abs(f.vx) < 0.5) f.vx = 0; }
            if (f.state === 'walk' || f.state === 'dash' || f.state === 'dash_back') f.state = 'idle';
            ...
            if (f.hitStun <= 0 && f.stunTimer <= 0 && f.state !== 'dash_back' && f.state !== 'block') {
                ...
                if (isCloseEnough) {
                    if (window.classStats && window.classStats[type] && typeof window.classStats[type].executeUltimate === 'function') {
                        f.stamina = 0; let baseDmg = 50 * (f.currentDmgMod || 1); if (!f.isPlayer) baseDmg = 35 * (f.currentDmgMod || 1); window.playSound(400, 'sine', 0.5, 0.6); window.shakeScreen(15, 10); window.spawnParticles(f.x, f.y, "#f1c40f", true); window.floatingTexts.push({ x: f.x, y: f.y - 100, text: f.isPlayer ? "🔥 ULTIMATE!" : "⚠️ DANGER!", color: f.isPlayer ? "#ff4757" : "#ff0000", alpha: 1.5, vx: 0, vy: -2, font: "italic 900 35px 'Arial Black', Impact", life: 50, scale: 2.0, targetScale: 1.0, scaleVel: 0, rot: 0, noGravity: true }); f.isFacingRight = distToTarget > 0; window.classStats[type].executeUltimate(f, closestTarget, baseDmg); f.vx = 0; window.speedLinesAlpha = 1.0; 
        ...
        if (!launchedUltimate && f.attackTimer <= 0 && f.hitStun <= 0 && f.dashTimer <= 0 && f.stunTimer <= 0 && !window.gameOver && f.hp > 0) {
            if (f.isDragon) {
                if (f.hp > 0 && f.hp <= f.maxHp * 0.3 && !f.isEvolved) { f.isEvolved = true; window.screenFlash = 0; window.shakeScreen(50, 15); window.playSound(50, 'sawtooth', 2.0, 1.0, true); f.color = "#8e44ad"; f.scale *= 1.25; window.floatingTexts.push({ x: f.x, y: f.y - 150, text: "🐉🔥", color: "#8e44ad", alpha: 1.5, vx: 0, vy: -2, font: "italic 900 45px 'Arial Black', Impact", life: 100, scale: 2.0, targetScale: 1.0, scaleVel: 0, rot: 0, noGravity: true }); window.shockwaves.push({x: f.x, y: window.GROUND_Y, r: 10, maxR: 500, color: "#8e44ad", alpha: 1, speed: 25}); window.targetCamOrbitAngle = 0.5; }
                ...
                if (targetFighter && targetFighter.hp > 0) {
                    ...
                    if (f.aiDelay <= 0) {
                        ...
                        if (randAction < 0.4) { f.state = 'cast'; f.attackTimer = 20; window.playSound(400, 'sine', 0.2, 0.4); window.spawnProjectile(f.x, f.y - 50, f.isFacingRight ? 15 : -15, 0, 8, "#9b59b6", Math.floor(15 * f.dmgMod), window.p1); if(Math.random() < 0.5) window.spawnProjectile(f.x, f.y - 60, f.isFacingRight ? 14 : -14, -3, 8, "#9b59b6", Math.floor(15 * f.dmgMod), window.p1); window.floatingTexts.push({ x: f.x, y: f.y - 80, text: "🥷", color: "#9b59b6", alpha: 1.5, vx: 0, vy: -2, font: "900 30px Arial", life: 30, scale: 1.5, targetScale: 1.0, scaleVel: 0, rot: 0, noGravity: true }); window.targetCamOrbitAngle = f.isFacingRight ? -0.1 : 0.1; } 
                        else if (randAction < 0.7) { window.spawnParticles(f.x, f.y, "#2c3e50"); f.x = window.p1.x + (window.p1.isFacingRight ? -80 : 80); f.y = window.p1.y; f.isFacingRight = window.p1.x > f.x; window.spawnParticles(f.x, f.y, "#9b59b6"); f.state = 'spinning_backfist'; f.attackTimer = 15; window.playSound(200, 'square', 0.2, 0.6); if(typeof window.takeDamage === 'function') window.takeDamage(window.p1, Math.floor(20 * f.dmgMod), "#9b59b6", false, false); window.floatingTexts.push({ x: f.x, y: f.y - 80, text: "💨", color: "#8e44ad", alpha: 1.5, vx: 0, vy: -2, font: "900 30px Arial", life: 30, scale: 1.5, targetScale: 1.0, scaleVel: 0, rot: 0, noGravity: true }); window.targetCamOrbitAngle = f.isFacingRight ? 0.2 : -0.2; } 
        ...
        else { f.vy += window.GRAVITY; if (f.vy > 0 && f.y + f.vy >= window.GROUND_Y && !f.onGround) { window.spawnDust(f.x, window.GROUND_Y); } }
        ...
        for (let j = i + 1; j < allFighters.length; j++) { 
            ...
            if (dist < 80 && f1.attackTimer > 5 && f1.attackTimer < 20 && f2.attackTimer > 5 && f2.attackTimer < 20) {
                ...
                if (!f1.clashCooldown && !f2.clashCooldown && f1.isFacingRight !== f2.isFacingRight) {
                    ...
                    window.playSound(400, 'square', 0.1, 0.4, true);
                    let midX = (f1.x + f2.x)/2; let midY = (f1.y + f2.y)/2 - 30;
                    window.spawnParticles(midX, midY, "#f1c40f", true);
                    if (window.shockwaves.length < MAX_SHOCKWAVES) {
                        window.shockwaves.push({x: midX, y: midY, r: 10, maxR: 150, color: "#ffffff", alpha: 0.8, speed: 20});
                    }
                    window.floatingTexts.push({ x: midX, y: midY - 60, text: "⚔️ BRAWL!", color: "#f1c40f", alpha: 1.5, vx: 0, vy: -2, font: "italic 900 25px 'Arial Black', Impact", life: 60, scale: 1.5, targetScale: 1.0, scaleVel: 0, rot: (Math.random()-0.5)*0.1, noGravity: true });
                }
            }
    ...
    // 🟢 XỬ LÝ SÁT THƯƠNG CHỜ (PENDING HIT)
    allFighters.forEach(f => {
        if (f && f.pendingHit && f.hp > 0) {
            let hit = f.pendingHit;
            hit.delay--;
            
            if (hit.delay <= 0) {
                let target = hit.target;
                let reach = 85 * (f.scale || 1);
                
                if (target && target.hp > 0 && Math.abs(f.x - target.x) <= reach + 35) {
                    let isCrit = false;
                    let finalDmg = hit.baseDmg;

                    if (hit.isFinisher) {
                        isCrit = true; 
                        finalDmg *= 3.5;
                        window.shakeScreen(15, 12); target.vx = (f.isFacingRight ? 6 : -6); target.state = 'hurt'; target.hitStun = 50; 
                        window.spawnParticles(target.x, target.y - 40, "#ff4757", true); 
                        window.floatingTexts.push({ x: target.x, y: target.y - 80, text: "💥", color: "#ff4757", alpha: 1.5, vx: 0, vy: -2, font: "900 45px Arial", life: 50, scale: 2.0, targetScale: 1.0, scaleVel: 0, rot: 0, noGravity: true });
                        window.speedLinesAlpha = 1.0; window.actionCamOffsetX = f.isFacingRight ? 80 : -80; 
                        window.targetCamOrbitAngle = f.isFacingRight ? 0.3 : -0.3;
                    } else {
                        if (Math.random() < f.critChance) { 
                            isCrit = true; finalDmg *= f.critMult; 
                            window.floatingTexts.push({ x: target.x + (Math.random()*40-20), y: target.y - 60, text: "💢", color: "#f1c40f", alpha: 1.5, vx: 0, vy: -2, font: "italic 900 25px 'Arial Black', Impact", life: 40, scale: 1.5, targetScale: 1.0, scaleVel: 0, rot: 0, noGravity: true }); 
                        } else { window.playSound(350, 'sine', 0.1, 0.1, false); }
                        if ((f.state === 'low_kick' || f.state === 'teep_kick') && Math.random() < 0.4) { target.stunTimer = 35; target.state = 'stunned'; window.floatingTexts.push({ x: target.x, y: target.y - 50, text: "🦵", color: "#e67e22", alpha: 1.5, vx: 0, vy: -2, font: "900 25px Arial", life: 40, scale: 1.2, targetScale: 1.0, scaleVel: 0, rot: 0, noGravity: true }); }
                        if ((f.state === 'uppercut' || f.state === 'elbow_strike') && Math.random() < 0.3) { finalDmg *= 1.5; window.spawnParticles(target.x, target.y - 60, "#c0392b", true); window.floatingTexts.push({ x: target.x, y: target.y - 50, text: "🩸", color: "#c0392b", alpha: 1.5, vx: 0, vy: -2, font: "900 25px Arial", life: 40, scale: 1.2, targetScale: 1.0, scaleVel: 0, rot: 0, noGravity: true }); }
                        target.vx = (f.isFacingRight ? 3 : -3); target.hitStun = 18; target.state = 'hurt';
                    }

                    if (typeof window.takeDamage === 'function') window.takeDamage(target, Math.floor(finalDmg), isCrit ? "#ff4757" : "#fff", isCrit, false, f.isFacingRight);
                    
                    f.comboHits = (f.comboHits || 0) + 1; f.comboDisplayTimer = 90; f.comboAlpha = 1; 
                    f.stamina = Math.min(100, f.stamina + (isCrit ? 5.0 : 1.5));
                    
                    let slashAngle = (f.state.includes('uppercut') || f.state.includes('high_kick')) ? -Math.PI / 5 : (Math.random() - 0.5) * 0.4;
                    if (typeof window.spawnSlash === 'function') window.spawnSlash(target.x, target.y - 35, f.isFacingRight, isCrit ? "#ff4757" : "#ecf0f1", isCrit, hit.isFinisher ? 2.5 : 1.5, slashAngle);
                } else if (f.onGround) {
                    if (typeof window.spawnDust === 'function') window.spawnDust(f.x + (f.isFacingRight ? 40 : -40), f.y);
                }
                f.pendingHit = null; 
            }
        }
    ...
    // 🟢 NÂNG CẤP VẬT LÝ CHO DÒNG TEXT
    for (let i = window.floatingTexts.length - 1; i >= 0; i--) { 
        let t = window.floatingTexts[i]; 
        
        if (!t.noGravity) {
            // Logic cho số sát thương rơi xuống đất
            t.vy += window.GRAVITY * 0.4; 
            t.x += t.vx; 
            t.y += t.vy; 
            
            if (t.y >= window.GROUND_Y - 5) {
                t.y = window.GROUND_Y - 5;
                t.vy *= -0.5; 
                t.vx *= 0.6;  
            }
        } else {
            // Logic cho Tên Skill / Hiệu ứng nổi bật (Bay nhẹ hoặc đứng im)
            t.x += t.vx;
            t.y += t.vy;
            t.vx *= 0.85; 
            t.vy *= 0.85; 
    ...
    // 🟢 HỆ THỐNG ĐẠO DIỄN KỊCH BẢN (CINEMATIC DIRECTOR)
    if (window.activeSequences && window.activeSequences.length > 0) {
        for (let i = window.activeSequences.length - 1; i >= 0; i--) {
            let seq = window.activeSequences[i];
            seq.events.forEach(e => {
                if (e.frame === seq.currentFrame) e.action();
            });
            seq.currentFrame++;
            let maxFrame = Math.max(...seq.events.map(e => e.frame));
            if (seq.currentFrame > maxFrame) {
                window.activeSequences.splice(i, 1);
            }
        }
    }
    ...
    if (window.gameOver && window.p1) {
        midPointX = window.p1.x;
    } else if (window.p1 && window.enemies.length > 0) {
    ...
    if (!window.isSpinningCam) {
        window.orbitFocusX += (midPointX - window.orbitFocusX) * (window.gameOver ? 0.05 : 0.15);
    }

    let closest = window.p1 ? window.getClosestEnemy(window.p1, window.enemies) : null;
    let actionPan = (!window.gameOver && closest && !window.isSpinningCam) ? (window.p1.vx + (closest.vx || 0)) * 6 : 0;
    
    window.targetCamX = (window.canvas.width / 2) - window.orbitFocusX + window.actionCamOffsetX + actionPan;
    
    if (window.gameOver) {
        window.targetCamY = (window.canvas.height / 2) - window.GROUND_Y + 120 + window.actionCamOffsetY; 
    } else {
        window.targetCamY = (window.canvas.height / 2) - window.orbitFocusY + window.actionCamOffsetY + 60;
    }

    if (window.gameOver) {
        window.targetZoom = 1.8; 
        window.targetTilt = 0; 
        if(window.isSpinningCam) window.targetCamY += 40; 
    } 
    else if (closest && !window.gameOver) {
