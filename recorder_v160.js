// ==========================================
// RECORDER.JS - V75.9 THE ALGORITHM BREAKER (ULTIMATE ANTI-LAG FACECAM)
// ĐÃ FIX & GIỮ NGUYÊN: Intro God-Tier, Vỡ kính 3D, Kanji Manga, Kill-Cam, Viral Post Kit.
// NÂNG CẤP MỚI NHẤT (V75.9): OFFSCREEN CANVAS BUFFERING & ASYNC LOAD.
// Tối ưu hóa: Facecam load ngầm background, khóa FPS Facecam ở 24FPS (giảm 66% tải CPU), chống giật lag tuyệt đối khi combat!
// ==========================================
...
window.initRecorder = function() {
    ...
    // KHO BẢN NHÁP DÀNH RIÊNG CHO FACECAM ĐỂ TRÁNH LAG
    if (!window._facecamBufferCanvas) {
        window._facecamBufferCanvas = document.createElement("canvas");
        window._facecamBufferCanvas.width = 240; 
        window._facecamBufferCanvas.height = 280;
        window._facecamBufferCtx = window._facecamBufferCanvas.getContext("2d", {alpha: false, willReadFrequently: false});
    }
};
...
window.startRecording = function() {
    ...
    // SETUP ASYNC FACECAM VIDEO:
    window._facecamVideoReady = false; 
    if (window._facecamVideo) {
        ...
        window._facecamVideo.muted = true;
        window._facecamVideo.loop = true;
        window._facecamVideo.playsInline = true;
        window._facecamVideo.preload = "auto";
        window._facecamVideo.style.display = "none"; 
        
        // Khi video tải đủ buffer để play, ta mới giương cờ cho phép vẽ
        window._facecamVideo.oncanplay = function() {
            window._facecamVideoReady = true;
            window._facecamVideo.play().catch(e=>console.log("Async Play Error:", e));
        };
    }
    // Gán source random và kệ nó load ngầm, game vẫn chạy mượt
    ...
    window._lastFacecamDrawTime = 0; // Bộ đếm FPS cho facecam
...
window.captureFrames = function() {
    ...
    if (renderNormalV) {
        ...
        if (!isOutroActive) {
            ...
            // ========================================================
            // 3. FACECAM (THUẬT TOÁN BUFFER KHÓA 24FPS CHỐNG LAG GPU)
            // ========================================================
            if (window._facecamVideo && window._facecamVideoReady) { 
                ...
                if (vW > 0 && vH > 0) {
                    ...
                    // CHỈ RÚT KHUNG HÌNH TỪ VIDEO KHI QUA 41ms (~24 FPS), CÒN GAME VẪN 60 FPS
                    if (now - window._lastFacecamDrawTime > 41) {
                        window._facecamBufferCtx.fillStyle = "#000";
                        window._facecamBufferCtx.fillRect(0,0,240,280);
                        // Vẽ rút gọn vào canvas nháp (Rất nhẹ cho GPU)
                        window._facecamBufferCtx.drawImage(window._facecamVideo, (240 - camW)/2, (280 - camH)/2, camW, camH);
                        window._lastFacecamDrawTime = now;
                    }

                    // Tọa độ Neo lề phải cố định
                    ...
                    // Vẽ cái bản nháp ra (thao tác Canvas-to-Canvas là siêu nhẹ đối với trình duyệt)
                    ctxV.drawImage(window._facecamBufferCanvas, -maxW/2, -maxH/2, maxW, maxH);
                    ctxV.restore();
                    
                    // Viền Neon bên ngoài (Vẽ 60fps để có độ chớp mượt mà theo âm thanh)
                    ctxV.save(); ctxV.translate(camX, camY); ctxV.lineWidth = 5;
