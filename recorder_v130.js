// ==========================================
// RECORDER.JS - V70.7 GLOBAL CINEMA EDITION (MULTI-LINGUAL SAKUGA)
// ĐÃ FIX: Tên nhân vật hiển thị nổi 3D tuyệt đẹp, không bao giờ bị che khuất do zoom.
// CẬP NHẬT: Kịch bản phim đa ngôn ngữ (Hàn, Nhật, Trung, Anh, Việt, Tây Ban Nha...).
// CẢM XÚC: Action dồn dập, chuẩn Trailer Quốc tế, giữ chân 100% người xem.
...
// ====================================================================
// ĐẠO DIỄN: THE MULTI-LINGUAL SAKUGA (ÉP TIM 3.2 GIÂY)
// ====================================================================
window.drawProceduralIntro = function(ctx, w, h, progress) {
    ...
    // HÀM VẼ NHÂN VẬT (BỎ VẼ CHỮ Ở ĐÂY ĐỂ TRÁNH BỊ CẮT)
    ...
    if (act === 0) {
        drawFighterArtistic(e1, 0, 100, 'idle', false, prm.themeP2);
    } 
    else if (act === 1) {
        ...
        drawFighterArtistic(e1, pushP2, 150, 'attack', false, prm.themeP2);
        drawFighterArtistic(window.p1, pushP1, 150, 'attack', true, prm.themeP1);
    ...
    // --- TÊN NHÂN VẬT (NỔI 3D SÁT MÉP VIỀN ĐEN KHÔNG BỊ CẮT) ---
    if (act === 0 || act === 1) {
        ctx.save();
        ...
        let nameY = barHeight + 30; // Bay ngay phía dưới viền đen trên cùng
        
        ctx.shadowColor = cTheme.aura; ctx.shadowBlur = 20;
        ctx.lineWidth = 8; ctx.strokeStyle = "#000";
        ctx.strokeText(cName, w/2, nameY);
        ctx.shadowBlur = 0; ctx.fillStyle = "#fff"; ctx.fillText(cName, w/2, nameY);
        ctx.restore();
    }
    ...
    if (act === 0) {
        ...
        // Hỗ trợ hiển thị chữ Tiếng Hàn, Tiếng Nhật, Tiếng Trung
...
// AUDIO & RECORDER CORE (GIỮ NGUYÊN BÊN TRONG CÁC KHỐI SYSTEM QUAY HOẠT ĐỘNG HOÀN HẢO)
