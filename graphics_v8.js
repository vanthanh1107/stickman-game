...
// NÂNG CẤP: GẮN AVATAR HÌNH ẢNH 3D LÀM ĐẦU NHÂN VẬT THỰC TẾ (KHÔNG VIỀN MÀU)
// ==========================================
...
window.drawStickman = function(ctx, p, isTrail = false) {
    ...
    if (!customDrawSuccess) {
        ...
        // ==========================================
        if (typeof window.drawAvatarHead === 'function') {
            window.drawAvatarHead(ctx, head, p, isTrail);
        }
...
// ==========================================
// HÀM TIỆN ÍCH DÙNG CHUNG: VẼ ĐẦU AVATAR 3D ĐỘC QUYỀN
// ==========================================
window.drawAvatarHead = function(ctx, head, p, isTrail, headSize = 34) {
    let avatarUrl = p.avatarUrl;
    if (!avatarUrl && p.classId && window.classStats && window.classStats[p.classId]) {
        avatarUrl = window.classStats[p.classId].avatarUrl;
    }
    if (!avatarUrl && window.currentLoadedChar) {
        avatarUrl = window.currentLoadedChar.avatarUrl; // fallback for menu
    }
    ...
    const drawFallbackHead = () => {
        ...
        if (!isTrail) {
            ...
            ctx.beginPath(); ctx.arc(head.x + 4, head.y - 2, 2.5, 0, Math.PI*2); ctx.fill(); ctx.shadowBlur = 0; 
        }
    };
    ...
    if (avatarUrl) {
        ...
        if (img && img.complete && img.naturalWidth > 0) {
            ...
            // 3D Shading
            let faceShadow = ctx.createRadialGradient(
                headSize * 0.15, -headSize * 0.15, 0, 
                0, 0, headSize / 2
            );
            faceShadow.addColorStop(0, "rgba(255, 255, 255, 0.2)"); 
            faceShadow.addColorStop(0.5, "rgba(0, 0, 0, 0)"); 
            faceShadow.addColorStop(1, "rgba(0, 0, 0, 0.8)"); 

            ctx.fillStyle = faceShadow;
            ctx.fill(); 
            ctx.restore();

            // Đã xóa phần vẽ viền Neon theo yêu cầu
        } else {
            drawFallbackHead();
        }
    } else {
