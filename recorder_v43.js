// ==========================================
// RECORDER.JS - BẢN HOÀN HẢO CHO CONTENT CREATOR (V59.0 - MANGA TRASH-TALK + EMOJI)
// [FIXED] Sửa Outro bị cắt sớm: Kéo dài thời gian quay sau K.O lên 6 giây (350 frames).
// [NEW] ANIMATED EMOJIS: Bổ sung các icon Giận dữ, Hài hước, Sợ hãi nảy tưng tưng bên cạnh khung thoại Intro.
// [RETAINED] Màn hình chẻ đôi kiểu Manga, Thumbnail 3D Text siêu Meme nhúng vào 0.5s đầu tiên.
// ==========================================
...
// ==========================================
// MỚI: HỆ THỐNG VẼ ANIMATED INTRO (MANGA TRASH TALK + EMOJIS)
// ==========================================
window.drawAnimatedIntro = function(ctx, w, h, isVertical, progress) {
    ...
    let e1 = window.enemies && window.enemies.length > 0 ? window.enemies[0] : window.p1;
    let easeOut = 1 - Math.pow(1 - Math.min(1, progress * 4), 3); // Trượt nhanh vào
    
    let p1TargetX = isVertical ? w*0.5 : w*0.3; let p1TargetY = isVertical ? h*0.25 : h*0.7;
    let p1StartX = isVertical ? -w : -w*0.5; let p1X = p1StartX + (p1TargetX - p1StartX) * easeOut;

    let p2TargetX = isVertical ? w*0.5 : w*0.7; let p2TargetY = isVertical ? h*0.75 : h*0.7;
    let p2StartX = isVertical ? w*2 : w*1.5; let p2X = p2StartX + (p2TargetX - p2StartX) * easeOut;
    ...
    ctx.translate(p1X, p1TargetY); ctx.shadowColor = "#00f3ff"; ctx.shadowBlur = 40;
    ...
    ctx.closePath(); ctx.clip();
    ctx.fillStyle = "rgba(255, 0, 50, 0.2)"; ctx.fill(); 
    ...
    // VẼ KHUNG THOẠI + EMOJI NẢY BÊN CẠNH
    const drawSpeechAndEmoji = (x, y, text, isP1, emojiStr) => {
        ctx.save(); ctx.translate(x, y);
        let tw = ctx.measureText(text).width + 80;
        let th = 100;
        
        // Vẽ khung thoại nghiêng
        ctx.save();
        ...
        ctx.fill(); ctx.stroke();
        ctx.restore(); // Hủy nghiêng để vẽ chữ và Emoji thẳng
        ...
        // Vẽ Emoji Nảy (Bouncing Emoji)
        ctx.save();
        let emX = isP1 ? tw/2 : -tw/2;
        let emY = -th/2 - 10;
        let tPulse = Date.now() / 80; // Nhịp nảy
        ctx.translate(emX, emY + Math.sin(tPulse) * 10); // Nảy tưng tưng
        ctx.rotate(Math.sin(tPulse * 0.5) * 0.3); // Lắc nhẹ
        ctx.font = "80px Arial"; ctx.shadowColor = "#000"; ctx.shadowBlur = 15; ctx.shadowOffsetX = 5; ctx.shadowOffsetY = 5;
    ...
    // Khung P1 nảy lên
    if (progress > 0.3) {
        let p1Pop = Math.min(1, (progress - 0.3) * 10); 
        ctx.save(); ctx.translate(isVertical ? w*0.5 : w*0.25, isVertical ? h*0.1 : h*0.3); ctx.scale(p1Pop, p1Pop);
        drawSpeechAndEmoji(0, 0, window.trashTalkP1, true, window.introEmojiP1); ctx.restore();
    }
    // Khung P2 đáp trả
    if (progress > 0.5) {
        ...
        drawSpeechAndEmoji(0, 0, window.trashTalkP2, false, window.introEmojiP2); ctx.restore();
    }

    // SNAP-CUT FLASH (Chớp sáng trắng ở cuối để bay thẳng vào game)
    if (progress > 0.9) {
...
window.startRecording = function() {
    ...
    // TẠO LỜI THOẠI TRASH TALK VÀ EMOJI NGẪU NHIÊN CHO INTRO
...
window.captureFrames = function() {
    ...
    // 🔥 [AUTO CUT] KÉO DÀI THỜI GIAN ĐỂ NGƯỜI XEM KỊP ĐỌC OUTRO (KHOẢNG ~6 GIÂY SAU K.O)
    ...
    if (window.introHoldFrames > 0) {
        ...
        if (window.introHoldFrames === 5) window.playSound(100, 'sawtooth', 0.5, 0.8, true); 
