...
// Tối ưu Layout: Game -> Red Banner -> HUD -> Live Chat -> 3D Avatar
// ==========================================
...
// Thay link ảnh này bằng link trực tiếp (.png, .jpg) của Mẫu 3D Sexy
window.SEXY_3D_MODEL_URL = "https://i.imgur.com/k8L1z8l.jpeg"; 
...
// Danh sách bình luận Toxic/Hài hước
...
// ====================================================================
// HỆ THỐNG VẼ MODEL 3D SEXY & AUDIO VISUALIZER
// ====================================================================
function drawVTuberCommentator(ctx, x, y, audioPeak) {
    if (!window.vtuberImg) {
        window.vtuberImg = new Image();
        window.vtuberImg.crossOrigin = "Anonymous";
        window.vtuberImg.loaded = false;
        window.vtuberImg.error = false;
        window.vtuberImg.onload = function() { window.vtuberImg.loaded = true; };
        window.vtuberImg.onerror = function() { window.vtuberImg.error = true; };
    ...
    // Nhịp tim / Độ giật của Avatar theo tiếng đập
    let pulse = 1 + (audioPeak * 0.15);
    let floatY = Math.sin(t / 250) * 10;
    ctx.translate(x, y + floatY);
    ctx.scale(pulse, pulse);

    let radius = 160; // Kích thước vòng tròn Avatar

    // Vẽ Vòng Sóng Âm (Audio Visualizer Ring) đằng sau Avatar
    if (window.analyserData) {
        ctx.lineWidth = 6;
        ctx.lineCap = "round";
        for (let i = 0; i < 32; i++) {
            let val = window.analyserData[i * 2] / 255;
            let barLength = 20 + (val * 80); // Độ dài tia sóng âm
    ...
    // Vẽ Ảnh 3D Sexy hoặc vẽ Chữ 3D nếu ảnh bị lỗi (Fallback)
    if (window.vtuberImg.loaded && !window.vtuberImg.error && window.vtuberImg.naturalWidth > 0) {
        ctx.drawImage(window.vtuberImg, -radius, -radius, radius * 2, radius * 2);
    } else {
        // NỀN FALLBACK SIÊU ĐẸP NẾU ẢNH LỖI (Không bao giờ bị trống không)
        ctx.fillStyle = "#0f172a";
        ctx.fill();
        let grad = ctx.createLinearGradient(-radius, -radius, radius, radius);
        grad.addColorStop(0, "#ff0055"); grad.addColorStop(1, "#00f3ff");
        ctx.fillStyle = grad;
        ...
        ctx.font = "900 45px 'Arial Black'";
        ctx.fillText("MODEL", 0, 45);
    }
    ctx.restore();
...
window.captureFrames = function() {
    ...
    // Kiểm tra xem đã hết trận và bảng Outro hiện lên chưa
    ...
    // ==========================================
    // KHU VỰC VẼ CHUNG TRƯỚC KHI TÁCH NHÁNH (Game -> Lưới 3D)
    // ==========================================
    ...
    if (!isOutroActive) {
        ...
        // Dải màu đỏ cảnh báo (Ngay sát dưới viền Game)
        let bannerY = retainY; // Y = ~607px
        ...
        // Emojis bay lên (Rơi từ dưới đáy lên ngang banner)
        ...
        // --- THANH MÁU HUD (Từ Y = ~690 đến ~850) ---
        ...
        // --- KHUNG LIVE CHAT (Neo từ Y = 900 đến 1400) ---
        if (!window._chatSystemInit) {
            ...
            // Đổ sẵn 7 bình luận lúc mới tải
            for(let i = 0; i < 7; i++) {
        ...
        let boxWidth = 960;
        let boxHeight = 500;
        let boxX = 540; // Giữa màn hình
        let boxY = 1150; // Trọng tâm tại Y = 1150 -> Phạm vi Y: 900 - 1400
        ...
        // --- AVATAR 3D SEXY (Từ Y = 1500 đến 1800) ---
        let vtuberY = 1650; // Neo ở đáy màn hình
        ...
        // LÚC NÀY GAME SẼ ẨN HOÀN TOÀN KHUNG CHAT VÀ AVATAR
        // ==========================================
