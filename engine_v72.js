// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 39.5 [ULTIMATE BRAINROT EDITION]
// [RESTORED] Giữ nguyên UI Emoji, Text Manga, Anti-Glare, Hiệu ứng Râu Trắng, và Meme Edition V38.5.
// [NEW BRAINROT] Combo Rank (W RIZZ, TOUCH GRASS), Đi bộ Jojo (MENACING), Super Saiyan (OVER 9000), Thiên thạch Piano/Đe sắt, và Let Him Cook Parry!
// ==========================================
...
window.spawnMangaSFX = function(x, y, isCrit, customText = null, customColor = null) {
    let sfxList = ['ゴゴゴ', 'ドドド', 'ドム', 'ズバーン', 'バキッ']; let text = customText || sfxList[Math.floor(Math.random() * sfxList.length)]; let size = isCrit ? (65 + Math.random()*20) : (40 + Math.random()*10);
    window.mangaSfx.push({ x: x + (Math.random()*60-30), y: y + (Math.random()*40-20), vx: (Math.random()-0.5)*4, vy: -2 - Math.random()*3, life: 40, maxLife: 40, text: text, size: size, isCrit: isCrit, ang: (Math.random()-0.5)*0.5, color: customColor });
}
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce, attackerDirRight = true) {
    ...
    if (target.state === 'dash' || target.state === 'dash_back') {
        if (target.dashTimer > 2 && target.dashTimer < 14) {
            ...
            let evadeTexts = ["⚡ NOPE.", "⚡ SKILL ISSUE", "⚡ TOO SLOW!", "⚡ MISS ME?"];
            let eText = evadeTexts[Math.floor(Math.random() * evadeTexts.length)];
            window.floatingTexts.push({ x: target.x, y: target.y - 80, text: eText, color: "#00f3ff", alpha: 1.5, vx: (Math.random()-0.5)*2, vy: -5, font: "italic 900 35px 'Arial Black', Impact", life: 50, scale: 2.5, targetScale: 1.0, scaleVel: 0, rot: (Math.random()-0.5)*0.3 });
    ...
    if (target.state === 'block') { 
        if (target.stamina < 15) {
            ...
            window.floatingTexts.push({ x: target.x, y: target.y - 90, text: "💔 EMOTIONAL DAMAGE!", color: "#ff003c", alpha: 1.5, vx: 0, vy: -4, font: "italic 900 35px 'Arial Black', Impact", life: 70, scale: 3.0, targetScale: 1.2, scaleVel: 0, rot: (Math.random()-0.5)*0.4 });
            ...
            // [NEW BRAINROT] PARRY GOD -> LET HIM COOK
            window.floatingTexts.push({ x: target.x, y: target.y - 120, text: "👨‍🍳 LET HIM COOK!", color: "#f1c40f", alpha: 1.5, vx: 0, vy: -3, font: "italic 900 40px 'Arial Black'", life: 60, scale: 3.0, targetScale: 1.0, scaleVel: 0, rot: (Math.random()-0.5)*0.2 });
    ...
    if (finalDmg > 0) {
        ...
        if (!isCrit && Math.random() < 0.1) {
            dmgText = "BONK!";
            window.playSound(800, 'square', 0.1, 0.5, true); 
        }
        ...
        if (isCrit) {
            window.screenFlash = 0.15; 
            window.impactFrameCount = 1; 
        ...
        if (target.hp <= 0) {
            ...
            if (isNearWall && isCrit && typeof window.triggerStageTransition === 'function') { window.triggerStageTransition(target); } 
                ...
                if (isCrit) {
                    ...
                    if (Math.random() > 0.5) {
                        target.vy = -60; 
                        window.floatingTexts.push({ x: target.x, y: target.y - 120, text: "🚀 YEETED!", color: "#fff", alpha: 1.5, vx: 0, vy: -2, font: "italic 900 65px 'Arial Black', Impact", life: 100, scale: 4.0, targetScale: 1.2, scaleVel: 0, rot: (Math.random()-0.5)*0.2 });
                    } else {
                        window.floatingTexts.push({ x: target.x, y: target.y - 120, text: "💀 LETHAL FINISH!", color: "#ff003c", alpha: 1.5, vx: 0, vy: -2, font: "italic 900 65px 'Arial Black', Impact", life: 100, scale: 4.0, targetScale: 1.2, scaleVel: 0, rot: (Math.random()-0.5)*0.2 });
                    }
                ...
                window.impactFrameTimer = 0; window.chromaTimer = 40; window.playSound(80, 'square', 1.5, 0.8, true); window.koGlitchTimer = 60; target.state = 'ko_falling'; target.koTimer = 100; if(target.vy > -10) target.vy = -12; target.onGround = false;
            }
...
window.update = function() {
    ...
    if (!window.gameOver && !isTimeStopped) {
        ...
        // [NEW BRAINROT] CARTOON DROP MÀ CÓ THẬT
        let meteorChance = 0.002 + (window.matchTimer / 3600) * 0.01; 
        if (Math.random() < meteorChance && window.projectiles.length < 5) { 
            let items = ["🎹", "🗿", "📺", "💣"];
            let item = items[Math.floor(Math.random()*items.length)];
            window.projectiles.push({ x: Math.random() * window.canvas.width, y: -100, vx: 0, vy: 12 + Math.random() * 5, radius: 35, color: "#e67e22", dmg: 69, target: null, isCartoonDrop: true, emoji: item }); 
        }
    ...
    if (!isTimeStopped) {
        for (let i = window.projectiles.length - 1; i >= 0; i--) {
            ...
            // [NEW BRAINROT] CARTOON DROP COLLISION
            if (proj.isCartoonDrop) {
                if (proj.y >= window.GROUND_Y) {
                    window.shakeScreen(20, 10); window.playSound(200, 'sawtooth', 0.5, 0.6, true);
                    window.floatingTexts.push({x: proj.x, y: window.GROUND_Y - 50, text: "LMAO", color: "#fff", alpha: 1.5, vx: 0, vy: -5, font: "900 40px Impact", life: 50, scale: 2.0, targetScale: 1.0, scaleVel: 0, rot: (Math.random()-0.5)*0.5});
                    if (window.shockwaves.length < MAX_SHOCKWAVES) window.shockwaves.push({x: proj.x, y: window.GROUND_Y, r: 10, maxR: 250, color: "#fff", alpha: 1, speed: 15});
                    let allActiveFighters = [window.p1].concat(window.enemies);
                    allActiveFighters.forEach(fighter => { if (fighter && fighter.hp > 0 && Math.abs(fighter.x - proj.x) < 120 && fighter.y >= window.GROUND_Y - 50) { if(typeof window.takeDamage === 'function') window.takeDamage(fighter, proj.dmg, "#fff", true, false, true); fighter.vx = Math.sign(fighter.x - proj.x) * 15; fighter.state = 'hurt'; fighter.hitStun = 40; } }); 
                    window.projectiles.splice(i, 1);
                }
            }
            else if (proj.isMeteor) {
    ...
    if (!isTimeStopped) {
        window.enemies.forEach(e => { 
            if (e.hp <= 0 && !e.deathTriggered) {
                ...
                let emojis = ["🍿", "🎉", "🌭", "😂", "💀", "🍔", "🤡"];
                for(let c=0; c<12; c++) {
                    window.customObjs.push({
                        x: e.x + (Math.random()-0.5)*40, y: e.y - 20,
                        vx: (Math.random()-0.5)*15, vy: -Math.random()*15 - 5,
                        text: emojis[Math.floor(Math.random()*emojis.length)],
                        color: "#fff", font: "30px Arial", life: 100, maxLife: 100, spin: true, ang: 0
                    });
                }
    ...
    allFighters.forEach(f => {
        ...
        if (f.state === 'idle' && !isTimeStopped) { 
            ...
            if (Math.random() < 0.01 && f.hp > 0 && f.hp < f.maxHp && window.matchTimer % 30 === 0) {
                window.customObjs.push({ x: f.x, y: f.y - 50, vx: 0, vy: -2, text: "🍔", color: "#fff", font: "30px Arial", life: 30, maxLife: 30, spin: false, ang: 0 });
                f.hp = Math.min(f.maxHp, f.hp + 2);
                window.floatingTexts.push({ x: f.x, y: f.y - 80, text: "+2 NOM!", color: "#2ecc71", alpha: 1.5, vx: 0, vy: -3, font: "900 20px Arial", life: 30, scale: 1.5, targetScale: 1.0, scaleVel: 0, rot: 0 });
                window.playSound(400, 'square', 0.1, 0.2);
            }
        }
        
        // [NEW BRAINROT] DÁNG ĐI JOJO (MENACING)
        if (f.state === 'walk' && Math.random() < 0.15 && window.matchTimer % 6 === 0 && f.hp > 0) {
            window.mangaSfx.push({ x: f.x + (Math.random()*40-20), y: f.y - 80 + (Math.random()*20-10), vx: 0, vy: -1.5, life: 30, maxLife: 30, text: "ゴ", size: 35, isCrit: false, ang: (Math.random()-0.5)*0.2, color: "#8e44ad" });
        }
        ...
        if (f.isRage) { 
            ...
            // [NEW BRAINROT] HIS POWER LEVEL IS OVER 9000!!!
            if (!f.powerLevelTriggered) {
                f.powerLevelTriggered = true;
                window.playSound(400, 'sawtooth', 1.0, 1.0, true); window.shakeScreen(30, 15);
                window.floatingTexts.push({ x: f.x, y: f.y - 120, text: "OVER 9000!!!", color: "#f1c40f", alpha: 2.0, vx: 0, vy: -4, font: "italic 900 50px Impact", life: 80, scale: 3.5, targetScale: 1.0, scaleVel: 0, rot: 0 });
                if (window.shockwaves.length < MAX_SHOCKWAVES) window.shockwaves.push({x: f.x, y: window.GROUND_Y, r: 10, maxR: 600, color: "#f1c40f", alpha: 1, speed: 20});
            }
            
            // Ép màu Aura thành Super Saiyan
            let auraColor = "#f1c40f"; 
            if(window.particles.length < MAX_PARTICLES) { window.particles.push({ x: f.x + (Math.random() - 0.5) * 50, y: window.GROUND_Y, vx: (Math.random() - 0.5) * 2, vy: -Math.random() * 10 - 5, life: 40, maxLife: 40, color: auraColor, size: Math.random() * 10 + 5, isAuraFlame: true }); }
            ...
            if (Math.random() < 0.05) window.spawnLightning(f.x, f.y - 40, auraColor, 20, 3);
        }
...
window.draw = function() {
    ...
    if (window.isLoading) {
        ...
        window.ctx.fillStyle = `hsl(${tConf.hue}, 90%, 25%)`;
        window.ctx.fillRect(0, 0, window.canvas.width, window.canvas.height);
        
        window.ctx.save();
        window.ctx.translate(cx, cy);
        window.ctx.rotate(Date.now() / 2000);
        window.ctx.fillStyle = `hsl(${tConf.hue + 30}, 100%, 50%)`;
        window.ctx.globalAlpha = 0.4;
        for (let i = 0; i < tConf.rayCount; i++) {
            window.ctx.beginPath();
            window.ctx.moveTo(0, 0);
            let angle1 = (Math.PI * 2 / tConf.rayCount) * i;
            let angle2 = (Math.PI * 2 / tConf.rayCount) * (i + 0.5);
            window.ctx.lineTo(Math.cos(angle1) * 2000, Math.sin(angle1) * 2000);
            window.ctx.lineTo(Math.cos(angle2) * 2000, Math.sin(angle2) * 2000);
            window.ctx.fill();
        }
        window.ctx.restore();

        window.ctx.save();
        window.ctx.translate(cx, cy - 50 + Math.sin(Date.now()/150)*20);
        window.ctx.rotate(Math.sin(Date.now()/200)*0.2);
        window.ctx.font = "250px Arial";
        window.ctx.textAlign = "center";
        window.ctx.textBaseline = "middle";
        window.ctx.shadowBlur = 50; window.ctx.shadowColor = "#000";
        window.ctx.fillText(tConf.emoji, 0, 0);
        window.ctx.restore();

        window.ctx.font = "italic 900 85px 'Arial Black', Impact";
        window.ctx.textAlign = "center";
        window.ctx.lineWidth = 15; window.ctx.strokeStyle = "#000";
        window.ctx.strokeText(tConf.title, cx, cy + 150);
        window.ctx.fillStyle = "#fff";
        window.ctx.fillText(tConf.title, cx, cy + 150);

        window.ctx.shadowBlur = 0; window.ctx.strokeStyle = "#fff"; window.ctx.lineWidth = 6; 
        window.ctx.strokeRect(cx - 250, window.canvas.height - 80, 500, 24);
        window.ctx.fillStyle = "#ff4757"; 
        window.ctx.fillRect(cx - 247, window.canvas.height - 77, (window.loadProgress / 100) * 494, 18);
        window.ctx.fillStyle = "#fff"; window.ctx.font = "bold 20px monospace"; 
        window.ctx.fillText(`RENDERING MASTERPIECE... ${Math.floor(window.loadProgress)}%`, cx, window.canvas.height - 20); 
        return; 
    }
    ...
    try {
        ...
        window.projectiles.forEach(proj => { 
            let pr = project3D(proj); 
            if (proj.isCartoonDrop) {
                window.ctx.save(); window.ctx.translate(pr.drawX, pr.drawY);
                window.ctx.font = `${60 * pr.drawScale}px Arial`; window.ctx.textAlign = "center"; window.ctx.textBaseline = "middle";
                window.ctx.fillText(proj.emoji, 0, 0);
                window.ctx.restore();
            } else {
                window.ctx.beginPath(); window.ctx.arc(pr.drawX, pr.drawY, proj.radius * pr.drawScale, 0, Math.PI * 2); window.ctx.fillStyle = proj.color; window.ctx.shadowBlur = 15; window.ctx.shadowColor = proj.color; window.ctx.fill(); if(proj.isMeteor) { window.ctx.beginPath(); window.ctx.arc(pr.drawX, pr.drawY, (proj.radius + 10) * pr.drawScale, 0, Math.PI * 2); window.ctx.fillStyle = "rgba(230, 126, 34, 0.4)"; window.ctx.fill(); } window.ctx.shadowBlur = 0; 
            }
        });
        ...
        if (window.dimensionCracks && window.dimensionCracks.length > 0) {
            ...
            window.dimensionCracks.forEach(c => {
                ...
                const drawCrackPath = (path, width) => {
                    ...
                    window.ctx.beginPath();
                    let p0 = project3D(path[0]);
                    window.ctx.moveTo(p0.drawX, p0.drawY);
                    for (let i = 1; i < path.length; i++) {
                        let pi = project3D(path[i]);
                        window.ctx.lineTo(pi.drawX, pi.drawY);
                    }
                    window.ctx.lineWidth = width * alpha * p0.drawScale;
                    window.ctx.stroke();
                };
                ...
                window.ctx.strokeStyle = "rgba(220, 255, 255, 0.5)"; 
                drawCrackPath(c.main, 12);
                c.branches.forEach(b => drawCrackPath(b, 5));

                window.ctx.shadowBlur = 0;
                window.ctx.strokeStyle = "#ffffff";
                drawCrackPath(c.main, 3);
                c.branches.forEach(b => drawCrackPath(b, 1.5));
            });
            window.ctx.restore();
        }
        ...
        if (window.mangaSfx) {
            window.mangaSfx.forEach(sfx => {
                ...
                window.ctx.font = `900 italic ${sfx.size}px Impact, Arial Black, sans-serif`; window.ctx.lineWidth = 4; window.ctx.strokeStyle = "#000"; window.ctx.strokeText(sfx.text, 0, 0); window.ctx.fillStyle = sfx.color || (sfx.isCrit ? "#ff003c" : "#ffffff"); window.ctx.fillText(sfx.text, 0, 0); window.ctx.restore();
            });
        }
        ...
        // [NEW BRAINROT] CẬP NHẬT RANK COMBO MEME
        let renderComboRank = function(fighter, xPos, align) {
            if (fighter && fighter.comboHits >= 2) {
                let alpha = Math.max(0, fighter.comboAlpha || 1); let hits = fighter.comboHits; let rank = "BRUH"; let rankColor = "#bdc3c7"; let rankSize = 40; let rankGlow = 10;
                if (hits >= 4) { rank = "MID"; rankColor = "#2ecc71"; rankSize = 45; } 
                if (hits >= 6) { rank = "OKAY"; rankColor = "#3498db"; rankSize = 50; } 
                if (hits >= 8) { rank = "SHEESH"; rankColor = "#9b59b6"; rankSize = 55; rankGlow = 15; } 
                if (hits >= 12) { rank = "W RIZZ"; rankColor = "#f1c40f"; rankSize = 65; rankGlow = 20; } 
                if (hits >= 16) { rank = "TOUCH GRASS"; rankColor = "#e67e22"; rankSize = 75; rankGlow = 25; } 
                if (hits >= 20) { rank = "MOM GET THE CAMERA!"; rankColor = "#ff003c"; rankSize = 90 + Math.sin(Date.now()/50)*10; rankGlow = 35; }
