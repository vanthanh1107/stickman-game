// ==========================================
// RECORDER.JS - V69.6 PROCEDURAL MASTERPIECE (INFINITE VARIATIONS)
// ĐÃ FIX: Tạo sinh ngẫu nhiên 100% Intro (Màu sắc, Hiệu ứng nổ, Hạt, Góc cắt).
// CẢM XÚC: Cinematic, rung giật theo nhịp tim, không video nào giống video nào.
...
window.introDuration = 2500; 
window._sfxFlags = { s1: false, s2: false, s3: false };
window.introParams = null; // Chứa kịch bản Intro ngẫu nhiên
...
// ====================================================================
// PROCEDURAL ENGINE (TẠO INTRO NGẪU NHIÊN)
// ====================================================================
const THEMES = [
    { id: "fire", c1: "#4a0000", c2: "#0a0000", aura: "#ff003c", line: "rgba(255, 0, 60, 0.4)", text: "#ff003c" },
    { id: "ice", c1: "#002b4a", c2: "#000a12", aura: "#00f3ff", line: "rgba(0, 243, 255, 0.4)", text: "#00f3ff" },
    { id: "toxic", c1: "#0a3a00", c2: "#001200", aura: "#39ff14", line: "rgba(57, 255, 20, 0.4)", text: "#39ff14" },
    { id: "void", c1: "#2a004a", c2: "#080012", aura: "#b100ff", line: "rgba(177, 0, 255, 0.4)", text: "#b100ff" },
    { id: "gold", c1: "#4a3500", c2: "#120a00", aura: "#ffb800", line: "rgba(255, 184, 0, 0.4)", text: "#ffb800" },
    { id: "blood", c1: "#3a0000", c2: "#000000", aura: "#8a0303", line: "rgba(138, 3, 3, 0.4)", text: "#ff0000" }
];

window.generateIntroParams = function() {
    const r = (arr) => arr[Math.floor(Math.random() * arr.length)];
    // Random 2 hệ màu không trùng nhau
    let t1 = r(THEMES); let t2 = r(THEMES.filter(t => t.id !== t1.id));
    return {
        themeP1: t1, themeP2: t2,
        splitType: r(["diagonal_1", "diagonal_2", "horizontal"]),
        particleType: r(["speedlines", "embers", "glitch_blocks"]),
        clashEffect: r(["flash", "void", "invert", "blood"]),
...
window.bakeThumbnailsForVideo = function(titleText) {
    ...
    try {
        ...
        let prm = window.introParams; // Dùng màu đã sinh ngẫu nhiên làm nền Thumb
        ...
        ctxV.fillStyle = prm.themeP2.text; ctxV.fillText("-999,999 💢", 0,0); ctxV.fillStyle = "#fff"; ctxV.fillText("-999,999 💢", -4,-4); ctxV.restore();
        ...
        lines.forEach((line, index) => {
            ...
            for(let d=15; d>0; d--) { ctxV.strokeText(line, d, yOffset + d); ctxV.fillStyle = prm.themeP1.c1; ctxV.fillText(line, d, yOffset + d); }
            ctxV.strokeText(line, 0, yOffset); ctxV.fillStyle = prm.themeP1.aura; ctxV.fillText(line, 0, yOffset); ctxV.fillStyle = "#ffffff"; ctxV.fillText(line, -3, yOffset - 3);
        }); ctxV.restore();
    } catch (e) { console.error("Lỗi Bake Thumbnail:", e); }
};

// ====================================================================
// PROCEDURAL EPIC INTRO (CHẠY DỰA TRÊN THÔNG SỐ ĐÃ RANDOM)
// ====================================================================
window.drawProceduralIntro = function(ctx, w, h, progress) {
    ...
    // 1. PHASE CẢNH BÁO (0.0 -> 0.15)
    if (progress < 0.15) {
        ctx.fillStyle = "#0a0a0a"; ctx.fillRect(0, 0, w, h);
        let shake = (Math.random() - 0.5) * 50; ctx.translate(w/2 + shake, h/2 + shake);
        ctx.textAlign = "center"; ctx.textBaseline = "middle";
        ctx.fillStyle = prm.themeP2.aura; ctx.font = "italic 900 90px Impact";
        ctx.fillText(prm.warningText, 0, -100);
        ctx.fillStyle = "#fff"; ctx.font = "900 65px 'Arial Black'"; ctx.fillText(window.bannerText1, 0, 50);
        for(let i=0; i<15; i++) {
    ...
    // TÍNH TỌA ĐỘ SPLIT (Chẻ ngẫu nhiên)
    let pCut = Math.min(1, (progress - 0.15) / 0.5); 
    let easeCut = 1 - Math.pow(1 - pCut, 3);
    let sTopLeft, sTopRight, sBotLeft, sBotRight;
    
    if (prm.splitType === "diagonal_1") { // Dấu sắc ( / )
        sTopLeft = h*0.35 + (easeCut * h*0.15); sTopRight = h*0.65 - (easeCut * h*0.15);
        sBotLeft = sTopLeft; sBotRight = sTopRight;
    } else if (prm.splitType === "diagonal_2") { // Dấu huyền ( \ )
        sTopLeft = h*0.65 - (easeCut * h*0.15); sTopRight = h*0.35 + (easeCut * h*0.15);
        sBotLeft = sTopLeft; sBotRight = sTopRight;
    } else { // Ngang ( - )
        sTopLeft = h*0.5; sTopRight = h*0.5;
        sBotLeft = h*0.5; sBotRight = h*0.5;
    }

    let rumble = 0; if (progress > 0.6) rumble = (Math.random() - 0.5) * 50 * ((progress - 0.6) * 3);
    ctx.translate(rumble, rumble);

    // HÀM VẼ HIỆU ỨNG HẠT NGẪU NHIÊN
    const drawParticles = (color, reverse) => {
        ctx.save(); ctx.fillStyle = color;
        if (prm.particleType === "speedlines") {
            for(let i=0; i<40; i++) { ctx.rotate(Math.PI / 20); let speed = (Date.now() * 3) % 1000; if(reverse) speed = 1000 - speed; ctx.fillRect(150 + speed, -10, 1500, 10 + Math.random()*20); }
        } else if (prm.particleType === "embers") {
            for(let i=0; i<50; i++) {
                let x = (Math.sin(i*11) * w) - w/2;
                let y = h - ((Date.now()*0.3 + i*50) % h); if(reverse) y = h-y;
                ctx.beginPath(); ctx.arc(x, y, 2 + Math.random()*8, 0, Math.PI*2); ctx.fill();
            }
        } else if (prm.particleType === "glitch_blocks") {
            for(let i=0; i<20; i++) {
                let x = (Math.sin(i*33) * w) - w/2; let y = (Math.cos(i*77) * h) - h/2;
                if(Math.random() > 0.8) ctx.fillRect(x, y, 50+Math.random()*150, 10+Math.random()*20);
            }
        }
        ctx.restore();
    };

    const drawCharMega = (charObj, cx, cy, isP1, theme) => {
        if(!charObj) return; ctx.save(); ctx.translate(cx, cy); if(!isP1) ctx.scale(-1, 1);
        ...
        ctx.shadowBlur = 60; ctx.shadowColor = theme.aura; ctx.scale(1.15, 1.15);
    ...
    // --- NỬA TRÊN: BOSS ---
    ctx.save();
    ctx.beginPath(); ctx.moveTo(0,0); ctx.lineTo(w,0); ctx.lineTo(w, sTopRight); ctx.lineTo(0, sTopLeft); ctx.clip();
    let gradBoss = ctx.createRadialGradient(w/2, h*0.2, 0, w/2, h*0.2, w);
    gradBoss.addColorStop(0, prm.themeP2.c1); gradBoss.addColorStop(1, prm.themeP2.c2);
    ctx.fillStyle = gradBoss; ctx.fillRect(0,0,w,h);
    ctx.save(); ctx.translate(w/2, h*0.25); drawParticles(prm.themeP2.line, false); ctx.restore();
    
    let bossX = w*0.75 + (1-easeCut)*w*0.5; let bossY = h*0.28;
    if(progress > 0.65) { let dash = (progress - 0.65)/0.2; bossX -= dash * w*0.25; bossY += dash * h*0.08; }
    drawCharMega(e1, bossX, bossY, false, prm.themeP2);
    ctx.restore();

    // --- NỬA DƯỚI: PLAYER ---
    ctx.save();
    ctx.beginPath(); ctx.moveTo(0,h); ctx.lineTo(w,h); ctx.lineTo(w, sBotRight); ctx.lineTo(0, sBotLeft); ctx.clip();
    let gradP1 = ctx.createRadialGradient(w/2, h*0.8, 0, w/2, h*0.8, w);
    gradP1.addColorStop(0, prm.themeP1.c1); gradP1.addColorStop(1, prm.themeP1.c2);
    ctx.fillStyle = gradP1; ctx.fillRect(0,0,w,h);
    ctx.save(); ctx.translate(w/2, h*0.75); drawParticles(prm.themeP1.line, true); ctx.restore();
    
    let p1X = w*0.25 - (1-easeCut)*w*0.5; let p1Y = h*0.72;
    if(progress > 0.65) { let dash = (progress - 0.65)/0.2; p1X += dash * w*0.25; p1Y -= dash * h*0.08; }
    drawCharMega(window.p1, p1X, p1Y, true, prm.themeP1);
    ctx.restore();

    // --- ĐƯỜNG CẮT (THE SPLIT LINE) ---
    ctx.beginPath(); ctx.moveTo(0, sTopLeft); ctx.lineTo(w, sTopRight);
    ctx.lineWidth = 15; ctx.strokeStyle = "#fff"; ctx.shadowBlur = 30; ctx.shadowColor = "#fff"; ctx.stroke(); ctx.shadowBlur = 0;

    // --- IMPACT FRAMES SAKUGA (0.80 - 0.85) ---
    if (progress > 0.80 && progress < 0.85) {
        if (Math.floor(progress * 150) % 2 === 0) {
            ctx.fillStyle = "#ffffff"; ctx.globalCompositeOperation = "difference"; ctx.fillRect(0,0,w,h); ctx.globalCompositeOperation = "source-over";
        }
    }

    // --- TEXT CẢM XÚC CHỬI NHAU ---
    if (progress > 0.25 && progress < 0.85) {
        let tScale = 1 + Math.sin(progress * 40) * 0.05; 
        ctx.textAlign = "center"; ctx.textBaseline = "middle";
        const renderSlamText = (text, tx, ty, color, rot) => {
            ...
            ctx.strokeText(text, 0,0); ctx.shadowColor = "transparent"; 
            ctx.fillStyle = color; ctx.fillText(text, 0,0); ctx.fillStyle = "#fff"; ctx.fillText(text, -3,-3);
            ctx.restore();
        };
        // Căn vị trí Text tùy theo góc cắt để không lấn sang nửa kia
        let tyP2 = prm.splitType === "diagonal_2" ? h*0.2 : h*0.13;
        let tyP1 = prm.splitType === "diagonal_2" ? h*0.8 : h*0.87;
        renderSlamText(window.trashTalkP2, w*0.65, tyP2, prm.themeP2.text, -0.05); 
        renderSlamText(window.trashTalkP1, w*0.35, tyP1, prm.themeP1.text, 0.05);  
    }

    // --- BÙM! CHUYỂN CẢNH ĐA DẠNG DỰA TRÊN CLASH EFFECT ---
    if (progress >= 0.85) {
        let flashP = (progress - 0.85) / 0.15;
        let overlayColor = "rgba(255,255,255,1)"; // flash
        let vsColor = prm.themeP1.aura;
        
        if (prm.clashEffect === "void") { overlayColor = `rgba(0,0,0,${1 - flashP})`; vsColor = "#fff"; }
        else if (prm.clashEffect === "blood") { overlayColor = `rgba(138,3,3,${1 - flashP})`; vsColor = "#000"; }
        else if (prm.clashEffect === "invert") { ctx.globalCompositeOperation = "difference"; overlayColor = `rgba(255,255,255,${1 - flashP})`; }
        else { overlayColor = `rgba(255,255,255,${1 - flashP})`; } // default flash

        ctx.fillStyle = overlayColor; ctx.fillRect(0,0,w,h); ctx.globalCompositeOperation = "source-over";
        
        ctx.save(); ctx.translate(w/2, h/2); let pop = 1 + (flashP * 6); ctx.scale(pop, pop);
        ctx.font = "italic 900 200px Impact"; ctx.textAlign = "center"; ctx.textBaseline = "middle";
        ctx.lineWidth = 35; ctx.strokeStyle = "#000"; ctx.strokeText("VS", 0,0); ctx.fillStyle = vsColor; ctx.fillText("VS", 0,0);
        ctx.restore();
    }
...
window.startRecording = function() {
    ...
    // TẠO KỊCH BẢN INTRO NGẪU NHIÊN CHO LẦN QUAY NÀY
    ...
    window.introStartTime = Date.now(); window._sfxFlags = { s1: false, s2: false, s3: false };
