// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 11.0 [120FPS UNLOCKED PRO]
// [KỸ THUẬT: FIXED TIMESTEP 60HZ PHYSICS + UNLOCKED 120HZ+ RENDERING]
// [TÍNH NĂNG: LOADING SCREEN, FLASH STEP, GUARD BREAK, AWAKENING, SIN CITY]
// ==========================================
...
window.initGameEngine = function() {
    ...
    let loadInterval = setInterval(() => {
        ...
        if (window.loadProgress >= 100) {
            ...
            setTimeout(() => { window.isLoading = false; window.playSound(800, 'sine', 0.5, 0.5); }, 500);
        }
    }, 40);
};
...
window.playSound = function(freq, type, duration, vol, isImpact = false) { 
    ...
    try {
        ...
        if (isImpact) { 
            ...
            snap.connect(snapGain); snapGain.connect(window.audioCtx.destination); if (window.isRecording && window.recordAudioDestination) { snapGain.connect(window.recordAudioDestination); } snap.start(t); snap.stop(t + 0.05);
        } else { 
...
window.spawnDust = function(x, y) { 
    if (window.particles.length > MAX_PARTICLES) return;
    for(let i=0; i<6; i++) { window.particles.push({ x: x + (Math.random()*30-15), y: y, vx: (Math.random()-0.5)*5, vy: -Math.random()*3 - 0.5, life: 20, maxLife: 20, color: "rgba(189, 195, 199, 0.4)", size: Math.random() * 8 + 5 }); } 
}

window.triggerCinematic = function(caster, callback) { window.cinematicTimer = 50; window.cinematicCaster = caster; window.cinematicCallback = callback; window.targetZoom = 1.15; window.playSound(400, 'sine', 0.4, 0.2, false); window.speedLinesAlpha = 1.0; }
...
window.triggerTimeStop = function(frames, caster) { 
    window.timeStopTimer = frames; window.timeStopCaster = caster; window.applyFilter('invert', frames); 
...
window.spawnEnvDamage = function(x, y, type, scale, isBurning = false) {
    ...
    for(let i=0; i<numCracks; i++) {
        ...
        for(let s = 1; s <= segments; s++) {
            ...
            if (Math.random() < 0.4 && s < segments) {
                let branchAngle = currAngle + (Math.random() > 0.5 ? 1 : -1) * (0.5 + Math.random() * 0.5); let bLen = segLen * (0.6 + Math.random() * 0.6);
                cracks.push([{x: currX, y: currY}, {x: currX + Math.cos(branchAngle) * bLen, y: currY + Math.sin(branchAngle) * bLen}]);
            }
        }
        cracks.push(path);
    }
    ...
    if (window.particles.length < MAX_PARTICLES) {
        for(let d=0; d < 8 * scale; d++) { window.particles.push({ x: x + (Math.random()-0.5)*30, y: y + (Math.random()-0.5)*20, vx: (Math.random()-0.5)*10, vy: -Math.random()*10 - 4, life: 40, maxLife: 40, color: isBurning ? "#e74c3c" : "#57606f", size: Math.random()*4 + 2, isRubble: true }); }
    }
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce) {
    ...
    if (target.state === 'dash' || target.state === 'dash_back') {
        if (target.dashTimer > 2 && target.dashTimer < 12) {
            window.playSound(800, 'sine', 0.5, 1.0, true); window.applyFilter('invert', 5); window.slowMoTimer = 20; window.speedLinesAlpha = 1.0; 
            window.floatingTexts.push({ x: target.x, y: target.y - 80, text: "⚡ FLASH STEP!", color: "#f1c40f", alpha: 1, vx: 0, vy: -4, font: "900 35px Arial", life: 50 });
            window.spawnParticles(target.x, target.y - 40, "#f1c40f", true);
            if (window.shockwaves.length < MAX_SHOCKWAVES) window.shockwaves.push({x: target.x, y: target.y - 40, r: 5, maxR: 300, color: "#f1c40f", alpha: 1, speed: 20});
            target.stamina = Math.min(100, target.stamina + 100); target.x += target.isFacingRight ? -100 : 100; target.iFrames = 20; return; 
        }
    }
    ...
    if (target.state === 'block') { 
        if (target.stamina < 15) {
            ...
            allFighters.forEach(e => { if (e !== target && e.hp > 0 && Math.abs(e.x - target.x) < 140) { e.state = 'hurt'; e.hitStun = 50; e.vx = target.isFacingRight ? 10 : -10; window.spawnDust(e.x, e.y); } });
            return;
        } else { target.stamina -= finalDmg * 0.5; finalDmg *= 0.2; }
    }
    
    if (target.shield > 0) {
        target.shield -= finalDmg; if (target.shield < 0) { finalDmg = -target.shield; target.shield = 0; } else { finalDmg = 0; }
    ...
    if (finalDmg > 0) {
        ...
        if (target.superArmor <= 0 && target.state !== 'stunned') { target.state = 'hurt'; target.hitStun = isCrit ? 20 : 12; target.attackTimer = 0; target.comboStep = 0; }
        ...
        } else if (isCrit) { window.impactFrameTimer = 2; window.hitStopFrames = 2; window.shakeScreen(10, 8); window.targetZoom = 1.1; window.chromaTimer = 10; window.playSound(180, 'square', 0.3, 0.6, true);
        } else { window.hitStopFrames = 0; window.shakeScreen(3, 3); window.playSound(250, 'sine', 0.15, 0.3, true); }
    }
};
...
window.attack = function(attacker, targetGroup) {
    ...
    if (attacker.comboStep >= 4 || Math.random() < 0.15) { selectedMove = moves_Finisher[Math.floor(Math.random() * moves_Finisher.length)]; isFinisher = true; attacker.comboStep = 0; } 
    ...
    if (isFinisher) {
        isCrit = true; finalDmg = baseDmg * 3.5; window.shakeScreen(15, 12); target.vx = (attacker.isFacingRight ? 5 : -5); target.state = 'hurt'; target.hitStun = 45; window.spawnParticles(target.x, target.y - 40, "#ff4757", true); window.floatingTexts.push({ x: target.x, y: target.y - 80, text: "💥", color: "#ff4757", alpha: 1, vx: (Math.random()-0.5)*2, vy: -4, font: "900 45px Arial", life: 50 });
    } else {
...
// ==========================================
// 5. VÒNG LẶP CORE PHYSICS UPDATE (KHÓA CHẶT 60 HZ ĐỂ KHÔNG HỎNG GAME)
// ==========================================
window.update = function() {
    ...
    // CHUYỂN TẤT CẢ BIẾN THỜI GIAN CỦA HIỆU ỨNG VÀO ĐÂY (TRÁNH LỖI MẤT HÌNH NHANH Ở 120FPS)
    ...
    if (!isTimeStopped) {
        ...
        for (let i = window.particles.length - 1; i >= 0; i--) { 
            ...
            else if (pt.isGlass) { pt.vy += window.GRAVITY * 0.7; if (pt.y > window.GROUND_Y) { pt.y = window.GROUND_Y; pt.vy *= -0.6; pt.vx *= 0.5; } } 
            ...
            for (let j = i + 1; j < allFighters.length; j++) { 
                ...
                if (dist < 80 && f1.attackTimer > 5 && f1.attackTimer < 20 && f2.attackTimer > 5 && f2.attackTimer < 20) {
                    if (!f1.clashCooldown && !f2.clashCooldown && f1.isFacingRight !== f2.isFacingRight) {
                        ...
                        let midX = (f1.x + f2.x)/2; let midY = (f1.y + f2.y)/2 - 30; window.spawnParticles(midX, midY, "#ffffff", true);
    ...
    allFighters.forEach(f => {
        ...
        if (f.x < wallBound) { 
            ...
            } else if (f.hitStun > 0 && f.vx < -3 && !f.wallBounced && f.state !== 'wall_splat') { f.wallBounced = true; f.vx = 4; f.hitStun = 15; window.shakeScreen(10, 4); if (!f.wallDamageCooldown) { window.spawnEnvDamage(wallBound, f.y, 'wall_left', f.scale || 1, false); f.wallDamageCooldown = 60; } if(typeof window.takeDamage === 'function') window.takeDamage(f, Math.floor(Math.random() * 4) + 4, "#fff", false, true); window.playSound(100, 'sine', 0.2, 0.3, true); window.spawnDust(f.x, f.y); } else if(f.state !== 'walk' && f.state !== 'dash_back' && f.state !== 'wall_splat') f.vx = 0;
        }
        ...
        for (let i = window.projectiles.length - 1; i >= 0; i--) { 
            ...
            if (proj.isMeteor) {
                ...
                if (proj.y >= window.GROUND_Y) {
                    ...
                    if (window.shockwaves.length < MAX_SHOCKWAVES) window.shockwaves.push({x: proj.x, y: window.GROUND_Y, r: 10, maxR: 150, color: "#e74c3c", alpha: 1, speed: 10}); window.spawnEnvDamage(proj.x, window.GROUND_Y, 'crater', 1.5, true);
...
window.draw = function() {
    ...
    if (window.isLoading) {
        ...
        window.ctx.fillStyle = "#00f3ff"; window.ctx.font = "italic 900 40px Arial"; window.ctx.textAlign = "center"; window.ctx.shadowBlur = 15; window.ctx.shadowColor = "#00f3ff"; window.ctx.fillText("INITIALIZING ENGINE 11.0...", cx, cy - 50);
    ...
    try {
        ...
        if (t2 === "flowing_water" || t2 === "flowing_lava") {
            ...
            const drawWaveLayer = (color, amplitude, frequency, phase, offsetY) => {
                ...
                for (let x = startX; x <= endX; x += 40) { let y = waterBaseY + offsetY + Math.sin((x * frequency) + waveSpeed + phase) * amplitude; window.ctx.lineTo(x, y); } window.ctx.lineTo(endX, window.canvas.height + 400); window.ctx.fill();
            };
        ...
        if (window.envDamage && window.envDamage.length > 0) {
            ...
            window.envDamage.forEach(dmg => {
                ...
                if (dmg.type === 'crater') {
                    ...
                    if (dmg.isBurning) { let pulse = 0.5 + Math.abs(Math.sin(Date.now() / 150)) * 0.5; let burnGrad = window.ctx.createRadialGradient(0, 0, 0, 0, 0, (dmg.radius || 40) * 1.5); burnGrad.addColorStop(0, `rgba(255, 150, 0, ${pulse * 0.9 * alpha})`); burnGrad.addColorStop(0.3, `rgba(255, 30, 0, ${pulse * 0.5 * alpha})`); burnGrad.addColorStop(1, "rgba(0,0,0,0)"); window.ctx.fillStyle = burnGrad; window.ctx.beginPath(); window.ctx.arc(0, 0, (dmg.radius || 40) * 1.5, 0, Math.PI * 2); window.ctx.fill(); } else { window.ctx.fillStyle = `rgba(15, 15, 15, ${0.8 * alpha})`; window.ctx.beginPath(); window.ctx.arc(0, 0, (dmg.radius || 40), 0, Math.PI * 2); window.ctx.fill(); }
                    window.ctx.restore();
                }
                ...
                dmg.cracks.forEach(path => {
                    if (Array.isArray(path) && path.length > 0) {
                        ...
                        window.ctx.beginPath(); path.forEach((pt, idx) => { if (idx === 0) window.ctx.moveTo(pt.x, pt.y); else window.ctx.lineTo(pt.x, pt.y); }); window.ctx.stroke(); window.ctx.strokeStyle = dmg.isBurning ? `rgba(255, 230, 100, ${1.0 * alpha})` : `rgba(0, 0, 0, ${0.9 * alpha})`; window.ctx.lineWidth = baseLw * 0.4; window.ctx.shadowBlur = 0; window.ctx.beginPath(); path.forEach((pt, idx) => { if (idx === 0) window.ctx.moveTo(pt.x, pt.y); else window.ctx.lineTo(pt.x, pt.y); }); window.ctx.stroke();
                    }
                });
                window.ctx.restore();
            });
       ...
       if (window.envHazards && window.envHazards.length > 0) {
        ...
        window.weatherParticles.forEach(w => { 
            ...
            else if (window.currentWeather === 'fireflies') { let glow = 0.3 + Math.abs(Math.sin(Date.now()/350 + w.x)) * 0.7; window.ctx.fillStyle = `rgba(241, 196, 15, ${glow})`; window.ctx.shadowBlur = 8; window.ctx.shadowColor = "#f1c40f"; window.ctx.beginPath(); window.ctx.arc(w.x, w.y, w.size * 1.4, 0, Math.PI*2); window.ctx.fill(); window.ctx.shadowBlur = 0; } 
        ...
        // HIỆU ỨNG GRAYSCALE (SIN CITY FINISHER)
        ...
        if (window.p1) {
            ...
            allFighters.forEach(p => { 
                if (p && p.hp > 0 && p.trailArr) { 
                    p.trailArr.forEach(t => { 
                        ...
                        if (trailP.isDragon && typeof window.drawDragon === 'function') window.drawDragon(window.ctx, trailP, true); else if (trailP.isBruceLee && typeof window.drawBruceLee === 'function') window.drawBruceLee(window.ctx, trailP); else if (trailP.isSamurai && typeof window.drawSamurai === 'function') window.drawSamurai(window.ctx, trailP); else if (trailP.isNinja && typeof window.drawNinja === 'function') window.drawNinja(window.ctx, trailP); else if (typeof window.drawStickman === 'function') window.drawStickman(window.ctx, trailP, true); 
                    }); 
                } 
            });
            window.ctx.globalCompositeOperation = "source-over";
            ...
            window.enemies.forEach(e => { 
                ...
                if (e.state === 'ko_falling' || e.state === 'dead') { 
                    ...
                    if(clone.isDragon && typeof window.drawDragon === 'function') window.drawDragon(window.ctx, clone); else if (clone.isBruceLee && typeof window.drawBruceLee === 'function') window.drawBruceLee(window.ctx, clone); else if (clone.isSamurai && typeof window.drawSamurai === 'function') window.drawSamurai(window.ctx, clone); else if (clone.isNinja && typeof window.drawNinja === 'function') window.drawNinja(window.ctx, clone); else if (typeof window.drawStickman === 'function') window.drawStickman(window.ctx, clone); 
                } else { 
        ...
        if (window.p1 && window.p1.hp > 0 && window.p1.hp <= window.p1.maxHp * 0.2 && !window.gameOver) {
            window.isLowHpPulsing += 0.1; let bloodPulse = Math.abs(Math.sin(window.isLowHpPulsing)) * 0.4;
            ...
            bloodGrad.addColorStop(0, "rgba(255, 0, 0, 0)"); bloodGrad.addColorStop(1, `rgba(255, 0, 0, ${bloodPulse})`); window.ctx.fillStyle = bloodGrad; window.ctx.globalCompositeOperation = 'multiply'; window.ctx.fillRect(0, 0, window.canvas.width, window.canvas.height); window.ctx.globalCompositeOperation = 'source-over';
        }
        ...
        if (window.gameOver && window.matchEndTimer >= 90 && window.endIconType) {
            ...
            if (window.endIconType === 'win') { window.ctx.shadowBlur = 35; window.ctx.shadowColor = "#f1c40f"; window.ctx.fillText("🏆", 0, 0); } else if (window.endIconType === 'lose') { window.ctx.shadowBlur = 35; window.ctx.shadowColor = "#ff4757"; window.ctx.fillText("💀", 0, 0); }
            window.ctx.restore();
        }
        window.ctx.restore();
        ...
        // TỐI ƯU CỰC MẠNH: CHROMA RENDER TỪ CANVAS ĐỆM CHỐNG LAG
...
// ==========================================
// 7. CORE VẬT LÝ FIXED TIMESTEP & MỞ KHÓA 120 FPS
// ==========================================
window.lastFrameTime = 0; 
window.physicsAccumulator = 0;
window.PHYSICS_STEP = 1000 / 60; // KHÓA CỨNG LOGIC Ở 60 KHUNG HÌNH/GIÂY
...
window.gameLoop = function(timestamp) { 
    ...
    if (deltaTime > 250) deltaTime = 250; // Tránh treo game khi người dùng chuyển Tab
    window.physicsAccumulator += deltaTime;

    // BƯỚC 1: Xử lý Đấm/Đá/Trừ Máu chuẩn 60 lần/giây để không làm xáo trộn Frame-data
    ...
    // BƯỚC 2: Cập nhật Camera và Render chạy theo tần số quét thực của màn hình (120Hz/144Hz+)
...
// TỰ ĐỘNG KHỞI ĐỘNG HỆ THỐNG PRELOAD KHI FILE VỪA ĐƯỢC ĐỌC XONG
