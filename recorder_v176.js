// ==========================================
// RECORDER.JS - V76.5 THE ANIME STORY MODE EDITION
// ĐÃ FIX & GIỮ NGUYÊN: Intro God-Tier, Vỡ kính 3D, Kill-Cam, Chữ nảy MrBeast.
// NÂNG CẤP V76.5 (NEW MAX): 
// 1. TỐI ƯU HÓA 1000%: Bỏ Facecam, tải video ngay lập tức (Instant Download).
// 2. CINEMATIC STORY MODE: Tự động chèn các câu thoại đạo lý, khịa nhau giữa trận đấu khi máu giảm (Hiệu ứng gõ chữ Typewriter, viền đen điện ảnh).
// ==========================================
...
// ==========================================
// VẼ CINEMATIC QUOTES (STORY MODE DIALOGUE)
// ==========================================
window.drawCinematicQuote = function(ctx, quote, elapsed, w, h) {
    ...
    // Cinematic bars (Viền đen 2 đầu để tạo hiệu ứng màn hình rộng điện ảnh)
    ctx.fillStyle = "#000";
    ctx.fillRect(0, 0, w, 150);
    ctx.fillRect(0, h - 150, w, 150);
    
    // Làm mờ toàn bộ khung hình phía sau
    ctx.fillStyle = "rgba(0,0,0,0.6)";
    ctx.fillRect(0, 150, w, h - 300);

    // Dialogue Box
    let boxH = 220;
    let boxY = h - 150 - boxH - 20;
    ctx.fillStyle = "rgba(10, 15, 30, 0.9)";
    ctx.strokeStyle = quote.speaker === "P1" ? "#00f3ff" : "#ff003c";
    ctx.lineWidth = 5;
    if(ctx.roundRect) {
        ctx.beginPath();
        ctx.roundRect(40, boxY, w - 80, boxH, 20);
        ctx.fill(); ctx.stroke();
    } else {
    ...
    ctx.fillStyle = quote.speaker === "P1" ? "#00f3ff" : "#ff003c";
    ctx.font = "900 40px 'Arial Black', sans-serif";
    ctx.textAlign = "left";
    ctx.fillText(speakerName, 70, boxY + 60);
    
    // Đường kẻ ngang phân cách
    ctx.beginPath();
    ctx.moveTo(70, boxY + 80);
    ctx.lineTo(w - 70, boxY + 80);
    ctx.strokeStyle = "rgba(255,255,255,0.2)";
    ctx.lineWidth = 2;
    ctx.stroke();

    // Typewriter Text (Hiệu ứng gõ chữ từng ký tự)
    let charsToShow = Math.floor((elapsed - 300) / 40); // 40ms/ký tự
    ...
    for (let i = 0; i < words.length; i++) {
        ...
        if (ctx.measureText(testLine).width > w - 140 && i > 0) {
...
window.stopRecording = function() { 
    ...
    window.showRenderToast("⏳ ĐANG LƯU CLIP CỐT TRUYỆN...");
...
window.captureFrames = function() {
    ...
    if (renderNormalV) {
        ...
        // TÍNH PHẦN TRĂM MÁU ĐỂ KÍCH HOẠT HỘI THOẠI CỐT TRUYỆN
        ...
        // XỬ LÝ VÀ VẼ CINEMATIC QUOTES NGAY TRÊN GAMEPLAY MÀ KHÔNG CHE TIKTOK HUD
            ...
            for (let q of window._storyQuotes) {
                if (!q.triggered) {
                    ...
                    if (targetHp <= q.triggerHp && targetHp > 0) {
                        q.triggered = true;
                        window._activeQuote = q;
                        window._activeQuoteStartTime = Date.now();
                        break; 
                    }
                }
            }
        ...
        if (!isOutroActive) {
            ...
            for(let i = -1; i < 5; i++) { ctxV.fillText("🚨 CHỜ ĐẾN CUỐI ⏩ KHÔNG ĐƯỢC CHỚP MẮT 🚨", i*880 - offsetBanner, bannerY + 30); }
            ...
            if (window.p1) {
                ...
                if (p1HpPct > 0) { if(!window._cachedGradients.hpP1) { window._cachedGradients.hpP1 = ctxV.createLinearGradient(187, 0, 487, 0); window._cachedGradients.hpP1.addColorStop(0, "#00f2fe"); window._cachedGradients.hpP1.addColorStop(1, "#4facfe"); } drawSkewedPath(ctxV, 187, hudBaseY + 73, (300 * p1HpPct) | 0, 53, true); ctxV.fillStyle = window._cachedGradients.hpP1; ctxV.fill(); }
                ...
                if (repEnemyObj) {
                    ...
                    if (p2HpPct > 0) { if(!window._cachedGradients.hpP2) { window._cachedGradients.hpP2 = ctxV.createLinearGradient(580, 0, 880, 0); window._cachedGradients.hpP2.addColorStop(0, "#ff0844"); window._cachedGradients.hpP2.addColorStop(1, "#ffb199"); } let eHpWidth = (300 * p2HpPct) | 0; drawSkewedPath(ctxV, 580 + (300 - eHpWidth), hudBaseY + 73, eHpWidth, 53, false); ctxV.fillStyle = window._cachedGradients.hpP2; ctxV.fill(); }
...
// ========================================================
// LƯU TRỰC TIẾP VIDEO (INSTANT DOWNLOAD - KHÔNG GHÉP RENDER NỮA)
// ========================================================
window.mergeAndDownloadVideo = function(vidId) {
    let vid = window.savedVideos.find(v => v.id === vidId);
    if (!vid) return;

    // Tải trực tiếp (Instant Download) thay vì tốn thời gian Render hậu kỳ
