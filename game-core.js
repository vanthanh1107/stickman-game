...
function matchStart() {
    ...
    gameOver = false; matchResolved = false; 
...
function update() {
    ...
    [p1, p2].forEach(p => {
        ...
        if (Math.abs(overlapX) < 40) { 
            ...
            p1.x -= pushForce * sign; p2.x += pushForce * sign;
        }
        ...
        let gameContext = { 
            ...
            dash: (fighter, forceX, forceY) => { fighter.vx = forceX; if(forceY) fighter.vy = forceY; fighter.state = 'dash'; fighter.attackTimer = 15; fighter.iFrames = 10; },
            teleport: (fighter, destX, destY) => { fighter.x = destX; if(destY) fighter.y = destY; fighter.state = 'cast'; fighter.attackTimer = 10; },
        ...
        if (p.attackTimer === 0 && p.hitStun === 0 && p.dashTimer <= 0) {
            let dist = enemy.x - p.x; p.isFacingRight = dist > 0; let absDist = Math.abs(dist);
            ...
            if (p.aiDelay <= 0) {
                ...
                if (!usedSkill) {
                    if (absDist > 60) { 
                        p.vx = Math.sign(dist) * p.currentSpeed; p.state = 'walk';
                    } else { 
                        let rand = Math.random();
                        if (enemy.attackTimer > 0 || enemy.state === 'dash') {
                            if (rand < 0.6) {
                                p.dashTimer = 12; p.dashDir = -Math.sign(dist); p.state = 'dash_back'; p.iFrames = 12; p.attackTimer = 12;
                                for(let i=0; i<4; i++) {
                                ...
                                p.state = 'block'; p.attackTimer = 15;
                            ...
                            if (decidedToAttack) {
                                if (p.comboTimer > 0 && p.comboStep < 2) {
                                    ...
                                    p.comboStep = 0; attack(p, enemy, 'punch'); p.vx = Math.sign(dist) * 2; 
                                }
    ...
    for (let i = projectiles.length - 1; i >= 0; i--) {
        ...
        if (Math.sqrt(dx*dx + dy*dy) < proj.radius + 20) {
            if(proj.onHit) proj.onHit(); takeDamage(proj.target, proj.dmg, `🎇 -${proj.dmg}`, "#9b59b6"); projectiles.splice(i, 1);
...
function drawStickman(ctx, p, isTrail = false) {
    ...
    ctx.strokeStyle = "#fff"; ctx.shadowBlur = p.iFrames > 0 ? 25 : 8; ctx.shadowColor = p.iFrames > 0 ? "#bdc3c7" : p.color; 
    ctx.lineWidth = 5; ctx.lineCap = 'round'; ctx.lineJoin = 'round';
    if (isTrail) { ctx.globalAlpha = 0.3; ctx.shadowBlur = 0; }

    let bounce = (p.state === 'walk') ? Math.abs(Math.sin(Date.now() / 100)) * 5 : 0;
    let maxT = (p.state === 'punch') ? 15 : 20;
    let pext = 0; if (p.state === 'punch' && p.comboStep === 1) pext = 10;
    ...
    if (p.drawMethod) {
        try {
            ...
            ctx.restore(); return; 
        } catch (e) { console.error("Lỗi chạy drawCode Google Sheet:", e); }
    }
    ...
    if (p.state === 'hurt') {
        ...
        handR = {x: 10, y: -55 + bounce}; elbowR = {x: 15, y: -35 + bounce}; handL = {x: 0, y: -55 + bounce}; elbowL = {x: -10, y: -35 + bounce};
    ...
    ctx.fillStyle = p.color; ctx.beginPath(); ctx.arc(handL.x, handL.y, 6, 0, Math.PI*2); ctx.fill(); ctx.beginPath(); ctx.arc(handR.x, handR.y, 6, 0, Math.PI*2); ctx.fill(); 
...
function draw() {
    ...
    floatingTexts.forEach(t => { ctx.fillStyle = t.color; ctx.fillText(t.text, t.x, t.y); }); ctx.restore(); 
    if (gameOver && p1 && p2) {
