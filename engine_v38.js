// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 32.0 [THE PERFECT FRAMING]
// [FIXED] PERFECT Y-FRAMING: Camera luôn lấy nét trọn vẹn từ đầu đến chân và bóng dưới sàn.
// [FIXED] GROUNDED 3D ROTATION: Chân nhân vật khóa chặt vào mặt đất khi quay 360 độ.
// [BASE] Giữ nguyên toàn bộ tuyệt kỹ: 360 Orbit, Kinetic Text Lò xo, Lethal Void, RTX Floor.
// ==========================================
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce, attackerDirRight = true) {
    ...
    if (target.state === 'block') { 
        if (target.stamina < 15) {
            ...
            let allFighters = [window.p1].concat(window.enemies); allFighters.forEach(e => { if (e !== target && e.hp > 0 && Math.abs(e.x - target.x) < 140) { e.state = 'hurt'; e.hitStun = 50; e.vx = target.isFacingRight ? 12 : -12; window.spawnDust(e.x, e.y); } });
            return;
        ...
        if (target.hp <= 0) {
            ...
            if (isNearWall && isCrit && typeof window.triggerStageTransition === 'function') { window.triggerStageTransition(target); } 
                ...
                if (isCrit) {
                    ...
                    window.isSpinningCam = true; window.orbitFocusX = target.x; window.targetTimeScale = 0.02; 
...
window.update = function() {
    ...
    // [VIRAL 32.0] PERFECT FRAMING ALGORITHM
    if (window.isSpinningCam) {
        window.camOrbitAngle += 0.08; 
    } else {
    ...
    if (window.p1 && window.introTimer === 0) {
        ...
        let midX = window.canvas.width / 2;
        let midY = window.GROUND_Y - 80;
        
        if (closest) {
            midX = (window.p1.x + closest.x) / 2;
            midY = ((window.p1.y + closest.y) / 2) - 30; 
        } else if (window.p1) {
            midX = window.p1.x;
            midY = window.p1.y - 30;
        }
        ...
        if (!window.isSpinningCam && window.targetCamOrbitAngle === 0) {
            window.orbitFocusX += (midX - window.orbitFocusX) * 0.1;
            window.orbitFocusY += (midY - window.orbitFocusY) * 0.1; 
        }

        let actionPan = (!window.gameOver && closest && window.slowMoTimer <= 0) ? (window.p1.vx + (closest.vx || 0)) * 6 : 0;
        
        window.targetCamX = (window.canvas.width / 2) - window.orbitFocusX + window.actionCamOffsetX + actionPan;
        // KHÓA MÁY QUAY THEO TRỤC Y HOÀN HẢO (HIỆN RÕ CHÂN + BÓNG KÍNH RTX)
        window.targetCamY = (window.canvas.height / 2) - window.orbitFocusY + window.actionCamOffsetY;
        ...
        window.targetCamX = 0; window.targetCamY = 0; window.targetZoom = 1; window.targetTilt = 0; 
        window.orbitFocusX = window.canvas ? window.canvas.width/2 : 0;
        window.orbitFocusY = window.GROUND_Y - 80;
    }
}
...
window.draw = function() {
    ...
    // TÍNH TOÁN 3D (Z-Axis Math) - Bản V32.0 (Sửa lỗi bay lên trời)
    ...
    let project3D = (obj) => {
        ...
        let pZ = dx * sinA;
        // Khóa chặt Y với mặt đất, không cho bay lơ lửng khi xoay 360 độ
    ...
    try {
        ...
        window.ctx.save();
        window.ctx.translate((window.camX + bgPanOffset) * 0.95, 0); // V32.0 Khóa trục Y phông nền
        ...
        window.ctx.translate((window.camX + bgPanOffset) * 0.85, 0);
        ...
        window.ctx.save(); window.ctx.translate((window.camX + bgPanOffset) * 0.6, 0); 
        ...
        window.ctx.save(); window.ctx.translate((window.camX + bgPanOffset) * 0.2, 0); 
