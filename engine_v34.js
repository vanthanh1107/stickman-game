// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 28.0 [THE 2.5D MATRIX CAMERA]
// [BASE] Giữ nguyên 100% độ mượt và Kinetic Typography của V27.1.
// [NEW] DYNAMIC CAMERA YAW & SKEW: Xoay trục máy quay, bóp méo không gian tạo ảo giác 3D.
// [NEW] ACTION ANGLE SNAP: Văng góc quay chéo khi có đòn kết liễu hoặc chí mạng.
// [NEW] 3D HOLO-GRID: Mặt sàn lưới không gian hội tụ về điểm ảo chân trời.
// [NEW] EXTREME FOREGROUND BOKEH: Lớp tiền cảnh siêu gần, lướt mờ ảo nới rộng chiều sâu Z-Axis.
// ==========================================
...
// [VIRAL 28.0] 2.5D CAMERA VARS
window.actionCamSkew = 0; // Trục nghiêng chéo 3D
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce, attackerDirRight = true) {
    ...
    if (target.state === 'block') { 
        if (target.stamina < 15) {
            ...
            // [VIRAL 28.0] Xoay góc máy khi khiên vỡ
            window.actionCamSkew = attackerDirRight ? 0.1 : -0.1;
            
            window.targetZoom = 1.5; window.dynamicBlur = 10;
            window.hitStopFrames = 25; 
            window.impactAberration = 25; 
            window.chromaTimer = 20; window.vhsGlitchTimer = 20;
    ...
    if (finalDmg > 0) {
        ...
        let textVx = (attackerDirRight ? 1 : -1) * (Math.random() * 6 + 3); 
        let textVy = -8 - Math.random() * 6; 
        let initialScale = isCrit ? 5.5 : 2.5; 
        let targetScale = isCrit ? 1.5 : 1.0; 
        
        window.floatingTexts.push({ 
            x: target.x, y: target.y - 60, text: dmgText, 
            color: color || (isCrit ? "#ff4757" : "#fff"), 
            alpha: 1.5, vx: textVx, vy: textVy, 
            font: isCrit ? "italic 900 40px 'Arial Black', Impact" : "italic 900 30px 'Arial Black', Impact", 
            life: 45, scale: initialScale, targetScale: targetScale, scaleVel: 0, rot: (Math.random()-0.5)*0.5 
        });
        ...
        for(let i=0; i<sparkCount; i++) { 
            ...
            window.impactSparks.push({ x: target.x, y: target.y - 40, vx: vx, vy: vy, life: 25 + Math.random()*15, maxLife: 40, color: color || "#f1c40f", scale: 1.0, zSpeed: Math.random() > 0.5 ? 1.05 : 0.95 }); 
        }
        ...
        if (isCrit) {
            ...
            // [VIRAL 28.0] VĂNG CAMERA CHÉO GÓC KHI CHÍ MẠNG
            window.actionCamSkew = attackerDirRight ? 0.08 : -0.08;
            ...
            window.hitStopFrames = 10; 
            window.targetTimeScale = 1.0; 
            setTimeout(()=>{ window.targetTimeScale = 0.15; setTimeout(()=>{ window.targetTimeScale = 1.0; }, 300); }, 10 * 16); 
        ...
        if (target.hp <= 0) {
            ...
            if (isNearWall && isCrit && typeof window.triggerStageTransition === 'function') { window.triggerStageTransition(target); } 
                ...
                if (isCrit) {
                    ...
                    window.floatingTexts.push({ x: target.x, y: target.y - 120, text: "💀 LETHAL!", color: "#ff003c", alpha: 1.5, vx: 0, vy: -2, font: "italic 900 65px 'Arial Black', Impact", life: 90, scale: 5.0, targetScale: 1.2, scaleVel: 0, rot: (Math.random()-0.5)*0.2 });
                    ...
                    // [VIRAL 28.0] VĂNG CAMERA MẠNH NHẤT KHI K.O
                    window.actionCamSkew = attackerDirRight ? 0.2 : -0.2;
                    ...
                    window.impactAberration = 20; 
                    // [VIRAL 28.0] Nghiêng máy nhẹ
                    window.actionCamSkew = attackerDirRight ? 0.05 : -0.05;
                    window.targetTimeScale = 1.0;
                    setTimeout(()=>{ window.targetTimeScale = 0.2; setTimeout(()=>{window.targetTimeScale = 1.0; }, 1000); }, 10 * 16);
                }
...
window.attack = function(attacker, targetGroup) {
    ...
    if (isFinisher) {
        ...
        // [VIRAL 28.0] Xoay máy ảnh khi dồn sức tung Finisher
        window.actionCamSkew = attacker.isFacingRight ? 0.15 : -0.15;
    } else {
        if (Math.random() < attacker.critChance) { isCrit = true; finalDmg = baseDmg * attacker.critMult; } else { window.playSound(350, 'sine', 0.1, 0.1, false); }
...
window.update = function() {
    ...
    window.actionCamSkew *= 0.85; // [VIRAL 28.0] Phục hồi độ méo 3D về 0
    ...
    if (!isTimeStopped) {
        window.enemies.forEach(e => { 
            if (e.hp <= 0 && !e.deathTriggered) {
                ...
                if (window.p1 && window.p1.hp > 0) { let heal = Math.floor(window.p1.maxHp * 0.08); window.p1.hp = Math.min(window.p1.maxHp, window.p1.hp + heal); 
                    window.floatingTexts.push({ x: window.p1.x, y: window.p1.y - 80, text: `+${heal} 💚`, color: "#2ed573", alpha: 1.5, vx: (Math.random()-0.5)*4, vy: -6, font: "italic 900 30px 'Arial Black', Impact", life: 45, scale: 2.0, targetScale: 1.0, scaleVel: 0, rot: 0 }); 
                }
            }
        });
            ...
            if (f.hitStun <= 0 && f.stunTimer <= 0 && f.state !== 'dash_back' && f.state !== 'block') {
                ...
                if (isCloseEnough) {
                    if (window.classStats && window.classStats[type] && typeof window.classStats[type].executeUltimate === 'function') {
                        ...
                        window.actionCamSkew = f.isFacingRight ? 0.1 : -0.1; // Chéo máy khi xả Ulti
                    } else if (typeof window.useUltimate === 'function') { window.useUltimate(f, closestTarget); f.vx = 0; window.speedLinesAlpha = 1.0; window.actionCamSkew = f.isFacingRight ? 0.1 : -0.1; }
            ...
            if (f.isDragon) {
                ...
                if (targetFighter && targetFighter.hp > 0) {
                    ...
                    if (f.aiDelay <= 0) {
                        ...
                        if (randAction < 0.45) { f.state = 'breathe_fire'; f.attackTimer = 40; f.vx = 0; window.playSound(250, 'sawtooth', 0.6, 0.3, false); let fireCount = 0; let fireInterval = setInterval(() => { if (window.gameOver || f.hp <= 0 || !window.p1) { clearInterval(fireInterval); return; } let fireVx = f.isFacingRight ? (f.isEvolved ? 15 : 11) : (f.isEvolved ? -15 : -11); if (f.isEvolved) { window.spawnProjectile( f.x + (f.isFacingRight ? 55 : -55), f.y - 65, fireVx, -3, 15, "#9b59b6", Math.floor(35 * f.dmgMod), window.p1 ); window.spawnProjectile( f.x + (f.isFacingRight ? 55 : -55), f.y - 65, fireVx, 0, 15, "#8e44ad", Math.floor(35 * f.dmgMod), window.p1 ); window.spawnProjectile( f.x + (f.isFacingRight ? 55 : -55), f.y - 65, fireVx, 3, 15, "#9b59b6", Math.floor(35 * f.dmgMod), window.p1 ); } else { window.spawnProjectile( f.x + (f.isFacingRight ? 55 : -55), f.y - 65, fireVx, (Math.random() - 0.5) * 4, 13, "#e74c3c", Math.floor(25 * f.dmgMod), window.p1 ); } fireCount++; if (fireCount >= (f.isEvolved ? 8 : 6)) clearInterval(fireInterval); }, f.isEvolved ? 60 : 85); window.actionCamSkew = f.isFacingRight ? -0.1 : 0.1; } 
                ...
                if (targetFighter && targetFighter.hp > 0) {
                    ...
                    if (f.aiDelay <= 0) {
                        ...
                        if (absDist > 110) { f.state = 'walk'; f.vx = Math.sign(dist) * f.currentSpeed * 1.5; if (Math.random() < 0.3) { f.state = 'dash'; f.dashTimer = 8; f.dashDir = Math.sign(dist); window.spawnDust(f.x, f.y); window.actionCamSkew = f.isFacingRight ? 0.05 : -0.05; } } 
                        else if (randAction < 0.33) { f.state = 'one_inch_punch'; f.attackTimer = 22; f.vx = Math.sign(dist) * 4; window.playSound(380, 'square', 0.25, 0.9, true); if (absDist < 65) { if (typeof window.takeDamage === 'function') window.takeDamage(window.p1, Math.floor(32 * f.currentDmgMod), "#f1c40f", true, false); window.p1.vx = Math.sign(dist) * 6; window.shakeScreen(15, 12); window.floatingTexts.push({ x: window.p1.x, y: window.p1.y - 80, text: "🗣️", color: "#f1c40f", alpha: 1.5, vx: 0, vy: -5, font: "900 40px Arial", life: 50, scale: 2.5, targetScale: 1.0, scaleVel: 0, rot: 0 }); window.actionCamSkew = f.isFacingRight ? 0.1 : -0.1; } } 
                        else if (randAction < 0.66) { f.state = 'high_kick'; f.attackTimer = 28; f.vx = Math.sign(dist) * 5; let kickCount = 0; let kickInterval = setInterval(() => { if (window.gameOver || f.hp <= 0 || !window.p1) { clearInterval(kickInterval); return; } if (Math.abs(window.p1.x - f.x) < 85) { if (typeof window.takeDamage === 'function') window.takeDamage(window.p1, Math.floor(12 * f.currentDmgMod), "#ecf0f1", false, false); window.p1.vx = Math.sign(dist) * 3; window.shakeScreen(4, 3); } kickCount++; if (kickCount >= 3) clearInterval(kickInterval); }, 70); window.floatingTexts.push({ x: f.x, y: f.y - 100, text: "👟", color: "#f1c40f", alpha: 1.5, vx: 0, vy: -4, font: "900 40px Arial", life: 40, scale: 2.0, targetScale: 1.0, scaleVel: 0, rot: 0 }); window.actionCamSkew = f.isFacingRight ? -0.05 : 0.05; } 
                ...
                if (targetFighter && targetFighter.hp > 0) {
                    ...
                    if (f.aiDelay <= 0) {
                        ...
                        if (absDist > 250 && Math.random() < 0.6) { f.state = 'cast'; f.attackTimer = 25; window.playSound(300, 'sine', 0.3, 0.6); window.spawnSlash(f.x + (f.isFacingRight? 50:-50), f.y - 40, f.isFacingRight, "#fff", true, 2.0, Math.PI/2); window.spawnProjectile(f.x, f.y - 40, f.isFacingRight ? 12 : -12, 0, 15, "#fff", Math.floor(25 * f.dmgMod), window.p1); window.floatingTexts.push({ x: f.x, y: f.y - 80, text: "🗡️", color: "#fff", alpha: 1.5, vx: 0, vy: -4, font: "900 40px Arial", life: 40, scale: 2.0, targetScale: 1.0, scaleVel: 0, rot: 0 }); window.actionCamSkew = f.isFacingRight ? 0.08 : -0.08; } 
                        else { f.state = 'dash'; f.dashTimer = 15; f.dashDir = Math.sign(dist); f.currentSpeed *= 2.5; f.iFrames = 20; window.playSound(400, 'sawtooth', 0.4, 0.8, true); window.actionCamSkew = f.isFacingRight ? 0.15 : -0.15; setTimeout(() => { if(window.gameOver || f.hp <= 0 || !window.p1) return; if(Math.abs(window.p1.x - f.x) < 180) { if(typeof window.takeDamage === 'function') window.takeDamage(window.p1, Math.floor(40 * f.dmgMod), "#e74c3c", true, false); window.shakeScreen(20, 15); window.p1.vx = Math.sign(dist) * 6; window.actionCamSkew = f.isFacingRight ? -0.1 : 0.1; } window.spawnSlash(f.x, window.p1.y - 30, f.isFacingRight, "#e74c3c", true, 3.0, (Math.random()-0.5)); window.floatingTexts.push({ x: f.x, y: f.y - 80, text: "⚡", color: "#e74c3c", alpha: 1.5, vx: 0, vy: -5, font: "900 40px Arial", life: 40, scale: 2.5, targetScale: 1.0, scaleVel: 0, rot: 0 }); }, 200); }
                ...
                if (targetFighter && targetFighter.hp > 0) {
                    ...
                    if (f.aiDelay <= 0) {
                        ...
                        if (randAction < 0.4) { f.state = 'cast'; f.attackTimer = 20; window.playSound(400, 'sine', 0.2, 0.4); window.spawnProjectile(f.x, f.y - 50, f.isFacingRight ? 15 : -15, 0, 8, "#9b59b6", Math.floor(15 * f.dmgMod), window.p1); if(Math.random() < 0.5) window.spawnProjectile(f.x, f.y - 60, f.isFacingRight ? 14 : -14, -3, 8, "#9b59b6", Math.floor(15 * f.dmgMod), window.p1); window.floatingTexts.push({ x: f.x, y: f.y - 80, text: "🥷", color: "#9b59b6", alpha: 1.5, vx: 0, vy: -4, font: "900 40px Arial", life: 30, scale: 2.0, targetScale: 1.0, scaleVel: 0, rot: 0 }); window.actionCamSkew = f.isFacingRight ? -0.05 : 0.05; } 
                        else if (randAction < 0.7) { window.spawnParticles(f.x, f.y, "#2c3e50"); f.x = window.p1.x + (window.p1.isFacingRight ? -80 : 80); f.y = window.p1.y; f.isFacingRight = window.p1.x > f.x; window.spawnParticles(f.x, f.y, "#9b59b6"); f.state = 'spinning_backfist'; f.attackTimer = 15; window.playSound(200, 'square', 0.2, 0.6); if(typeof window.takeDamage === 'function') window.takeDamage(window.p1, Math.floor(20 * f.dmgMod), "#9b59b6", false, false); window.floatingTexts.push({ x: f.x, y: f.y - 80, text: "💨", color: "#8e44ad", alpha: 1.5, vx: 0, vy: -4, font: "900 40px Arial", life: 30, scale: 2.0, targetScale: 1.0, scaleVel: 0, rot: 0 }); window.actionCamSkew = f.isFacingRight ? 0.08 : -0.08; } 
                ...
                if (closest && closest.hp > 0) {
                    ...
                    if (absDist > reach) { f.vx = Math.sign(dist) * f.currentSpeed; f.state = 'walk'; if (Math.random() < 0.1 && f.onGround) window.spawnDust(f.x, f.y); } 
                        ...
                        if (f.aiDelay <= 0) {
                            ...
                            if (!usedSkill) { let rand = Math.random(); if (closest.attackTimer > 0 || closest.state === 'dash') { if (rand < 0.4) { f.dashTimer = 10; f.dashDir = -Math.sign(dist); f.state = 'dash_back'; f.iFrames = 10; f.attackTimer = 10; window.spawnDust(f.x, f.y); window.actionCamSkew = f.isFacingRight ? -0.05 : 0.05; } else { if(typeof window.attack === 'function') window.attack(f, targetList); } } else { if (rand < 0.9) { if (f.comboTimer > 0 && f.comboStep < 14) f.comboStep++; else f.comboStep = 0; f.comboTimer = 50; if(typeof window.attack === 'function') window.attack(f, targetList); } else { f.vx = -Math.sign(dist) * f.currentSpeed * 1.5; f.state = 'walk'; } } }
                        }
                    }
            ...
            for (let j = i + 1; j < allFighters.length; j++) { 
                ...
                if (dist < 80 && f1.attackTimer > 5 && f1.attackTimer < 20 && f2.attackTimer > 5 && f2.attackTimer < 20) {
                    if (!f1.clashCooldown && !f2.clashCooldown && f1.isFacingRight !== f2.isFacingRight) {
                        ...
                        window.actionCamSkew = f1.isFacingRight ? 0.2 : -0.2; // Quay chéo khi đụng đòn
                    }
                }
    ...
    allFighters.forEach(f => {
        ...
        if (f.x < wallBound) { 
            ...
            if (f.hitStun > 0 && f.vx < -8 && !f.wallBounced) {
                ...
                window.floatingTexts.push({ x: f.x, y: f.y - 80, text: "💥 WALL SPLAT!", color: "#f1c40f", alpha: 1.5, vx: 3, vy: -5, font: "italic 900 30px 'Arial Black', Impact", life: 40, scale: 2.5, targetScale: 1.0, scaleVel: 0, rot: 0 });
                window.actionCamSkew = -0.15; // Văng máy vào tường trái
        ...
        if (window.canvas && f.x > window.canvas.width - wallBound) { 
            ...
            if (f.hitStun > 0 && f.vx > 8 && !f.wallBounced) {
                ...
                window.playSound(200, 'square', 0.4, 0.8, true); window.spawnDust(f.x, f.y);
                window.floatingTexts.push({ x: f.x, y: f.y - 80, text: "💥 WALL SPLAT!", color: "#f1c40f", alpha: 1.5, vx: -3, vy: -5, font: "italic 900 30px 'Arial Black', Impact", life: 40, scale: 2.5, targetScale: 1.0, scaleVel: 0, rot: 0 });
                window.actionCamSkew = 0.15; // Văng máy vào tường phải
            } else if (f.hitStun > 0 && f.vx > 3 && !f.wallBounced && f.state !== 'wall_splat') {
                f.wallBounced = true; f.vx = -4; f.hitStun = 15; window.shakeScreen(10, 4); 
                if (!f.wallDamageCooldown) { window.spawnEnvDamage(window.canvas.width - wallBound, f.y, 'wall_right', f.scale || 1, false); f.wallDamageCooldown = 60; }
                if(typeof window.takeDamage === 'function') window.takeDamage(f, Math.floor(Math.random() * 4) + 4, "#fff", false, true); 
    ...
    for (let i = window.floatingTexts.length - 1; i >= 0; i--) { 
        ...
        if (t.life !== undefined) { 
            t.vy += window.GRAVITY * 0.6; 
            t.x += t.vx; 
            t.y += t.vy; 
            t.life--; 
            
            if (t.scale !== undefined && t.targetScale !== undefined) {
                t.scaleVel = (t.scaleVel || 0) + (t.targetScale - t.scale) * 0.25;
                t.scaleVel *= 0.75; 
                t.scale += t.scaleVel;
            }
...
window.draw = function() {
    ...
    if (window.isLoading) {
        ...
        window.ctx.fillStyle = "#00f3ff"; window.ctx.font = "italic 900 40px Arial"; window.ctx.textAlign = "center"; window.ctx.shadowBlur = 15; window.ctx.shadowColor = "#00f3ff"; window.ctx.fillText("2.5D MATRIX CAMERA INITIALIZING...", cx, cy - 50);
        ...
        window.ctx.fillStyle = "#fff"; window.ctx.font = "bold 16px monospace"; window.ctx.fillText(`RENDERING CAMERA YAW/SKEW 3D... ${Math.floor(window.loadProgress)}%`, cx, cy + 60);
        return; 
    }
    ...
    try {
        ...
        // [VIRAL 28.0] DYNAMIC CAMERA YAW & SKEW (TRANSFORM 3D MATRIX)
        // Dựa vào vận tốc camera để làm méo góc nhìn.
        let dynamicYaw = Math.max(0.85, 1 - Math.abs(window.camVelocityX) * 0.001);
        let dynamicSkew = -(window.camVelocityX * 0.002) + window.actionCamSkew; // Kết hợp Skew hệ thống và Cố tình Skew lúc đánh
        ...
        // Transform ma trận 3 chiều giả lập (2.5D Mode-7)
        ...
        // [VIRAL 28.0] HOLO-GRID LƯỚI KHÔNG GIAN HỘI TỤ ĐIỂM ẢO 3D
        window.ctx.save();
        window.ctx.strokeStyle = `rgba(255, 255, 255, 0.05)`;
        window.ctx.lineWidth = 2;
        window.ctx.beginPath();
        let vanishingPointX = window.canvas.width / 2 + window.camX * 0.3; // Điểm ảo lệch theo cam
        let floorY = window.GROUND_Y;
        for(let i = -4000; i <= 4000; i += 150) {
            window.ctx.moveTo(vanishingPointX, floorY);
            // Kéo dài ra viền màn hình phía trước, tạo lưới phối cảnh chéo 3D
            window.ctx.lineTo(i - window.camX * 1.5, window.canvas.height + 800);
        }
        // Kẻ vạch ngang tạo khối
        for(let j = 0; j <= 800; j += 40) {
             let wY = floorY + j*j*0.005; // Càng gần camera khoảng cách lưới càng thưa
             if(wY > window.canvas.height + 300) break;
             window.ctx.moveTo(-3000, wY);
             window.ctx.lineTo(window.canvas.width + 3000, wY);
        }
        window.ctx.stroke();
        window.ctx.restore();
        ...
        // [VIRAL 28.0] EXTREME FOREGROUND BOKEH (Mờ ảo tạo độ sâu vô cực)
        window.ctx.save();
        window.ctx.filter = 'blur(15px)';
        window.ctx.globalCompositeOperation = 'screen';
        let timeB = Date.now() / 1000;
        window.ctx.fillStyle = window.p1 ? (window.p1.isFacingRight ? 'rgba(255, 50, 50, 0.15)' : 'rgba(50, 150, 255, 0.15)') : 'rgba(255, 255, 255, 0.1)';
        for(let b=0; b<6; b++) {
            // Hạt di chuyển cực nhanh (gấp 2.5 lần tốc độ camera)
            let bx = ((b * 500 + timeB * 400 - window.camX * 2.5) % 3000) - 500;
            let by = window.canvas.height/2 + Math.sin(b + timeB)*300;
            window.ctx.beginPath();
            window.ctx.arc(bx, by, 80 + b*20, 0, Math.PI*2);
            window.ctx.fill();
        }
        window.ctx.restore();
        ...
        window.floatingTexts.forEach(t => { 
            ...
            window.ctx.shadowColor = t.color; 
            window.ctx.fillText(t.text, 0, 0); 
            window.ctx.restore();
        }); 

        window.ctx.restore(); // END CAMERA TRANSFORM

        // Lớp ngoài cùng không bị vặn méo bởi camera
...
// ==========================================
// 7. [VIRAL 28.0] TIME-ENGINE VÀ PHYSICS LOOP CHUẨN XÁC
// ==========================================
