// ==========================================
// ENVIRONMENT.JS - TRẠM MÔI TRƯỜNG ĐA VŨ TRỤ (V3.1 - PERFECT PHYSICS)
// [FIXED] Hiệu ứng không còn xuất hiện ngoài Menu.
// [FIXED] Mở rộng không gian (Bounds) để mưa/tuyết không bị biến mất khi Camera zoom.
// ==========================================
...
window.setupEnvironment = function() {
    ...
    // Phân bổ hạt phủ rộng ra ngoài khung hình (từ -1000 đến +2000) để Camera lia không bị hụt
    for (let i = 0; i < ptCount; i++) {
        window.weatherParticles.push({
            x: -1000 + Math.random() * 3000, 
            y: -1000 + Math.random() * 2500,
            speed: Math.random() * 3 + (window.currentWeather.includes('rain') ? 10 : 1.5),
...
// 2. BỘ CẬP NHẬT VẬT LÝ
window.updateWeatherPhysics = function() {
    if (window.timeStopTimer > 0) return;

    let wType = window.currentWeather;
    // Tọa độ trần và đáy siêu rộng để không bị lỗi biến mất khi Camera Zoom
    let minX = -1000;
    let maxX = window.canvas ? window.canvas.width + 1000 : 2000;
    let minY = -1000;
    let maxY = window.canvas ? window.canvas.height + 1000 : 1500;
    ...
    if (window.weatherParticles) {
        window.weatherParticles.forEach(w => { 
            if (['toxic', 'ash', 'fireflies'].includes(wType)) { 
                ...
                if(w.y < minY) { w.y = maxY; w.x = minX + Math.random() * (maxX - minX); } 
            } 
            else if (wType === 'matrix_rain') {
                w.y += w.speed * 1.6; 
                if(w.y > maxY) { w.y = minY; w.x = minX + Math.random() * (maxX - minX); w.char = Math.random() > 0.5 ? "1" : "0"; }
            } 
                ...
                if(w.y > maxY || w.x < minX) { w.y = minY - Math.random()*200; w.x = minX + Math.random() * (maxX - minX); }
            } 
            else { 
                w.y += w.speed; w.x += ((wType === 'rain' || wType === 'blood_rain') ? -3 : Math.sin(w.y/50)*2) + (window.globalWind || 0); 
                if(w.y > maxY) { w.y = minY; w.x = minX + Math.random() * (maxX - minX); } 
            }
        });
    }
    ...
    if (!window.gameOver && window.matchTimer) {
        ...
        if (Math.random() < meteorChance && window.projectiles.length < 10) { 
            window.projectiles.push({ x: Math.random() * (window.canvas?window.canvas.width:800), y: -500, vx: (Math.random() - 0.5) * 4, vy: 12 + Math.random() * 6, radius: 12 + Math.random() * 8, color: "#e67e22", dmg: 45, target: null, isMeteor: true }); 
        }
...
window.drawWeatherEffects = function(ctx) {
    ...
    if (['snow', 'ash', 'toxic'].includes(wType)) {
        ...
        ctx.beginPath();
        window.weatherParticles.forEach(w => { ctx.moveTo(w.x, w.y); ctx.lineTo(w.x - (wType === 'rain' ? 8 : 6), w.y + (wType === 'rain' ? 20 : 30)); });
        ctx.stroke();
    }
    else {
        window.weatherParticles.forEach(w => { 
...
// ==========================================
// 4. MÓC ĐỒNG BỘ VÀO ENGINE (KHÔNG DÙNG SETINTERVAL)
// ==========================================
...
if (window.update && !window._envHookedUpdate) {
    ...
    window.update = function() {
        // TỰ ĐỘNG KHỞI TẠO MAP Ở FRAME ĐẦU TIÊN CỦA TRẬN ĐẤU
        if (window.matchTimer === 1 && !window._envSetupDone) {
            window.setupEnvironment();
            window._envSetupDone = true;
        } else if (window.matchTimer === 0) {
            // Khi thoát ra menu, reset cờ để trận sau tạo map mới
            window._envSetupDone = false; 
        }

        // Chạy vật lý thời tiết nến đang đánh nhau
        if (window.isLoopRunning) window.updateWeatherPhysics();
...
if (window.draw && !window._envHookedDraw) {
    ...
    window.draw = function() {
        ...
        // CHỈ VẼ KHI CHẮC CHẮN ĐANG Ở TRONG BATTLE SCREEN
        if (window.isLoopRunning && window.ctx && window.canvas && !window.isLoading) {
            window.ctx.save();
            
            // Lấy lại góc camera chính xác của Engine
