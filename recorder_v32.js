// ==========================================
// RECORDER.JS - BẢN HOÀN HẢO CHO CONTENT CREATOR (V57.1 - STABLE + ULTRA MEME THUMBNAIL)
// [RETAINED] Giữ nguyên toàn bộ logic chống lỗi 0-byte, UI inline và hệ thống Capture ổn định.
// [NEW] CỖ MÁY CHẾ MEME: Text 3D đùn khối, Nền Sunburst/Manga, Vòng đỏ, Mắt laze đỏ bựa.
// [FIXED] Snap-cut 1.5s đầu tiên sắc lẹm (bỏ mờ dần) để làm Intro Hook chuẩn TikTok/Shorts.
// ==========================================
...
window.StoryModeAI = {
    ...
    generateScript: function(hero, enemy) {
        ...
        const narratives = [
            ...
            { title: `bro unlocked the FORBIDDEN stickman technique 🤫💻`, lines: ["Here is a secret trick pro gamers use to completely dominate this fight.", "If you force the parry window, you reset their entire attack pattern.", `Watch how ${h} uses it here to completely humiliate them.`] },
...
// ==========================================
// 🎨 THUMBNAIL GENERATOR (CỖ MÁY TẠO POSTER MEME SIÊU CLICKBAIT)
// ==========================================
window.bakeThumbnailsForVideo = function(titleText) {
    ...
    // Vẽ cho Bản Ngang
    window.ctx = ctxH;
    drawCharSafe(ctxH, window.p1, 450, 850, 4.5, true, `hsl(${hue1}, 100%, 60%)`);
    drawCharSafe(ctxH, e1, 1470, 850, 4.5, false, `hsl(${hue2}, 100%, 60%)`);

    // Vẽ cho Bản Dọc
    window.ctx = ctxV;
    drawCharSafe(ctxV, window.p1, 540, 1600, 5.5, true, `hsl(${hue1}, 100%, 60%)`);
    drawCharSafe(ctxV, e1, 540, 700, 5.5, false, `hsl(${hue2}, 100%, 60%)`);
    ...
    const drawClickbaitProps = (ctx, w, h, isVertical) => {
        ctx.save();
        if (Math.random() > 0.2) {
            let cx = isVertical ? w/2 : w/2 + (Math.random() > 0.5 ? 250 : -250);
            let cy = isVertical ? h*0.4 + Math.random()*200 : h/2 + Math.random()*100;
            ...
            ctx.font = "150px Arial"; ctx.fillText(rx > w/2 ? "⬅️" : "➡️", cx + (rx > w/2 ? -200 : 100), cy);
        }

        if (Math.random() > 0.4) {
            let cx = isVertical ? w*0.7 : w*0.75; let cy = isVertical ? h*0.6 : h*0.35;
            ...
            ctx.fillStyle = "#fff"; ctx.fillText("-999,999 💢", -5,-5);
        ...
        if (Math.random() > 0.5) {
            ...
            let sx = isVertical ? w*0.3 : w*0.25; let sy = isVertical ? h*0.85 : h*0.75;
    ...
    // 4. TIÊU ĐỀ ĐÙN KHỐI 3D (3D EXTRUDED TEXT)
    const draw3DTitle = (ctx, w, h, text, isVertical) => {
        ...
        ctx.translate(w/2, isVertical ? h*0.15 : h*0.15); 
        ctx.rotate(-0.06);
        ctx.textAlign = "center"; ctx.textBaseline = "middle";
        
        let fontSize = isVertical ? 110 : 130;
        ...
        // Vẽ chữ VS ngầu
        ctx.font = `italic 900 ${fontSize*1.8}px 'Arial Black', Impact`;
        let vsY = isVertical ? -fontSize*2 : fontSize;
        ctx.lineWidth = 35; ctx.strokeStyle = "#000"; ctx.strokeText("VS", 0, vsY);
        ctx.fillStyle = "#fff"; ctx.fillText("VS", 0, vsY);
        ctx.fillStyle = "#ff4757"; ctx.fillText("VS", -8, vsY - 8);
        ctx.restore();
    };

    let shortTitle = (titleText || "EPIC STICKMAN FIGHT").replace(/#.*/g, '').trim();
    draw3DTitle(ctxH, 1920, 1080, shortTitle, false);
    draw3DTitle(ctxV, 1080, 1920, true);
};
...
window.startRecording = function() {
    ...
    // TẠO THUMBNAIL VÀ ĐẶT TIME FRAME (1.5 GIÂY)
...
window.captureFrames = function() {
    ...
    // NHÚNG POSTER THUMBNAIL (GIỮ 1.5S RỒI SNAP-CUT)
    if (window.thumbnailHoldFrames > 0) {
        ...
        return; // Ép dừng capture khung hình game
    }
