// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 10.0
// [PHIÊN BẢN NEXT-GEN - TỐI ƯU HÓA CHỐNG LAG MỨC TỐI ĐA]
// [ĐỒ HỌA 2.5D PARALLAX CHIỀU SÂU & HIỆU ỨNG ÁNH SÁNG BLOOM]
// ==========================================
...
// 🚀 [TỐI ƯU HÓA MỨC TỐI ĐA] Khởi tạo Offscreen Canvas 1 lần duy nhất để chống tràn RAM
...
// ==========================================
// 2. HỆ THỐNG SPAWN VFX & GOD-MODE API
// ==========================================
...
window.triggerTimeStop = function(frames, caster) { window.timeStopTimer = frames; window.timeStopCaster = caster; window.applyFilter('invert', frames); window.playSound(100, 'square', 1.0, 1.0, true); window.shakeScreen(20, 15); window.shockwaves.push({x: caster.x, y: caster.y, r: 10, maxR: 1500, color: "#fff", alpha: 1, speed: 40}); }
...
window.spawnEnvDamage = function(x, y, type, scale, isBurning = false) {
    let cracks = []; let numCracks = (type === 'crater') ? 8 + Math.floor(Math.random()*5) : 5 + Math.floor(Math.random()*3); let maxRadius = 0;
    for(let i=0; i<numCracks; i++) {
        let angle; if (type === 'crater') { let step = (Math.PI * 0.8) / Math.max(1, numCracks - 1); angle = Math.PI * 0.1 + step * i + (Math.random() - 0.5) * 0.2; }
        ...
        let len = (40 + Math.random()*60) * scale; if (len > maxRadius) maxRadius = len;
        let path = [{x: 0, y: 0}]; let segments = 4 + Math.floor(Math.random()*3); let currX = 0, currY = 0; let currAngle = angle;
        for(let s = 1; s <= segments; s++) {
            ...
            if (Math.random() < 0.45 && s < segments) {
                let branchAngle = currAngle + (Math.random() > 0.5 ? 1 : -1) * (0.5 + Math.random() * 0.5); let bLen = segLen * (0.6 + Math.random() * 0.6);
                cracks.push([ {x: currX, y: currY}, {x: currX + Math.cos(branchAngle) * bLen, y: currY + Math.sin(branchAngle) * bLen}, {x: currX + Math.cos(branchAngle)*bLen + Math.cos(branchAngle+(Math.random()-0.5))*bLen*0.5, y: currY + Math.sin(branchAngle)*bLen + Math.sin(branchAngle+(Math.random()-0.5))*bLen*0.5} ]);
            }
        }
        cracks.push(path);
    }
    for(let d=0; d < 12 * scale; d++) { window.particles.push({ x: x + (Math.random()-0.5)*30, y: y + (Math.random()-0.5)*20, vx: (Math.random()-0.5)*10, vy: -Math.random()*10 - 4, life: 60, maxLife: 60, color: isBurning ? "#e74c3c" : "#57606f", size: Math.random()*5 + 3, isRubble: true }); }
    ...
    if (window.envDamage.length > 5) { window.envDamage.shift(); }
};
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce) {
    ...
    if (target.state === 'block' && target.attackTimer >= 22) { 
        window.playSound(600, 'triangle', 0.4, 0.9, true); window.spawnParticles(target.x, target.y - 40, "#00ffff", true); window.shockwaves.push({x: target.x, y: target.y - 40, r: 5, maxR: 200, color: "#00ffff", alpha: 1, speed: 15});
    ...
    if (finalDmg > 0) {
        ...
        // 🌌 ĐỒ HỌA NEXT-GEN: Hit Sparks tia lửa bắn mạnh khi Crit
        if (isCrit) {
            for(let sp = 0; sp < 5; sp++) {
                window.impactSparks.push({ x: target.x, y: target.y - 40, vx: (Math.random()-0.5)*20, vy: (Math.random()-0.5)*20, life: 15, maxLife: 15, color: "#fff" });
            }
        }

        if (target.hp <= 0) {
            window.slowMoTimer = 60; window.screenFlash = 0; window.impactFrameTimer = 0; window.hitStopFrames = 8; 
...
window.attack = function(attacker, targetGroup) {
    ...
    if (isFinisher) {
        isCrit = true; finalDmg = baseDmg * 3.5; window.shakeScreen(15, 12); target.vx = (attacker.isFacingRight ? 5 : -5); target.state = 'hurt'; target.hitStun = 45;
        window.spawnParticles(target.x, target.y - 40, "#ff4757", true); window.floatingTexts.push({ x: target.x, y: target.y - 80, text: "💥", color: "#ff4757", alpha: 1, vx: (Math.random()-0.5)*2, vy: -4, font: "900 45px Arial", life: 50 });
    } else {
    ...
    let staminaGain = isCrit ? 5.0 : 1.5; attacker.stamina = Math.min(100, attacker.stamina + staminaGain);
    window.spawnSlash(target.x, target.y - 35, attacker.isFacingRight, isCrit ? "#ff4757" : "#ecf0f1", isCrit, isFinisher ? 1.8 : 1.2, slashAngle);
};

// ==========================================
// 5. VÒNG LẶP UPDATE CHÍNH (VẬT LÝ & TƯƠNG TÁC)
// ==========================================
window.update = function() {
    ...
    if (window.screenFlash > 0) window.screenFlash -= 0.05; else if (window.screenFlash < 0) { window.screenFlash += 1; if (window.screenFlash > 0) window.screenFlash = 0; }
    ...
    if (!isTimeStopped) {
        for (let i = window.envDamage.length - 1; i >= 0; i--) {
            ...
            if (dmg.life !== undefined) {
                dmg.life--; if (dmg.life <= 0) { window.envDamage.splice(i, 1); continue; }
                if (dmg.isBurning && Math.random() < 0.15) { window.particles.push({ x: dmg.x + (Math.random()-0.5) * dmg.radius * 0.8, y: window.GROUND_Y, vx: (Math.random()-0.5), vy: -Math.random()*5 - 1, life: 40, maxLife: 40, color: Math.random() > 0.4 ? "#e74c3c" : "#f1c40f", size: Math.random()*4+1 }); }
            }
        }
    }
    ...
    if (!isTimeStopped) {
        for (let i = window.envHazards.length - 1; i >= 0; i--) {
            ...
            if (haz.timer <= 0) {
                if (haz.type === 'lightning') {
                    ...
                    for(let k=0; k<15; k++) window.particles.push({ x: haz.x + (Math.random()-0.5)*40, y: window.GROUND_Y, vx: (Math.random()-0.5)*12, vy: -10 - Math.random()*15, life: 40, maxLife: 40, color: "#e67e22", size: Math.random()*12+5 });
        ...
        window.weatherParticles.forEach(w => { 
            if (window.currentWeather === 'toxic' || window.currentWeather === 'ash') { w.y -= w.speed * 0.5; w.x += Math.sin(w.y/30)*2 + window.globalWind; if(w.y < -20) { w.y = window.canvas.height + 20; w.x = Math.random() * 1200 - 300; } } 
            else { w.y += w.speed; w.x += ((window.currentWeather === 'rain') ? -3 : Math.sin(w.y/50)*2) + window.globalWind; if(w.y > window.canvas.height + 20) { w.y = -20; w.x = Math.random() * 1200 - 300; } }
        });
        ...
        // 🚀 [TỐI ƯU HÓA] Giới hạn số lượng rác bộ nhớ (Array Capping) chống giật lag khi chơi lâu
        if (window.particles.length > 250) window.particles = window.particles.slice(-250);
        if (window.floatingTexts.length > 50) window.floatingTexts = window.floatingTexts.slice(-50);
        if (window.impactSparks.length > 60) window.impactSparks = window.impactSparks.slice(-60);
    }
    ...
    allFighters.forEach(f => {
        ...
        if (f.hp <= 0) { 
            ...
            if (f.vy > 0 && f.y + f.vy >= window.GROUND_Y && !f.onGround) { window.spawnDust(f.x, window.GROUND_Y); if (f.vy > 8 || f.state === 'ko_falling') { window.shakeScreen(f.vy > 10 ? 8 : 5, 4); window.spawnEnvDamage(f.x, window.GROUND_Y, 'crater', f.scale || 1, false); } else if (f.vy > 6) { window.shakeScreen(4, 2); } } 
            f.y += f.vy; f.x += f.vx; f.vx *= 0.93; if (f.y >= window.GROUND_Y) { f.y = window.GROUND_Y; f.vy = 0; f.vx = 0; f.onGround = true; f.state = 'dead'; } return; 
        }
        ...
        if (window.currentWeather === 'snow') { f.currentSpeed *= 0.65; } else if (window.currentWeather === 'rain') { f.currentSpeed *= 1.25; } else if (window.currentWeather === 'ash') { f.currentDmgMod *= 1.30; } else if (window.currentWeather === 'toxic') { f.currentDmgMod *= 0.80; if (window.matchTimer % 90 === 0 && f.hp > 1 && !window.gameOver) { f.hp -= 1; window.particles.push({x: f.x, y: f.y-30, vx:0, vy:-1, life:20, maxLife:20, color:"#2ecc71", size:4}); } }
            ...
            if (f.hitStun <= 0 && f.stunTimer <= 0 && f.state !== 'dash_back' && f.state !== 'block') {
                ...
                if (isCloseEnough) {
                    if (window.classStats && window.classStats[type] && typeof window.classStats[type].executeUltimate === 'function') {
                        ...
                        window.playSound(400, 'sine', 0.5, 0.6); window.shakeScreen(15, 10); window.spawnParticles(f.x, f.y, "#f1c40f", true); let ultText = f.isPlayer ? "🔥 ULTIMATE!" : "⚠️ DANGER!"; let ultColor = f.isPlayer ? "#ff4757" : "#ff0000";
            ...
            for (let j = i + 1; j < allFighters.length; j++) { 
                ...
                if (dist < 80 && f1.attackTimer > 5 && f1.attackTimer < 20 && f2.attackTimer > 5 && f2.attackTimer < 20) {
                    if (!f1.clashCooldown && !f2.clashCooldown && f1.isFacingRight !== f2.isFacingRight) {
                        ...
                        window.shockwaves.push({x: midX, y: midY, r: 5, maxR: 200, color: "#ffffff", alpha: 1, speed: 15});
    ...
    allFighters.forEach(f => {
        if (!f || f.hp <= 0) return; let wallBound = 35 * (f.scale || 1);
        
        if (f.x < wallBound) { 
            f.x = wallBound; 
            if (f.hitStun > 0 && f.vx < -8 && !f.wallBounced) { f.wallBounced = true; f.vx = 0; f.vy = 0; f.state = 'wall_splat'; f.hitStun = 50; window.shakeScreen(15, 8); window.chromaTimer = 10; if (!f.wallDamageCooldown) { window.spawnEnvDamage(wallBound, f.y, 'wall_left', f.scale || 1, false); f.wallDamageCooldown = 60; } if(typeof window.takeDamage === 'function') window.takeDamage(f, Math.floor(Math.random() * 5) + 15, "#f1c40f", true, false); window.playSound(200, 'square', 0.4, 0.8, true); window.spawnDust(f.x, f.y); window.floatingTexts.push({ x: f.x, y: f.y - 80, text: "💥 WALL SPLAT!", color: "#f1c40f", alpha: 1, vx: 2, vy: -2, font: "900 30px Arial", life: 40 }); }
    ...
    if (window.p1 && window.introTimer === 0) {
        ...
        if (window.gameOver && closest) {
            ...
            let maxPanX = window.canvas.width * 0.85; let desiredCamX = (window.canvas.width / 2) - midX; window.targetCamX = Math.max(-maxPanX, Math.min(maxPanX, desiredCamX));
            let jumpFollowY = (window.GROUND_Y - midY) * 0.5; window.targetCamY = Math.max(0, Math.min(280, jumpFollowY)); 
            ...
            // 🌌 [ĐỒ HỌA NEXT-GEN] Nhịp đập Camera giật thót khi máu thấp tăng sự kịch tính
            if (p1Low && eLow) { window.targetTilt = 0.05 * Math.sin(window.matchTimer * 0.1); } 
            else if (p1Low || eLow) { window.targetTilt = 0.03 * Math.sin(window.matchTimer * 0.06); } 
...
// ==========================================
// 6. HỆ THỐNG VẼ ĐỒ HỌA TRUNG TÂM (DRAW) - NEXT GEN
// ==========================================
window.draw = function() {
    if (!window.canvas) { window.canvas = document.getElementById("battleCanvas"); if(window.canvas) window.ctx = window.canvas.getContext("2d"); } 
    if (!window.canvas || !window.ctx) return;
    
    // Resize Offscreen Canvas để chuẩn khớp với kích thước thật (Chống méo hình)
    if (window.offscreenCanvas && (window.offscreenCanvas.width !== window.canvas.width || window.offscreenCanvas.height !== window.canvas.height)) {
    ...
    try {
        ...
        groundGrad.addColorStop(0, cmap.ground); groundGrad.addColorStop(1, "#000000"); 
            ...
            window.envDamage.forEach(dmg => {
                ...
                dmg.cracks.forEach(path => {
                    if (Array.isArray(path) && path.length > 0) {
                        if (dmg.isBurning) {
                            window.ctx.strokeStyle = `rgba(255, 50, 0, ${0.8 * alpha})`; window.ctx.lineWidth = baseLw + 2; window.ctx.shadowBlur = 15; window.ctx.shadowColor = "#ff4757";
                        } else {
                            window.ctx.strokeStyle = `rgba(10, 10, 10, ${0.5 * alpha})`; window.ctx.lineWidth = baseLw + 2; window.ctx.shadowBlur = 5; window.ctx.shadowColor = "#000";
                        }
        ...
        // 🌌 ĐỒ HỌA NEXT-GEN: Glow nhẹ cực đẹp cho Projectiles
        window.projectiles.forEach(proj => { 
            window.ctx.save();
            window.ctx.shadowBlur = 15; window.ctx.shadowColor = proj.color;
        ...
        window.lasers.forEach(l => {
            ...
            window.ctx.shadowBlur = 20; window.ctx.shadowColor = l.color; window.ctx.fillStyle = l.color;
            let currentWidth = l.width * (0.8 + Math.random() * 0.4); let startX = l.x; let endX = l.isRight ? window.canvas.width + 500 : -500;
            window.ctx.fillRect(l.isRight ? startX : endX, l.y - currentWidth/2, Math.abs(endX - startX), currentWidth);
            window.ctx.fillStyle = "#fff"; window.ctx.fillRect(l.isRight ? startX : endX, l.y - currentWidth/4, Math.abs(endX - startX), currentWidth/2);
        ...
        window.customObjs.forEach(obj => {
            window.ctx.save(); window.ctx.translate(obj.x, obj.y);
            if (obj.spin) window.ctx.rotate(obj.ang);
            window.ctx.font = obj.font || "30px Arial"; window.ctx.fillStyle = obj.color || "#fff";
            window.ctx.textAlign = "center"; window.ctx.textBaseline = "middle";
            window.ctx.fillText(obj.text, 0, 0); window.ctx.restore();
        });
        ...
        // 🌌 ĐỒ HỌA NEXT-GEN: Các tia lửa ImpactSparks phát sáng xé gió
        window.impactSparks.forEach(isp => { 
            window.ctx.save(); window.ctx.translate(isp.x, isp.y); window.ctx.globalAlpha = Math.max(0, Math.min(1, isp.life / isp.maxLife)); 
            window.ctx.shadowBlur = 8; window.ctx.shadowColor = isp.color; window.ctx.fillStyle = isp.color; 
            window.ctx.beginPath(); let len = Math.sqrt(isp.vx*isp.vx + isp.vy*isp.vy) * 2; let ang = Math.atan2(isp.vy, isp.vx); 
            window.ctx.rotate(ang); window.ctx.ellipse(0, 0, len, 1.5, 0, 0, Math.PI*2); window.ctx.fill(); window.ctx.restore(); 
        });
        ...
        if (cmap.id === 'matrix_grid' || cmap.id === 'river_styx' || cmap.weather === 'snow' || cmap.weather === 'rain') {
            window.ctx.save();
            window.ctx.translate(0, window.GROUND_Y); window.ctx.scale(1, -0.4); window.ctx.translate(0, -window.GROUND_Y); window.ctx.globalAlpha = 0.15;
        ...
        // 🌌 ĐỒ HỌA NEXT-GEN: Đổ bóng mượt mà hơn
        ...
        allFighters.forEach(p => {
            if (p && p.hp >= 0) {
                ...
                let shadowGrad = window.ctx.createRadialGradient(p.x, window.GROUND_Y, 0, p.x, window.GROUND_Y, 35 * shadowScale);
                shadowGrad.addColorStop(0, `rgba(0, 0, 0, ${0.5 * shadowScale})`);
                shadowGrad.addColorStop(1, `rgba(0, 0, 0, 0)`);
                window.ctx.fillStyle = shadowGrad; window.ctx.beginPath(); window.ctx.ellipse(p.x, window.GROUND_Y, 35 * shadowScale, 7 * shadowScale, 0, 0, Math.PI * 2); window.ctx.fill();
            }
        });
        window.ctx.restore();
        ...
        // 🌌 ĐỒ HỌA NEXT-GEN: Thêm lớp Tiền Cảnh (Foreground Parallax) mờ lướt qua màn hình tạo chiều sâu
        window.ctx.save();
        window.ctx.translate(window.camX * 0.6, window.camY * 0.6); // Lớp này dịch chuyển ngược và nhanh hơn Camera
        for(var i = -800; i < window.canvas.width + 1200; i += 400) {
            let fgType = cmap.bg1Type || "city";
            window.ctx.fillStyle = `rgba(0, 0, 0, 0.45)`; // Bóng mờ cận cảnh
            if (fgType === "city" || fgType === "pillars") {
                window.ctx.fillRect(i, window.GROUND_Y - 250, 80, window.canvas.height);
            } else if (fgType === "trees" || fgType === "pines") {
                window.ctx.beginPath(); window.ctx.arc(i+40, window.GROUND_Y - 150, 100, 0, Math.PI*2); window.ctx.fill();
                window.ctx.fillRect(i+20, window.GROUND_Y - 100, 40, window.canvas.height);
            } else if (fgType === "ruins") {
                window.ctx.fillRect(i, window.GROUND_Y - 100, 120, window.canvas.height);
                window.ctx.clearRect(i+20, window.GROUND_Y - 50, 80, 100);
            }
        }
        window.ctx.restore();
        ...
        // 🌌 ĐỒ HỌA NEXT-GEN: Pulsing Vignette Điện Ảnh nhịp đập khi máu thấp
        let isLowHP = (window.p1 && window.p1.hp < window.p1.maxHp * 0.2 && !window.gameOver);
        let pulseGlow = isLowHP ? (0.65 + Math.sin(Date.now() / 150) * 0.2) : 0.65;
        ...
        vGrad.addColorStop(1, isLowHP ? `rgba(150, 0, 0, ${pulseGlow})` : `rgba(0,0,0,${pulseGlow})`);
        window.ctx.fillStyle = vGrad;
        ...
        if (window.p1 && window.p1.comboHits >= 2) {
            ...
            window.ctx.fillStyle = "#ff9f43"; window.ctx.textAlign = "left";
            window.ctx.shadowBlur = 10; window.ctx.shadowColor = "#ff9f43";
        ...
        if (maxEnemyCombo) {
            ...
            window.ctx.font = "italic 900 24px Arial"; 
            window.ctx.fillStyle = "#ff4757"; window.ctx.textAlign = "right";
            window.ctx.shadowBlur = 10; window.ctx.shadowColor = "#ff4757";
        ...
        if (window.gameOver && window.matchEndTimer >= 90 && window.endIconType) {
            ...
            window.ctx.translate(window.canvas.width / 2, window.canvas.height / 2); window.ctx.scale(easeScale * 1.2, easeScale * 1.2);
            window.ctx.font = "140px Arial"; window.ctx.textAlign = "center"; window.ctx.textBaseline = "middle";
        ...
        if (window.introTimer > 0 && !window.gameOver && window.p1) {
            ...
            if (window.introTimer > 60) {
                ...
                window.ctx.font = "italic 900 100px Arial"; window.ctx.fillStyle = "#ff4757"; window.ctx.strokeStyle = "#fff"; window.ctx.lineWidth = 4;
                window.ctx.shadowBlur = 30; window.ctx.shadowColor = "#ff4757"; window.ctx.strokeText("🥊 FIGHT! 🥊", 0, 30); window.ctx.fillText("🥊 FIGHT! 🥊", 0, 30); window.ctx.restore(); 
            }
        }
        ...
        // 🚀 [TỐI ƯU HÓA MỨC TỐI ĐA] Sử dụng Offscreen Canvas có sẵn, không gọi document.createElement nữa
        if (window.chromaTimer > 0 && window.offscreenCanvas && window.offscreenCtx) {
            let shift = window.chromaTimer * 1.5;
            window.offscreenCtx.clearRect(0, 0, window.canvas.width, window.canvas.height);
            window.offscreenCtx.drawImage(window.canvas, 0, 0);
            
            window.ctx.setTransform(1, 0, 0, 1, 0, 0);
            window.ctx.globalCompositeOperation = 'lighter';
            window.ctx.globalAlpha = 0.4;
            window.ctx.drawImage(window.offscreenCanvas, shift, 0);
            window.ctx.drawImage(window.offscreenCanvas, -shift, 0);
            window.ctx.globalAlpha = 1.0;
            window.ctx.globalCompositeOperation = 'source-over';
            window.chromaTimer--;
