// ==========================================
// RECORDER.JS - V67.5 (BUTTER SMOOTH + FIX 0-SECOND CAPCUT BUG)
// Nâng cấp V67.5: Tự động Inject Metadata Duration giúp video Edit được trên mọi App
// Giữ nguyên lõi V67.4: Frame Pacing 16.66ms, Alpha Blending Glitch, Text MaxWidth
// ==========================================

// [V67.5] Tự động tải thư viện sửa lỗi Duration 0 giây của WebM
if (typeof window.ysFixWebm === 'undefined') {
...
window._recordingStartTime = 0; // Biến tính toán thời lượng chuẩn
...
window.startRecording = function() {
    ...
    // [V67.5] BẮT ĐẦU BẤM GIỜ TẠI ĐÂY ĐỂ FIX LỖI CAPCUT
        ...
        if (stoppedCount === 2) { 
            setTimeout(() => {
                if (window.recordedChunksH.length === 0 || window.recordedChunksV.length === 0) {
                    alert("⚠️ Lỗi trích xuất video! Vui lòng F5 trang web, đợi 3 giây rồi mới bắt đầu chơi lại."); return; 
                }
                
                // [V67.5] TÍNH TOÁN DURATION VÀ INJECT METADATA
                let safeFileName = window.sanitizeFileName(window.StoryModeAI.viralTitle);
                let durationMs = Date.now() - window._recordingStartTime;
                
                let processVideo = (chunks, mimeType) => {
                    return new Promise((resolve) => {
                        let blob = new Blob(chunks, { type: mimeType });
                        // Chỉ fix lỗi cho file WebM nếu thư viện đã tải thành công
                        if (window.currentVideoExt === "webm" && typeof window.ysFixWebm === 'function') {
                            window.ysFixWebm(blob, durationMs, function(fixedBlob) { resolve(fixedBlob); });
                        } else {
                            resolve(blob);
                        }
                    });
