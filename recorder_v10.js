// ==========================================
// RECORDER.JS - BẢN TỐI THƯỢNG GIỮ CHÂN NGƯỜI DÙNG (V46.0)
// ĐÃ VÁ LỖI HIỂN THỊ, TỐI ƯU VÙNG AN TOÀN TIKTOK (SAFE ZONE)
// TÍCH HỢP: LƯỚI KHÔNG GIAN (CYBER-GRID), AUDIO-REACTIVE STICKER, THANH TIẾN TRÌNH
// ==========================================
...
// ==========================================
// 🧠 HỆ THỐNG STORYTELLING AI: KHO KIẾN THỨC RANDOM (GLOBAL TRIVIA)
// ==========================================
window.StoryModeAI = {
    scriptLines: [],      
    currentLineIndex: 0,  
    ...
    knowledgeCategories: [
        ...
        [
            ...
            "It was an illegal deal between students at Stanford University and MIT.",
            "Technically, e-commerce started with this trade way before Amazon existed.",
            "Technology has come a very long way since those wild days.",
            "And today, we have advanced AI fighter engines capable of absolute destruction.",
    ...
    playNextLine: function() {
        ...
        fetchPromise
            ...
            .catch(err => {
                ...
                // Cố gắng route fallback audio vào luồng ghi hình
                try {
                    let fbSrc = window.audioCtx.createMediaElementSource(fallbackAudio);
                    fbSrc.connect(window.masterRecordDestination);
                    fbSrc.connect(window.audioCtx.destination);
                } catch(e){}
...
// ==========================================
// RENDER KHUNG HÌNH (VÁ LỖI VÀ CHUẨN HOÁ TIKTOK SAFE ZONE)
// ==========================================
window.captureFrames = function() {
    ...
    try {
        ...
        // RENDER DỌC + LƯỚI KHÔNG GIAN (CYBER GRID)
        ...
        // =====================================
        // [RETENTION HACK] VẼ TIKTOK STICKER "WAIT FOR IT 🤯" BẬT THEO TIẾNG ĐỘNG
        // =====================================
        let audioPeak = 0;
        if (window.analyserData) audioPeak = window.analyserData[3] / 255; 
        
        ctxV.save();
        ctxV.translate(540, 250); 
        ctxV.rotate(-0.05);       
        let scaleSize = 1 + (audioPeak * 0.15); // Nhảy múa theo tiếng đấm
        ...
        // =====================================
        // VẼ SUB, SÓNG ÂM VÀ THANH TIẾN TRÌNH Ở SAFE ZONE (Y=1350)
        // =====================================
        if (window.StoryModeAI.displayedText.length > 0) {
            drawAudioVisualizer(ctxV, 540, 1310, 600, 100); 
            drawAudioVisualizer(ctxH, 960, 840, 600, 100);

            // TEXT VIDEO DỌC TIKTOK (Y=1350 an toàn không bị UI che)
            ctxV.save(); ctxV.textAlign = "center"; ctxV.textBaseline = "top";
            ctxV.font = "900 48px 'Montserrat', 'Arial Black', sans-serif";
            ctxV.fillStyle = "#f1c40f"; ctxV.strokeStyle = "#000000"; ctxV.lineWidth = 12; ctxV.lineJoin = "round";
            wrapText(ctxV, window.StoryModeAI.displayedText, 540, 1350, 950, 65);
            ctxV.restore();
            ...
            ctxH.fillStyle = "#f1c40f"; ctxH.strokeStyle = "#000000"; ctxH.lineWidth = 12; ctxH.lineJoin = "round";
        ...
        console.error("Lỗi Render Record Canvas:", err);
    }
};
