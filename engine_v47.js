// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 38.0 [CINEMATIC BEAUTY EDITION]
// [RESTORED] Giữ nguyên 100% logic V37.0.
// [NEW] Atmospheric Bokeh - Hạt ánh sáng mờ ảo tiền cảnh, tạo chiều sâu 3D làm dịu mắt.
// [NEW] Space Ripples - Vòng sóng ánh sáng không gian dày đặc khi bạo kích.
// [NEW] Cinematic Vignette - Đổ bóng góc viền giúp tập trung thị giác vào trung tâm.
// [BASE] Matrix 360 Camera, Lightning Arcs, Energy Pillars, Black Holes, RGB Split.
// ==========================================
...
// [BEAUTY UPGRADE 38.0] MẢNG VFX CHIỀU SÂU & GỢN SÓNG
...
// [BEAUTY UPGRADE 38.0] Vòng sóng không gian gợn
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce, attackerDirRight = true) {
    ...
    if (finalDmg > 0) {
        ...
        if (isCrit) {
            ...
            // [EPIC UPGRADE 37.0 & 38.0] Lightning & Space Ripples on Crit
        ...
        if (target.hp <= 0) {
            ...
            if (isNearWall && isCrit && typeof window.triggerStageTransition === 'function') { window.triggerStageTransition(target); } 
                ...
                if (isCrit) {
                    ...
                    // [EPIC UPGRADE 37.0 & 38.0] Bật Hố Đen & Energy Pillar & Ripple cực đại
...
window.update = function() {
    ...
    // [EPIC UPGRADE 37.0 & 38.0] Cập nhật mảng VFX mới
    ...
    if (!isTimeStopped) {
        ...
        for (let i = window.particles.length - 1; i >= 0; i--) { 
            ...
            // [EPIC UPGRADE 37.0] Lực hút hố đen
            if (activeBlackHole) {
    ...
    allFighters.forEach(f => {
        ...
        if (f.isRage) { 
            ...
            // [EPIC UPGRADE 37.0] Sét tự động văng ra khi Rage
...
window.draw = function() {
    ...
    try {
        ...
        if (window.p1) {
            ...
            // [VIRAL 37.0] ĐÂY LÀ CHỖ TÁCH MÀU CHROMA KHI BẠO KÍCH (RGB SPLIT PASSES)
        ...
        // [EPIC UPGRADE 37.0] VẼ HỐ ĐEN (BLACK HOLES)
        ...
        // [EPIC UPGRADE 37.0] VẼ ENERGY PILLARS
        window.ctx.globalCompositeOperation = 'lighter';
        ...
        // [EPIC UPGRADE 37.0] HYPER SLASHES 
        ...
        // [EPIC UPGRADE 37.0] VẼ SÉT ĐỘNG HỌC (LIGHTNING ARCS)
        window.ctx.globalCompositeOperation = 'lighter';
        ...
        // [BEAUTY UPGRADE 38.0] VẼ GỢN SÓNG KHÔNG GIAN DÀY (SPACE RIPPLES)
        window.ctx.globalCompositeOperation = 'lighter';
        ...
        if (window.bokehs.length > 0) {
            ...
            window.bokehs.forEach(b => {
                ...
                window.ctx.globalAlpha = Math.min(1, b.life / 50) * 0.4; // Rất dịu
                window.ctx.fillStyle = b.color;
                window.ctx.filter = 'blur(10px)'; // Nhòe quang học
        ...
        // [BEAUTY UPGRADE 38.0] VIGNETTE FOCUS (Làm tối góc viền để dịu mắt)
        window.ctx.save();
