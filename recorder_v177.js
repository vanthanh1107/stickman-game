// ==========================================
// RECORDER.JS - V76.6 SLEEK CINEMATIC UI EDITION
// ĐÃ FIX & GIỮ NGUYÊN: Tải tức thì (Instant Download), Story Mode Dialogue, Anime Slash.
// NÂNG CẤP V76.6 (BEAUTY MAX): 
// 1. ESPORTS HUD: Giao diện máu, avatar hình tròn phát sáng, tối giản, thanh lịch.
// 2. GLASSMORPHISM CHAT: Khung chat trong suốt viền Neon, mở rộng không gian video.
// 3. PREMIUM DIALOGUE: Hộp thoại Story Mode chuẩn game AAA, Cinematic bars mượt mà.
// ==========================================
...
// ==========================================
// HỆ THỐNG CỐT TRUYỆN (STORY MODE)
// ==========================================
window.CINEMATIC_QUOTES_TEMPLATE = [
    { triggerHp: 0.8, speaker: "P2", text: "Ngươi nghĩ mình có thể đánh bại ta sao? Quá ngây thơ!", duration: 4000 },
    { triggerHp: 0.6, speaker: "P1", text: "Ta không chiến đấu vì chiến thắng, ta chiến đấu vì lý tưởng!", duration: 4000 },
    { triggerHp: 0.4, speaker: "P2", text: "Lý tưởng của ngươi thật thảm hại. Hãy nếm mùi tuyệt vọng đi!", duration: 4000 },
    { triggerHp: 0.2, speaker: "P1", text: "Dù cơ thể này gục ngã... tinh thần ta vẫn sẽ cháy sáng!", duration: 4500 }
];
window._storyQuotes = [];
...
// ==========================================
// VẼ CINEMATIC QUOTES (STORY MODE DIALOGUE) - PREMIUM UI
// ==========================================
window.drawCinematicQuote = function(ctx, quote, elapsed, w, h) {
    ctx.save();
    let alpha = 1;
    // Fade in/out mượt mà
    ...
    // Cinematic bars thu gọn lại (120px) cho tinh tế hơn
    ctx.fillStyle = "#000";
    ctx.fillRect(0, 0, w, 120);
    ctx.fillRect(0, h - 120, w, 120);
    
    // Làm mờ toàn bộ khung hình phía sau (Dark overlay)
    ctx.fillStyle = "rgba(0,0,0,0.5)";
    ctx.fillRect(0, 120, w, h - 240);

    // Premium Dialogue Box - Đặt phía dưới
    let boxW = w - 80;
    let boxH = 180;
    let boxX = 40;
    let boxY = h - 120 - boxH - 30;

    // Gradient nền hộp thoại tinh tế (Từ màu nhân vật nhạt dần ra đen)
    ...
    // Viền trái sáng accent line (Đặc trưng của UI hiện đại)
    ctx.fillStyle = themeColor;
    if(ctx.roundRect) {
        ctx.beginPath(); ctx.roundRect(boxX, boxY, 8, boxH, {tl: 15, bl: 15, tr: 0, br: 0}); ctx.fill();
    } else {
        ctx.fillRect(boxX, boxY, 8, boxH);
    }

    // Tên nhân vật (Speaker Name)
    ...
    ctx.fillText(speakerName, boxX + 35, boxY + 25);
    
    // Typewriter Text (Hiệu ứng gõ chữ từng ký tự)
    let charsToShow = Math.floor((elapsed - 400) / 35); // 35ms/ký tự
    if (charsToShow < 0) charsToShow = 0;
    let displayedText = quote.text.substring(0, charsToShow);
    
    ctx.fillStyle = "#e2e8f0"; // Màu xám sáng dịu mắt
    ctx.font = "600 30px 'Segoe UI', Arial, sans-serif";
    ctx.textBaseline = "top";
    
    // Tự động xuống dòng (Word wrap) mượt mà
    let words = displayedText.split(" ");
    let line = "";
    let lineY = boxY + 75;
    for (let i = 0; i < words.length; i++) {
        let testLine = line + words[i] + " ";
        if (ctx.measureText(testLine).width > boxW - 70 && i > 0) {
            ctx.fillText(line, boxX + 35, lineY);
            line = words[i] + " ";
            lineY += 40;
        } else {
            line = testLine;
    ...
    // Blinking cursor ở cuối chữ
    if (Math.floor(Date.now() / 300) % 2 === 0 && charsToShow < quote.text.length) {
        let cursorX = boxX + 35 + ctx.measureText(line).width;
        ctx.fillStyle = themeColor;
...
window.startRecording = function() {
    ...
    window._storyQuotes = JSON.parse(JSON.stringify(window.CINEMATIC_QUOTES_TEMPLATE));
...
window.captureFrames = function() {
    ...
    if (renderNormalV) {
        ...
        // VIỀN NGĂN CÁCH GAME VÀ UI DƯỚI (Phát sáng neon xịn xò)
        ctxV.save();
        ...
        if (window._activeQuote) {
            ...
            if (elapsedQuote < window._activeQuote.duration) {
                window.drawCinematicQuote(ctxV, window._activeQuote, elapsedQuote, 1080, splitGameHeight);
        ...
        // ==========================================
        // VẼ PHẦN DƯỚI (UI MỚI SIÊU GỌN, ĐẸP)
        // ==========================================
        let retainY = splitGameHeight; let retainHeight = 1920 - retainY;
        
        // Background tối giản, sang trọng thay vì Grid cục mịch
        let botGrad = ctxV.createLinearGradient(0, retainY, 0, 1920);
        botGrad.addColorStop(0, "#080b12"); // Xanh đen siêu tối
        botGrad.addColorStop(1, "#151122"); // Tím than cực tối
        ctxV.fillStyle = botGrad; 
        ctxV.fillRect(0, retainY, 1080, retainHeight); 
        
        // Banner chạy gọn gàng nằm ngay dưới game
        if (!isOutroActive) {
        ...
        if (!isOutroActive) {
            ...
            // ==========================================
            // ESPORTS HUD REDESIGN (HIỆN ĐẠI HƠN)
            // ==========================================
            if (window.p1) {
                ...
                // Chữ VS ở giữa siêu tinh tế
                ctxV.textAlign = "center"; ctxV.textBaseline = "middle";
                ctxV.font = "italic 900 45px 'Arial Black'";
                ctxV.fillStyle = "#334155";
                ctxV.fillText("VS", 540, hudBaseY);
                ctxV.fillStyle = "#fff";
                ctxV.fillText("VS", 537, hudBaseY - 3);
            }

            // ==========================================
            // POLL SECTION (ĐẶT LÊN TRÊN CHAT CHO GỌN)
            // ==========================================
            if (window.p1 && repEnemyObj) {
                ...
                // Nút VS tròn ngăn giữa thanh bình chọn
            ...
            // ==========================================
            // GLASSMORPHISM LIVE CHAT TỐI ƯU
            // ==========================================
            ...
            // Nền kính trong suốt sang trọng
            ctxV.fillStyle = "rgba(15, 23, 42, 0.4)"; 
            ctxV.strokeStyle = "rgba(255, 255, 255, 0.1)"; ctxV.lineWidth = 1;
            ...
            // Viền Accent phát sáng trên cùng
            ...
            // Highlight chat box khi âm thanh to (Hype)
...
// ========================================================
// LƯU TRỰC TIẾP VIDEO (INSTANT DOWNLOAD)
// ========================================================
...
// ========================================================
// STUDIO ARCHIVE UI (HTML) ĐƯỢC REDESIGN SANG TRỌNG HƠN
// ========================================================
window.updateVideoListUI = function() {
    ...
    if (!container) { 
        ...
        // Dark theme trong suốt, viền mỏng tinh tế
