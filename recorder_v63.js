// ==========================================
// RECORDER.JS - BẢN HOÀN HẢO CHO CONTENT CREATOR (V66.12 - 120HZ OVERDRIVE ENGINE)
// [ULTIMATE FIX] Nâng tốc độ bắt khung hình lên 120 FPS triệt tiêu hoàn toàn Micro-stutter.
// [ULTIMATE FIX] Bật 'desynchronized: true' ép Canvas render thẳng qua GPU (Zero Lag).
// [ULTIMATE FIX] Xóa bỏ Chunking 1s, quay liền mạch để video không bị khựng nhịp nối.
// [RETAINED] Toàn bộ hệ thống giữ chân người xem (TikTok Meta, Profile Magnet, 3D Grid).
// ==========================================
...
window.initRecorder = function() {
    // [FIX 120Hz] Thêm cờ desynchronized: true và willReadFrequently: true để ép GPU vẽ với độ trễ bằng 0
    ...
    window.recordCtxH = window.recordCanvasH.getContext("2d", {alpha: false, desynchronized: true, willReadFrequently: true});
    ...
    window.recordCtxV = window.recordCanvasV.getContext("2d", {alpha: false, desynchronized: true, willReadFrequently: true});
};
...
window.startRecording = function() {
    ...
    // [FIX 120Hz] Ép trình duyệt chụp ảnh màn hình game với tần số 120 FPS để loại bỏ hoàn toàn Jitter (Lệch nhịp)
    ...
    // [FIX TỐI THƯỢNG] Ép Bitrate lên 35 Mbps và Ưu tiên dùng Card màn hình (H264/AVC) để nén
    let options = { videoBitsPerSecond: 35000000 }; 
    window.currentVideoExt = "mp4";
    let mimeType = "";

    // Tìm kiếm Codec phần cứng tốt nhất
    ...
    // [FIX 120Hz] Khởi chạy liền mạch không cắt lát, chống đơ hình ở các giây chuyển tiếp
