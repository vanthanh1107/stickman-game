// ==========================================
// RECORDER.JS - V76.5 THE RETENTION GOD (VIDEO FACECAM EDITION)
// ĐÃ FIX: Intro, Vỡ kính, Kill-Cam, Bảng Chat đụng Win Rate, Thông báo Follow ngang LIVE.
// UPDATE MỚI: Hỗ trợ nạp VIDEO Streamer (.mp4) thay vì ảnh tĩnh. Xóa UI rườm rà ở Facecam.
// DỜI VỊ TRÍ: Chữ "Double Tap" dời ra giữa màn hình game (phía dưới) để không đè HUD.
// ==========================================
...
// ==========================================
// THÊM LINK VIDEO STREAMER CỦA BẠN VÀO ĐÂY (Phải là link file .mp4 hoặc .webm trực tiếp)
// Ví dụ: "https://domain.com/video-streamer.mp4"
window.FACECAM_VIDEO_URL = "https://www.w3schools.com/html/mov_bbb.mp4"; // Link video mẫu đang để tạm
// ==========================================
...
window._fakeCombo = 0; window._lastComboTime = 0; 
...
window.startRecording = function() {
    ...
    // Kích hoạt Video Streamer khi bắt đầu ghi hình
    if (!window._facecamVideo) {
        window._facecamVideo = document.createElement('video');
        window._facecamVideo.crossOrigin = "Anonymous";
        window._facecamVideo.autoplay = true;
        window._facecamVideo.loop = true;
        window._facecamVideo.muted = true; // Bắt buộc mute để trình duyệt cho phép tự động chạy (autoplay)
        window._facecamVideo.playsInline = true;
        window._facecamVideo.src = window.FACECAM_VIDEO_URL;
    }
    window._facecamVideo.play().catch(e => console.log("Video Facecam cần tương tác để chạy: ", e));
...
window.stopRecording = function() { 
    ...
    if (window._facecamVideo) { window._facecamVideo.pause(); }
};
...
window.captureFrames = function() {
    ...
    if (renderNormalV) {
        ...
        if (Date.now() - window._lastComboTime > 1500) window._fakeCombo = 0; 
        ...
        // HIỆU ỨNG TÂM LÝ 2: CHIM MỒI DOUBLE TAP Ở GIỮA BÊN DƯỚI MÀN HÌNH GAME
        if (!isKillCamActive && elapsed > 3000) {
            ctxV.save();
            let baitScale = 1 + Math.sin(Date.now() / 150) * 0.15;
            // Đã dời ra vị trí trung tâm, cách lề dưới game 120px
            ctxV.translate(540, splitGameHeight - 120);
        ...
        // HỆ THỐNG THÔNG BÁO FOLLOW / ROSE TIKTOK (BOTTOM LEFT, BAY LÊN)
        ...
        let alertStartX = 20; 
        let alertStartY_Left = splitGameHeight - 15; 
        ...
        if (!isOutroActive) {
            ...
            // 3. FACECAM STREAMER DẠNG VIDEO (KHÔNG CÒN GIAO DIỆN THỪA)
            ctxV.save();
            ...
            // Nếu video stream load thành công và đang phát
            if (window._facecamVideo && window._facecamVideo.readyState >= 2) {
                let zoom = 1.0 + (audioPeak * 0.25);
                ctxV.save(); ctxV.scale(zoom, zoom);
                // Vẽ Video (giống như vẽ ảnh)
                ctxV.drawImage(window._facecamVideo, -camW/2, -camH/2, camW, camH);
                ctxV.restore();
            } else {
                // NẾU VIDEO LỖI HOẶC CHƯA LOAD ĐƯỢC -> HIỂN THỊ CHẾ ĐỘ "RAGE MODE"
