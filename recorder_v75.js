// ==========================================
// RECORDER.JS - V67.3 BUTTER SMOOTH 60 FPS (FULL VERSION)
// Tích hợp: Intro Manga, VTuber, HUD, Binge-Watch Grid, Full UI Archive
// Đột phá: captureStream(0) Sync, ShadowBlur Removal, Batch Path Rendering
// ==========================================
...
window.videoTrackH = null; window.videoTrackV = null; // Track reference cho Manual Frame Request
...
// Object Pooling: Khởi tạo sẵn mảng cố định tránh rác RAM
...
// [V67.3 OPTIMIZED] Loại bỏ hoàn toàn shadowBlur gây tụt FPS
...
window.startRecording = function() {
    ...
    // [V67.3] Tinh chỉnh Bitrate chuẩn 8 Mbps cho WebM VP8 mượt mà nhất
...
// Text Memoization (Cache dòng chữ để tránh CPU bị quá tải vì split và measureText)
...
// [V67.3 BATCH DRAWING] Gộp tất cả các cột Visualizer thành 1 đường Path duy nhất
function drawAudioVisualizer(ctx, x, y, width, height) {
    ...
    ctx.beginPath(); // Mở Path duy nhất
    ...
    ctx.fill(); // Tô màu 1 lần duy nhất cho toàn bộ cột
    ctx.restore();
}
...
window.captureFrames = function() {
    ...
    // ==========================================
    // 3. BACKGROUND LƯỚI KHÔNG GIAN 3D (TỐI ƯU PATH2D)
    // ==========================================
    ...
    ctxV.fill(); // Vẽ tất cả hạt bụi trong 1 lệnh duy nhất
    ctxV.globalCompositeOperation = "source-over";
    ...
    if (window.p1 && !window.gameOver && window.introTimer <= 120) {
        ...
        if (renderNormalH) {
            ...
            if (repEnemyObj) {
                ...
                ctxH.lineWidth = 10; ctxH.strokeStyle = "#000"; ctxH.strokeText(eName, 1730, 96); ctxH.fillStyle = "#fff"; ctxH.fillText(eName, 1730, 96);
    ...
    // ==========================================
    // 6. BINGE-WATCH GRID (GIẢ LẬP TRANG CÁ NHÂN TIKTOK)
    // ==========================================
    ...
    // [V67.3 FIX LÕI KHỰNG VIDEO] Ép Recorder chụp đúng khoảnh khắc khi Canvas vừa vẽ xong
