// ==========================================
// RECORDER.JS - V69.9 THE LORE EDITION (CINEMATIC STORYTELLING)
// ĐÃ FIX: Sinh ra Cốt truyện, Phụ đề điện ảnh, Viền đen Letterbox.
// CẢM XÚC: Như một trailer Anime ngắn 3.5 giây, có chiều sâu, có xung đột rõ ràng.
...
window.introDuration = 3500; // Tăng lên 3.5 giây để kể chuyện
...
// CÁC CỐT TRUYỆN (LORES) ĐỂ TẠO CHIỀU SÂU
const LORES = [
...
// ====================================================================
// ĐẠO DIỄN: CINEMATIC STORYTELLING INTRO
// ====================================================================
window.drawProceduralIntro = function(ctx, w, h, progress) {
    ...
    // 1. CẢNH BÁO MỞ ĐẦU (0.0 -> 0.05) - Nhanh (175ms)
    if (progress < 0.05) {
    ...
    // 2. CHIA GIAI ĐOẠN ĐẠO DIỄN (Timeline)
    // Cut 1: 0.05 -> 0.40 (Boss nói)
    // Cut 2: 0.40 -> 0.70 (Player đáp)
    // Cut 3: 0.70 -> 0.85 (Tích tụ Aura Silhouette)
    // Cut 4: 0.85 -> 1.0 (Lao vào + Flashbang)
    ...
    if (!window._sfxCuts[cutIndex]) {
        if (cutIndex < 2 && typeof window.playSound === 'function') window.playSound(100 + Math.random()*100, 'sine', 0.1, 0.8); // Tiếng Ù uuu cinematic
        if (cutIndex === 2 && typeof window.playSound === 'function') window.playSound(400, 'square', 0.2, 0.4); // Tích tụ
        if (cutIndex === 3 && typeof window.playSound === 'function') window.playSound(100, 'sawtooth', 0.6, 1.0, true); // Bùmm
    ...
    ctx.save();
    let cutShake = 0;
    if (cutIndex === 2) cutShake = (Math.random() - 0.5) * 40 * cutProg; // Rung mạnh dần lúc tích tụ
    if (cutIndex === 3 && cutProg < 0.2) cutShake = (Math.random() - 0.5) * 100; // Rung cực gắt lúc nổ

    // Panning (Lướt camera chậm phong cách điện ảnh)
    let panX = cutIndex < 2 ? (cutProg * 100) - 50 : 0; 
    
    ctx.translate(w/2 + cutShake + panX, h/2 + cutShake);
    ctx.rotate(cut.rot);
    let currentZoom = cut.zoom + (cutProg * 0.15); // Zoom chậm rãi
    if (cutIndex === 3) currentZoom = cut.zoom + (cutProg * 2); // Zoom giật vỡ lúc Clash
    ...
    // Hạt điện ảnh bay bay lơ lửng (Atmospheric Embers)
    ctx.fillStyle = bgTheme.aura; ctx.globalAlpha = 0.5;
    for(let i=0; i<40; i++) {
    ...
    const drawFighterArtistic = (charObj, cx, cy, forcedState, isFacingRight, theme, fx) => {
        ...
        else if (fx === 'cinematic') ctx.filter = "contrast(120%) saturate(110%)";
    ...
    if (cut.focus === 'p1') {
        drawFighterArtistic(window.p1, 0, 100, cut.state, true, prm.themeP1, cut.fx);
    } 
    else if (cut.focus === 'p2') {
        drawFighterArtistic(e1, 0, 100, cut.state, false, prm.themeP2, cut.fx);
    } 
    else { // Clash
    ...
    // IMPACT FRAMES Âm bản chớp giật lúc lao vào
    ...
    ctx.restore(); // THOÁT KHỎI CAMERA ZOOM ĐỂ VẼ OVERLAY (VIỀN ĐEN + SUB)

    // --- CINEMATIC LETTERBOX (VIỀN ĐEN TRÊN DƯỚI) ---
    // Viền đen giữ nguyên lúc nói chuyện, bắt đầu vỡ/mở ra lúc tích tụ và biến mất lúc Clash
    let barHeight = 250; // Cao 250px
    if (cutIndex === 2) barHeight = 250 - (cutProg * 100); // Rụt lại 1 chút tạo tension
    if (cutIndex === 3) barHeight = 0; // Biến mất hoàn toàn

    if (barHeight > 0) {
        ctx.fillStyle = "#000000";
        ctx.fillRect(0, 0, w, barHeight); // Cạnh trên
        ctx.fillRect(0, h - barHeight, w, barHeight); // Cạnh dưới
    }

    // --- CINEMATIC SUBTITLES (PHỤ ĐỀ NHƯ XEM PHIM) ---
    if (cut.txt && cutIndex < 2) {
        ctx.save();
        // Hiệu ứng chữ từ từ hiện ra (Fade in)
        ctx.globalAlpha = Math.min(1, cutProg * 3); 
        ctx.textAlign = "center"; ctx.textBaseline = "bottom";
        ctx.font = "bold 45px Arial";
        
        let subY = h - barHeight + 80; // Đặt đè lên viền đen dưới một chút cho đẹp
        
        // Vẽ viền đen cho chữ rõ
        ctx.lineWidth = 6; ctx.strokeStyle = "#000"; ctx.strokeText(`"${cut.txt}"`, w/2, subY);
        // Màu phụ đề vàng cinematic
        ctx.fillStyle = "#f1c40f"; ctx.fillText(`"${cut.txt}"`, w/2, subY);
        ctx.restore();
    }

    // --- SNAPPY TEXT (CHỮ NẢY LÊN LÚC TÍCH TỤ) ---
