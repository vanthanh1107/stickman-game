// ==========================================
// RECORDER.JS - BẢN HOÀN HẢO CHO CONTENT CREATOR (V61.0 - ULTRA SMOOTH 60FPS OPTIMIZATION)
// [FIXED] Khắc phục triệt để lỗi Video giật lag/rớt frame bằng Fast Pseudo-Bloom (Bỏ blur filter nặng nề).
// [FIXED] Sửa luồng MediaRecorder chạy nguyên khối mượt mà 60FPS, xả đệm an toàn chống file 0-byte.
// [UPGRADED] Tăng Video Bitrate lên 12Mbps cho hình ảnh sắc nét khi hành động tốc độ cao.
// [RETAINED] Màn hình Intro Manga Trash Talk, Thumbnail 3D Meme, Auto-Cut sau K.O.
// ==========================================
...
// Dữ liệu rác (Meme) lưu cho mỗi trận
...
// ==========================================
// 🎨 THUMBNAIL GENERATOR (TẠO POSTER MEME CỰC DỊ)
// ==========================================
...
window.drawAnimatedIntro = function(ctx, w, h, isVertical, progress) {
    ...
    ctx.lineWidth = 25; ctx.strokeStyle = "#fff";
    ...
    ctx.translate(p1X, p1TargetY); ctx.shadowColor = "#00f3ff"; ctx.shadowBlur = 50;
    if(typeof window.drawStickman === 'function') window.drawStickman(ctx, p1Clone);
    ctx.fillStyle = "#f1c40f"; ctx.font = "900 35px Arial Black"; ctx.fillText(window.statBadgeP1, -80, 50);
    ctx.restore();
    ...
    ctx.translate(p2X, p2TargetY); ctx.scale(-1, 1); ctx.shadowColor = "#ff003c"; ctx.shadowBlur = 50;
    ...
    const drawSpeechAndEmoji = (x, y, text, isP1, emojiStr, popProgress) => {
        ...
        let bounce = Math.sin(popProgress * Math.PI * 1.5) * (1 - popProgress) * 0.5 + 1;
        if (popProgress >= 1) bounce = 1;
        ctx.scale(bounce, bounce);

        let tw = ctx.measureText(text).width + 80; let th = 100;
        ctx.save(); ctx.transform(1, 0, -0.15, 1, 0, 0); 
        ctx.fillStyle = "#fff"; ctx.strokeStyle = "#000"; ctx.lineWidth = 10; ctx.shadowColor = "#000"; ctx.shadowOffsetX = 12; ctx.shadowOffsetY = 12;
        ctx.fillRect(-tw/2, -th/2, tw, th); ctx.strokeRect(-tw/2, -th/2, tw, th);
        ctx.beginPath(); 
        if(isP1) { ctx.moveTo(tw/2 - 30, 0); ctx.lineTo(tw/2 + 70, 70); ctx.lineTo(tw/2 - 50, 30); }
        else { ctx.moveTo(-tw/2 + 30, 0); ctx.lineTo(-tw/2 - 70, 70); ctx.lineTo(-tw/2 + 50, 30); }
        ctx.fill(); ctx.stroke(); ctx.restore(); 

        ctx.fillStyle = isP1 ? "#0055ff" : "#c0392b"; ctx.shadowOffsetX = 0; ctx.shadowOffsetY = 0;
        ctx.font = "italic 900 42px 'Arial Black', Impact"; ctx.textAlign = "center"; ctx.textBaseline = "middle";
        ctx.fillText(text, 0, 0);
        
        ctx.save();
        let emX = isP1 ? tw/2 + 40 : -tw/2 - 40; let emY = -th/2 - 20;
        let tPulse = Date.now() / 60; 
        ctx.translate(emX, emY + Math.sin(tPulse) * 20); ctx.rotate(Math.sin(tPulse * 0.5) * 0.5);
        ctx.font = "90px Arial"; ctx.shadowColor = "#000"; ctx.shadowBlur = 20; ctx.fillText(emojiStr, 0, 0);
        
        if(Math.random() > 0.5) { ctx.fillStyle = "#f1c40f"; ctx.font = "900 70px Impact"; ctx.fillText("⁉️", -100, -80); } 
        else { ctx.font = "70px Arial"; ctx.fillText("🧂", -100, -80); }
        ctx.restore();
        ctx.restore();
    };

    ctx.font = "italic 900 40px 'Arial Black', Impact"; 
    let p1Pop = Math.min(1, Math.max(0, (progress - 0.25) * 6)); 
    drawSpeechAndEmoji(isVertical ? w*0.5 : w*0.25, isVertical ? h*0.12 : h*0.3, window.trashTalkP1, true, window.introEmojiP1, p1Pop);
    
    let p2Pop = Math.min(1, Math.max(0, (progress - 0.45) * 6)); 
    drawSpeechAndEmoji(isVertical ? w*0.5 : w*0.75, isVertical ? h*0.88 : h*0.7, window.trashTalkP2, false, window.introEmojiP2, p2Pop);

    if (progress > 0.85) {
        let flashAlpha = Math.min(1, (progress - 0.85) * 6.6);
        ctx.globalCompositeOperation = 'screen';
        ctx.drawImage(ctx.canvas, -40 * flashAlpha, 0);
        ctx.fillStyle = `rgba(255, 0, 50, ${flashAlpha * 0.8})`; ctx.fillRect(-w, -h, w*3, h*3);
        ctx.drawImage(ctx.canvas, 40 * flashAlpha, 0);
        ctx.fillStyle = `rgba(0, 200, 255, ${flashAlpha * 0.8})`; ctx.fillRect(-w, -h, w*3, h*3);
        ctx.globalCompositeOperation = 'source-over';
...
window.startRecording = function() {
    ...
    // Yêu cầu luồng với 60fps để video mượt
    ...
    let options = { videoBitsPerSecond: 12000000 }; // 12 Mbps chống nhòe
    window.currentVideoExt = "webm";
    if (MediaRecorder.isTypeSupported('video/mp4;codecs=avc1,mp4a.40.2')) { options = { mimeType: 'video/mp4;codecs=avc1,mp4a.40.2', videoBitsPerSecond: 12000000 }; window.currentVideoExt = "mp4"; } 
    else if (MediaRecorder.isTypeSupported('video/webm;codecs=vp8,opus')) { options = { mimeType: 'video/webm;codecs=vp8,opus', videoBitsPerSecond: 12000000 }; window.currentVideoExt = "webm"; }
    ...
    // KHO TRASH TALK CHO MỖI TRẬN ĐẤU (Bốc ngẫu nhiên)
    ...
    window.thumbnailHoldFrames = 30; // 0.5s Thumbnail tĩnh
    window.introHoldFrames = window.totalIntroFrames; // 2.5s Intro Động
    ...
    const finalizeRecordings = () => {
        ...
        if (stoppedCount === 2) { 
            ...
            }, 800); // CHỜ 800ms ĐỂ XẢ HẾT ĐỆM, FIX 0-BYTE FILE
        }
    };

    window.mediaRecorderH.onstop = finalizeRecordings; window.mediaRecorderV.onstop = finalizeRecordings;
    
    // KHÔNG DÙNG start(1000) ĐỂ TRÁNH MẤT FRAME TRÊN PHẦN MỀM EDIT. CHỈ CHẠY LIỀN KHỐI.
...
window.stopRecording = function() { 
    ...
    // CHUẨN XÁC: GỌI requestData TRƯỚC KHI STOP ĐỂ ĐẢM BẢO CHUNK CUỐI CÙNG ĐƯỢC XUẤT RA
    if (window.mediaRecorderH && window.mediaRecorderH.state !== "inactive") { 
        try { window.mediaRecorderH.requestData(); } catch(e){}
        try { window.mediaRecorderH.stop(); } catch(e){} 
    }
    if (window.mediaRecorderV && window.mediaRecorderV.state !== "inactive") { 
        try { window.mediaRecorderV.requestData(); } catch(e){}
...
window.captureFrames = function() {
    ...
    // 🔥 TỰ ĐỘNG CẮT VIDEO SAU K.O KHOẢNG ~6 GIÂY
    ...
    // [CFR HACK] Vẽ một chấm cực nhỏ 1px thay đổi độ mờ liên tục ở góc để ép trình duyệt luôn render 60 FPS
    ...
    // GIAI ĐOẠN 1: IN THUMBNAIL TĨNH (0.5s)
    ...
    // GIAI ĐOẠN 2: INTRO MANGA TRASH TALK NẢY EMOJI VÀ ZOOM CẬN (2.5s)
    if (window.introHoldFrames > 0) {
        let progress = 1 - (window.introHoldFrames / window.totalIntroFrames);
        
        if (window.introHoldFrames === Math.floor(window.totalIntroFrames * 0.75)) window.playSound(400, 'square', 0.2, 0.6);
        if (window.introHoldFrames === Math.floor(window.totalIntroFrames * 0.50)) window.playSound(300, 'square', 0.2, 0.6);
    ...
    // GIAI ĐOẠN 3: CAPTURE GAMEPLAY BÌNH THƯỜNG (Dùng Pseudo-Bloom nhanh)
    ...
    // FAST BLOOM
    ctxH.globalCompositeOperation = 'screen'; ctxH.globalAlpha = 0.15 + (audioPeak * 0.1); 
    ctxH.drawImage(window.canvas, 0, 0, window.canvas.width, window.canvas.height, shakeX - 4, shakeY, 1928, 1080);
    ctxH.drawImage(window.canvas, 0, 0, window.canvas.width, window.canvas.height, shakeX + 4, shakeY, 1928, 1080);
    ctxH.globalAlpha = 1.0; ctxH.globalCompositeOperation = 'overlay';
    ctxH.fillStyle = "rgba(20, 30, 50, 0.15)"; ctxH.fillRect(0, 0, 1920, 1080);
    ctxH.globalCompositeOperation = 'source-over';
    ...
    // FAST BLOOM
    ctxV.globalCompositeOperation = 'screen'; ctxV.globalAlpha = 0.2 + (audioPeak * 0.15); 
    ctxV.drawImage(window.canvas, 0, 0, window.canvas.width, window.canvas.height, -424 + shakeX, 420 + shakeY, 1928, 1080);
    ctxV.drawImage(window.canvas, 0, 0, window.canvas.width, window.canvas.height, -416 + shakeX, 420 + shakeY, 1928, 1080);
    ctxV.globalAlpha = 1.0; ctxV.globalCompositeOperation = 'overlay';
    ctxV.fillStyle = "rgba(20, 30, 50, 0.2)"; ctxV.fillRect(0, 0, 1080, 1920);
    ctxV.globalCompositeOperation = 'source-over';
