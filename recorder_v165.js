// ==========================================
// RECORDER.JS - V76.3 THE ALGORITHM BREAKER (PERFECT LOOP META)
// ĐÃ FIX & GIỮ NGUYÊN: Intro God-Tier, Vỡ kính 3D, Kanji Manga, Kill-Cam, Viral Post Kit.
// NÂNG CẤP V76.2: Facecam Render Hậu Kỳ Không Giật Lag (Chỉ hiện lúc trong Game). 
// NÂNG CẤP V76.3 (NEW): Tích hợp Perfect Loop! Frame cuối Outro tự động mờ dần (Fade) 
//                       khớp hoàn toàn 100% với Frame đầu của Intro. Hack thời lượng xem TikTok!
// ==========================================
...
// QUẢN LÝ VIDEO FACECAM (Chỉ tải lúc bấm nút Render Tải Xuống)
...
window.startRecording = function() {
    ...
    // TỐI ƯU HÓA: Chỉ Random tên file Facecam, KHÔNG LOAD để bảo vệ CPU
    ...
    // Đặt lại các biến theo dõi thời gian
    ...
    window.mediaRecorderV.onstop = () => {
        setTimeout(() => {
            ...
            window.savedVideos.unshift({ 
                ...
                // Lưu thời gian bắt đầu và kết thúc gameplay để lát hậu kỳ ghép Facecam
...
window.captureFrames = function() {
    ...
    // THEO DÕI THỜI GIAN BẮT ĐẦU VÀ KẾT THÚC CỦA TRẬN GAME (Bỏ qua Intro/Outro)
    ...
    if (renderNormalV) {
        ...
        if (!isOutroActive) {
            ...
            // ========================================================
            // 3. FACECAM GÁI XINH (MÀN HÌNH CHỜ TRONG LÚC CHƠI GAME KHÔNG RUNG LẮC)
            // ========================================================
            ctxV.save();
            ...
            ctxV.fillStyle = "rgba(0,0,0,0.8)"; // Nền đen chờ ghép video hậu kỳ
            ctxV.fill();
            ...
            // ==========================================
            // TÍNH NĂNG ĐỘT PHÁ: PERFECT LOOP (VÒNG LẶP HOÀN HẢO)
            // Khớp frame kết thúc của Outro với Thumbnail đầu tiên.
            // ==========================================
            if (window.matchEndTimer > 250 && window.bakedThumbV) {
                let loopFadeAlpha = Math.min(1, (window.matchEndTimer - 250) / 100); 
                ctxV.save();
                ctxV.globalAlpha = loopFadeAlpha;
...
// ========================================================
// HÀM GHÉP FACECAM VÀO VIDEO (POST-PROCESSING RENDERING)
// ========================================================
window.mergeAndDownloadVideo = async function(vidId) {
    ...
    // CHÌA KHÓA BẤT TỬ: Đợi cả 2 video tải xong 100% meta data mới bắt đầu
    try {
        await Promise.all([
        ...
        if (!rawVid.paused) {
            ...
            if (isDuringGameplay) {
                ...
                if (vW > 0 && vH > 0) {
                    ...
                    // Vị trí tĩnh tuyệt đối, khớp hoàn toàn với cái lỗ màu đen
                    ...
                    // Đồ lại cái viền Neon cho Facecam sống động
                    ctx.save();
                    ctx.translate(camX, camY);
                    ctx.lineWidth = 5;
