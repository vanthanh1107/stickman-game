// ==========================================
// RECORDER.JS - V70.6 THE 3-SECOND HOOK (DIRECTOR'S CUT)
// ĐÃ FIX: Áp dụng kỹ thuật "In Medias Res" - Xóa màn hình đen, nhảy thẳng vào Action.
// CẬP NHẬT: Ép nhịp điệu xuống 3.2 giây. Thêm Strobe Flash FX và CRT Scanlines.
// CẢM XÚC: Hút mắt ngay 0.1s đầu tiên, không thể rời mắt, Action dồn dập.
...
// QUẢN LÝ INTRO ĐIỆN ẢNH (ÉP NHỊP XUỐNG 3.2 GIÂY)
...
const LORES = [
    ...
    { chapter: "ACT III: LAST STAND", p2Sub: "You have nothing left.", p1Sub: "I still have my anger." },
    { chapter: "EPISODE 4: THE REMATCH", p2Sub: "Back for another beating?", p1Sub: "I came back to bury you." },
    { chapter: "FINAL CHAPTER: ENDGAME", p2Sub: "Know your place, kid.", p1Sub: "Your era ends tonight." },
    { chapter: "ACT II: THE ANOMALY", p2Sub: "What kind of monster are you?!", p1Sub: "The glitch in your system." },
    { chapter: "EPILOGUE: VENGEANCE", p2Sub: "I don't even remember you.", p1Sub: "You will scream my name." }
];
...
window.generateLiveChatEvent = function() {
    ...
    const tmpls = [() => `THE LORE IS CRAZY 😭`, () => `Bro has anime protagonist aura ☢️`, () => `Absolute cinema 🍿🎬`, () => `Nah this edit is too clean 🔥`, () => `RIP BOZO 📉`];
...
// ====================================================================
// ĐẠO DIỄN: IN-MEDIAS-RES (THE 3-SECOND HOOK)
// ====================================================================
window.drawProceduralIntro = function(ctx, w, h, progress) {
    ...
    // TIMELINE MỚI: 3.2s Dồn dập
    let act = 0; let localProg = 0;
    if (progress < 0.3) { act = 0; localProg = progress / 0.3; } // 0 -> 960ms: Boss vỗ mặt
    else if (progress < 0.6) { act = 1; localProg = (progress - 0.3) / 0.3; } // 960ms -> 1920ms: Player đáp
    else if (progress < 0.75) { act = 2; localProg = (progress - 0.6) / 0.15; } // 1920ms -> 2400ms: STROBE FLASH Tension
    else { act = 3; localProg = (progress - 0.75) / 0.25; } // 2400ms -> 3200ms: Clash nổ tung

    // ÂM THANH THEO TIMELINE MỚI
    if (!window._sfxCuts[act]) {
        if (act === 0 && typeof window.playSound === 'function') window.playSound(100, 'sine', 0.5, 1.0); // BÙM Mở màn (Kéo sự chú ý)
        if (act === 1 && typeof window.playSound === 'function') window.playSound(200, 'triangle', 0.2, 0.5); // Vút chuyển cảnh
        if (act === 2 && typeof window.playSound === 'function') window.playSound(60, 'square', 0.5, 0.4); // Nhịp tim điện tử giật
        if (act === 3 && typeof window.playSound === 'function') window.playSound(120, 'sawtooth', 0.8, 1.5, true); // VỤ NỔ
    ...
    const drawFighterArtistic = (charObj, cx, cy, forcedState, isFacingRight, theme, charName) => {
        ...
        if (charName && act < 2) {
            ctx.save(); ctx.translate(0, -260); ctx.scale(0.85, 0.85); 
            ctx.textAlign = "center"; ctx.textBaseline = "middle";
            ctx.font = "900 34px 'Arial Black', sans-serif"; 
            ctx.shadowColor = theme.aura; ctx.shadowBlur = 20;
        ...
        let clone = Object.assign({}, charObj, {x:0, y:0, scale: 3.8, isFacingRight: true, state: forcedState}); // Scale to hơn chút cho cận cảnh
    ...
    // --- STROBE FLASH TENSION (ACT 2) - NHỊP ĐIỆU CỰC CĂNG ---
    if (act === 2) {
        let rumble = (Math.random() - 0.5) * 80; ctx.translate(rumble, rumble);
        
        // Chớp qua lại giữa 2 nhân vật mỗi 2 frame
        ...
        ctx.globalCompositeOperation = "difference"; // Âm bản đảo màu cực dị
        
        ctx.translate(w/2, h/2 + 100); ctx.scale(5,5); // Zoom kịch kim
        if (isP1Flash) drawFighterArtistic(window.p1, 0, 0, 'dash', true, tTheme, "");
        else drawFighterArtistic(e1, 0, 0, 'dash', false, tTheme, "");
    ...
    // --- SETUP CAMERA CHO ACT 0, 1, 3 ---
    ctx.save();
    let cutShake = 0; 
    if (act === 3 && localProg < 0.3) cutShake = (Math.random() - 0.5) * 120; // Rung tung nóc lúc nổ

    // Panning siêu tốc
    let panDirection = act === 0 ? 1 : -1;
    let panX = (localProg * 120) * panDirection - (60 * panDirection); 
    if (act === 3) panX = 0; 
    
    let currentZoom = act < 3 ? (4.2 + localProg * 0.5) : (3.0 + localProg * 3); 
    ...
    if (act === 0) {
    ...
    else if (act === 3) { // THE CLASH BÙNG NỔ
        ...
        let flareWidth = w * 3 * localProg; let flareHeight = 30 + Math.random() * 60;
        ...
        ctx.translate(0, 150);
        for(let i=0; i<40; i++) {
            let ang = Math.random() * Math.PI * 2; let dist = localProg * 1000 * Math.random();
            ctx.fillStyle = Math.random() > 0.5 ? "#fff" : prm.themeP1.aura;
            ctx.globalAlpha = 1 - localProg; ctx.fillRect(Math.cos(ang)*dist, Math.sin(ang)*dist, 25*(1-localProg), 5);
        }
    ...
    // --- CINEMATIC LETTERBOX ---
    let barHeight = 220; 
    if (act === 3) barHeight = 0; 
    if (barHeight > 0) {
        ctx.fillStyle = "#000000"; ctx.fillRect(0, 0, w, barHeight); ctx.fillRect(0, h - barHeight, w, barHeight); 
    }

    // --- LAYER 1: TÊN CHAPTER (CHỈ HIỆN Ở ACT 0 TRƯỢT NGANG VÀO) ---
    if (act === 0) {
        ctx.save();
        let chapterX = w/2 + (1-Math.pow(localProg, 0.3)) * 500; // Trượt từ phải sang
        ctx.textAlign = "center"; ctx.textBaseline = "middle";
        ctx.globalAlpha = Math.min(1, localProg * 3);
        ctx.fillStyle = prm.themeP2.aura; ctx.font = "italic 900 35px 'Arial Black', sans-serif"; ctx.letterSpacing = "6px";
        ctx.fillText(prm.lore.chapter, chapterX, barHeight / 2);
        ctx.restore();
    }

    // --- LAYER 2: PHỤ ĐỀ TRUYỆN (ACT 0, 1) ---
    if (act === 0 || act === 1) {
        ctx.save();
        let subText = act === 0 ? prm.lore.p2Sub : prm.lore.p1Sub;
        ctx.globalAlpha = Math.min(1, localProg * 5); // Pop lên siêu nhanh
    ...
    // --- LAYER 3: CRT SCANLINES CHO TOÀN BỘ INTRO ---
    drawCRTScanlines();
