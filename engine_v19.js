// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 17.5 [VIRAL & ESPORTS UI]
// [FIXED 100%: LỖI DÍNH HÌNH/GHOSTING BẰNG HARD-RESET MATRIX]
// [UI TỐI ƯU: COMBO SSS GỌN GÀNG, CHUYÊN NGHIỆP, KHÔNG CHE NHÂN VẬT]
// ==========================================
...
window.draw = function() {
    ...
    if (window.isLoading) {
        ...
        window.ctx.fillStyle = "#00f3ff"; window.ctx.font = "italic 900 40px Arial"; window.ctx.textAlign = "center"; window.ctx.shadowBlur = 15; window.ctx.shadowColor = "#00f3ff"; window.ctx.fillText("INITIALIZING VIRAL ENGINE 17.5...", cx, cy - 50);
    ...
    try {
        ...
        // [VIRAL 17.5] HỆ THỐNG DEVIL MAY CRY COMBO SSS RANKING (RESIZED & OPTIMIZED)
        let renderComboRank = function(fighter, xPos, align) {
            if (fighter && fighter.comboHits >= 2) {
                ...
                let rank = "D"; let rankColor = "#bdc3c7"; let rankSize = 18; let rankGlow = 5; let rankText = "NICE";
                if (hits >= 4) { rank = "C"; rankColor = "#2ecc71"; rankText = "COOL!"; }
                if (hits >= 6) { rank = "B"; rankColor = "#3498db"; rankSize = 20; rankText = "BRUTAL!"; }
                if (hits >= 8) { rank = "A"; rankColor = "#9b59b6"; rankSize = 22; rankGlow = 8; rankText = "AWESOME!"; }
                if (hits >= 12) { rank = "S"; rankColor = "#f1c40f"; rankSize = 25; rankGlow = 10; rankText = "SAVAGE!"; }
                if (hits >= 16) { rank = "SS"; rankColor = "#e67e22"; rankSize = 28; rankGlow = 12; rankText = "SICK!!"; }
                if (hits >= 20) { rank = "SSS"; rankColor = "#ff003c"; rankSize = 32; rankGlow = 15; rankText = "SMOKIN' SICK!!!"; }
                
                window.ctx.globalAlpha = alpha; window.ctx.textAlign = align;
                let bounceY = Math.sin(Date.now() / 100) * 3;
                let baseY = 85; 
                ...
                window.ctx.fillText(`${rank} - ${rankText}`, xPos, baseY + bounceY);
                
                window.ctx.font = "italic 900 16px Arial"; window.ctx.fillStyle = "#fff"; window.ctx.shadowBlur = 3; window.ctx.shadowColor = "#000";
                window.ctx.fillText(`🔥 ${hits} HITS`, xPos, baseY + rankSize + 5 + bounceY);
                window.ctx.shadowBlur = 0;
            }
        };

        renderComboRank(window.p1, 30, "left");
        ...
        renderComboRank(maxEnemyCombo, window.canvas.width - 30, "right");
