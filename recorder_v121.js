// ==========================================
// RECORDER.JS - V69.8 KINETIC SAKUGA AMV EDITION
// ĐÃ FIX: Nhân vật vừa vặn, không bị zoom quá to, nhìn rõ toàn thân.
// CẬP NHẬT NGHỆ THUẬT: Hiệu ứng bóng đen (Silhouette), nhiễu màu (Chromatic), viền điện ảnh.
...
// ====================================================================
// PROCEDURAL ENGINE (ĐẠO DIỄN CẮT CẢNH NGHỆ THUẬT)
// ====================================================================
const THEMES = [
...
window.generateIntroParams = function() {
    ...
    // Kịch bản Đạo Diễn (5 Cuts nghệ thuật)
    let cuts = [];
    const states = ['attack', 'cast', 'jump', 'dash'];
    
    // 1: Boss Giới Thiệu (Góc nghiêng nhẹ)
    cuts.push({ focus: 'p2', state: r(states), camX: 0, camY: 80, zoom: 1.3, rot: -0.05, fx: 'none', txt: "" });
    // 2: Player Giới Thiệu
    cuts.push({ focus: 'p1', state: r(states), camX: 0, camY: 80, zoom: 1.3, rot: 0.05, fx: 'none', txt: "" });
    // 3: Silhouette (Bóng Đen Nghệ Thuật)
    cuts.push({ focus: r(['p1', 'p2']), state: 'attack', camX: 0, camY: 50, zoom: 1.4, rot: 0.15, fx: 'silhouette', txt: r(["WEAK.", "TOO SLOW", "AURA."]) });
    // 4: Chromatic (Nhiễu màu tốc độ cao)
    cuts.push({ focus: r(['p1', 'p2']), state: r(states), camX: 0, camY: 50, zoom: 1.5, rot: -0.1, fx: 'chromatic', txt: r(["CLIPPED", "DOMINATE"]) });
    // 5: The Clash (Lao vào nhau góc rộng)
    cuts.push({ focus: 'clash', state: 'attack', camX: 0, camY: 100, zoom: 1.1, rot: 0, fx: 'impact', txt: "" });

    return { themeP1: t1, themeP2: t2, montageCuts: cuts, warningText: r(["⚠️ WARNING ⚠️", "☢️ ANOMALY DETECTED ☢️", "🚨 NEW CHALLENGER 🚨", "🔥 AURA CLASH 🔥"]) };
};
...
// ====================================================================
// PROCEDURAL EPIC INTRO: SAKUGA KINETIC MONTAGE (CHỈNH SỬA ZOOM)
// ====================================================================
window.drawProceduralIntro = function(ctx, w, h, progress) {
    ...
    // 3. COMBAT MONTAGE PHASE (0.1 -> 0.85)
    let montageProg = (progress - 0.1) / 0.75; 
    let cutIndex = Math.floor(montageProg * 5); if(cutIndex > 4) cutIndex = 4;
    let cutProg = (montageProg * 5) - cutIndex; 
    let cut = prm.montageCuts[cutIndex];

    if (!window._sfxCuts[cutIndex]) {
        if (typeof window.playSound === 'function') window.playSound(200 + Math.random()*300, 'square', 0.2, 0.4);
        window._sfxCuts[cutIndex] = true;
    }

    // --- SETUP TỌA ĐỘ CAMERA ---
    ctx.save();
    let cutShake = Math.max(0, 1 - cutProg * 3) * 30 * (Math.random() - 0.5); // Giật đầu mỗi Cut
    let currentZoom = cut.zoom + (cutProg * 0.2); // Đẩy dần màn hình vào trong mượt mà
    
    // Đưa trục tọa độ vào giữa màn hình để Scale và Quay không bị lệch
    ctx.translate(w/2 + cutShake, h/2 + cutShake);
    ctx.rotate(cut.rot + (cutProg * 0.05));
    ctx.scale(currentZoom, currentZoom);
    ctx.translate(cut.camX, cut.camY); // Dịch chuyển ống kính

    // VẼ NỀN BACKGROUND SAKUGA
    ...
    // Tia tốc độ tỏa ra từ tâm (Radial Burst Speedlines)
    ctx.fillStyle = bgTheme.aura; ctx.globalAlpha = 0.3;
    for(let i=0; i<30; i++) {
        ctx.save(); ctx.rotate((Math.PI * 2 / 30) * i);
        let sp = (Date.now() * 4) % 800;
        ctx.fillRect(50 + sp, -10, 800, 10 + Math.random()*20);
        ctx.restore();
    }
    ctx.globalAlpha = 1.0;

    // HÀM VẼ NHÂN VẬT CHUẨN SCALE VÀ NGHỆ THUẬT
    const drawFighterArtistic = (charObj, cx, cy, forcedState, isFacingRight, theme, fx) => {
        if(!charObj) return; ctx.save(); ctx.translate(cx, cy); if(!isFacingRight) ctx.scale(-1, 1);
        
        // CỐ ĐỊNH SCALE NHÂN VẬT = 3.5 (Đảm bảo nhìn rõ toàn thân và vũ khí)
        let clone = Object.assign({}, charObj, {x:0, y:0, scale: 3.5, isFacingRight: true, state: forcedState});
        
        // Áp dụng Filter Nghệ Thuật
        if (fx === 'silhouette') ctx.filter = "brightness(0) contrast(200)";
        else if (fx === 'chromatic') ctx.filter = `drop-shadow(8px 0px 0px rgba(255,0,0,0.6)) drop-shadow(-8px 0px 0px rgba(0,255,255,0.6))`;

        // Vẽ Aura
        if (fx !== 'silhouette') {
            ctx.save(); ctx.globalAlpha = 0.5; ctx.shadowBlur = 40; ctx.shadowColor = theme.aura; ctx.scale(1.15, 1.15);
    ...
    // --- RENDER DỰA TRÊN KỊCH BẢN (FOCUS) ---
    // Tâm canvas lúc này là (0,0)
    if (cut.focus === 'p1') {
        drawFighterArtistic(window.p1, 0, 120, cut.state, true, prm.themeP1, cut.fx);
    } 
    else if (cut.focus === 'p2') {
        drawFighterArtistic(e1, 0, 120, cut.state, false, prm.themeP2, cut.fx);
    } 
    else { // Clash - 2 đứa lao vào giữa
        let easeIn = Math.pow(cutProg, 2);
        let pushP1 = -300 + (easeIn * 250); 
        let pushP2 = 300 - (easeIn * 250);
        drawFighterArtistic(e1, pushP2, 150, 'attack', false, prm.themeP2, cut.fx);
        drawFighterArtistic(window.p1, pushP1, 150, 'attack', true, prm.themeP1, cut.fx);
        
        // Vụ nổ tóe lửa ở giữa
        if(cutProg > 0.6) {
            ctx.beginPath(); ctx.arc(0, 50, (cutProg-0.6)*800, 0, Math.PI*2);
            ctx.fillStyle = "rgba(255,255,255," + (1-cutProg)*2.5 + ")"; ctx.fill();
        }
    }

    // IMPACT FRAMES Âm bản chớp giật
    if (cut.fx === 'impact' && cutProg < 0.2) {
    ...
    ctx.restore(); // THOÁT KHỎI CAMERA ZOOM ĐỂ VẼ OVERLAY

    // --- VIGNETTE ĐIỆN ẢNH (Tối 4 góc viền) ---
    ...
    // --- SNAPPY TEXT (CHỮ NẢY LÊN THEO NHỊP) ---
    if (cut.txt && cutProg < 0.6) {
        ctx.save(); ctx.translate(w/2, h/2); 
        let txtScale = 1 + Math.pow(cutProg, 0.5) * 0.5; // Snap scale
    ...
    // --- TRASH TALK CHEN NGANG ---
    if (cutIndex === 1 || cutIndex === 3) {
        let align = cutIndex === 1 ? -1 : 1;
        ctx.save(); ctx.translate(w*0.5, h*0.85); ctx.rotate(0.05 * align);
        ctx.font = "italic 900 65px Impact"; ctx.textAlign = "center";
        ctx.lineWidth = 15; ctx.strokeStyle = "#000"; ctx.strokeText(window.trashTalkP1, 0,0);
        ctx.fillStyle = prm.themeP1.text || "#00f3ff"; ctx.fillText(window.trashTalkP1, 0,0); ctx.restore();
    }
