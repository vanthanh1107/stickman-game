...
// ĐÃ FIX: LỖI DROP FRAME & LỖI HIỂN THỊ SAI THỜI LƯỢNG VIDEO
// ĐÃ NÂNG CẤP: AI SINH TIÊU ĐỀ + AI SINH TOXIC CHAT (SIÊU ĐA DẠNG)
// ==========================================
...
// ====================================================================
// ENGINE AI NỘI BỘ: TẠO NGƯỜI XEM & TOXIC COMMENTS (SIÊU ĐA DẠNG)
// ====================================================================
window.generateLiveChatEvent = function() {
    const r = (arr) => arr[Math.floor(Math.random() * arr.length)];

    // 1. TẠO TÊN NGƯỜI XEM RANDOM (VÔ HẠN)
    let randomName = "";
    let randomColor = "#ffffff";
    const isCeleb = Math.random() < 0.12; // 12% tỷ lệ Celeb chui vào chat

    if (isCeleb) {
        const CELEB_LIST = [
            { name: "Elon Musk 🚀", color: "#00f3ff" }, { name: "Cristiano Ronaldo ⚽", color: "#ffeb3b" },
            { name: "IShowSpeed 🐕", color: "#ff4757" }, { name: "MrBeast 💰", color: "#2ecc71" },
            { name: "Faker_T1 🐐", color: "#ff0055" }, { name: "Gordon Ramsay 🍳", color: "#ffffff" },
            { name: "Lionel Messi 🐐", color: "#70a1ff" }, { name: "Snoop Dogg 🌿", color: "#2ed573" },
            { name: "Drake 🦉", color: "#ffa502" }, { name: "Andrew Tate 🏎️", color: "#ff7f50" }
        ];
        let celeb = r(CELEB_LIST);
        randomName = celeb.name;
        randomColor = celeb.color;
    } else {
        const namePrefixes = ["user", "Toxic", "Pro", "Noob", "Sigma", "Lil_", "Drip", "Ghost", "Shadow", "Slayer", "I_am_", "Real_", "Fake_", "Alpha"];
        const nameSuffixes = ["Gamer", "Boy", "God", "Demon", "King", "Master", "Z", "X", "_007", "69", "99", "O_O"];
        const isAnon = Math.random() > 0.4;
        
        if (isAnon) {
            randomName = "user" + Math.floor(100000 + Math.random() * 900000); 
        } else {
            randomName = r(namePrefixes) + r(nameSuffixes) + (Math.random() > 0.5 ? Math.floor(Math.random() * 999) : "");
        }
        const colors = ["#ff4757", "#2ed573", "#1e90ff", "#ffa502", "#ff6b81", "#7bed9f", "#70a1ff", "#eccc68", "#ffffff", "#00f3ff", "#ffeb3b", "#ff9ff3"];
        randomColor = r(colors);
    }

    // 2. TẠO COMMENT TOXIC VỚI KHO TỪ VỰNG KHỔNG LỒ
    const subjects = [
        "Bro", "Blud", "Lil bro", "This guy", "Dude", "My man", "He", "Bro really", "Dawg", "Broski", 
        "Chat,", "Ain't no way bro", "My guy", "Buddy", "Bro literally", "This gameplay"
    ];
    const actions = [
        "plays like he has 999 ping", "is using a steering wheel", "skipped the tutorial", 
        "is playing with his toes", "has negative IQ", "is literally trash", 
        "is sweating for zero views", "got carried by lag", "is pressing random buttons",
        "forgets how to play", "thinks he is him", "needs to touch grass",
        "looks lost", "is fighting for his life", "is playing on a toaster", 
        "is trying to derank", "forgot his hands today", "is allergic to winning", 
        "is playing with the monitor off", "paid $0 for this game and still got scammed",
        "is mashing like my little brother", "is fighting demons rn", "is NOT cooking",
        "fumbled so hard"
    ];
    const comparisons = [
        "My grandma has better combos", "A potato could play better", 
        "Even my dog is cringing", "NPC behavior spotted", "This is why aliens ignore us",
        "Worst gameplay I have ever seen", "My blind cousin plays better", "I could do this with my monitor off",
        "Mobile game ads have better gameplay", "A literal bot is harder to beat", 
        "My left foot can combo better", "A roomba could win this", "Stormtrooper aim but for combos"
    ];
    const insults = [
        "Skill issue", "Absolute garbage", "0 IQ gameplay", "Literal trash", 
        "Brain dead combo", "Delete the game", "Uninstall pls", "So bad it hurts", "Cringe", "Pack it up bro",
        "Washed", "Cooked", "Free elo", "Negative aura", "NPC", "Get good", "Hold this L", "Rip bozo",
        "Bro is NOT him", "Lmao", "Uninstalling on his behalf"
    ];
    const gear = ["combo", "keyboard", "monitor", "gaming chair", "wifi", "mouse", "PC", "controller", "brain", "router"];
    const shortReactions = ["L", "W boss", "RIP", "F", "bro...", "nahhh", "bruh", "what is this", "im crying", "pls stop", "LMAO", "NAHHH", "AINT NO WAY"];
    const emojis = ["💀", "😭", "🗑️", "🤡", "📉", "🥶", "📸", "🗿", "🤢", "💩", "🤬", "👎", "🤣", "🤦‍♂️", "💀💀", "💀😭", "🔥", "❌", "🙏"];
    ...
    // 3. MIX 21 CẤU TRÚC KHÁC NHAU ĐỂ TẠO CÂU ĐỘC NHẤT
    const templates = [
        () => `${r(subjects)} ${r(actions)} ${r(emojis)}`,
        () => `${r(comparisons)} ${r(emojis)}`,
        () => `Imagine ${r(actions).replace("is ", "").replace("has ", "having ")} ${r(emojis)}`,
        () => `${r(insults)}! ${r(emojis)}`,
        () => `What color is your ${r(gear)}? Broke ${r(emojis)}`,
        () => `Send me location. I smash him. 🦅`,
        () => `Someone take his ${r(gear)} away immediately ${r(emojis)}`,
        () => `${r(shortReactions).toUpperCase()} WHAT WAS THAT ${r(emojis)}`,
        () => `${r(insults)}. ${r(subjects)} ${r(actions)} 🤡`,
        () => `Bro's ${r(gear)} is crying right now 😭`,
        () => `Reported for being this bad 💀`,
        () => `${r(shortReactions)} ${r(emojis)}`, 
        () => `${r(emojis)}${r(emojis)}${r(emojis)}`, 
        () => `Who let bro cook?? ${r(emojis)}`,
        () => `I lost brain cells watching this ${r(emojis)}`,
        () => `Give the boss a raise 📈`,
        () => `Bro is ${r(["cooked", "washed", "done for", "finished"])} ${r(emojis)}`,
        () => `Chat is this real? ${r(emojis)}`,
        () => `${randomButtons()} 🤡`,
        () => `Bro's aura is -9999 📉`,
        () => `Even TikTok is lagging from this gameplay 🗑️`
    ];

    let randomMsg = r(templates)();
...
// ====================================================================
// ENGINE AI NỘI BỘ TẠO TIÊU ĐỀ & HASHTAG
// ====================================================================
window.StoryModeAI = {
    ...
    // Bộ nhớ lưu các tiêu đề đã tạo để chống trùng lặp trong phiên làm việc
    _usedTitles: new Set(),

    // AI NỘI BỘ: Sinh tiêu đề & Hashtag ngách Stickman
    generateViralStickmanTitle: function(hero, enemy) {
        // 1. Tập dữ liệu từ vựng (Clickbait, Niche Stickman, Cảm xúc)
        const adjectives = ["GOD TIER", "ILLEGAL", "TOXIC", "BROKEN", "999 IQ", "DISGUSTING", "INSANE", "1 HP", "SECRET", "IMPOSSIBLE"];
        const actions = ["COMBO", "RAGE QUIT", "CLUTCH", "DISRESPECT", "SPAM", "ONE SHOT", "FINISHER", "DODGE", "COUNTER"];
        const subjects = ["TRYHARD", "SPAMMER", "BOSS", "NOOB", "PRO PLAYER", "KID", (enemy || "BOSS").toUpperCase()];
        const reactions = ["💀", "😭", "🤬", "🤡", "🔥", "🤯", "🥶", "📉", "🤫", "⚠️"];
        const stickmanTerms = ["Stickman", "Ragdoll", "Physics", "Stick Fight"];

        // 2. Công thức tạo tiêu đề 
        const templates = [
            `BRO THINKS HE IS [SUBJECT] [REACTION]`,
            `THE MOST [ADJ] [ACTION] IN [STICKMAN] HISTORY [REACTION]`,
            `WHEN A [ADJ] [SUBJECT] MEETS A PRO [REACTION]`,
            `NEVER DO THIS [ACTION] AGAINST [SUBJECT] [REACTION]`,
            `I BROKE THE [STICKMAN] [ACTION] LIMIT [REACTION]`,
            `THIS [ADJ] [ACTION] MADE HIM [RAGE_ACTION] [REACTION]`,
            `HOW TO BEAT [SUBJECT] WITH ONLY 1 [ACTION] [REACTION]`,
            `BRO SKIPPED THE [STICKMAN] TUTORIAL [REACTION]`,
            `IS THIS THE BEST [ACTION] COMBO EVER? [REACTION]`,
            `PUNISHING A TOXIC [SUBJECT] [REACTION]`
        ];

        // 3. Xử lý logic trộn từ ngẫu nhiên
        const randomItem = (arr) => arr[Math.floor(Math.random() * arr.length)];
        let template = randomItem(templates);
        
        let rawTitle = template
            .replace(/\[ADJ\]/g, () => randomItem(adjectives))
            .replace(/\[ACTION\]/g, () => randomItem(actions))
            .replace(/\[SUBJECT\]/g, () => randomItem(subjects))
            .replace(/\[REACTION\]/g, () => randomItem(reactions))
            .replace(/\[STICKMAN\]/g, () => randomItem(stickmanTerms))
            .replace(/\[RAGE_ACTION\]/g, () => randomItem(["UNINSTALL", "CRY", "RAGE QUIT", "SMASH HIS KEYBOARD"]));

        // 4. Tạo bộ Hashtag (Lấy random 4-5 tags mỗi lần để tự nhiên nhất)
        const allTags = [
            "#stickman", "#stickmanfight", "#gaming", "#fyp", "#combos", 
            "#rage", "#skillissue", "#gamingtiktok", "#shorts", "#stickmananimation", 
            "#ragdoll", "#epicgamer", "#toxic", "#godtier", "#clutch"
        ];
        let shuffledTags = allTags.sort(() => 0.5 - Math.random());
        let selectedTags = shuffledTags.slice(0, 4 + Math.floor(Math.random() * 2)).join(" ");

        // Ghép Title và Tags
        let finalTitle = `${rawTitle} ${selectedTags}`;

        // 5. Chống trùng lặp (Đệ quy tìm tên mới nếu bị trùng)
...
window.startRecording = function() {
    ...
    // [FIX THỜI GIAN]: DỂ TRỐNG THAY VÌ START(1000) ĐỂ BROWSER TỰ ĐỘNG CHỐT HEADER VIDEO KHI KẾT THÚC
...
window.stopRecording = function() { 
    ...
    // [FIX ĐỂ CHỐT FILE HOÀN CHỈNH]: Dừng hẳn VideoTrack khi nhấn Stop
...
window.captureFrames = function() {
    ...
    if (!isOutroActive) {
        ...
        // --- KHỞI TẠO DỮ LIỆU CHAT & THÔNG BÁO BẰNG AI ---
        ...
        if (Date.now() - window._lastAlertTime > 2500 + Math.random() * 3000) {
            ...
            if (window._liveChats && window._liveChats.length > 0) {
                ...
                randomUser = "user" + Math.floor(Math.random() * 999999);
            }
