// ==========================================
// RECORDER.JS - V75.0 THE ALGORITHM BREAKER (DIRECTOR'S CUT)
...
// TÂM LÝ HỌC V75: Super Chat Donation, Clutch Mode 1HP, Khôi phục 100% Chat Toxic đa vùng miền.
// ==========================================
...
window.CREATOR_HANDLE = "VIETNAM SPACEX"; 
...
// ====================================================================
// HỆ THỐNG LIVE CHAT TOXIC ĐA NGÔN NGỮ BẢN FULL (ANTI-SPAM MEMORY)
// ====================================================================
window.CELEB_LIST = [
    ...
    { name: "ConNhaNguoiTa", color: "#2ed573" }, { name: "ThanhTroll", color: "#ff0055" }
];

const TOXIC_MSGS = [
    // Brainrot / Western English
    ...
    "THE LORE IS CRAZY 😭", "Bro has anime protagonist aura ☢️", "NAHHH jit trippin 💀", "What the sigma 🗿",
    "Negative aura 📉", "Bro's ping is higher than his IQ 🤡",
    
    // Tiếng Việt Khịa / Toxic mặn mòi
    ...
    "Phản xạ của người già à bro?", "Lùa gà à 🐧", "Cầm chân đánh à bạn?", "Bảo sao mãi k lên rank 🤡",
    "Mẹ gánh còng lưng", "Ảo game à =))))", "Bấm auto attack à 🤖",
...
// ====================================================================
// ĐẠO DIỄN: KỊCH BẢN INTRO (ICON BAY -> COMPACT CHAT -> BUBBLES -> VS)
// ====================================================================
window.drawProceduralIntro = function(ctx, w, h, progress) {
    ...
    if (act === 0) {
        ...
        if(!window._introBottomChats) { window._introBottomChats = []; }
        
        // HIỆU ỨNG CHAT SCROLL LIÊN TỤC TRONG INTRO
        if (Math.random() < 0.4) { // Tần suất đẩy chat mới vào
            window._introBottomChats.push(window.generateLiveChatEvent());
            if (window._introBottomChats.length > 4) window._introBottomChats.shift();
        }
...
window.startRecording = function() {
    ...
    window._baitTriggered = false; window._baitType = -1; window._donationAlert = null;
...
window.captureFrames = function() {
    ...
    // TÍNH NĂNG MỚI: SUPER CHAT / DONATION BAIT
    ...
    if (renderNormalV) {
        ...
        if (isClutchMode && !isOutroActive) {
            let hb = Math.sin(Date.now() / 150);
            ctxV.fillStyle = `rgba(255, 0, 0, ${0.15 + hb * 0.15})`;
            ctxV.fillRect(0, 0, 1080, splitGameHeight);
            ctxV.font = "900 45px 'Arial Black'"; ctxV.textAlign = "center";
            ctxV.fillStyle = "rgba(255, 0, 0, 0.6)"; 
            ctxV.fillText("⚠️ CRITICAL HP ⚠️", 540, splitGameHeight - 50);
        }
        ...
        // VẼ SẠN CHỐNG CHỈ ĐỊNH LÊN TRÊN GAME
        drawBaitSạn(ctxV, 1080, splitGameHeight);

        // HIỂN THỊ SUPER CHAT DONATION
        if (window._donationAlert) {
            let da = window._donationAlert;
            da.life--;
            ctxV.save();
            let daY = 150;
            if (da.life > 230) daY = 150 - (da.life - 230)*10; 
            if (da.life < 20) daY = 150 - (20 - da.life)*10; 
            ctxV.translate(540, daY);
            ctxV.fillStyle = "rgba(255, 215, 0, 0.95)";
            ctxV.shadowColor = "#ffb800"; ctxV.shadowBlur = 20;
            ctxV.beginPath(); if(ctxV.roundRect) ctx.roundRect(-300, -45, 600, 90, 45); else ctx.fillRect(-300, -45, 600, 90); ctxV.fill();
            ctxV.shadowBlur = 0;
            ctxV.fillStyle = "#000"; ctxV.textAlign = "center"; ctxV.textBaseline = "middle";
            ctxV.font = "900 24px 'Arial Black'";
            ctxV.fillText(`💰 ${da.name} donated $100:`, 0, -18);
            ctxV.font = "bold 22px Arial";
            ctxV.fillText(`"${da.msg}"`, 0, 18);
            ctxV.restore();
        ...
        if (!isOutroActive) {
            ...
            if (window.p1) {
                ...
                let p1Stam = Math.max(0, window.p1.stamina / 100); let eHp = 0, eMax = window.totalEnemyMaxHp || 1, eStam = 0;
                ...
                if (p1HpRatio > 0) { if(!window._cachedGradients.hpP1) { window._cachedGradients.hpP1 = ctxV.createLinearGradient(187, 0, 487, 0); window._cachedGradients.hpP1.addColorStop(0, "#00f2fe"); window._cachedGradients.hpP1.addColorStop(1, "#4facfe"); } drawSkewedPath(ctxV, 187, hudBaseY + 73, (300 * p1HpRatio) | 0, 53, true); ctxV.fillStyle = window._cachedGradients.hpP1; ctxV.fill(); }
            ...
            if (!window._chatSystemInit) { window._chatSystemInit = true; window._liveChats = []; window._lastChatUpdate = Date.now(); window._nextChatDelay = 1000; window._fakeViewers = 350 + Math.floor(Math.random() * 500); for(let i = 0; i < 8; i++) { let c = window.generateLiveChatEvent(isClutchMode); window.precalcChatText(c, ctxV); window._liveChats.push(c); } }
            let chatNow = Date.now();
            if (chatNow - window._lastChatUpdate > window._nextChatDelay) { window._lastChatUpdate = chatNow; window._nextChatDelay = isClutchMode ? 300 + Math.random()*500 : 1000 + Math.random() * 2000; let newChat = window.generateLiveChatEvent(isClutchMode); window.precalcChatText(newChat, ctxV); window._liveChats.push(newChat); if (window._liveChats.length > 12) window._liveChats.shift(); window._fakeViewers += Math.floor(Math.random() * 41) - 15; if(window._fakeViewers < 200) window._fakeViewers += Math.floor(Math.random() * 50); if(window._fakeViewers > 2500) window._fakeViewers -= Math.floor(Math.random() * 50); }
            ...
            let brandName = window.CREATOR_HANDLE || "Sticklom";
