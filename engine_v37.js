// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 31.0 [THE 360 MATRIX ORBIT]
// [BASE] Giữ nguyên 100% mã nguồn V28.0 (Kinetic Text, Lethal Void, RTX Floor).
// [NEW] 360 MATRIX ORBIT: Phép chiếu không gian 3D giả lập xoay camera quanh nhân vật.
// [NEW] ACTION ANGLE SNAP: Văng góc máy chéo 70 độ khi có đụng chiêu (Clash) và Ultimate.
// [NEW] 3D PARTICLE PROJECTION: Mọi hạt tia lửa, số máu đều tuân theo hệ trục Z-Axis.
// ==========================================
...
// [VIRAL 31.0] HỆ THỐNG MÁY QUAY ĐA CHIỀU 360 ĐỘ
...
window.initGameEngine = function() {
    ...
    window.viralColorGrade = { contrast: 120 + Math.random() * 20, brightness: 85 + Math.random() * 15, saturation: 95 + Math.random() * 30, hue: Math.floor((Math.random() - 0.5) * 30) };
    ...
    for (let i = 0; i < imgData.data.length; i += 4) { let val = Math.random() * 255; imgData.data[i] = val; imgData.data[i+1] = val; imgData.data[i+2] = val; imgData.data[i+3] = Math.random() * 25; }
...
window.triggerTimeStop = function(frames, caster) { window.targetTimeScale = 0.05; setTimeout(() => { window.targetTimeScale = 1.5; setTimeout(() => { window.targetTimeScale = 1.0; }, 300); }, frames * 16); window.timeStopCaster = caster; window.applyFilter('invert', frames); window.playSound(100, 'square', 1.0, 1.0, true); window.shakeScreen(30, 20); if (window.shockwaves.length < MAX_SHOCKWAVES) { window.shockwaves.push({x: caster.x, y: caster.y, r: 10, maxR: 1500, color: "#fff", alpha: 1, speed: 40}); } }
...
window.spawnGlassShatter = function(x, y) { window.playSound(600, 'sawtooth', 0.5, 0.8, true); window.shakeScreen(25, 18); if (window.particles.length > MAX_PARTICLES) return; for(let i=0; i<25; i++) { window.particles.push({ x: x + (Math.random()*40-20), y: y + (Math.random()*40-20), vx: (Math.random()-0.5)*18, vy: -Math.random()*18, life: 30, maxLife: 30, color: "rgba(100, 255, 255, 0.9)", size: Math.random()*6+2, isGlass: true }); } }
window.spawnLensFlare = function(x, y, color, scale) { window.lensFlares.push({x: x, y: y, color: color, scale: scale, life: 30, maxLife: 30}); }
window.spawnMangaSFX = function(x, y, isCrit) { let sfxList = ['ゴゴゴ', 'ドドド', 'ドム', 'ズバーン', 'バキッ']; let text = sfxList[Math.floor(Math.random() * sfxList.length)]; let size = isCrit ? (55 + Math.random()*20) : (35 + Math.random()*10); window.mangaSfx.push({ x: x + (Math.random()*60-30), y: y + (Math.random()*40-20), vx: (Math.random()-0.5)*4, vy: -2 - Math.random()*3, life: 40, maxLife: 40, text: text, size: size, isCrit: isCrit, ang: (Math.random()-0.5)*0.5 }); }
window.triggerDimensionShatter = function() { window.dimensionCracks = []; let cx = window.canvas ? window.canvas.width/2 : 600; let cy = window.canvas ? window.canvas.height/2 : 300; for(let i=0; i<8; i++) { let ang = (Math.PI*2/8)*i + (Math.random()-0.5); let len = 600 + Math.random()*300; let points = [{x: cx, y: cy}]; let dist = 0; let currX = cx; let currY = cy; while(dist < len) { let step = 40 + Math.random()*60; dist+=step; ang += (Math.random()-0.5)*0.8; currX += Math.cos(ang)*step; currY += Math.sin(ang)*step; points.push({x: currX, y: currY}); } window.dimensionCracks.push({points: points, life: 45, maxLife: 45}); } }
window.spawnEnvDamage = function(x, y, type, scale, isBurning = false) { let cracks = []; let numCracks = (type === 'crater') ? 6 + Math.floor(Math.random()*4) : 4 + Math.floor(Math.random()*3); let maxRadius = 0; for(let i=0; i<numCracks; i++) { let angle; if (type === 'crater') { let step = (Math.PI * 0.8) / Math.max(1, numCracks - 1); angle = Math.PI * 0.1 + step * i + (Math.random() - 0.5) * 0.2; } else if (type === 'wall_left') angle = -Math.PI/2 + (Math.random() * Math.PI * 0.8) + Math.PI*0.1; else angle = Math.PI/2 + (Math.random() * Math.PI * 0.8) - Math.PI*0.4; let len = (30 + Math.random()*50) * scale; if (len > maxRadius) maxRadius = len; let path = [{x: 0, y: 0}]; let segments = 3 + Math.floor(Math.random()*2); let currX = 0, currY = 0; let currAngle = angle; for(let s = 1; s <= segments; s++) { let segLen = len / segments; currAngle += (Math.random() - 0.5) * 0.8; currX += Math.cos(currAngle) * segLen; currY += Math.sin(currAngle) * segLen; path.push({x: currX, y: currY}); if (Math.random() < 0.4 && s < segments) { let branchAngle = currAngle + (Math.random() > 0.5 ? 1 : -1) * (0.5 + Math.random() * 0.5); let bLen = segLen * (0.6 + Math.random() * 0.6); cracks.push([{x: currX, y: currY}, {x: currX + Math.cos(branchAngle) * bLen, y: currY + Math.sin(branchAngle) * bLen}]); } } cracks.push(path); } if (window.particles.length < MAX_PARTICLES) { for(let d=0; d < 8 * scale; d++) { window.particles.push({ x: x + (Math.random()-0.5)*30, y: y + (Math.random()-0.5)*20, vx: (Math.random()-0.5)*10, vy: -Math.random()*10 - 4, life: 40, maxLife: 40, color: isBurning ? "#e74c3c" : "#57606f", size: Math.random()*4 + 2, isRubble: true }); } } window.envDamage.push({ x: x, y: y, type: type, cracks: cracks, scale: scale, radius: maxRadius, isBurning: isBurning, life: 800, maxLife: 800 }); if (window.envDamage.length > 3) { window.envDamage.shift(); } };
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce, attackerDirRight = true) {
    ...
    if (target.state === 'block') { 
        if (target.stamina < 15) {
            ...
            // [VIRAL 31.0] GÓC XOAY ĐA CHIỀU KHI VỠ KHIÊN
            window.targetCamOrbitAngle = attackerDirRight ? 0.6 : -0.6; // Xoay máy 35 độ
            window.orbitFocusX = target.x;
            setTimeout(() => { window.targetCamOrbitAngle = 0; }, 800);
    ...
    if (finalDmg > 0) {
        ...
        for(let i=0; i<sparkCount; i++) { 
            let vx = baseDirX * (Math.random() * 20 + 5) + (Math.random()-0.5)*5; let vy = (Math.random()-0.5)*15 - 5;
        ...
        if (isCrit) {
            window.screenFlash = 1.0; window.impactFrameCount = 4; window.targetZoom = 1.45; setTimeout(() => { window.targetZoom = 1.1; }, 100); 
            window.impactAberration = 15; window.spawnForegroundDebris(target.x, target.y, attackerDirRight);
            
            // [VIRAL 31.0] GÓC QUAY CHÉO KHI ĐÁNH CHÍ MẠNG (Tạo chiều sâu 3D)
            window.targetCamOrbitAngle = attackerDirRight ? -0.4 : 0.4;
            window.orbitFocusX = target.x;
            setTimeout(() => { window.targetCamOrbitAngle = 0; }, 600);
        ...
        if (target.hp <= 0) {
            ...
            if (isNearWall && isCrit && typeof window.triggerStageTransition === 'function') { window.triggerStageTransition(target); } 
                ...
                if (isCrit) {
                    window.screenFlash = 1.5; window.invertFrames = 6; window.speedLinesAlpha = 1.0; window.spawnLensFlare(target.x, target.y - 40, "#ff003c", 5.0);
                    ...
                    // [VIRAL 31.0] QUAY CAMERA 360 ĐỘ TRONG TÍCH TẮC KẾT LIỄU
                    window.isSpinningCam = true;
                    window.orbitFocusX = target.x;
                    window.targetTimeScale = 0.02; 
                    setTimeout(()=>{ 
                        window.targetTimeScale = 1.0; window.cameraTilt = 0; 
                        window.isSpinningCam = false; window.targetCamOrbitAngle = 0; 
                    }, 2500); 
...
window.update = function() {
    ...
    window.actionCamOffsetX *= 0.85; window.actionCamOffsetY *= 0.85; window.dynamicBlur *= 0.9;
            ...
            if (f.hitStun <= 0 && f.stunTimer <= 0 && f.state !== 'dash_back' && f.state !== 'block') {
                ...
                if (isCloseEnough) {
                    if (window.classStats && window.classStats[type] && typeof window.classStats[type].executeUltimate === 'function') {
                        ...
                        // [VIRAL 31.0] GÓC QUAY CHÉO KHI XẢ ULTIMATE VÀ ORBIT QUANH MỤC TIÊU
                        window.targetCamOrbitAngle = f.isFacingRight ? -0.7 : 0.7; // Xoay chéo máy
                        window.orbitFocusX = closestTarget.x;
                        setTimeout(() => { window.targetCamOrbitAngle = 0; }, 2000);

                    } else if (typeof window.useUltimate === 'function') { window.useUltimate(f, closestTarget); f.vx = 0; window.targetCamOrbitAngle = f.isFacingRight ? -0.7 : 0.7; }
            ...
            for (let j = i + 1; j < allFighters.length; j++) { 
                ...
                if (dist < 80 && f1.attackTimer > 5 && f1.attackTimer < 20 && f2.attackTimer > 5 && f2.attackTimer < 20) {
                    if (!f1.clashCooldown && !f2.clashCooldown && f1.isFacingRight !== f2.isFacingRight) {
                        ...
                        window.spawnLensFlare(midX, midY, "#00f3ff", 4.0);
                        window.impactAberration = 25; 
                        window.triggerDimensionShatter(); 
                        
                        // [VIRAL 31.0] GÓC QUAY CHÉO KHI ĐỤNG CHIÊU
                        window.targetCamOrbitAngle = (Math.random() > 0.5 ? 1 : -1) * 1.2; 
                        window.orbitFocusX = midX;
                        setTimeout(() => { window.targetCamOrbitAngle = 0; }, 1500);
    ...
    // [VIRAL 31.0] CAMERA 360 ĐỘ VÀ GÓC MÁY QUAY CHÉO
    if (window.isSpinningCam) {
        window.camOrbitAngle += 0.08; // Xoay vòng liên tục khi K.O
    } else {
        window.camOrbitAngle += (window.targetCamOrbitAngle - window.camOrbitAngle) * 0.1; // Góc máy mượt
    }
    ...
    if (!window.isSpinningCam && window.targetCamOrbitAngle === 0) {
        window.orbitFocusX += (midPointX - window.orbitFocusX) * 0.1;
    }

    // Áp dụng góc xoay vào Camera Focus
    window.targetCamX = (window.canvas.width / 2) - window.orbitFocusX + window.actionCamOffsetX;
    window.targetCamY = Math.max(0, (window.GROUND_Y - 150) * 0.5) + window.actionCamOffsetY;
}
...
window.draw = function() {
    ...
    // [VIRAL 31.0] HÀM PROJECT 3D CHIẾU KHÔNG GIAN
    ...
    let project3D = (obj) => {
        ...
        if (typeof obj.isFacingRight !== 'undefined') {
            if (Math.abs(window.camOrbitAngle % (Math.PI*2)) > 0.1) {
                pFacing = pX < window.orbitFocusX;
            }
        }
    ...
    if (window.isLoading) {
        ...
        window.ctx.fillStyle = "#00f3ff"; window.ctx.font = "italic 900 40px Arial"; window.ctx.textAlign = "center"; window.ctx.shadowBlur = 15; window.ctx.shadowColor = "#00f3ff"; window.ctx.fillText("360 ORBIT INITIALIZING...", cx, cy - 50);
    ...
    try {
        ...
        window.ctx.save();
        window.ctx.translate((window.camX + bgPanOffset) * 0.95, window.camY * 0.8); 
        window.ctx.fillRect(-6000, -3000, window.canvas.width + 12000, window.canvas.height + 6000);
        window.ctx.restore();

        window.ctx.save(); window.ctx.globalCompositeOperation = "screen"; let rayShift = Date.now() / 3000;
        window.ctx.translate((window.camX + bgPanOffset) * 0.85, window.camY * 0.7);
        for(let r=0; r<24; r++) { let rayAlpha = 0.05 + Math.abs(Math.sin(rayShift + r)) * 0.05; window.ctx.fillStyle = `rgba(255, 255, 255, ${rayAlpha})`; window.ctx.beginPath(); window.ctx.moveTo(-1500 + r*200 + Math.sin(rayShift)*100, -800); window.ctx.lineTo(-1400 + r*200 + Math.sin(rayShift)*100, -800); window.ctx.lineTo(-1100 + r*250, window.GROUND_Y); window.ctx.lineTo(-1300 + r*250, window.GROUND_Y); window.ctx.fill(); }
        window.ctx.restore();

        window.ctx.save(); window.ctx.translate((window.camX + bgPanOffset) * 0.6, window.camY * 0.4); 
        window.ctx.fillStyle = cmap.bg2;
        for(var i = -4500; i < window.canvas.width + 4500; i += 150) {
            window.ctx.beginPath(); window.ctx.moveTo(i, window.GROUND_Y); window.ctx.lineTo(i+75, window.GROUND_Y-120+Math.sin(i)*30); window.ctx.lineTo(i+150, window.GROUND_Y); window.ctx.fill();
        }
        window.ctx.restore();

        window.ctx.save(); window.ctx.translate((window.camX + bgPanOffset) * 0.2, window.camY * 0.1); 
        window.ctx.fillStyle = cmap.bg1;
        for(var i = -4500; i < window.canvas.width + 4500; i += 120) {
            let h = 100 + Math.abs(Math.sin(i))*80;
            window.ctx.fillRect(i, window.GROUND_Y-h, 70, h);
        }
        window.ctx.restore();
        ...
        for(let i = -6000; i <= 6000; i += 150) { window.ctx.moveTo(vanishingPointX, window.GROUND_Y); window.ctx.lineTo(i - window.camX * 1.5, window.canvas.height + 800); }
        ...
        if (window.screenFlash > 0) { window.ctx.globalCompositeOperation = 'screen'; window.ctx.fillStyle = `rgba(255, 255, 255, ${window.screenFlash})`; window.ctx.fillRect(0, 0, window.canvas.width, window.canvas.height); window.ctx.globalCompositeOperation = 'source-over'; }
