// ==========================================
// RECORDER.JS - BẢN HOÀN HẢO CHO CONTENT CREATOR (V60.0 - KINETIC ANIME INTRO)
// [NEW] KINETIC INTRO: Khung hình tự động Zoom, Rung bần bật khi va chạm, Bong bóng thoại nảy lò xo.
// [NEW] WARNING TAPE: Dải băng cảnh báo nguy hiểm di chuyển ở mép màn hình.
// [NEW] RGB GLITCH: Hiệu ứng nhiễu sóng xé màn hình khi chuyển cảnh vào Gameplay.
...
window.drawAnimatedIntro = function(ctx, w, h, isVertical, progress) {
    ...
    // [KINETIC ZOOM] Khung hình từ từ phóng to để tạo độ nghẹt thở
    let zoom = 1 + (progress * 0.15); // Phóng to 15% dần dần
    ctx.translate(w/2, h/2); ctx.scale(zoom, zoom); ctx.translate(-w/2, -h/2);

    // [SCREEN SHAKE] Rung bần bật lúc 2 khung hình đập vào nhau
    if (progress > 0.2 && progress < 0.35) {
        let intensity = (0.35 - progress) * 80;
        ctx.translate((Math.random() - 0.5) * intensity, (Math.random() - 0.5) * intensity);
    }

    // Nền Speedline manga
    ctx.fillStyle = "#111"; ctx.fillRect(-w, -h, w*3, h*3); // Đổ tràn viền vì có zoom
    ctx.save(); ctx.translate(w/2, h/2); ctx.rotate(Math.PI/12);
    let speedOffset = (progress * 4000) % 200;
    ...
    let e1 = window.enemies && window.enemies.length > 0 ? window.enemies[0] : window.p1;
    let easeOut = 1 - Math.pow(1 - Math.min(1, progress * 5), 3); // Slide vào thần tốc
    ...
    if (isVertical) { ctx.moveTo(0,-h); ctx.lineTo(w,-h); ctx.lineTo(w, h*0.48); ctx.lineTo(0, h*0.52); }
    else { ctx.moveTo(-w,0); ctx.lineTo(w*0.55, 0); ctx.lineTo(w*0.45, h); ctx.lineTo(-w, h); }
    ...
    // Panel Phải/Dưới (P2)
    ctx.save(); ctx.beginPath();
    if (isVertical) { ctx.moveTo(0, h*0.52); ctx.lineTo(w, h*0.48); ctx.lineTo(w, h*2); ctx.lineTo(0, h*2); }
    else { ctx.moveTo(w*0.55, 0); ctx.lineTo(w*2, 0); ctx.lineTo(w*2, h); ctx.lineTo(w*0.45, h); }
    ctx.closePath(); ctx.clip();
    ...
    ctx.translate(p2X, p2TargetY); ctx.scale(-1, 1); ctx.shadowColor = "#ff003c"; ctx.shadowBlur = 40;
    ...
    else { ctx.moveTo(w*0.55, -h); ctx.lineTo(w*0.45, h*2); }
    ctx.stroke();

    // [HAZARD TAPE] Dải băng cảnh báo 2 đầu cực ngầu
    const drawHazardTape = (yPos, tapeDir) => {
        ctx.save(); ctx.translate(0, yPos);
        ctx.fillStyle = "#ffcc00"; ctx.fillRect(-w, -25, w*3, 50);
        ctx.fillStyle = "#000";
        let tapeOffset = (progress * 1500 * tapeDir) % 80;
        for (let i = -w; i < w*2; i+=80) {
            ctx.beginPath(); ctx.moveTo(i - tapeOffset, -25); ctx.lineTo(i + 35 - tapeOffset, -25); ctx.lineTo(i - tapeOffset, 25); ctx.lineTo(i - 35 - tapeOffset, 25); ctx.fill();
        }
        ctx.font = "900 25px 'Arial Black'"; ctx.fillStyle = "#fff"; ctx.textAlign = "center"; ctx.textBaseline = "middle";
        ctx.shadowColor = "#000"; ctx.shadowBlur = 10;
        for(let j = -1; j <= 2; j++) { ctx.fillText("⚠️ WARNING: EXTREME DANGER ⚠️", w/2 + (j*800) - tapeOffset*5, 0); }
        ctx.restore();
    };
    drawHazardTape(30, 1); drawHazardTape(h - 30, -1);

    // VẼ KHUNG THOẠI TRASH TALK NẢY (ELASTIC BOUNCE)
    const drawSpeechAndEmoji = (x, y, text, isP1, emojiStr, popProgress) => {
        ...
        ctx.fillStyle = "#fff"; ctx.strokeStyle = "#000"; ctx.lineWidth = 8; ctx.shadowColor = "#000"; ctx.shadowOffsetX = 10; ctx.shadowOffsetY = 10;
        ...
        ctx.save();
        let emX = isP1 ? tw/2 : -tw/2; let emY = -th/2 - 10;
        let tPulse = Date.now() / 60; 
        ctx.translate(emX, emY + Math.sin(tPulse) * 15); ctx.rotate(Math.sin(tPulse * 0.5) * 0.4);
        ctx.font = "80px Arial"; ctx.shadowColor = "#000"; ctx.shadowBlur = 15; ctx.fillText(emojiStr, 0, 0);
        ctx.restore();
    ...
    drawSpeechAndEmoji(isVertical ? w*0.5 : w*0.25, isVertical ? h*0.1 : h*0.3, window.trashTalkP1, true, window.introEmojiP1, p1Pop);
    
    // Nảy Khung 2
    let p2Pop = Math.min(1, Math.max(0, (progress - 0.5) * 6)); 
    drawSpeechAndEmoji(isVertical ? w*0.5 : w*0.75, isVertical ? h*0.85 : h*0.7, window.trashTalkP2, false, window.introEmojiP2, p2Pop);

    // [RGB GLITCH] Chuyển cảnh vỡ màn hình xé màu
    if (progress > 0.85) {
        let flashAlpha = Math.min(1, (progress - 0.85) * 6.6);
        ctx.globalCompositeOperation = 'screen';
        
        ctx.drawImage(ctx.canvas, -30 * flashAlpha, 0);
        ctx.fillStyle = `rgba(255, 0, 0, ${flashAlpha * 0.8})`; ctx.fillRect(-w, -h, w*3, h*3);
        
        ctx.drawImage(ctx.canvas, 30 * flashAlpha, 0);
        ctx.fillStyle = `rgba(0, 255, 255, ${flashAlpha * 0.8})`; ctx.fillRect(-w, -h, w*3, h*3);
...
window.startRecording = function() {
    ...
    // RANDOM THOẠI TRASH TALK NGẦU LÒI
    ...
    // BAKE THUMBNAIL TĨNH CHO 0.5S ĐẦU TIÊN
    window.bakeThumbnailsForVideo(window.StoryModeAI.viralTitle);
    window.thumbnailHoldFrames = 30; // 0.5 giây ảnh tĩnh Meme
    window.introHoldFrames = window.totalIntroFrames; // 2.5 giây Intro Động
...
window.captureFrames = function() {
    ...
    // 🔥 TỰ ĐỘNG CẮT VIDEO SAU K.O KHOẢNG 6 GIÂY (Cho người xem kịp đọc)
    ...
    // GIAI ĐOẠN 1: IN THUMBNAIL TĨNH (0.5s ĐỂ BẮT ẢNH BÌA)
    ...
    if (window.introHoldFrames > 0) {
        ...
        return; // Đứng lại ở đây, không render gameplay
    }
