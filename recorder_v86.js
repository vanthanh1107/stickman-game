...
// Kèm theo TOXIC LIVE CHAT & CELEB POP-UP COMMENTS (1 giây/1 comment)
// ==========================================
...
window.captureFrames = function() {
    ...
    // CHÍNH GIỮA: VTuber hiện ra bình luận
    if (!window.gameOver && window.introTimer > 120) { drawVTuberCommentator(ctxV, 540, splitGameHeight + 250, audioPeak, window.StoryModeAI.isTyping); }
    ...
    // ==========================================
    // 6. BINGE-WATCH GRID (HIỆN LÊN KHI HẾT TRẬN)
    // ==========================================
    if (window.gameOver && window.matchEndTimer > 90) { 
        ...
        let drawOutroCTA = (ctx, w, h, isMobile) => {
            ...
            if (isMobile) {
                ...
                // Múi giờ Outro Button được đẩy cao lên một chút để chừa chỗ dưới đáy cho Khung Chat
                ctx.textAlign = "center";
                let btnWidth = 640; let btnHeight = 160; let btnY = cy + 150 + floatY; 
    ...
    // ==========================================
    // 7. TOXIC LIVE CHAT & CELEB POP-UP (LUÔN LUÔN NỔI LÊN ĐẾN HẾT VIDEO)
    // ==========================================
    
    if (!window.retentionToolsInit) {
        window.retentionToolsInit = true;
        
        window.chatNames = ["IShowSpeed", "ElonMusk", "MrBeast", "KaiCenat", "xQc", "Faker_T1", "Tyler1", "GordonRamsay", "DonaldTrump", "SnoopDogg", "AndrewTate", "PewDiePie", "Ninja", "Drake", "KanyeWest", "IShowSpeed_Fan", "SigmaGrindset"];
        window.chatMsgs = [
            "Bro plays like he's using a steering wheel 💀", 
            "My grandma has better combos 🤡", 
            "DELETE THE GAME NOW", 
            "Is this a bot or just a bad player? 🤖", 
            "Uninstall pls 🗑️", 
            "I'm billing you for my wasted time 🤬", 
            "Even Jesus can't save this gameplay 🙏", 
            "Bro's fighting demons... and losing.", 
            "I could do this blindfolded 🙈", 
            "Imagine sweating this hard and still being trash 🗑️",
            "Someone take his PC away immediately.",
            "This is why aliens don't visit us 👽",
            "Bro's actively trying to get negative views.",
            "GordonRamsay: This gameplay is absolutely RAW! 🥩",
            "Skill issue spotted 📸",
            "Bro is sweating over a mobile game 💦",
            "Touch grass immediately 🌱"
        ];
        window.hypeMsgs = ["HE IS LITERALLY HACKING 🚨", "REPORTED FOR AIMBOT 🎯", "NAHHHH WHAT WAS THAT COMBO 💀💀💀", "BRO COOKED 🔥🔥🔥", "W W W W W W", "WTF IS THIS DAMAGE?!", "MY EYES ARE BLEEDING BUT I CAN'T LOOK AWAY", "I STOPPED BREATHING", "Bro unlocked Ultra Instinct 🥶", "MOM GET THE CAMERA 📸", "ABSOLUTE CINEMA 🍿"];
        
        window.fakeChat = [];
        
        // Render đầy luôn 7 tin nhắn ngay từ đầu để tránh lỗi trống trơn biến mất
        for(let i = 0; i < 7; i++) {
            window.fakeChat.push({
                name: window.chatNames[Math.floor(Math.random() * window.chatNames.length)],
                msg: window.chatMsgs[Math.floor(Math.random() * window.chatMsgs.length)],
                color: `hsl(${Math.random() * 360}, 80%, 70%)`
            });
        }
        window.lastChatTime = Date.now();

        // --- KHỞI TẠO POP COMMENT SYSTEM ---
        window.popCommentState = {
            lastTime: 0,
            current: null,
            colors: ["#ff4757", "#2ed573", "#1e90ff", "#3742fa", "#ffa502", "#ff6348", "#ff00ff", "#00d2d3"],
            list: [
                {name: "@IShowSpeed", text: "BRO IS LITERALLY TRASH 😭🗑️"},
                {name: "@Faker", text: "I could win this blindfolded..."},
                {name: "@ElonMusk", text: "Deleting his account next week. 🚀"},
                {name: "@GordonRamsay", text: "THIS COMBO IS F***ING RAW! 🤬"},
                {name: "@KaiCenat", text: "NAHHH BRO IS COOKED 💀💀💀"},
                {name: "@MrBeast", text: "I'll pay someone $50k to unsee this."},
                {name: "@Drake", text: "Even I wouldn't write this script ✍️"},
                {name: "@Tyler1", text: "GET THIS TRASH OFF MY SCREEN!!! AHHHH"},
                {name: "@xQc", text: "Düd wtf is this düd 💀"},
                {name: "@SnoopDogg", text: "Bro needs to chill and touch grass 🌿"},
                {name: "@Ninja", text: "Not seeing enough movement 🤡"},
                {name: "@AndrewTate", text: "What color is your combo? Broke behavior."},
                {name: "@ConorMcGregor", text: "Absolute garbage! Get out! 🥊"},
                {name: "@Khabib", text: "Send me location. I smash him. 🦅"},
                {name: "@Jynxzi", text: "BAD AIM! BAD MECHANICS! 😡"}
            ]
        };
    }

    if (window.introTimer > 120) {
        let nowTime = Date.now();
        let isHype = audioPeak > 0.6; // Đang combat căng

        // Cứ 1.5 - 3.5 giây nhả 1 comment cho Live Chat, riêng lúc combat thì cứ 0.15s nhả 1 cái
        let chatDelay = isHype ? 150 : (1500 + Math.random() * 2000); 
        ...
        if (nowTime - window.lastChatTime > chatDelay) {
            ...
            if (window.fakeChat.length > 7) window.fakeChat.shift(); // Giữ chuẩn 7 comment full khung
        }

        ctxV.save();
        let chatBoxWidth = 850;
        let chatBoxHeight = 450;
        let chatBoxX = 540 - (chatBoxWidth / 2); // Căn giữa TikTok
        let chatBoxY = 1440; // Đặt neo vĩnh viễn ở sát đáy màn hình để không đè vào VTuber hay Nút Outro

        ctxV.translate(chatBoxX, chatBoxY); 
        
        // Vẽ background kính mờ cho hộp Chat
        ctxV.fillStyle = "rgba(10, 15, 30, 0.75)";
        ctxV.strokeStyle = "rgba(0, 243, 255, 0.4)";
        ctxV.lineWidth = 3;
        if(ctxV.roundRect) { 
            ctxV.beginPath(); ctxV.roundRect(0, 0, chatBoxWidth, chatBoxHeight, 20); ctxV.fill(); ctxV.stroke(); 
        } else { 
            ctxV.fillRect(0, 0, chatBoxWidth, chatBoxHeight); ctxV.strokeRect(0, 0, chatBoxWidth, chatBoxHeight); 
        }
        ctxV.beginPath(); if(ctxV.roundRect) ctxV.roundRect(0, 0, chatBoxWidth, chatBoxHeight, 20); ctxV.clip();

        // Vẽ Header Khung Chat
        ctxV.fillStyle = "#ff0055"; ctxV.fillRect(0, 0, chatBoxWidth, 60);
        ctxV.fillStyle = "#fff"; 
        ctxV.font = "900 32px 'Arial Black'"; ctxV.textAlign = "left"; ctxV.textBaseline = "middle";
        ctxV.fillText("🔴 LIVE CHAT", 25, 30);
        
        ctxV.font = "900 24px 'Arial'"; ctxV.textAlign = "right";
        ctxV.fillText("👥 1.2M VIEWERS", chatBoxWidth - 25, 30);

        ctxV.font = "bold 32px 'Arial'"; ctxV.textAlign = "left"; ctxV.textBaseline = "top";
        
        // Vòng lặp render chat ngược từ dưới lên (cảm giác cuộn êm)
        for (let i = 0; i < window.fakeChat.length; i++) {
            let c = window.fakeChat[i];
            let drawY = chatBoxHeight - 20 - ((window.fakeChat.length - i) * 52); 
            
            // Vẽ Tên Player
            ctxV.fillStyle = c.color;
            ctxV.fillText(c.name + ":", 25, drawY);
            
            // Vẽ Comment Toxic
            ctxV.fillStyle = "#ffffff";
            let nameWidth = ctxV.measureText(c.name + ": ").width;
            ctxV.fillText(c.msg, 25 + nameWidth + 12, drawY);
        }

        // Viền đỏ chớp liên tục lúc gồng chiêu
        if (isHype) {
            ctxV.strokeStyle = "rgba(255, 0, 85, 0.5)"; ctxV.lineWidth = 10;
            if(ctxV.roundRect) { ctxV.beginPath(); ctxV.roundRect(0, 0, chatBoxWidth, chatBoxHeight, 20); ctxV.stroke(); }
        }
        ctxV.restore();

        // ==========================================
        // 8. POP-UP CELEBRITY STICKER COMMENT (ĐỔI SAU MỖI 1 GIÂY)
        // ==========================================
        if (nowTime - window.popCommentState.lastTime >= 1000) {
            window.popCommentState.lastTime = nowTime;
            let randCeleb = window.popCommentState.list[Math.floor(Math.random() * window.popCommentState.list.length)];
            window.popCommentState.current = {
                name: randCeleb.name,
                text: randCeleb.text,
                avatarColor: window.popCommentState.colors[Math.floor(Math.random() * window.popCommentState.colors.length)]
            };
        }

        if (window.popCommentState.current) {
            const drawPopSticker = (ctx, cx, cy, scaleMult) => {
                ctx.save();
                ctx.translate(cx, cy);
                ...
                // Box giao diện Tiktok Comment
                ctx.fillStyle = "rgba(255, 255, 255, 0.95)";
                ctx.shadowColor = "rgba(0, 0, 0, 0.3)";
                ctx.shadowBlur = 20;
                ctx.shadowOffsetY = 10;
                ctx.beginPath();
                if(ctx.roundRect) ctx.roundRect(-boxW/2, -boxH/2, boxW, boxH, 30);
                else ctx.rect(-boxW/2, -boxH/2, boxW, boxH);
                ctx.fill();
                ctx.shadowBlur = 0; ctx.shadowOffsetY = 0;

                // Avatar User
                ctx.fillStyle = window.popCommentState.current.avatarColor;
                ctx.beginPath();
                ctx.arc(-boxW/2 + 80, 0, 50, 0, Math.PI*2);
                ctx.fill();
                ctx.fillStyle = "#fff";
                ctx.font = "bold 50px Arial";
                ctx.textAlign = "center";
                ctx.textBaseline = "middle";
                ctx.fillText(window.popCommentState.current.name.charAt(1).toUpperCase(), -boxW/2 + 80, 5);

                // Nút Thả tim (Tiktok Like button)
                ctx.fillStyle = "#9ca3af";
                ctx.font = "35px Arial";
                ctx.fillText("🤍", boxW/2 - 60, -10);
                ctx.font = "bold 20px Arial";
                ctx.fillText("99.9K", boxW/2 - 60, 30);

                // Thông tin Name
                ctx.textAlign = "left";
                ctx.fillStyle = "#6b7280"; 
                ctx.font = "bold 32px 'Arial'";
                ctx.fillText(window.popCommentState.current.name + " • just now", -boxW/2 + 155, -35);

                // Nội dung Comment
                ctx.fillStyle = "#111827"; 
                ctx.font = "bold 38px 'Arial'";
                ctx.fillText(window.popCommentState.current.text, -boxW/2 + 155, 20);

                ctx.restore();
            };

            let popPulse = 1 + (audioPeak * 0.08); // Nhảy kích cỡ nhẹ theo nhạc
            
            // Vẽ trên bản Dọc (Giữa màn hình, nằm dưới bụng VTuber)
            drawPopSticker(ctxV, 540, 1150, popPulse);
            
            // Vẽ trên bản Ngang
            if (renderNormalH) {
                drawPopSticker(ctxH, 960, 920, popPulse);
            }
        }
    }
