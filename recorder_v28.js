// ==========================================
// RECORDER.JS - BẢN HOÀN HẢO CHO CONTENT CREATOR (V54.3 - EMBEDDED THUMBNAIL FIXED)
...
window.startRecording = function() {
    // Nếu có trận cũ quay dở, chốt sổ liền
    ...
    const finalizeRecordings = () => {
        ...
        if (stoppedCount === 2) { 
            ...
            }, 500); // Chờ 500ms để trình duyệt gom đủ Chunk
        }
    };

    window.mediaRecorderH.onstop = finalizeRecordings; window.mediaRecorderV.onstop = finalizeRecordings;
    
    // [FIX] BẮT BUỘC TRÌNH DUYỆT XUẤT DATA MỖI 1 GIÂY ĐỂ CHỐNG LỖI CORRUPT HOẶC EMPTY FILE
...
window.stopRecording = function() { 
    ...
    // NGƯNG DÙNG requestData() KHI DÙNG KÈM start(timeslice) Ở CÁC TRÌNH DUYỆT CHROMIUM TRÁNH GÂY LỖI
...
window.captureFrames = function() {
    ...
    // 🔥 [AUTO CUT] TỰ ĐỘNG TẮT RECORD SAU KHI K.O 2 GIÂY
    ...
    // 🔥 XỬ LÝ NHÚNG THUMBNAIL TRỰC TIẾP VÀO VIDEO (1.5 GIÂY ĐẦU TIÊN LÀ ẢNH)
    if (window.thumbnailHoldFrames > 0) {
        ...
        return; // Ép dừng capture khung hình game, để video lúc này chỉ quay lại cái Thumbnail tĩnh!
    }
