// ==========================================
// RECORDER.JS - V69.0 CLEAN CHAT LAYOUT (PERFORMANCE & UI OPTIMIZED)
// Tối ưu Layout: Clean Game -> HUD -> Poll -> Smart Alerts -> Full Height ChatBox
// ĐÃ XÓA: Con mèo phông xanh & Vòng xoay sóng nhạc.
// ĐÃ NÂNG CẤP: Kéo dài bảng chat, Huy hiệu 🔴 LIVE đặt nổi góc trên phải.
// ĐÃ FIX: Lỗi giật lag game, lỗi Modal tải video, thông báo Rendering.
...
// ====================================================================
// KHO NGƯỜI NỔI TIẾNG ĐÌNH ĐÁM TOÀN CẦU (100% DÙNG CHO CHAT)
// ====================================================================
window.CELEB_LIST = [
    ...
    { name: "Joe Biden ⛳", color: "#3498db" }, { name: "Obama 🍦", color: "#ffffff" },
    { name: "Ryan Reynolds 🤣", color: "#e74c3c" }, { name: "Will Smith 😤", color: "#f39c12" },
    { name: "Justin Bieber 🕺", color: "#00f3ff" }, { name: "Goku 🐉", color: "#ff9ff3" },
    { name: "Batman 🦇", color: "#ffffff" }, { name: "Thanos 🧤", color: "#a29bfe" },
    { name: "Homelander 🇺🇸", color: "#ff4757" }, { name: "Saitama 👊", color: "#f39c12" }
];

// ====================================================================
// ENGINE AI: TẠO TOXIC COMMENTS SIÊU ĐA DẠNG DÀNH RIÊNG CHO CELEB
// ====================================================================
window.generateLiveChatEvent = function() {
    const r = (arr) => arr[Math.floor(Math.random() * arr.length)];

    let celeb = r(window.CELEB_LIST);
    let randomName = celeb.name;
    let randomColor = celeb.color;

    const targets = ["Bro", "Blud", "This dude", "Lil bro", "My guy", "OP", "The player", "Broski", "Dawg", "Chat,", "Buddy"];
    const brutalActions = [
        ...
        "is the definition of dogwater", "should be permanently banned", "is fighting against air and losing",
        "has the reaction time of a dead turtle", "is single-handedly ruining this game",
        "paid $0 for this game and still got scammed", "is playing on a smart fridge", 
        "makes me want to bleach my eyes", "is allergic to hitting a combo",
        "is sweating for zero views", "forgets how to play", "thinks he is him",
        "is playing with the monitor off"
    ];
    const supremeInsults = [
        "Absolute garbage", "Brain rot gameplay", "Negative aura", "Uninstall immediately",
        "I'm billing you for my wasted time", "Even my dead goldfish plays better",
        "Sell your PC bro", "Throw your keyboard out the window", "Literally dogmeat",
        "You are completely washed", "This is pure torture to watch", "Skill issue of the century",
        "NPC behavior spotted", "My grandma hits better combos", "0 IQ gameplay"
    ];
    const savageShorts = [
        "L", "LMAO NAHH", "WASHED", "COOKED", "RIP BOZO", "PLEASE STOP", "MY EYES", "TRASH", "AINT NO WAY",
        "DELETE THIS", "CRINGE", "ZERO IQ", "NPC DETECTED", "BOT BEHAVIOR", "PACK IT UP BRO", "FREE ELO"
    ];
    const gear = ["combo", "keyboard", "monitor", "gaming chair", "wifi", "mouse", "PC", "brain", "router"];
    const emojis = ["💀", "😭", "🗑️", "🤡", "📉", "🤮", "💩", "🤬", "👎", "🤣", "🤦‍♂️", "💀💀", "💀😭", "🚑", "♿", "☢️"];

    const randomButtons = () => {
        const btns = ["A", "B", "X", "Y", "R1", "L2", "Space", "Click"];
        return `${r(btns)} + ${r(btns)} + ${r(btns)} spammer`;
    };
    ...
    const toxicityTemplates = [
        ...
        () => `I lost brain cells watching this. ${r(savageShorts)} ${r(emojis)}`,
        ...
        () => `${r(targets)} thought he cooked... he burnt down the whole kitchen 🔥💀`,
        () => `Never play this game again. For everyone's sake. 🙏🗑️`,
        () => `Bro is literally an NPC. 🤖📉`,
        () => `${r(targets)} makes the tutorial look hard 😭`,
        () => `A roomba could get a better combo 🤡`,
        () => `What color is your ${r(gear)}? Broke ${r(emojis)}`,
        () => `Someone take his ${r(gear)} away immediately ${r(emojis)}`,
        () => `${r(savageShorts).toUpperCase()} WHAT WAS THAT ${r(emojis)}`,
        () => `Bro's ${r(gear)} is crying right now 😭`,
        () => `Reported for being this bad 💀`,
        () => `${randomButtons()} 🤡`,
        () => `Bro's aura is -9999 📉`,
        () => `Even TikTok is lagging from this gameplay 🗑️`
    ];
...
// ====================================================================
// ENGINE AI NỘI BỘ TẠO TIÊU ĐỀ & HASHTAG (ĐÃ ĐỔI NGÁCH)
// ====================================================================
window.StoryModeAI = {
    ...
    generateViralStickmanTitle: function(hero, enemy) {
        ...
        const actions = ["PSYCHOLOGICAL WARFARE", "LOBOTOMY COMBO", "SCHIZO MOVEMENT", "GASLIGHTING", "TAS MOVEMENT", "PIXEL PERFECT PARRY", "EDGE-GUARDING"];
        ...
        const templates = [
            ...
            `THE MOST [ADJ] DISRESPECT IN GAMING HISTORY [REACTION]`,
            `EDGING THE PARRY: [ADJ] [ACTION] EXPLAINED [REACTION]`
        ];
        ...
        const allTags = ["#schizomovement", "#psychologicalwarfare", "#villainarc", "#gaming", "#brainrot", "#tas", "#movementplayer", "#lobotomy", "#disrespect", "#clutch", "#iq999", "#ragequit", "#forbidden"];
        let shuffledTags = allTags.sort(() => 0.5 - Math.random());
        let selectedTags = shuffledTags.slice(0, 4).join(" ");
    ...
    generateScript: function(hero, enemy) { return { title: this.generateViralStickmanTitle(hero, enemy), lines: [] }; },
    init: function(hero, enemy) { let narrative = this.generateScript(hero, enemy); this.scriptLines = narrative.lines; this.viralTitle = narrative.title; this.currentLineIndex = 0; this.fullText = ""; this.displayedText = ""; this.charIndex = 0; this.isTyping = false; this.scriptProgress = 0; },
    playNextLine: function() {}, stop: function() {}
};

window.sanitizeFileName = function(str) { return str.replace(/[^a-z0-9\s_-]/gi, '').trim().replace(/\s+/g, '_'); };

// ==========================================
// HUY HIỆU 🔴 LIVE NỔI BÊN PHẢI BẢNG CHAT
// ==========================================
function drawLiveBadge(ctx, x, y, audioPeak) {
    ...
    ctx.beginPath(); 
    if(ctx.roundRect) ctx.roundRect(-70, -25, 140, 50, 15); else ctx.fillRect(-70, -25, 140, 50); 
    ctx.fill();
    ctx.shadowBlur = 0;
    ctx.fillStyle = "#fff"; 
...
window.bakeThumbnailsForVideo = function(titleText) {
    ...
    try {
        ...
        const drawCharSafe = (ctx, charObj, cx, cy, scale, isFacingRight) => {
            ...
            if (clone.isDragon && typeof window.drawDragon === 'function') window.drawDragon(ctx, clone); else if (typeof window.drawStickman === 'function') window.drawStickman(ctx, clone);
            ctx.restore();
        };
        ...
        const draw3DTitle = (ctx, w, h, text, isVertical) => {
            ctx.save();
            let grad = ctx.createRadialGradient(w/2, h/2, h*0.3, w/2, h/2, Math.max(w,h)); grad.addColorStop(0, "rgba(0,0,0,0)"); grad.addColorStop(1, "rgba(0,0,0,0.85)");
...
window.drawAnimatedIntro = function(ctx, w, h, isVertical, progress) {
    ...
    const drawMarquee = (yPos, text, speedMult, bgCol) => {
        ctx.save(); ctx.translate(0, yPos); ctx.rotate(isVertical ? -0.06 : -0.03); ctx.fillStyle = bgCol; ctx.fillRect(-w, -35, w*3, 70);
    ...
    ctx.save(); let p2Clone = Object.assign({}, e1, {x:0, y:0, scale: isVertical ? 4.7 : 6.0, state: 'cast', isFacingRight: false}); ctx.translate(p2X, p2TargetY); ctx.scale(-1, 1);
    ...
    if (isVertical) { ctx.moveTo(-w,-h); ctx.lineTo(w*2,-h); ctx.lineTo(w*2, h*0.48); ctx.lineTo(-w, h*0.52); } else { ctx.moveTo(-w,-h); ctx.lineTo(w*0.55, -h); ctx.lineTo(w*0.45, h*2); ctx.lineTo(-w, h*2); } ctx.fill();
    ctx.save(); ctx.beginPath();
    if (isVertical) { ctx.moveTo(-w,-h); ctx.lineTo(w*2,-h); ctx.lineTo(w*2, h*0.48); ctx.lineTo(-w, h*0.52); } else { ctx.moveTo(-w,-h); ctx.lineTo(w*0.55, -h); ctx.lineTo(w*0.45, h*2); ctx.lineTo(-w, h*2); } ctx.clip(); 
    ...
    ctx.save(); let p1Clone = Object.assign({}, window.p1, {x:0, y:0, scale: isVertical ? 4.7 : 6.0, state: 'cast', isFacingRight: true}); ctx.translate(p1X, p1TargetY); 
    ...
    if (isVertical) { ctx.moveTo(-w, h*0.52 + w*0.02); ctx.lineTo(w*2, h*0.48 - w*0.02); } else { ctx.moveTo(w*0.55 + h*0.02, -h); ctx.lineTo(w*0.45 - h*0.02, h*2); } ctx.stroke();
    ...
    let p1Pop = Math.min(1, Math.max(0, (progress - 0.2) * 8)); drawSpeechAndEmoji(isVertical ? w*0.5 : w*0.25, isVertical ? h*0.15 : h*0.25, window.trashTalkP1, true, window.introEmojiP1, p1Pop);
    let p2Pop = Math.min(1, Math.max(0, (progress - 0.4) * 8)); drawSpeechAndEmoji(isVertical ? w*0.5 : w*0.75, isVertical ? h*0.85 : h*0.75, window.trashTalkP2, false, window.introEmojiP2, p2Pop);
    if (progress > 0.85) {
...
window.initRecorder = function() {
    ...
    window.recordCanvasH.width = 1920; window.recordCanvasH.height = 1080; window.recordCanvasH.style.cssText = "position: absolute; top: 0; left: 0; width: 1px; height: 1px; opacity: 0.01; pointer-events: none; z-index: -9999;";
...
// ==========================================
// TOAST NOTIFICATION KHI RENDERING
// ==========================================
    ...
    const finalizeRecordings = () => {
        ...
        if (stoppedCount === 2) { 
            setTimeout(() => {
                ...
                if (window.recordedChunksH.length === 0 || window.recordedChunksV.length === 0) { alert("⚠️ Video extraction failed! Please refresh the page (F5), wait 3 seconds, and try again."); return; }
...
// ==========================================
// NGẮT RECORD AN TOÀN TRÁNH MẤT FILE
// ==========================================
...
window.captureFrames = function() {
    ...
    // GIỚI HẠN RECORDER 30 FPS ĐỂ GAME MƯỢT MÀ
    let now = Date.now();
    if (now - window._lastCaptureTime < 33) return; 
    ...
    if (window.thumbnailHoldFrames > 0) {
        if (window.bakedThumbH) ctxH.drawImage(window.bakedThumbH, 0, 0, 1920, 1080); window.thumbnailHoldFrames--; renderNormalH = false; 
    } 
        ...
        window.drawAnimatedIntro(ctxH, 1920, 1080, false, progress); window.introHoldFrames--; renderNormalH = false; 
    }
    ...
    if (!isOutroActive) {
        ...
        if (!window._chatSystemInit) {
            ...
            for(let i = 0; i < 8; i++) { window._liveChats.push(window.generateLiveChatEvent()); }
        }
        let chatNow = Date.now();
        if (chatNow - window._lastChatUpdate > window._nextChatDelay) {
            window._lastChatUpdate = chatNow; window._nextChatDelay = 1000 + Math.random() * 2000; window._liveChats.push(window.generateLiveChatEvent());
            if (window._liveChats.length > 12) window._liveChats.shift(); // Lưu nhiều chat hơn để trôi
        ...
        // --- SETUP THÔNG SỐ KHUNG CHAT KÉO DÀI ---
        let boxWidth = 960;
        let boxHeight = 780; // Tăng chiều cao lên rất nhiều (cũ 440)
        let boxX = 540; 
        let boxY = 1450; // Kéo tâm bảng chat xuống dưới
        
        let chatTopY = boxY - boxHeight/2; // Tính toán ra khoảng Y = 1060

        // --- HIỂN THỊ ALERT Ở NGAY TRÊN BẢNG CHAT MỘT CHÚT ---
        let alertStartY = chatTopY - 20; 
        for (let i = window._liveAlerts.length - 1; i >= 0; i--) {
            let al = window._liveAlerts[i];
            al.life -= 0.015; al.yOffset += 1.2; 
        ...
        // --- VẼ HUY HIỆU 🔴 LIVE NỔI BÊN PHẢI ---
        // Vị trí: Đặt ngay góc trên bên phải của bảng chat
        let liveBadgeX = boxX + boxWidth/2 - 70; // X = 950
        let liveBadgeY = chatTopY - 35; // Nổi lên trên mép viền chatbox 1 chút
        drawLiveBadge(ctxV, liveBadgeX, liveBadgeY, audioPeak);

        // --- KHUNG LIVE CHAT MỞ RỘNG ---
        ...
        for (let i = window._liveChats.length - 1; i >= 0; i--) { 
            ...
            let words = chat.msg.split(' '); let lines = []; let currentLine = "";
            for(let n = 0; n < words.length; n++) {
                let testLine = currentLine + words[n] + " "; let metrics = ctxV.measureText(testLine);
                if(metrics.width > maxMsgWidth && n > 0) { lines.push(currentLine.trim()); currentLine = words[n] + " "; } 
            ...
            for(let l = lines.length - 1; l >= 0; l--) {
                ...
                if (l === 0) {
                    ...
                    ctxV.fillStyle = "#ffffff"; ctxV.fillText(" " + lines[l], -boxWidth/2 + 30 + nameWidth, currentY);
                } else {
                    ctxV.fillStyle = "#ffffff"; ctxV.fillText(" " + lines[l], -boxWidth/2 + 30 + nameWidth, currentY);
                }
                currentY -= lineHeight; 
        ...
        let drawOutroCTA = (ctx, w, h, isMobile) => {
            ...
            if (isMobile) {
                ctx.fillStyle = "#ffffff"; ctx.font = `900 65px 'Arial Black', sans-serif`; ctx.fillText("BINGE WATCH MY PAGE 👀", cx, cy - 500 + floatY);
                ctx.fillStyle = "#ff0050"; ctx.font = `900 45px 'Montserrat', sans-serif`; ctx.fillText("👇 MORE COMBOS BELOW 👇", cx, cy - 430 + floatY);
                let gridY = cy - 350 + floatY; ctx.lineWidth = 4;
                for(let i=-1; i<=1; i++) {
                    let rx = cx + i*340 - 150;
                    ctx.fillStyle = "#111827"; ctx.strokeStyle = (i===0) ? "#00f3ff" : "#334155";
                    ctx.beginPath(); if(ctx.roundRect) ctx.roundRect(rx, gridY, 300, 450, 20); else ctx.rect(rx, gridY, 300, 450); ctx.fill(); ctx.stroke();
                    ctx.fillStyle = "#ffffff"; ctx.font = "900 35px Arial Black"; ctx.textAlign = "left";
                    let views = (i===-1) ? "1.2M" : (i===0) ? "3.4M" : "800K"; ctx.fillText(`▶ ${views}`, rx + 20, gridY + 410);
                }
                ctx.textAlign = "center"; let btnWidth = 640; let btnHeight = 160; let btnY = cy + 200 + floatY; 
                ...
                ctx.fillStyle = btnGrad; ctx.beginPath(); if(ctx.roundRect) ctx.roundRect(-btnWidth/2, -btnHeight/2, btnWidth, btnHeight, btnHeight/2); else ctx.rect(-btnWidth/2, -btnHeight/2, btnWidth, btnHeight); ctx.fill();
                ...
                ctx.fillStyle = "#00f3ff"; ctx.font = `italic 900 107px 'Arial Black', sans-serif`; ctx.fillText("CREATE YOUR OWN CHARACTER", cx, cy - 200 + floatY);
                ctx.fillStyle = "#ffeb3b"; ctx.font = `900 73px 'Arial Black', sans-serif`; ctx.fillText("BY DESCRIBING IT!", cx, cy - 80 + floatY);
                ctx.fillStyle = "#ffffff"; ctx.font = `bold 47px 'Montserrat', sans-serif`; ctx.fillText("Check Link in Bio / Comments 👇", cx, cy + 27 + floatY);
