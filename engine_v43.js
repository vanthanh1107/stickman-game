// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 37.0 [THE GODLIKE IMPACT]
// [BASE] Giữ nguyên 100% của V36.0 (Matrix Orbit 360, Kinetic Text, Lethal Void).
// [NEW] DYNAMIC POINT ILLUMINATION: Nguồn sáng 3D bùng nổ ngay điểm va chạm, hắt sáng cả môi trường.
// [NEW] COMBO ESCALATION CAMERA: Combo càng dài, camera càng ép sát và vặn xoắn góc nhìn.
// [NEW] ROTATIONAL SCREEN SHAKE: Rung lắc màn hình kết hợp vặn nghiêng tạo độ nặng vật lý.
// [NEW] MEATY HIT-LAG: Thời gian khựng đòn tăng tiến theo số hit combo.
// ==========================================
...
// [VIRAL 37.0] THE GODLIKE IMPACT VARS
window.impactLights = []; // Lưu trữ các điểm sáng vật lý
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce, attackerDirRight = true) {
    ...
    if (finalDmg > 0) {
        ...
        // [VIRAL 37.0] DYNAMIC POINT ILLUMINATION
...
window.attack = function(attacker, targetGroup) {
    ...
    // [VIRAL 37.0] MEATY HIT-LAG SCALING (Đòn combo càng cao, độ khựng thời gian càng gắt)
...
window.update = function() {
    ...
    if (window.impactLights) {
        window.impactLights.forEach(l => l.life--);
        window.impactLights = window.impactLights.filter(l => l.life > 0);
    }
    ...
    // [VIRAL 37.0] COMBO ESCALATION CAMERA: Ép góc quay theo số Hit
    let maxComboHits = Math.max(window.p1 ? window.p1.comboHits || 0 : 0, closest ? closest.comboHits || 0 : 0);
    let comboZoomBonus = Math.min(0.5, maxComboHits * 0.04); 
    let comboTiltBonus = maxComboHits > 4 ? Math.sin(window.matchTimer * 0.3) * (Math.min(0.1, maxComboHits * 0.01)) : 0;
    ...
    if (window.gameOver && closest) {
        ...
        window.targetZoom = Math.max(0.9, Math.min(1.35 + comboZoomBonus, dynamicZoom + comboZoomBonus));
...
window.draw = function() {
    ...
    if (window.isLoading) {
        ...
        window.ctx.fillStyle = "#00f3ff"; window.ctx.font = "italic 900 40px Arial"; window.ctx.textAlign = "center"; window.ctx.shadowBlur = 15; window.ctx.shadowColor = "#00f3ff"; window.ctx.fillText("GODLIKE IMPACT INITIALIZING...", cx, cy - 50);
        ...
        window.ctx.fillStyle = "#fff"; window.ctx.font = "bold 16px monospace"; window.ctx.fillText(`RENDERING DYNAMIC LIGHTING... ${Math.floor(window.loadProgress)}%`, cx, cy + 60);
        return; 
    }
    ...
    try {
        ...
        // [VIRAL 37.0] ROTATIONAL SCREEN SHAKE (Rung lắc có vặn xoắn)
        ...
        // [VIRAL 37.0] DYNAMIC POINT ILLUMINATION (ĐÈN HẮT TỪ ĐIỂM VA CHẠM)
        if (window.impactLights && window.impactLights.length > 0) {
            ...
            window.impactLights.forEach(light => {
                let pr = project3D(light);
                let prog = light.life / light.maxLife;
                window.ctx.globalAlpha = prog * (light.isCrit ? 0.9 : 0.6);
                let r = (light.isCrit ? 600 : 300) * Math.sin(prog * Math.PI) * pr.drawScale; // Sáng bùng rồi tắt
                if (r > 0) {
                    let grad = window.ctx.createRadialGradient(pr.drawX, pr.drawY, 0, pr.drawX, pr.drawY, r);
                    grad.addColorStop(0, light.color);
                    grad.addColorStop(1, 'rgba(0,0,0,0)');
                    window.ctx.fillStyle = grad;
                    window.ctx.beginPath();
                    window.ctx.arc(pr.drawX, pr.drawY, r, 0, Math.PI*2);
                    window.ctx.fill();
                }
            });
            window.ctx.restore();
