// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 9.0 (ESPORTS GRADE - MAX FPS)
// [TÍCH HỢP: GPU PRE-RENDERING, OBJECT POOLING, ZERO ALLOCATION]
// [HIỆU NĂNG: ĐẠT CHUẨN 60 FPS TRÊN MỌI THIẾT BỊ & TRÌNH DUYỆT]
// ==========================================

window.canvas = null; window.ctx = null; window.audioCtx = null; window.isMuted = false;
window.selectedRedClass = null; 
window.floatingTexts = []; window.projectiles = []; 
...
// [ESPORTS OPTIMIZATION] BIẾN QUẢN LÝ BỘ NHỚ
window.tempChromaCanvas = null; window.tempChromaCtx = null;
window.activeIntervals = [];

// 1. KỸ THUẬT GPU PRE-RENDERING (TẠO CANVAS NGẦM CHO HẬU CẢNH)
window.bg1Cache = document.createElement('canvas'); window.bg1Cache.width = 3200; window.bg1Cache.height = 800;
window.bg2Cache = document.createElement('canvas'); window.bg2Cache.width = 3200; window.bg2Cache.height = 800;
window.cachedMapId = null;

// 2. KỸ THUẬT OBJECT POOLING (BỂ CHỨA HẠT CHỐNG LAG)
const MAX_PARTICLES = 250; 
const MAX_SHOCKWAVES = 5;
window.particlePool = [];
for (let i = 0; i < MAX_PARTICLES; i++) {
    window.particlePool.push({ active: false, x: 0, y: 0, vx: 0, vy: 0, life: 0, maxLife: 0, color: "", size: 0, isCoin: false, isRubble: false });
}
window.getFreeParticle = function() {
    for (let i = 0; i < window.particlePool.length; i++) { if (!window.particlePool[i].active) return window.particlePool[i]; } return null;
};
window.addParticle = function(x, y, vx, vy, life, color, size, isCoin = false, isRubble = false) {
    let pt = window.getFreeParticle();
    if(pt) { pt.active = true; pt.x = x; pt.y = y; pt.vx = vx; pt.vy = vy; pt.life = life; pt.maxLife = life; pt.color = color; pt.size = size; pt.isCoin = isCoin; pt.isRubble = isRubble; }
};

// ==========================================
// 0. HÀM HARD-RESET (CHỐNG RÒ RỈ RAM QUA TỪNG VÁN GAME)
// ==========================================
window.resetEngineState = function() {
    window.floatingTexts = []; window.projectiles = []; 
    window.traps = []; window.slashes = []; window.shockwaves = []; window.impactSparks = [];
    window.auras = []; window.lasers = []; window.customObjs = []; 
    window.envHazards = []; window.envDamage = [];
    
    window.particlePool.forEach(pt => pt.active = false); // Dọn sạch bể chứa hạt
    ...
    window.shakeTime = 0; window.chromaTimer = 0; window.screenFlash = 0; 
    window.matchTimer = 0; window.timeStopTimer = 0; window.koGlitchTimer = 0;
    window.camX = 0; window.camY = 0; window.currentZoom = 1; window.cameraTilt = 0;
};

// ==========================================
// PRE-RENDER MAP (VẼ 1 LẦN DUY NHẤT VÀO CANVAS NGẦM ĐỂ GPU XỬ LÝ)
// ==========================================
window.preRenderMap = function(cmap) {
    let ctx1 = window.bg1Cache.getContext('2d'); ctx1.clearRect(0,0,3200,800);
    let ctx2 = window.bg2Cache.getContext('2d'); ctx2.clearRect(0,0,3200,800);
    
    let t2 = cmap.bg2Type || "mountains";
    if (t2 !== "flowing_water" && t2 !== "flowing_lava") {
        ctx2.fillStyle = cmap.bg2;
        for(let i = -800; i < 2400; i += 150) {
            let dx = i + 800; 
            if (t2 === "mountains") { ctx2.beginPath(); ctx2.moveTo(dx, window.GROUND_Y); ctx2.lineTo(dx+75, window.GROUND_Y-120+Math.sin(i)*30); ctx2.lineTo(dx+150, window.GROUND_Y); ctx2.fill(); }
            else if (t2 === "pyramids") { ctx2.beginPath(); ctx2.moveTo(dx, window.GROUND_Y); ctx2.lineTo(dx+100, window.GROUND_Y-150); ctx2.lineTo(dx+200, window.GROUND_Y); ctx2.fill(); ctx2.fillRect(dx+40, window.GROUND_Y-50, 120, 5); ctx2.fillRect(dx+60, window.GROUND_Y-80, 80, 5); }
            else if (t2 === "river") { ctx2.beginPath(); ctx2.ellipse(dx+75, window.GROUND_Y-15, 100, 10, 0, 0, Math.PI*2); ctx2.fill(); ctx2.ellipse(dx+20, window.GROUND_Y-30, 60, 5, 0, 0, Math.PI*2); ctx2.fill(); }
            else if (t2 === "clouds") { ctx2.beginPath(); ctx2.arc(dx, window.GROUND_Y-180+Math.sin(i)*30, 60, 0, Math.PI*2); ctx2.arc(dx+50, window.GROUND_Y-150+Math.cos(i)*20, 50, 0, Math.PI*2); ctx2.fill(); }
            else if (t2 === "stars") { ctx2.beginPath(); ctx2.arc(dx+Math.sin(i)*50, window.GROUND_Y-250+Math.cos(i)*100, 3+Math.random()*4, 0, Math.PI*2); ctx2.fill(); }
        }
    }
    
    let t1 = cmap.bg1Type || "city";
    ctx1.fillStyle = cmap.bg1;
    for(let i = -800; i < 2400; i += 120) {
        let dx = i + 800; let h = 100 + Math.abs(Math.sin(i))*80;
        if (t1 === "city") { ctx1.fillRect(dx, window.GROUND_Y-h, 70, h); if(i%3===0) ctx1.clearRect(dx+10, window.GROUND_Y-h+20, 15, 20); }
        else if (t1 === "trees") { ctx1.fillRect(dx+25, window.GROUND_Y-h, 20, h); ctx1.beginPath(); ctx1.arc(dx+35, window.GROUND_Y-h, 45, 0, Math.PI*2); ctx1.fill(); }
        else if (t1 === "pines") { ctx1.fillRect(dx+25, window.GROUND_Y-30, 10, 30); ctx1.beginPath(); ctx1.moveTo(dx, window.GROUND_Y-20); ctx1.lineTo(dx+30, window.GROUND_Y-h); ctx1.lineTo(dx+60, window.GROUND_Y-20); ctx1.fill(); ctx1.beginPath(); ctx1.moveTo(dx-10, window.GROUND_Y-10); ctx1.lineTo(dx+30, window.GROUND_Y-h+40); ctx1.lineTo(dx+70, window.GROUND_Y-10); ctx1.fill(); }
        else if (t1 === "pillars") { ctx1.fillRect(dx+10, window.GROUND_Y-h, 40, h); ctx1.fillRect(dx, window.GROUND_Y-20, 60, 20); ctx1.fillRect(dx, window.GROUND_Y-h, 60, 15); }
        else if (t1 === "graves") { ctx1.beginPath(); ctx1.arc(dx+30, window.GROUND_Y-60, 30, Math.PI, 0); ctx1.lineTo(dx+60, window.GROUND_Y); ctx1.lineTo(dx, window.GROUND_Y); ctx1.fill(); ctx1.fillRect(dx+25, window.GROUND_Y-100, 10, 30); ctx1.fillRect(dx+15, window.GROUND_Y-90, 30, 5); }
        else if (t1 === "digital") { ctx1.fillStyle = "rgba(0, 255, 0, 0.15)"; ctx1.font="bold 20px monospace"; ctx1.fillText("01", dx, window.GROUND_Y-h); }
    }
    window.cachedMapId = cmap.id || (cmap.bg1Type + cmap.bg2Type);
};
...
window.playSound = function(freq, type, duration, vol, isImpact = false) { 
    ...
    try {
        ...
        if (isImpact) { 
            osc.type = type === 'sine' ? 'triangle' : type; osc.frequency.setValueAtTime(freq, t); osc.frequency.exponentialRampToValueAtTime(15, t + Math.min(0.15, duration)); gain.gain.setValueAtTime(safeVol, t); gain.gain.exponentialRampToValueAtTime(0.01, t + duration); 
...
// ==========================================
// 2. HỆ THỐNG SPAWN VFX (ĐÃ TÍCH HỢP OBJECT POOLING)
// ==========================================
...
window.spawnParticles = function(x, y, color, isCrit = false) { 
    ...
    for(let i=0; i<count; i++) { 
        let angle = Math.random() * Math.PI * 2; let speed = Math.random() * (isCrit?12:6) + 2; 
        window.addParticle(x, y - 30, Math.cos(angle)*speed, Math.sin(angle)*speed, 20, color, Math.random() * 4 + 2);
    } 
}
window.spawnDust = function(x, y) { 
    for(let i=0; i<6; i++) { window.addParticle(x + (Math.random()*30-15), y, (Math.random()-0.5)*5, -Math.random()*3 - 0.5, 20, "rgba(189, 195, 199, 0.4)", Math.random() * 8 + 5); } 
}
...
window.triggerTimeStop = function(frames, caster) { window.timeStopTimer = frames; window.timeStopCaster = caster; window.applyFilter('invert', frames); window.playSound(100, 'square', 1.0, 1.0, true); window.shakeScreen(20, 15); if (window.shockwaves.length < MAX_SHOCKWAVES) { window.shockwaves.push({x: caster.x, y: caster.y, r: 10, maxR: 1500, color: "#fff", alpha: 1, speed: 40}); } }
...
// ==========================================
// 3. HỆ THỐNG VẾT NỨT MÔI TRƯỜNG & VẬT LÝ
// ==========================================
window.spawnEnvDamage = function(x, y, type, scale, isBurning = false) {
    ...
    for(let d=0; d < 8 * scale; d++) { window.addParticle(x + (Math.random()-0.5)*30, y + (Math.random()-0.5)*20, (Math.random()-0.5)*10, -Math.random()*10 - 4, 40, isBurning ? "#e74c3c" : "#57606f", Math.random()*4 + 2, false, true); }
...
window.getClosestEnemy = function(source, targetsArray) { if (!targetsArray || targetsArray.length === 0) return null; let closest = targetsArray[0]; let minDist = Math.abs(source.x - closest.x); for (let i = 1; i < targetsArray.length; i++) { if (targetsArray[i].hp <= 0) continue; let d = Math.abs(source.x - targetsArray[i].x); if (d < minDist) { minDist = d; closest = targetsArray[i]; } } return closest.hp > 0 ? closest : null; }
window.takeDamage = function(target, amount, color, isCrit, wallBounce) {
    if (!target || target.hp <= 0 || target.iFrames > 0) return; let finalDmg = amount;
    if (target.state === 'block' && target.attackTimer >= 22) { 
        window.playSound(600, 'triangle', 0.4, 0.9, true); window.spawnParticles(target.x, target.y - 40, "#00ffff", true); if (window.shockwaves.length < MAX_SHOCKWAVES) window.shockwaves.push({x: target.x, y: target.y - 40, r: 5, maxR: 200, color: "#00ffff", alpha: 1, speed: 15}); window.floatingTexts.push({ x: target.x, y: target.y - 80, text: "🛡️ PERFECT PARRY!", color: "#00ffff", alpha: 1, vx: 0, vy: -3, font: "900 35px Arial", life: 50 });
        target.stamina = Math.min(100, target.stamina + 25); window.shakeScreen(15, 8); window.hitStopFrames = 12; window.chromaTimer = 8;
        let allFighters = [window.p1].concat(window.enemies); allFighters.forEach(e => { if (e !== target && e.hp > 0 && Math.abs(e.x - target.x) < 140) { e.state = 'hurt'; e.hitStun = 50; e.vx = target.isFacingRight ? 10 : -10; window.spawnDust(e.x, e.y); } }); return;
    }
    ...
    if (finalDmg > 0) {
        target.hp -= finalDmg; if (target.hp < 0) target.hp = 0; let dmgText = isCrit ? `💥 -${Math.floor(finalDmg)}` : `-${Math.floor(finalDmg)}`; window.floatingTexts.push({ x: target.x + (Math.random()*40-20), y: target.y - 60 - Math.random()*20, text: dmgText, color: color || (isCrit ? "#ff4757" : "#fff"), alpha: 1, vx: (Math.random()-0.5)*2, vy: -2 - Math.random()*2, font: isCrit ? "900 32px Arial" : "bold 24px Arial", life: 40 });
        ...
        if (wallBounce) { target.vx = target.isFacingRight ? -4 : 4; } if (typeof window.updateHPUIs === 'function') window.updateHPUIs();
        if (target.hp <= 0) {
            let isNearWall = (target.x < 80) || (target.x > window.canvas.width - 80);
            if (isNearWall && isCrit && typeof window.triggerStageTransition === 'function') { window.triggerStageTransition(target); } else { window.slowMoTimer = 60; window.screenFlash = 0; window.impactFrameTimer = 0; window.hitStopFrames = 8; window.shakeScreen(30, 20); window.targetZoom = 1.4; window.chromaTimer = 25; window.playSound(80, 'square', 1.5, 0.8, true); window.koGlitchTimer = 60; target.state = 'ko_falling'; target.koTimer = 100; target.vy = -10; target.onGround = false; }
        } else if (isCrit) { window.impactFrameTimer = 2; window.hitStopFrames = 2; window.shakeScreen(10, 8); window.targetZoom = 1.1; window.chromaTimer = 10; window.playSound(180, 'square', 0.3, 0.6, true); } else { window.hitStopFrames = 0; window.shakeScreen(3, 3); window.playSound(250, 'sine', 0.15, 0.3, true); }
    }
};
window.attack = function(attacker, targetGroup) {
    if (!attacker || attacker.hp <= 0) return; let target = window.getClosestEnemy(attacker, targetGroup); if (!target || target.hp <= 0) { attacker.state = 'jab'; attacker.attackTimer = 10; return; }
    ...
    if (attacker.comboStep >= 4 || Math.random() < 0.15) { selectedMove = moves_Finisher[Math.floor(Math.random() * moves_Finisher.length)]; isFinisher = true; attacker.comboStep = 0; } else { if (MathDist < 55) { selectedMove = moves_Close[Math.floor(Math.random() * moves_Close.length)]; } else if (MathDist < 90) { selectedMove = moves_Mid[Math.floor(Math.random() * moves_Mid.length)]; } else { selectedMove = moves_Far[Math.floor(Math.random() * moves_Far.length)]; } }
    if (MathDist > reach && !isFinisher) { attacker.vx = (attacker.isFacingRight ? 1 : -1) * attacker.currentSpeed * 3; attacker.state = 'dash'; attacker.attackTimer = 12; window.spawnDust(attacker.x, attacker.y); return; }
    attacker.state = selectedMove; attacker.attackTimer = isFinisher ? 30 : 18; attacker.vx = (attacker.isFacingRight ? 1 : -1) * (isFinisher ? 5 : 1.5); 
    let baseDmg = 12 * attacker.currentDmgMod; let finalDmg = baseDmg; let slashAngle = 0; 
    if (['uppercut', 'dragon_uppercut', 'knee_strike', 'high_kick'].includes(selectedMove)) slashAngle = -Math.PI / 5; else if (['axe_kick', 'elbow_strike', 'spinning_heel'].includes(selectedMove)) slashAngle = Math.PI / 5; else if (['low_kick'].includes(selectedMove)) slashAngle = Math.PI / 8; else slashAngle = (Math.random() - 0.5) * 0.2; 
    if (isFinisher) { isCrit = true; finalDmg = baseDmg * 3.5; window.shakeScreen(15, 12); target.vx = (attacker.isFacingRight ? 5 : -5); target.state = 'hurt'; target.hitStun = 45; window.spawnParticles(target.x, target.y - 40, "#ff4757", true); window.floatingTexts.push({ x: target.x, y: target.y - 80, text: "💥", color: "#ff4757", alpha: 1, vx: (Math.random()-0.5)*2, vy: -4, font: "900 45px Arial", life: 50 }); } else {
        if (Math.random() < attacker.critChance) { isCrit = true; finalDmg = baseDmg * attacker.critMult; window.floatingTexts.push({ x: target.x + (Math.random()*40-20), y: target.y - 60, text: "💢", color: "#f1c40f", alpha: 1, vx: 0, vy: -2, font: "italic 900 30px Arial", life: 30 }); } else { window.playSound(350, 'sine', 0.1, 0.1, false); }
        if ((selectedMove === 'low_kick' || selectedMove === 'teep_kick') && Math.random() < 0.4) { target.stunTimer = 35; target.state = 'stunned'; window.floatingTexts.push({ x: target.x, y: target.y - 50, text: "🦵", color: "#e67e22", alpha: 1, vx: 0, vy: -1, font: "900 35px Arial", life: 40 }); }
        if ((selectedMove === 'uppercut' || selectedMove === 'elbow_strike') && Math.random() < 0.3) { finalDmg *= 1.5; window.spawnParticles(target.x, target.y - 60, "#c0392b", true); window.floatingTexts.push({ x: target.x, y: target.y - 50, text: "🩸", color: "#c0392b", alpha: 1, vx: 0, vy: -1, font: "900 35px Arial", life: 40 }); }
...
// ==========================================
// 5. VÒNG LẶP UPDATE CHÍNH (UPDATE)
// ==========================================
window.update = function() {
    ...
    let isTimeStopped = window.timeStopTimer > 0; if (isTimeStopped) window.timeStopTimer--;
    ...
    if (window.screenFlash > 0) { window.screenFlash -= 0.05; if (window.screenFlash <= 0.001) window.screenFlash = 0; } else if (window.screenFlash < 0) { window.screenFlash += 1; if (window.screenFlash >= 0) window.screenFlash = 0; }
    ...
    if (!isTimeStopped) {
        for (let i = window.envDamage.length - 1; i >= 0; i--) {
            let dmg = window.envDamage[i]; if (dmg.life <= 0) { window.envDamage.splice(i, 1); continue; } dmg.life--;
            if (dmg.isBurning && Math.random() < 0.15) { window.addParticle(dmg.x + (Math.random()-0.5) * dmg.radius * 0.8, window.GROUND_Y, Math.random()-0.5, -Math.random()*5 - 1, 40, Math.random() > 0.4 ? "#e74c3c" : "#f1c40f", Math.random()*4+1); }
        }
    }
    ...
    if (!isTimeStopped) {
        for (let i = window.envHazards.length - 1; i >= 0; i--) {
            ...
            if (haz.type === 'lightning') {
                if (haz.state === 'warning' && haz.timer <= 0) {
                    haz.state = 'striking'; haz.timer = 12; window.playSound(300, 'sawtooth', 0.8, 0.8, true); window.screenFlash = 0.8; window.shakeScreen(20, 15); window.spawnEnvDamage(haz.x, window.GROUND_Y, 'crater', 1.2, false); 
                ...
                for(let k=0; k<15; k++) window.addParticle(haz.x + (Math.random()-0.5)*40, window.GROUND_Y, (Math.random()-0.5)*12, -10 - Math.random()*15, 40, "#e67e22", Math.random()*12+5);
                allFighters.forEach(f => { if(f && f.hp > 0 && Math.abs(f.x - haz.x) < 80 && f.y >= window.GROUND_Y - 120) { if(typeof window.takeDamage==='function') window.takeDamage(f, 40, "#e74c3c", true, false); f.vy = -16; f.onGround = false; f.state = 'ko_falling'; f.koTimer = 40; f.hitStun = 45; } }); window.envHazards.splice(i, 1);
            }
        }
        ...
        // [POOLING TỐI ƯU] XỬ LÝ HẠT (PARTICLES) BẰNG BỂ CHỨA
        for (let i = 0; i < window.particlePool.length; i++) { 
            let pt = window.particlePool[i]; 
        ...
        if (Math.random() < 0.12) { window.addParticle(Math.random() * window.canvas.width, window.GROUND_Y, (Math.random() - 0.5) * 1, -Math.random() * 2 - 0.5, 40, "rgba(255, 159, 67, 0.35)", Math.random() * 3 + 1); }
    }
    ...
    if (!isTimeStopped) {
        window.enemies.forEach(e => { 
            if (e.hp <= 0 && !e.deathTriggered) {
                ...
                for(let c=0; c<5; c++) window.addParticle(e.x, e.y - 20, (Math.random()-0.5)*8, -Math.random()*8, 60, "#f1c40f", 4, true, false);
    ...
    allFighters.forEach(f => {
        ...
        if (f.comboDisplayTimer > 0) { f.comboDisplayTimer--; f.comboAlpha = 1; } else if (f.comboHits > 0) { f.comboAlpha = (f.comboAlpha || 1) - 0.02; if (f.comboAlpha <= 0) { f.comboAlpha = 0; f.comboHits = 0; } }
        ...
        else if (window.currentWeather === 'toxic') { f.currentDmgMod *= 0.80; if (window.matchTimer % 90 === 0 && f.hp > 1 && !window.gameOver) { f.hp -= 1; window.addParticle(f.x, f.y-30, 0, -1, 20, "#2ecc71", 4); } }

        if (f.isRage) { f.currentSpeed *= 1.5; f.currentDmgMod *= 1.5; f.aiDelay = 0; window.addParticle(f.x + (Math.random() - 0.5) * 40, f.y - Math.random() * 80, (Math.random() - 0.5) * 2, -Math.random() * 6 - 2, 30, "#ff4757", Math.random() * 6 + 3); if (Math.random() < 0.05) window.shakeScreen(2, 2); }
        if (f.hp > 0 && f.stamina < 100) f.stamina += (f.isRage ? 0.4 : (f.regen * 0.2 || 0.05)); if (f.stamina > 100) f.stamina = 100;
        if (f.stamina < 10) f.isExhausted = true; if (f.stamina > 40) f.isExhausted = false; if (f.isExhausted) f.currentSpeed *= 0.6;

        for (let i = f.buffs.length - 1; i >= 0; i--) { let b = f.buffs[i]; b.life--; if (b.life <= 0) { f.buffs.splice(i, 1); continue; } if (b.stat === 'dmg') f.currentDmgMod += b.value; if (b.stat === 'speed') f.currentSpeed += b.value; if (b.life % 15 === 0) window.addParticle(f.x + (Math.random()*20-10), f.y - 10, 0, -2, 10, "#f1c40f", 2); }
            ...
            if (f.hitStun <= 0 && f.stunTimer <= 0 && f.state !== 'dash_back' && f.state !== 'block') {
                ...
                if (isCloseEnough) {
                    if (window.classStats && window.classStats[type] && typeof window.classStats[type].executeUltimate === 'function') {
                        ...
                        window.playSound(400, 'sine', 0.5, 0.6); window.shakeScreen(15, 10); window.spawnParticles(f.x, f.y, "#f1c40f", true); let ultText = f.isPlayer ? "🔥 ULTIMATE!" : "⚠️ DANGER!";
                    ...
                    } else if (typeof window.useUltimate === 'function') { window.useUltimate(f, closestTarget); f.vx = 0; }
                    ...
                    if (f.aiDelay <= 0) {
                        ...
                        if (randAction < 0.45) { 
                            ...
                            let fireInterval = setInterval(() => { 
                                if (window.gameOver || f.hp <= 0 || !window.p1) { clearInterval(fireInterval); return; } let fireVx = f.isFacingRight ? (f.isEvolved ? 15 : 11) : (f.isEvolved ? -15 : -11); 
                                if (f.isEvolved) { window.spawnProjectile( f.x + (f.isFacingRight ? 55 : -55), f.y - 65, fireVx, -3, 15, "#9b59b6", Math.floor(35 * f.dmgMod), window.p1 ); window.spawnProjectile( f.x + (f.isFacingRight ? 55 : -55), f.y - 65, fireVx, 0, 15, "#8e44ad", Math.floor(35 * f.dmgMod), window.p1 ); window.spawnProjectile( f.x + (f.isFacingRight ? 55 : -55), f.y - 65, fireVx, 3, 15, "#9b59b6", Math.floor(35 * f.dmgMod), window.p1 ); } else { window.spawnProjectile( f.x + (f.isFacingRight ? 55 : -55), f.y - 65, fireVx, (Math.random() - 0.5) * 4, 13, "#e74c3c", Math.floor(25 * f.dmgMod), window.p1 ); } 
                                fireCount++; if (fireCount >= (f.isEvolved ? 8 : 6)) clearInterval(fireInterval); 
                            }, f.isEvolved ? 60 : 85); 
                            window.activeIntervals.push(fireInterval); 
                        } 
                            ...
                            window.activeIntervals.push(scratchInterval); 
                ...
                if (targetFighter && targetFighter.hp > 0) {
                    ...
                    if (f.aiDelay <= 0) {
                        ...
                        if (absDist > 110) { f.state = 'walk'; f.vx = Math.sign(dist) * f.currentSpeed * 1.5; if (Math.random() < 0.3) { f.state = 'dash'; f.dashTimer = 8; f.dashDir = Math.sign(dist); window.spawnDust(f.x, f.y); } } 
                            ...
                            f.state = 'high_kick'; f.attackTimer = 28; f.vx = Math.sign(dist) * 5; 
                            let kickCount = 0; 
                            ...
                            window.activeIntervals.push(kickInterval); 
    ...
    allFighters.forEach(f => {
        ...
        if (f.x < wallBound) { 
            ...
            if (f.hitStun > 0 && f.vx < -8 && !f.wallBounced) {
                ...
                window.playSound(200, 'square', 0.4, 0.8, true); window.spawnDust(f.x, f.y); window.floatingTexts.push({ x: f.x, y: f.y - 80, text: "💥 WALL SPLAT!", color: "#f1c40f", alpha: 1, vx: 2, vy: -2, font: "900 30px Arial", life: 40 });
            } else if (f.hitStun > 0 && f.vx < -3 && !f.wallBounced && f.state !== 'wall_splat') {
                f.wallBounced = true; f.vx = 4; f.hitStun = 15; window.shakeScreen(10, 4); if (!f.wallDamageCooldown) { window.spawnEnvDamage(wallBound, f.y, 'wall_left', f.scale || 1, false); f.wallDamageCooldown = 60; } if(typeof window.takeDamage === 'function') window.takeDamage(f, Math.floor(Math.random() * 4) + 4, "#fff", false, true); window.playSound(100, 'sine', 0.2, 0.3, true); window.spawnDust(f.x, f.y); 
        ...
        if (window.canvas && f.x > window.canvas.width - wallBound) { 
            ...
            if (f.hitStun > 0 && f.vx > 8 && !f.wallBounced) {
                ...
                f.wallBounced = true; f.vx = -4; f.hitStun = 15; window.shakeScreen(10, 4); if (!f.wallDamageCooldown) { window.spawnEnvDamage(window.canvas.width - wallBound, f.y, 'wall_right', f.scale || 1, false); f.wallDamageCooldown = 60; } if(typeof window.takeDamage === 'function') window.takeDamage(f, Math.floor(Math.random() * 4) + 4, "#fff", false, true); window.playSound(100, 'sine', 0.2, 0.3, true); window.spawnDust(f.x, f.y); 
            } else if(f.state !== 'walk' && f.state !== 'dash_back' && f.state !== 'wall_splat') f.vx = 0;
        }
    ...
    if (!isTimeStopped) {
        for (let i = window.projectiles.length - 1; i >= 0; i--) { 
            ...
            if (proj.isMeteor) {
                window.addParticle(proj.x + (Math.random()-0.5)*10, proj.y, 0, -2, 15, "#f1c40f", Math.random()*4+2);
    ...
    if (window.p1 && window.introTimer === 0) {
        ...
        if (window.gameOver && closest) {
            ...
            if (window.p1 && window.p1.hp > 0) { focusX = window.p1.x; focusY = window.p1.y; } else { let aliveEnemy = window.enemies.find(e => e.hp > 0); if (aliveEnemy) { focusX = aliveEnemy.x; focusY = aliveEnemy.y; } }
...
// ==========================================
// 6. HỆ THỐNG VẼ ĐỒ HỌA TRUNG TÂM (TỐI ƯU HÓA SIÊU MƯỢT)
// ==========================================
window.draw = function() {
    ...
    try {
        ...
        if (window.filterTimer > 0 && window.screenFilter) {
            ...
            window.ctx.globalCompositeOperation = 'source-over'; if (window.filterTimer <= 0) window.screenFilter = null;
        }
        ...
        if (window.impactFrameTimer > 0) { window.impactFrameTimer--; } window.ctx.globalCompositeOperation = "source-over"; 
        ...
        let currentMapId = cmap.id || (cmap.bg1Type + cmap.bg2Type);
        
        // KIỂM TRA VÀ CẬP NHẬT PRE-RENDER GPU NẾU MAP THAY ĐỔI
        if (window.cachedMapId !== currentMapId) { window.preRenderMap(cmap); }
        ...
        // VẼ HẬU CẢNH TỪ CACHE CANVAS NGẦM (SIÊU NHANH)
        window.ctx.save(); window.ctx.translate(-window.camX * 0.7, -window.camY * 0.3);
        let t2 = cmap.bg2Type || "mountains";
        if (t2 === "flowing_water" || t2 === "flowing_lava") {
            ...
            const drawWaveLayer = (color, amplitude, frequency, phase, offsetY) => {
                window.ctx.fillStyle = color; window.ctx.beginPath(); let startX = -800; let endX = window.canvas.width + 1200;
            ...
            window.ctx.drawImage(window.bg2Cache, -800, 0); // VẼ 1 PHÁT XONG LUÔN
        }
        window.ctx.restore();

        window.ctx.save(); window.ctx.translate(-window.camX * 0.4, -window.camY * 0.15); 
        window.ctx.drawImage(window.bg1Cache, -800, 0); // VẼ HÀNG TRĂM TÒA NHÀ/CÂY CHỈ VỚI 1 DÒNG CODE
        window.ctx.restore();
        ...
        window.ctx.fillStyle = cmap.line; window.ctx.fillRect(window.WALL_PADDING, 0, 4, window.canvas.height); window.ctx.fillStyle = cmap.line; window.ctx.fillRect(window.canvas.width - window.WALL_PADDING, 0, 4, window.canvas.height); 
        ...
        if (window.envDamage && window.envDamage.length > 0) {
            ...
            window.envDamage.forEach(dmg => {
                ...
                if (dmg.type === 'crater') {
                    ...
                    if (dmg.isBurning) { let pulse = 0.5 + Math.abs(Math.sin(Date.now() / 150)) * 0.5; let burnGrad = window.ctx.createRadialGradient(0, 0, 0, 0, 0, (dmg.radius || 40) * 1.5); burnGrad.addColorStop(0, `rgba(255, 150, 0, ${pulse * 0.9 * alpha})`); burnGrad.addColorStop(0.3, `rgba(255, 30, 0, ${pulse * 0.5 * alpha})`); burnGrad.addColorStop(1, "rgba(0,0,0,0)"); window.ctx.fillStyle = burnGrad; window.ctx.beginPath(); window.ctx.arc(0, 0, (dmg.radius || 40) * 1.5, 0, Math.PI * 2); window.ctx.fill();
                    } else { window.ctx.fillStyle = `rgba(15, 15, 15, ${0.8 * alpha})`; window.ctx.beginPath(); window.ctx.arc(0, 0, (dmg.radius || 40), 0, Math.PI * 2); window.ctx.fill(); } window.ctx.restore();
                }
                let baseLw = (dmg.isBurning ? 4 : 3) * (dmg.scale || 1);
                dmg.cracks.forEach(path => {
                    if (Array.isArray(path) && path.length > 0) {
                        if (dmg.isBurning) { window.ctx.strokeStyle = `rgba(255, 50, 0, ${0.8 * alpha})`; window.ctx.lineWidth = baseLw + 2; window.ctx.shadowBlur = 15; window.ctx.shadowColor = "#ff4757"; } else { window.ctx.strokeStyle = `rgba(10, 10, 10, ${0.5 * alpha})`; window.ctx.lineWidth = baseLw + 2; window.ctx.shadowBlur = 5; window.ctx.shadowColor = "#000"; }
                        ...
                        window.ctx.strokeStyle = dmg.isBurning ? `rgba(255, 230, 100, ${1.0 * alpha})` : `rgba(0, 0, 0, ${0.9 * alpha})`; window.ctx.lineWidth = baseLw * 0.4; window.ctx.shadowBlur = 0; window.ctx.beginPath(); path.forEach((pt, idx) => { if (idx === 0) window.ctx.moveTo(pt.x, pt.y); else window.ctx.lineTo(pt.x, pt.y); }); window.ctx.stroke();
                    }
                }); window.ctx.restore();
       ...
       if (window.envHazards && window.envHazards.length > 0) {
            ...
            window.envHazards.forEach(haz => {
                if (haz.type === 'lightning') {
                    if (haz.state === 'warning') {
                        let pulse = 0.5 + Math.sin(haz.timer * 0.4) * 0.5; window.ctx.fillStyle = `rgba(0, 243, 255, ${0.05 + pulse * 0.25})`; window.ctx.beginPath(); window.ctx.ellipse(haz.x, window.GROUND_Y, 35 + pulse * 25, 8, 0, 0, Math.PI*2); window.ctx.fill();
                        if (Math.random() < 0.3) { window.ctx.strokeStyle = "rgba(0, 243, 255, 0.8)"; window.ctx.lineWidth = 1; window.ctx.beginPath(); window.ctx.moveTo(haz.x + (Math.random()-0.5)*40, window.GROUND_Y); window.ctx.lineTo(haz.x + (Math.random()-0.5)*40, window.GROUND_Y - 20 - Math.random()*40); window.ctx.stroke(); }
                        ...
                        let startX = haz.x + (Math.random() - 0.5) * 150; let startY = window.GROUND_Y - 800; let targetX = haz.x; let targetY = window.GROUND_Y; let steps = 14; let path = [{x: startX, y: startY}]; let branches = [];
                        ...
                        branches.forEach(b => { window.ctx.beginPath(); window.ctx.moveTo(b.x, b.y); let bx = b.x, by = b.y; for(let j=0; j<4; j++) { bx += (b.tx - b.x)/4 + (Math.random()-0.5)*30; by += (b.ty - b.y)/4; window.ctx.lineTo(bx, by); } window.ctx.stroke(); }); window.ctx.shadowBlur = 0;
                        window.ctx.fillStyle = `rgba(255, 255, 255, ${0.4 + Math.random()*0.6})`; window.ctx.beginPath(); window.ctx.ellipse(haz.x, window.GROUND_Y, 70, 15, 0, 0, Math.PI*2); window.ctx.fill();
                    }
        ...
        window.weatherParticles.forEach(w => { 
            if (window.currentWeather === 'snow') { window.ctx.fillStyle = "rgba(255, 255, 255, 0.8)"; window.ctx.beginPath(); window.ctx.arc(w.x, w.y, w.size, 0, Math.PI*2); window.ctx.fill(); } 
                ...
                let glow = 0.3 + Math.abs(Math.sin(Date.now()/350 + w.x)) * 0.7; window.ctx.fillStyle = `rgba(241, 196, 15, ${glow})`; window.ctx.shadowBlur = 8; window.ctx.shadowColor = "#f1c40f"; window.ctx.beginPath(); window.ctx.arc(w.x, w.y, w.size * 1.4, 0, Math.PI*2); window.ctx.fill(); window.ctx.shadowBlur = 0;
            ...
            } else if (window.currentWeather === 'shooting_stars') { if (Math.random() > 0.95) return; let tailX = w.x + w.speed * 2; let tailY = w.y - w.speed * 3; let grad = window.ctx.createLinearGradient(w.x, w.y, tailX, tailY); grad.addColorStop(0, "rgba(255, 255, 255, 1)"); grad.addColorStop(1, "rgba(0, 243, 255, 0)"); window.ctx.strokeStyle = grad; window.ctx.lineWidth = w.size * 0.8; window.ctx.beginPath(); window.ctx.moveTo(w.x, w.y); window.ctx.lineTo(tailX, tailY); window.ctx.stroke(); }
        }); window.ctx.restore();
        ...
        window.auras.forEach(a => {
            let prog = a.life / a.maxLife; window.ctx.globalCompositeOperation = 'lighter'; window.ctx.globalAlpha = prog * 0.8; let aGrad = window.ctx.createRadialGradient(a.x, a.y - 10, 0, a.x, a.y - 10, a.r); aGrad.addColorStop(0, a.color); aGrad.addColorStop(1, "rgba(0,0,0,0)");
            window.ctx.fillStyle = aGrad; if (a.life % 4 > 1) { window.ctx.beginPath(); window.ctx.ellipse(a.x, window.GROUND_Y, a.r, a.r*0.3, 0, 0, Math.PI*2); window.ctx.fill(); } window.ctx.globalAlpha = 1.0; window.ctx.globalCompositeOperation = 'source-over'; a.life--;
        }); window.auras = window.auras.filter(a => a.life > 0);

        window.lasers.forEach(l => {
            let prog = l.life / l.maxLife; window.ctx.globalCompositeOperation = 'lighter'; window.ctx.globalAlpha = prog; window.ctx.shadowBlur = 20; window.ctx.shadowColor = l.color; window.ctx.fillStyle = l.color; let currentWidth = l.width * (0.8 + Math.random() * 0.4); let startX = l.x; let endX = l.isRight ? window.canvas.width + 500 : -500; window.ctx.fillRect(l.isRight ? startX : endX, l.y - currentWidth/2, Math.abs(endX - startX), currentWidth); window.ctx.fillStyle = "#fff"; window.ctx.fillRect(l.isRight ? startX : endX, l.y - currentWidth/4, Math.abs(endX - startX), currentWidth/2);
        ...
        allFighters.forEach(p => {
            if (p && p.hp >= 0) { let heightDist = Math.max(0, window.GROUND_Y - p.y); let shadowScale = Math.max(0.15, 1 - heightDist / 250) * (p.scale || 1); window.ctx.fillStyle = `rgba(0, 0, 0, ${0.45 * shadowScale})`; window.ctx.beginPath(); window.ctx.ellipse(p.x, window.GROUND_Y, 35 * shadowScale, 7 * shadowScale, 0, 0, Math.PI * 2); window.ctx.fill(); }
        }); window.ctx.restore();
        ...
        if (window.p1) {
            ...
            allFighters.forEach(p => { 
                if (p && p.hp > 0 && p.trailArr) { 
                    p.trailArr.forEach(t => { 
                        // KỸ THUẬT ZERO-ALLOCATION CHO BÓNG MỜ (MƯỢN TẠM THUỘC TÍNH)
                        let oX = p.x, oState = p.state, oFace = p.isFacingRight, oCol = p.color, oAlph = p.alpha, oSca = p.scale;
                        p.x = t.x; p.state = t.state; p.isFacingRight = t.isFacingRight; p.color = t.color; p.alpha = t.alpha; p.scale = t.scale;
                        
                        if (p.isDragon && typeof window.drawDragon === 'function') window.drawDragon(window.ctx, p, true); 
                        ...
                        else if (typeof window.drawStickman === 'function') window.drawStickman(window.ctx, p, true); 
                        
                        p.x = oX; p.state = oState; p.isFacingRight = oFace; p.color = oCol; p.alpha = oAlph; p.scale = oSca;
                    }); 
                } 
            });
            window.ctx.globalCompositeOperation = "source-over";
            ...
            window.enemies.forEach(e => { 
                ...
                if (e.state === 'ko_falling' || e.state === 'dead') { 
                    window.ctx.translate(e.x, e.y); let angle = Math.PI / 2; if (e.state === 'ko_falling') { let progress = (100 - e.koTimer) / 30; if (progress > 1) progress = 1; angle = progress * (Math.PI / 2); } let fallDir = e.isFacingRight ? -1 : 1; window.ctx.rotate(angle * fallDir); 
                    // ZERO-ALLOCATION CHO THI THỂ
                    let oX = e.x, oY = e.y; e.x = 0; e.y = 0;
                    if(e.isDragon && typeof window.drawDragon === 'function') window.drawDragon(window.ctx, e); else if (e.isBruceLee && typeof window.drawBruceLee === 'function') window.drawBruceLee(window.ctx, e); else if (e.isSamurai && typeof window.drawSamurai === 'function') window.drawSamurai(window.ctx, e); else if (e.isNinja && typeof window.drawNinja === 'function') window.drawNinja(window.ctx, e); else if (typeof window.drawStickman === 'function') window.drawStickman(window.ctx, e); 
                    e.x = oX; e.y = oY;
                } else { 
                    if(e.isDragon && typeof window.drawDragon === 'function') window.drawDragon(window.ctx, e); else if (e.isBruceLee && typeof window.drawBruceLee === 'function') window.drawBruceLee(window.ctx, e); else if (e.isSamurai && typeof window.drawSamurai === 'function') window.drawSamurai(window.ctx, e); else if (e.isNinja && typeof window.drawNinja === 'function') window.drawNinja(window.ctx, e); else if (typeof window.drawStickman === 'function') window.drawStickman(window.ctx, e); 
                }
        ...
        // VẼ HẠT VỚI HỆ THỐNG OBJECT POOLING
        for (let i = 0; i < window.particlePool.length; i++) {
            let pt = window.particlePool[i];
            if (!pt.active) continue;
        ...
        let vGrad = window.ctx.createRadialGradient(vX, vY, window.canvas.height * 0.4, vX, vY, window.canvas.width * 0.75); vGrad.addColorStop(0, "rgba(0,0,0,0)"); vGrad.addColorStop(1, "rgba(0,0,0,0.65)"); window.ctx.fillStyle = vGrad; window.ctx.fillRect(0, 0, window.canvas.width, window.canvas.height);
        ...
        if (window.gameOver && window.matchEndTimer >= 90 && window.endIconType) {
            window.ctx.save(); let popScale = Math.min(1, (window.matchEndTimer - 90) / 25); let easeScale = Math.sin(popScale * Math.PI / 2); window.ctx.translate(window.canvas.width / 2, window.canvas.height / 2); window.ctx.scale(easeScale * 1.2, easeScale * 1.2); window.ctx.font = "140px Arial"; window.ctx.textAlign = "center"; window.ctx.textBaseline = "middle";
            if (window.endIconType === 'win') { window.ctx.shadowBlur = 35; window.ctx.shadowColor = "#f1c40f"; window.ctx.fillText("🏆", 0, 0); } else if (window.endIconType === 'lose') { window.ctx.shadowBlur = 35; window.ctx.shadowColor = "#ff4757"; window.ctx.fillText("💀", 0, 0); } window.ctx.restore();
        } window.ctx.restore();
        ...
        if (window.cinematicTimer > 0 && window.cinematicCaster) {
            ...
            // KỸ THUẬT ZERO ALLOCATION CHO CINEMATIC AVATAR
            let avaX = window.canvas.width - slideX; 
            let oX = window.cinematicCaster.x, oY = window.cinematicCaster.y, oState = window.cinematicCaster.state, oFacing = window.cinematicCaster.isFacingRight;
            window.cinematicCaster.x = avaX; window.cinematicCaster.y = stripY + 70; window.cinematicCaster.state = 'cast'; window.cinematicCaster.isFacingRight = true;
            if(window.cinematicCaster.isDragon && typeof window.drawDragon === 'function') window.drawDragon(window.ctx, window.cinematicCaster); else if(typeof window.drawStickman === 'function') window.drawStickman(window.ctx, window.cinematicCaster);
            window.cinematicCaster.x = oX; window.cinematicCaster.y = oY; window.cinematicCaster.state = oState; window.cinematicCaster.isFacingRight = oFacing;
        }
        
        // ==========================================
        // MÀN HÌNH VERSUS (VS SCREEN) TỐI ƯU HÓA GPU
        // ==========================================
        if (window.introTimer > 0 && !window.gameOver && window.p1) {
            ...
            if (window.introTimer > 60) {
                ...
                // ZERO-ALLOCATION CHO VS AVATAR PLAYER 1
                let oX = window.p1.x, oY = window.p1.y, oState = window.p1.state, oFacing = window.p1.isFacingRight, oScale = window.p1.scale;
                window.p1.x = slideX1; window.p1.y = window.GROUND_Y + 50; window.p1.state = window.p1.introState || 'idle'; window.p1.isFacingRight = true; window.p1.scale = (window.p1.scale || 1) * 1.5;
                if(typeof window.drawStickman === 'function') window.drawStickman(window.ctx, window.p1);
                window.p1.x = oX; window.p1.y = oY; window.p1.state = oState; window.p1.isFacingRight = oFacing; window.p1.scale = oScale;
                
                // ZERO-ALLOCATION CHO VS AVATAR ENEMY
                if (repEnemy) { 
                    let eX = repEnemy.x, eY = repEnemy.y, eState = repEnemy.state, eFacing = repEnemy.isFacingRight, eScale = repEnemy.scale;
                    repEnemy.x = slideX2; repEnemy.y = window.GROUND_Y + 50; repEnemy.state = repEnemy.isDragon ? 'idle' : (repEnemy.introState || 'idle'); repEnemy.isFacingRight = false; repEnemy.scale = (repEnemy.scale || 1) * 1.5;
                    if (repEnemy.isDragon && typeof window.drawDragon === 'function') window.drawDragon(window.ctx, repEnemy); else if (repEnemy.isBruceLee && typeof window.drawBruceLee === 'function') window.drawBruceLee(window.ctx, repEnemy); else if (repEnemy.isSamurai && typeof window.drawSamurai === 'function') window.drawSamurai(window.ctx, repEnemy); else if (repEnemy.isNinja && typeof window.drawNinja === 'function') window.drawNinja(window.ctx, repEnemy); else if (typeof window.drawStickman === 'function') window.drawStickman(window.ctx, repEnemy); 
                    repEnemy.x = eX; repEnemy.y = eY; repEnemy.state = eState; repEnemy.isFacingRight = eFacing; repEnemy.scale = eScale;
                }
        ...
        if (window.koGlitchTimer > 0) {
            window.ctx.setTransform(1, 0, 0, 1, 0, 0); if (window.koGlitchTimer > 45) { window.ctx.fillStyle = `rgba(0, 0, 0, ${1 - (window.koGlitchTimer - 45) / 15})`; window.ctx.fillRect(0, 0, window.canvas.width, window.canvas.height); } else { window.ctx.fillStyle = `rgba(0, 0, 0, 0.6)`; window.ctx.fillRect(0, 0, window.canvas.width, window.canvas.height); }
        ...
        if (window.chromaTimer > 0) {
            if (!window.tempChromaCanvas) { window.tempChromaCanvas = document.createElement('canvas'); window.tempChromaCanvas.width = window.canvas.width; window.tempChromaCanvas.height = window.canvas.height; window.tempChromaCtx = window.tempChromaCanvas.getContext('2d'); } 
            else if (window.tempChromaCanvas.width !== window.canvas.width || window.tempChromaCanvas.height !== window.canvas.height) { window.tempChromaCanvas.width = window.canvas.width; window.tempChromaCanvas.height = window.canvas.height; window.tempChromaCtx = window.tempChromaCanvas.getContext('2d'); }
            let shift = window.chromaTimer * 1.5; window.tempChromaCtx.clearRect(0, 0, window.canvas.width, window.canvas.height); window.tempChromaCtx.drawImage(window.canvas, 0, 0);
            window.ctx.setTransform(1, 0, 0, 1, 0, 0); window.ctx.globalCompositeOperation = 'lighter'; window.ctx.globalAlpha = 0.4; window.ctx.drawImage(window.tempChromaCanvas, shift, 0); window.ctx.drawImage(window.tempChromaCanvas, -shift, 0);
