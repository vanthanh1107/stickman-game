...
// [TÍCH HỢP SẴN: MÀN HÌNH VS CINEMATIC VÀ HỆ THỐNG VẬT LÝ GRAPPLING CẬN CHIẾN]
// ==========================================
        ...
        for (let i = window.envHazards.length - 1; i >= 0; i--) {
            ...
            if (haz.type === 'lightning') {
                if (haz.state === 'warning' && haz.timer <= 0) {
                    ...
                    window.spawnEnvDamage(haz.x, window.GROUND_Y, 'crater', 1.2, false); 
...
window.draw = function() {
    ...
    try {
       ...
       if (window.envHazards && window.envHazards.length > 0) {
                ...
                if (haz.type === 'lightning') {
                    if (haz.state === 'warning') {
                        ...
                        if (Math.random() < 0.3) {
                            ...
                            window.ctx.lineTo(haz.x + (Math.random()-0.5)*40, window.GROUND_Y - 20 - Math.random()*40); window.ctx.stroke();
                        }
                    } 
                        ...
                        branches.forEach(b => {
                            window.ctx.beginPath(); window.ctx.moveTo(b.x, b.y);
                            let bx = b.x, by = b.y;
                            for(let j=0; j<4; j++) {
                                bx += (b.tx - b.x)/4 + (Math.random()-0.5)*30;
                                by += (b.ty - b.y)/4;
                                window.ctx.lineTo(bx, by);
                            }
                            window.ctx.stroke();
                        });
                        window.ctx.shadowBlur = 0;
                    ...
                    window.ctx.fillStyle = `rgba(231, 76, 60, ${0.1 + Math.sin(haz.timer)/5})`; 
                    window.ctx.beginPath(); window.ctx.ellipse(haz.x, window.GROUND_Y, 60 + Math.sin(haz.timer)*10, 15, 0, 0, Math.PI*2); window.ctx.fill(); 
                }
            });
        ...
        // ==========================================
        // MÀN HÌNH VERSUS (VS SCREEN) CHÉO GÓC + AVATAR
        // ==========================================
        if (window.introTimer > 0 && !window.gameOver && window.p1) {
            ...
            if (window.introTimer > 60) {
                let progress = Math.min(1, (160 - window.introTimer) / 40); 
                let cx = window.canvas.width / 2;
                let cy = window.canvas.height / 2;

                let p1Name = (window.p1.className || "PLAYER 1").toUpperCase();
                let repEnemy = window.enemies && window.enemies.length > 0 ? window.enemies[0] : null;
                let p2Name = repEnemy ? (repEnemy.className || "ENEMY").toUpperCase() : "ENEMY";

                // 1. Nền chéo góc (Trái Xanh Cyber - Phải Đỏ Neon)
                window.ctx.fillStyle = "#001b36"; 
                window.ctx.fillRect(0, 0, window.canvas.width, window.canvas.height);
                window.ctx.beginPath();
                window.ctx.moveTo(cx + 150, 0);
                window.ctx.lineTo(window.canvas.width, 0);
                window.ctx.lineTo(window.canvas.width, window.canvas.height);
                window.ctx.lineTo(cx - 150, window.canvas.height);
                window.ctx.fillStyle = "#2b000b"; 
                window.ctx.fill();

                // Vi mạch nền chéo
                window.ctx.lineWidth = 2;
                window.ctx.strokeStyle = "rgba(255, 255, 255, 0.05)";
                for(let i = -200; i < window.canvas.width + 200; i += 40) {
                    window.ctx.beginPath(); window.ctx.moveTo(i, 0); window.ctx.lineTo(i - 200, window.canvas.height); window.ctx.stroke();
                }

                // Vạch điện vàng chia cắt
                window.ctx.beginPath(); window.ctx.moveTo(cx + 150, 0); window.ctx.lineTo(cx - 150, window.canvas.height);
                window.ctx.lineWidth = 8; window.ctx.strokeStyle = "#f1c40f"; window.ctx.stroke();

                // 2. Chữ Tên trượt vào khung hình
                let easeOut = 1 - Math.pow(1 - progress, 4); 
                let textX1 = -300 + (cx - 100 - (-300)) * easeOut;
                let textX2 = window.canvas.width + 300 - ((window.canvas.width + 300) - (cx + 100)) * easeOut;

                window.ctx.font = "900 50px Arial";
                window.ctx.textAlign = "right"; window.ctx.textBaseline = "middle";
                window.ctx.fillStyle = "#00f3ff"; window.ctx.shadowBlur = 20; window.ctx.shadowColor = "#00f3ff";
                window.ctx.fillText(p1Name, textX1, cy - 80);
                
                window.ctx.textAlign = "left"; window.ctx.fillStyle = "#ff003c"; window.ctx.shadowColor = "#ff003c";
                window.ctx.fillText(p2Name, textX2, cy - 80);
                window.ctx.shadowBlur = 0;

                // 3. Nhân vật phóng lớn trượt vào (Avatar)
                let targetX1 = 200; let targetX2 = window.canvas.width - 200; 
                let slideX1 = -100 + (targetX1 - (-100)) * easeOut; 
                let slideX2 = window.canvas.width + 100 - (window.canvas.width + 100 - targetX2) * easeOut;
                
                let p1Clone = Object.assign({}, window.p1, {x: slideX1, y: window.GROUND_Y + 50, state: window.p1.introState || 'idle', isFacingRight: true, scale: (window.p1.scale || 1) * 1.5}); 
                if(typeof window.drawStickman === 'function') window.drawStickman(window.ctx, p1Clone);
                
                if (repEnemy) { 
                    let p2Clone = Object.assign({}, repEnemy, {x: slideX2, y: window.GROUND_Y + 50, state: repEnemy.isDragon ? 'idle' : (repEnemy.introState || 'idle'), isFacingRight: false, scale: (repEnemy.scale || 1) * 1.5}); 
                ...
                // 4. Chữ VS Nện cực mạnh xuống giữa màn hình
                if (window.introTimer <= 140) {
                    let vsProgress = Math.min(1, (140 - window.introTimer) / 10);
                    let vsScale = Math.max(1, 5 - vsProgress * 4);
                    window.ctx.save();
                    window.ctx.translate(cx, cy + 50);
                    window.ctx.scale(vsScale, vsScale);
                    window.ctx.font = "italic 900 90px Arial"; window.ctx.textAlign = "center";
                    window.ctx.fillStyle = "#fff"; window.ctx.shadowColor = "#f1c40f"; window.ctx.shadowBlur = 25;
                    window.ctx.fillText("VS", 0, 0); 
                    window.ctx.restore();
                }

            } else { 
                // XÓA DẦN MÀN HÌNH VS ĐỂ VÀO GAME
                let fadeAlpha = window.introTimer / 60;
                window.ctx.fillStyle = `rgba(0, 0, 0, ${fadeAlpha})`;
                window.ctx.fillRect(0, 0, window.canvas.width, window.canvas.height);

                let scale = 1 + ((60 - window.introTimer) / 60) * 0.4; 
                window.ctx.save(); 
                window.ctx.translate(window.canvas.width/2, window.canvas.height/2); 
                window.ctx.scale(scale, scale); 
                window.ctx.font = "italic 900 110px Arial"; 
                ...
                window.ctx.strokeText("🥊 FIGHT! 🥊", 0, 0); 
                window.ctx.fillText("🥊 FIGHT! 🥊", 0, 0); 
                window.ctx.restore(); 
            }
            window.ctx.restore();
        }
...
// ==========================================
// EXPANSION PATCH: HỆ THỐNG GRAPPLING & INTERACTION 
// (Bổ sung 5 cơ chế tương tác Vật lý Cận chiến)
// ==========================================
(function initGrapplingSystem() {
    console.log("🔥 Đã kích hoạt hệ thống Grappling & Tương tác Vật lý!");

    // ----------------------------------------------------
    // 1. CẤY HOẠT ẢNH MỚI VÀO GRAPHICS ENGINE
    // ----------------------------------------------------
    if (typeof window.drawBaseLimb !== 'undefined') {
        const originalDrawBaseLimb = window.drawBaseLimb;
        window.drawBaseLimb = function(ctx, p, bounce, ext, pext, isTrail) {
            let progress = (p.attackTimer > 0 && p.maxT) ? 1 - (p.attackTimer / p.maxT) : ext;
            
            // 1. Khóa Tay (Clinch) - Hai tay ghì chặt về phía trước
            if (p.state === 'clinch') {
                return {
                    head: {x: 10, y: -45 + bounce}, neck: {x: 5, y: -35 + bounce}, pelvis: {x: 0, y: -15},
                    handL: {x: 25, y: -40 + bounce}, elbowL: {x: 15, y: -35 + bounce},
                    handR: {x: 25, y: -30 + bounce}, elbowR: {x: 15, y: -25 + bounce},
                    footL: {x: -15, y: 0}, kneeL: {x: -10, y: -10}, footR: {x: 20, y: 0}, kneeR: {x: 15, y: -10}
                };
            }
            // 2. Nhào Lộn (Leapfrog) - Cuộn tròn xoay vòng trên không
            else if (p.state === 'leapfrog') {
                let spin = Date.now() / 50; 
                return {
                    head: {x: Math.cos(spin)*15, y: -25 + Math.sin(spin)*15}, 
                    neck: {x: 0, y: -20}, pelvis: {x: 0, y: -15},
                    handL: {x: Math.cos(spin+1)*15, y: -25 + Math.sin(spin+1)*15}, elbowL: {x: 0, y: -20},
                    handR: {x: Math.cos(spin+2)*15, y: -25 + Math.sin(spin+2)*15}, elbowR: {x: 0, y: -20},
                    footL: {x: Math.cos(spin+3)*15, y: -25 + Math.sin(spin+3)*15}, kneeL: {x: 0, y: -20},
                    footR: {x: Math.cos(spin+4)*15, y: -25 + Math.sin(spin+4)*15}, kneeR: {x: 0, y: -20}
                };
            }
            // 3. Tông Bế (Tackle) - Cúi thấp vai húc tới
            else if (p.state === 'tackle_dash') {
                return {
                    head: {x: 35, y: -25}, neck: {x: 20, y: -20}, pelvis: {x: 0, y: -15},
                    handL: {x: 40, y: -10}, elbowL: {x: 25, y: -10},
                    handR: {x: 45, y: -5}, elbowR: {x: 30, y: -5},
                    footL: {x: -25, y: -5}, kneeL: {x: -10, y: -10}, footR: {x: 15, y: 0}, kneeR: {x: 10, y: -10}
                };
            }
            else if (p.state === 'tackle_hold') { // Dáng người bị bế
                return {
                    head: {x: -10, y: 0}, neck: {x: 0, y: -10}, pelvis: {x: 10, y: -30},
                    handL: {x: -20, y: -20}, elbowL: {x: -10, y: -15}, handR: {x: -15, y: -25}, elbowR: {x: 0, y: -20},
                    footL: {x: 25, y: -45}, kneeL: {x: 20, y: -35}, footR: {x: 30, y: -40}, kneeR: {x: 20, y: -30}
                };
            }
            // 4. Quật Ngã (Judo) - Túm lấy vặn người quật
            else if (p.state === 'judo_throw') {
                return {
                    head: {x: -15, y: -40}, neck: {x: -5, y: -30}, pelvis: {x: 5, y: -15},
                    handL: {x: -35, y: 10}, elbowL: {x: -20, y: 0}, 
                    handR: {x: -30, y: 15}, elbowR: {x: -15, y: 0},
                    footL: {x: -10, y: 0}, kneeL: {x: -5, y: -10}, footR: {x: 20, y: 0}, kneeR: {x: 10, y: -10}
                };
            }
            else if (p.state === 'thrown') { // Dáng người bị ném lộn nhào
                return {
                    head: {x: 0, y: 15}, neck: {x: 0, y: 0}, pelvis: {x: 0, y: -20},
                    handL: {x: -20, y: -10}, elbowL: {x: -10, y: -5}, handR: {x: 20, y: -10}, elbowR: {x: 10, y: -5},
                    footL: {x: -15, y: -50}, kneeL: {x: -10, y: -35}, footR: {x: 15, y: -50}, kneeR: {x: 10, y: -35}
                };
            }
            // 5. Đè Ngựa (Mount) - Ngồi lên bụng nện tay xuống
            else if (p.state === 'mount') {
                return {
                    head: {x: 5, y: -40}, neck: {x: 0, y: -25}, pelvis: {x: 0, y: 10},
                    handL: {x: 15, y: progress <= 0.5 ? 25 : -30}, elbowL: {x: 10, y: 0}, 
                    handR: {x: 20, y: progress > 0.5 ? 25 : -30}, elbowR: {x: 10, y: 0},
                    footL: {x: -15, y: 20}, kneeL: {x: -10, y: 5}, footR: {x: 15, y: 20}, kneeR: {x: 10, y: 5}
                };
            }
            else if (p.state === 'mounted') { // Kẻ bị đè xòe tay chân ra
                return {
                    head: {x: -20, y: 18}, neck: {x: -10, y: 18}, pelvis: {x: 15, y: 18},
                    handL: {x: -10, y: 0}, elbowL: {x: 0, y: 10}, handR: {x: -5, y: -5}, elbowR: {x: 5, y: 10},
                    footL: {x: 35, y: 18}, kneeL: {x: 25, y: 5}, footR: {x: 40, y: 18}, kneeR: {x: 30, y: 5}
                };
            }

            // Fallback về hoạt ảnh mặc định cũ
            return originalDrawBaseLimb(ctx, p, bounce, ext, pext, isTrail);
        };
    }

    // ----------------------------------------------------
    // 2. CẤY LOGIC VA CHẠM VÀO VÒNG LẶP ENGINE
    // ----------------------------------------------------
    if (typeof window.update !== 'undefined') {
        const originalUpdate = window.update;
        window.update = function() {
            let allFighters = [window.p1].concat(window.enemies);
            
            // QUÉT VA CHẠM ĐỂ KÍCH HOẠT TƯƠNG TÁC
            for(let i=0; i<allFighters.length; i++) {
                for(let j=i+1; j<allFighters.length; j++) {
                    ...
                    let dist = Math.abs(f1.x - f2.x);
                    let isF1FacingF2 = f1.isFacingRight === (f2.x > f1.x);
                    let isF2FacingF1 = f2.isFacingRight === (f1.x > f2.x);

                    // 2.1 Nhào Lộn (Leapfrog) qua đầu địch khi Dash sát nhau
                    if (dist < 55 && f1.state === 'dash' && isF1FacingF2 && f2.state !== 'ko_falling' && f2.state !== 'thrown') {
                        f1.state = 'leapfrog'; f1.vy = -12; f1.vx = f1.isFacingRight ? 10 : -10; f1.onGround = false; f1.iFrames = 25; f1.attackTimer = 25;
                        f2.state = 'stunned'; f2.stunTimer = 25; f2.vx = 0; // Địch bị khựng lại
                        if(typeof window.playSound === 'function') window.playSound(500, 'sine', 0.2, 0.4);
                        if(typeof window.spawnParticles === 'function') window.spawnParticles(f1.x, f1.y, "#00f3ff");
                        window.floatingTexts.push({x: f1.x, y: f1.y-60, text: "🤸 VAULT!", color: "#00f3ff", alpha: 1, vx: 0, vy: -2, font: "900 25px Arial", life: 30});
                    }

                    // 2.2 Khóa Tay Giằng Co (Clinch) khi cùng đi bộ đâm vào nhau
                    if (dist < 40 && (f1.state === 'walk' || f1.state === 'dash') && (f2.state === 'walk' || f2.state === 'dash') && isF1FacingF2 && isF2FacingF1 && f1.onGround && f2.onGround) {
                        if (!f1.clinchTimer && !f2.clinchTimer && f1.state !== 'clinch' && f2.state !== 'clinch') {
                            f1.state = 'clinch'; f2.state = 'clinch'; f1.vx = 0; f2.vx = 0;
                            f1.clinchTimer = 50; f2.clinchTimer = 50; // Giằng co 50 frames
                            if(typeof window.playSound === 'function') window.playSound(200, 'square', 0.2, 0.5, true);
                            if(typeof window.shakeScreen === 'function') window.shakeScreen(15, 5);
                            window.floatingTexts.push({x: (f1.x+f2.x)/2, y: f1.y-70, text: "⚔️ CLINCH!", color: "#e67e22", alpha: 1, vx: 0, vy: -1, font: "900 30px Arial", life: 40});
                        }
                    }
                    // Giải quyết kết quả Khóa Tay
                    if (f1.state === 'clinch' && f2.state === 'clinch') {
                        f1.x = f2.x + (f1.isFacingRight ? -30 : 30); // Ép 2 nhân vật dính nhau
                        f1.clinchTimer--; f2.clinchTimer--;
                        f1.x += (Math.random()-0.5)*4; f2.x += (Math.random()-0.5)*4; // Lắc lư do dùng sức
                        if (f1.clinchTimer <= 0) {
                            let winner = (f1.stamina + Math.random()*20) > (f2.stamina + Math.random()*20) ? f1 : f2;
                            let loser = winner === f1 ? f2 : f1;
                            winner.state = 'knee_strike'; winner.attackTimer = 15; winner.stamina -= 15;
                            if(typeof window.takeDamage === 'function') window.takeDamage(loser, 15, "#fff", false, false);
                            loser.state = 'hurt'; loser.hitStun = 30; loser.vx = winner.isFacingRight ? 8 : -8;
                        }
                    }

                    // 2.3 Tông Bế Ép Tường (Tackle & Carry)
                    let checkTackle = (atk, def) => {
                        if (atk.state === 'tackle_dash' && dist < 50 && def.state !== 'tackle_hold' && def.y >= window.GROUND_Y - 50) {
                            def.state = 'tackle_hold'; def.iFrames = 20; def.hitStun = 20; // Bị bế lên
                            if(typeof window.shakeScreen === 'function') window.shakeScreen(5, 5);
                            if(typeof window.playSound === 'function') window.playSound(150, 'square', 0.2, 0.4, true);
                        }
                        if (atk.state === 'tackle_dash' && def.state === 'tackle_hold') {
                            atk.vx = atk.isFacingRight ? 18 : -18;
                            def.x = atk.x + (atk.isFacingRight ? 35 : -35); // Bị đẩy đi lùi
                            def.y = atk.y - 10; def.vx = 0; def.vy = 0; def.onGround = false;
                            
                            let wallBound = 50; // Quét xem có chạm mép tường không
                            if (def.x <= wallBound || def.x >= window.canvas.width - wallBound) {
                                atk.state = 'idle'; atk.vx = atk.isFacingRight ? -6 : 6; atk.attackTimer = 0;
                                def.state = 'wall_splat'; def.y = window.GROUND_Y; def.hitStun = 50;
                                if(typeof window.takeDamage === 'function') window.takeDamage(def, 45, "#ff4757", true, false); // Nổ ST lớn
                                if(typeof window.shakeScreen === 'function') window.shakeScreen(25, 15);
                                if(typeof window.playSound === 'function') window.playSound(100, 'sawtooth', 0.5, 1.0, true);
                                if(typeof window.spawnEnvDamage === 'function') window.spawnEnvDamage(def.x, def.y, def.x <= wallBound ? 'wall_left' : 'wall_right', 2, true);
                                window.floatingTexts.push({x: def.x, y: def.y-80, text: "💥 WALL CRUSH!", color: "#ff4757", alpha: 1, vx: 0, vy: -3, font: "900 45px Arial", life: 50});
                            }
                        }
                    };
                    checkTackle(f1, f2); checkTackle(f2, f1);

                    // 2.4 Đè Ngựa Đấm Nện (Ground & Pound)
                    let checkMount = (atk, def) => {
                        if (atk.state === 'dash' && dist < 45 && (def.state === 'wall_splat' || def.state === 'ko_falling') && def.y >= window.GROUND_Y - 15) {
                            atk.state = 'mount'; atk.mountTimer = 80; atk.vx = 0; atk.y = window.GROUND_Y;
                            def.state = 'mounted'; def.vx = 0; def.vy = 0; def.y = window.GROUND_Y; def.koTimer = 100;
                            if(typeof window.playSound === 'function') window.playSound(200, 'square', 0.2, 0.4);
                            window.floatingTexts.push({x: atk.x, y: atk.y-80, text: "🤼 MOUNTED!", color: "#e74c3c", alpha: 1, vx: 0, vy: -2, font: "900 35px Arial", life: 40});
                        }
                        if (atk.state === 'mount' && def.state === 'mounted') {
                            def.x = atk.x + (atk.isFacingRight ? 25 : -25); // Khóa sát mặt đất
                            atk.mountTimer--; 
                            atk.attackTimer = atk.mountTimer % 15; // Phục vụ Animation nhịp đấm giã gạo
                            if (atk.mountTimer > 0 && atk.mountTimer % 15 === 0) {
                                if(typeof window.takeDamage === 'function') window.takeDamage(def, 6, "#ff0000", false, false);
                                if(typeof window.shakeScreen === 'function') window.shakeScreen(5, 5);
                            }
                            if (atk.mountTimer <= 0) { // Hết giờ, bị đạp văng ra
                                atk.state = 'hurt'; atk.hitStun = 20; atk.vx = atk.isFacingRight ? -8 : 8; atk.vy = -6; atk.onGround = false;
                                def.state = 'idle'; def.iFrames = 20;
                            }
                        }
                    };
                    checkMount(f1, f2); checkMount(f2, f1);
                }
            }
            
            // Trả về luồng update gốc của engine để chạy tiếp các physics cơ bản
            originalUpdate();
        };
    }

    // ----------------------------------------------------
    // 3. CẤY JUDO REVERSAL (Quật Ngã) VÀO HỆ THỐNG TAKEDAMAGE
    // ----------------------------------------------------
    if (typeof window.takeDamage !== 'undefined') {
        const originalTakeDamage = window.takeDamage;
        window.takeDamage = function(target, amount, color, isCrit, wallBounce) {
            
            // Kích hoạt khi Đỡ đòn hoàn hảo (Perfect Parry)
            if (target && target.state === 'block' && target.attackTimer >= 22) {
                let attacker = window.getClosestEnemy(target, target.isPlayer ? window.enemies : [window.p1]);
                
                // Nếu kẻ địch ở sát sườn -> Vật qua vai thay vì chỉ hất văng
                if (attacker && Math.abs(attacker.x - target.x) < 70 && attacker.y >= window.GROUND_Y - 20) {
                    target.state = 'judo_throw'; target.attackTimer = 35; target.stamina = Math.min(100, target.stamina + 30);
                    target.vx = 0; target.isFacingRight = attacker.x > target.x;
                    
                    attacker.state = 'thrown'; attacker.hitStun = 40;
                    attacker.vx = target.isFacingRight ? -12 : 12; // Bị văng ngược ra sau lưng
                    attacker.vy = -10; attacker.onGround = false;
                    
                    if(typeof window.playSound === 'function') window.playSound(300, 'square', 0.5, 0.9, true);
                    if(typeof window.shakeScreen === 'function') window.shakeScreen(20, 10);
                    window.floatingTexts.push({ x: target.x, y: target.y - 80, text: "☯️ REVERSAL!", color: "#2ecc71", alpha: 1, vx: 0, vy: -3, font: "900 35px Arial", life: 50 });
                    
                    // Deal sát thương khi kẻ địch đập lưng xuống đất
                    setTimeout(() => {
                        if (attacker.hp > 0 && typeof originalTakeDamage === 'function') {
                            originalTakeDamage(attacker, 25, "#2ecc71", true, false);
                            window.spawnDust(attacker.x, window.GROUND_Y);
                        }
                    }, 300);
                    
                    return; // Return để hủy bỏ lệnh trừ máu gốc
                }
            }
            // Nếu không thỏa mãn đk quật ngã, chạy trừ máu như bình thường
            originalTakeDamage(target, amount, color, isCrit, wallBounce);
        };
    }

    // ----------------------------------------------------
    // 4. CẤY CHIÊU TACKLE (Bế ép tường) VÀO DANH SÁCH CHIÊU THỨC
    // ----------------------------------------------------
    if (typeof window.attack !== 'undefined') {
        const originalAttack = window.attack;
        window.attack = function(attacker, targetGroup) {
            let target = window.getClosestEnemy(attacker, targetGroup);
            
            // Tỷ lệ 15% kích hoạt Tông Ép Tường khi đứng ở xa và đủ Stamina
            if (target && target.hp > 0 && Math.abs(attacker.x - target.x) > 100 && Math.random() < 0.15 && attacker.stamina > 20) {
                attacker.isFacingRight = target.x > attacker.x;
                attacker.state = 'tackle_dash';
                attacker.attackTimer = 40;
                attacker.vx = (attacker.isFacingRight ? 1 : -1) * attacker.currentSpeed * 2.5; // Lao cực nhanh
                attacker.stamina -= 15;
                if(typeof window.playSound === 'function') window.playSound(250, 'sine', 0.3, 0.5);
                window.spawnDust(attacker.x, attacker.y);
                window.floatingTexts.push({ x: attacker.x, y: attacker.y - 50, text: "🚂 TACKLE!", color: "#f39c12", alpha: 1, vx: 0, vy: -1, font: "900 25px Arial", life: 30 });
                return; // Return để bỏ qua đánh đấm thường
            }
            // Trả về đánh đấm thường
            originalAttack(attacker, targetGroup);
        };
    }
})();
