// ==========================================
// RECORDER.JS - BẢN HOÀN HẢO CHO CONTENT CREATOR (V66.21 - ESPORTS ZERO-GC ENGINE)
// [CRITICAL FIX] Dùng "Object Pooling" cho Particles/Emojis, triệt tiêu Rác bộ nhớ (Zero Garbage Collection) gây khựng giật.
// [CRITICAL FIX] Dùng "Lookup Table" (Tính toán trước lưới 3D) thay vì dùng Math.pow() 4800 lần/giây.
// [CRITICAL FIX] Ưu tiên Hardware H.264 (avc1) MP4 + Bitrate 18Mbps cho chuẩn quay màn hình eSports.
// [RETAINED] Đầy đủ Meta: VTuber Reaction, Cinematic Zoom, Binge-Watch Fake Grid.
// ==========================================
...
// [ESPORTS FIX] Pre-calculate 3D Grid Math (Tính trước lưới 3D để giảm tải CPU)
window.precalcGridY = new Float32Array(40);
for(let y = 1; y < 40; y++) window.precalcGridY[y] = Math.pow(y, 1.8) * 2.5;

// [ESPORTS FIX] Object Pooling cho Particles và Emojis (KHÔNG push, KHÔNG splice để tránh dọn rác RAM)
...
function drawVTuberCommentator(ctx, x, y, audioPeak, isTyping) {
    ...
    ctx.shadowBlur = 15;
    ctx.beginPath();
    if(ctx.roundRect) ctx.roundRect(-120, -90, 240, 180, 30); 
    else ctx.rect(-120, -90, 240, 180);
    ctx.fill(); ctx.stroke();

    ctx.shadowBlur = 0;
    ctx.fillStyle = "#020617"; 
    ctx.beginPath();
    if(ctx.roundRect) ctx.roundRect(-100, -75, 200, 150, 15);
    else ctx.rect(-100, -75, 200, 150);
    ctx.fill();
    ...
    if (isBlinking) {
        ctx.fillRect(-60, -30, 40, 6);
        ctx.fillRect(20, -30, 40, 6);
    } else {
        ctx.save(); ctx.translate(-45, -35); ctx.rotate(0.3);
        ctx.beginPath(); ctx.ellipse(0, 0, 18, 25, 0, 0, Math.PI*2); ctx.fill();
        ctx.restore();
    ...
    ctx.beginPath();
    if(ctx.roundRect) ctx.roundRect(-35, 25, 70, mouthHeight, 10);
    else ctx.fillRect(-35, 25, 70, mouthHeight);
    ctx.fill();

    ctx.shadowBlur = 0;
    ctx.fillStyle = "#ffeb3b";
    ctx.fillRect(120, -20, 15, 40);
    ctx.beginPath(); ctx.arc(140, 0, 10, 0, Math.PI*2); ctx.fill();

    ctx.rotate(-tilt); 
    ctx.fillStyle = "#ff0000";
    ctx.beginPath(); if(ctx.roundRect) ctx.roundRect(-180, -120, 80, 30, 6); else ctx.fillRect(-180, -120, 80, 30); ctx.fill();
...
window.bakeThumbnailsForVideo = function(titleText) {
    ...
    try {
        ...
        const drawCharSafe = (ctx, charObj, cx, cy, scale, isFacingRight) => {
            ...
            ctx.shadowColor = `hsl(${isFacingRight ? hue1 : hue2}, 100%, 70%)`; ctx.shadowBlur = 40;
            for(let i=0; i<3; i++) {
        ...
        const drawClickbaitProps = (ctx, w, h, isVertical) => {
            ctx.save();
            let rx = isVertical ? w/2 : w/2 + (Math.random() > 0.5 ? 200 : -200);
            let ry = isVertical ? h*0.4 + Math.random()*150 : h/2 + Math.random()*80;
            ctx.strokeStyle = "#ff0000"; ctx.lineWidth = 18; ctx.shadowColor = "#000"; ctx.shadowBlur = 15;
            ...
            ctx.font = "italic 900 80px Impact"; ctx.textAlign = "center";
            ctx.lineWidth = 18; ctx.strokeStyle = "#000"; ctx.strokeText("-999,999 💢", 0,0);
        ...
        const draw3DTitle = (ctx, w, h, text, isVertical) => {
            ...
            let depth = 15; let colors = [["#f1c40f", "#d35400"], ["#00f3ff", "#0055ff"], ["#ff4757", "#8b0000"]];
...
window.initRecorder = function() {
    ...
    window.recordCtxH = window.recordCanvasH.getContext("2d", {alpha: false, desynchronized: true});
...
window.startRecording = function() {
    ...
    // Reset Object Pools (Chỉ gán lại active=false, không tạo mới array để tránh GC)
    ...
    // [ESPORTS FIX] Ưu tiên Hardware Encoding (avc1) + Mức băng thông Esports 18Mbps.
    let options = { videoBitsPerSecond: 18000000 }; 
    window.currentVideoExt = "mp4";
    let selectedMime = "";
    
    const possibleTypes = [
        'video/mp4; codecs="avc1.42E01E, mp4a.40.2"', // Hardware H264
        'video/mp4; codecs="avc1"',                   // Fallback H264
        'video/webm; codecs="h264"',                  // Hardware WebM wrapper
        'video/webm; codecs="vp8"'                    // Software fallback cực nhẹ
    ];
    
    for (let type of possibleTypes) {
        if (MediaRecorder.isTypeSupported(type)) {
            selectedMime = type;
            if(type.includes("webm")) window.currentVideoExt = "webm";
            break;
        }
    }
    ...
    const finalizeRecordings = () => {
        ...
        if (stoppedCount === 2) { 
            setTimeout(() => {
                ...
                let mimeH = window.mediaRecorderH.mimeType || "video/mp4";
                let mimeV = window.mediaRecorderV.mimeType || "video/mp4";
                
                let blobH = new Blob(window.recordedChunksH, { type: mimeH }); let videoUrlH = URL.createObjectURL(blobH);
                let blobV = new Blob(window.recordedChunksV, { type: mimeV }); let videoUrlV = URL.createObjectURL(blobV);
...
function renderMobileSafeCombo(ctx, fighter, xPos, yPos, align, isMobile) {
    if (fighter && fighter.comboHits >= 2) {
        ...
        ctx.font = `italic 900 ${(36 * scale)|0}px 'Arial Black', Impact`;
        ctx.fillStyle = fighter.isPlayer ? "#ff9f43" : "#ff4757"; ctx.shadowColor = ctx.fillStyle; ctx.shadowBlur = (10 * scale)|0;
        ctx.fillText(`🔥 ${hits} HITS`, xPos | 0, yPos | 0);
        ctx.font = `italic 900 ${(26 * scale)|0}px 'Arial Black', Impact`; ctx.fillStyle = rankColor; ctx.shadowColor = rankColor; ctx.shadowBlur = (15 * scale)|0;
...
function drawAudioVisualizer(ctx, x, y, width, height) {
    ...
    ctx.save();
    ctx.beginPath(); 
    for (let i = 0; i < 12; i++) { // numBars / 2
        let barHeight = (window.analyserData[i + 2] / 255) * height; 
        if (barHeight < 5) barHeight = 5; 
        ctx.fillStyle = (i%2===0) ? "#00f3ff" : "#f1c40f"; // [ESPORTS FIX] Bỏ tạo Gradient mỗi frame
...
window.captureFrames = function() {
    ...
    // Đã gỡ lệnh Scale thừa. Vẽ 1:1 siêu nhẹ máy.
    ...
    if (audioPeak > 0.6) {
        ...
        cameraZoom = 1.0 + (audioPeak - 0.6) * 0.15; 
    }

    // --- RENDER BẢN NGANG 1080P ---
    if (renderNormalH) {
        // [ESPORTS FIX] Tắt ImageSmoothing hoàn toàn khi không zoom để tối ưu
        ctxH.imageSmoothingEnabled = (cameraZoom > 1.0); 
        
        let barHeightH = 80;
        
        ctxH.save();
        if (cameraZoom > 1.0) {
            ctxH.translate(960, 540); ctxH.scale(cameraZoom, cameraZoom); ctxH.translate(-960, -540);
        }
        
        ctxH.drawImage(window.canvas, 0, 0, window.canvas.width, window.canvas.height, shakeX | 0, (shakeY + barHeightH) | 0, 1920, 1080 - barHeightH*2); 
        
        if (audioPeak > 0.85) {
            let glitchStr = ((audioPeak - 0.85) * 60) | 0;
            ctxH.globalCompositeOperation = 'screen';
            ctxH.fillStyle = 'rgba(255, 0, 0, 0.5)'; 
            ctxH.drawImage(window.canvas, 0, 0, window.canvas.width, window.canvas.height, -glitchStr, (shakeY + barHeightH) | 0, 1920 + glitchStr, 1080 - barHeightH*2);
            ctxH.fillStyle = 'rgba(0, 255, 255, 0.5)'; 
            ctxH.drawImage(window.canvas, 0, 0, window.canvas.width, window.canvas.height, glitchStr, (shakeY + barHeightH) | 0, 1920 + glitchStr, 1080 - barHeightH*2);
            ctxH.globalCompositeOperation = 'source-over';
            
            ctxH.fillStyle = `rgba(255, 255, 255, ${(audioPeak - 0.85) * 2})`;
            ctxH.fillRect(0, barHeightH, 1920, 1080 - barHeightH*2);
        }
        ctxH.restore();
        
        ctxH.fillStyle = "#000000";
        ctxH.fillRect(0, 0, 1920, barHeightH);
        ctxH.fillRect(0, 1080 - barHeightH, 1920, barHeightH);
    }

    // --- RENDER BẢN DỌC 1080P ---
    let splitGameHeight = window.canvas ? Math.floor(1080 * (window.canvas.height / window.canvas.width)) : 607;
    ctxV.imageSmoothingEnabled = (cameraZoom > 1.0);
    
    ctxV.save();
    if (cameraZoom > 1.0) {
        ctxV.translate(540, splitGameHeight/2); ctxV.scale(cameraZoom, cameraZoom); ctxV.translate(-540, -splitGameHeight/2);
    }

    ctxV.drawImage(window.canvas, 0, 0, window.canvas.width, window.canvas.height, shakeX | 0, shakeY | 0, 1080, splitGameHeight); 

    if (audioPeak > 0.85) {
        let glitchStr = ((audioPeak - 0.85) * 50) | 0;
        ctxV.globalCompositeOperation = 'screen'; 
        ctxV.fillStyle = 'rgba(255, 0, 0, 0.5)'; 
        ...
        ctxV.fillStyle = 'rgba(0, 255, 255, 0.5)'; 
        ctxV.drawImage(window.canvas, 0, 0, window.canvas.width, window.canvas.height, glitchStr, shakeY | 0, 1080 + glitchStr, splitGameHeight);
        ctxV.globalCompositeOperation = 'source-over';
        
        ctxV.fillStyle = `rgba(255, 255, 255, ${(audioPeak - 0.85) * 2})`;
        ctxV.fillRect(0, 0, 1080, splitGameHeight);
    }
    ctxV.restore();

    if (audioPeak > 0.4) {
        ctxV.save(); ctxV.translate(540, (splitGameHeight / 2) | 0);
    ...
    // ==========================================
    // 3. BACKGROUND LƯỚI KHÔNG GIAN 3D (ZERO-GC SỬ DỤNG MẢNG PRE-CALC)
    // ==========================================
    let retainY = splitGameHeight; 
    let retainHeight = 1920 - retainY;
    let tNow = performance.now(); 
    
    // [ESPORTS FIX] Đơn giản hóa thuật toán vẽ Gradient
    let hueShift = ((tNow / 20) % 60) | 0;
    ctxV.fillStyle = `hsl(${260 + hueShift}, 100%, 8%)`; 
    ...
    ctxV.beginPath(); 
    for(let x = -20; x <= 20; x++) {
        ctxV.moveTo(540, retainY);
        ctxV.lineTo(540 + x * 200, 1920);
    }
    let zSpeed = (tNow / 15) % 20; 
    for(let y = 1; y < 40; y++) {
        // [ESPORTS FIX] Lấy từ mảng tính sẵn thay vì dùng Math.pow trong vòng lặp 60fps
        let actualY = retainY + window.precalcGridY[y] + zSpeed;
        if (actualY <= 1920) {
            ctxV.moveTo(0, actualY | 0); ctxV.lineTo(1080, actualY | 0);
        }
    }
    ctxV.stroke(); 

    // --- HIỆU ỨNG 1: HẠT NĂNG LƯỢNG (OBJECT POOLING ZERO-GC) ---
    if (Math.random() < 0.4) {
        let p = window.retentionParticles.find(p => !p.active);
        if (p) {
            p.active = true; p.x = Math.random() * 1080; p.y = 1920 + 50;
            p.s = Math.random() * 8 + 4; p.v = Math.random() * 8 + 4;
            p.h = Math.random() > 0.5 ? 300 : 190; p.age = 0;
        }
    }
    ctxV.globalCompositeOperation = "screen";
    ctxV.shadowBlur = 0; 
    for (let i = 0; i < 40; i++) {
        ...
        if (p.y < retainY) p.active = false; // Tái chế hạt
    }
    ctxV.globalCompositeOperation = "source-over";

    if (!window.gameOver && window.introTimer > 120) {
        drawVTuberCommentator(ctxV, 540, splitGameHeight + 280, audioPeak, window.StoryModeAI.isTyping);
    }

    let bannerY = retainY + 25;
    ctxV.fillStyle = "#ff0055"; 
    ctxV.fillRect(0, retainY, 1080, 50);
    ctxV.fillStyle = "#fff";
    ctxV.font = "900 35px 'Arial Black'";
    ctxV.textAlign = "left"; ctxV.textBaseline = "middle";
    let offsetBanner = ((tNow / 6) % 1000) | 0; 
    for(let i = -1; i < 5; i++) {
        ctxV.fillText("🚨 WAIT FOR THE END ⏩ DO NOT BLINK 🚨", i*850 - offsetBanner, bannerY);
    }

    // --- HIỆU ỨNG 3: EMOJIS (OBJECT POOLING ZERO-GC) ---
    if (audioPeak > 0.35 && Math.random() < 0.25 && !window.gameOver) { 
        ...
        if (re) {
            ...
            re.v = 10 + Math.random() * 8; re.e = emojiList[(Math.random() * emojiList.length) | 0];
    ...
    for (let i = 0; i < 15; i++) {
        ...
        ctxV.rotate(re.r + Math.sin(tNow/200)*0.2); 
        ...
        ctxV.restore();
        if (re.y < retainY) re.active = false; // Tái chế Emoji
    }

    // ==========================================
    // TRAFFIC ROUTING (MỎ NEO THỊ GIÁC ĐIỀU HƯỚNG FOLLOWER)
    // ==========================================
    if (!window.gameOver && tNow % 1000 < 600) {
        ctxV.save();
        ctxV.translate(1040, splitGameHeight - 40);
    ...
    if (!window.gameOver) {
        ctxV.save();
        let bouncerY = 850 + Math.sin(tNow / 150) * 30; 
        ctxV.translate(1012, bouncerY); 
        ctxV.rotate(-0.1); 
    ...
    if (window.p1 && !window.gameOver && window.introTimer <= 120) {
        ...
        if (renderNormalH) {
            ...
            if (img1) { ctxH.save(); ctxH.beginPath(); if (ctxH.roundRect) ctxH.roundRect(93, 33 + 80, 73, 73, 8); else ctxH.rect(93, 33 + 80, 73, 73); ctxH.clip(); ctxH.drawImage(img1, 93, 33 + 80, 73, 73); ctxH.restore(); ctxH.lineWidth = 5; ctxH.strokeStyle = "#00f3ff"; ctxH.strokeRect(93, 33 + 80, 73, 73); }
            ctxH.lineWidth = 10; ctxH.strokeStyle = "#000"; ctxH.strokeText(p1Name, 193, 96 + 80); ctxH.fillStyle = "#fff"; ctxH.fillText(p1Name, 193, 96 + 80);
            drawSkewedPath(ctxH, 107, 120 + 80, 1000, 60, true); ctxH.fillStyle = "rgba(0,0,0,0.7)"; ctxH.fill(); ctxH.lineWidth = 7; ctxH.strokeStyle = "rgba(255,255,255,0.9)"; ctxH.stroke();
            if (p1Hp > 0) { let hpGrad = ctxH.createLinearGradient(107, 0, 1107, 0); hpGrad.addColorStop(0, "#ff4757"); hpGrad.addColorStop(1, "#ff7f50"); drawSkewedPath(ctxH, 107, 120 + 80, (1000 * p1Hp) | 0, 60, true); ctxH.fillStyle = hpGrad; ctxH.fill(); }
            ctxH.fillStyle = "rgba(0,0,0,0.8)"; ctxH.fillRect(80, 193 + 80, 533, 20); ctxH.fillStyle = "#f1c40f"; ctxH.fillRect(80, 193 + 80, (533 * p1Stam) | 0, 20);

            if (repEnemyObj) {
                ctxH.textAlign = "right"; 
                if (img2) { ctxH.save(); ctxH.beginPath(); if (ctxH.roundRect) ctxH.roundRect(1750, 33 + 80, 73, 73, 8); else ctxH.rect(1750, 33 + 80, 73, 73); ctxH.clip(); ctxH.drawImage(img2, 1750, 33 + 80, 73, 73); ctxH.restore(); ctxH.lineWidth = 5; ctxH.strokeStyle = "#ff003c"; ctxH.strokeRect(1750, 33 + 80, 73, 73); }
                ctxH.lineWidth = 10; ctxH.strokeStyle = "#000"; ctxH.strokeText(eName, 1730, 96 + 80); ctxH.fillStyle = "#fff"; ctxH.fillText(eName, 1730, 96 + 80);
                drawSkewedPath(ctxH, 750, 120 + 80, 1000, 60, false); ctxH.fillStyle = "rgba(0,0,0,0.7)"; ctxH.fill(); ctxH.lineWidth = 7; ctxH.strokeStyle = "rgba(255,255,255,0.9)"; ctxH.stroke();
                if (p2Hp > 0) { let hpGrad = ctxH.createLinearGradient(750, 0, 1750, 0); hpGrad.addColorStop(0, "#c0392b"); hpGrad.addColorStop(1, "#e74c3c"); let eHpWidth = (1000 * p2Hp) | 0; drawSkewedPath(ctxH, 750 + (1000 - eHpWidth), 120 + 80, eHpWidth, 60, false); ctxH.fillStyle = hpGrad; ctxH.fill(); }
                ctxH.fillStyle = "rgba(0,0,0,0.8)"; ctxH.fillRect(1300, 193 + 80, 533, 20); ctxH.fillStyle = "#f1c40f"; ctxH.fillRect(1300 + (533 - ((533 * eStam) | 0)), 193 + 80, (533 * eStam) | 0, 20);
            }
        }
        ...
        if (repEnemyObj) {
            ...
            ctxV.lineWidth = 9; ctxV.strokeStyle = "#000"; ctxV.strokeText(eName, 900, hudBaseY + 53); ctxV.fillStyle = "#fff"; ctxV.fillText(eName, 900, hudBaseY + 53);
