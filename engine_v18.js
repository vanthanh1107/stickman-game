...
// [THUẬT TOÁN TỐI ƯU SHORT-FORM: TIKTOK / YOUTUBE SHORTS / REELS VIRAL]
// [TÍNH NĂNG: ANTI-AI MUTATION, ANIME IMPACT FRAMES, SPRING CAMERA 2.0, HARD-RESET VRAM]
// ==========================================
...
// [VIRAL 16.0] BIẾN ĐIỆN ẢNH & ĐỘT BIẾN CHỐNG THUẬT TOÁN AI
window.vignetteAlpha = 0.5; window.isLowHpPulsing = 0; window.speedLinesAlpha = 0; 
window.impactFrameCount = 0; // Đếm khung hình Anime Impact Frame
window.cameraZoomVel = 0; // Vận tốc lò xo Camera
...
window.initGameEngine = function() {
    ...
    // [VIRAL 16.0] ĐỘT BIẾN TRẬN ĐẤU (Biến đổi ngẫu nhiên để vô hiệu hóa thuật toán quét AI)
...
window.triggerTimeStop = function(frames, caster) { 
    ...
    if (window.shockwaves.length < MAX_SHOCKWAVES) { window.shockwaves.push({x: caster.x, y: caster.y, r: 10, maxR: 1500, color: "#fff", alpha: 1, speed: 45}); }
}
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce) {
    ...
    if (target.state === 'dash' || target.state === 'dash_back') {
        if (target.dashTimer > 2 && target.dashTimer < 14) {
            window.playSound(800, 'sine', 0.5, 1.0, true); window.applyFilter('invert', 6); window.slowMoTimer = 25; window.speedLinesAlpha = 1.0; 
            window.floatingTexts.push({ x: target.x, y: target.y - 80, text: "⚡ FLASH STEP!", color: "#f1c40f", alpha: 1, vx: 0, vy: -4, font: "900 40px Arial", life: 50 });
            window.spawnParticles(target.x, target.y - 40, "#f1c40f", true);
            if (window.shockwaves.length < MAX_SHOCKWAVES) window.shockwaves.push({x: target.x, y: target.y - 40, r: 5, maxR: 350, color: "#f1c40f", alpha: 1, speed: 25});
            target.stamina = Math.min(100, target.stamina + 100); target.x += target.isFacingRight ? -110 : 110; target.iFrames = 25; return; 
        }
    }
    ...
    if (target.state === 'block') { 
        if (target.stamina < 15) {
            ...
            window.hitStopFrames = 15; window.chromaTimer = 15; window.targetZoom = 1.4; window.vhsGlitchTimer = 20;
        } 
    ...
    if (finalDmg > 0) {
        ...
        if (isCrit) {
            // [VIRAL 16.0] ANIME IMPACT FRAME & DYNAMIC TIA LỬA NỔ
            window.impactFrameCount = 3; 
            for(let i=0; i<12; i++) {
                window.impactSparks.push({ x: target.x, y: target.y - 40, vx: (Math.random()-0.5)*22, vy: (Math.random()-0.5)*22, life: 25 + Math.random()*15, maxLife: 40, color: color || "#f1c40f" });
            }
        }
        ...
        if (wallBounce) { target.vx = target.isFacingRight ? -5 : 5; } 
        ...
        if (target.hp <= 0) {
            ...
            if (isNearWall && isCrit && typeof window.triggerStageTransition === 'function') { window.triggerStageTransition(target); } 
                ...
                if (isCrit) {
                    ...
                    window.floatingTexts.push({ x: target.x, y: target.y - 120, text: "💀 LETHAL FINISH!", color: "#ff003c", alpha: 1, vx: 0, vy: -1, font: "900 65px Arial", life: 80 });
                    window.slowMoTimer = 85; window.targetZoom = 1.55; window.hitStopFrames = 18; window.shakeScreen(45, 28);
        ...
        } else if (isCrit) { window.impactFrameTimer = 3; window.hitStopFrames = 5; window.shakeScreen(15, 10); window.targetZoom = 1.2; window.chromaTimer = 12; window.playSound(180, 'square', 0.3, 0.6, true);
        } else { window.hitStopFrames = 1; window.shakeScreen(4, 4); window.playSound(250, 'sine', 0.15, 0.3, true); }
    }
};
...
window.attack = function(attacker, targetGroup) {
    ...
    // [VIRAL 16.0] TĂNG TỶ LỆ FINISHER ĐỂ TẠO CÁC MÀN ĐẤM NHAU DỒN DẬP HƠN
    ...
    if (MathDist > reach && !isFinisher) { attacker.vx = (attacker.isFacingRight ? 1 : -1) * attacker.currentSpeed * 3.2; attacker.state = 'dash'; attacker.attackTimer = 10; window.spawnDust(attacker.x, attacker.y); return; }

    attacker.state = selectedMove; attacker.attackTimer = isFinisher ? 30 : 18; attacker.vx = (attacker.isFacingRight ? 1 : -1) * (isFinisher ? 5.5 : 1.8); 
    ...
    if (isFinisher) {
        isCrit = true; finalDmg = baseDmg * 3.5; window.shakeScreen(18, 14); target.vx = (attacker.isFacingRight ? 5.5 : -5.5); target.state = 'hurt'; target.hitStun = 48; window.spawnParticles(target.x, target.y - 40, "#ff4757", true); window.floatingTexts.push({ x: target.x, y: target.y - 80, text: "💥", color: "#ff4757", alpha: 1, vx: (Math.random()-0.5)*2, vy: -4, font: "900 45px Arial", life: 50 });
    } else {
        ...
        target.vx = (attacker.isFacingRight ? 2.5 : -2.5); target.hitStun = 15; target.state = 'hurt';
    }
...
window.update = function() {
    ...
    // [VIRAL 16.0] THUẬT TOÁN BẮT ĐẦU TRẬN ĐẤU NHANH (INSTANT HOOK INTRO)
    if (window.introTimer > 0) { 
        window.introTimer -= 2; // Giảm thời gian chờ intro xuống gấp đôi
    ...
    // BỘ XÓA THỜI GIAN VẼ LỜI BỌC KHÔNG BỊ LẶP LẠI
...
window.draw = function() {
    ...
    // HARD-RESET MATRIX TRƯỚC KHI VẼ ĐỂ CHỐNG DÍNH HÌNH 100%
    ...
    // [VIRAL 16.0] ANIME IMPACT FRAME RENDERING PASS (ĐỊNH DẠNG TƯƠNG PHẢN CAO TẠO ĐỘ LỰC)
    if (window.impactFrameCount > 0) {
        ...
        allFighters.forEach(p => {
            if (p && p.hp > 0) {
                ...
                window.ctx.arc(p.x, p.y - 40, 30 * (p.scale||1), 0, Math.PI*2);
                window.ctx.fill();
            }
        });
        return; // Kết thúc sớm pass này để chớp trắng đen
    }
    ...
    if (window.isLoading) {
        ...
        window.ctx.fillStyle = "#00f3ff"; window.ctx.font = "italic 900 40px Arial"; window.ctx.textAlign = "center"; window.ctx.shadowBlur = 15; window.ctx.shadowColor = "#00f3ff"; window.ctx.fillText("INITIALIZING VIRAL ENGINE 16.0...", cx, cy - 50);
    ...
    try {
        ...
        // [VIRAL 16.0] MUTATION FILTER (Áp dụng xoay màu Hue ngẫu nhiên cho môi trường để vượt qua quét AI)
        ...
        // TẮT TẤT CẢ FILTER ĐỂ VẼ NHÂN VẬT VÀ VFX NGUYÊN BẢN RỰC RỠ
        window.ctx.filter = 'none';
        ...
        window.particles.forEach(pt => { 
            ...
            else { window.ctx.shadowBlur = 12; window.ctx.shadowColor = pt.color; window.ctx.beginPath(); window.ctx.arc(pt.x, pt.y, pt.size, 0, Math.PI*2); window.ctx.fill(); window.ctx.shadowBlur = 0; if (pt.isCoin) { window.ctx.strokeStyle = "#d35400"; window.ctx.lineWidth = 1; window.ctx.stroke(); } }
        ...
        if (window.p1 && window.p1.comboHits >= 2) { window.ctx.globalAlpha = Math.max(0, window.p1.comboAlpha || 1); window.ctx.font = "italic 900 26px Arial"; window.ctx.fillStyle = "#ff9f43"; window.ctx.textAlign = "left"; window.ctx.shadowBlur = 10; window.ctx.shadowColor = "#ff9f43"; window.ctx.fillText(`🔥 ${window.p1.comboHits} HITS`, 20, 70 + Math.sin(Date.now() / 100) * 2); }
        ...
        if (maxEnemyCombo) { window.ctx.globalAlpha = Math.max(0, maxEnemyCombo.comboAlpha || 1); window.ctx.font = "italic 900 26px Arial"; window.ctx.fillStyle = "#ff4757"; window.ctx.textAlign = "right"; window.ctx.shadowBlur = 10; window.ctx.shadowColor = "#ff4757"; window.ctx.fillText(`🔥 ${maxEnemyCombo.comboHits} HITS`, window.canvas.width - 20, 70 + Math.sin(Date.now() / 100) * 2); }
...
window.gameLoop = function(timestamp) { 
    ...
    // [VIRAL 16.0] CAMERA LÒ XO NẢY ĐỘNG KHI CÓ VA CHẠM MẠNH
    let dZoom = window.targetZoom - window.currentZoom;
    window.cameraZoomVel += dZoom * 0.18;
    window.cameraZoomVel *= 0.72; // Lực ma sát lò xo
