// ==========================================
// ENGINE.JS - ABSOLUTE ULTIMATE ANIME MASTERPIECE 39.0 [EXTREME SHONEN EDITION]
// [RESTORED] Giữ nguyên UI Emoji, Text Manga, Splash Meme, Anti-Glare & Không gian Râu Trắng.
// [NEW ANIME TROPES INTEGRATED]
// 1. Lãnh Địa Triển Khai (Domain Expansion)
// 2. Va chạm Không trung Tốc độ cao (High-Speed Air Clash)
// 3. Trảm Sát Chậm (Iaijutsu Delayed Slash)
// 4. Gồng Aura Phản Trọng Lực (Anti-Gravity Ki Debris)
// 5. Sunburst Manga Impact Frames (Tia tốc độ hội tụ)
// 6. Chớp Sáng Con Mắt Chí Mạng (Eye Glint / Newtype Flash)
// ==========================================
...
window.originalMap = null;

const MAX_PARTICLES = 350; const MAX_SHOCKWAVES = 20;
...
// ==========================================
// 2B. CÁC HIỆU ỨNG ANIME TROPES MỚI
// ==========================================

// 1. LÃNH ĐỊA TRIỂN KHAI (JUJUTSU KAISEN)
window.triggerDomainExpansion = function(caster, domainName, themeColor, durationFrames) {
    window.timeStopTimer = 60; 
    window.screenFilter = 'invert'; window.filterTimer = 30;
    window.playSound(200, 'square', 1.5, 1.0, true);
    
    window.floatingTexts.push({
        x: window.canvas.width / 2, y: window.canvas.height / 2,
        text: `🤞 LÃNH ĐỊA TRIỂN KHAI: ${domainName.toUpperCase()}`,
        color: themeColor, alpha: 2.0, vx: 0, vy: -1, 
        font: "italic 900 55px 'Arial Black', Impact", 
        life: 120, scale: 0.1, targetScale: 1.3, scaleVel: 0, rot: 0
    });

    setTimeout(() => {
        window.originalMap = Object.assign({}, window.currentMap);
        window.currentMap = { 
            sky: "#000000", bg1: themeColor, bg2: "#111", ground: "#000", 
            line: themeColor, weather: "matrix_rain", bg1Type: "digital", bg2Type: "flowing_lava" 
        };
        
        window.screenFlash = 0.8; window.shakeScreen(30, 20);
        window.playSound(100, 'sawtooth', 2.0, 1.0, true);
        window.lethalVoid = durationFrames; 
        
        caster.superArmor = durationFrames;
        caster.buffs.push({stat: 'dmg', value: 2.0, life: durationFrames});
        
        let targetGroup = caster.isPlayer ? window.enemies : [window.p1];
        targetGroup.forEach(enemy => {
            if(enemy && enemy.hp > 0) enemy.buffs.push({stat: 'speed', value: -1.5, life: durationFrames});
        });

        setTimeout(() => {
            if (window.originalMap) window.currentMap = window.originalMap;
            window.screenFlash = 0.5; window.playSound(600, 'sine', 0.5, 0.8, true);
            window.triggerDimensionShatter(window.canvas.width/2, window.canvas.height/2);
        }, durationFrames * 16);
    }, 1000);
};

// 2. VA CHẠM KHÔNG TRUNG TỐC ĐỘ CAO (DBZ / BLEACH)
window.triggerAirClash = function(f1, f2, durationFrames) {
    f1.isInAirClash = true; f2.isInAirClash = true;
    window.targetCamY = -300; window.targetZoom = 1.5;
    
    let clashCount = 0;
    let clashInterval = setInterval(() => {
        if(clashCount > durationFrames/5 || window.gameOver) {
            clearInterval(clashInterval);
            f1.isInAirClash = false; f2.isInAirClash = false;
            f1.y = -100; f2.y = -100; f1.vy = 15; f2.vy = 15;
            window.targetCamY = window.GROUND_Y - 150;
            return;
        }
        
        let clashX = window.canvas.width/2 + (Math.random()-0.5)*600;
        let clashY = -200 + (Math.random()-0.5)*300;
        
        window.playSound(Math.random()*200 + 400, 'square', 0.1, 0.5, true);
        window.spawnSlash(clashX, clashY, Math.random()>0.5, "#fff", true, 2.0, Math.random()*Math.PI);
        if (window.shockwaves.length < MAX_SHOCKWAVES) window.shockwaves.push({x: clashX, y: clashY, r: 10, maxR: 300, color: "#f1c40f", alpha: 1, speed: 25});
        window.spawnParticles(clashX, clashY, "#00f3ff", true);
        window.shakeScreen(3, 8);
        
        clashCount++;
    }, 50);
};

// 3. TRẢM SÁT CHẬM (IAIJUTSU / DEMON SLAYER)
window.executeIaijutsu = function(caster, target) {
    window.speedLinesAlpha = 1.0;
    window.playSound(800, 'sine', 0.8, 0.5, false);
    
    caster.state = 'dash';
    caster.x = target.x + (caster.isFacingRight ? 150 : -150); 
    window.spawnDust(caster.x, caster.y);
    
    target.stunTimer = 90; target.state = 'stunned';
    window.targetTimeScale = 0.05; 
    window.applyFilter('grayscale', 60); 
    
    setTimeout(() => {
        window.targetTimeScale = 1.0;
        window.playSound(400, 'triangle', 0.2, 1.0, true); 
        
        for(let i=0; i<5; i++) {
            setTimeout(() => {
                window.spawnSlash(target.x + (Math.random()-0.5)*50, target.y - 50 + (Math.random()-0.5)*50, Math.random()>0.5, "#ff003c", true, 2.5, Math.random()*Math.PI);
                if (typeof window.takeDamage === 'function') window.takeDamage(target, 25 * caster.currentDmgMod, "#ff003c", true, false, caster.isFacingRight);
            }, i * 100);
        }
...
// 4. GỒNG AURA PHẢN TRỌNG LỰC
window.spawnAntiGravityDebris = function(x, y, radius, color) {
    if (window.particles.length > MAX_PARTICLES) return;
    if(Math.random() < 0.1) window.spawnEnvDamage(x + (Math.random()-0.5)*radius, window.GROUND_Y, 'crater', 0.5, false);

    for(let i=0; i<3; i++) {
        window.particles.push({ 
            x: x + (Math.random() - 0.5) * radius, 
            y: window.GROUND_Y + 10, 
            vx: (Math.random() - 0.5) * 1, 
            vy: -Math.random() * 2 - 0.5, 
            life: 80 + Math.random()*40, maxLife: 120, 
            color: color || "#57606f", 
            size: Math.random() * 8 + 3, 
            isAntiGravityRubble: true 
        });
    }
};

// 5. CHỚP SÁNG CON MẮT CHÍ MẠNG (EYE GLINT / NEWTYPE FLASH)
window.triggerEyeGlint = function(fighter, color) {
    window.targetZoom = 1.8;
    window.actionCamOffsetX = fighter.isFacingRight ? 50 : -50;
    window.targetTimeScale = 0.05;
    window.playSound(900, 'sine', 0.5, 1.0, true);
    window.lethalVoid = 15;
    
    window.spawnLaser(
        fighter.x + (fighter.isFacingRight ? 20 : -20), 
        fighter.y - 70, 
        fighter.isFacingRight, 
        color || "#ff003c", 
        3, 
        20
    );
...
window.update = function() {
    ...
    if (!isTimeStopped) {
        ...
        for (let i = window.particles.length - 1; i >= 0; i--) { 
            ...
            else if (pt.isAntiGravityRubble) { pt.vy -= 0.05; pt.vx += (Math.random()-0.5)*0.2; pt.size *= 0.99; }
    ...
    allFighters.forEach(f => {
        ...
        if (f.isRage) { 
            ...
            if (Math.random() < 0.05) window.spawnAntiGravityDebris(f.x, f.y, 80, f.isPlayer ? "#ff4757" : "#9b59b6");
        }
                ...
                if (targetFighter && targetFighter.hp > 0) {
                    ...
                    if (f.aiDelay <= 0) {
                        ...
                        if (absDist > 250 && Math.random() < 0.6) { f.state = 'cast'; f.attackTimer = 25; window.playSound(300, 'sine', 0.3, 0.6); window.spawnSlash(f.x + (f.isFacingRight? 50:-50), f.y - 40, f.isFacingRight, "#fff", true, 2.0, Math.PI/2); window.spawnProjectile(f.x, f.y - 40, f.isFacingRight ? 12 : -12, 0, 15, "#fff", Math.floor(25 * f.dmgMod), window.p1); window.floatingTexts.push({ x: f.x, y: f.y - 80, text: "🗡️", color: "#fff", alpha: 1.5, vx: 0, vy: -4, font: "900 40px Arial", life: 40, scale: 2.0, targetScale: 1.0, scaleVel: 0, rot: 0 }); window.targetCamOrbitAngle = f.isFacingRight ? 0.2 : -0.2; } 
                            ...
                            if(Math.random() < 0.4) { window.executeIaijutsu(f, window.p1); }
                            else {
                                f.state = 'dash'; f.dashTimer = 15; f.dashDir = Math.sign(dist); f.currentSpeed *= 2.5; f.iFrames = 20; window.playSound(400, 'sawtooth', 0.4, 0.8, true); window.targetCamOrbitAngle = f.isFacingRight ? 0.3 : -0.3; setTimeout(() => { if(window.gameOver || f.hp <= 0 || !window.p1) return; if(Math.abs(window.p1.x - f.x) < 180) { if(typeof window.takeDamage === 'function') window.takeDamage(window.p1, Math.floor(40 * f.dmgMod), "#e74c3c", true, false); window.shakeScreen(20, 15); window.p1.vx = Math.sign(dist) * 6; window.targetCamOrbitAngle = f.isFacingRight ? -0.2 : 0.2; } window.spawnSlash(f.x, window.p1.y - 30, f.isFacingRight, "#e74c3c", true, 3.0, (Math.random()-0.5)); window.floatingTexts.push({ x: f.x, y: f.y - 80, text: "⚡", color: "#e74c3c", alpha: 1.5, vx: 0, vy: -5, font: "900 40px Arial", life: 40, scale: 2.5, targetScale: 1.0, scaleVel: 0, rot: 0 }); }, 200); 
                            }
                        }
            ...
            for (let j = i + 1; j < allFighters.length; j++) { 
                ...
                if (dist < 80 && f1.attackTimer > 5 && f1.attackTimer < 20 && f2.attackTimer > 5 && f2.attackTimer < 20) {
                    if (!f1.clashCooldown && !f2.clashCooldown && f1.isFacingRight !== f2.isFacingRight) {
                        ...
                        // TY LỆ 25% KÍCH HOẠT VA CHẠM KHÔNG TRUNG (HIGH-SPEED AIR CLASH)
                        if(Math.random() < 0.25) {
                            window.triggerAirClash(f1, f2, 60);
                        } else {
...
window.draw = function() {
    ...
    if (window.invertFrames > 0) {
        ...
        sortedFighters.forEach(p => { 
            if (p && p.hp > 0 && !p.isInAirClash && typeof window.drawStickman === 'function') { 
    ...
    // MANGA SUNBURST IMPACT FRAMES
    if (window.impactFrameCount > 0) {
        window.ctx.save(); 
        let isBlackFrame = window.impactFrameCount % 2 === 0;
        window.ctx.fillStyle = isBlackFrame ? "#000000" : "#ffffff"; 
        window.ctx.fillRect(0, 0, window.canvas.width, window.canvas.height); 
        
        let cx = window.canvas.width/2;
        let cy = window.canvas.height/2;
        window.ctx.fillStyle = isBlackFrame ? "#ffffff" : "#000000";
        window.ctx.beginPath();
        let rays = 24;
        for(let i=0; i<rays; i++) {
            let angle1 = (Math.PI * 2 / rays) * i;
            let angle2 = (Math.PI * 2 / rays) * (i + 0.25);
            window.ctx.moveTo(cx, cy);
            window.ctx.lineTo(cx + Math.cos(angle1)*2000, cy + Math.sin(angle1)*2000);
            window.ctx.lineTo(cx + Math.cos(angle2)*2000, cy + Math.sin(angle2)*2000);
        }
        window.ctx.fill();

        sortedFighters.forEach(p => { 
            if (p && p.hp > 0 && !p.isInAirClash) { 
                ...
                let sketchColor = isBlackFrame ? "#ff003c" : "#000000";
    ...
    try {
        ...
        if (window.p1) {
            ...
            sortedFighters.forEach(p => { 
                if (p && p.hp > 0 && !p.isInAirClash && p.trailArr) { 
