...
// Tối ưu Layout: Game -> Red Banner -> HUD -> Live Chat -> GREEN SCREEN CAT AVATAR
// ==========================================
...
// ====================================================================
// 🎯 KHU VỰC TÙY CHỈNH: VIDEO MÈO NỀN XANH, NGƯỜI NỔI TIẾNG & BÌNH LUẬN 
// ====================================================================

// Thay bằng TÊN FILE VIDEO MÈO CỦA BẠN (Cần để file video cùng thư mục với file web)
...
// ====================================================================
// HỆ THỐNG VẼ VIDEO MÈO (XÓA PHÔNG XANH TRỰC TIẾP) & AUDIO VISUALIZER
// ====================================================================
function drawVTuberCommentator(ctx, x, y, audioPeak) {
    // Khởi tạo Video và Canvas ẩn để lọc phông xanh
    if (!window.catVideoObj) {
        ...
        window.catVideoObj.muted = true; // Tắt tiếng video mèo để không đè vào tiếng game
        window.catVideoObj.crossOrigin = "anonymous";
        window.catVideoObj.play().catch(e => console.log("Trình duyệt chặn Auto-play Video Mèo", e));
    ...
    // Cắt khung hình tròn để render Mèo vào trong
    ctx.clip();
    
    // XỬ LÝ CHROMA KEY (TÁCH NỀN XANH) THỜI GIAN THỰC
    if (window.catVideoObj.readyState >= 2) { // Kích thước video đã sẵn sàng
        let vCanvas = window.catChromaCanvas;
        let vCtx = window.catChromaCtx;
        
        // Vẽ frame video mèo vào canvas ẩn
        ...
        // Quét từng điểm ảnh để xóa nền xanh lá (Green Screen)
        for (let i = 0; i < l; i++) {
            let r = frameData.data[i * 4 + 0];
            let g = frameData.data[i * 4 + 1];
            let b = frameData.data[i * 4 + 2];
            
            // Công thức phát hiện màu xanh lá
            if (g > 80 && g > r * 1.2 && g > b * 1.2) {
                frameData.data[i * 4 + 3] = 0; // Alpha = 0 (Làm trong suốt)
            }
        }
        
        // Cập nhật lại hình ảnh đã xóa phông
        vCtx.putImageData(frameData, 0, 0);
        
        // Vẽ canvas con mèo đã trong suốt vào màn hình chính
        ctx.drawImage(vCanvas, -radius, -radius, radius * 2, radius * 2);
    } else {
        // Fallback Chữ nếu Video lỗi tải
        ctx.shadowBlur = 0;
...
window.captureFrames = function() {
    ...
    // Kiểm tra xem đã kết thúc game và vào Outro chưa
    ...
    if (!isOutroActive) {
        ...
        if (!window._chatSystemInit) {
            ...
            // Đổ sẵn 6 bình luận lúc mới tải
            for(let i = 0; i < 6; i++) {
        ...
        if (chatNow - window._lastChatUpdate > window._nextChatDelay) {
            ...
            window._liveChats.push({ name: randomCeleb.name, color: randomCeleb.color, msg: randomMsg });
            if (window._liveChats.length > 6) window._liveChats.shift();
        }
        ...
        // --- MÈO GREEN SCREEN (Dưới bảng chat) ---
        ...
        // LÚC NÀY GAME SẼ ẨN HOÀN TOÀN KHUNG CHAT VÀ AVATAR MÈO
        // ==========================================
