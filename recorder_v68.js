// ==========================================
// RECORDER.JS - BẢN HOÀN HẢO CHO CONTENT CREATOR (V66.17 - VTUBER REACTION META)
// [NEW] Robot AI Bình luận viên (Cyber-Caster): Bay lơ lửng ở giữa màn hình dọc.
// [NEW] Auto Lip-Sync: Miệng Robot tự động nhép/há hốc dựa theo nhịp Audio và Giọng đọc.
// [NEW] Hiệu ứng chớp mắt tự nhiên và gắn thẻ "🔴 LIVE" giả lập Livestream.
// [RETAINED] Chuẩn RAW 250Mbps (Max Smooth 60FPS), Lưới Neon 3D, RGB Glitch.
// ==========================================
...
// ==========================================
// HỆ THỐNG VẼ BÌNH LUẬN VIÊN ẢO (CYBER-CASTER)
// ==========================================
function drawVTuberCommentator(ctx, x, y, audioPeak, isTyping) {
    let t = Date.now();
    ctx.save();
    
    // Quỹ đạo lơ lửng (Hovering)
    let floatY = Math.sin(t / 250) * 15;
    let tilt = Math.sin(t / 600) * 0.05;
    ctx.translate(x, y + floatY);
    ctx.rotate(tilt);

    // Vỏ TV (Mặt Robot)
    ctx.fillStyle = "#1e293b"; // Màu xám đen
    ctx.strokeStyle = "#00f3ff"; // Viền Neon Xanh
    ctx.lineWidth = 10;
    ctx.shadowColor = "#00f3ff";
    ctx.shadowBlur = 25;
    ctx.beginPath();
    if(ctx.roundRect) ctx.roundRect(-160, -130, 320, 260, 45); 
    else ctx.rect(-160, -130, 320, 260);
    ctx.fill(); ctx.stroke();

    // Màn hình hiển thị
    ctx.shadowBlur = 0;
    ctx.fillStyle = "#020617"; // Đen thẳm
    ctx.beginPath();
    if(ctx.roundRect) ctx.roundRect(-135, -105, 270, 210, 25);
    else ctx.rect(-135, -105, 270, 210);
    ctx.fill();

    // Nhấp nháy chớp mắt (Blinking Logic)
    let isBlinking = (t % 3500 < 150);
    
    // Mắt Robot
    ctx.fillStyle = audioPeak > 0.6 ? "#ff0055" : "#ffeb3b"; // Đổi màu mắt đỏ khi gắt
    ctx.shadowColor = ctx.fillStyle;
    ctx.shadowBlur = 15;
    
    if (isBlinking) {
        // Mắt nhắm
        ctx.fillRect(-85, -45, 55, 10);
        ctx.fillRect(30, -45, 55, 10);
    } else {
        // Mắt mở (Hình elip xếch góc ngầu)
        ctx.save(); ctx.translate(-60, -50); ctx.rotate(0.3);
    ...
    // CÁI MIỆNG (LIP-SYNC TỰ ĐỘNG BẰNG AUDIO PEAK)
    // Nếu đang nói hoặc có âm thanh to, miệng há rộng ra
    let mouthHeight = 10 + (audioPeak * 80); 
    if (!isTyping && audioPeak < 0.2) mouthHeight = 10; // Ngậm miệng nếu đang im

    ctx.fillStyle = "#ff0055";
    ctx.shadowColor = "#ff0055";
    ctx.beginPath();
    if(ctx.roundRect) ctx.roundRect(-50, 30, 100, mouthHeight, 15);
    else ctx.fillRect(-50, 30, 100, mouthHeight);
    ctx.fill();

    // Cái Ăng-ten (Tai nghe)
    ctx.shadowBlur = 0;
    ctx.fillStyle = "#ffeb3b";
    ctx.fillRect(160, -30, 20, 60);
    ctx.beginPath(); ctx.arc(190, 0, 15, 0, Math.PI*2); ctx.fill();

    // Huy hiệu "🔴 LIVE"
    ctx.rotate(-tilt); // Dựng thẳng lại chữ
    ctx.fillStyle = "#ff0000";
    ctx.beginPath(); if(ctx.roundRect) ctx.roundRect(-240, -170, 110, 45, 10); else ctx.fillRect(-240, -170, 110, 45); ctx.fill();
    ctx.fillStyle = "#fff"; ctx.font = "900 24px 'Arial Black'"; ctx.textAlign = "center"; ctx.textBaseline = "middle";
    ctx.fillText("🔴 LIVE", -185, -147);
...
window.startRecording = function() {
    ...
    let options = { videoBitsPerSecond: 250000000 }; // Ép xả Raw 250 Mbps cực nét
    window.currentVideoExt = "webm";
...
window.stopRecording = function() { 
    ...
    if (window.recordCtxV) { window.recordCtxV.fillStyle = "#ffffff"; window.recordCtxV.fillRect(0,0,1080,1920); }
    if (window.recordCtxH) { window.recordCtxH.fillStyle = "#ffffff"; window.recordCtxH.fillRect(0,0,1920,1080); }
...
window.captureFrames = function() {
    ...
    try {
        ctxH.fillStyle = "#000000"; ctxH.fillRect(0,0,2560,1440);
        ctxV.fillStyle = "#000000"; ctxV.fillRect(0,0,1440,2560);
        ...
        // ==========================================
        // VẼ VTUBER BÌNH LUẬN VIÊN ẢO TẠI ĐÂY (TRUNG TÂM RETENTION ZONE)
        // ==========================================
        if (!window.gameOver && window.introTimer > 120) {
            // Tính toán vị trí đẹp: Chính giữa dọc, khoảng trống giữa thanh máu và Sub
            drawVTuberCommentator(ctxV, 720, splitGameHeight + 430, audioPeak, window.StoryModeAI.isTyping);
        }
        ...
        if (window.p1 && !window.gameOver && window.introTimer <= 120) {
            ...
            // --- HUD DỌC 1080P ---
            let hudBaseY = splitGameHeight + 60; 
