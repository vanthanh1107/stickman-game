...
// [BẢN CẬP NHẬT: AUTO-CAST SKILLS & SIÊU API GAME CONTEXT]
// ==========================================
...
window.update = function() {
    ...
    if (!window.gameOver && !isTimeStopped) {
        ...
        // Thiên thạch ngẫu nhiên
        if (Math.random() < hazardChance && window.projectiles.length < 10) { 
            window.projectiles.push({ x: Math.random() * window.canvas.width, y: -100, vx: (Math.random() - 0.5) * 4, vy: 8 + Math.random() * 6, radius: 12 + Math.random() * 8, color: "#e67e22", dmg: 45, target: null, isMeteor: true }); 
        }
        
        // Sét đánh ngẫu nhiên 
        if (Math.random() < hazardChance) { 
            window.envHazards.push({ type: 'lightning', x: Math.random() * window.canvas.width, timer: 45, state: 'warning' }); 
        }

        // Lava cho map tro tàn
                    ...
                    if (window.particles.length < MAX_PARTICLES) { 
                        for(let p=0; p<20; p++) { 
                            window.particles.push({ 
                                x: proj.x + (Math.random()-0.5)*50, 
                                y: window.GROUND_Y, 
                                vx: (Math.random()-0.5)*8, 
                                vy: -Math.random()*8 - 6, 
                                life: 45 + Math.random()*20, 
                                maxLife: 65, 
                                color: Math.random() > 0.4 ? "#f1c40f" : "#ff4757", 
    ...
    // ==========================================
    // ⭐ SIÊU API GAME CONTEXT: HỖ TRỢ FULL TÍNH NĂNG CHO NHÂN VẬT ⭐
    // ==========================================
    let gameContext = { 
        // 1. Vũ khí & Sát thương
        takeDamage: window.takeDamage, 
        spawnProjectile: window.spawnProjectile, 
        spawnTrap: window.spawnTrap,
        spawnSlash: window.spawnSlash,
        spawnLaser: window.spawnLaser,
        
        // 2. Hiệu ứng Hình ảnh (VFX) & UI
        spawnParticles: window.spawnParticles, 
        floatingTexts: window.floatingTexts,
        spawnLightning: window.spawnLightning,
        spawnBlackHole: window.spawnBlackHole,
        triggerCinematic: window.triggerCinematic,
        spawnAura: window.spawnAura,
        spawnSpaceRipple: window.spawnSpaceRipple,
        spawnMangaSFX: window.spawnMangaSFX,
        spawnEnergyPillar: window.spawnEnergyPillar,
        spawnInk: window.spawnInk,
        
        // 3. Thao tác Môi trường & Thời gian
        shakeScreen: window.shakeScreen, 
        playSound: window.playSound,
        triggerTimeStop: window.triggerTimeStop,
        applyFilter: window.applyFilter,
        
        // 4. Di chuyển & Buff Đặc biệt
        dash: (f, fx, fy) => { 
            f.vx = fx; if(fy) f.vy = fy; 
            f.state = 'dash'; f.attackTimer = 15; f.iFrames = 10; 
            window.spawnParticles(f.x, f.y, "#bdc3c7"); 
        }, 
        teleport: (f, dx, dy) => { 
            window.spawnParticles(f.x, f.y, "#8e44ad"); 
            f.x = dx; if(dy) f.y = dy; 
            f.state = 'cast'; f.attackTimer = 10; 
            window.spawnParticles(f.x, f.y, "#8e44ad"); 
        }, 
        addBuff: (f, st, v, fr) => { 
            f.buffs.push({stat: st, value: v, life: fr, maxLife: fr}); 
        }, 
        setInvulnerable: (f, fr) => { f.iFrames = fr; },
        
        // 5. Điều khiển Camera Tự do
        camera: {
            setZoom: (z) => { window.targetZoom = z; },
            setTilt: (t) => { window.targetTilt = t; },
            setOrbitAngle: (a) => { window.targetCamOrbitAngle = a; },
            shakeUi: (p1, p2) => { window.uiShakeP1 = p1; window.uiShakeP2 = p2; }
        },
    ...
    allFighters.forEach(f => {
        ...
        // XỬ LÝ ULTIMATE (Tương thích ngược)
            ...
            if (f.hitStun <= 0 && f.stunTimer <= 0 && f.state !== 'dash_back' && f.state !== 'block') {
                ...
                if (isCloseEnough) {
                    if (window.classStats && window.classStats[type] && typeof window.classStats[type].executeUltimate === 'function') {
                        f.stamina = 0; let baseDmg = 50 * (f.currentDmgMod || 1); if (!f.isPlayer) baseDmg = 35 * (f.currentDmgMod || 1); window.playSound(400, 'sine', 0.5, 0.6); window.shakeScreen(15, 10); window.spawnParticles(f.x, f.y, "#f1c40f", true); window.floatingTexts.push({ x: f.x, y: f.y - 100, text: f.isPlayer ? "🔥 ULTIMATE!" : "⚠️ DANGER!", color: f.isPlayer ? "#ff4757" : "#ff0000", alpha: 1.5, vx: 0, vy: -6, font: "italic 900 45px 'Arial Black', Impact", life: 50, scale: 3.0, targetScale: 1.0, scaleVel: 0, rot: 0 }); f.isFacingRight = distToTarget > 0; 
                        
                        // Cập nhật: Truyền gameContext vào executeUltimate
                        window.classStats[type].executeUltimate(f, closestTarget, baseDmg, gameContext); 
                        
                        f.vx = 0; window.speedLinesAlpha = 1.0; 
        ...
        // ==========================================
        // ⭐ VÒNG LẶP SKILL ĐỘNG VÀ AI (THAY THẾ CODE CŨ) ⭐
        // ==========================================
        if (!launchedUltimate && f.attackTimer <= 0 && f.hitStun <= 0 && f.dashTimer <= 0 && f.stunTimer <= 0 && !window.gameOver && f.hp > 0) {
            ...
            if (f.aiDelay <= 0) {
                ...
                if (closestTarget && closestTarget.hp > 0) {
                    let dist = closestTarget.x - f.x; 
                    f.isFacingRight = dist > 0; 
                    let absDist = Math.abs(dist);
                    
                    // 1. Trừ Cooldown của tất cả kỹ năng
                    if (f.skills && Array.isArray(f.skills)) {
                        f.skills.forEach(s => { if (s.currentCooldown > 0) s.currentCooldown--; });
                        
                        // 2. Xét duyệt từng kỹ năng
                        for (let i = 0; i < f.skills.length; i++) {
                            let skill = f.skills[i];
                            if (skill.currentCooldown > 0) continue;
                            if (skill.staminaCost && f.stamina < skill.staminaCost) continue;
                            
                            let minR = skill.minRange || 0;
                            let maxR = skill.maxRange || 9999;
                            
                            if (absDist >= minR && absDist <= maxR) {
                                // Kiểm tra điều kiện phụ (nếu có)
                                if (skill.condition && !skill.condition(f, closestTarget, gameContext)) continue;

                                // Tung chiêu!
                                if (skill.staminaCost) f.stamina -= skill.staminaCost;
                                skill.currentCooldown = skill.cooldown || 60; 
                                
                                try {
                                    skill.execute(f, closestTarget, gameContext);
                                } catch (e) { console.error("Lỗi khi chạy skill:", e); }
                                
                                skillCasted = true;
                                break; // Chỉ tung 1 skill mỗi lượt suy nghĩ
                            }
                        }
                    }
                    
                    // 3. Hành vi mặc định nếu không có kỹ năng nào xài được
                    if (!skillCasted) {
                        let reach = 65 * Math.max(f.scale||1, closestTarget.scale||1);
                        if (absDist > reach) { 
                            f.vx = Math.sign(dist) * f.currentSpeed; f.state = 'walk'; 
                            if (Math.random() < 0.1 && f.onGround) window.spawnDust(f.x, f.y); 
                        } else {
                            f.vx = 0; if (f.state === 'walk') f.state = 'idle';
                            let rand = Math.random(); 
                            if (closestTarget.attackTimer > 0 || closestTarget.state === 'dash') { 
                                if (rand < 0.4) { 
                                    f.dashTimer = 10; f.dashDir = -Math.sign(dist); f.state = 'dash_back'; f.iFrames = 10; f.attackTimer = 10; window.spawnDust(f.x, f.y); window.targetCamOrbitAngle = f.isFacingRight ? -0.1 : 0.1; 
                                } else { 
                                    if(typeof window.attack === 'function') window.attack(f, targetGroup); 
                                } 
                            } else { 
                                if (rand < 0.9) { 
                                    if (f.comboTimer > 0 && f.comboStep < 14) f.comboStep++; else f.comboStep = 0; f.comboTimer = 50; if(typeof window.attack === 'function') window.attack(f, targetGroup); 
                                } else { 
                                    f.vx = -Math.sign(dist) * f.currentSpeed * 1.5; f.state = 'walk'; 
                                } 
                            }
                        }
                    }
    ...
    // --- ĐIỀU CHỈNH CAMERA CHUYÊN NGHIỆP CUỐI TRẬN ---
