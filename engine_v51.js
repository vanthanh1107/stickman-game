// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 40.0 [THE DIMENSIONAL BREAKER]
// [NEW] STAGE BREAK: Đánh kẻ địch vào tường bằng đòn mạnh sẽ vỡ màn hình và đổi Map/Thời tiết tức thời!
// [NEW] FLOOR-BOUNCE SPARKS: Tia lửa văng ra sẽ đập xuống mặt kính và nảy lên chân thực.
// [NEW] PHANTOM DASH: Lướt tàng hình biến thành luồng ánh sáng.
// [NEW] ADRENALINE VIGNETTE: Viền màn hình co bóp theo nhịp tim khi yếu máu.
// [RESTORED & VERIFIED] Giữ nguyên 100% tất cả code cũ (360 Orbit, Meme Thumbnail, Witch Time, Kinetic Text).
// ==========================================
...
window.triggerDimensionShatter = function(x, y) {
    ...
    for(let i = 0; i < numMainCracks; i++) {
        ...
        while(dist < len) {
            ...
            dist += step; currentAng += (Math.random() - 0.5) * 0.9; 
            ...
            if (Math.random() < 0.45) {
                ...
                while(sDist < subLen) {
                    ...
                    sDist += sStep; subAng += (Math.random() - 0.5) * 0.6;
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce, attackerDirRight = true) {
    ...
    if (finalDmg > 0) {
        ...
        // [VIRAL 40.0] STAGE BREAK KHI ĐẬP TƯỜNG BẰNG ĐÒN CHÍ MẠNG
        ...
        if (wallBounce || (isNearWallNow && isCrit)) { 
            ...
            if (isCrit) {
                ...
                window.targetCamOrbitAngle = Math.PI; // Quay hẳn 180 độ
                window.triggerStageTransitionEffect(); // ĐỔI MAP!
            }
        } 
        ...
        } else if (isCrit) { window.impactFrameTimer = 2; window.hitStopFrames = 6; window.shakeScreen(12, 10); window.chromaTimer = 12; window.playSound(180, 'square', 0.3, 0.6, true); } 
...
window.attack = function(attacker, targetGroup) {
    ...
    if (MathDist > reach && !isFinisher) { 
        ...
        // [VIRAL 40.0] PHANTOM DASH TÀNG HÌNH
        attacker.iFrames = 8;
        window.spawnParticles(attacker.x, attacker.y, "#bdc3c7", true);
        return; 
    }
...
window.update = function() {
    ...
    if (!isTimeStopped) {
        ...
        // [VIRAL 40.0] HẠT TIA LỬA NẢY MẶT SÀN (FLOOR BOUNCE PHYSICS)
        ...
        for (let i = window.particles.length - 1; i >= 0; i--) { 
            ...
            else { pt.vy += window.GRAVITY * 0.9; if(pt.y >= window.GROUND_Y) { pt.y = window.GROUND_Y; pt.vy *= -0.3; pt.vx *= 0.7; } } // Nảy nhẹ cho hạt bụi/lửa
    ...
    allFighters.forEach(f => {
            ...
            if (f.hitStun <= 0 && f.stunTimer <= 0 && f.state !== 'dash_back' && f.state !== 'block') {
                ...
                if (isCloseEnough) {
                    if (window.classStats && window.classStats[type] && typeof window.classStats[type].executeUltimate === 'function') {
                        ...
                        window.spawnEyeFlash(f.x, f.y - 60, f.isPlayer ? "#ff4757" : "#ff0000", f.isFacingRight); 
        ...
        if (f.state === 'wall_splat') { f.vy = 0; f.vx = 0; if (f.hitStun < 25) { f.state = 'hurt'; f.vy = 2; f.vx = f.x < window.canvas.width/2 ? 3 : -3; } } 
            ...
            if (f.vy > 0 && f.y + f.vy >= window.GROUND_Y && !f.onGround) { 
                ...
                if (f.state === 'juggle' && f.vy > 10 && f.hp > 0) {
                    f.vy = -f.vy * 0.5; f.y = window.GROUND_Y - 1; window.playSound(200, 'square', 0.2, 0.5, true);
                    window.floatingTexts.push({ x: f.x, y: f.y - 80, text: "💥 BOUNCE!", color: "#f1c40f", alpha: 1.5, vx: 0, vy: -3, font: "italic 900 30px 'Arial Black', Impact", life: 40, scale: 2.0, targetScale: 1.0, scaleVel: 0, rot: 0 });
                }
            } 
        }
        ...
        if (f.y >= window.GROUND_Y) { 
            f.y = window.GROUND_Y; 
            if (f.state !== 'juggle') { f.vy = 0; f.onGround = true; f.state = (f.hp <= 0) ? 'dead' : 'idle'; }
    ...
    if (window.gameOver && closest) {
        ...
        window.targetZoom = Math.max(0.9, Math.min(1.25 + comboZoomBonus, dynamicZoom + comboZoomBonus));
        
        let p1Low = window.p1.hp < window.p1.maxHp * 0.3; let eLow = closest.hp < closest.maxHp * 0.3;
        if (p1Low && eLow) { window.targetTilt = 0.05 * Math.sin(window.matchTimer * 0.06) + comboTiltBonus; } 
        else if (p1Low || eLow) { window.targetTilt = 0.025 * Math.sin(window.matchTimer * 0.04) + comboTiltBonus; } 
        else { window.targetTilt = comboTiltBonus; }
        
        // [VIRAL 40.0] ADRENALINE VIGNETTE PULSE KHI YẾU MÁU
...
window.draw = function() {
    ...
    if (window.invertFrames > 0) {
        ...
        sortedFighters.forEach(p => { 
            if (p && p.hp > 0 && typeof window.drawStickman === 'function') { 
                // [VIRAL 40.0] ẨN NHÂN VẬT NẾU ĐANG PHANTOM DASH
    ...
    if (window.impactFrameCount > 0) {
        ...
        sortedFighters.forEach(p => { 
            if (p && p.hp > 0) { 
                if (p.iFrames > 20 && (p.state === 'dash' || p.state === 'dash_back')) return; // Phantom Dash
    ...
    if (window.isLoading) {
        ...
        window.ctx.fillStyle = "#fff"; window.ctx.font = "bold 20px monospace"; window.ctx.fillText(`DIMENSIONAL BREAKER... ${Math.floor(window.loadProgress)}%`, cx, window.canvas.height - 20); 
        return; 
    }
    ...
    try {
        ...
        sortedFighters.forEach(p => { 
            if (p && p.hp > 0 && typeof window.drawStickman === 'function') { 
                if (p.iFrames > 20 && (p.state === 'dash' || p.state === 'dash_back')) return; // Ẩn hình phản chiếu khi phantom dash
                window.ctx.save(); window.ctx.translate(p.drawX, p.drawY); if (!p.drawFacingRight) window.ctx.scale(-1, 1); 
                let clone = Object.assign({}, p, { x: 0, y: 0, scale: p.drawScale }); 
                if(clone.isDragon && typeof window.drawDragon === 'function') window.drawDragon(window.ctx, clone); else window.drawStickman(window.ctx, clone); window.ctx.restore(); 
            } 
        });
        window.ctx.restore();
        ...
        // [VIRAL 40.0] NHỊP TIM ADRENALINE VIGNETTE KHI YẾU MÁU
        ...
        window.ctx.save(); window.ctx.globalCompositeOperation = 'multiply';
        window.screenInks.forEach(ink => { window.ctx.globalAlpha = Math.max(0, ink.life / ink.maxLife) * 0.8; window.ctx.fillStyle = ink.color; window.ctx.beginPath(); window.ctx.arc(ink.x, ink.y + ink.slideY, 30 * ink.scale, 0, Math.PI*2); window.ctx.arc(ink.x - 20, ink.y - 40 + ink.slideY, 15 * ink.scale, 0, Math.PI*2); window.ctx.arc(ink.x + 30, ink.y - 20 + ink.slideY, 10 * ink.scale, 0, Math.PI*2); window.ctx.fill(); window.ctx.fillRect(ink.x - 5*ink.scale, ink.y + ink.slideY, 10*ink.scale, ink.slideY * 2); });
        window.ctx.restore();
