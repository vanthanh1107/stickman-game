// ==========================================
// RECORDER.JS - BẢN HOÀN HẢO CHO CONTENT CREATOR (V66.16 - PURE WEBM MAX BITRATE)
// [PERFECT FIX] Khóa cứng xuất file WebM (VP8 Codec) - Con ruột trình duyệt, độ mượt vô địch!
// [ULTIMATE FPS] Bitrate 250 Mbps: Xả RAW dữ liệu, cấm trình duyệt nén file để cứu CPU.
// [ULTIMATE FPS] Khóa đồng bộ 60 FPS, 'desynchronized: true' GPU Bypass (0 Lag).
// [RETAINED] Mọi Meta TikTok: AI Word Highlight, RGB Glitch, Follow Bouncer, Profile Grid.
// ==========================================
...
window.currentVideoExt = "webm"; // CHUẨN WEBM ĐỂ ĐẠT ĐỘ MƯỢT TUYỆT ĐỐI
window.savedVideos = [];
window.filmDustY = 0; 
...
window.startRecording = function() {
    ...
    // Thu hình chuẩn 60FPS để khớp chính xác với FPS của game, không bị đúp khung hình
    ...
    // [KHÓA TỬ HUYỆT ĐỊNH DẠNG] Ép xuất WEBM VP8 + Bitrate cực đại 250 Mbps
    let options = { videoBitsPerSecond: 250000000 }; 
    window.currentVideoExt = "webm";

    // VP8 được tối ưu hóa cho web siêu nhẹ, không gây trễ.
    ...
    try {
        ...
        if (window.p1 && !window.gameOver && window.introTimer <= 120) {
            ...
            if (repEnemyObj) {
                ...
                drawSkewedPath(ctxV, 747, hudBaseY + 73, 507, 53, false); ctxV.fillStyle = "rgba(0,0,0,0.7)"; ctxV.fill(); ctxV.lineWidth = 7; ctxstrokeStyle = "rgba(255,255,255,0.9)"; ctxV.stroke();
