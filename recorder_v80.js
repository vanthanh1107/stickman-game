// ==========================================
// RECORDER.JS - V67.7 FINAL PERFECTION (ANTI-CRASH & TIMELINE FIX)
// Sửa lỗi kẹt vòng lặp làm mất bảng Download.
// Tự động ép chuẩn WebM Duration, có cơ chế Bypass nếu mạng lỗi.
// ==========================================

// Tự động tải thư viện sửa lỗi Timeline và Duration cho Video
if (!window._webmFixerLoading) {
    window._webmFixerLoading = true;
    window._webmFixerFailed = false;
    let script = document.createElement('script');
    script.src = 'https://unpkg.com/fix-webm-duration';
...
// Biến tính toán thời lượng chuẩn xác
...
window.startRecording = function() {
    ...
    // [V67.7 FIX TIMELINE] TUYỆT ĐỐI CHỈ DÙNG WEBM ĐỂ CHỐNG LỖI TIMELINE CAPCUT
    window.currentVideoExt = "webm"; 
    ...
    const finalizeRecordings = () => {
        ...
        if (stoppedCount === 2) { 
            setTimeout(() => {
                ...
                // [V67.7 ANTI-CRASH] Thuật toán bơm Metadata có cơ chế thoát hiểm an toàn
                let processVideo = (chunks, mimeType) => {
                    return new Promise((resolve) => {
                        ...
                        let attempts = 0;
                        let checkAndFix = () => {
                            let fixerFunc = window.ysFixWebm || window.fixWebmDuration;
                            
                            if (typeof fixerFunc === 'function') {
                                try {
                                    fixerFunc(blob, durationMs, function(fixedBlob) { 
                                        resolve(fixedBlob || blob); 
                                    });
                                } catch (e) {
                                    console.error("Lỗi khi Fix WebM:", e);
                                    resolve(blob);
                                }
                            } else if (window._webmFixerFailed || attempts > 10) { 
                                // NẾU QUÁ 5 GIÂY MÀ THƯ VIỆN KO TẢI ĐƯỢC -> BYPASS ĐỂ HIỆN NÚT DOWNLOAD
                                console.warn("Bypass WebM fix (Lỗi mạng hoặc bị chặn)");
                                resolve(blob); 
                            } else {
                                attempts++;
