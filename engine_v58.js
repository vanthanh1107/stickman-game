// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 38.4 [TRUE COMBAT & OPTIMIZED EDITION]
// [RESTORED] Giữ nguyên 100% UI Emoji, Text Manga, Thumbnail, Anti-Glare và Không gian Râu Trắng của bản gốc.
// [OPTIMIZED] Sửa triệt để lỗi Crash khi dùng đòn đánh liên hoàn. Tối ưu FPS, chống giật lag Canvas.
// [NEW COMBAT] Thêm hệ thống Hất tung (Juggle), Đập đất nảy (Ground Bounce), Đòn liên hoàn an toàn (Safe Flurry), và Chuỗi Combo.
// ==========================================
...
// [TỐI ƯU HÓA] Giảm nhẹ số lượng hạt rác để bảo vệ FPS
...
// ==========================================
// 4. HỆ THỐNG VẬT LÝ NHẬN SÁT THƯƠNG & ATTACK (CẬP NHẬT COMBO)
// ==========================================
window.takeDamage = function(target, amount, color, isCrit, wallBounce, attackerDirRight = true) {
    ...
    if (finalDmg > 0) {
        ...
        if (isCrit) {
            window.screenFlash = 0.15; // Giữ Anti-Glare
        ...
        if (target.hp <= 0) {
            ...
            if (isNearWall && isCrit && typeof window.triggerStageTransition === 'function') { window.triggerStageTransition(target); } 
                ...
                if (isCrit) {
                    ...
                    window.targetZoom = 1.8; window.actionCamOffsetX = 0; window.actionCamOffsetY = -20; 
                    window.dynamicBlur = 25; window.cameraTilt = (Math.random() > 0.5 ? 1 : -1) * 0.15; 
...
window.attack = function(attacker, targetGroup) {
    ...
    // [NEW COMBAT] TỔ CHỨC LẠI CHIÊU THỨC THEO TIER ĐỂ COMBO HỢP LÝ HƠN
    let moves_Light = ['jab', 'low_kick'];
    let moves_Medium = ['hook', 'cross', 'knee_strike'];
    let moves_Heavy = ['elbow_strike', 'backfist', 'spinning_heel'];
    let moves_Launcher = ['uppercut', 'dragon_uppercut', 'high_kick']; // Hất tung lên trời
    let moves_Spike = ['axe_kick', 'palm_strike']; // Cắm đập xuống đất
    let moves_Finisher = ['asura_strike', 'dempsey_roll', 'one_inch_punch'];

    let selectedMove = 'jab'; let isFinisher = false; let isCrit = false;
    let finisherChance = attacker.isRage ? 0.35 : 0.20;

    // [NEW COMBAT] DASH ATTACK (Lao vào khi ở xa)
    if (MathDist > reach + 40 && attacker.comboStep === 0) { 
        selectedMove = Math.random() > 0.5 ? 'shoulder_bash' : 'sliding_kick';
        attacker.vx = (attacker.isFacingRight ? 1 : -1) * attacker.currentSpeed * 4.5; 
        attacker.state = 'dash'; attacker.attackTimer = 18; 
        window.spawnDust(attacker.x, attacker.y); 
        window.speedLinesAlpha = 0.8; window.actionCamOffsetX = attacker.isFacingRight ? 50 : -50; 
        return; 
    }

    // [NEW COMBAT] LOGIC CHUỖI COMBO (Light -> Medium -> Heavy/Launcher -> Finisher)
    if (attacker.comboStep >= 3 || Math.random() < finisherChance) { 
        selectedMove = moves_Finisher[Math.floor(Math.random() * moves_Finisher.length)]; 
        isFinisher = true; attacker.comboStep = 0; 
    } else {
        if (target.onGround === false && target.y < window.GROUND_Y - 40) {
            // Khi kẻ địch đang lơ lửng trên không trung, ưu tiên xài đòn cắm đất (Spike) hoặc Hất tiếp
            selectedMove = Math.random() > 0.6 ? moves_Spike[Math.floor(Math.random() * moves_Spike.length)] : moves_Launcher[Math.floor(Math.random() * moves_Launcher.length)];
        } else {
            if (attacker.comboStep === 0) selectedMove = moves_Light[Math.floor(Math.random() * moves_Light.length)];
            else if (attacker.comboStep === 1) selectedMove = moves_Medium[Math.floor(Math.random() * moves_Medium.length)];
            else selectedMove = (Math.random() > 0.5) ? moves_Heavy[Math.floor(Math.random() * moves_Heavy.length)] : moves_Launcher[Math.floor(Math.random() * moves_Launcher.length)];
        }
    }

    attacker.state = selectedMove; attacker.attackTimer = isFinisher ? 35 : 22; attacker.vx = (attacker.isFacingRight ? 1 : -1) * (isFinisher ? 6 : 2.5); 
    let baseDmg = 12 * attacker.currentDmgMod; let finalDmg = baseDmg; let slashAngle = 0; 
    
    // [NEW COMBAT] GÁN TÍNH CHẤT VẬT LÝ CHO TỪNG LOẠI ĐÒN
    if (moves_Launcher.includes(selectedMove)) { 
        slashAngle = -Math.PI / 3; window.actionCamOffsetY = 50; 
        target.vy = -14; target.onGround = false; // Hất tung!
        target.hitStun = 35; 
        window.playSound(400, 'square', 0.2, 0.5, true);
    } 
    else if (moves_Spike.includes(selectedMove)) { 
        slashAngle = Math.PI / 3; window.actionCamOffsetY = -40; 
        if (!target.onGround) {
            target.vy = 18; // Cắm phập xuống đất tốc độ cao
            target.hitStun = 45;
            finalDmg *= 1.5;
            window.floatingTexts.push({ x: target.x, y: target.y - 70, text: "⏬ SMASH!", color: "#e67e22", alpha: 1.5, vx: 0, vy: -3, font: "900 35px Arial", life: 40, scale: 2.0, targetScale: 1.0, scaleVel: 0, rot: 0 });
        }
    } 
    else if (selectedMove === 'low_kick' || selectedMove === 'sliding_kick') { slashAngle = Math.PI / 8; } 
    else { slashAngle = (Math.random() - 0.5) * 0.2; }

    // [NEW COMBAT & TỐI ƯU HÓA] ĐÒN FINISHER MULTI-HIT AN TOÀN (SAFE FLURRY)
    // Dùng cơ chế đếm thay vì setInterval để chống lỗi Crash
    if (isFinisher) {
        if (selectedMove === 'dempsey_roll' || selectedMove === 'asura_strike') {
            isCrit = true; 
            ...
            attacker.flurryTarget = target;
            attacker.flurryTimer = selectedMove === 'asura_strike' ? 30 : 20; 
            attacker.flurryHits = 0;
            attacker.flurryMaxHits = selectedMove === 'asura_strike' ? 5 : 3;
            attacker.flurryBaseDmg = baseDmg;
            attacker.attackTimer = attacker.flurryTimer + 15; // Giữ animation lâu hơn
            return; // Trả về để flurry logic tự chạy trong hàm Update
        } else {
            // One Inch Punch (1 đòn chí mạng cực mạnh)
            isCrit = true; finalDmg = baseDmg * 4.0; window.shakeScreen(15, 12); target.vx = (attacker.isFacingRight ? 10 : -10); target.state = 'hurt'; target.hitStun = 60; window.spawnParticles(target.x, target.y - 40, "#ff4757", true); 
        ...
        target.vx = (attacker.isFacingRight ? 3 : -3); target.hitStun = 20; target.state = 'hurt';
    }
...
window.update = function() {
    ...
    // [TỐI ƯU HÓA] Không query DOM liên tục nếu không cần thiết
    if (window.uiShakeP1 > 0) { window.uiShakeP1--; let w1 = document.getElementById("hp-wrapper-1"); if (w1) w1.style.transform = `translate(${(Math.random()*6-3)}px, ${(Math.random()*6-3)}px)`; }
    else if (window.uiShakeP1 === 0) { let w1 = document.getElementById("hp-wrapper-1"); if (w1) w1.style.transform = "none"; window.uiShakeP1 = -1; }
    if (window.uiShakeP2 > 0) { window.uiShakeP2--; let w2 = document.getElementById("hp-wrapper-2"); if (w2) w2.style.transform = `translate(${(Math.random()*6-3)}px, ${(Math.random()*6-3)}px)`; } 
    else if (window.uiShakeP2 === 0) { let w2 = document.getElementById("hp-wrapper-2"); if (w2) w2.style.transform = "none"; window.uiShakeP2 = -1; }
    ...
    allFighters.forEach(f => {
        ...
        // [NEW COMBAT & TỐI ƯU HÓA] HỆ THỐNG FLURRY (LIÊN HOÀN) AN TOÀN TRONG UPDATE
        if (f.flurryTimer > 0) {
            f.flurryTimer--;
            let target = f.flurryTarget;
            // Kiểm tra xem cả 2 còn sống và tồn tại không
            if (target && target.hp > 0 && f.hp > 0 && !window.gameOver) {
                if (f.flurryTimer % 5 === 0 && f.flurryHits < f.flurryMaxHits) {
                    window.shakeScreen(5, 5); target.vx = (f.isFacingRight ? 1 : -1); target.hitStun = 20;
                    if(typeof window.takeDamage === 'function') window.takeDamage(target, f.flurryBaseDmg * 0.6, "#f1c40f", false, false, f.isFacingRight);
                    window.spawnSlash(target.x + (Math.random()-0.5)*20, target.y - 30 + (Math.random()-0.5)*20, f.isFacingRight, "#f1c40f", false, 1.5, (Math.random()-0.5)*Math.PI);
                    f.flurryHits++;
                    f.comboHits = (f.comboHits || 0) + 1;
                }
                // Hit cuối cùng bùng nổ
                if (f.flurryTimer === 1 && target.hp > 0) {
                    window.shakeScreen(20, 15); target.vx = (f.isFacingRight ? 8 : -8); target.vy = -5; target.onGround = false;
                    if(typeof window.takeDamage === 'function') window.takeDamage(target, f.flurryBaseDmg * 2.5, "#ff4757", true, false, f.isFacingRight);
                    window.spawnSlash(target.x, target.y - 35, f.isFacingRight, "#ff4757", true, 3.5, 0);
                    window.floatingTexts.push({ x: target.x, y: target.y - 80, text: "💥", color: "#ff4757", alpha: 1.5, vx: (Math.random()-0.5)*4, vy: -6, font: "900 65px Arial", life: 50, scale: 2.5, targetScale: 1.0, scaleVel: 0, rot: (Math.random()-0.5)*0.3 });
                }
            } else {
                f.flurryTimer = 0; // Kẻ địch chết, ngắt flurry
            }
        }

        // [NEW COMBAT] HỆ THỐNG VẬT LÝ BOUNCE (NẢY LÊN TỪ MẶT ĐẤT)
        if (f.hp <= 0 || f.state === 'hurt' || f.state === 'ko_falling') { 
            ...
            if (f.vy > 0 && f.y + f.vy >= window.GROUND_Y && !f.onGround) { 
                ...
                if (f.vy > 14) { 
                    ...
                    f.hitStun += 10;
                    window.playSound(400, 'square', 0.2, 0.5, true);
                    window.floatingTexts.push({ x: f.x, y: f.y - 50, text: "💥 BOUNCE!", color: "#f1c40f", alpha: 1.5, vx: (Math.random()-0.5)*2, vy: -4, font: "italic 900 30px 'Arial Black', Impact", life: 30, scale: 2.0, targetScale: 1.0, scaleVel: 0, rot: (Math.random()-0.5)*0.2 });
                } else if (f.vy > 8) {
                    window.shakeScreen(f.vy > 10 ? 8 : 5, 4); window.spawnEnvDamage(f.x, window.GROUND_Y, 'crater', f.scale || 1, false);
                }
            } 
            
            f.y += f.vy; f.x += f.vx; f.vx *= 0.93; 
            if (f.y >= window.GROUND_Y && f.vy > -1 && f.vy <= 1) { f.y = window.GROUND_Y; f.vy = 0; f.vx = 0; f.onGround = true; if(f.hp <= 0) f.state = 'dead'; } 
            if (f.hp <= 0) return; 
        } else {
            f.vy += window.GRAVITY; 
            ...
            if (f.isDragon) {
                ...
                if (closest && closest.hp > 0) {
                    ...
                    if (absDist > reach) { f.vx = Math.sign(dist) * f.currentSpeed; f.state = 'walk'; if (Math.random() < 0.1 && f.onGround) window.spawnDust(f.x, f.y); } 
                        ...
                        if (f.aiDelay <= 0) {
                            f.aiDelay = f.isPlayer ? Math.floor(Math.random() * 4) + 2 : Math.floor(Math.random() * 8) + 6; let usedSkill = false;
...
window.draw = function() {
    ...
    try {
        ...
        // VẼ HIỆU ỨNG NỨT KHÔNG GIAN (WHITEBEARD EFFECT)
