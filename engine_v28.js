// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 26.0 [THE APEX CAMERAMAN]
// [NEW] ANTICIPATION PANNING: Camera biết quay qua quay lại, lia đón đầu hướng di chuyển.
// [NEW] ANIMATED SPEED LINES: Tia tốc độ Anime cuộn chạy liên tục vào tâm.
// [NEW] HYPER-RESPONSIVE ZOOM & IMPACT FLASH: Camera co bóp theo nhịp đấm, chớp sáng chân không.
// ==========================================
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce, attackerDirRight = true) {
    ...
    if (target.state === 'block') { 
        if (target.stamina < 15) {
            ...
            // [VIRAL 26.0] Zoom Co bóp
            window.targetZoom = 1.5; window.dynamicBlur = 10;
            window.hitStopFrames = 20; window.chromaTimer = 20; window.vhsGlitchTimer = 20;
        } 
    ...
    if (finalDmg > 0) {
        ...
        if (isCrit) {
            // [VIRAL 26.0] CHỚP SÁNG VÀ CO BÓP ZOOM KHI CRIT
            window.screenFlash = 1.0; 
            window.impactFrameCount = 4; 
            window.targetZoom = 1.4; // Co vào
            setTimeout(() => { window.targetZoom = 1.1; }, 100); // Nhả ra liền
        ...
        if (target.hp <= 0) {
            ...
            if (isNearWall && isCrit && typeof window.triggerStageTransition === 'function') { window.triggerStageTransition(target); } 
                ...
                if (isCrit) {
                    window.screenFlash = 1.5; // Chớp lóa mắt lúc chết
                    window.invertFrames = 6; window.applyFilter('grayscale', 90); window.speedLinesAlpha = 1.0; window.spawnLensFlare(target.x, target.y - 40, "#ff003c", 5.0);
...
window.update = function() {
    ...
    // Đạo diễn trả góc quay về mặc định
    ...
    if (window.p1 && window.introTimer === 0) {
        ...
        // [VIRAL 26.0] ANTICIPATION PANNING (GIA TỐC CAMERA)
        if (window.gameOver && closest) {
            ...
            // Tính toán lực quay qua quay lại dựa trên tốc độ di chuyển của 2 bên
...
window.draw = function() {
    ...
    if (window.isLoading) {
        ...
        window.ctx.fillStyle = "#00f3ff"; window.ctx.font = "italic 900 40px Arial"; window.ctx.textAlign = "center"; window.ctx.shadowBlur = 15; window.ctx.shadowColor = "#00f3ff"; window.ctx.fillText("APEX CAMERAMAN INITIALIZING...", cx, cy - 50);
    ...
    try {
        ...
        // [VIRAL 26.0] ANIMATED SPEED LINES (TIA TỐC ĐỘ CUỘN VÀO TÂM)
        if (window.speedLinesAlpha > 0) {
            ...
            let timeOffset = Date.now() / 15; // Tạo độ cuộn liên tục
        ...
        // [VIRAL 26.0] PURE IMPACT FLASH (Chớp lóa mắt che khuất trước khi âm bản)
...
// ==========================================
// 7. [VIRAL 26.0] AFTER EFFECTS TIME-RAMPING & UNLOCKED 120Hz LOOP
// ==========================================
