// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 21.0 [THE CINEMATOGRAPHER]
// [CINEMATIC ALGORITHM: ĐƯA GAME 2D LÊN TẦM PHIM ĐIỆN ẢNH BOM TẤN]
// [TÍNH NĂNG: DOLLY ZOOM, DUTCH ANGLE, CINEMATIC BARS, GLOBAL ILLUMINATION]
// ==========================================
...
// [VIRAL 21.0] BIẾN ĐẠO DIỄN QUAY PHIM
window.mangaSfx = []; 
...
window.cinemaBarsHeight = 0; // Viền đen điện ảnh
window.targetCinemaBars = 0;
window.globalIllumination = null; // Ánh sáng môi trường tự động
...
// ==========================================
// 1. HỆ THỐNG ÂM THANH & GLOBAL ILLUMINATION
// ==========================================
window.initGameEngine = function() {
    window.isLoading = true; window.loadProgress = 0;
    
    // [VIRAL 21.0] TẠO MÀU ÁNH SÁNG MÔI TRƯỜNG NGẪU NHIÊN CHỐNG AI & TẠO MOOD CHUYÊN NGHIỆP
    const lightingThemes = [
        { mix: 'overlay', color: 'rgba(255, 120, 50, 0.15)' }, // Golden Hour (Nắng chiều)
        { mix: 'multiply', color: 'rgba(80, 80, 200, 0.15)' }, // Midnight (Đêm sâu)
        { mix: 'color-burn', color: 'rgba(255, 0, 50, 0.08)' }, // Blood Moon (Trăng máu)
        { mix: 'screen', color: 'rgba(0, 255, 200, 0.05)' }, // Cyberpunk Neon
        { mix: 'hard-light', color: 'rgba(150, 100, 50, 0.1)' } // Gritty War (Khói bụi chiến tranh)
    ];
...
// ==========================================
// 2. HỆ THỐNG SPAWN VFX (VẬT LÝ 3D GROUND)
// ==========================================
...
// [VIRAL 21.0] Sóng bụi lan trên mặt đất tạo chiều sâu 3D
...
window.triggerCinematic = function(caster, callback) { window.cinematicTimer = 50; window.cinematicCaster = caster; window.cinematicCallback = callback; window.targetZoom = 1.35; window.targetCinemaBars = 60; window.playSound(400, 'sine', 0.4, 0.2, false); window.speedLinesAlpha = 1.0; }
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce) {
    ...
    if (finalDmg > 0) {
        ...
        if (isCrit) {
            ...
            window.targetCinemaBars = 60; // Đẩy viền đen vào khi chí mạng
            setTimeout(() => { if(!window.gameOver) window.targetCinemaBars = 0; }, 600); // Rút viền đen ra
        ...
        if (target.hp <= 0) {
            ...
            if (isNearWall && isCrit && typeof window.triggerStageTransition === 'function') { window.triggerStageTransition(target); } 
                ...
                // [VIRAL 21.0] DOLLY ZOOM & DUTCH ANGLE FINISHER
                if (isCrit) {
                    ...
                    window.targetZoom = 1.8; // Zoom tử thần cực gắt
                    window.cameraTilt = (Math.random() > 0.5 ? 1 : -1) * 0.15; // Cầm máy nghiêng (Dutch Angle)
                    window.targetCinemaBars = 120; // Viền đen bóp chặt khung hình
                    
                    window.hitStopFrames = 25; window.shakeScreen(60, 35);
                    window.vhsGlitchTimer = 35; window.triggerDimensionShatter(); 
                    window.targetTimeScale = 0.05; setTimeout(()=>{ window.targetTimeScale = 1.0; window.cameraTilt = 0; }, 2000); // Slowmo siêu dài
                } else { window.hitStopFrames = 12; window.shakeScreen(30, 20); window.targetZoom = 1.45; window.targetCinemaBars = 80; window.targetTimeScale = 0.2; setTimeout(()=>{window.targetTimeScale = 1.0; window.targetCinemaBars = 0;}, 1000); }
...
window.update = function() {
    ...
    // Cập nhật Cinema Bars (Thu vào nở ra mượt mà)
    ...
    if (!isTimeStopped) {
        ...
        // [VIRAL 21.0] BỤI ĐẤT PHỐI CẢNH 3D
    ...
    if (window.p1 && window.introTimer === 0) {
        ...
        if (window.gameOver && closest) {
            ...
            // Dolly Zoom effect retained during game over
        } 
...
window.draw = function() {
    ...
    // Smooth cinematic bars interpolation
    window.cinemaBarsHeight += (window.targetCinemaBars - window.cinemaBarsHeight) * 0.1;
    ...
    if (window.isLoading) {
        ...
        window.ctx.fillStyle = "#00f3ff"; window.ctx.font = "italic 900 40px Arial"; window.ctx.textAlign = "center"; window.ctx.shadowBlur = 15; window.ctx.shadowColor = "#00f3ff"; window.ctx.fillText("DIRECTOR'S CUT INITIALIZING...", cx, cy - 50);
    ...
    try {
        ...
        let camBreathX = Math.sin(Date.now() / 1500) * 4 + Math.cos(Date.now() / 900) * 2;
        let camBreathY = Math.cos(Date.now() / 1200) * 4 + Math.sin(Date.now() / 800) * 2;
        ...
        // [VIRAL 21.0] ADVANCED PARALLAX BACKGROUND
        ...
        window.ctx.save();
        window.ctx.translate(window.camX * 0.9, window.camY * 0.5); // Sky moves very little
        ...
        window.ctx.translate(window.camX * 0.8, window.camY * 0.4);
        ...
        window.ctx.save(); window.ctx.translate(-window.camX * 0.4, -window.camY * 0.2); // Mountains move mid-speed
        ...
        window.ctx.save(); window.ctx.translate(-window.camX * 0.15, -window.camY * 0.05); // City moves fastest (closest to foreground)
        ...
        // [VIRAL 21.0] MÀU SẮC CHIẾU SÁNG MÔI TRƯỜNG GLOBAL (TIME OF DAY)
        if (window.globalIllumination) {
            window.ctx.save();
            window.ctx.globalCompositeOperation = window.globalIllumination.mix;
            window.ctx.fillStyle = window.globalIllumination.color;
            window.ctx.fillRect(-3000, -3000, window.canvas.width + 6000, window.canvas.height + 6000);
            window.ctx.restore();
        }
        ...
        window.particles.forEach(pt => { 
            ...
            // [VIRAL 21.0] VẬT LÝ 3D CHO SÓNG BỤI (BỤI DƯỚI ĐẤT SẼ DẸT LẠI THÀNH ELIP)
            if (pt.isGroundDust) {
        ...
        // VẼ MÀN HÌNH VIỀN ĐEN ĐIỆN ẢNH (CINEMATIC BARS) NẰM TRÊN TẤT CẢ (NGOẠI TRỪ UI COMBO)
...
// ==========================================
// 7. [VIRAL 21.0] AFTER EFFECTS TIME-RAMPING & UNLOCKED 120Hz LOOP
// ==========================================
