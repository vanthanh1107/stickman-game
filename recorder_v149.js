// ==========================================
// RECORDER.JS - V76.5 THE ALGORITHM BREAKER (PERFECT EDITION)
// ĐÃ FIX & GIỮ NGUYÊN: Intro God-Tier, Vỡ kính 3D, Kanji Manga, Kill-Cam, Viral Post Kit.
// TÂM LÝ HỌC: Mưa Tim, Sạn cố ý (Bait), Sai chính tả mồi Comment, Facecam Gái xinh React.
// V76.5 (RESTORED & UPGRADED): Khôi phục Thông báo Tặng Hoa hồng/Like (Bay góc trái), Nút LIVE góc phải, Nhịp tim (BPM) & Tốc độ tay (APM) ảo.
// ==========================================
...
// ====================================================================
// HỆ THỐNG LIVE CHAT TOXIC ĐA NGÔN NGỮ
// ====================================================================
window.CELEB_LIST = [
...
window.captureFrames = function() {
    ...
    if (renderNormalV) {
        ...
        ctxV.beginPath(); if(ctxV.roundRect) ctx.roundRect(-liveW/2, -liveH/2, liveW, liveH, 22.5); else ctxV.fillRect(-liveW/2, -liveH/2, liveW, liveH);
        ctxV.fill();
        ...
        // =========================================================
        // THÔNG BÁO TẶNG QUÀ / THẢ TIM (LIVE ALERTS) - NẰM BÊN TRÁI
        // =========================================================
        ...
        let alertStartY = splitGameHeight - 100;
        for (let i = window._liveAlerts.length - 1; i >= 0; i--) {
            let al = window._liveAlerts[i]; al.life -= 0.015; al.yOffset += 1.2;
            ctxV.save(); ctxV.globalAlpha = Math.max(0, Math.min(1, al.life * 1.5));
            ctxV.font = "bold 22px Arial"; let textW = ctxV.measureText(al.text).width;
            ctxV.translate(250, alertStartY - al.yOffset); 
            ctxV.fillStyle = "rgba(0, 0, 0, 0.5)"; ctxV.strokeStyle = "rgba(255, 255, 255, 0.2)"; ctxV.lineWidth = 2;
            if(ctxV.roundRect) { ctxV.beginPath(); ctxV.roundRect(-textW/2 - 15, -20, textW + 30, 40, 20); ctxV.fill(); ctxV.stroke(); }
            else { ctxV.fillRect(-textW/2 - 15, -20, textW + 30, 40); }
            ctxV.fillStyle = "#fff"; ctxV.textAlign = "center"; ctxV.textBaseline = "middle"; ctxV.fillText(al.text, 0, 0);
            ctxV.restore();
        ...
        // =========================================================
        // THÊM TÍNH NĂNG FACECAM GÁI XINH REACT VÀO TRONG GAME
        // =========================================================
        if (!window._facecamImg) {
            window._facecamImg = new Image();
            window._facecamImg.crossOrigin = "Anonymous";
            window._facecamImg.src = window.FACECAM_IMAGE_URL || "https://images.unsplash.com/photo-1593305841991-05c297ba4575?w=400&q=80"; 
        }
        
        if (window._facecamImg && window._facecamImg.complete) {
            ctxV.save();
            let camX = 850; let camY = 220; let camW = 200; let camH = 280;
            
            // Giật camera khi có tiếng lớn (Combat)
            if (audioPeak > 0.5) {
                camX += (Math.random() - 0.5) * 15;
                camY += (Math.random() - 0.5) * 15;
            }
            ctxV.translate(camX, camY);
            
            // Khung bo tròn
            ctxV.beginPath();
            if(ctxV.roundRect) ctxV.roundRect(-camW/2, -camH/2, camW, camH, 20); else ctxV.rect(-camW/2, -camH/2, camW, camH);
            ctxV.clip();
            
            // Phóng to mặt theo cường độ âm thanh
            let zoom = 1.0 + (audioPeak * 0.35);
            ...
            ctxV.fillStyle = "#ff0000";
            ctxV.beginPath();
            if(ctxV.roundRect) ctxV.roundRect(-camW/2 + 10, -camH/2 + 10, 60, 25, 5); else ctxV.rect(-camW/2 + 10, -camH/2 + 10, 60, 25);
            ctxV.fill();
            ctxV.fillStyle = "#fff"; ctxV.font = "bold 14px Arial"; ctxV.textAlign = "center"; ctxV.textBaseline = "middle";
            ctxV.fillText("LIVE", -camW/2 + 40, -camH/2 + 24);
            ctxV.restore();
        }
