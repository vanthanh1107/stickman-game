// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 30.0 [KINETIC TYPOGRAPHY]
// [NEW] KINETIC TYPOGRAPHY: Số sát thương văng vật lý, bật nảy lò xo 3D (Spring Physics).
// [NEW] COMIC OUTLINES: Đổ viền đen dày, Font nghiêng dồn dập chuẩn eSports.
// [KEEP] 2.5D FOREGROUND DEBRIS & LETHAL VOID TỪ V27.
// ==========================================
...
// ==========================================
// 4. HỆ THỐNG VẬT LÝ NHẬN SÁT THƯƠNG (CÓ KINETIC TEXT)
// ==========================================
window.takeDamage = function(target, amount, color, isCrit, wallBounce, attackerDirRight = true) {
    ...
    if (finalDmg > 0) {
        ...
        // [VIRAL 30.0] KINETIC TYPOGRAPHY DAMAGE NUMBERS
        ...
        if (target.hp <= 0) {
            ...
            if (isNearWall && isCrit && typeof window.triggerStageTransition === 'function') { window.triggerStageTransition(target); } 
                ...
                if (isCrit) {
                    ...
                    window.targetTimeScale = 0.02; setTimeout(()=>{ window.targetTimeScale = 1.0; window.cameraTilt = 0; }, 2500); 
...
window.attack = function(attacker, targetGroup) {
    ...
    if (isFinisher) {
        ...
        if ((selectedMove === 'low_kick' || selectedMove === 'teep_kick') && Math.random() < 0.4) { target.stunTimer = 35; target.state = 'stunned'; window.floatingTexts.push({ x: target.x, y: target.y - 50, text: "🦵", color: "#e67e22", alpha: 1.5, vx: 0, vy: -3, font: "900 35px Arial", life: 40, scale: 2.0, targetScale: 1.0, scaleVel: 0 }); }
        if ((selectedMove === 'uppercut' || selectedMove === 'elbow_strike') && Math.random() < 0.3) { finalDmg *= 1.5; window.spawnParticles(target.x, target.y - 60, "#c0392b", true); window.floatingTexts.push({ x: target.x, y: target.y - 50, text: "🩸", color: "#c0392b", alpha: 1.5, vx: 0, vy: -3, font: "900 35px Arial", life: 40, scale: 2.0, targetScale: 1.0, scaleVel: 0 }); }
    ...
    if (!isTimeStopped) {
        window.enemies.forEach(e => { 
            if (e.hp <= 0 && !e.deathTriggered) {
                ...
                if (window.p1 && window.p1.hp > 0) { let heal = Math.floor(window.p1.maxHp * 0.08); window.p1.hp = Math.min(window.p1.maxHp, window.p1.hp + heal); window.floatingTexts.push({ x: window.p1.x, y: window.p1.y - 80, text: `+${heal} 💚`, color: "#2ed573", alpha: 1.5, vx: (Math.random()-0.5)*4, vy: -6, font: "bold 24px Arial", life: 45 }); }
            }
        });
            ...
            if (f.hitStun <= 0 && f.stunTimer <= 0 && f.state !== 'dash_back' && f.state !== 'block') {
                ...
                if (isCloseEnough) {
                    if (window.classStats && window.classStats[type] && typeof window.classStats[type].executeUltimate === 'function') {
                        f.stamina = 0; let baseDmg = 50 * (f.currentDmgMod || 1); if (!f.isPlayer) baseDmg = 35 * (f.currentDmgMod || 1); window.playSound(400, 'sine', 0.5, 0.6); window.shakeScreen(15, 10); window.spawnParticles(f.x, f.y, "#f1c40f", true); window.floatingTexts.push({ x: f.x, y: f.y - 100, text: f.isPlayer ? "🔥 ULTIMATE!" : "⚠️ DANGER!", color: f.isPlayer ? "#ff4757" : "#ff0000", alpha: 1.5, vx: 0, vy: -6, font: "italic 900 45px 'Arial Black', Impact", life: 50, scale: 3.0, targetScale: 1.0, scaleVel: 0, rot: (Math.random()-0.5)*0.2 }); f.isFacingRight = distToTarget > 0; window.classStats[type].executeUltimate(f, closestTarget, baseDmg); f.vx = 0; window.speedLinesAlpha = 1.0; 
            ...
            if (f.isDragon) {
                ...
                if (targetFighter && targetFighter.hp > 0) {
                    ...
                    if (f.aiDelay <= 0) {
                        ...
                        if (absDist > 250 && Math.random() < 0.6) { f.state = 'cast'; f.attackTimer = 25; window.playSound(300, 'sine', 0.3, 0.6); window.spawnSlash(f.x + (f.isFacingRight? 50:-50), f.y - 40, f.isFacingRight, "#fff", true, 2.0, Math.PI/2); window.spawnProjectile(f.x, f.y - 40, f.isFacingRight ? 12 : -12, 0, 15, "#fff", Math.floor(25 * f.dmgMod), window.p1); window.floatingTexts.push({ x: f.x, y: f.y - 80, text: "🗡️", color: "#fff", alpha: 1.5, vx: 0, vy: -4, font: "900 40px Arial", life: 40, scale: 2.0, targetScale: 1.0, scaleVel: 0, rot: 0 }); } 
                ...
                if (targetFighter && targetFighter.hp > 0) {
                    ...
                    if (f.aiDelay <= 0) {
                        ...
                        if (randAction < 0.4) { f.state = 'cast'; f.attackTimer = 20; window.playSound(400, 'sine', 0.2, 0.4); window.spawnProjectile(f.x, f.y - 50, f.isFacingRight ? 15 : -15, 0, 8, "#9b59b6", Math.floor(15 * f.dmgMod), window.p1); if(Math.random() < 0.5) window.spawnProjectile(f.x, f.y - 60, f.isFacingRight ? 14 : -14, -3, 8, "#9b59b6", Math.floor(15 * f.dmgMod), window.p1); window.floatingTexts.push({ x: f.x, y: f.y - 80, text: "🥷", color: "#9b59b6", alpha: 1.5, vx: 0, vy: -4, font: "900 40px Arial", life: 30, scale: 2.0, targetScale: 1.0, scaleVel: 0, rot: 0 }); } 
                        else if (randAction < 0.7) { window.spawnParticles(f.x, f.y, "#2c3e50"); f.x = window.p1.x + (window.p1.isFacingRight ? -80 : 80); f.y = window.p1.y; f.isFacingRight = window.p1.x > f.x; window.spawnParticles(f.x, f.y, "#9b59b6"); f.state = 'spinning_backfist'; f.attackTimer = 15; window.playSound(200, 'square', 0.2, 0.6); if(typeof window.takeDamage === 'function') window.takeDamage(window.p1, Math.floor(20 * f.dmgMod), "#9b59b6", false, false); window.floatingTexts.push({ x: f.x, y: f.y - 80, text: "💨", color: "#8e44ad", alpha: 1.5, vx: 0, vy: -4, font: "900 40px Arial", life: 30, scale: 2.0, targetScale: 1.0, scaleVel: 0, rot: 0 }); } 
...
window.draw = function() {
    ...
    try {
        ...
        // [VIRAL 30.0] VẼ KINETIC TYPOGRAPHY (CHỮ 3D VĂNG QUÁN TÍNH)
        window.floatingTexts.forEach(t => { 
            ...
            window.ctx.textAlign = "center"; // Đặt tâm text ở giữa để Scale chuẩn 3D
            window.ctx.textBaseline = "middle";
            window.ctx.globalAlpha = Math.max(0, Math.min(1, t.alpha)); 
            
            // Vẽ viền đen dày dặn chuẩn eSports
            window.ctx.lineWidth = 4; window.ctx.strokeStyle = "#000";
            window.ctx.strokeText(t.text, 0, 0);
            
            // Vẽ lõi màu phát sáng
            window.ctx.shadowBlur = 15; window.ctx.shadowColor = t.color; 
...
// ==========================================
// 7. [VIRAL 30.0] TIME-ENGINE VÀ PHYSICS LOOP CHUẨN XÁC
// ==========================================
