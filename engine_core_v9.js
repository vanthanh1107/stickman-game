...
// [BẢN HOÀN CHỈNH ULTIMATE + CINEMATIC DIRECTOR: HỖ TRỢ LÀM PHIM/COMBO DÀI]
// ==========================================
...
// 🟢 HÀM KHỞI CHẠY ĐẠO DIỄN KỊCH BẢN (CINEMATIC DIRECTOR)
                ...
                if (target && target.hp > 0 && Math.abs(f.x - target.x) <= reach + 35) {
                    ...
                    if (hit.isFinisher) {
                        ...
                        if (Math.random() < f.critChance) { 
                            ...
                            window.floatingTexts.push({ x: target.x + (Math.random()*40-20), y: target.y - 60, text: "💢", color: "#f1c40f", alpha: 1.5, vx: 0, vy: -4, font: "italic 900 35px 'Arial Black', Impact", life: 30, scale: 2.0, targetScale: 1.0, scaleVel: 0, rot: 0 }); 
                        } else { window.playSound(350, 'sine', 0.1, 0.1, false); }
                        if ((f.state === 'low_kick' || f.state === 'teep_kick') && Math.random() < 0.4) { target.stunTimer = 35; target.state = 'stunned'; window.floatingTexts.push({ x: target.x, y: target.y - 50, text: "🦵", color: "#e67e22", alpha: 1.5, vx: 0, vy: -3, font: "900 35px Arial", life: 40, scale: 1.8, targetScale: 1.0, scaleVel: 0, rot: 0 }); }
                        if ((f.state === 'uppercut' || f.state === 'elbow_strike') && Math.random() < 0.3) { finalDmg *= 1.5; window.spawnParticles(target.x, target.y - 60, "#c0392b", true); window.floatingTexts.push({ x: target.x, y: target.y - 50, text: "🩸", color: "#c0392b", alpha: 1.5, vx: 0, vy: -3, font: "900 35px Arial", life: 40, scale: 1.8, targetScale: 1.0, scaleVel: 0, rot: 0 }); }
