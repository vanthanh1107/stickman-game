...
// Kèm theo SCROLLING CELEBRITY LIVE CHAT (2-3s/Comment)
// ==========================================
...
window.captureFrames = function() {
    ...
    // CHÍNH GIỮA: VTuber Commentator (Được dời sang phải một chút để cân hình)
    if (!window.gameOver && window.introTimer > 120) { 
        drawVTuberCommentator(ctxV, 800, splitGameHeight + 250, audioPeak, window.StoryModeAI.isTyping); 
    }
    ...
    // ==========================================
    // 7. TOXIC CELEB CHAT (CHẠY LIÊN TỤC TRÊN MÀN HÌNH ĐẾN KHI HẾT VIDEO)
    // ==========================================
    if (!window.celebChatInit) {
        window.celebChatInit = true;
        window.celebChatList = [];
        window.lastCelebChatTime = Date.now();
        
        // Danh sách tên Celeb kèm màu đặc trưng
        window.celebNames = [
        ...
        // Danh sách comment chê bai siêu toxic
        window.celebMsgs = [
    ...
    // Hiển thị chat kể từ giây thứ 2 cho tới lúc video hoàn toàn kết thúc
    if (window.introTimer > 120) {
        let nowTime = Date.now();
        
        // Tốc độ hiện chữ: 2.5 - 3.5 giây hiện 1 comment để dễ đọc
        if (nowTime - window.lastCelebChatTime > (2500 + Math.random() * 1000)) {
            window.lastCelebChatTime = nowTime;
            
            let randomCeleb = window.celebNames[Math.floor(Math.random() * window.celebNames.length)];
            let randomMsg = window.celebMsgs[Math.floor(Math.random() * window.celebMsgs.length)];
            
            window.celebChatList.push({
                name: randomCeleb.name,
                color: randomCeleb.color,
                msg: randomMsg
            });

            // Chỉ giữ tối đa 10 comment hiển thị trên màn hình
            if (window.celebChatList.length > 10) {
                window.celebChatList.shift();
            }
        }

        const drawLiveChat = (ctx, isVertical) => {
            ctx.save();
            let startX = isVertical ? 40 : 80;
            // Neo vị trí chat ở góc dưới bên trái, comment mới nhất ở dưới cùng
            let startY = isVertical ? 1750 : 1000; 
            let lineHeight = isVertical ? 55 : 45;
            
            ctx.textAlign = "left";
            ctx.textBaseline = "bottom";
            
            // Vẽ 1 lớp mờ mờ Gradient đằng sau lưng để chữ luôn dễ đọc trên mọi nền màu
            let bgGrad = ctx.createLinearGradient(0, startY - (11 * lineHeight), 0, startY);
            bgGrad.addColorStop(0, "rgba(0,0,0,0)");
            bgGrad.addColorStop(1, "rgba(0,0,0,0.6)");
            ctx.fillStyle = bgGrad;
            ctx.fillRect(0, startY - (11 * lineHeight) - 50, isVertical ? 900 : 1200, (11 * lineHeight) + 100);

            // Vẽ tiêu đề "🔴 LIVE COMMENTS" trên cùng của danh sách
            ctx.shadowColor = "rgba(0,0,0,0.8)";
            ctx.shadowBlur = 6;
            ctx.fillStyle = "#ff0055";
            ctx.font = isVertical ? "900 30px 'Arial Black'" : "900 24px 'Arial Black'";
            ctx.fillText("🔴 LIVE COMMENTS", startX, startY - (window.celebChatList.length * lineHeight) - 20);

            // Vẽ từng dòng chữ (Cuộn dần từ dưới lên)
            for (let i = 0; i < window.celebChatList.length; i++) {
                let chat = window.celebChatList[i];
                let yPos = startY - ((window.celebChatList.length - 1 - i) * lineHeight);
                
                ctx.font = isVertical ? "bold 34px Arial" : "bold 28px Arial";
                
                // Vẽ tên Celeb (Màu đặc trưng)
                ctx.fillStyle = chat.color;
                ctx.fillText(chat.name + ":", startX, yPos);
                
                // Vẽ Comment (Màu trắng)
                let nameWidth = ctx.measureText(chat.name + ":").width;
                ctx.fillStyle = "#ffffff";
                ctx.fillText(" " + chat.msg, startX + nameWidth, yPos);
            }
            ctx.restore();
        };

        // Render ra cả 2 video ngang và dọc
        drawLiveChat(ctxV, true);
        if (renderNormalH) drawLiveChat(ctxH, false);
    }
