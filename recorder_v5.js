...
// ĐÃ NÂNG CẤP AUTO-CAPTURE THU TOÀN BỘ MỌI ÂM THANH
// TÍCH HỢP HỆ THỐNG "BÌNH LUẬN VIÊN AI" CHO VIDEO TẢI XUỐNG
// [NEW] TÍCH HỢP TỰ ĐỘNG SINH TIÊU ĐỀ YOUTUBE/TIKTOK CLICKBAIT (TIẾNG ANH)
// ==========================================
...
// ==========================================
// THUẬT TOÁN SINH TIÊU ĐỀ VIRAL (YOUTUBE / TIKTOK - ENGLISH)
// ==========================================
window.generateViralTitle = function(hero, enemy) {
    const templates = [
        "🔥 YOU WON'T BELIEVE WHAT {hero} DID TO {enemy}! (SHOCKING)",
        "😱 {hero} DESTROYS {enemy} IN SECONDS! (GONE WRONG)",
        "🚨 {enemy} REGRETS CHALLENGING {hero}! (INSTANT KARMA)",
        "💥 EPIC COMEBACK! {hero} OUTPLAYS {enemy} AT 1% HP!",
        "👑 THE MOST INSANE {hero} GALAXY BRAIN MOMENT VS {enemy}!",
        "💀 {hero} SENDS {enemy} TO THE SHADOW REALM! (BRUTAL)",
        "📺 NO ONE EXPECTED THIS ENDING! {hero} vs {enemy} FULL FIGHT!",
        "⚡ {hero} GOES GOD MODE! {enemy} HAD NO CHANCE!",
        "🏆 RANK 1 {hero} EXPOSES {enemy}'S WEAKNESS! (MUST WATCH)",
        "🤯 99% OF PLAYERS CAN'T DO THIS! {hero} HUMILIATES {enemy}!",
        "⚠️ WARNING: BRUTAL BEATDOWN! {hero} vs {enemy} UNEDITED!",
        "🥶 {hero} IS COMPLETELY BROKEN! WATCH THEM ANNIHILATE {enemy}!",
        "😈 HOW TO RUIN {enemy}'S CAREER USING {hero}!",
        "💯 ZERO TO HERO! {hero} CRUSHES {enemy} WITH THIS INSANE TRICK!",
        "👀 {enemy} THOUGHT THEY WON... UNTIL {hero} DID THIS!"
    ];
    let rand = templates[Math.floor(Math.random() * templates.length)];
    return rand.replace(/{hero}/g, hero.toUpperCase()).replace(/{enemy}/g, enemy.toUpperCase());
};

// Loại bỏ các ký tự đặc biệt để làm tên file hợp lệ cho máy tính
window.sanitizeFileName = function(str) {
    return str.replace(/[^a-z0-9\s_-]/gi, '').trim().replace(/\s+/g, '_');
};

// ==========================================
// HỆ THỐNG BÌNH LUẬN VIÊN AI TỰ ĐỘNG
// ==========================================
window.AICommentator = {
    timer: null,
    isSpeaking: false,
    // BẠN CÓ THỂ THÊM/SỬA CÁC CÂU BÌNH LUẬN HÀI HƯỚC Ở ĐÂY
    phrases: [
        "Ái chà, một pha ra đòn khá cồng kềnh!",
        "Đánh thế này thì bao giờ mới xong, mỏi tay quá!",
        "Ui da, đòn này đau đấy, nhìn mà xót cả ruột!",
        "Lên luôn, đừng nhát thế, lao vào đấm tới tấp đi!",
        "Thanh máu đang tụt nhanh hơn cách người yêu cũ trở mặt!",
        "Ối giời ơi, né đòn khét lẹt, đúng là hảo hán!",
        "Tình hình căng thẳng quá, mồ hôi rơi ướt hết cả sàn rồi!",
        "Đánh trượt kìa, pha xử lý đi vào lòng đất!"
    ],
    speak: function(text) {
        if (this.isSpeaking || !window.isRecording) return;
        this.isSpeaking = true;
        
        // MẸO: Dùng API ẩn của Google Dịch để lấy file MP3 thật, qua đó Recorder mới thu âm được
        let url = `https://translate.google.com/translate_tts?ie=UTF-8&tl=vi&client=tw-ob&q=${encodeURIComponent(text)}`;
        let voice = new Audio(url);
        voice.crossOrigin = "anonymous";
        voice.volume = 1.0; // Âm lượng bình luận viên
        
        voice.onended = () => { this.isSpeaking = false; };
        voice.onerror = () => { this.isSpeaking = false; };
        
        // Hệ thống Auto-Capture sẽ tự động lấy âm thanh này chèn vào video
        voice.play().catch(e => {
            this.isSpeaking = false;
            console.log("Trình duyệt chặn tiếng AI do chưa tương tác:", e);
        });
    },
    start: function() {
        this.stop();
        // Đọc câu mào đầu khi vừa bấm bắt đầu trận
        setTimeout(() => this.speak("Trận đấu bắt đầu! Chúc hai võ sĩ thi đấu tốt, không sứt mẻ gì nhiều!"), 1000);
        
        // Random phát bình luận mỗi 5 đến 8 giây
        this.timer = setInterval(() => {
            if (window.isRecording && window.p1 && window.p1.hp > 0 && !window.gameOver) {
                let r = Math.floor(Math.random() * this.phrases.length);
                this.speak(this.phrases[r]);
            }
...
window.startRecording = function() {
    ...
    if (window.p1 && window.classStats && window.classStats[window.p1.classId]) {
        ...
        charName = stats.className || "WARRIOR";
    ...
    const finalizeRecordings = () => {
        ...
        if (stoppedCount === 2) {
            setTimeout(() => {
                ...
                // Lấy Tên Kẻ Thù lúc kết thúc trận để làm Tiêu Đề
                let enemyName = "UNKNOWN BOSS";
                if (window.enemies && window.enemies.length > 0) {
                    let e0 = window.enemies[0];
                    if (e0.isDragon) enemyName = "DRAGON BOSS";
                    else if (e0.isBruceLee) enemyName = "BRUCE LEE";
                    else if (e0.isSamurai) enemyName = "SAMURAI";
                    else if (e0.isNinja) enemyName = "NINJA";
                    else enemyName = e0.className || "BOSS";
                }

                // Sinh tiêu đề clickbait và chuỗi tên file an toàn
                let viralTitle = window.generateViralTitle(charName, enemyName);
                let safeFileName = window.sanitizeFileName(viralTitle);
                ...
                window.savedVideos.push({ 
                    ...
                    viralTitle: viralTitle,        // Thêm tiêu đề giật gân
                    safeFileName: safeFileName     // Tên file hợp lệ
                });
    ...
    // GỌI AI BÌNH LUẬN VIÊN KHI BẮT ĐẦU QUAY
    window.AICommentator.start();
};
...
window.copyToClipboard = function(text) {
    navigator.clipboard.writeText(text).then(() => {
        alert("✅ Title copied! Ready to paste into TikTok/YouTube.");
    }).catch(err => {
        ...
        html += `<div style="display: flex; flex-direction: column; background: #141a27; padding: 15px; border-radius: 8px; border: 1px solid #334155; box-shadow: inset 0 0 5px rgba(0,0,0,0.3);">
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        ...
                        <div style="display: flex; gap: 10px;">
                            <a href="${vid.urlH}" download="[HORIZONTAL]_${vid.safeFileName}.${vid.ext}" style="background: #ff003c; color: #fff; text-decoration: none; padding: 8px 15px; border-radius: 4px; font-family: 'Teko', sans-serif; font-size: 18px; font-weight: 600; letter-spacing: 1px; box-shadow: 0 2px 5px rgba(255,0,60,0.3); transition: 0.2s;">📥 HORIZONTAL</a>
                            <a href="${vid.urlV}" download="[VERTICAL]_${vid.safeFileName}.${vid.ext}" style="background: #00f3ff; color: #0a0d14; text-decoration: none; padding: 8px 15px; border-radius: 4px; font-family: 'Teko', sans-serif; font-size: 18px; font-weight: 600; letter-spacing: 1px; box-shadow: 0 2px 5px rgba(0,243,255,0.3); transition: 0.2s;">📱 VERTICAL (TikTok)</a>
