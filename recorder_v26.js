// ==========================================
// RECORDER.JS - BẢN HOÀN HẢO CHO CONTENT CREATOR (V55.2 - ULTRA STABLE MEME EDITION)
// [FIXED] Sửa triệt để lỗi Video Rỗng/Không hiện bảng UI do xung đột bộ nhớ đệm (MediaRecorder flush bug).
// [FIXED] Ép trình duyệt ghi hình mỗi 1000ms để chống kẹt luồng, đảm bảo 100% video được lưu khi thoát.
// [RETAINED] MEME THUMBNAIL GENERATOR & AUTO-EMBED (Chế ảnh bìa cực dị tự động nhúng vào 1.5s đầu video).
// ==========================================
...
window.bakeThumbnailsForVideo = function(titleText) {
    ...
    const drawTextOverlays = (ctx, w, h) => {
        ...
        let mainEmoji = ["🤡", "💀", "😎", "🌭", "🔥", "👽", "🗿", "🤯"][Math.floor(Math.random() * 8)];
        let tTitle = titleText || "EPIC BRAWL";

        ctx.font = `900 ${h*0.08}px 'Arial Black', Impact`; ctx.textAlign = "center";
        ctx.lineWidth = 25; ctx.strokeStyle = "#000"; ctx.strokeText(`${tTitle} ${mainEmoji}`, w/2, h * 0.15);
        ctx.fillStyle = "#f1c40f"; ctx.fillText(`${tTitle} ${mainEmoji}`, w/2, h * 0.15);
...
window.startRecording = function() {
    if (window.isRecording) {
        console.warn("Ép lưu Video cũ trước khi bắt đầu trận mới...");
        ...
        if (stoppedCount === 2) { 
            setTimeout(() => {
                if (window.recordedChunksH.length === 0 || window.recordedChunksV.length === 0) {
                    console.log("Cảnh báo: Dữ liệu video rỗng, có thể do trình duyệt kẹt luồng.");
                }
                ...
                // Mặc định luôn tạo Blob dù có rỗng hay không để tránh lỗi chết UI
    ...
    // [FIX TRỌNG TÂM] Ép trình duyệt xả file video vào bộ nhớ mỗi 1 giây. Chống mất video.
...
window.stopRecording = function() { 
    ...
    // Dùng Stop an toàn, tuyệt đối không dùng requestData gây lỗi mất file ở Chrome
...
window.captureFrames = function() {
    ...
    // 🔥 [AUTO CUT] TỰ ĐỘNG TẮT RECORD KHI TRẬN ĐẤU KẾT THÚC CHUẨN 120 FRAMES (Khoảng 2 giây sau K.O)
...
window.updateVideoListUI = function() {
    ...
    container.style.display = "block"; // Hiện lại nếu bị ẩn
};
