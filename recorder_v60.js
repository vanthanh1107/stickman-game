// ==========================================
// RECORDER.JS - BẢN HOÀN HẢO CHO CONTENT CREATOR (V66.7 - ULTIMATE TIKTOK META)
// [NEW] Hiệu ứng CapCut AI Captions: Tự động Highlight vàng từ đang nói (Word-by-word).
// [NEW] RGB Glitch & Chromatic Aberration: Tách màu 3D khi tung đòn mạnh.
// [NEW] Lưới không gian 3D (True 3D Vanishing Grid) trượt với tốc độ cao.
// [NEW] Tia tốc độ Anime (Action Lines) xuất hiện lúc cao trào.
// [FIX] Khóa mượt 60FPS bằng thuật toán Bitwise Rendering.
// ==========================================
...
function wrapTextTikTok(ctx, text, x, y, maxWidth, lineHeight) {
    ...
    for(let i=0; i<lines.length; i++) {
        ...
        let startX = x - lineWidth / 2; // Căn giữa toàn bộ cụm
        let currentX = startX;
        ...
        for(let j=0; j<lineWords.length; j++) {
            ...
            ctx.save();
            if(isLastWord) {
                ctx.fillStyle = "#ffeb3b"; // Vàng Neon (Highlight)
...
window.captureFrames = function() {
    ...
    // HIỆU ỨNG TÁCH MÀU GLITCH RGB TRÊN BẢN DỌC TIKTOK
    if (audioPeak > 0.6) {
        let glitchStr = ((audioPeak - 0.6) * 30) | 0;
        ctxV.globalCompositeOperation = 'screen'; 
        ctxV.fillStyle = 'rgba(255, 0, 0, 0.4)'; // Đỏ
        ...
        ctxV.fillStyle = 'rgba(0, 255, 255, 0.4)'; // Xanh lơ
        ctxV.drawImage(window.canvas, 0, 0, window.canvas.width, window.canvas.height, glitchStr, shakeY | 0, 1440 + glitchStr, splitGameHeight);
        ctxV.globalCompositeOperation = 'source-over';
    }

    // HIỆU ỨNG ACTION LINES (Tia tốc độ Anime) bay vào khi đánh bùng nổ
    if (audioPeak > 0.4) {
    ...
    // Synthwave Colors: Hot Pink to Deep Blue
    ...
    // Thuật toán vẽ Lưới Chiều Sâu 3D Siêu Mượt
    ctxV.strokeStyle = "rgba(0, 255, 200, 0.15)"; // Cyan Neon
    ctxV.lineWidth = 3;
    ctxV.beginPath(); 
    
    // Đường chéo tụ về 1 điểm (Vanishing point)
    for(let x = -20; x <= 20; x++) {
        ctxV.moveTo(720, retainY);
        ctxV.lineTo(720 + x * 200, 2560);
    }
    // Đường ngang cong dần ra xa để tạo độ sâu 3D
    ...
    // --- HIỆU ỨNG 1: HẠT NĂNG LƯỢNG (BAY LƯỢN SÓNG SINE-WAVE) ---
    ...
    if (Math.random() < 0.4 && window.retentionParticles.length < 40) {
        window.retentionParticles.push({
            ...
            h: Math.random() > 0.5 ? 300 : 190, // 300 = Pink/Purple, 190 = Cyan
            age: 0
        });
    }
    ctxV.globalCompositeOperation = "screen";
    ...
    // --- HIỆU ỨNG 2: THANH MARQUEE CẢNH BÁO ---
    ...
    // --- HIỆU ỨNG 3: EMOJIS LIVESTREAM REACTION (POP-IN + SINE-WAVE) ---
    ...
    if (!window.gameOver) {
        ...
        ctxV.fillStyle = "#ff0055"; ctxV.shadowColor = "rgba(255,0,85,0.8)"; ctxV.shadowBlur = 10;
    ...
    if (!window.gameOver) {
        ...
        if (window.StoryModeAI.displayedText.length > 0) {
            ...
            // [NEW] SỬ DỤNG HÀM TIKTOK HIGHLIGHT CHO BẢN DỌC
