...
// TÂM LÝ HỌC (NEW): Intro Zoom Chat Toxic, Mưa Icon Hài Hước, Sạn cố ý, Sai chính tả mồi Comment.
// ==========================================
...
// Hàm cố tình viết sai chính tả ngẫu nhiên (Bait Comment)
...
// ====================================================================
// HÀM VẼ "SẠN" (BAIT) - Tạo tranh cãi và tăng Comment
// ====================================================================
...
// ====================================================================
// ĐẠO DIỄN: KỊCH BẢN INTRO (ICON BAY + ZOOM LIVE CHAT)
// ====================================================================
window.drawProceduralIntro = function(ctx, w, h, progress) {
    ...
    // 0 - 0.8s: P1 + Mưa Icon
    // 0.8s - 1.44s: Zoom To Khung Chat 
    // 1.44s - 2.24s: P2 + Bubbles Chat + Lore
    // 2.24s - 2.56s: VS Dash
    // 2.56s - 3.20s: Clash Glass
    if (progress < 0.25) { act = 0; localProg = progress / 0.25; } 
    else if (progress < 0.45) { act = 1; localProg = (progress - 0.25) / 0.20; } 
    else if (progress < 0.70) { act = 2; localProg = (progress - 0.45) / 0.25; } 
    else if (progress < 0.80) { act = 3; localProg = (progress - 0.70) / 0.10; } 
    else { act = 4; localProg = (progress - 0.80) / 0.20; } 
    ...
    if (!window._sfxCuts[act]) {
        ...
        if (act === 3 && typeof window.playSound === 'function') window.playSound(60, 'square', 0.5, 0.4); 
        if (act === 4 && typeof window.playSound === 'function') window.playSound(120, 'sawtooth', 0.8, 1.5, true); 
    ...
    const drawFighterArtistic = (charObj, cx, cy, forcedState, isFacingRight, theme, charName, isTrail = false) => {
        ...
        if (charName && (act === 0 || act === 2) && !isTrail) {
    ...
    if (act === 3) {
        ...
        ctx.save(); ctx.filter = `drop-shadow(25px 0px 0px red) drop-shadow(-25px 0px 0px cyan)`;
        drawFighterArtistic(targetChar, 0, 0, 'dash', isP1Flash, tTheme, "");
        ctx.restore(); ctx.globalCompositeOperation = "source-over"; drawCRTScanlines();
    ...
    if (act === 0) { currentZoom = 2.5 - localProg * 0.5; panX = (1 - localProg) * 60; cutShake = (Math.random()-0.5)*15; } 
    else if (act === 1) { currentZoom = 2.0; cutShake = (Math.random()-0.5)*5; } 
    else if (act === 2) { currentZoom = 1.8 + localProg * 0.2; panX = -(1 - localProg) * 60; } 
    else if (act === 4) { currentZoom = 1.2 + Math.pow(localProg, 3) * 1.6; if (localProg > 0.7) cutShake = (Math.random() - 0.5) * 80; }
    ...
    if (act === 0) {
        ...
        if(!window._introEmojis || window._introEmojis.length === 0) {
            ...
            for(let c=0; c<20; c++) {
                window._introEmojis.push({
                    ...
                    s: 0.1, maxS: 1 + Math.random()*2, rot: (Math.random()-0.5)*1
                });
            }
        }
        ...
        // CINEMATIC ZOOM TOXIC LIVE CHAT
        ctx.restore(); 
        ctx.save();
        ctx.translate(w/2 + cutShake, h/2 + cutShake);
        let chatZoom = 1.0 + Math.sin(localProg * Math.PI) * 0.05;
        ctx.scale(chatZoom, chatZoom);

        let boxWidth = 900; let boxHeight = 500;
        ctx.fillStyle = "rgba(10, 15, 30, 0.95)";
        ctx.strokeStyle = "rgba(0, 243, 255, 0.8)";
        ctx.lineWidth = 6;
        ctx.beginPath(); if(ctx.roundRect) ctx.roundRect(-boxWidth/2, -boxHeight/2, boxWidth, boxHeight, 20); else ctx.fillRect(-boxWidth/2, -boxHeight/2, boxWidth, boxHeight);
        ctx.fill(); ctx.stroke();
        
        ctx.fillStyle = "#ff0055"; ctx.fillRect(-boxWidth/2, -boxHeight/2, boxWidth, 70);
        ctx.fillStyle = "#fff"; ctx.font = "900 35px 'Arial Black'"; ctx.textAlign = "left"; ctx.textBaseline = "middle";
        ctx.fillText("💬 TOXIC LIVE CHAT", -boxWidth/2 + 30, -boxHeight/2 + 35);
        
        if(!window._introCinematicChats || window._introCinematicChats.length === 0) {
            window._introCinematicChats = [];
            for(let i=0; i<5; i++) window._introCinematicChats.push(window.generateLiveChatEvent());
        }
        
        ctx.textAlign = "left"; ctx.font = "bold 38px Arial";
        let chatY = -boxHeight/2 + 130;
        let numChatsToShow = Math.floor(localProg * 6); // Rớt dần từ 0 đến 5 dòng
        for(let i=0; i<numChatsToShow && i < window._introCinematicChats.length; i++) {
            let c = window._introCinematicChats[i];
            ctx.fillStyle = c.color; ctx.fillText(c.name + ":", -boxWidth/2 + 30, chatY);
            let nW = ctx.measureText(c.name + ":").width;
            ctx.fillStyle = "#ffffff"; 
            let msg = c.msg.length > 30 ? c.msg.substring(0, 30) + "..." : c.msg;
            ctx.fillText(" " + msg, -boxWidth/2 + 30 + nW, chatY);
            chatY += 70;
        }
        ctx.restore();
        ctx.save(); ctx.translate(w/2 + cutShake + panX, h/2 + cutShake); ctx.scale(currentZoom, currentZoom); ctx.translate(0, 100); 
        ...
        // P2 + BUBBLE CHAT LƠ LỬNG
        drawFighterArtistic(e1, 0, 100, 'cast', false, prm.themeP2, realP2Name);
        
        ctx.restore(); 
        ctx.save();
        ctx.translate(w/2 + cutShake, h/2 - 300 + cutShake);
        ...
        ctx.beginPath(); if(ctx.roundRect) ctx.roundRect(-400, 0, 350, 60, 30); else ctx.fillRect(-400, 0, 350, 60); ctx.fill();
        ctx.fillStyle = "#000"; ctx.fillText(window._p2Bubbles[0].substring(0, 20) + "...", -225, 30);
        
        // Bubble phải
        if (localProg > 0.4) {
            ctx.fillStyle = "rgba(255,255,255,0.95)"; 
            ctx.beginPath(); if(ctx.roundRect) ctx.roundRect(50, 150, 350, 60, 30); else ctx.fillRect(50, 150, 350, 60); ctx.fill();
            ctx.fillStyle = "#000"; ctx.fillText(window._p2Bubbles[1].substring(0, 20) + "...", 225, 180);
        }
        ctx.restore();
        ctx.save(); ctx.translate(w/2 + cutShake + panX, h/2 + cutShake); ctx.scale(currentZoom, currentZoom); ctx.translate(0, 100); 
    } 
    else if (act === 4) {
        // CLASH (VỠ KÍNH VÀ LAO VÀO)
        let easeIn = Math.pow(localProg, 4); 
        let pushP1 = -400 + (easeIn * 280); let pushP2 = 400 - (easeIn * 280);
        
        for(let t=1; t<=3; t++) {
            let prevProg = Math.max(0, localProg - t*0.05); let prevEaseIn = Math.pow(prevProg, 4);
            let trP1 = -400 + (prevEaseIn * 280); let trP2 = 400 - (prevEaseIn * 280);
        ...
        for(let i=0; i<35; i++) {
            ...
            ctx.fillStyle = Math.random() > 0.4 ? "#fff" : prm.themeP1.aura; ctx.globalAlpha = (1 - localProg) * (0.4 + speed*0.6);
            ctx.save(); ctx.translate(Math.cos(ang)*dist, Math.sin(ang)*dist); ctx.rotate(ang); ctx.fillRect(0, 0, size * 5, size); ctx.restore();
        }
    ...
    let barHeight = 0;
    if (act === 0 || act === 2) barHeight = 180 + (localProg * 40); 
    else if (act === 3) barHeight = 220 + (localProg * 80); 
    else if (act === 4) barHeight = 300 - (localProg * 1200); 
    barHeight = Math.max(0, barHeight);

    if (barHeight > 0) { ctx.fillStyle = "#000000"; ctx.fillRect(0, 0, w, barHeight); ctx.fillRect(0, h - barHeight, w, barHeight); }

    // Wager Badge
    if (barHeight > 0 && prm.badge && act !== 0 && act !== 1) { 
        ctx.save(); ctx.textAlign = "center"; ctx.textBaseline = "middle"; ctx.font = "900 22px 'Arial Black', sans-serif";
        ctx.shadowColor = "#ffeb3b"; ctx.shadowBlur = 10; ctx.fillStyle = "#ffeb3b"; ctx.fillText(prm.badge, w/2, barHeight - 35); ctx.restore();
    }

    // Lore Chapter (Typo Bait)
    if (act === 2 && barHeight > 0) {
        ctx.save(); let chapterX = w/2 + (1-Math.pow(localProg, 0.3)) * 500; 
        ctx.textAlign = "center"; ctx.textBaseline = "middle"; ctx.globalAlpha = Math.min(1, localProg * 3);
        ctx.fillStyle = prm.themeP2.aura; ctx.font = "italic 900 40px 'Arial Black', 'Noto Sans CJK SC', 'Noto Sans KR', sans-serif"; 
        ctx.letterSpacing = "3px"; ctx.fillText(prm.lore.chapter, chapterX, barHeight / 2); ctx.restore();
    }

    if ((act === 0 || act === 2) && barHeight > 0) {
        ctx.save(); let subText = act === 0 ? prm.lore.p2Sub : prm.lore.p1Sub; ctx.globalAlpha = Math.min(1, localProg * 5); 
        ctx.textAlign = "center"; ctx.textBaseline = "bottom"; ctx.font = "italic bold 42px 'Arial', sans-serif"; 
        let subY = h - barHeight + 85; ctx.lineWidth = 5; ctx.strokeStyle = "#000"; ctx.strokeText(`"${subText}"`, w/2, subY);
        ctx.fillStyle = "#f1c40f"; ctx.fillText(`"${subText}"`, w/2, subY); ctx.restore();
    }

    drawCRTScanlines();

    if (act === 4 && localProg > 0.75) {
        let shatterProg = (localProg - 0.75) * 4; 
        ctx.save(); ctx.translate(w/2, h/2); ctx.strokeStyle = "#00f3ff"; ctx.lineWidth = 8; ctx.shadowColor = "#00f3ff"; ctx.shadowBlur = 20;
        for(let i=0; i<12; i++) {
        ...
        if (shatterProg > 0.5) { let whiteFlash = (shatterProg - 0.5) * 2; ctx.fillStyle = `rgba(255, 255, 255, ${whiteFlash})`; ctx.fillRect(-w, -h, w*2, h*2); }
        ctx.restore();
    }
...
window.startRecording = function() {
    ...
    window._introChatSpam = []; window._baitTriggered = false; window._baitType = -1;
...
window.captureFrames = function() {
    ...
    if (renderNormalV) {
        ...
        // VẼ SẠN CHỐNG CHỈ ĐỊNH LÊN TRÊN GAME (Nhưng dưới HUD)
        ...
        // =========================================================
        // WATERMARK ĐÓNG DẤU BẢN QUYỀN (GIỮA MÀN HÌNH, CHÌM MỜ)
        // =========================================================
        ...
        if (!isOutroActive) {
            ...
            if (!window._chatSystemInit) { window._chatSystemInit = true; window._liveChats = []; window._lastChatUpdate = Date.now(); window._nextChatDelay = 1000; window._fakeViewers = 350 + Math.floor(Math.random() * 500); for(let i = 0; i < 8; i++) { let c = window.generateLiveChatEvent(); window.precalcChatText(c, ctxV); window._liveChats.push(c); } }
            ...
            ctxV.fillStyle = "#ff0055"; ctxV.fillRect(-boxWidth/2, -boxHeight/2, boxWidth, 70); ctxV.fillStyle = "#fff"; ctxV.font = "900 36px 'Arial Black'"; ctxV.textAlign = "left"; ctxV.textBaseline = "middle"; 
            let brandName = window.CREATOR_HANDLE || "VIETNAM SPACEX";
            ctxV.fillText("🚀 " + brandName + " CHAT", -boxWidth/2 + 30, -boxHeight/2 + 35); ctxV.font = "900 28px 'Arial'"; ctxV.textAlign = "right"; ctxV.fillText("👥 " + window._fakeViewers.toLocaleString() + " VIEWERS", boxWidth/2 - 30, -boxHeight/2 + 35);
