// ==========================================
// RECORDER.JS - BẢN HOÀN HẢO CHO CONTENT CREATOR (V49.0 - CONVERSION OPTIMIZED)
...
// [NEW] OUTRO CTA FUNNEL: Tự động hiện màn hình "Tạo game của bạn -> Link in bio" khi hết trận
// ==========================================
...
function renderMobileSafeCombo(ctx, fighter, xPos, yPos, align, isMobile) {
    if (fighter && fighter.comboHits >= 2) {
        let scale = isMobile ? 1.4 : 1; // Trên đt sẽ to hơn cho dễ nhìn
...
// ==========================================
// RENDER KHUNG HÌNH CHÍNH (ĐÃ THÊM CTA VÀ COMBO)
// ==========================================
window.captureFrames = function() {
    ...
    // =====================================
    // VẼ HUD MÁU & CĂN CHỈNH COMBO THÔNG MINH
    // =====================================
    ...
    if (window.p1 && !window.gameOver && window.introTimer <= 120) {
        ...
        // HUD DỌC (CẮT DƯỚI ĐỂ VỪA VỚI MOBILE)
    ...
    // VẼ COMBO CHO MOBILE (CĂN VÀO TRONG, KHÔNG BỊ CẮT)
    if (!window.gameOver) {
    ...
    // =====================================
    // NÚT SUBSCRIBE 🔴 (THÚC ĐẨY CHUYỂN ĐỔI)
    // =====================================
    if (!window.gameOver) {
        ctxV.save();
        ctxV.translate(540, 240); // Đặt ở vị trí mắt người xem dễ nhìn thấy nhất trên TikTok
        let subScale = 1 + (audioPeak * 0.12); 
        ctxV.scale(subScale, subScale);
        
        ctxV.fillStyle = "#ff0000"; // Màu đỏ YouTube chuẩn
    ...
    // =====================================
    // MÀN HÌNH CHỐT SALE CUỐI VIDEO (OUTRO CTA FUNNEL)
    // Kêu gọi người xem truy cập link để tạo game
    // =====================================
    if (window.gameOver && window.matchEndTimer > 60) {
        let outroAlpha = Math.min(1, (window.matchEndTimer - 60) / 40); // Mờ dần vào trong 40 frames
        
        let drawOutroCTA = (ctx, w, h, isMobile) => {
            ctx.save();
            ctx.globalAlpha = outroAlpha;
            ctx.fillStyle = "rgba(10, 13, 20, 0.95)"; // Nền tối làm nổi bật Text
            ...
            ctx.font = `italic 900 ${isMobile ? 70 : 80}px 'Arial Black', sans-serif`;
            
            if (isMobile) {
                ctx.fillText("WANT TO MAKE", cx, cy - 250);
                ctx.fillText("GAMES LIKE THIS?", cx, cy - 170);
            } else {
                ctx.fillText("WANT TO MAKE GAMES LIKE THIS?", cx, cy - 120);
            }

            // Chữ hướng dẫn nhỏ hơn
            ctx.shadowBlur = 0;
            ctx.fillStyle = "#ffffff";
            ctx.font = `bold ${isMobile ? 45 : 40}px 'Montserrat', sans-serif`;
            ctx.fillText("Check the Link in Bio / Comments 👇", cx, cy - 20);

            // Vẽ một nút bấm giả (Mock Button) để thu hút click
            let btnWidth = isMobile ? 450 : 400;
            let btnHeight = isMobile ? 110 : 90;
            let btnY = cy + 100;
            
            // Nút phát sáng nhấp nháy theo âm thanh
            let btnPulse = 1 + (audioPeak * 0.05);
