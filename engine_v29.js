// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 27.0 [THE ESPORTS STADIUM]
// [NEW] 2.5D FOREGROUND DEBRIS: Mảnh vỡ văng thẳng vào ống kính, làm mờ Depth of Field.
// [NEW] IMPACT CHROMATIC ABERRATION: Tách màu quang học bạo lực ngay khoảnh khắc chạm đòn.
// [NEW] LETHAL VOID FINISHER: Tắt sáng toàn bộ thế giới khi K.O, chỉ giữ lại nhân vật.
// [NEW] DISTORTED REFLECTION: Mặt sàn phản chiếu bị bóp méo bởi sóng xung kích.
// ==========================================
...
// [VIRAL 27.0] 2.5D FOREGROUND DEBRIS (Bay vào ống kính)
window.spawnForegroundDebris = function(x, y, isRight) {
    let count = 6 + Math.floor(Math.random() * 4);
    for(let i=0; i<count; i++) {
        let vx = (isRight ? 1 : -1) * (15 + Math.random() * 20); // Bắn mạnh theo lực
        let vy = -10 - Math.random() * 15;
        let scaleSpeed = 1.05 + Math.random() * 0.05; // To dần vì bay về phía màn hình
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce, attackerDirRight = true) {
    ...
    if (target.state === 'block') { 
        if (target.stamina < 15) {
            ...
            window.floatingTexts.push({ x: target.x, y: target.y - 90, text: "💔 GUARD CRUSH!", color: "#ff003c", alpha: 1, vx: 0, vy: -3, font: "900 45px Arial", life: 60 });
            
            window.actionCamOffsetX = 0; window.actionCamOffsetY = -30;
            window.targetZoom = 1.5; window.dynamicBlur = 10;
            window.hitStopFrames = 25; 
            window.impactAberration = 25; // [VIRAL 27.0] Tách màu lúc vỡ khiên
            window.chromaTimer = 20; window.vhsGlitchTimer = 20;
            window.spawnForegroundDebris(target.x, target.y, attackerDirRight); // Mảnh khiên bắn vào ống kính
        } 
            ...
            window.floatingTexts.push({ x: target.x, y: target.y - 80, text: "🛡️ PERFECT PARRY!", color: "#00ffff", alpha: 1, vx: 0, vy: -3, font: "900 40px Arial", life: 50 });
    ...
    if (finalDmg > 0) {
        ...
        if (isCrit) {
            ...
            window.targetZoom = 1.45; // Co vào sâu hơn
            setTimeout(() => { window.targetZoom = 1.1; }, 100); 
            
            // [VIRAL 27.0] CHROMATIC ABERRATION VÀ MẢNH VỠ 3D
            ...
            window.impactAberration = 4; // Rách màu cực nhẹ khi dính đòn thường
        }
        ...
        if (target.hp <= 0) {
            ...
            if (isNearWall && isCrit && typeof window.triggerStageTransition === 'function') { window.triggerStageTransition(target); } 
                ...
                if (isCrit) {
                    ...
                    window.floatingTexts.push({ x: target.x, y: target.y - 120, text: "💀 LETHAL FINISH!", color: "#ff003c", alpha: 1, vx: 0, vy: -1, font: "900 75px Arial", life: 90 });
                    ...
                    // [VIRAL 27.0] KÍCH HOẠT LETHAL VOID (Tắt đèn toàn bản đồ)
                    window.lethalVoid = 80;
                    window.impactAberration = 40; 
                    window.spawnForegroundDebris(target.x, target.y, attackerDirRight);
                    window.spawnForegroundDebris(target.x, target.y, attackerDirRight); // Spawn double for K.O.
...
window.attack = function(attacker, targetGroup) {
    ...
    if (isFinisher) {
        isCrit = true; finalDmg = baseDmg * 3.5; window.shakeScreen(20, 15); target.vx = (attacker.isFacingRight ? 6 : -6); target.state = 'hurt'; target.hitStun = 50; window.spawnParticles(target.x, target.y - 40, "#ff4757", true); window.floatingTexts.push({ x: target.x, y: target.y - 80, text: "💥", color: "#ff4757", alpha: 1, vx: (Math.random()-0.5)*2, vy: -4, font: "900 55px Arial", life: 50 });
        window.speedLinesAlpha = 1.0; window.actionCamOffsetX = attacker.isFacingRight ? 100 : -100; 
    } else {
        if (Math.random() < attacker.critChance) { isCrit = true; finalDmg = baseDmg * attacker.critMult; window.floatingTexts.push({ x: target.x + (Math.random()*40-20), y: target.y - 60, text: "💢", color: "#f1c40f", alpha: 1, vx: 0, vy: -2, font: "italic 900 35px Arial", life: 30 }); } else { window.playSound(350, 'sine', 0.1, 0.1, false); }
            ...
            for (let j = i + 1; j < allFighters.length; j++) { 
                ...
                if (dist < 80 && f1.attackTimer > 5 && f1.attackTimer < 20 && f2.attackTimer > 5 && f2.attackTimer < 20) {
                    if (!f1.clashCooldown && !f2.clashCooldown && f1.isFacingRight !== f2.isFacingRight) {
                        ...
                        window.impactAberration = 25; // [VIRAL 27.0] Clash Split
...
window.draw = function() {
    ...
    // [VIRAL 27.0] IMPACT CHROMATIC ABERRATION (Tách màu chấn động khi vũ khí chạm thịt)
    let impactShift = 0;
    ...
    try {
        ...
        // [VIRAL 27.0] ABERRATION SHIFT (Tách màu lúc va chạm)
        if (impactShift !== 0) { window.ctx.translate(impactShift, 0); }
        ...
        // [VIRAL 27.0] LETHAL VOID (XÓA BỎ NỀN KHI KẾT LIỄU TẠO ĐỘ LỰC)
        ...
        // [VIRAL 27.0] DISTORTED FLOOR REFLECTION (RUNG ĐỘNG SÀN KÍNH)
        window.ctx.save();
        ...
        // Tạo biến dạng dập dềnh khi màn hình rung
        ...
        // [VIRAL 27.0] VẼ MẢNH VỠ 3D BĂNG XUYÊN MÀN HÌNH (DEPTH OF FIELD)
        if (window.foregroundDebris && window.foregroundDebris.length > 0) {
            window.ctx.save();
            window.ctx.filter = 'blur(4px)'; // Bị mờ do bay sát vào camera (Bokeh effect)
            window.foregroundDebris.forEach(d => {
                window.ctx.save();
        ...
        // [VIRAL 27.0] CHROMATIC ABERRATION KHI CHẠM ĐÒN (RGB SPLIT)
        if (impactShift !== 0) {
            window.ctx.save();
...
// ==========================================
// 7. [VIRAL 27.0] AFTER EFFECTS TIME-RAMPING & UNLOCKED 120Hz LOOP
// ==========================================
