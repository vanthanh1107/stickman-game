...
try {
    if (typeof firebase !== 'undefined') {
        const firebaseConfig = { 
            apiKey: "AIzaSyDZ1g9V9K9X4gWcBRsGkDEN9OEnWuKgXzg", 
            authDomain: "vietnamspacex-be507.firebaseapp.com", 
            databaseURL: "https://vietnamspacex-be507-default-rtdb.asia-southeast1.firebasedatabase.app", 
            projectId: "vietnamspacex-be507" 
        };
    ...
    if (database) {
        database.ref('players/' + currentUid).once('value').then(async (snapshot) => {
            if(snapshot.exists()) { 
                ...
                currentPlayer.elo = parseInt(data.elo) || 1000; 
                currentPlayer.coins = parseInt(data.coins) || 0; 
    ...
    database.ref('players').on('value', (snapshot) => {
        ...
        latestPlayersData.forEach((p, idx) => { 
            if (displayCount < 10) { 
                ...
                globalHTML += `<div class="rank-item ${topClass}"><span><b>#${idx+1}</b> ${flag} ${p.name}</span><span>⚔️ ${parseInt(p.elo)||1000}</span></div>`; 
                displayCount++; 
            } 
        });
...
function renderCountryPlayers() { 
    ...
    filteredPlayers.forEach((p, idx) => { 
        let topClass = (idx === 0) ? "top1" : ""; 
        let flag = getFlagEmoji(p.countryCode); 
        countryHTML += `<div class="rank-item ${topClass}"><span><b>#${idx+1}</b> ${flag} ${p.name}</span><span>⚔️ ${parseInt(p.elo)||1000}</span></div>`; 
    }); 
...
function parseCSVData(csvText) {
    ...
    for (let i = 1; i < result.length; i++) {
        ...
        if (rowObj.id) {
            ...
            let drawMethod = null; 
            try { if (rowObj.drawCode) drawMethod = new Function('ctx', 'p', 'bounce', 'ext', 'pext', 'isTrail', rowObj.drawCode); } 
            catch (e) { console.error("Lỗi biên dịch drawCode:", e); }
