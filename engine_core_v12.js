...
// [BẢN HOÀN CHỈNH ULTIMATE: FLUID COMBAT - XÓA BỎ 100% KHỰNG HÌNH & SLOW MOTION]
// ==========================================
...
window.triggerTimeStop = function(frames, caster) { 
    // ĐÃ XÓA TIMESTOP SLOW-MOTION - MỌI THỨ VẪN CHẠY MƯỢT MÀ THEO THỜI GIAN THỰC!
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce, attackerDirRight = true) {
    ...
    if (target.state === 'dash' || target.state === 'dash_back') {
        if (target.dashTimer > 2 && target.dashTimer < 14) {
            ...
            window.floatingTexts.push({ x: target.x, y: target.y - 80, text: "⚡ EVADED!", color: "#00f3ff", alpha: 1.5, vx: (Math.random()-0.5)*6, vy: -10, font: "italic 900 35px 'Arial Black', Impact", life: 70, scale: 2.5, targetScale: 1.0, scaleVel: 0, rot: (Math.random()-0.5)*0.3 });
    ...
    if (target.state === 'block') { 
        if (target.stamina < 15) {
            ...
            window.floatingTexts.push({ x: target.x, y: target.y - 90, text: "💔 CRUSH!", color: "#ff003c", alpha: 1.5, vx: (Math.random()-0.5)*4, vy: -10, font: "italic 900 45px 'Arial Black', Impact", life: 70, scale: 4.0, targetScale: 1.2, scaleVel: 0, rot: (Math.random()-0.5)*0.4 });
            ...
            // Đã xóa Hit-stop khi Guard Break
            ...
            window.floatingTexts.push({ x: target.x, y: target.y - 80, text: "🛡️ PARRY!", color: "#00ffff", alpha: 1.5, vx: (Math.random()-0.5)*4, vy: -9, font: "italic 900 40px 'Arial Black', Impact", life: 70, scale: 3.0, targetScale: 1.0, scaleVel: 0, rot: (Math.random()-0.5)*0.2 });
    ...
    if (finalDmg > 0) {
        ...
        let textVx = (attackerDirRight ? 1 : -1) * (Math.random() * 6 + 3); let textVy = -9 - Math.random() * 6; let initialScale = isCrit ? 4.0 : 2.0; let targetScale = isCrit ? 1.2 : 1.0; 
        window.floatingTexts.push({ x: target.x, y: target.y - 60, text: dmgText, color: color || (isCrit ? "#ff4757" : "#fff"), alpha: 1.5, vx: textVx, vy: textVy, font: isCrit ? "italic 900 40px 'Arial Black', Impact" : "italic 900 30px 'Arial Black', Impact", life: 80, scale: initialScale, targetScale: targetScale, scaleVel: 0, rot: (Math.random()-0.5)*0.3 });
        ...
        if (isCrit) {
            ...
            // XÓA SLOW-MOTION CHÍ MẠNG VÀ HIT STOP ĐỂ GAME MƯỢT 100%
        ...
        if (target.hp <= 0) {
            ...
            if (isNearWall && isCrit && typeof window.triggerStageTransition === 'function') { window.triggerStageTransition(target); } 
                ...
                if (isCrit) {
                    ...
                    window.floatingTexts.push({ x: target.x, y: target.y - 120, text: "💀 K.O!", color: "#ff003c", alpha: 1.5, vx: (Math.random()-0.5)*6, vy: -12, font: "italic 900 65px 'Arial Black', Impact", life: 100, scale: 4.0, targetScale: 1.2, scaleVel: 0, rot: (Math.random()-0.5)*0.2 });
                ...
                } else { window.shakeScreen(20, 15); window.targetZoom = 1.4; window.actionCamOffsetY = -30; window.impactAberration = 15; } // Đã xóa slow mo khi chết thường
...
window.update = function() {
    ...
    // 🟢 XÓA CHẶN RETURN Ở ĐÂY CHO CINEMATIC HOẠT ĐỘNG SONG SONG!
    ...
    allFighters.forEach(f => {
        ...
        if (!launchedUltimate && f.attackTimer <= 0 && f.hitStun <= 0 && f.dashTimer <= 0 && f.stunTimer <= 0 && !window.gameOver && f.hp > 0) {
            if (f.isDragon) {
                if (f.hp > 0 && f.hp <= f.maxHp * 0.3 && !f.isEvolved) { f.isEvolved = true; window.screenFlash = 0; window.shakeScreen(50, 15); window.playSound(50, 'sawtooth', 2.0, 1.0, true); f.color = "#8e44ad"; f.scale *= 1.25; window.floatingTexts.push({ x: f.x, y: f.y - 150, text: "🐉🔥", color: "#8e44ad", alpha: 1.5, vx: 0, vy: -5, font: "italic 900 60px 'Arial Black', Impact", life: 100, scale: 3.0, targetScale: 1.0, scaleVel: 0, rot: 0 }); window.shockwaves.push({x: f.x, y: window.GROUND_Y, r: 10, maxR: 500, color: "#8e44ad", alpha: 1, speed: 25}); window.targetCamOrbitAngle = 0.5; }
        ...
        for (let j = i + 1; j < allFighters.length; j++) { 
            ...
            if (dist < 80 && f1.attackTimer > 5 && f1.attackTimer < 20 && f2.attackTimer > 5 && f2.attackTimer < 20) {
                ...
                if (!f1.clashCooldown && !f2.clashCooldown && f1.isFacingRight !== f2.isFacingRight) {
                    ...
                    window.floatingTexts.push({ x: midX, y: midY - 60, text: "⚔️ BRAWL!", color: "#f1c40f", alpha: 1.5, vx: (Math.random()-0.5)*6, vy: -7, font: "italic 900 35px 'Arial Black', Impact", life: 60, scale: 2.0, targetScale: 1.0, scaleVel: 0, rot: (Math.random()-0.5)*0.1 });
                }
            }
            ...
            if (hit.delay <= 0) {
                ...
                if (target && target.hp > 0 && Math.abs(f.x - target.x) <= reach + 35) {
                    ...
                    if (hit.isFinisher) {
                        ...
                        window.floatingTexts.push({ x: target.x, y: target.y - 80, text: "💥", color: "#ff4757", alpha: 1.5, vx: (Math.random()-0.5)*4, vy: -8, font: "900 65px Arial", life: 70, scale: 2.5, targetScale: 1.0, scaleVel: 0, rot: (Math.random()-0.5)*0.3 });
                        ...
                        if (Math.random() < f.critChance) { 
                            ...
                            window.floatingTexts.push({ x: target.x + (Math.random()*40-20), y: target.y - 60, text: "💢", color: "#f1c40f", alpha: 1.5, vx: (Math.random()-0.5)*4, vy: -6, font: "italic 900 35px 'Arial Black', Impact", life: 60, scale: 2.0, targetScale: 1.0, scaleVel: 0, rot: 0 }); 
                        } else { window.playSound(350, 'sine', 0.1, 0.1, false); }
                        if ((f.state === 'low_kick' || f.state === 'teep_kick') && Math.random() < 0.4) { target.stunTimer = 35; target.state = 'stunned'; window.floatingTexts.push({ x: target.x, y: target.y - 50, text: "🦵", color: "#e67e22", alpha: 1.5, vx: (Math.random()-0.5)*4, vy: -5, font: "900 35px Arial", life: 60, scale: 1.8, targetScale: 1.0, scaleVel: 0, rot: 0 }); }
                        if ((f.state === 'uppercut' || f.state === 'elbow_strike') && Math.random() < 0.3) { finalDmg *= 1.5; window.spawnParticles(target.x, target.y - 60, "#c0392b", true); window.floatingTexts.push({ x: target.x, y: target.y - 50, text: "🩸", color: "#c0392b", alpha: 1.5, vx: (Math.random()-0.5)*4, vy: -5, font: "900 35px Arial", life: 60, scale: 1.8, targetScale: 1.0, scaleVel: 0, rot: 0 }); }
