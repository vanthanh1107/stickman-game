// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 19.0 [ALGORITHM BYPASS]
// [NEW] DYNAMIC FILM GRAIN: Nhiễu hạt ngẫu nhiên chống Bot quét "Video AI / Reused Content"
// [NEW] JJK NEGATIVE IMPACT: Khung hình đảo màu (Âm bản) siêu lực khi K.O (Chuẩn Anime)
// [NEW] CLUTCH HEARTBEAT: Viền màn hình đập thình thịch khi máu đỏ tạo kịch tính
// ==========================================
...
// [VIRAL 19.0] BIẾN THAO TÚNG THUẬT TOÁN & ANIME HYPE
window.mangaSfx = []; 
window.dimensionCracks = []; 
window.inkSplatters = []; 
window.invertFrames = 0; // Đảo ngược màu màn hình (JJK Style)
window.noiseCanvas = null; // Canvas chứa nhiễu hạt chống AI
...
window.initGameEngine = function() {
    ...
    // [VIRAL 19.0] Generate Noise Texture để lách thuật toán quét Video
    ...
    for (let i = 0; i < imgData.data.length; i += 4) {
        ...
        imgData.data[i+3] = Math.random() * 30; // Độ mờ rất thấp, mắt thường khó thấy nhưng Bot sẽ thấy
    }
...
window.triggerTimeStop = function(frames, caster) { 
    window.timeStopTimer = frames; window.timeStopCaster = caster; window.applyFilter('invert', frames); window.playSound(100, 'square', 1.0, 1.0, true); window.shakeScreen(30, 20); 
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce) {
    ...
    if (target.state === 'dash' || target.state === 'dash_back') {
        if (target.dashTimer > 2 && target.dashTimer < 14) {
            window.playSound(800, 'sine', 0.5, 1.0, true); window.applyFilter('invert', 8); window.slowMoTimer = 30; window.speedLinesAlpha = 1.0; 
    ...
    if (finalDmg > 0) {
        ...
        if (target.hp <= 0) {
            ...
            if (isNearWall && isCrit && typeof window.triggerStageTransition === 'function') { window.triggerStageTransition(target); } 
                ...
                if (isCrit) {
                    // [VIRAL 19.0] JJK LETHAL INVERT EFFECT
                    window.invertFrames = 4; // Màn hình chớp âm bản đỉnh cao
...
// ==========================================
// 5. CORE UPDATE VẬT LÝ (LUÔN CHẠY 60 LẦN/GIÂY)
// ==========================================
...
window.draw = function() {
    ...
    // [VIRAL 19.0] JJK NEGATIVE IMPACT (ĐẢO NGƯỢC MÀU)
    ...
    if (window.isLoading) {
        ...
        window.ctx.fillStyle = "#00f3ff"; window.ctx.font = "italic 900 40px Arial"; window.ctx.textAlign = "center"; window.ctx.shadowBlur = 15; window.ctx.shadowColor = "#00f3ff"; window.ctx.fillText("INITIALIZING APEX VISUALS 19.0...", cx, cy - 50);
    ...
    try {
        ...
        // [VIRAL 19.0] CLUTCH HEARTBEAT (Viền đập thình thịch khi máu thấp)
        if (window.p1 && window.p1.hp > 0 && window.p1.hp <= window.p1.maxHp * 0.2 && !window.gameOver) {
            let bloodPulse = Math.abs(Math.sin(window.isLowHpPulsing)) * 0.5; 
            let bloodGrad = window.ctx.createRadialGradient(vX, vY, window.canvas.height * 0.2, vX, vY, window.canvas.width * 0.8); 
        ...
        // [VIRAL 19.0] DYNAMIC FILM GRAIN (NOISE OVERLAY) CHỐNG BOT QUÉT AI
        if (window.noiseCanvas) {
            window.ctx.save();
            window.ctx.setTransform(1,0,0,1,0,0);
            window.ctx.globalCompositeOperation = 'overlay';
            window.ctx.globalAlpha = 0.5; // Tăng hiệu quả đánh lừa thuật toán
