// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 41.0 [THE HYPER-FLUID COMBAT]
// [FIXED] Phục hồi toàn bộ Animation vung tay/chân (Xóa bỏ Object Clone lỗi gây cứng đơ nhân vật).
// [NEW] HYPER-SPEED ESCALATION: Combo càng cao tốc độ đòn đánh càng nhanh, không bị khựng lâu.
// [NEW] KINETIC COMBAT FLOW: Hit-stop giảm xuống mức cực ngắn chuẩn eSports, đánh liên tục như súng máy.
// [BASE] Matrix 360 Orbit Camera, Ground Bounce, Lethal Void, RTX Floor, Kinetic Text.
// ==========================================
...
window.initGameEngine = function() {
    ...
    window.viralColorGrade = { contrast: 110 + Math.random() * 15, brightness: 90 + Math.random() * 5, saturation: 100 + Math.random() * 20, hue: Math.floor((Math.random() - 0.5) * 20) };
    ...
    let loadInterval = setInterval(() => { window.loadProgress += Math.random() * 5 + 2; if (window.loadProgress >= 100) { window.loadProgress = 100; clearInterval(loadInterval); setTimeout(() => { window.isLoading = false; window.playSound(800, 'sine', 0.5, 0.5); }, 300); } }, 30);
};
...
window.spawnScreenInk = function(color) { for(let i=0; i<3; i++) { window.screenInks.push({ x: Math.random() * (window.canvas ? window.canvas.width : 800), y: Math.random() * (window.canvas ? window.canvas.height : 600), color: color, life: 120, maxLife: 120, scale: Math.random() * 2 + 1, slideY: 0 }); } }
...
window.spawnParticles = function(x, y, color, isCrit = false) { if (window.particles.length > MAX_PARTICLES) return; let count = isCrit ? 15 : 6; for(let i=0; i<count; i++) { let angle = Math.random() * Math.PI * 2; let speed = Math.random() * (isCrit?20:10) + 2; window.particles.push({ x: x, y: y - 30, vx: Math.cos(angle)*speed, vy: Math.sin(angle)*speed, life: 35, maxLife: 35, color: color, size: Math.random() * 5 + 2 }); } }
...
window.triggerDimensionShatter = function(x, y) {
    ...
    for(let i = 0; i < numMainCracks; i++) {
        ...
        let len = 350 + Math.random() * 300; let branches = []; let points = [{x: cx, y: cy}];
        let currX = cx; let currY = cy; let dist = 0; let currentAng = baseAng;
        while(dist < len) {
            let step = 30 + Math.random() * 40; dist += step; currentAng += (Math.random() - 0.5) * 0.9; 
            currX += Math.cos(currentAng) * step; currY += Math.sin(currentAng) * step; points.push({x: currX, y: currY});
            if (Math.random() < 0.45) {
                let subPoints = [{x: currX, y: currY}]; let subAng = currentAng + (Math.random() > 0.5 ? 1 : -1) * (0.6 + Math.random()*0.8);
                ...
                while(sDist < subLen) {
                    ...
                    sX += Math.cos(subAng) * sStep; sY += Math.sin(subAng) * sStep; subPoints.push({x: sX, y: sY});
...
window.triggerStageTransitionEffect = function() {
    ...
    window.screenFlash = 2.0; window.playSound(600, 'sawtooth', 1.0, 1.0, true);
    ...
    const lightingThemes = [ { mix: 'overlay', color1: 'rgba(255, 120, 30, 0.3)', color2: 'rgba(10, 0, 20, 0.7)' }, { mix: 'hard-light', color1: 'rgba(0, 180, 255, 0.2)', color2: 'rgba(0, 5, 30, 0.8)' }, { mix: 'color-burn', color1: 'rgba(255, 30, 30, 0.2)', color2: 'rgba(0, 0, 0, 0.8)' }, { mix: 'difference', color1: 'rgba(255, 255, 255, 0.1)', color2: 'rgba(0, 0, 0, 0.9)' } ];
    window.globalIllumination = lightingThemes[Math.floor(Math.random() * lightingThemes.length)];
}
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce, attackerDirRight = true) {
    ...
    if (target.state === 'dash' || target.state === 'dash_back') {
        if (target.dashTimer > 2 && target.dashTimer < 14) {
            ...
            target.stamina = 100; target.x += target.isFacingRight ? -150 : 150; target.iFrames = 60; target.state = 'idle'; 
    ...
    if (target.state === 'block') { 
        if (target.stamina < 15) {
            ...
            window.targetZoom = 1.5; window.dynamicBlur = 10; window.hitStopFrames = 10; window.impactAberration = 15; window.chromaTimer = 20; window.vhsGlitchTimer = 20; window.spawnForegroundDebris(target.x, target.y, attackerDirRight); 
        } 
            ...
            target.stamina = Math.min(100, target.stamina + 30); window.shakeScreen(12, 8); window.hitStopFrames = 5; window.chromaTimer = 10; window.impactAberration = 10;
    ...
    if (finalDmg > 0) {
        ...
        if (isCrit) {
            window.screenFlash = 0.15; window.impactFrameCount = 1; window.targetZoom = 1.25; setTimeout(() => { window.targetZoom = 1.1; }, 100); 
            ...
            window.spawnLightning(target.x, target.y-40, color || "#f1c40f", 30, 3); window.spawnSpaceRipple(target.x, target.y - 40, color || "#f1c40f");
        ...
        if (target.hp <= 0) {
            ...
            if (isNearWall && isCrit && typeof window.triggerStageTransition === 'function') { window.triggerStageTransition(target); } 
                ...
                if (isCrit) {
                    window.screenFlash = 0.4; window.invertFrames = 3; window.speedLinesAlpha = 1.0; window.spawnLensFlare(target.x, target.y - 40, "#ff003c", 5.0);
                    ...
                    window.targetZoom = 1.8; window.actionCamOffsetX = 0; window.actionCamOffsetY = -20; window.dynamicBlur = 25; window.cameraTilt = (Math.random() > 0.5 ? 1 : -1) * 0.15; 
                    window.lethalVoid = 100; window.impactAberration = 30; window.spawnForegroundDebris(target.x, target.y, attackerDirRight); window.spawnForegroundDebris(target.x, target.y, attackerDirRight); 
                    window.hitStopFrames = 30; window.shakeScreen(50, 35); window.vhsGlitchTimer = 45; 
                    window.triggerDimensionShatter(target.x, target.y - 50); window.spawnBlackHole(target.x, target.y, 400, 150); window.spawnEnergyPillar(target.x, window.GROUND_Y, "#ff003c", 100, 80); window.spawnSpaceRipple(target.x, target.y - 40, "#ff003c");
                ...
                } else { window.hitStopFrames = 10; window.shakeScreen(20, 15); window.targetZoom = 1.4; window.actionCamOffsetY = -30; window.targetTimeScale = 0.25; window.impactAberration = 15; setTimeout(()=>{window.targetTimeScale = 1.0; }, 900); }
...
window.attack = function(attacker, targetGroup) {
    ...
    if (MathDist > reach && !isFinisher) { 
        ...
        window.speedLinesAlpha = 0.8; window.actionCamOffsetX = attacker.isFacingRight ? 50 : -50; attacker.iFrames = 8; window.spawnParticles(attacker.x, attacker.y, "#bdc3c7", true); return; 
    }
    ...
    if (isFinisher) { window.spawnEyeFlash(attacker.x, attacker.y - 60, "#ff4757", attacker.isFacingRight); window.hitStopFrames = 10; }

    // [VIRAL 41.0] HYPER SPEED ESCALATION (Combo càng nhiều đánh càng nhanh)
    ...
    // [VIRAL 41.0] Giảm triệt để Hit-lag để combat liên tục
...
window.update = function() {
    ...
    if (!isTimeStopped) {
        ...
        for (let i = window.impactSparks.length - 1; i >= 0; i--) { 
            ...
            isp.life--; if (isp.zSpeed) { isp.scale *= isp.zSpeed; } 
            if (isp.y >= window.GROUND_Y) { isp.y = window.GROUND_Y; isp.vy *= -0.5; isp.vx *= 0.8; }
            if (isp.life <= 0) window.impactSparks.splice(i, 1); 
        }
        
        for (let i = window.particles.length - 1; i >= 0; i--) { 
            let pt = window.particles[i]; 
            if (activeBlackHole) { let dx = activeBlackHole.x - pt.x; let dy = activeBlackHole.y - pt.y; let dist = Math.sqrt(dx*dx + dy*dy); if (dist > 10) { pt.vx += (dx/dist) * 3; pt.vy += (dy/dist) * 3; } pt.life -= 0.5; } 
            ...
            for (let j = i + 1; j < allFighters.length; j++) { 
                ...
                if (dist < 80 && f1.attackTimer > 5 && f1.attackTimer < 20 && f2.attackTimer > 5 && f2.attackTimer < 20) {
                    if (!f1.clashCooldown && !f2.clashCooldown && f1.isFacingRight !== f2.isFacingRight) {
                        ...
                        window.spawnLensFlare(midX, midY, "#00f3ff", 4.0); window.speedLinesAlpha = 1.0; window.impactAberration = 25; window.triggerDimensionShatter(midX, midY); 
                        
                        window.invertFrames = 3; window.isSpinningCam = true; window.orbitFocusX = midX; window.targetTimeScale = 0.05;
    ...
    if (window.p1 && window.enemies.length > 0) {
        let aliveEnemy = window.getClosestEnemy(window.p1, window.enemies) || window.enemies[0]; midPointX = (window.p1.x + aliveEnemy.x) / 2;
    ...
    window.targetCamX = (window.canvas.width / 2) - window.orbitFocusX + window.actionCamOffsetX + (actionPan * 0.6); 
    let zoomYCompensate = (window.targetZoom > 1.0) ? (window.targetZoom - 1.0) * 150 : 0;
    window.targetCamY = (window.canvas.height / 2) - window.orbitFocusY + window.actionCamOffsetY + 60 + zoomYCompensate;

    if (window.gameOver && closest) { window.targetZoom = 1.6; if(window.isSpinningCam) window.targetCamY += 40; } 
    else if (closest && !window.gameOver && window.slowMoTimer <= 0) {
        let distance = Math.abs(window.p1.x - closest.x); let dynamicZoom = 1.25 - (distance / 800) * 0.35; window.targetZoom = Math.max(0.9, Math.min(1.25 + comboZoomBonus, dynamicZoom + comboZoomBonus));
        let p1Low = window.p1.hp < window.p1.maxHp * 0.3; let eLow = closest.hp < closest.maxHp * 0.3;
        if (p1Low && eLow) { window.targetTilt = 0.05 * Math.sin(window.matchTimer * 0.06) + comboTiltBonus; } else if (p1Low || eLow) { window.targetTilt = 0.025 * Math.sin(window.matchTimer * 0.04) + comboTiltBonus; } else { window.targetTilt = comboTiltBonus; }
        if (p1Low) window.isLowHpPulsing += 0.2;
    } else if (window.slowMoTimer > 0) { window.targetTilt = 0.025 * Math.sin(window.slowMoTimer * 0.1) + comboTiltBonus; }
}
...
window.draw = function() {
    ...
    let project3D = (obj) => {
        let dx = obj.x - window.orbitFocusX; let pX = window.orbitFocusX + dx * cosA; let pZ = dx * sinA; let pY = obj.y - pZ * 0.02; 
    ...
    if (window.invertFrames > 0) {
        ...
        sortedFighters.forEach(p => { 
            if (p && p.hp > 0 && typeof window.drawStickman === 'function') { 
                ...
                window.ctx.save(); 
                let oldX = p.x; let oldY = p.y; let oldScale = p.scale; let oldFacing = p.isFacingRight;
                p.x = 0; p.y = 0; p.scale = p.drawScale; p.isFacingRight = p.drawFacingRight;
                ...
                p.x = oldX; p.y = oldY; p.scale = oldScale; p.isFacingRight = oldFacing;
                window.ctx.restore(); 
            } 
    ...
    if (window.isLoading) {
        ...
        window.ctx.fillStyle = "#fff"; window.ctx.font = "bold 20px monospace"; window.ctx.fillText(`RENDERING HYPER COMBAT... ${Math.floor(window.loadProgress)}%`, cx, window.canvas.height - 20); 
        return; 
    }
    ...
    try {
        ...
        // [VIRAL 41.0] VẼ REFLECTION SỬ DỤNG GÁN CHUẨN ĐỂ KHÔNG MẤT ANIMATION TAY/CHÂN
        sortedFighters.forEach(p => { 
            if (p && p.hp > 0 && typeof window.drawStickman === 'function') { 
                if (p.iFrames > 20 && (p.state === 'dash' || p.state === 'dash_back')) return; 
                window.ctx.save(); window.ctx.translate(p.drawX, p.drawY); if (!p.drawFacingRight) window.ctx.scale(-1, 1); 
                
                // MẸO: Lưu trữ tọa độ thật, thay đổi tạm thời để vẽ rồi trả lại (Không Clone)
                let oldX = p.x; let oldY = p.y; let oldScale = p.scale; 
                p.x = 0; p.y = 0; p.scale = p.drawScale;
                ...
                // Phục hồi
                p.x = oldX; p.y = oldY; p.scale = oldScale;
                window.ctx.restore(); 
            } 
        });
        window.ctx.restore();
        ...
        // [VIRAL 41.0] VẼ NHÂN VẬT CHÍNH (KHÔNG DÙNG CLONE ĐỂ KHÔNG BỊ ĐƠ ANIMATION)
        if (window.p1) {
            ...
            renderPasses.forEach(pass => {
                sortedFighters.forEach(p => { 
                    ...
                    if (p.state === 'ko_falling' || p.state === 'dead') { 
                        window.ctx.translate(p.drawX + pass.o, p.drawY); let angle = Math.PI / 2; if (p.state === 'ko_falling') { let progress = (100 - p.koTimer) / 30; if (progress > 1) progress = 1; angle = progress * (Math.PI / 2); } let fallDir = p.drawFacingRight ? -1 : 1; window.ctx.rotate(angle * fallDir); 
                        ...
                        if (p.iFrames > 20 && (p.state === 'dash' || p.state === 'dash_back')) { 
                            // Ẩn nhân vật khi Phantom Dash, vẽ mờ mờ luồng khí
                            window.ctx.globalAlpha = 0.2;
                            window.ctx.filter = 'blur(5px)';
                        }
                        window.ctx.translate(p.drawX + pass.o, p.drawY); if(!p.drawFacingRight) window.ctx.scale(-1, 1); 
                        if(p.isDragon && typeof window.drawDragon === 'function') window.drawDragon(window.ctx, p); else if (p.isBruceLee && typeof window.drawBruceLee === 'function') window.drawBruceLee(window.ctx, p); else if (p.isSamurai && typeof window.drawSamurai === 'function') window.drawSamurai(window.ctx, p); else if (p.isNinja && typeof window.drawNinja === 'function') window.drawNinja(window.ctx, p); else if (typeof window.drawStickman === 'function') window.drawStickman(window.ctx, p); 
                    }
