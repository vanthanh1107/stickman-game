// ==========================================
// RECORDER.JS - BẢN TỐI THƯỢNG GIỮ CHÂN NGƯỜI DÙNG (INTERNET KNOWLEDGE EDITION)
// TÍCH HỢP: MA TRẬN KIẾN THỨC RANDOM, HIỆU ỨNG GÕ CHỮ, SÓNG ÂM, THANH TIẾN TRÌNH TIKTOK
// ==========================================
...
// ==========================================
// 🧠 HỆ THỐNG STORYTELLING AI: MA TRẬN KIẾN THỨC VÀ MẸO HAY (GLOBAL TRIVIA)
// ==========================================
window.StoryModeAI = {
    scriptLines: [],      
    currentLineIndex: 0,  
    ...
    charIndex: 0,
    isTyping: false,
    scriptProgress: 0, // Tiến trình câu chuyện (0 -> 1)

    // Kho lưu trữ các chủ đề kiến thức gây tò mò cực mạnh
    knowledgeCategories: [
        // Chủ đề 1: Thao túng tâm lý ngoài đời
        [
            ...
            "Yawning is highly contagious due to a biological system called mirror neurons in human brains.",
        ...
        // Chủ đề 2: Sự thật vũ trụ / Thời gian
        [
            ...
            "Einstein proved that super heavy objects like pyramids literally warp the fabric of space and time.",
            "So if you stand next to a pyramid, you are technically aging slower than the rest of the world.",
        ...
        // Chủ đề 3: Sự thật về cơ thể người
        [
            "Your brain is actually actively deleting the vision of your own nose all day long.",
            "Right now, your eyes can see your nose clearly, but your brain chooses to ignore it.",
            "This process is called unconscious selective attention, preventing you from going insane.",
        ...
        // Chủ đề 4: Mẹo đọc vị người khác
        [
            ...
            "But if they are constructing a fake story or lying, their eyes will often move up and to the right.",
            "This is because the right hemisphere of the brain controls creative imagination.",
        ...
        // Chủ đề 5: Sự thật kỳ lạ về Internet / Công nghệ
        [
            "The first item ever sold on the internet was actually a bag of marijuana in 1972.",
            "It was an deal between students at Stanford University and MIT using the early internet called Arpanet.",
            "Technically, e-commerce started with an illegal trade way before Amazon or eBay existed.",
            "Technology has come a very long way since those wild student days.",
            "And today, we have advanced A I fighter engines capable of absolute destruction.",
            "Watch how this broken mechanic completely obliterates the boss. Wait for it!"
        ]
    ],

    generateScript: function() {
        // Bốc ngẫu nhiên một bộ kiến thức/mẹo hay từ kho dữ liệu
        let randomCategory = this.knowledgeCategories[Math.floor(Math.random() * this.knowledgeCategories.length)];
        return [...randomCategory]; // Copy mảng câu thoại
    },
    ...
    playNextLine: function() {
        ...
        // Tính toán phần trăm tiến trình kịch bản
        ...
        fetchPromise
            ...
            .then(decodedData => {
                ...
                let gainNode = window.audioCtx.createGain(); gainNode.gain.value = 1.6; // Tăng âm lượng giọng nói một chút
                ...
                source.onended = () => {
                    ...
                    // Khi kết thúc mảng thoại, đẩy tiến trình lên 100%
...
function drawAudioVisualizer(ctx, x, y, width, height) {
    ...
    for (let i = 0; i < 20; i++) {
        ...
        if (ctx.roundRect) {
            ctx.beginPath(); ctx.roundRect(posX, y - barHeight, barWidth, barHeight, 5); ctx.fill();
        } else {
...
window.captureFrames = function() {
    ...
    // =====================================
    // VẼ HUD MÁU & AVATAR ĐÚNG VỊ TRÍ GỐC TRÊN KHUNG RECORD
    // =====================================
    if (window.p1 && !window.gameOver && window.introTimer <= 120) {
        ...
        // HUD DỌC (Trả về đỉnh khu vực đấm nhau Y=450)
    ...
    // =====================================
    // [RETENTION HACK 1] VẼ TIKTOK STICKER "WAIT FOR THE END 🤯"
    // Được treo nghiêng tinh tế ở khoảng trống đen phía trên
    // =====================================
    let badgePulse = Math.abs(Math.sin(Date.now() / 400));
    ctxV.save();
    ctxV.translate(540, 220); // Đặt lơ lửng giữa vùng đen phía trên HUD máu
    ctxV.rotate(-0.05);       // Độ nghiêng chuẩn meme Tiktok
    ctxV.fillStyle = `rgba(255, 10, 50, ${0.85 + badgePulse * 0.15})`;
    ctxV.shadowColor = "rgba(0,0,0,0.9)"; ctxV.shadowBlur = 15;
    ctxV.beginPath(); 
    if(ctxV.roundRect) ctxV.roundRect(-230, -35, 460, 70, 16); else ctxV.rect(-230, -35, 460, 70); 
    ctxV.fill();
    ctxV.fillStyle = "#ffffff"; ctxV.font = "900 34px 'Arial Black', Gadget, sans-serif"; ctxV.textAlign = "center"; ctxV.textBaseline = "middle";
    ctxV.shadowBlur = 0;
    ...
    // =====================================
    // VẼ SUB, SÓNG ÂM VÀ [HACK 2] THANH TIẾN TRÌNH NEON 
    // =====================================
    if (window.StoryModeAI.displayedText.length > 0) {
        // Vẽ Audio Visualizer sóng âm Podcast
        drawAudioVisualizer(ctxV, 540, 1600, 500, 80); 
        drawAudioVisualizer(ctxH, 960, 840, 500, 80);

        // TEXT VIDEO DỌC TIKTOK (Mép dưới cùng)
        ...
        wrapText(ctxV, window.StoryModeAI.displayedText, 540, 1640, 950, 60);
        ctxV.restore();
    ...
    // [HACK 2] SCRIPT PROGRESS BAR - Thanh tiến trình Neon cuốn hút mắt
    let progressWidthV = 1080 * window.StoryModeAI.scriptProgress;
    ctxV.save();
    ctxV.fillStyle = "rgba(255,255,255,0.1)"; ctxV.fillRect(0, 1910, 1080, 10); // Thanh nền
    let gradV = ctxV.createLinearGradient(0, 0, progressWidthV, 0); gradV.addColorStop(0, "#00f3ff"); gradV.addColorStop(1, "#f1c40f");
    ctxV.fillStyle = gradV; ctxV.shadowColor = "#00f3ff"; ctxV.shadowBlur = 10;
    ctxV.fillRect(0, 1910, progressWidthV, 10); // Thanh tiến trình thực tế
    ctxV.restore();
    ...
    ctxH.fillStyle = gradH; ctxH.shadowColor = "#00f3ff"; ctxH.shadowBlur = 10;
