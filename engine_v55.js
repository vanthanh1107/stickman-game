// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 45.0 [THE APEX ANIMATOR]
// [RESTORED] KHÔNG LỖI ANIMATION: Animation đánh tay, đá chân, vung vũ khí mượt mà 100% không bị đơ.
// [RESTORED] CAMERA CHUẨN ESPORTS: Giữ góc máy rộng, thấy rõ toàn thân và chân của nhân vật.
// [INTEGRATED] DBZ TELEPORT CLASH: Clash sẽ làm cả 2 văng lên trời, dịch chuyển và chém nhau liên tục.
// [INTEGRATED] CINEMATIC HEAVY STRIKE: Đòn chí mạng làm camera nghiêng chéo (Dutch Angle), khựng thời gian.
// [INTEGRATED] DYNAMIC SPEED LINES: Sọc tốc độ Anime khi Lướt, Né Hoàn Hảo và K.O.
// [BASE] Matrix Orbit 360, Nảy sàn (Ground Bounce), Hất tung (Juggle), Mực văng Camera, RTX Floor.
// ==========================================
...
window.spawnScreenInk = function(color) { 
    for(let i=0; i<3; i++) {
        window.screenInks.push({
            x: Math.random() * (window.canvas ? window.canvas.width : 800),
            y: Math.random() * (window.canvas ? window.canvas.height : 600),
            color: color, life: 120, maxLife: 120, scale: Math.random() * 2 + 1,
            slideY: 0
        });
    }
}
...
window.triggerDimensionShatter = function(x, y) {
    ...
    let numMainCracks = 5 + Math.floor(Math.random() * 4); 
    for(let i = 0; i < numMainCracks; i++) {
        let baseAng = (Math.PI * 2 / numMainCracks) * i + (Math.random() - 0.5) * 0.6;
        let len = 350 + Math.random() * 300; 
        let branches = [];
        
        let points = [{x: cx, y: cy}];
        let currX = cx; let currY = cy;
        let dist = 0;
        let currentAng = baseAng;
        
        while(dist < len) {
            let step = 30 + Math.random() * 40; 
            dist += step; 
            currentAng += (Math.random() - 0.5) * 0.9; 
            currX += Math.cos(currentAng) * step; 
            currY += Math.sin(currentAng) * step; 
            points.push({x: currX, y: currY});
            
            if (Math.random() < 0.45) {
                let subPoints = [{x: currX, y: currY}];
                let subAng = currentAng + (Math.random() > 0.5 ? 1 : -1) * (0.6 + Math.random()*0.8);
                let subLen = step * (1.5 + Math.random() * 2);
                let sDist = 0; let sX = currX; let sY = currY;
                while(sDist < subLen) {
                    let sStep = 20 + Math.random()*20;
                    sDist += sStep;
                    subAng += (Math.random() - 0.5) * 0.6;
                    sX += Math.cos(subAng) * sStep;
                    sY += Math.sin(subAng) * sStep;
...
window.triggerStageTransitionEffect = function() {
    ...
    window.screenFlash = 2.0; 
    window.playSound(600, 'sawtooth', 1.0, 1.0, true);
    ...
    window.viralColorGrade.hue = Math.floor(Math.random() * 360);
    window.viralColorGrade.contrast = 130 + Math.random() * 30;
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce, attackerDirRight = true) {
    ...
    if (target.state === 'dash' || target.state === 'dash_back') {
        if (target.dashTimer > 2 && target.dashTimer < 14) {
            ...
            window.applyFilter('invert', 15); window.speedLinesAlpha = 1.0; 
    ...
    if (finalDmg > 0) {
        ...
        if (isCrit) {
            ...
            // [VIRAL 45.0] CINEMATIC HEAVY STRIKE - Ép góc máy, giật khung hình
            window.targetZoom = 1.35; 
            window.targetTilt = attackerDirRight ? 0.12 : -0.12;
            window.orbitFocusX = target.x; 
            setTimeout(() => { window.targetZoom = 1.0; window.targetTilt = 0; window.targetCamOrbitAngle = 0; }, 350);
            ...
            if(Math.random() < 0.4) window.spawnScreenInk(color || "#ff003c");

            for(let i=0; i<8; i++) { window.impactSparks.push({ x: target.x, y: target.y - 40, vx: (Math.random()-0.5)*25, vy: (Math.random()-0.5)*25, life: 20 + Math.random()*10, maxLife: 30, color: color || "#f1c40f" }); }
            
            // Đóng băng không gian tích tắc
            window.targetTimeScale = 0.05; 
            setTimeout(()=>{ window.targetTimeScale = 1.2; setTimeout(()=>{ window.targetTimeScale = 1.0; }, 200); }, 150);
            
            window.spawnLightning(target.x, target.y-40, color || "#f1c40f", 30, 4);
        ...
        let isNearWallNow = (target.x < window.WALL_PADDING + 20) || (target.x > window.canvas.width - window.WALL_PADDING - 20);
        if (wallBounce || (isNearWallNow && isCrit)) { 
            target.vx = target.isFacingRight ? -12 : 12; target.state = 'hurt'; 
            if (isCrit) {
                window.floatingTexts.push({ x: target.x, y: target.y - 120, text: "💥 STAGE BREAK!", color: "#ff4757", alpha: 1.5, vx: 0, vy: -2, font: "italic 900 65px 'Arial Black', Impact", life: 80, scale: 4.0, targetScale: 1.0, scaleVel: 0, rot: 0 });
                window.targetCamOrbitAngle = Math.PI; 
                window.triggerStageTransitionEffect(); 
            }
        } 
        ...
        if (target.hp <= 0) {
            ...
            if (isNearWall && isCrit && typeof window.triggerStageTransition === 'function') { window.triggerStageTransition(target); } 
                ...
                if (isCrit) {
                    ...
                    window.spawnBlackHole(target.x, target.y, 400, 150); window.spawnEnergyPillar(target.x, window.GROUND_Y, "#ff003c", 100, 80); window.spawnSpaceRipple(target.x, target.y - 40, "#ff003c");
                    window.spawnScreenInk("#ff0000"); window.spawnScreenInk("#000000");

                    window.isSpinningCam = true; window.orbitFocusX = target.x; window.targetTimeScale = 0.05; 
        ...
        } else if (isCrit) { window.impactFrameTimer = 2; window.hitStopFrames = 6; window.shakeScreen(15, 12); window.chromaTimer = 12; window.playSound(180, 'square', 0.3, 0.6, true); } 
        else { window.hitStopFrames = 1; window.shakeScreen(6, 5); window.playSound(250, 'sine', 0.15, 0.3, true); }
    }
};
...
window.attack = function(attacker, targetGroup) {
    ...
    if (isFinisher) {
        isCrit = true; finalDmg = baseDmg * 3.5; window.shakeScreen(18, 15); target.vx = (attacker.isFacingRight ? 6 : -6); 
            ...
            for (let j = i + 1; j < allFighters.length; j++) { 
                ...
                if (dist < 80 && f1.attackTimer > 5 && f1.attackTimer < 20 && f2.attackTimer > 5 && f2.attackTimer < 20) {
                    if (!f1.clashCooldown && !f2.clashCooldown && f1.isFacingRight !== f2.isFacingRight) {
                        ...
                        // [VIRAL 45.0] TELEPORT CLASH - BAY LÊN KHÔNG TRUNG ĐỂ TRANH CHẤP
                        f1.vy = -18; f2.vy = -18; 
...
window.draw = function() {
    ...
    if (window.isLoading) {
        ...
        window.ctx.fillStyle = "#fff"; window.ctx.font = "bold 20px monospace"; window.ctx.fillText(`APEX ANIMATOR LOADING... ${Math.floor(window.loadProgress)}%`, cx, window.canvas.height - 20); 
        return; 
    }
    ...
    try {
        ...
        // [VIRAL 44.0] VẼ ĐƯỜNG NÉT TỐC ĐỘ ANIME (SPEED LINES)
