// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 42.0 [THE TRUE MASTERPIECE]
// [FIXED] PHỤC HỒI ANIMATION: Khớp tay, chân, cử động ra đòn liên hoàn hoạt động mượt mà 100% trong không gian 3D.
// [FIXED] TRẢ LẠI GÓC MÁY RỘNG: Mở rộng Camera, trả lại tỷ lệ toàn thân (nhìn rõ từ đầu đến chân và bóng) dù zoom nhịp điệu.
// [BASE] Matrix Orbit 360, Kinetic Text, Ground Bounce, Witch Time, RTX Floor đều giữ nguyên vẹn!
// ==========================================
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce, attackerDirRight = true) {
    ...
    if (target.state === 'block') { 
        if (target.stamina < 15) {
            ...
            // [VIRAL 42.0] Trả Zoom lại chuẩn, không sát mặt nữa
    ...
    if (finalDmg > 0) {
        ...
        if (isCrit) {
            ...
            // [VIRAL 42.0] Phục hồi Zoom rộng chuẩn eSports
        ...
        if (target.hp <= 0) {
            ...
            if (isNearWall && isCrit && typeof window.triggerStageTransition === 'function') { window.triggerStageTransition(target); } 
                ...
                if (isCrit) {
                    ...
                    window.targetZoom = 1.5; // [VIRAL 42.0] Trả lại góc bao quát khi K.O
                    window.actionCamOffsetX = 0; 
                    window.actionCamOffsetY = -20; 
                    window.dynamicBlur = 25; 
                    window.cameraTilt = (Math.random() > 0.5 ? 1 : -1) * 0.15; 
...
window.attack = function(attacker, targetGroup) {
    ...
    if (MathDist > reach && !isFinisher) { 
        ...
        window.speedLinesAlpha = 0.8; window.actionCamOffsetX = attacker.isFacingRight ? 50 : -50; 
...
window.update = function() {
            ...
            for (let j = i + 1; j < allFighters.length; j++) { 
                ...
                if (dist < 80 && f1.attackTimer > 5 && f1.attackTimer < 20 && f2.attackTimer > 5 && f2.attackTimer < 20) {
                    if (!f1.clashCooldown && !f2.clashCooldown && f1.isFacingRight !== f2.isFacingRight) {
                        ...
                        window.targetZoom = 1.3; window.screenFlash = 0.5; 
    ...
    // [VIRAL 42.0] Tinh chỉnh lại tỷ lệ Zoom để không mất chân
    ...
    if (window.gameOver && closest) {
        window.targetZoom = 1.4; // Trả lại góc rộng vừa phải khi K.O
        ...
        // [VIRAL 42.0] Trả lại thông số Zoom rộng của eSports
...
window.draw = function() {
    ...
    if (window.invertFrames > 0) {
        ...
        sortedFighters.forEach(p => { 
            if (p && p.hp > 0 && typeof window.drawStickman === 'function') { 
                ...
                // [VIRAL 42.0] Phục hồi tay chân bằng cách không đổi p.x, p.y
                window.ctx.save();
    ...
    if (window.isLoading) {
        ...
        window.ctx.fillStyle = "#fff"; window.ctx.font = "bold 20px monospace"; window.ctx.fillText(`RENDERING PERFECT ANIMATION... ${Math.floor(window.loadProgress)}%`, cx, window.canvas.height - 20); 
        return; 
    }
    ...
    try {
        ...
        // [VIRAL 42.0] VẼ REFLECTION VÀ TRẢ LẠI IK CHUẨN XÁC
        sortedFighters.forEach(p => { 
            if (p && p.hp > 0 && typeof window.drawStickman === 'function') { 
                if (p.iFrames > 20 && (p.state === 'dash' || p.state === 'dash_back')) return; 
        ...
        // [VIRAL 42.0] VẼ NHÂN VẬT CHÍNH (KHÔNG PHÁ HỎNG IK ANIMATION)
        if (window.p1) {
