// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 38.5 [MEME & SILLY EDITION]
// [RESTORED] Giữ nguyên UI Emoji, Text Manga, Anti-Glare và Hiệu ứng Râu Trắng.
// [NEW SILLY MECHANICS] Đấm văng lên trời (YEETED), Né đòn cà khịa (SKILL ISSUE), 
// Vỡ Block hiện "EMOTIONAL DAMAGE!", Ăn vặt hồi máu khi đứng im, và nổ ra bắp rang bơ khi chết.
// ==========================================
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce, attackerDirRight = true) {
    ...
    if (target.state === 'dash' || target.state === 'dash_back') {
        if (target.dashTimer > 2 && target.dashTimer < 14) {
            ...
            // [MEME UPGRADE] Dòng chữ cà khịa khi né đòn
    ...
    if (target.state === 'block') { 
        if (target.stamina < 15) {
            ...
            // [MEME UPGRADE] Sát thương tinh thần
    ...
    if (finalDmg > 0) {
        ...
        // [MEME UPGRADE] Đòn đánh thường thỉnh thoảng sẽ hiện Bonk
        let dmgText = isCrit ? `💥 -${Math.floor(finalDmg)}` : `-${Math.floor(finalDmg)}`;
        if (!isCrit && Math.random() < 0.1) {
            dmgText = "BONK!";
            window.playSound(800, 'square', 0.1, 0.5, true); // Âm thanh tấu hài
        }
        ...
        if (target.hp <= 0) {
            ...
            if (isNearWall && isCrit && typeof window.triggerStageTransition === 'function') { window.triggerStageTransition(target); } 
                ...
                if (isCrit) {
                    ...
                    // [MEME UPGRADE] Đấm văng lên mặt trăng
                    if (Math.random() > 0.5) {
                        target.vy = -60; // Bay màu đúng nghĩa
...
window.update = function() {
    ...
    if (!isTimeStopped) {
        window.enemies.forEach(e => { 
            if (e.hp <= 0 && !e.deathTriggered) {
                ...
                // [MEME UPGRADE] Nổ ra rác và đồ ăn tấu hài
                let emojis = ["🍿", "🎉", "🌭", "😂", "💀", "🍔", "🤡"];
                for(let c=0; c<12; c++) {
    ...
    allFighters.forEach(f => {
        ...
        if (f.state === 'idle' && !isTimeStopped) { 
            ...
            // [MEME UPGRADE] Rảnh quá lôi đồ ăn ra gặm
