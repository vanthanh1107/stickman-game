// ==========================================
// RECORDER.JS - V69.4 TIKTOK ANIME CUT-IN EDITION
// ĐÃ TỐI ƯU: Khóa cứng thời gian Intro đúng 2.5s (Time-Based), không bao giờ bị kéo dài 6s.
// CẬP NHẬT: Hiệu ứng chẻ màn hình (Diagonal Split Cut-in) chuẩn Manga / Persona 5.
...
// ====================================================================
// SIÊU INTRO: MANGA / PERSONA CUT-IN DIAGONAL SPLIT
// ====================================================================
window.drawAnimeCutInIntro = function(ctx, w, h, progress) {
    ...
    // Giai đoạn 1 (0.0 - 0.15): Cảnh báo dạo đầu (Glitch Warning)
    if (progress < 0.15) {
    ...
    // Rung màn hình cực mạnh lúc chuẩn bị va chạm
    if (progress > 0.7 && progress < 0.85) {
        let rumble = (Math.random() - 0.5) * 60;
        ctx.translate(rumble, rumble);
    }

    // Giai đoạn 2 (0.15 - 0.85): Chẻ đôi màn hình chéo (Diagonal Split)
    let pCut = Math.min(1, (progress - 0.15) / 0.6); // 0 -> 1 smooth
    let easeCut = 1 - Math.pow(1 - pCut, 4); // Chạy nhanh rồi chậm lại

    const drawSpeedlines = (color, reverse) => {
        ctx.save(); ctx.fillStyle = color;
        for(let i=0; i<30; i++) {
            ctx.rotate(Math.PI / 15);
            let speed = (Date.now() * 2) % 800;
            if(reverse) speed = 800 - speed;
            ctx.fillRect(150 + speed, -10, 1500, 15 + Math.random()*25);
        }
        ctx.restore();
    };
    ...
    const drawCharMega = (charObj, cx, cy, isP1) => {
        ...
        let clone = Object.assign({}, charObj, {x:0, y:0, scale: 8.0, isFacingRight: true, state: 'cast'});
        // Bóng mờ phía sau
        ctx.save(); ctx.globalAlpha = 0.5; ctx.shadowBlur = 50; ctx.shadowColor = isP1 ? "#00f3ff" : "#ff003c";
        ctx.scale(1.2, 1.2);
    ...
    // --- NỬA TRÊN: BOSS (Màu Đỏ) ---
    ctx.save();
    ctx.beginPath();
    ctx.moveTo(0, 0); ctx.lineTo(w, 0); 
    ctx.lineTo(w, h*0.65 - (easeCut * h*0.1)); // Góc phải chạy lên
    ctx.lineTo(0, h*0.35 + (easeCut * h*0.1)); // Góc trái chạy xuống
    ctx.clip();
    
    // Nền Đỏ Boss
    let gradBoss = ctx.createRadialGradient(w/2, h*0.25, 0, w/2, h*0.25, w);
    gradBoss.addColorStop(0, "#4a0000"); gradBoss.addColorStop(1, "#0a0000");
    ctx.fillStyle = gradBoss; ctx.fillRect(0,0,w,h);
    
    ctx.save(); ctx.translate(w/2, h*0.25); drawSpeedlines("rgba(255, 0, 60, 0.3)", false); ctx.restore();
    
    // Boss di chuyển từ ngoài vào
    let bossX = w*1.2 - (easeCut * w*0.4);
    let bossY = h*0.1 + (easeCut * h*0.15);
    drawCharMega(e1, bossX, bossY, false);
    
    // Trash talk Boss
    ctx.save(); ctx.translate(w*0.3, h*0.15); ctx.rotate(-0.08);
    ctx.font = "italic 900 60px Impact"; ctx.textAlign = "center";
    ctx.lineWidth = 15; ctx.strokeStyle = "#000"; ctx.strokeText(window.trashTalkP2, 0,0);
    ctx.fillStyle = "#ff003c"; ctx.fillText(window.trashTalkP2, 0,0); ctx.fillStyle = "#fff"; ctx.fillText(window.trashTalkP2, -3,-3);
    ctx.restore();
    ctx.restore(); // Hết clip nửa trên

    // --- NỬA DƯỚI: PLAYER (Màu Xanh) ---
    ctx.save();
    ctx.beginPath();
    ctx.moveTo(0, h); ctx.lineTo(w, h); 
    ctx.lineTo(w, h*0.65 - (easeCut * h*0.1)); 
    ctx.lineTo(0, h*0.35 + (easeCut * h*0.1)); 
    ctx.clip();
    
    // Nền Xanh Player
    let gradP1 = ctx.createRadialGradient(w/2, h*0.75, 0, w/2, h*0.75, w);
    gradP1.addColorStop(0, "#002b4a"); gradP1.addColorStop(1, "#000a12");
    ctx.fillStyle = gradP1; ctx.fillRect(0,0,w,h);
    
    ctx.save(); ctx.translate(w/2, h*0.75); drawSpeedlines("rgba(0, 243, 255, 0.2)", true); ctx.restore();
    
    // Player di chuyển từ ngoài vào
    let p1X = -w*0.2 + (easeCut * w*0.4);
    let p1Y = h*0.9 - (easeCut * h*0.15);
    drawCharMega(window.p1, p1X, p1Y, true);

    // Trash talk Player
    ctx.save(); ctx.translate(w*0.7, h*0.85); ctx.rotate(0.08);
    ctx.font = "italic 900 60px Impact"; ctx.textAlign = "center";
    ctx.lineWidth = 15; ctx.strokeStyle = "#000"; ctx.strokeText(window.trashTalkP1, 0,0);
    ctx.fillStyle = "#00f3ff"; ctx.fillText(window.trashTalkP1, 0,0); ctx.fillStyle = "#fff"; ctx.fillText(window.trashTalkP1, -3,-3);
    ctx.restore();
    ctx.restore(); // Hết clip nửa dưới

    // --- ĐƯỜNG CHÉO SÁNG RỰC (THE SPLIT LINE) ---
    ctx.beginPath();
    ctx.moveTo(0, h*0.35 + (easeCut * h*0.1));
    ctx.lineTo(w, h*0.65 - (easeCut * h*0.1));
    ctx.lineWidth = 25; ctx.strokeStyle = "#ffffff";
    ctx.shadowBlur = 40; ctx.shadowColor = "#ffeb3b";
    ctx.stroke(); ctx.stroke(); // Gọi 2 lần cho sáng
    ctx.shadowBlur = 0;

    // Giai đoạn 3 (0.85 - 1.0): Vụ nổ Flashbang chuyển cảnh
    if (progress >= 0.85) {
        let flashP = (progress - 0.85) / 0.15; // 0 -> 1
        ctx.fillStyle = `rgba(255, 255, 255, ${1 - flashP})`;
        ctx.fillRect(0,0,w,h);
        
        ctx.save(); ctx.translate(w/2, h/2); 
        let pop = 1 + (flashP * 3); // Zoom to dần
        ctx.scale(pop, pop);
        ctx.font = "italic 900 200px Impact"; ctx.textAlign = "center"; ctx.textBaseline = "middle";
        ctx.lineWidth = 30; ctx.strokeStyle = "#000"; 
...
window.captureFrames = function() {
    ...
    let now = Date.now();
    if (now - window._lastCaptureTime < 33) return; // 30FPS Throttling cho video nhẹ
    ...
    if (elapsed < 150) {
        // Vẽ thumbnail tĩnh trong 150ms đầu để TikTok tự bắt làm ảnh bìa
        if (window.bakedThumbV) ctxV.drawImage(window.bakedThumbV, 0, 0, 1080, 1920); 
        renderNormalV = false; 
    } else if (elapsed < window.introDuration) {
        // Phase Intro Anime Cut-in (Sau 150ms -> 2500ms)
        let introProgress = (elapsed - 150) / (window.introDuration - 150); // Đi từ 0.0 đến 1.0
        
        // Đồng bộ âm thanh theo Progress thay vì frame
        if (!window._sfxFlags.s1 && introProgress > 0.05 && typeof window.playSound === 'function') { window.playSound(150, 'sawtooth', 0.2, 0.4); window._sfxFlags.s1 = true; } // Bíp cảnh báo
        if (!window._sfxFlags.s2 && introProgress > 0.15 && typeof window.playSound === 'function') { window.playSound(300, 'square', 0.2, 0.6); window._sfxFlags.s2 = true; } // Xẹt (cắt màn hình)
        if (!window._sfxFlags.s3 && introProgress > 0.85 && typeof window.playSound === 'function') { window.playSound(100, 'sawtooth', 0.6, 1.0, true); window._sfxFlags.s3 = true; } // BÙM (Flashbang)
