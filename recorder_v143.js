...
// TÂM LÝ HỌC (NEW): Intro loạn nhịp Toxic, Sạn cố ý (Pin yếu, Tin nhắn Mẹ), Sai chính tả mồi Comment.
// ==========================================
...
// HỆ THỐNG CHAT FLOOD INTRO
window._introChatSpam = [];
...
function makeTypo(str) {
    if(Math.random() > 0.5) return str; // 50% cơ hội đúng
    let arr = str.split('');
...
window.CELEB_LIST = [
    ...
    { name: "TienBip", color: "#00ffcc" }, { name: "ThanhTroll", color: "#ff66b2" }
];
...
const TOXIC_MSGS = [
    ...
    "Tuyển bạn này vào đánh giải... làng 🤣", "Khóc đi 😭", "Gà 🐔🐔", "ㅋㅋ 봇인줄", "실화냐? 💀", "草", "太菜了吧 😂"
];
...
window.StoryModeAI = {
    ...
    generateViralPostKit: function() {
        ...
        const pinnedComments = [
            ...
            `👇 PINNED: Did anyone notice the typo at the start? 😭📉` // Nhắc người xem về Sạn
        ];
        const r = (arr) => arr[Math.floor(Math.random() * arr.length)];
...
function drawBaitSạn(ctx, w, h) {
    ...
    if (timeSinceBait > 3500) { window._baitType = -1; return; } // Tắt sau 3.5s
    ...
    if (window._baitType === 0) {
        // BAIT 0: THÔNG BÁO PIN YẾU IPHONE (Ở giữa màn hình)
        ...
        // BAIT 1: TIN NHẮN MESSENGER CỦA MẸ (Trượt từ trên xuống)
        ...
        // BAIT 2: PING 999MS CHỚP ĐỎ (Góc phải trên)
...
// ====================================================================
// ĐẠO DIỄN: THE GOD-TIER SAKUGA (THE HOOK & TYPO LORE)
// ====================================================================
window.drawProceduralIntro = function(ctx, w, h, progress) {
    ...
    // Tinh chỉnh lại mốc thời gian: 
    // Act 0 (0-1.2s): P1 + Toxic Chat Flood
    // Act 1 (1.2-2.2s): P2 + Typo Lore
    // Act 2 (2.2-2.6s): Dash
    // Act 3 (2.6-3.2s): Clash
    if (progress < 0.35) { act = 0; localProg = progress / 0.35; } 
    else if (progress < 0.65) { act = 1; localProg = (progress - 0.35) / 0.30; } 
    else if (progress < 0.80) { act = 2; localProg = (progress - 0.65) / 0.15; } 
    ...
    if (act === 0) { currentZoom = 2.5 - localProg * 0.5; panX = (1 - localProg) * 60; cutShake = (Math.random()-0.5)*15; } // P1 Zoom In + Shake nhẹ
    else if (act === 1) { currentZoom = 1.8 + localProg * 0.2; panX = -(1 - localProg) * 60; } // P2 Pan
    else if (act === 3) {
    ...
    if (act === 0) {
        // ACT 0: HIỂN THỊ NHÂN VẬT 1 (HERO)
        drawFighterArtistic(window.p1, 0, 100, 'idle', true, prm.themeP1, realP1Name);

        // HIỆU ỨNG CHAT FLOOD LOẠN NHỊP (PSYCHOLOGICAL WARFARE)
        ctx.restore(); // Thoát camera transform để vẽ HUD đè lên trên
        ctx.save();
        if(window._introChatSpam.length === 0) {
            for(let c=0; c<15; c++) {
                let msg = TOXIC_MSGS[Math.floor(Math.random()*TOXIC_MSGS.length)];
                window._introChatSpam.push({
                    msg: msg,
                    x: Math.random() * w, y: Math.random() * h,
                    vx: (Math.random()-0.5)*10, vy: (Math.random()-0.5)*10,
                    scale: 1 + Math.random()*2,
                    rot: (Math.random()-0.5)*0.5,
                    col: CELEB_LIST[Math.floor(Math.random()*CELEB_LIST.length)].color
                });
            }
        }
        for(let c of window._introChatSpam) {
            c.x += c.vx; c.y += c.vy; c.scale += 0.05;
            ctx.save(); ctx.translate(c.x, c.y); ctx.rotate(c.rot); ctx.scale(c.scale, c.scale);
            ctx.font = "900 30px 'Arial Black', sans-serif";
            ctx.textAlign = "center"; ctx.textBaseline = "middle";
            ctx.lineWidth = 6; ctx.strokeStyle = "#000"; ctx.strokeText(c.msg, 0, 0);
            ctx.fillStyle = c.col; ctx.fillText(c.msg, 0, 0);
            ctx.restore();
        }
        ctx.restore();
        ctx.save(); // Vào lại camera giả lập
        ctx.translate(w/2 + cutShake + panX, h/2 + cutShake); ctx.scale(currentZoom, currentZoom); ctx.translate(0, 100); 
    } 
    else if (act === 1) {
        // ACT 1: HIỂN THỊ NHÂN VẬT 2 (BOSS) VÀ SẠN LORE
    ...
    // Wager Badge
    if (barHeight > 0 && prm.badge && act !== 0) { // Không hiện badge lúc đang loạn chat để tập trung chat
    ...
    // Lore Chapter (Với Typo Bait)
    if (act === 1 && barHeight > 0) {
        ctx.save();
        ...
        ctx.letterSpacing = "3px"; ctx.fillText(prm.lore.chapter, chapterX, barHeight / 2); // Hiển thị chữ sai chính tả đã random
        ctx.restore();
    }
...
window.captureFrames = function() {
    ...
    if (renderNormalV) {
        ...
        // =========================================================
        let brandWM = window.CREATOR_HANDLE || "VIETNAM SPACEX";
        ctxV.save();
