...
// Ép buộc chuẩn xuất video sang MP4 (H.264/AVC) tương thích 100% với CapCut & Premiere
// Kèm theo RETENTION MULTIPLIER (Fake Chat & Brainrot Mini-Game)
// ==========================================
...
window.currentVideoExt = "mp4"; // Ép chuẩn định dạng MP4
window.savedVideos = [];
window.filmDustY = 0; 
...
function drawVTuberCommentator(ctx, x, y, audioPeak, isTyping) {
    ...
    ctx.beginPath(); if(ctx.roundRect) ctx.roundRect(-100, -75, 200, 150, 15); else ctx.rect(-100, -75, 200, 150); ctx.fill();
    ...
    if (isBlinking) {
        ...
        ctx.save(); ctx.translate(-45, -35); ctx.rotate(0.3); ctx.beginPath(); ctx.ellipse(0, 0, 18, 25, 0, 0, Math.PI*2); ctx.fill(); ctx.restore();
        ctx.save(); ctx.translate(45, -35); ctx.rotate(-0.3); ctx.beginPath(); ctx.ellipse(0, 0, 18, 25, 0, 0, Math.PI*2); ctx.fill(); ctx.restore();
    }

    let mouthHeight = (!isTyping && audioPeak < 0.2) ? 8 : 8 + (audioPeak * 60); 
    ctx.fillStyle = "#ff0055";
    ctx.beginPath(); if(ctx.roundRect) ctx.roundRect(-35, 25, 70, mouthHeight, 10); else ctx.fillRect(-35, 25, 70, mouthHeight); ctx.fill();

    ctx.fillStyle = "#ffeb3b"; ctx.fillRect(120, -20, 15, 40); ctx.beginPath(); ctx.arc(140, 0, 10, 0, Math.PI*2); ctx.fill();

    ctx.rotate(-tilt); 
    ctx.fillStyle = "#ff0000"; ctx.beginPath(); if(ctx.roundRect) ctx.roundRect(-180, -120, 80, 30, 6); else ctx.fillRect(-180, -120, 80, 30); ctx.fill();
    ctx.fillStyle = "#fff"; ctx.font = "900 16px 'Arial Black'"; ctx.textAlign = "center"; ctx.textBaseline = "middle";
    ctx.fillText("🔴 LIVE", -140, -105);
...
window.startRecording = function() {
    ...
    // [V67.8 MP4 ENCODE] Ép buộc định dạng MP4 chuẩn H.264 / AVC1 tương thích 100% CapCut
    ...
    if (MediaRecorder.isTypeSupported('video/mp4; codecs="avc1,mp4a.40.2"')) {
        ...
        // Fallback an toàn nếu trình duyệt không hỗ trợ trực tiếp MP4 MediaRecorder
    ...
    const finalizeRecordings = () => {
        ...
        if (stoppedCount === 2) { 
            setTimeout(() => {
                ...
                // Trực tiếp tạo Blob MP4 chuẩn xác không cần qua bước fix phức tạp
...
function wrapTextTikTok(ctx, text, x, y, maxWidth, lineHeight) {
    let lines = [];
    if (window._textLinesCache.text === text && window._textLinesCache.maxWidth === maxWidth) {
        lines = window._textLinesCache.lines;
    } else {
        ...
        lines.push(currentLine.trim());
        window._textLinesCache.text = text;
        window._textLinesCache.maxWidth = maxWidth;
        window._textLinesCache.lines = lines;
    }
    
    ctx.textAlign = "left";
    let totalWords = lines.join(' ').split(' ').length;
    let wordCount = 0;
    ...
    for(let i=0; i<lines.length; i++) {
        ...
        for(let j=0; j<lineWords.length; j++) {
            ...
            ctx.fillStyle = isLastWord ? "#ffeb3b" : "#ffffff";
...
function renderMobileSafeCombo(ctx, fighter, xPos, yPos, align, isMobile) {
    if (fighter && fighter.comboHits >= 2) {
        ...
        ctx.font = `italic 900 ${Math.floor(26 * scale)}px 'Arial Black', Impact`; ctx.fillStyle = rankColor; 
...
function drawAudioVisualizer(ctx, x, y, width, height) {
    ...
    for (let i = 0; i < numBars / 2; i++) {
        ...
        ctx.rect((center + offsetX) | 0, (y - barHeight) | 0, barWidth | 0, barHeight | 0); 
        ctx.rect((center - offsetX - barWidth) | 0, (y - barHeight) | 0, barWidth | 0, barHeight | 0);
    }
    ctx.fill(); 
    ctx.restore();
}
...
window.captureFrames = function() {
    ...
    if (!window.gameOver && Date.now() % 1000 < 600) {
        ctxV.save(); ctxV.translate(1040, splitGameHeight - 40); ctxV.textAlign = "right"; ctxV.textBaseline = "middle";
        ctxV.font = "italic 900 35px 'Arial Black'"; ctxV.fillStyle = "#ffeb3b"; ctxV.fillText("🔥 PART 2 ON PROFILE 👉", 0, 0); ctxV.restore();
    }

    if (!window.gameOver) {
        ctxV.save();
        let bouncerY = 850 + Math.sin(Date.now() / 150) * 30; 
        ctxV.translate(1012, bouncerY); ctxV.rotate(-0.1); 
        ctxV.fillStyle = "#00f2fe"; ctxV.font = "900 65px 'Arial Black'"; ctxV.textAlign = "right"; ctxV.textBaseline = "middle";
        ctxV.fillText("FOLLOW ➕", -80, 0); ctxV.font = "120px Arial"; ctxV.fillText("👉", 20, 0); ctxV.restore();
    }

    // ==========================================
    // 3. BACKGROUND LƯỚI KHÔNG GIAN 3D
    // ==========================================
    ...
    // [CẬP NHẬT] Dời VTuber sang phía bên phải khung hình để chừa chỗ cho tính năng mới
    if (!window.gameOver && window.introTimer > 120) { drawVTuberCommentator(ctxV, 820, splitGameHeight + 250, audioPeak, window.StoryModeAI.isTyping); }

    let bannerY = retainY + 25;
    ctxV.fillStyle = "#ff0055"; ctxV.fillRect(0, retainY, 1080, 50);
    ...
    for(let i = -1; i < 5; i++) { ctxV.fillText("🚨 WAIT FOR THE END ⏩ DO NOT BLINK 🚨", i*850 - offsetBanner, bannerY); }

    // EMOJIS BAY LÊN
    if (audioPeak > 0.35 && Math.random() < 0.25 && !window.gameOver) { 
    ...
    // ==========================================
    // 5. HUD SYSTEM (MÁU, STAMINA, AVATAR)
    // ==========================================
    ...
    if (window.p1 && !window.gameOver && window.introTimer <= 120) {
        ...
        let maxNameWidthH = 400; 
        let maxNameWidthV = 280; 

        // --- HUD NGANG 1080P ---
        if (renderNormalH) {
            ctxH.lineJoin = "round"; ctxH.textAlign = "left"; ctxH.textBaseline = "alphabetic";
            ...
            ctxH.lineWidth = 7; ctxH.strokeStyle = "#000"; ctxH.font = "900 48px 'Arial Black', sans-serif"; 
            ctxH.strokeText(p1Name, 185, 96, maxNameWidthH); ctxH.fillStyle = "#fff"; ctxH.fillText(p1Name, 185, 96, maxNameWidthH);
            ...
            if (repEnemyObj) {
                ...
                if (img2) { ctxH.save(); ctxH.beginPath(); if (ctxH.roundRect) ctxH.roundRect(1750, 33, 73, 73, 8); else ctxH.rect(1750, 33, 73, 73); ctxH.clip(); ctxH.drawImage(img2, 1750, 33, 73, 73); ctxH.restore(); ctxH.lineWidth = 5; ctxH.strokeStyle = "#ff003c"; ctxH.strokeRect(1750, 33, 73, 73); }
                
                ctxH.lineWidth = 7; ctxH.strokeStyle = "#000"; ctxH.font = "900 48px 'Arial Black', sans-serif"; 
                ctxH.strokeText(eName, 1735, 96, maxNameWidthH); ctxH.fillStyle = "#fff"; ctxH.fillText(eName, 1735, 96, maxNameWidthH);
                
                drawSkewedPath(ctxH, 750, 120, 1000, 60, false); ctxH.fillStyle = "rgba(0,0,0,0.7)"; ctxH.fill(); ctxH.lineWidth = 7; ctxH.strokeStyle = "rgba(255,255,255,0.9)"; ctxH.stroke();
                if (p2Hp > 0) { let hpGrad = ctxH.createLinearGradient(750, 0, 1750, 0); hpGrad.addColorStop(0, "#c0392b"); hpGrad.addColorStop(1, "#e74c3c"); let eHpWidth = (1000 * p2Hp) | 0; drawSkewedPath(ctxH, 750 + (1000 - eHpWidth), 120, eHpWidth, 60, false); ctxH.fillStyle = hpGrad; ctxH.fill(); }
                ctxH.fillStyle = "rgba(0,0,0,0.8)"; ctxH.fillRect(1300, 193, 533, 20); ctxH.fillStyle = "#f1c40f"; ctxH.fillRect(1300 + (533 - ((533 * eStam) | 0)), 193, (533 * eStam) | 0, 20);
            }
        }
    ...
    // ==========================================
    // 5.5. RETENTION MULTIPLIER: FAKE CHAT + MINI GAME
    // ==========================================
    
    // --- KHỞI TẠO BIẾN (Chỉ chạy 1 lần) ---
    if (!window.retentionToolsInit) {
        window.retentionToolsInit = true;
        // Biến cho Chat
        window.fakeChat = []; window.lastChatTime = 0;
        window.chatNames = ["ToxicBoy99", "ProGamer_VN", "StickGod", "NoobSlayer", "CatLover", "DarkKnight", "SigmaMale", "ChillGuy", "TiktokScroller", "GigaChad"];
        window.chatMsgs = ["Bro is cooking 🔥", "NAHHH", "How??", "Scripted 💀", "W", "L", "Nerf boss pls", "Skill issue", "I stopped breathing", "MOM GET THE CAMERA", "Bro locked in 🥶"];
        window.hypeMsgs = ["🔥🔥🔥", "W W W W W", "OMG!!!", "HOLY W", "HE IS HIM", "💀💀💀💀💀", "BRO IS HACKING"];
        
        // Biến cho Mini-game
        window.brainrot = { jumpY: 0, velocity: 0, obstacles: [], speedMultiplier: 1 };
    }

    if (!window.gameOver && window.introTimer > 120) {
        let nowTime = Date.now();
        let isHype = audioPeak > 0.6; // Đang combat căng

        // ==========================================
        // TÍNH NĂNG 1: FAKE LIVE CHAT (GÓC TRÁI)
        // ==========================================
        let chatDelay = isHype ? 150 : (600 + Math.random() * 800); 
        
        if (nowTime - window.lastChatTime > chatDelay) {
            window.lastChatTime = nowTime;
            let msg = isHype ? window.hypeMsgs[Math.floor(Math.random() * window.hypeMsgs.length)] : window.chatMsgs[Math.floor(Math.random() * window.chatMsgs.length)];
            window.fakeChat.push({
                name: window.chatNames[Math.floor(Math.random() * window.chatNames.length)],
                msg: msg,
                color: `hsl(${Math.random() * 360}, 80%, 70%)`
            });
            if (window.fakeChat.length > 7) window.fakeChat.shift(); // Giữ 7 tin nhắn để vừa khung
        }

        ctxV.save();
        let chatBoxY = splitGameHeight + 50; 
        ctxV.translate(40, chatBoxY); // Đặt lệch sang trái
        
        // Vẽ background kính mờ cho Chat
        ctxV.fillStyle = "rgba(10, 15, 30, 0.4)";
        ctxV.strokeStyle = "rgba(0, 243, 255, 0.3)";
        ctxV.lineWidth = 2;
        if(ctxV.roundRect) { ctxV.beginPath(); ctxV.roundRect(0, 0, 480, 400, 15); ctxV.fill(); ctxV.stroke(); } 
        else { ctxV.fillRect(0, 0, 480, 400); ctxV.strokeRect(0, 0, 480, 400); }

        ctxV.beginPath(); if(ctxV.roundRect) ctxV.roundRect(0, 0, 480, 400, 15); ctxV.clip();

        ctxV.font = "bold 26px 'Arial'"; ctxV.textAlign = "left"; ctxV.textBaseline = "top";
        
        // Vẽ chữ "LIVE CHAT" header
        ctxV.fillStyle = "#ff0055"; ctxV.fillRect(0, 0, 480, 40);
        ctxV.fillStyle = "#fff"; ctxV.fillText("🔴 LIVE CHAT", 15, 7);

        // Render từng dòng chat
        for (let i = 0; i < window.fakeChat.length; i++) {
            let c = window.fakeChat[i];
            let drawY = 350 - ((window.fakeChat.length - 1 - i) * 45); // Cuộn từ dưới lên
            
            // Vẽ tên
            ctxV.fillStyle = c.color;
            ctxV.fillText(c.name + ":", 15, drawY);
            
            // Vẽ tin nhắn
            ctxV.fillStyle = "#ffffff";
            let nameWidth = ctxV.measureText(c.name + ": ").width;
            ctxV.fillText(c.msg, 15 + nameWidth, drawY);
        }
        ctxV.restore();


        // ==========================================
        // TÍNH NĂNG 3: SATISFYING CUBE JUMPER (BÊN DƯỚI)
        // ==========================================
        ctxV.save();
        let gameBoxY = splitGameHeight + 630; // Nằm bên dưới VTuber và Chat
        ctxV.translate(540, gameBoxY); // Căn giữa màn hình (X = 540)
        
        // Khung game thu nhỏ (Width: 900, Height: 260)
        ctxV.fillStyle = "#020617";
        if(ctxV.roundRect) { ctxV.beginPath(); ctxV.roundRect(-450, -130, 900, 260, 20); ctxV.fill(); ctxV.clip(); }
        else { ctxV.fillRect(-450, -130, 900, 260); }

        // Vẽ đường chạy không gian
        ctxV.fillStyle = "#0f172a"; ctxV.beginPath(); ctxV.moveTo(-450, 40); ctxV.lineTo(450, 40); ctxV.lineTo(550, 130); ctxV.lineTo(-550, 130); ctxV.fill();
        ctxV.strokeStyle = "#00f3ff"; ctxV.lineWidth = 2;
        
        // Tốc độ lưới trôi nhanh hơn khi combat
        window.brainrot.speedMultiplier = isHype ? 1.8 : 1.0;
        
        for(let i=0; i<10; i++) {
            let offset = ((nowTime / 3 * window.brainrot.speedMultiplier) + i * 90) % 450;
            ctxV.beginPath(); ctxV.moveTo(-450 - offset*0.5, 40 + offset*0.2); ctxV.lineTo(450 + offset*0.5, 40 + offset*0.2); ctxV.stroke();
        }

        // Khung chữ cảnh báo góc game
        ctxV.fillStyle = "rgba(255, 0, 85, 0.2)"; ctxV.fillRect(-450, -130, 900, 40);
        ctxV.fillStyle = "#ff4757"; ctxV.font = "900 24px 'Arial Black'"; ctxV.textAlign = "center";
        ctxV.fillText("⚠️ SATISFYING GAMEPLAY (DO NOT LOOK AWAY)", 0, -105);

        // Logic nhảy (Nếu âm thanh to -> Tự động nhảy)
        if (isHype && window.brainrot.jumpY === 0 && Math.random() < 0.3) { window.brainrot.velocity = -16; }
        window.brainrot.jumpY += window.brainrot.velocity;
        window.brainrot.velocity += 1.2; // Trọng lực
        if (window.brainrot.jumpY > 0) { window.brainrot.jumpY = 0; window.brainrot.velocity = 0; }

        // Logic sinh chướng ngại vật
        if (Math.random() < (0.02 * window.brainrot.speedMultiplier)) window.brainrot.obstacles.push(500); 
        
        ctxV.fillStyle = "#ffeb3b";
        for (let i=0; i<window.brainrot.obstacles.length; i++) {
            let obsX = window.brainrot.obstacles[i];
            window.brainrot.obstacles[i] -= (10 * window.brainrot.speedMultiplier); // Tốc độ trôi về trái
            
            // Vẽ chướng ngại vật (Gai tam giác)
            ctxV.beginPath();
            ctxV.moveTo(obsX, 40); ctxV.lineTo(obsX + 20, 0); ctxV.lineTo(obsX + 40, 40);
            ctxV.fill();
        }
        window.brainrot.obstacles = window.brainrot.obstacles.filter(x => x > -500);

        // Vẽ Cube nhảy (Player)
        let playerY = 40 + window.brainrot.jumpY;
        let isSpinning = window.brainrot.jumpY < 0;
        
        ctxV.save();
        ctxV.translate(-250, playerY - 20); // Vị trí khối hộp
        if (isSpinning) ctxV.rotate(window.brainrot.jumpY * 0.05); // Xoay vòng khi nhảy
        
        ctxV.fillStyle = "#00f3ff"; ctxV.strokeStyle = "#fff"; ctxV.lineWidth = 3;
        ctxV.fillRect(-20, -20, 40, 40);
        ctxV.strokeRect(-20, -20, 40, 40);
        
        // Mắt khối hộp
        ctxV.fillStyle = "#000"; 
        ctxV.fillRect(-10, -10, 6, 12); ctxV.fillRect(4, -10, 6, 12);
        ctxV.restore();

        // Chớp viền màn hình game khi đang combat gắt
        if (isHype) {
            ctxV.strokeStyle = "rgba(255, 0, 85, 0.5)"; ctxV.lineWidth = 10;
            if(ctxV.roundRect) { ctxV.beginPath(); ctxV.roundRect(-450, -130, 900, 260, 20); ctxV.stroke(); }
        }
        ctxV.restore();
    }
    ...
    if (!window.gameOver) {
        ...
        if (window.StoryModeAI.displayedText.length > 0) {
            ...
            if (renderNormalH) drawAudioVisualizer(ctxH, 960, 960, 800, 80);  
            
            let textPulse = 1 + (audioPeak * 0.1);

            ctxV.save(); ctxV.translate(540, visY + 30); ctxV.scale(textPulse, textPulse); ctxV.textAlign = "center"; ctxV.textBaseline = "top";
            ctxV.font = "900 65px 'Montserrat', 'Arial Black', sans-serif"; 
            ctxV.fillStyle = "#fff"; ctxV.strokeStyle = "#000000"; ctxV.lineWidth = 14; ctxV.lineJoin = "round";
            wrapTextTikTok(ctxV, window.StoryModeAI.displayedText, 0, 0, 1000, 80); 
            ctxV.restore();

            if (renderNormalH) {
                ctxH.save(); ctxH.translate(960, 980); ctxH.scale(textPulse, textPulse); ctxH.textAlign = "center"; ctxH.textBaseline = "top";
                ctxH.font = "900 45px 'Montserrat', 'Arial Black', sans-serif"; ctxH.fillStyle = "#fff"; ctxH.strokeStyle = "#000000"; ctxH.lineWidth = 11; ctxH.lineJoin = "round";
                wrapTextTikTok(ctxH, window.StoryModeAI.displayedText, 0, 0, 1800, 55); 
                ctxH.restore();
            }
        }
    }
