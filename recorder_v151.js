// ==========================================
// RECORDER.JS - V77.0 THE ALGORITHM BREAKER (STREAMER UI EDITION)
// ĐÃ FIX: Lỗi Outro đè Intro, Crash biến số, Cấu trúc lại Bố cục không che Game.
// NÂNG CẤP TỐI THƯỢNG: Facecam đặt nổi bật trên Bảng Chat, Chat cuộn mượt tốc độ siêu cao.
// TÂM LÝ HỌC: APM/BPM, Sạn cố ý (Bait), Sai chính tả, Donation Super Chat.
// ==========================================
...
// ẢNH FACECAM STREAMER CÂU VIEW (Gái xinh gaming)
window.FACECAM_IMAGE_URL = "https://images.unsplash.com/photo-1593305841991-05c297ba4575?w=400&q=80";
...
window._donationAlert = null;

// QUẢN LÝ CHAT SCROLL VÀ HIỆU ỨNG
...
window._chatScrollOffset = 0; // Biến tạo hiệu ứng cuộn mượt cho Chat

window._engagementParticles = Array.from({length: 20}, () => ({ active: false, x: 0, y: 0, v: 0, icon: "", alpha: 0, scale: 1 }));
window._retentionParticles = Array.from({length: 40}, () => ({ active: false, x: 0, y: 0, s: 0, v: 0, h: 0, age: 0 }));
window._retentionEmojis = Array.from({length: 15}, () => ({ active: false, x: 0, y: 0, v: 0, e: "", r: 0, age: 0 }));
...
// SỬ DỤNG WINDOW. ĐỂ TRÁNH LỖI CRASH KHI DÁN CODE NHIỀU LẦN
window.THEMES = [
...
window.WAGER_BADGES = ["🏆 GLOBAL TOP 1% RANKED DUEL", "💰 $10,000 WAGER MATCH", "💀 PERMA-BAN DEATHMATCH"];

window.makeTypo = function(str) {
...
window.generateIntroParams = function() {
    ...
    let t1 = r(window.THEMES); let t2 = r(window.THEMES.filter(t => t.id !== t1.id));
    let lore = r(window.LORES); let fakeChapter = window.makeTypo(lore.chapter);
    return { themeP1: t1, themeP2: t2, lore: { ...lore, chapter: fakeChapter }, badge: r(window.WAGER_BADGES) };
};
...
window.CELEB_LIST = [
    ...
    { name: "Faker 🐐", color: "#00f3ff" }, { name: "S1mple 🔫", color: "#ffeb3b" },
...
window.TOXIC_MSGS = [
    ...
    "Bro thinks he is him 😭", "My grandma plays better 👵", "He needs some milk 🥛", "Caught in 4K 📸", 
    "THE LORE IS CRAZY 😭", "Bro has anime protagonist aura ☢️", "What the sigma 🗿", "Negative aura 📉",
    "Đánh như cái máy khâu 🐔", "Xóa game đi bạn êi 🤡", "Quả xử lý cồng kềnh vãi 💀", "Mua acc à bạn?", 
    "Tuyển bạn này vào đánh giải... làng 🤣", "Khóc đi 😭", "Gà 🐔🐔", "Đánh mù mẹ mắt tao rồi 👁️👄👁️",
    "Phản xạ của người già à bro?", "Lùa gà à 🐧", "Cầm chân đánh à bạn?", "Ảo game à =))))", 
    "ㅋㅋ 봇인줄", "실화냐? 💀", "삭제해라 게임", "피지컬 보소 ㄷㄷ", "草", "何これｗｗｗ", 
    "弱すぎだろ", "やばい💀", "wwwwwwww", "太菜了吧 😂", "主播别丢人了", "666666", "回家种田吧", "牛逼！"
];

window.CLUTCH_MSGS = [
    "LET HIM COOK 🔥", "1 HP DREAM 💀", "OMG 😱", "HE IS THE HONORED ONE", 
    "DOMAIN EXPANSION", "STAND PROUD", "CLUTCH 🤯", "W W W W W W", "NO WAY ☢️", "MAIN CHARACTER ENERGY"
];

window._recentChatsMemory = [];

window.generateLiveChatEvent = function(isClutch = false) {
    const r = (arr) => arr[Math.floor(Math.random() * arr.length)];
    let celeb = r(window.CELEB_LIST);
    let msgList = isClutch ? window.CLUTCH_MSGS : window.TOXIC_MSGS;
    
    let msg = r(msgList);
    let attempts = 0;
    while (window._recentChatsMemory.includes(msg) && attempts < 10) { msg = r(msgList); attempts++; }
    
    window._recentChatsMemory.push(msg); 
    if (window._recentChatsMemory.length > 15) window._recentChatsMemory.shift();
...
window.precalcChatText = function(chatObj, ctx) {
    ...
    let maxMsgWidth = 920 - 60 - chatObj.nameWidth - 10; let words = chatObj.msg.split(' '); let lines = []; let currentLine = "";
...
function drawBaitSạn(ctx, w, h) {
    ...
    if (window._baitType === 0) {
        ...
        ctx.fillText("Low Battery", 0, -40); ctx.font = "20px Arial"; ctx.fillText("10% battery remaining.", 0, 0);
        ...
        ctx.fillStyle = "rgba(255, 255, 255, 0.98)"; ctx.shadowColor = "rgba(0,0,0,0.3)"; ctx.shadowBlur = 20;
        ...
        if (Math.floor(Date.now() / 200) % 2 === 0) {
            ...
            ctx.fillStyle = "#ff0000"; ctx.font = "bold 30px 'Arial Black', sans-serif"; ctx.textAlign = "center"; ctx.textBaseline = "middle"; ctx.fillText("📶 999ms", 60, 0);
        }
    }
    ctx.restore();
}
...
window.drawProceduralIntro = function(ctx, w, h, progress) {
    ...
    ctx.translate(w/2 + cutShake + panX, h/2 + cutShake); ctx.scale(currentZoom, currentZoom); ctx.translate(0, 100); 
    ...
    if (act === 0) {
        ...
        ctx.beginPath(); if(ctx.roundRect) ctx.roundRect(-chatW/2, -chatH/2, chatW, chatH, 20); else ctx.fillRect(-chatW/2, -chatH/2, chatW, chatH); ctx.fill(); ctx.stroke();
        ...
        if (Math.random() < 0.4) { window._introBottomChats.push(window.generateLiveChatEvent()); if (window._introBottomChats.length > 4) window._introBottomChats.shift(); }
        
        ctx.textAlign="left"; ctx.font="bold 30px Arial"; let cY = -chatH/2 + 80;
        for(let i=0; i < window._introBottomChats.length; i++) {
            ...
            let text = c.msg.length > 35 ? c.msg.substring(0,35)+"..." : c.msg; ctx.fillText(" " + text, -chatW/2 + 30 + nW, cY); cY += 50;
        }
        ctx.restore(); ctx.save(); ctx.translate(w/2 + cutShake + panX, h/2 + cutShake); ctx.scale(currentZoom, currentZoom); ctx.translate(0, 100); 
    }
    else if (act === 2) {
        drawFighterArtistic(e1, 0, 100, 'cast', false, prm.themeP2, realP2Name);
        ctx.restore(); ctx.save(); ctx.translate(w/2 + cutShake, h/2 - 250 + cutShake);
        ...
        if (localProg > 0.3) {
            ctx.fillStyle = "rgba(255,255,255,0.95)"; ctx.beginPath(); if(ctx.roundRect) ctx.roundRect(20, 60, 360, 70, 35); else ctx.fillRect(20, 60, 360, 70); ctx.fill();
        ...
        let pushP1 = -500 + (easeIn * 400); let pushP2 = 500 - (easeIn * 400);
        for(let t=1; t<=3; t++) {
            ...
            drawFighterArtistic(e1, trP2, 100, 'attack', false, prm.themeP2, null, true); drawFighterArtistic(window.p1, trP1, 100, 'attack', true, prm.themeP1, null, true);
        }
...
window.startRecording = function() {
    ...
    // FIX: CHẶN LỖI OUTRO ĐƯỢC VẼ RA KHI MỚI BẤM RECORD TỪ MÀN HÌNH END-GAME
    window.matchEndTimer = 0; 
    window.gameOver = false;

    window.introParams = window.generateIntroParams();

    window.recordedChunksV = []; window._retentionParticles.forEach(p => p.active = false); window._retentionEmojis.forEach(e => e.active = false);
...
window.captureFrames = function() {
    ...
    if (renderNormalV && !window._donationAlert && Math.random() < 0.005 && elapsed > 5000) {
        let donator = window.CELEB_LIST[Math.floor(Math.random()*window.CELEB_LIST.length)];
        let msgs = ["YOU ARE TRASH BRO 💀", "Uninstall right now 😂", "DO A FLIP!", "My eyes are bleeding 👁️👄👁️", "Bro is playing with his feet 🦶"];
        window._donationAlert = { name: donator.name, msg: msgs[Math.floor(Math.random()*msgs.length)], life: 250, color: donator.color };
    }
    ...
    if (renderNormalV) {
        ...
        // ⌨️ APM TRACKER & ❤️ HEART RATE (GÓC GAME)
        if (!window._apm) window._apm = 120; let targetApm = 120 + (audioPeak * 350); window._apm += (targetApm - window._apm) * 0.15;
        ctxV.save(); ctxV.translate(1080 - 130, 60); ctxV.fillStyle = "rgba(0, 0, 0, 0.65)";
        ctxV.beginPath(); if(ctxV.roundRect) ctxV.roundRect(-110, -25, 220, 50, 12); else ctxV.fillRect(-110, -25, 220, 50); ctxV.fill();
        ctxV.fillStyle = window._apm > 250 ? "#ff0055" : "#ffeb3b"; ctxV.font = "900 22px 'Arial Black'"; ctxV.textAlign = "center"; ctxV.textBaseline = "middle"; ctxV.fillText("⌨️ APM: " + Math.floor(window._apm), 0, 2); ctxV.restore();

        if (!window._bpm) window._bpm = 85; let targetBpm = 85 + (audioPeak * 110); window._bpm += (targetBpm - window._bpm) * 0.05;
        ctxV.save(); ctxV.translate(130, 60); ctxV.fillStyle = "rgba(0, 0, 0, 0.65)";
        ctxV.beginPath(); if(ctxV.roundRect) ctxV.roundRect(-110, -25, 220, 50, 12); else ctxV.fillRect(-110, -25, 220, 50); ctxV.fill();
        ctxV.fillStyle = window._bpm > 150 ? "#ff0055" : "#00ffcc"; ctxV.font = "900 22px 'Arial Black'"; ctxV.textAlign = "center"; ctxV.textBaseline = "middle";
        let heartPulse = 1 + Math.sin(Date.now() / (1000 / (window._bpm / 60))) * 0.2;
        ctxV.save(); ctxV.scale(heartPulse, heartPulse); ctxV.fillText("❤️", -65, 0); ctxV.restore(); ctxV.fillText(Math.floor(window._bpm) + " BPM", 15, 2); ctxV.restore();

        // NÚT LIVE TRỰC TIẾP TRONG GAME (GÓC PHẢI DƯỚI)
        if (!window._fakeViewers) window._fakeViewers = 350 + Math.floor(Math.random() * 500);
        ctxV.save(); let liveW = 210, liveH = 45; let liveX = 1080 - liveW / 2 - 15; let liveY = splitGameHeight - liveH / 2 - 15; 
        ctxV.translate(liveX, liveY); ctxV.fillStyle = "rgba(0, 0, 0, 0.75)";
        ctxV.beginPath(); if(ctxV.roundRect) ctxV.roundRect(-liveW/2, -liveH/2, liveW, liveH, 22.5); else ctxV.fillRect(-liveW/2, -liveH/2, liveW, liveH); ctxV.fill();
        let redPulse = 1 + (audioPeak * 0.4); ctxV.fillStyle = "#ff0000";
        ctxV.beginPath(); ctxV.arc(-liveW/2 + 25, 0, 7 * redPulse, 0, Math.PI*2); ctxV.fill();
        ctxV.fillStyle = "#fff"; ctxV.font = "bold 20px Arial"; ctxV.textAlign = "left"; ctxV.textBaseline = "middle";
        ctxV.fillText("LIVE  |  👁️ " + window._fakeViewers.toLocaleString(), -liveW/2 + 45, 2); ctxV.restore();

        let p1HpRatio = window.p1 ? Math.max(0, window.p1.hp / window.p1.maxHp) : 1;
        let isClutchMode = p1HpRatio > 0 && p1HpRatio < 0.25;
        if (isClutchMode && !isOutroActive) {
            let hb = Math.sin(Date.now() / 150); ctxV.fillStyle = `rgba(255, 0, 0, ${0.15 + hb * 0.15})`; ctxV.fillRect(0, 0, 1080, splitGameHeight);
            ctxV.font = "900 45px 'Arial Black'"; ctxV.textAlign = "center"; ctxV.fillStyle = "rgba(255, 0, 0, 0.6)"; ctxV.fillText("⚠️ CRITICAL HP ⚠️", 540, splitGameHeight - 50);
        }
        ...
        // VẼ PHẦN NỀN UI BÊN DƯỚI (KHÔNG LẤN LÊN GAME)
        ...
        // WATERMARK CHÌM Ở GIỮA
        let brandWM = window.CREATOR_HANDLE || "Sticklom";
        ctxV.save(); ctxV.translate(540, 960); ctxV.rotate(-0.15); ctxV.font = "900 70px 'Arial Black', sans-serif"; ctxV.textAlign = "center"; ctxV.textBaseline = "middle";
        ctxV.lineWidth = 4; ctxV.strokeStyle = "rgba(0, 0, 0, 0.15)"; ctxV.strokeText(brandWM, 0, 0); ctxV.fillStyle = "rgba(255, 255, 255, 0.25)"; ctxV.fillText(brandWM, 0, 0); ctxV.restore();

        // 1. CHAT BOX (Nằm trọn dưới đáy)
        let boxWidth = 960; let boxHeight = 700; 
        let boxX = 540; let boxY = 1920 - boxHeight/2 - 20; 
        let chatTopY = boxY - boxHeight/2;

        // 2. FACECAM (Đè ngay trên góc trái Chat Box)
        if (!window._facecamImg) { window._facecamImg = new Image(); window._facecamImg.crossOrigin = "Anonymous"; window._facecamImg.src = window.FACECAM_IMAGE_URL; }
        if (window._facecamImg && window._facecamImg.complete && !isOutroActive) {
            ctxV.save(); let camW = 320; let camH = 420; let camX = 60 + camW/2; let camY = chatTopY - camH/2 - 10;
            if (audioPeak > 0.5) { camX += (Math.random() - 0.5) * 15; camY += (Math.random() - 0.5) * 15; }
            ctxV.translate(camX, camY);
            ctxV.beginPath(); if(ctxV.roundRect) ctxV.roundRect(-camW/2, -camH/2, camW, camH, 20); else ctxV.rect(-camW/2, -camH/2, camW, camH); ctxV.clip();
            let zoom = 1.0 + (audioPeak * 0.35); ctxV.scale(zoom, zoom); ctxV.drawImage(window._facecamImg, -camW/2, -camH/2, camW, camH); ctxV.restore();
            ctxV.save(); ctxV.translate(camX, camY); ctxV.lineWidth = 6; ctxV.strokeStyle = (Math.floor(Date.now()/200)%2===0) ? "#ff0055" : "#00f3ff";
            ctxV.beginPath(); if(ctxV.roundRect) ctxV.roundRect(-camW/2, -camH/2, camW, camH, 20); else ctxV.rect(-camW/2, -camH/2, camW, camH); ctxV.stroke();
            ctxV.fillStyle = "#ff0000"; ctxV.beginPath(); if(ctxV.roundRect) ctxV.roundRect(-camW/2 + 10, -camH/2 + 10, 60, 25, 5); else ctxV.rect(-camW/2 + 10, -camH/2 + 10, 60, 25); ctxV.fill();
            ctxV.fillStyle = "#fff"; ctxV.font = "bold 14px Arial"; ctxV.textAlign = "center"; ctxV.textBaseline = "middle"; ctxV.fillText("LIVE", -camW/2 + 40, -camH/2 + 24); ctxV.restore();
        }

        // 3. THÔNG BÁO TẶNG QUÀ / THẢ TIM (Bên phải Facecam)
        if (!isOutroActive) {
            if (Date.now() - window._lastAlertTime > 2000 + Math.random() * 2000) { 
            ...
            let alertStartY = chatTopY - 30; 
            for (let i = window._liveAlerts.length - 1; i >= 0; i--) {
                let al = window._liveAlerts[i]; al.life -= 0.02; al.yOffset += 1.5;
                ctxV.save(); ctxV.globalAlpha = Math.max(0, Math.min(1, al.life * 1.5)); ctxV.font = "bold 24px Arial"; let textW = ctxV.measureText(al.text).width; 
                ctxV.translate(750, alertStartY - al.yOffset); 
                ctxV.fillStyle = "rgba(0, 0, 0, 0.5)"; ctxV.strokeStyle = "rgba(255, 255, 255, 0.2)"; ctxV.lineWidth = 2;
                if(ctxV.roundRect) { ctxV.beginPath(); ctxV.roundRect(-textW/2 - 20, 0, textW + 40, 46, 23); ctxV.fill(); ctxV.stroke(); } else { ctxV.fillRect(-textW/2 - 20, 0, textW + 40, 46); } 
        ...
        // 4. BẢNG WIN RATE POLL (Đè góc phải HUD)
        ...
        let repEnemyObj = window.enemies && window.enemies.length > 0 ? window.enemies[0] : null; let p2Hp = 0.5; 

        if (window.p1 && !isOutroActive) {
            ...
            let img1 = getHudImg(p1Url); let img2 = getHudImg(p2Url); let hudBaseY = splitGameHeight + 30; 
            
            // Draw Poll
            if (repEnemyObj) {
                ctxV.save(); let pollWidth = 550; let pollX = 1080 - 30 - pollWidth/2; let pollY = chatTopY - 60; ctxV.translate(pollX, pollY); 
                ...
                ctxV.fillStyle = "#000"; ctxV.font = "900 24px 'Arial Black'"; ctxV.textAlign = "left"; ctxV.fillText(`${Math.round(p1Pct*100)}%`, -pollWidth/2 + 20, 18); ctxV.textAlign = "right"; ctxV.fillText(`${Math.round((1-p1Pct)*100)}%`, pollWidth/2 - 20, 18); ctxV.restore();
            }
        }

        // HIỂN THỊ SUPER CHAT DONATION
        if (window._donationAlert && !isOutroActive) {
            let da = window._donationAlert; da.life--; ctxV.save();
            let daY = chatTopY - 200;
            if (da.life > 230) daY = chatTopY - 200 - (da.life - 230)*15; 
            if (da.life < 20) daY = chatTopY - 200 - (20 - da.life)*15; 
            ctxV.translate(750, daY); 
            ctxV.fillStyle = "rgba(255, 215, 0, 0.95)"; ctxV.shadowColor = "#ffb800"; ctxV.shadowBlur = 20;
            ctxV.beginPath(); if(ctxV.roundRect) ctxV.roundRect(-300, -45, 600, 90, 45); else ctxV.fillRect(-300, -45, 600, 90); ctxV.fill();
            ctxV.shadowBlur = 0; ctxV.fillStyle = "#000"; ctxV.textAlign = "center"; ctxV.textBaseline = "middle"; ctxV.font = "900 24px 'Arial Black'";
            ctxV.fillText(`💰 ${da.name} donated $100:`, 0, -18); ctxV.font = "bold 22px Arial"; ctxV.fillText(`"${da.msg}"`, 0, 18); ctxV.restore();
            if (da.life <= 0) window._donationAlert = null;
        }

        // ====================================================================
        // ENGINE CHAT SCROLL TỐC ĐỘ SIÊU CAO
        // ====================================================================
        if (!window._chatSystemInit) { 
            window._chatSystemInit = true; window._liveChats = []; window._lastChatUpdate = Date.now(); window._nextChatDelay = 150; window._chatScrollOffset = 0;
            for(let i = 0; i < 15; i++) { let c = window.generateLiveChatEvent(isClutchMode); window.precalcChatText(c, ctxV); window._liveChats.push(c); } 
        }

        if (window._chatScrollOffset === undefined) window._chatScrollOffset = 0;
        if (window._chatScrollOffset > 0) window._chatScrollOffset -= 4; // Trượt mượt lên trên (Cuộn Chat)
        if (window._chatScrollOffset < 0) window._chatScrollOffset = 0;

        let chatNowC = Date.now();
        if (chatNowC - window._lastChatUpdate > window._nextChatDelay && !isOutroActive) { 
            window._lastChatUpdate = chatNowC; 
            // Tốc độ chat siêu nhanh: 150ms -> 350ms
            window._nextChatDelay = isClutchMode ? 100 + Math.random() * 150 : 150 + Math.random() * 200; 
            let newChat = window.generateLiveChatEvent(isClutchMode); window.precalcChatText(newChat, ctxV); 
            window._liveChats.push(newChat); 
            if (window._liveChats.length > 20) window._liveChats.shift(); 
            window._chatScrollOffset += 45; // Nhích vị trí hiển thị xuống để chuẩn bị cuộn mượt lên
            if(window._chatScrollOffset > 135) window._chatScrollOffset = 135; 
        }

        if (!isOutroActive) {
            // VẼ KHUNG BẢNG CHAT
            ctxV.save(); ctxV.translate(boxX, boxY); ctxV.fillStyle = "rgba(10, 15, 30, 0.65)"; ctxV.strokeStyle = "rgba(0, 243, 255, 0.3)"; ctxV.lineWidth = 3;
            ...
            // Giới hạn vùng in Text Chat
            ctxV.save(); ctxV.beginPath(); ctxV.rect(-boxWidth/2, -boxHeight/2, boxWidth, boxHeight); ctxV.clip(); 
            
            let currentY = boxHeight/2 - 20 + window._chatScrollOffset; 
            let lineHeight = 45; ctxV.textAlign = "left"; ctxV.textBaseline = "bottom"; ctxV.font = "bold 34px Arial"; let headerBottomY = -boxHeight/2 + 20; 
            ...
            if (Math.random() < 0.3) {
                let p = window._retentionParticles.find(p => !p.active);
            ...
            for (let i = 0; i < window._retentionParticles.length; i++) {
                let p = window._retentionParticles[i]; if (!p.active) continue;
