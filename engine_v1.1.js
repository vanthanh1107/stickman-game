// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 10.5 [PERFORMANCE CORE]
// [PHIÊN BẢN ULTIMATE WEATHER & COMBAT - TỐI ƯU ANTI-LAG]
// [TÍNH NĂNG: LOADING SCREEN, FLASH STEP, GUARD BREAK, AWAKENING, SIN CITY FINISHER, SPEED LINES]
// ==========================================
...
// [NEW 10.5] BIẾN HỆ THỐNG PRELOAD & TỐI ƯU BỘ NHỚ (ANTI-LAG)
window.isLoading = true;
window.loadProgress = 0;
window.chromaCanvas = null; // Khởi tạo 1 lần duy nhất để chống tràn RAM
...
window.initGameEngine = function() {
    ...
    // TẠO BỘ NHỚ ĐỆM (OFFSCREEN CANVAS) TRƯỚC KHI VÀO GAME ĐỂ TRÁNH LAG
    ...
    // Giả lập quá trình Compile Shaders và chuẩn bị Assets
    let loadInterval = setInterval(() => {
        ...
        if (window.loadProgress >= 100) {
            ...
            setTimeout(() => { 
                ...
                window.playSound(800, 'sine', 0.5, 0.5); // Tiếng ting báo hiệu xong
            }, 500);
        }
    }, 40);
};
...
// Hiệu ứng Kính vỡ khi Guard Break
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce) {
    ...
    if (finalDmg > 0) {
        ...
        // KẾT LIỄU SIN CITY ĐIỆN ẢNH (CHỈ KHI CHÍ MẠNG)
        if (target.hp <= 0) {
...
window.update = function() {
    ...
    // Dừng cập nhật logic nếu đang Loading màn hình
...
window.draw = function() {
    ...
    // [NEW 10.5] VẼ MÀN HÌNH LOADING ĐIỆN ẢNH
    if (window.isLoading) {
        window.ctx.setTransform(1, 0, 0, 1, 0, 0);
        window.ctx.fillStyle = "#050505";
        window.ctx.fillRect(0, 0, window.canvas.width, window.canvas.height);
        
        window.ctx.strokeStyle = "rgba(0, 243, 255, 0.05)";
        ...
        window.ctx.fillStyle = "#00f3ff"; window.ctx.font = "italic 900 40px Arial"; window.ctx.textAlign = "center";
        window.ctx.shadowBlur = 15; window.ctx.shadowColor = "#00f3ff";
        window.ctx.fillText("INITIALIZING ENGINE 10.5...", cx, cy - 50);
        
        window.ctx.shadowBlur = 0; window.ctx.strokeStyle = "#333"; window.ctx.lineWidth = 4; window.ctx.strokeRect(cx - 250, cy, 500, 24);
        
        window.ctx.fillStyle = "#ff4757"; window.ctx.shadowBlur = 20; window.ctx.shadowColor = "#ff4757";
        window.ctx.fillRect(cx - 247, cy + 3, (window.loadProgress / 100) * 494, 18); window.ctx.shadowBlur = 0;
        
        window.ctx.fillStyle = "#fff"; window.ctx.font = "bold 16px monospace";
        window.ctx.fillText(`COMPILING ASSETS & SHADERS... ${Math.floor(window.loadProgress)}%`, cx, cy + 60);
    ...
    try {
       ...
       if (window.envHazards && window.envHazards.length > 0) {
        ...
        // HIỆU ỨNG GRAYSCALE (SIN CITY FINISHER) - Tách màu nhân vật và bối cảnh
        ...
        // VẼ GẠCH TỐC ĐỘ ANIME (ANIME SPEED LINES)
        ...
        // [ĐÃ TỐI ƯU ANTI-LAG] HIỆU ỨNG NHIỄU MÀU (CHROMA TIMER)
        if (window.chromaTimer > 0) {
            let shift = window.chromaTimer * 1.5; 
            
            if (!window.chromaCanvas) window.chromaCanvas = document.createElement('canvas');
            if (window.chromaCanvas.width !== window.canvas.width) {
                window.chromaCanvas.width = window.canvas.width; 
                window.chromaCanvas.height = window.canvas.height;
            }
            
            let tCtx = window.chromaCanvas.getContext('2d', { alpha: false }); 
            tCtx.drawImage(window.canvas, 0, 0);
            
            window.ctx.setTransform(1, 0, 0, 1, 0, 0); 
            window.ctx.globalCompositeOperation = 'lighter'; 
            window.ctx.globalAlpha = 0.4;
            window.ctx.drawImage(window.chromaCanvas, shift, 0); 
            window.ctx.drawImage(window.chromaCanvas, -shift, 0);
            window.ctx.globalAlpha = 1.0; 
            window.ctx.globalCompositeOperation = 'source-over'; 
            window.chromaTimer--;
...
// TỰ ĐỘNG KÍCH HOẠT HỆ THỐNG PRELOAD KHI VỪA KHỞI CHẠY (GIÚP CHỐNG LAG NGAY LẬP TỨC)
if (typeof window !== 'undefined') {
    setTimeout(() => {
        if(typeof window.initGameEngine === 'function') window.initGameEngine();
    }, 100);
}
