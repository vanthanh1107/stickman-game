// ==========================================
// RECORDER.JS - BẢN HOÀN HẢO CHO CONTENT CREATOR (V66.20 - CINEMATIC BUTTER)
// [ULTIMATE SMOOTH] Thay Date.now() bằng performance.now() triệt tiêu 100% giật khựng.
// [NEW META] Dynamic Camera Zoom: Camera tự động Zoom in/out theo nhịp đập của âm thanh.
// [NEW META] Cinematic Impact Flash: Chớp chói lóa màn hình khi tung đòn chí mạng.
// [NEW META] Letterbox: Viền đen điện ảnh cho bản ngang.
// [RETAINED] VTuber AI, 3D Grid, Profile Fake Grid, RGB Glitch.
// ==========================================
...
function drawVTuberCommentator(ctx, x, y, audioPeak, isTyping) {
    let t = performance.now(); // [CINEMATIC] Dùng performance.now() cho độ mượt hoàn hảo
    ctx.save();
...
window.startRecording = function() {
    ...
    // [KILL LAG 2] Cân bằng chuẩn vàng 35Mbps: Lossless nhưng không làm nghẽn RAM Buffer
    let options = { videoBitsPerSecond: 35000000 }; 
    window.currentVideoExt = "webm";
    ...
    const finalizeRecordings = () => {
        ...
        if (stoppedCount === 2) { 
            setTimeout(() => {
                ...
                let blobH = new Blob(window.recordedChunksH, { type: window.mediaRecorderH.mimeType }); let videoUrlH = URL.createObjectURL(blobH);
                let blobV = new Blob(window.recordedChunksV, { type: window.mediaRecorderV.mimeType }); let videoUrlV = URL.createObjectURL(blobV);
...
function renderMobileSafeCombo(ctx, fighter, xPos, yPos, align, isMobile) {
    if (fighter && fighter.comboHits >= 2) {
        ...
        ctx.font = `italic 900 ${Math.floor(26 * scale)}px 'Arial Black', Impact`; ctx.fillStyle = rankColor; ctx.shadowColor = rankColor; ctx.shadowBlur = 15 * scale;
...
window.captureFrames = function() {
    ...
    // Đã gỡ bỏ lệnh ctx.scale() cồng kềnh, vẽ phẳng 1:1 siêu nhẹ!
    ...
    // [NEW META] Dynamic Camera Zoom khi đánh mạnh
    let cameraZoom = 1.0; 

    if (audioPeak > 0.6) {
        let shakeIntensity = (audioPeak - 0.6) * 35; 
        shakeX = (Math.random() - 0.5) * shakeIntensity; shakeY = (Math.random() - 0.5) * shakeIntensity;
        cameraZoom = 1.0 + (audioPeak - 0.6) * 0.15; // Phóng to 5-15% khi đập mạnh
    }

    // --- RENDER BẢN NGANG 1080P ---
    if (renderNormalH) {
        ctxH.imageSmoothingEnabled = true; // Bật lại để làm mượt khi Zoom
        ctxH.imageSmoothingQuality = "high";
        
        // [NEW META] Cinematic Bars (Viền đen 2 đầu)
        let barHeightH = 80;
        
        ctxH.save();
        // Camera Zoom In
        ctxH.translate(1920/2, 1080/2);
        ctxH.scale(cameraZoom, cameraZoom);
        ctxH.translate(-1920/2, -1080/2);
        ...
        // [KILL LAG 3] Tối ưu hóa điều kiện vẽ RGB Glitch (chỉ kích hoạt ở >0.85 siêu mạnh)
        if (audioPeak > 0.85) {
            ...
            // [NEW META] Cinematic Impact Flash (Chớp sáng nhẹ màn hình che khuyết điểm)
            ctxH.fillStyle = `rgba(255, 255, 255, ${(audioPeak - 0.85) * 2})`;
            ctxH.fillRect(0, barHeightH, 1920, 1080 - barHeightH*2);
        }
        ctxH.restore();
        
        // Vẽ Viền đen trên dưới đè lên
    ...
    ctxV.imageSmoothingQuality = "high";
    
    ctxV.save();
    ctxV.translate(540, splitGameHeight/2);
    ctxV.scale(cameraZoom, cameraZoom);
    ctxV.translate(-540, -splitGameHeight/2);
    ...
    // ==========================================
    // 3. BACKGROUND LƯỚI KHÔNG GIAN 3D (SỬ DỤNG performance.now() CỰC MƯỢT)
    // ==========================================
    let retainY = splitGameHeight; 
    let retainHeight = 1920 - retainY;
    
    let tNow = performance.now(); // Lõi Micro-second
    
    let bgPulse = (tNow / 2000) % (Math.PI * 2);
    let gradBg = ctxV.createLinearGradient(0, retainY, 0, 1920);
    ...
    if (audioPeak > 0.35 && Math.random() < 0.25 && !window.gameOver && window.retentionEmojis.length < 15) { 
        ...
        window.retentionEmojis.push({
            x: 100 + Math.random() * 880, y: 1920 + 50,
            v: 10 + Math.random() * 8,
