// ==========================================
// RECORDER.JS - V69.1 PERFORMANCE OPTIMIZED (ZERO-LAG EDITION)
// Tối ưu Layout: Clean Game -> HUD -> Poll -> Smart Alerts -> Full Height ChatBox
// ĐÃ TỐI ƯU: Tách Render Loop, Pre-calculate Chat Text, Gradient Caching, Giảm GC Thrashing.
...
window.generateLiveChatEvent = function() {
    ...
    const toxicityTemplates = [
        ...
        () => `${r(targets)} makes the tutorial look hard 😭`, () => `A roomba could get a better combo 🤡`,
        () => `What color is your ${r(gear)}? Broke ${r(emojis)}`, () => `Someone take his ${r(gear)} away immediately ${r(emojis)}`,
        () => `${r(savageShorts).toUpperCase()} WHAT WAS THAT ${r(emojis)}`, () => `Bro's ${r(gear)} is crying right now 😭`,
        () => `Reported for being this bad 💀`, () => `${randomButtons()} 🤡`, () => `Bro's aura is -9999 📉`, () => `Even TikTok is lagging from this gameplay 🗑️`
    ];
...
// [TỐI ƯU 1] HÀM TIỀN XỬ LÝ TEXT: Tính toán wrap text ngay khi chat được tạo, không tính trong vòng lặp
...
window.StoryModeAI = {
    ...
    init: function(hero, enemy) { let n = this.generateScript(hero, enemy); this.viralTitle = n.title; },
...
window.bakeThumbnailsForVideo = function(titleText) {
    ...
    try {
        ...
        const drawClickbaitProps = (ctx, w, h, isVertical) => {
            ctx.save(); let rx = isVertical ? w/2 : w/2 + (Math.random() > 0.5 ? 200 : -200); let ry = isVertical ? h*0.4 + Math.random()*150 : h/2 + Math.random()*80;
        ...
        const draw3DTitle = (ctx, w, h, text, isVertical) => {
            ...
            ctx.fillStyle = grad; ctx.fillRect(0,0,w,h); ctx.translate(w/2, isVertical ? h*0.15 : h*0.15); ctx.rotate(-0.06); ctx.textAlign = "center"; ctx.textBaseline = "middle";
            let fontSize = isVertical ? 85 : 100; ctx.font = `italic 900 ${fontSize}px 'Arial Black', Impact`; let lines = [text];
...
window.initRecorder = function() {
    ...
    window.recordCanvasH = document.getElementById("hiddenRecordCanvasH") || document.createElement("canvas");
    if (!window.recordCanvasH.id) { window.recordCanvasH.id = "hiddenRecordCanvasH"; document.body.appendChild(window.recordCanvasH); }
...
// [TỐI ƯU 2] HÀM LOOP ĐỘC LẬP (Tách khỏi luồng Game)
...
// [TỐI ƯU 3] Xóa Hook cũ gây lag game
if (window._hookedDrawForRecorder && window.draw && window._originalDrawBeforeHook) {
    window.draw = window._originalDrawBeforeHook; // Khôi phục hàm draw gốc nếu có
}
// Nếu không thể khôi phục, chèn đè Hook an toàn (Bỏ setTimeout)
if (!window._hookedDrawForRecorder) {
    window._hookedDrawForRecorder = true;
    window._originalDrawBeforeHook = window.draw; // Lưu lại
...
window.startRecording = function() {
    ...
    window.videoTrackH = videoStreamH.getVideoTracks()[0]; window.videoTrackV = videoStreamV.getVideoTracks()[0];
...
window.stopRecording = function() { 
    ...
    window.isRecording = false; 
    cancelAnimationFrame(window._recordLoopId); // Dừng vòng lặp Recorder
    ...
    setTimeout(() => { if (window.videoTrackH) window.videoTrackH.stop(); if (window.videoTrackV) window.videoTrackV.stop(); }, 500);
...
window.captureFrames = function() {
    ...
    let now = Date.now();
    if (now - window._lastCaptureTime < 33) return; // Throttling ở 30FPS để cứu CPU
    ...
    if (!isOutroActive) {
        ...
        // RENDER CHAT SAU KHI ĐÃ ĐƯỢC TỐI ƯU HÓA TÍNH TOÁN
        ...
        let drawOutroCTA = (ctx, w, h, isMobile) => {
            ...
            if (isMobile) {
                ...
                for(let i=-1; i<=1; i++) {
                    let rx = cx + i*340 - 150; ctx.fillStyle = "#111827"; ctx.strokeStyle = (i===0) ? "#00f3ff" : "#334155";
                ...
                if(!window._cachedGradients.btnCTA) { window._cachedGradients.btnCTA = ctx.createLinearGradient(-btnWidth/2, 0, btnWidth/2, 0); window._cachedGradients.btnCTA.addColorStop(0, "#ff0050"); window._cachedGradients.btnCTA.addColorStop(1, "#00f2fe"); }
                ctx.fillStyle = window._cachedGradients.btnCTA; ctx.beginPath(); if(ctx.roundRect) ctx.roundRect(-btnWidth/2, -btnHeight/2, btnWidth, btnHeight, btnHeight/2); else ctx.rect(-btnWidth/2, -btnHeight/2, btnWidth, btnHeight); ctx.fill();
                ...
                let btnPulse = 1 + (audioPeak * 0.08); ctx.translate(cx, btnY); ctx.scale(btnPulse, btnPulse);
                if(!window._cachedGradients.btnCTA2) { window._cachedGradients.btnCTA2 = ctx.createLinearGradient(-btnWidth/2, 0, btnWidth/2, 0); window._cachedGradients.btnCTA2.addColorStop(0, "#00f2fe"); window._cachedGradients.btnCTA2.addColorStop(1, "#4facfe"); }
                ctx.fillStyle = window._cachedGradients.btnCTA2; ctx.beginPath(); if(ctx.roundRect) ctx.roundRect(-btnWidth/2, -btnHeight/2, btnWidth, btnHeight, btnHeight/2); else ctx.rect(-btnWidth/2, -btnHeight/2, btnWidth, btnHeight); ctx.fill();
                ctx.lineWidth = 5; ctx.strokeStyle = "rgba(255, 255, 255, 0.5)"; ctx.stroke();
