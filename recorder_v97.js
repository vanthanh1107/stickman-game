...
// Tối ưu Layout: Clean Game -> Smart Centered Alerts (Fixed Y) -> Multi-line Live Chat (Clipped) -> Green Screen Cat 
// ==========================================
...
// Các hàm phụ trợ (Meme BG, Draw Characters, Bake Thumbnail, Intro, v.v...)
...
window.captureFrames = function() {
    ...
    if (!isOutroActive) {
        ...
        // --- [SỬA LỖI] THÔNG BÁO LIVE ALERTS (HIỂN THỊ CHÍNH GIỮA) ---
        ...
        // Dời Thông Báo xuống khoảng Y = 890 (Nằm an toàn giữa HUD và Chat Box)
        let alertStartY = splitGameHeight + 280; 
        ...
        // --- KHUNG LIVE CHAT (HỖ TRỢ WORD WRAP & CẮT CHỮ KHỎI HEADER) ---
        ctxV.save();
        let boxWidth = 960;
        let boxHeight = 440;
        let boxX = 540; 
        ...
        ctxV.beginPath(); 
        if(ctxV.roundRect) ctxV.roundRect(-boxWidth/2, -boxHeight/2, boxWidth, boxHeight, 25); 
        ctxV.clip(); // Clip chính cho toàn bộ bảng
        ...
        // --- MẶT NẠ CẮT CHỮ: KHÔNG CHO CHỮ TRÔI ĐÈ LÊN HEADER ---
        ctxV.save();
        ctxV.beginPath();
        // Cắt bỏ phần header đỏ (70px trên cùng)
        ...
        let headerBottomY = -boxHeight/2 + 70; // Mốc giới hạn bên trên
        ...
        for (let i = window._liveChats.length - 1; i >= 0; i--) { 
            ...
            for(let l = lines.length - 1; l >= 0; l--) {
                // Nếu vượt quá giới hạn trên cùng thì ngừng vẽ đoạn còn lại
        ...
        // Hủy bỏ Mặt Nạ Cắt Chữ trước khi vẽ viền Glow
        ctxV.restore();
