// ==========================================
// RECORDER.JS - BẢN HOÀN HẢO CHO CONTENT CREATOR (V57.0 - ULTRA VIRAL STICKMAN MEME)
// [FIXED] Sửa dứt điểm lỗi "Fail - No Network" (Lỗi file rỗng khi tải video do luồng xử lý quá nhanh).
// [FIXED] Xóa lỗi Thumbnail bị trắng tinh. Snap-cut mượt mà vào gameplay sau 1.5s.
// [NEW] CỖ MÁY CHẾ MEME: Text 3D đùn khối, Nền Sunburst, Vòng đỏ, Mũi tên Clickbait.
// [UPGRADED] Tiêu đề AI chuyên biệt cho ngách Stickman Game thu hút lượt xem.
// ==========================================
...
// ==========================================
// 🧠 HỆ THỐNG STORYTELLING AI (CHUYÊN NGÁCH STICKMAN)
// ==========================================
window.StoryModeAI = {
...
window.bakeThumbnailsForVideo = function(titleText) {
    ...
    try {
        ...
        // 2. VẼ NHÂN VẬT VỚI HÀO QUANG (AURA) BỰ CHÀ BÁ
        const drawCharSafe = (ctx, charObj, cx, cy, scale, isFacingRight) => {
            ...
            ctx.shadowColor = `hsl(${isFacingRight ? hue1 : hue2}, 100%, 70%)`; 
            ctx.shadowBlur = 60;
        ...
        // Ngang: P1 trái, P2 phải
        drawCharSafe(ctxH, window.p1, 450, 850, 4.5, true);
        drawCharSafe(ctxH, e1, 1470, 850, 4.5, false);
        ...
        const drawClickbaitProps = (ctx, w, h, isVertical) => {
            ctx.save();
            // Vòng tròn đỏ và Mũi tên chỉ bừa
            ...
            // Chữ sát thương "-999,999"
            ctx.translate(isVertical ? w*0.7 : w*0.75, isVertical ? h*0.6 : h*0.35); 
        ...
        // 4. TIÊU ĐỀ 3D (EXTRUDED TEXT) YOUTUBER TRIỆU VIEW
        const draw3DTitle = (ctx, w, h, text, isVertical) => {
            ctx.save();
            // Phủ lớp Vignette tối góc để nổi chữ
        ...
        console.error("Lỗi khi Bake Thumbnail:", e);
    }
};
...
window.startRecording = function() {
    ...
    // 🔥 BAKE THUMBNAIL VÀ GIỮ FRAME 1.5 GIÂY (90 FRAMES)
    ...
    const finalizeRecordings = () => {
        stoppedCount++;
        if (stoppedCount === 2) { 
            // CHỜ 800ms ĐỂ TRÌNH DUYỆT GOM ĐỦ 100% CÁC MẢNH VIDEO CUỐI CÙNG (KHÔNG BAO GIỜ BỊ FILE RỖNG NỮA)
            setTimeout(() => {
                if (window.recordedChunksH.length === 0 || window.recordedChunksV.length === 0) {
                    console.log("Cảnh báo: Dữ liệu rỗng."); return;
                }
...
window.captureFrames = function() {
    ...
    // 🔥 [FIX] ÉP THUMBNAIL TĨNH VÀO 1.5s ĐẦU (NO FADE OUT)
    if (window.thumbnailHoldFrames > 0) {
        ...
        return; // Dừng lại ở đây, chuyển thẳng sang Gameplay khi hết đếm ngược (Snap-Cut cực mượt)
    }
