// ==========================================
// RECORDER.JS - BẢN HOÀN HẢO CHO CONTENT CREATOR (V62.0 - ULTRA PERFORMANCE & ZERO LAG)
// [FIXED] Loại bỏ lệnh blur() gây quá tải CPU; Thay bằng thuật toán Glow-Ghosting siêu nhẹ -> Tăng FPS lên tối đa.
// [FIXED] Bỏ cắt lát start(1000) gây khựng video; Ghi hình nguyên khối mượt mà chuẩn 60FPS.
...
// ==========================================
// 💥 HỆ THỐNG VẼ ANIMATED INTRO (MANGA TRASH TALK)
// ==========================================
window.drawAnimatedIntro = function(ctx, w, h, isVertical, progress) {
    ...
    // Tối ưu hóa vòng lặp vẽ Speedlines (Giảm số lượng để chống lag)
    for (let i = -w*1.5; i < w*1.5; i+=80) {
        ctx.fillStyle = (i%160===0) ? "rgba(255,255,255,0.15)" : "rgba(255,50,0,0.15)";
        ctx.fillRect(i - speedOffset, -h*1.5, 15 + Math.random()*15, h*3);
    }
    ctx.restore();
...
window.startRecording = function() {
    ...
    // [TỐI ƯU HÓA BITRATE] Mức 12Mbps là hoàn hảo cho 1080p60 WebM, vừa nét vừa mượt
    ...
    // [KHÔNG GỌI start(1000) ĐỂ TRÁNH MẤT FRAME TRÊN PHẦN MỀM EDIT, GHI LÀM 1 KHỐI DUY NHẤT]
...
window.stopRecording = function() { 
    ...
    // GỌI requestData TRƯỚC KHI STOP ĐỂ ĐẢM BẢO KHÔNG BỊ RỖNG FILE KHI CHẠY NGUYÊN KHỐI
...
window.captureFrames = function() {
    ...
    // [TỐI ƯU HÓA GPU] - Thay vì fill màu trong suốt cực nặng, ta chỉ cần clearRect 1px góc là đủ ép trình duyệt render khung hình.
    ctxH.clearRect(0,0,1,1);
    ctxV.clearRect(0,0,1,1);
    ...
    // FAST GLOW (TỐI ƯU SIÊU NHẸ CHỐNG LAG)
    if (audioPeak > 0.1) {
