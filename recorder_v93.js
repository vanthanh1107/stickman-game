...
// Tối ưu Layout: Game -> Banner -> HUD -> Live Chat -> Green Screen Cat + ULTIMATE RETENTION HOOKS
// ==========================================
...
// Giữ nguyên các hàm phụ trợ Bake Thumbnail, Intro, v.v...
...
window.captureFrames = function() {
    ...
    // --- [HOOK 2] SÁT THƯƠNG ẢO NỔ TUNG MÀN HÌNH ---
    if (!window._damagePopups) window._damagePopups = [];
    if (!isOutroActive && audioPeak > 0.75 && Math.random() < 0.4) {
        window._damagePopups.push({ 
            x: 200 + Math.random() * 600, 
            y: 200 + Math.random() * 300, 
            life: 1.0, 
            val: "-9" + Math.floor(10 + Math.random() * 89) + ",999 💢" 
        });
    }

    // Vẽ sát thương ảo đè lên game
    for (let i = window._damagePopups.length - 1; i >= 0; i--) {
        let dmg = window._damagePopups[i];
        dmg.y -= 3; // Bay lên
        dmg.life -= 0.03; // Mờ dần
        
        ctxV.save();
        ctxV.globalAlpha = Math.max(0, dmg.life);
        ctxV.translate(dmg.x, dmg.y);
        let s = 1 + (1 - dmg.life) * 0.5; // To dần
        ctxV.scale(s, s);
        ctxV.rotate((Math.random() - 0.5) * 0.2); // Rung nhẹ
        
        ctxV.font = "italic 900 65px Impact";
        ctxV.lineWidth = 10;
        ctxV.strokeStyle = "#000";
        ctxV.strokeText(dmg.val, 0, 0);
        ctxV.fillStyle = "#ff003c";
        ctxV.fillText(dmg.val, 0, 0);
        ctxV.fillStyle = "#fff";
        ctxV.fillText(dmg.val, -3, -3);
        ctxV.restore();

        if (dmg.life <= 0) window._damagePopups.splice(i, 1);
    }
    ...
    if (!isOutroActive) {
        ...
        if (window.p1) {
            ...
            if (repEnemyObj) {
                ...
                // --- [HOOK 3] THANH TIẾN TRÌNH ÁP LỰC ---
                let totalHpLeft = Math.max(0, 1 - p2Hp); // Tiến trình tính bằng máu địch đã mất
                ctxV.save();
                ctxV.fillStyle = "rgba(0,0,0,0.6)"; ctxV.fillRect(0, 1900, 1080, 20); // Dưới cùng màn hình
                ctxV.fillStyle = "#00f3ff"; ctxV.fillRect(0, 1900, 1080 * totalHpLeft, 20);
                
                ctxV.textAlign = "left"; ctxV.font = "bold 20px Arial";
                ctxV.shadowColor = "#000"; ctxV.shadowBlur = 4;
                ctxV.fillStyle = "#fff"; ctxV.fillText("BOSS DEFEAT PROGRESS: " + Math.floor(totalHpLeft*100) + "%", 15, 1890);
                ctxV.restore();
            }
        }
        ...
        if (chatNow - window._lastChatUpdate > window._nextChatDelay) {
            ...
            // Mắt xem tăng giảm mỗi khi có comment
