...
// Kèm theo TOXIC LIVE CHAT (Never disappears)
// ==========================================
...
window.captureFrames = function() {
    ...
    if (window.gameOver && window.matchEndTimer > 90) { 
        ...
        let drawOutroCTA = (ctx, w, h, isMobile) => {
            ...
            if (isMobile) {
                ...
                let btnWidth = 640; let btnHeight = 160; let btnY = cy + 150 + floatY; // Đã nhích lên trên
    ...
    // ==========================================
    // 7. BIG TOXIC CHAT BOX (LUÔN LUÔN NỔI LÊN TRÊN CÙNG ĐẾN HẾT VIDEO)
    // ==========================================
    ...
    // KHÔNG dùng điều kiện !window.gameOver nữa để chat nổi lên trên cùng cho tới khi dừng record
    if (window.introTimer > 120) {
        let nowTime = Date.now();
        let isHype = audioPeak > 0.6; // Đang combat căng

        // Cứ 1.5 - 3.5 giây nhả 1 comment, riêng lúc combat thì cứ 0.15s nhả 1 cái
    ...
    if (!window.gameOver) {
        ...
        if (window.StoryModeAI.displayedText.length > 0) {
            ...
            ctxV.save(); ctxV.translate(540, 800); // Kéo Subtitle lên phía trên đỉnh đầu VTuber
            ctxV.scale(textPulse, textPulse); ctxV.textAlign = "center"; ctxV.textBaseline = "top";
