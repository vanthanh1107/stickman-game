// ==========================================
// RECORDER.JS - BẢN HOÀN HẢO CHO CONTENT CREATOR (V66.13 - THE STABLE CORE)
// [FIX LỖI ĐƠ HÌNH] Chuyển đổi mã hóa sang định dạng WebM (VP8/VP9) - Ổn định 100%.
// [FIX GIẬT LAG] Hạ Bitrate về chuẩn 8 Mbps (Chuẩn YouTube 1080p60) để chống tràn bộ nhớ.
// [FIX GPU] Gỡ bỏ ctx.filter (Bộ lọc màu ngốn phần cứng) giúp FPS khóa cứng ở 60.
// [RETAINED] Bố cục TikTok, Meta Caption, Profile Magnet, 3D Grid.
// ==========================================
...
window.currentVideoExt = "webm"; // CHUYỂN SANG WEBM ĐỂ CHỐNG LỖI ĐƠ HÌNH CỦA MP4
window.savedVideos = [];
window.filmDustY = 0; 
...
window.initRecorder = function() {
    ...
    window.recordCtxH = window.recordCanvasH.getContext("2d", {alpha: false});
    ...
    window.recordCtxV = window.recordCanvasV.getContext("2d", {alpha: false});
};
...
window.startRecording = function() {
    ...
    // [FIX CORE] CHUYỂN SANG WEBM + HẠ BITRATE VỀ 8 Mbps ĐỂ CHỐNG LỖI ĐƠ HÌNH CỦA TRÌNH DUYỆT
    let options = { videoBitsPerSecond: 8000000 }; 
    window.currentVideoExt = "webm";
    
    // Ép ưu tiên sử dụng VP9/VP8 để phát lại mượt mà nhất trên Windows
    if (MediaRecorder.isTypeSupported('video/webm; codecs="vp9"')) {
        options.mimeType = 'video/webm; codecs="vp9"';
    } else if (MediaRecorder.isTypeSupported('video/webm; codecs="vp8"')) {
        options.mimeType = 'video/webm; codecs="vp8"';
    } else if (MediaRecorder.isTypeSupported('video/mp4; codecs="avc1"')) {
        options.mimeType = 'video/mp4; codecs="avc1"'; // Fallback cuối cùng
...
window.captureFrames = function() {
    ...
    try {
        ...
        // --- RENDER BẢN NGANG 1080P (Tắt filter chống ngốn GPU) ---
        if (renderNormalH) {
        ...
        // --- RENDER BẢN DỌC 1080P (Tắt filter) ---
