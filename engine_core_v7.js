...
// [BẢN CẬP NHẬT EPIC HAZARDS - THIÊN THẠCH & SÉT ÁNH SÁNG XANH]
// ==========================================
...
window.update = function() {
    ...
    if (!window.gameOver && !isTimeStopped) {
        ...
        // --- CẬP NHẬT: CHO PHÉP THIÊN THẠCH VÀ SÉT XUẤT HIỆN Ở MỌI MAP ---
        let hazardChance = 0.002 + (window.matchTimer / 3600) * 0.01; 
        
        // 1. Thiên thạch ngẫu nhiên
        ...
        for (let i = window.projectiles.length - 1; i >= 0; i--) {
            ...
            if (proj.isMeteor) {
                ...
                if (proj.y >= window.GROUND_Y) {
                    ...
                    // --- THIÊN THẠCH: Bay hạt năng lượng Đỏ/Vàng lên thay vì tạo vết nứt ---
        ...
        for (let i = window.envHazards.length - 1; i >= 0; i--) {
            ...
            if (haz.type === 'lightning') {
                if (haz.state === 'warning' && haz.timer <= 0) { 
                    ...
                    // --- SÉT ĐÁNH: Sinh hiệu ứng sấm chớp giáng xuống và văng hạt Xanh ---
                    window.spawnEnergyPillar(haz.x, window.GROUND_Y, "rgba(0, 243, 255, 0.4)", 50, 12); // Trụ ánh sáng xanh
                    for(let l=0; l<3; l++) window.spawnLightning(haz.x, window.GROUND_Y, "#00f3ff", 80, 5); // Tia sét chính
                    ...
                    if (window.particles.length < MAX_PARTICLES) { 
                        for(let p=0; p<20; p++) {
                            window.particles.push({ 
                                ...
                                y: window.GROUND_Y, 
                                vx: (Math.random()-0.5)*8, 
                                vy: -Math.random()*8 - 6, // Bay lên
                                life: 40 + Math.random()*20, 
                                maxLife: 60, 
                                color: Math.random() > 0.4 ? "#00f3ff" : "#ffffff", 
                                size: Math.random()*5+4, 
                                isAuraFlame: true // Miễn trọng lực, hạt lơ lửng bay lên
                            }); 
                        }
                    }
