// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 38.4 [TRUE COMBAT & COMBO EDITION]
// [RESTORED] Giữ nguyên 100% UI Emoji, Text Manga, Thumbnail, Anti-Glare và Không gian Râu Trắng của bản gốc.
// [NEW COMBAT] Thêm hệ thống Hất tung (Juggle), Đập đất nảy (Ground Bounce), Đòn liên hoàn (Multi-hit Flurry), và Chuỗi Combo chuẩn Game Đối Kháng.
// ==========================================
...
window.initGameEngine = function() {
    ...
    // TẠO DỮ LIỆU NGẪU NHIÊN CHO MÀN HÌNH THUMBNAIL (MEME SPLASH SCREEN)
...
// ==========================================
// 4. HỆ THỐNG VẬT LÝ NHẬN SÁT THƯƠNG & ATTACK (CẬP NHẬT COMBO/JUGGLE)
// ==========================================
window.takeDamage = function(target, amount, color, isCrit, wallBounce, attackerDirRight = true) {
    ...
    if (target.state === 'block') { 
        if (target.stamina < 15) {
            ...
            window.floatingTexts.push({ x: target.x, y: target.y - 90, text: "💔 GUARD BREAK!", color: "#ff003c", alpha: 1.5, vx: 0, vy: -4, font: "italic 900 45px 'Arial Black', Impact", life: 60, scale: 4.0, targetScale: 1.2, scaleVel: 0, rot: (Math.random()-0.5)*0.4 });
    ...
    if (finalDmg > 0) {
        ...
        if (isCrit) {
            window.screenFlash = 0.15; // [ANTI-GLARE] Giảm độ nháy trắng
            window.impactFrameCount = 1; 
            window.targetZoom = 1.25; setTimeout(() => { window.targetZoom = 1.1; }, 100); 
            window.impactAberration = 8; // [ANTI-GLARE] Giảm nhòe hình
        ...
        if (target.superArmor <= 0 && target.state !== 'stunned') { target.state = 'hurt'; target.hitStun = isCrit ? 25 : 15; target.attackTimer = 0; target.comboStep = 0; }
        ...
        if (target.hp <= 0) {
            ...
            if (isNearWall && isCrit && typeof window.triggerStageTransition === 'function') { window.triggerStageTransition(target); } 
                ...
                if (isCrit) {
                    window.screenFlash = 0.4; // [ANTI-GLARE] Giảm lóa cho Finish
                    ...
                    window.lethalVoid = 100; window.impactAberration = 30; // Giảm aberration
...
window.attack = function(attacker, targetGroup) {
    ...
    let moves_Launcher = ['uppercut', 'dragon_uppercut', 'high_kick']; // Hất tung
    let moves_Spike = ['axe_kick', 'palm_strike']; // Cắm đất
    ...
    if (attacker.comboStep >= 3 || Math.random() < finisherChance) { 
        ...
        if (target.onGround === false && target.y < window.GROUND_Y - 40) {
            // Khi kẻ địch trên không trung, ưu tiên xài đòn cắm đất (Spike) hoặc Hất tiếp
    ...
    if (moves_Launcher.includes(selectedMove)) { 
        ...
        if (!target.onGround) {
            target.vy = 18; // Cắm đất tốc độ cao
            target.hitStun = 45;
            finalDmg *= 1.5;
    ...
    // [NEW COMBAT] ĐÒN FINISHER MULTI-HIT (Đánh Liên Hoàn)
    if (isFinisher) {
        if (selectedMove === 'dempsey_roll' || selectedMove === 'asura_strike') {
            ...
            let hits = 0; let maxHits = selectedMove === 'asura_strike' ? 5 : 3;
            let flurry = setInterval(() => {
                if (hits >= maxHits || target.hp <= 0 || window.gameOver) { 
                    clearInterval(flurry); 
                    // Cú chốt cuối cùng
                    if (target.hp > 0) {
                        window.shakeScreen(20, 15); target.vx = (attacker.isFacingRight ? 8 : -8); target.vy = -5;
                        window.takeDamage(target, baseDmg * 2.5, "#ff4757", true, false, attacker.isFacingRight);
                        window.spawnSlash(target.x, target.y - 35, attacker.isFacingRight, "#ff4757", true, 3.5, 0);
                ...
                // Các hit nhỏ đệm vào
                window.shakeScreen(5, 5); target.vx = (attacker.isFacingRight ? 1 : -1); target.hitStun = 20;
                window.takeDamage(target, baseDmg * 0.6, "#f1c40f", false, false, attacker.isFacingRight);
                window.spawnSlash(target.x + (Math.random()-0.5)*20, target.y - 30 + (Math.random()-0.5)*20, attacker.isFacingRight, "#f1c40f", false, 1.5, (Math.random()-0.5)*Math.PI);
                hits++;
                attacker.comboHits = (attacker.comboHits || 0) + 1;
            }, 120);
            return; // Dừng lại vì flurry tự xử lý takeDamage
        } else {
        ...
        if (moves_Heavy.includes(selectedMove) && Math.random() < 0.3) { finalDmg *= 1.5; window.spawnParticles(target.x, target.y - 60, "#c0392b", true); window.floatingTexts.push({ x: target.x, y: target.y - 50, text: "🩸", color: "#c0392b", alpha: 1.5, vx: 0, vy: -3, font: "900 35px Arial", life: 40, scale: 1.8, targetScale: 1.0, scaleVel: 0, rot: 0 }); }
...
window.update = function() {
    ...
    allFighters.forEach(f => {
        ...
        // [NEW COMBAT] HỆ THỐNG VẬT LÝ BOUNCE KHI CHẠM ĐẤT TỐC ĐỘ CAO
        if (f.hp <= 0 || f.state === 'hurt' || f.state === 'ko_falling') { 
            ...
            if (f.hp <= 0) return; // Nếu đã chết thì ngừng process AI
        } else {
            f.vy += window.GRAVITY; 
            ...
            if (f.isDragon) {
                ...
                if (closest && closest.hp > 0) {
                    ...
                    if (absDist > reach) { f.vx = Math.sign(dist) * f.currentSpeed; f.state = 'walk'; if (Math.random() < 0.1 && f.onGround) window.spawnDust(f.x, f.y); } 
                        ...
                        if (f.aiDelay <= 0) {
                            // [NEW COMBAT] AI Delay ngắn lại một chút để chuỗi combo mượt hơn
