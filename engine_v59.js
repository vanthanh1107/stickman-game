// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 47.0 [THE PERFORMANCE BRAWLER]
// [OPTIMIZED] FPS BOOST: Tắt bộ lọc shadowBlur dư thừa trên hạt vật lý nhỏ, dùng Bitwise Math rút ngắn thời gian tính toán. FPS mượt mà!
// [NEW COMBAT] WALL BOUNCE: Đấm bay địch vào 2 mép màn hình sẽ đập vỡ tường vô hình và nảy ngược lại để Combo tiếp!
// [NEW COMBAT] AURA AWAKENING: Khi máu dưới 20%, tự động bùng nổ cột Haki hất văng mọi kẻ thù.
// [RESTORED] KHÔNG LỖI ANIMATION: Tay chân vung đấm đá mượt 100%, Zoom rộng bao quát toàn thân.
// [BASE] Matrix Orbit 360, Tàng hình Phantom Dash, Teleport Clash, Dutch Angle K.O, Meme Splash Screen.
// ==========================================
...
window.eyeFlashes = []; window.screenInks = []; 
window.speedLineAngles = Array.from({length: 45}, () => Math.random() * Math.PI * 2);
...
window.camOrbitAngle = 0; window.targetCamOrbitAngle = 0; window.isSpinningCam = false; 
window.orbitFocusX = 0; window.orbitFocusY = window.GROUND_Y - 60; 
...
const MAX_PARTICLES = 250; const MAX_SHOCKWAVES = 10; // Giảm giới hạn rác để tối ưu FPS
...
window.spawnEyeFlash = function(x, y, color, isRight) { window.eyeFlashes.push({ x: x, y: y, color: color, life: 20, maxLife: 20, isRight: isRight }); window.playSound(900, 'sawtooth', 0.2, 0.8, false); }
window.spawnScreenInk = function(color) { 
    for(let i=0; i<3; i++) { window.screenInks.push({ x: Math.random() * (window.canvas ? window.canvas.width : 800), y: Math.random() * (window.canvas ? window.canvas.height : 600), color: color, life: 120, maxLife: 120, scale: Math.random() * 2 + 1, slideY: 0 }); }
}
...
// HIỆU ỨNG NỨT KHÔNG GIAN 3D CỦA RÂU TRẮNG
window.triggerDimensionShatter = function(x, y) {
    window.dimensionCracks = [];
    let cx = x !== undefined ? x : (window.canvas ? window.canvas.width/2 : 600);
    let cy = y !== undefined ? y : (window.canvas ? window.canvas.height/2 - 100 : 300);
    
    let numMainCracks = 5 + (Math.random() * 4 | 0); 
    for(let i = 0; i < numMainCracks; i++) {
        let baseAng = (Math.PI * 2 / numMainCracks) * i + (Math.random() - 0.5) * 0.6;
        let len = 350 + Math.random() * 300; let branches = [];
        let points = [{x: cx, y: cy}]; let currX = cx; let currY = cy; let dist = 0; let currentAng = baseAng;
        
        while(dist < len) {
            let step = 30 + Math.random() * 40; dist += step; currentAng += (Math.random() - 0.5) * 0.9; 
            currX += Math.cos(currentAng) * step; currY += Math.sin(currentAng) * step; 
            points.push({x: currX, y: currY});
            
            if (Math.random() < 0.45) {
                let subPoints = [{x: currX, y: currY}];
                let subAng = currentAng + (Math.random() > 0.5 ? 1 : -1) * (0.6 + Math.random()*0.8);
                let subLen = step * (1.5 + Math.random() * 2); let sDist = 0; let sX = currX; let sY = currY;
                while(sDist < subLen) {
                    let sStep = 20 + Math.random()*20; sDist += sStep; subAng += (Math.random() - 0.5) * 0.6;
                    sX += Math.cos(subAng) * sStep; sY += Math.sin(subAng) * sStep;
                    subPoints.push({x: sX, y: sY});
                }
                branches.push(subPoints);
            }
        }
        window.dimensionCracks.push({main: points, branches: branches, life: 60, maxLife: 60});
    }
}

window.triggerStageTransitionEffect = function() {
    window.triggerDimensionShatter(); window.screenFlash = 2.0; window.playSound(600, 'sawtooth', 1.0, 1.0, true);
    let weathers = ['none', 'rain', 'snow', 'ash', 'toxic', 'shooting_stars', 'cosmic_dust', 'matrix_rain'];
    window.currentWeather = weathers[Math.floor(Math.random() * weathers.length)];
    window.viralColorGrade.hue = Math.floor(Math.random() * 360); window.viralColorGrade.contrast = 130 + Math.random() * 30;
    
    const lightingThemes = [
        { mix: 'overlay', color1: 'rgba(255, 120, 30, 0.3)', color2: 'rgba(10, 0, 20, 0.7)' },
        { mix: 'hard-light', color1: 'rgba(0, 180, 255, 0.2)', color2: 'rgba(0, 5, 30, 0.8)' },
        { mix: 'color-burn', color1: 'rgba(255, 30, 30, 0.2)', color2: 'rgba(0, 0, 0, 0.8)' },
        { mix: 'difference', color1: 'rgba(255, 255, 255, 0.1)', color2: 'rgba(0, 0, 0, 0.9)' }
    ];
    window.globalIllumination = lightingThemes[Math.floor(Math.random() * lightingThemes.length)];
}
...
window.spawnEnvDamage = function(x, y, type, scale, isBurning = false) {
    let cracks = []; let numCracks = (type === 'crater') ? 6 + (Math.random()*4|0) : 4 + (Math.random()*3|0); let maxRadius = 0;
    for(let i=0; i<numCracks; i++) {
        ...
        let path = [{x: 0, y: 0}]; let segments = 3 + (Math.random()*2|0); let currX = 0, currY = 0; let currAngle = angle;
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce, attackerDirRight = true) {
    ...
    if (target.state === 'dash' || target.state === 'dash_back') {
        if (target.dashTimer > 2 && target.dashTimer < 14) {
            ...
            window.targetTimeScale = 0.05; setTimeout(()=>{ window.targetTimeScale = 1.0; }, 1500);
            window.applyFilter('invert', 8); window.speedLinesAlpha = 1.0; 
            window.globalIllumination = { mix: 'hard-light', color1: 'rgba(155, 89, 182, 0.4)', color2: 'rgba(0, 0, 0, 0.8)' };
            
            window.floatingTexts.push({ x: target.x, y: target.y - 120, text: "🕒 PERFECT DODGE!", color: "#9b59b6", alpha: 1.5, vx: 0, vy: -2, font: "italic 900 50px 'Arial Black', Impact", life: 80, scale: 3.0, targetScale: 1.0, scaleVel: 0, rot: 0 });
            window.spawnParticles(target.x, target.y - 40, "#9b59b6", true);
            if (window.shockwaves.length < MAX_SHOCKWAVES) window.shockwaves.push({x: target.x, y: target.y - 40, r: 5, maxR: 600, color: "#9b59b6", alpha: 1, speed: 35});
            
            target.stamina = 100; target.x += target.isFacingRight ? -150 : 150; 
            target.iFrames = 60; target.state = 'idle'; 
            window.targetCamOrbitAngle = target.isFacingRight ? 0.8 : -0.8; setTimeout(() => { window.targetCamOrbitAngle = 0; }, 1500);
            return; 
        }
    }
    ...
    if (target.state === 'block') { 
        if (target.stamina < 15) {
            ...
            window.targetZoom = 1.2; window.dynamicBlur = 10; window.hitStopFrames = 20; window.impactAberration = 15; window.chromaTimer = 20; window.vhsGlitchTimer = 20; window.spawnForegroundDebris(target.x, target.y, attackerDirRight); 
        } 
            ...
            target.stamina = Math.min(100, target.stamina + 30); window.shakeScreen(12, 8); window.hitStopFrames = 8; window.chromaTimer = 10; window.impactAberration = 10;
    ...
    if (finalDmg > 0) {
        ...
        if (target.y < window.GROUND_Y - 20) {
            target.state = 'juggle'; target.vy = -6; 
            window.floatingTexts.push({ x: target.x, y: target.y - 90, text: "AERIAL!", color: "#f1c40f", alpha: 1.5, vx: 0, vy: -2, font: "italic 900 25px Arial", life: 30, scale: 1.5, targetScale: 1.0, scaleVel: 0, rot: 0 });
        }

        if (isCrit) {
            window.screenFlash = 0.15; window.impactFrameCount = 1; 
            window.targetZoom = 1.25; setTimeout(() => { window.targetZoom = 1.0; }, 150); 
            window.impactAberration = 8; window.spawnForegroundDebris(target.x, target.y, attackerDirRight);
            
            window.targetCamOrbitAngle = attackerDirRight ? -0.3 : 0.3; window.orbitFocusX = target.x; 
            window.targetTilt = attackerDirRight ? 0.08 : -0.08;
            setTimeout(() => { window.targetCamOrbitAngle = 0; window.targetTilt = 0; }, 500);
            ...
            if(Math.random() < 0.3) window.spawnScreenInk(color || "#ff003c");
        ...
        } else if (Math.random() > 0.5) { window.spawnMangaSFX(target.x, target.y - 50, false); window.targetZoom = 1.05; setTimeout(() => { window.targetZoom = 1.0; }, 50); window.impactAberration = 4; }

        if (target.superArmor <= 0 && target.state !== 'stunned' && target.state !== 'juggle') { target.state = 'hurt'; target.hitStun = isCrit ? 22 : 12; target.attackTimer = 0; target.comboStep = 0; }
        
        // [WALL BOUNCE LOGIC IN DAMAGE]
        if (wallBounce) { 
            target.vx = target.isFacingRight ? -12 : 12; 
            target.state = 'hurt'; 
            window.shakeScreen(10, 10);
            window.playSound(400, 'square', 0.2, 0.5, true);
            window.spawnGlassShatter(target.x, target.y);
            window.floatingTexts.push({ x: target.x, y: target.y - 50, text: "💥 WALL BOUNCE!", color: "#f1c40f", alpha: 1.5, vx: target.vx * 0.5, vy: -3, font: "italic 900 30px 'Arial Black'", life: 40, scale: 2.0, targetScale: 1.0, scaleVel: 0, rot: 0 });
        } 
        ...
        if (target.hp <= 0) {
            ...
            if (isNearWall && isCrit && typeof window.triggerStageTransition === 'function') { window.triggerStageTransition(target); } 
                ...
                if (isCrit) {
                    window.screenFlash = 0.4; 
                    window.invertFrames = 3; 
                    window.speedLinesAlpha = 1.0; window.spawnLensFlare(target.x, target.y - 40, "#ff003c", 5.0);
                    window.floatingTexts.push({ x: target.x, y: target.y - 120, text: "💀 LETHAL FINISH!", color: "#ff003c", alpha: 1.5, vx: 0, vy: -2, font: "italic 900 65px 'Arial Black', Impact", life: 100, scale: 5.0, targetScale: 1.2, scaleVel: 0, rot: (Math.random()-0.5)*0.2 });
                    
                    window.targetZoom = 1.5; 
                    window.actionCamOffsetX = 0; 
                    window.actionCamOffsetY = -20; 
                    window.dynamicBlur = 25; 
                    
                    window.cameraTilt = (Math.random() > 0.5 ? 1 : -1) * 0.25; 
                    
                    window.lethalVoid = 100; window.impactAberration = 30; 
                    window.spawnForegroundDebris(target.x, target.y, attackerDirRight); window.spawnForegroundDebris(target.x, target.y, attackerDirRight); 
                    
                    window.hitStopFrames = 30; window.shakeScreen(60, 50); window.vhsGlitchTimer = 45; 
                    
                    window.triggerDimensionShatter(target.x, target.y - 50); 
                    ...
                    window.spawnScreenInk("#ff0000"); window.spawnScreenInk("#000000");

                    window.isSpinningCam = true; 
                    window.orbitFocusX = target.x; 
                    window.targetTimeScale = 0.05; 
                    setTimeout(()=>{ window.targetTimeScale = 1.0; window.cameraTilt = 0; window.isSpinningCam = false; window.targetCamOrbitAngle = 0; }, 3000); 

                } else { window.hitStopFrames = 10; window.shakeScreen(20, 15); window.targetZoom = 1.2; window.actionCamOffsetY = -30; window.targetTimeScale = 0.25; window.impactAberration = 15; setTimeout(()=>{window.targetTimeScale = 1.0; }, 900); }
        ...
        } else if (isCrit) { window.impactFrameTimer = 2; window.hitStopFrames = 4; window.shakeScreen(12, 10); window.chromaTimer = 12; window.playSound(180, 'square', 0.3, 0.6, true); } 
...
window.attack = function(attacker, targetGroup) {
    ...
    if (MathDist > reach && !isFinisher) { 
        ...
        window.speedLinesAlpha = 1.0; 
        window.actionCamOffsetX = attacker.isFacingRight ? 50 : -50; 
        attacker.iFrames = 8; window.spawnParticles(attacker.x, attacker.y, "#bdc3c7", true); 
        return; 
    }

    if (selectedMove === 'uppercut' || selectedMove === 'dragon_uppercut') {
        target.vy = -16; target.onGround = false; target.state = 'juggle'; target.hitStun = 40; 
    }

    if (isFinisher) { window.spawnEyeFlash(attacker.x, attacker.y - 60, "#ff4757", attacker.isFacingRight); window.hitStopFrames = 8; }

    let comboSpeedBoost = Math.min(6, Math.floor((attacker.comboHits || 0) * 0.5));
    attacker.attackTimer = isFinisher ? 30 : Math.max(8, 18 - comboSpeedBoost); 
    attacker.state = selectedMove; attacker.vx = (attacker.isFacingRight ? 1 : -1) * (isFinisher ? 6 : 2.0); 
    ...
    if (isFinisher) {
        isCrit = true; finalDmg = baseDmg * 3.5; window.shakeScreen(18, 15); target.vx = (attacker.isFacingRight ? 12 : -12); // Tăng lực văng để dễ đập tường
        if(target.state !== 'juggle') { target.state = 'hurt'; target.hitStun = 50; }
        ...
        window.targetTilt = attacker.isFacingRight ? 0.08 : -0.08;
        setTimeout(() => { window.targetTilt = 0; }, 400);
        ...
        target.vx = (attacker.isFacingRight ? 5 : -5); // Tăng chút lực cho đòn thường
        if(target.state !== 'juggle') { target.hitStun = 18; target.state = 'hurt'; }
    }

    let extraLag = Math.min(5, (attacker.comboHits || 0));
    window.hitStopFrames = isCrit ? (4 + (extraLag/2|0)) : 1; 
...
window.update = function() {
    ...
    window.eyeFlashes.forEach(f => f.life--); window.eyeFlashes = window.eyeFlashes.filter(f => f.life > 0);
    window.screenInks.forEach(i => { i.life--; i.slideY += 0.5; }); window.screenInks = window.screenInks.filter(i => i.life > 0);
    ...
    if (Math.random() < 0.15 && window.bokehs.length < 35 && window.lethalVoid <= 0) { window.bokehs.push({ x: window.camX + Math.random()*1500 - 750, y: Math.random()*window.canvas.height + 100, vx: (Math.random()-0.5)*0.5, vy: -Math.random()*1 - 0.5, life: 250, maxLife: 250, size: Math.random()*20+15, z: Math.random()*2 + 1.5, color: Math.random() > 0.6 ? "#f1c40f" : "#00f3ff" }); }
    ...
    if (!isTimeStopped) {
        ...
        for (let i = window.impactSparks.length - 1; i >= 0; i--) { 
            let isp = window.impactSparks[i]; 
            isp.x += isp.vx; isp.y += isp.vy; isp.vy += window.GRAVITY * 0.8; 
            isp.life--; 
            if (isp.zSpeed) { isp.scale *= isp.zSpeed; } 
            if (isp.y >= window.GROUND_Y) {
                isp.y = window.GROUND_Y;
                isp.vy *= -0.5; isp.vx *= 0.8;  
            }
            if (isp.life <= 0) window.impactSparks.splice(i, 1); 
        }
        ...
        for (let i = window.particles.length - 1; i >= 0; i--) { 
            ...
            else { pt.vy += window.GRAVITY * 0.9; if(pt.y >= window.GROUND_Y) { pt.y = window.GROUND_Y; pt.vy *= -0.3; pt.vx *= 0.7; } } 
    ...
    if (!isTimeStopped) {
        window.enemies.forEach(e => { 
            if (e.hp <= 0 && !e.deathTriggered) {
                ...
                if (window.p1 && window.p1.hp > 0) { let heal = (window.p1.maxHp * 0.08) | 0; window.p1.hp = Math.min(window.p1.maxHp, window.p1.hp + heal); window.floatingTexts.push({ x: window.p1.x, y: window.p1.y - 80, text: `+${heal} 💚`, color: "#2ed573", alpha: 1.5, vx: (Math.random()-0.5)*4, vy: -6, font: "italic 900 30px 'Arial Black', Impact", life: 45, scale: 2.0, targetScale: 1.0, scaleVel: 0, rot: 0 }); }
            }
        });
    ...
    allFighters.forEach(f => {
        ...
        // [GOD TIER] Bóng mờ tàn ảnh tốc độ cao
        if (!isTimeStopped && (f.isRage || f.state === 'dash' || f.state === 'dash_back' || f.attackTimer > 15)) {
            if (!f.trailArr) f.trailArr = [];
            if (window.matchTimer % 2 === 0) {
                f.trailArr.push({x: f.x, timer: 0, isFacingRight: f.isFacingRight, state: f.state, color: f.isPlayer ? "#ff4757" : "#00f3ff", alpha: 0.6, scale: f.scale});
            }
        }
        if (f.trailArr) {
            f.trailArr.forEach(t => { t.timer++; t.alpha -= 0.05; });
            f.trailArr = f.trailArr.filter(t => t.alpha > 0);
        }

        if (isTimeStopped && f !== window.timeStopCaster) { f.trailArr = []; return; }
        
        // [NEW COMBAT] WALL BOUNCE PHYSICS - Bật tường tạo chuỗi combo
        if (f.state === 'hurt' || f.state === 'juggle') {
            if (f.x < window.WALL_PADDING || f.x > window.canvas.width - window.WALL_PADDING) {
                if (Math.abs(f.vx) > 8) { // Lực đập đủ mạnh
                    f.vx *= -0.7; // Nảy ngược lại
                    f.x = f.x < window.WALL_PADDING ? window.WALL_PADDING : window.canvas.width - window.WALL_PADDING;
                    window.shakeScreen(10, 10); window.playSound(400, 'square', 0.2, 0.5, true); window.spawnGlassShatter(f.x, f.y);
                    window.floatingTexts.push({ x: f.x, y: f.y - 50, text: "💥 WALL BOUNCE!", color: "#f1c40f", alpha: 1.5, vx: f.vx * 0.5, vy: -3, font: "italic 900 30px 'Arial Black'", life: 40, scale: 2.0, targetScale: 1.0, scaleVel: 0, rot: 0 });
                } else {
                    f.vx = 0; // Trượt tường nếu lực yếu
                    f.x = f.x < window.WALL_PADDING ? window.WALL_PADDING : window.canvas.width - window.WALL_PADDING;
                }
            }
        }
        ...
        if (f.attackTimer <= 0 && f.hitStun <= 0 && f.dashTimer <= 0 && f.stunTimer <= 0) { if (f.state !== 'idle' && f.state !== 'walk' && f.state !== 'juggle') f.state = 'idle'; }
        if (f.state === 'idle' || f.state === 'walk') f.iFrames = 0;

        // [NEW COMBAT] AURA AWAKENING - BÙNG NỔ HAKI KHI YẾU MÁU
        let wasRage = f.isRage;
        f.isRage = (f.hp > 0 && f.hp <= f.maxHp * 0.2); 
        f.currentSpeed = f.speed || 3; f.currentDmgMod = f.dmgMod || 1; 
        
        if (!wasRage && f.isRage) {
            window.shakeScreen(20, 15); window.playSound(200, 'square', 0.5, 1.0, true);
            window.spawnEnergyPillar(f.x, window.GROUND_Y, f.isPlayer ? "#ff4757" : "#9b59b6", 150, 40);
            if (window.shockwaves.length < MAX_SHOCKWAVES) window.shockwaves.push({x: f.x, y: f.y, r: 10, maxR: 800, color: f.isPlayer ? "#ff4757" : "#9b59b6", alpha: 1, speed: 40});
            window.floatingTexts.push({ x: f.x, y: f.y - 120, text: "🔥 AWAKENING!", color: f.isPlayer ? "#ff4757" : "#9b59b6", alpha: 1.5, vx: 0, vy: -5, font: "italic 900 50px 'Arial Black'", life: 60, scale: 3.0, targetScale: 1.0, scaleVel: 0, rot: 0 });
            allFighters.forEach(e => { if (e !== f && Math.abs(e.x - f.x) < 200) { e.vx = (e.x > f.x ? 15 : -15); e.hitStun = 30; e.state = 'hurt'; } });
        }
            ...
            if (f.hitStun <= 0 && f.stunTimer <= 0 && f.state !== 'dash_back' && f.state !== 'block') {
                ...
                if (isCloseEnough) {
                    if (window.classStats && window.classStats[type] && typeof window.classStats[type].executeUltimate === 'function') {
                        ...
                        window.targetCamOrbitAngle = f.isFacingRight ? -0.7 : 0.7; window.spawnEyeFlash(f.x, f.y - 60, f.isPlayer ? "#ff4757" : "#ff0000", f.isFacingRight); 
                    } else if (typeof window.useUltimate === 'function') { window.useUltimate(f, closestTarget); f.vx = 0; window.speedLinesAlpha = 1.0; window.targetCamOrbitAngle = f.isFacingRight ? -0.7 : 0.7; window.spawnEyeFlash(f.x, f.y - 60, "#ff4757", f.isFacingRight); }
        ...
        if (f.state === 'wall_splat') { f.vy = 0; f.vx = 0; if (f.hitStun < 25) { f.state = 'hurt'; f.vy = 2; f.vx = f.x < window.canvas.width/2 ? 3 : -3; } } 
            ...
            if (f.vy > 0 && f.y + f.vy >= window.GROUND_Y && !f.onGround) { 
                ...
                if (f.vy > 8 || f.state === 'ko_falling') { window.shakeScreen(f.vy > 10 ? 8 : 5, 4); window.spawnEnvDamage(f.x, window.GROUND_Y, 'crater', f.scale || 1, false); }
                if (f.state === 'juggle' && f.vy > 10 && f.hp > 0) { f.vy = -f.vy * 0.5; f.y = window.GROUND_Y - 1; window.playSound(200, 'square', 0.2, 0.5, true); window.floatingTexts.push({ x: f.x, y: f.y - 80, text: "💥 BOUNCE!", color: "#f1c40f", alpha: 1.5, vx: 0, vy: -3, font: "italic 900 30px 'Arial Black', Impact", life: 40, scale: 2.0, targetScale: 1.0, scaleVel: 0, rot: 0 }); }
            } 
        }

        f.y += f.vy; 
        if (f.y >= window.GROUND_Y) { f.y = window.GROUND_Y; if (f.state !== 'juggle') { f.vy = 0; f.onGround = true; f.state = (f.hp <= 0) ? 'dead' : 'idle'; } } 
            ...
            for (let j = i + 1; j < allFighters.length; j++) { 
                ...
                if (dist < 80 && f1.attackTimer > 5 && f1.attackTimer < 20 && f2.attackTimer > 5 && f2.attackTimer < 20) {
                    if (!f1.clashCooldown && !f2.clashCooldown && f1.isFacingRight !== f2.isFacingRight) {
                        f1.clashCooldown = 40; f2.clashCooldown = 40; f1.attackTimer = 0; f2.attackTimer = 0; 
                        
                        // [ANIME APEX] TELEPORT CLASH - BAY LÊN KHÔNG TRUNG ĐỂ TRANH CHẤP
                        f1.vy = -18; f2.vy = -18; 
                        f1.onGround = false; f2.onGround = false;
                        f1.vx = f1.isFacingRight ? -6 : 6; f2.vx = f2.isFacingRight ? -6 : 6; 
                        
                        window.shakeScreen(40, 30); window.playSound(500, 'square', 0.2, 0.8, true);
                        window.targetZoom = 1.6; window.screenFlash = 0.5; window.impactAberration = 35;
                        
                        let midX = (f1.x + f2.x)/2; let midY = (f1.y + f2.y)/2 - 100; 
                        window.spawnParticles(midX, midY, "#ffffff", true);
                        if (window.shockwaves.length < MAX_SHOCKWAVES) window.shockwaves.push({x: midX, y: midY, r: 10, maxR: 600, color: "#ffffff", alpha: 1, speed: 25});
                        
                        window.floatingTexts.push({ x: midX, y: midY - 60, text: "⚔️ ORA ORA ORA!", color: "#f1c40f", alpha: 1.5, vx: 0, vy: -5, font: "italic 900 60px 'Arial Black', Impact", life: 50, scale: 3.5, targetScale: 1.0, scaleVel: 0, rot: (Math.random()-0.5)*0.2 });
                        
                        window.spawnLensFlare(midX, midY, "#00f3ff", 6.0); window.speedLinesAlpha = 1.0;
                        window.triggerDimensionShatter(midX, midY); 
                        
                        for(let k=0; k<10; k++){
                            setTimeout(() => {
                                let tx = midX + (Math.random()-0.5)*200;
                                let ty = midY + (Math.random()-0.5)*150;
                                if(typeof window.spawnSlash === 'function') window.spawnSlash(tx, ty, Math.random() > 0.5, ["#ff003c", "#00f3ff", "#f1c40f", "#ffffff"][Math.floor(Math.random()*4)], true, 2.0 + Math.random()*2, Math.random()*Math.PI*2);
                                if(typeof window.shakeScreen === 'function') window.shakeScreen(5, 12);
                                if(typeof window.playSound === 'function') window.playSound(400, 'square', 0.1, 0.3);
                            }, k * 60);
                        }

                        window.invertFrames = 3; window.isSpinningCam = true; window.targetTilt = 0.1;
                        window.orbitFocusX = midX; window.targetTimeScale = 0.02;
                        setTimeout(() => { window.targetTimeScale = 1.0; window.isSpinningCam = false; window.targetCamOrbitAngle = 0; window.targetTilt = 0; }, 1200);
                    }
                }
    ...
    if (!window.isSpinningCam) { window.orbitFocusX += (midPointX - window.orbitFocusX) * 0.15; }
    ...
    let maxComboHits = Math.max(window.p1 ? window.p1.comboHits || 0 : 0, closest ? closest.comboHits || 0 : 0);
    let comboZoomBonus = Math.min(0.15, maxComboHits * 0.015); 
    let comboTiltBonus = maxComboHits > 4 ? Math.sin(window.matchTimer * 0.3) * (Math.min(0.05, maxComboHits * 0.005)) : 0;

    window.targetCamX = (window.canvas.width / 2) - window.orbitFocusX + window.actionCamOffsetX + (actionPan * 0.5); 
    let zoomYCompensate = (window.targetZoom > 1.0) ? (window.targetZoom - 1.0) * 150 : 0;
    window.targetCamY = (window.canvas.height / 2) - window.orbitFocusY + window.actionCamOffsetY + 40 + zoomYCompensate;

    if (window.gameOver && closest) {
        window.targetZoom = 1.4; 
        if(window.isSpinningCam) window.targetCamY += 40; 
    } 
    else if (closest && !window.gameOver && window.slowMoTimer <= 0 && window.impactFrameCount <= 0) {
        let distance = Math.abs(window.p1.x - closest.x); 
        let dynamicZoom = 1.15 - (distance / 1200) * 0.25; 
        window.targetZoom = Math.max(0.95, Math.min(1.15, dynamicZoom));
        ...
        if (p1Low) window.isLowHpPulsing += 0.2;
...
window.draw = function() {
    ...
    window.ctx.fillStyle = `rgba(0, 0, 0, ${blurStrength})`; window.ctx.fillRect(0, 0, window.canvas.width, window.canvas.height);
    ...
    if (window.invertFrames > 0) {
        ...
        sortedFighters.forEach(p => { 
            if (p && p.hp > 0 && typeof window.drawStickman === 'function') { 
                if (p.iFrames > 20 && (p.state === 'dash' || p.state === 'dash_back')) return;
                
                window.ctx.save();
                window.ctx.translate(p.drawX - p.x, p.drawY - p.y); 
                let scaleRatio = p.drawScale / (p.scale || 1);
                window.ctx.translate(p.x, p.y);
                window.ctx.scale(scaleRatio, scaleRatio);
                let renderFacingRight = p.drawFacingRight;
                if (typeof p.isFacingRight !== 'undefined' && p.isFacingRight !== renderFacingRight) { window.ctx.scale(-1, 1); }
                window.ctx.translate(-p.x, -p.y);
                
                if(p.isDragon && typeof window.drawDragon === 'function') window.drawDragon(window.ctx, p); else window.drawStickman(window.ctx, p); 
                window.ctx.restore(); 
            } 
    ...
    if (window.impactFrameCount > 0) {
        ...
        sortedFighters.forEach(p => { 
            if (p && p.hp > 0) { 
                ...
                window.ctx.save(); window.ctx.translate(window.canvas.width / 2, window.canvas.height / 2); window.ctx.scale(window.currentZoom, window.currentZoom); window.ctx.translate(-window.canvas.width / 2 + window.camX, -window.canvas.height / 2 + window.camY); 
                
                window.ctx.translate(p.drawX - p.x, p.drawY - p.y);
                let scaleRatio = p.drawScale / (p.scale || 1);
                window.ctx.translate(p.x, p.y);
                window.ctx.scale(scaleRatio, scaleRatio);
                if (typeof p.isFacingRight !== 'undefined' && p.isFacingRight !== p.drawFacingRight) { window.ctx.scale(-1, 1); }
                window.ctx.translate(-p.x, -p.y);
                
                let sketchColor = window.impactFrameCount % 2 === 0 ? "#ff003c" : "#000000";
                if (typeof window.drawStickman === 'function') { window.ctx.strokeStyle = sketchColor; window.ctx.lineWidth = 15; window.ctx.lineCap = "round"; window.ctx.lineJoin = "round"; window.ctx.beginPath(); window.ctx.moveTo(p.x, p.y); window.ctx.lineTo(p.x, p.y-60); window.ctx.lineTo(p.x-20, p.y-30); window.ctx.moveTo(p.x, p.y-60); window.ctx.lineTo(p.x+20, p.y-30); window.ctx.moveTo(p.x, p.y); window.ctx.lineTo(p.x-20, p.y+40); window.ctx.moveTo(p.x, p.y); window.ctx.lineTo(p.x+20, p.y+40); window.ctx.stroke(); window.ctx.fillStyle = sketchColor; window.ctx.beginPath(); window.ctx.arc(p.x, p.y-80, 20, 0, Math.PI*2); window.ctx.fill(); }
                window.ctx.restore(); 
            } 
        }); window.ctx.restore(); return; 
    }
    ...
    if (window.isLoading) {
        let cx = window.canvas.width / 2; let cy = window.canvas.height / 2; let tConf = window.thumbnailConfig || { hue: 0, title: "LOADING", emoji: "🔥", rayCount: 16 };
        ...
        for (let i = 0; i < tConf.rayCount; i++) { window.ctx.beginPath(); window.ctx.moveTo(0, 0); let angle1 = (Math.PI * 2 / tConf.rayCount) * i; let angle2 = (Math.PI * 2 / tConf.rayCount) * (i + 0.5); window.ctx.lineTo(Math.cos(angle1) * 2000, Math.sin(angle1) * 2000); window.ctx.lineTo(Math.cos(angle2) * 2000, Math.sin(angle2) * 2000); window.ctx.fill(); }
        window.ctx.restore();
        ...
        window.ctx.shadowBlur = 0; window.ctx.strokeStyle = "#fff"; window.ctx.lineWidth = 6; window.ctx.strokeRect(cx - 250, window.canvas.height - 80, 500, 24); window.ctx.fillStyle = "#ff4757"; window.ctx.fillRect(cx - 247, window.canvas.height - 77, (window.loadProgress / 100) * 494, 18);
        window.ctx.fillStyle = "#fff"; window.ctx.font = "bold 20px monospace"; window.ctx.fillText(`FPS BOOSTING & RENDERING... ${window.loadProgress | 0}%`, cx, window.canvas.height - 20); 
        return; 
    }
    ...
    try {
        ...
        if (window.shakeTime > 0) { window.ctx.translate((Math.random() - 0.5) * window.shakeMag, (Math.random() - 0.5) * window.shakeMag); window.ctx.rotate((Math.random() - 0.5) * 0.02 * window.shakeMag * (Math.PI / 180)); }
        
        window.camDriftX = (window.camDriftX || 0) + (Math.random() - 0.5) * 0.5; window.camDriftY = (window.camDriftY || 0) + (Math.random() - 0.5) * 0.5; window.camDriftX *= 0.95; window.camDriftY *= 0.95;
        let camBreathX = Math.sin(Date.now() / 1300) * 3 + Math.cos(Date.now() / 700) * 2 + window.camDriftX; let camBreathY = Math.cos(Date.now() / 1100) * 3 + Math.sin(Date.now() / 900) * 2 + window.camDriftY;
        ...
        let dynamicYaw = Math.max(0.85, 1 - Math.abs(window.camVelocityX) * 0.001); let dynamicSkew = -(window.camVelocityX * 0.002) + window.actionCamSkew; 
        
        window.ctx.translate(window.canvas.width / 2, window.canvas.height / 2); window.ctx.scale(actualZoom, actualZoom); if (window.cameraTilt) window.ctx.rotate(window.cameraTilt);
        window.ctx.transform(dynamicYaw, 0, dynamicSkew, 1, 0, 0); window.ctx.translate(-window.canvas.width / 2 + window.camX, -window.canvas.height / 2 + window.camY);
        ...
        window.ctx.fillStyle = skyGrad; window.ctx.save(); let cxSky = (window.camX + bgPanOffset) * 0.95; window.ctx.translate(cxSky, 0); window.ctx.fillRect(-cxSky - 1500, -3000, window.canvas.width + 3000, window.canvas.height + 6000); window.ctx.restore();
        ...
        for(let r = startR; r < endR; r += 250) { let rayAlpha = 0.05 + Math.abs(Math.sin(rayShift + r * 0.01)) * 0.05; window.ctx.fillStyle = `rgba(255, 255, 255, ${rayAlpha})`; window.ctx.beginPath(); window.ctx.moveTo(r + Math.sin(rayShift)*100, -800); window.ctx.lineTo(r + 100 + Math.sin(rayShift)*100, -800); window.ctx.lineTo(r + 400, window.GROUND_Y); window.ctx.lineTo(r + 200, window.GROUND_Y); window.ctx.fill(); } window.ctx.restore();
        ...
        if (t2 === "flowing_water" || t2 === "flowing_lava") {
            ...
            const drawWaveLayer = (color, amplitude, frequency, phase, offsetY) => { window.ctx.fillStyle = color; window.ctx.beginPath(); let startX = -cx2 - 1500; let endX = -cx2 + window.canvas.width + 1500; window.ctx.moveTo(startX, window.canvas.height + 400); window.ctx.lineTo(startX, waterBaseY + offsetY); for (let x = startX; x <= endX; x += 40) { let y = waterBaseY + offsetY + Math.sin((x * frequency) + waveSpeed + phase) * amplitude; window.ctx.lineTo(x, y); } window.ctx.lineTo(endX, window.canvas.height + 400); window.ctx.fill(); };
        ...
        window.ctx.save(); let reflectionGrad = window.ctx.createLinearGradient(0, window.GROUND_Y, 0, window.GROUND_Y + 300); reflectionGrad.addColorStop(0, "rgba(255, 255, 255, 0.2)"); reflectionGrad.addColorStop(1, "rgba(0, 0, 0, 0.8)"); 
        window.ctx.globalAlpha = 0.35; window.ctx.translate(0, window.GROUND_Y); window.ctx.scale(1, -0.6); let distY = window.shakeTime > 0 ? Math.sin(Date.now() / 50) * 10 : 0; window.ctx.translate(0, -window.GROUND_Y + distY);
        ...
        sortedFighters.forEach(p => { 
            if (p && p.hp > 0 && typeof window.drawStickman === 'function') { 
                ...
                else window.drawStickman(window.ctx, p); 
                window.ctx.restore(); 
            } 
        });
        window.ctx.restore();
        ...
        if (window.p1) {
            ...
            sortedFighters.forEach(p => { 
                if (p && p.hp > 0 && p.trailArr) { 
                    p.trailArr.forEach(t => { 
                        ...
                        let trailP = Object.assign({}, p, {x: t.x, y: p.y, state: t.state, isFacingRight: proj.drawFacingRight, color: t.color, alpha: t.alpha, scale: proj.drawScale * scaleDown}); 
                        
                        window.ctx.translate(proj.drawX - trailP.x, proj.drawY - trailP.y); 
                        window.ctx.translate(trailP.x, trailP.y);
                        if (!proj.drawFacingRight) window.ctx.scale(-1, 1);
                        window.ctx.translate(-trailP.x, -trailP.y);
                        
                        if (trailP.isDragon && typeof window.drawDragon === 'function') window.drawDragon(window.ctx, trailP, true); 
                        else if (typeof window.drawStickman === 'function') window.drawStickman(window.ctx, trailP, true); window.ctx.restore();
                    }); 
                } 
            });
            ...
            renderPasses.forEach(pass => {
                sortedFighters.forEach(p => { 
                    window.ctx.save(); window.ctx.globalAlpha = 1.0; window.ctx.filter = pass.c;
                    
                    if (p.state === 'ko_falling' || p.state === 'dead') { 
                        window.ctx.translate(p.drawX + pass.o - p.x, p.drawY - p.y); 
                        let scaleRatio = p.drawScale / (p.scale || 1);
                        window.ctx.translate(p.x, p.y);
                        window.ctx.scale(scaleRatio, scaleRatio);
                        
                        let angle = Math.PI / 2; if (p.state === 'ko_falling') { let progress = (100 - p.koTimer) / 30; if (progress > 1) progress = 1; angle = progress * (Math.PI / 2); } 
                        let fallDir = p.drawFacingRight ? -1 : 1; window.ctx.rotate(angle * fallDir); 
                        
                        let renderFacingRight = p.drawFacingRight;
                        if (typeof p.isFacingRight !== 'undefined' && p.isFacingRight !== renderFacingRight) { window.ctx.scale(-1, 1); }
                        window.ctx.translate(-p.x, -p.y);
                        
                        if(p.isDragon && typeof window.drawDragon === 'function') window.drawDragon(window.ctx, p); else if (p.isBruceLee && typeof window.drawBruceLee === 'function') window.drawBruceLee(window.ctx, p); else if (p.isSamurai && typeof window.drawSamurai === 'function') window.drawSamurai(window.ctx, p); else if (p.isNinja && typeof window.drawNinja === 'function') window.drawNinja(window.ctx, p); else if (typeof window.drawStickman === 'function') window.drawStickman(window.ctx, p); 
                    } else { 
                        if (p.iFrames > 20 && (p.state === 'dash' || p.state === 'dash_back')) { 
                            window.ctx.globalAlpha = 0.2;
                            window.ctx.filter = 'blur(5px)';
                        }
        ...
        window.ctx.globalCompositeOperation = 'lighter';
        window.eyeFlashes.forEach(f => {
            let pr = project3D(f); window.ctx.save(); window.ctx.translate(pr.drawX, pr.drawY);
            let prog = 1 - (f.life / f.maxLife); window.ctx.globalAlpha = Math.max(0, 1 - Math.pow(prog, 2));
            window.ctx.fillStyle = "#fff"; window.ctx.shadowBlur = 30; window.ctx.shadowColor = f.color;
            let w = 800 * Math.sin(prog * Math.PI) * pr.drawScale; let h = 4 * pr.drawScale;
            window.ctx.fillRect(-w/2, -h/2, w, h); window.ctx.fillStyle = f.color; window.ctx.fillRect(-w/2, -h, w, h*2); window.ctx.restore();
        });
        window.ctx.globalCompositeOperation = 'source-over';
        ...
        window.slashes.forEach(s => { let pr = project3D(s); window.ctx.save(); window.ctx.translate(pr.drawX, pr.drawY); if (!s.isRight) window.ctx.scale(-1, 1); window.ctx.scale(s.scale * pr.drawScale, s.scale * pr.drawScale); window.ctx.rotate(s.rotation || 0); let prog = 1 - (s.life / s.maxLife); window.ctx.globalAlpha = Math.max(0, 1 - Math.pow(prog, 2)); window.ctx.beginPath(); window.ctx.arc(0, 0, 50 + prog * 30, -Math.PI/2 + prog*1.5, Math.PI/2 - prog*1.5); window.ctx.lineWidth = 20 * (1 - prog); window.ctx.strokeStyle = "#fff"; window.ctx.lineCap = "round"; window.ctx.shadowBlur = 30; window.ctx.shadowColor = s.color; window.ctx.stroke(); window.ctx.beginPath(); window.ctx.arc(0, 0, 45 + prog * 35, -Math.PI/2 + prog*1.2, Math.PI/2 - prog*1.2); window.ctx.lineWidth = 40 * (1 - prog); window.ctx.strokeStyle = s.color; window.ctx.globalAlpha *= 0.5; window.ctx.stroke(); window.ctx.restore(); });
        
        // [OPTIMIZED FPS] TẮT BỘ LỌC SHADOWBLUR CHO HẠT NHỎ
        window.particles.forEach(pt => { 
            ...
            if (pt.isGroundDust) { window.ctx.beginPath(); window.ctx.ellipse(pr.drawX, window.GROUND_Y - pr.drawZ*0.15 - pt.size/2, pt.size * 1.5 * pr.drawScale, pt.size * 0.4 * pr.drawScale, 0, 0, Math.PI*2); window.ctx.fill(); }
                ...
                // XÓA SHADOWBLUR Ở ĐÂY ĐỂ CỨU FPS
                window.ctx.beginPath(); window.ctx.arc(pr.drawX, pr.drawY, pt.size*pr.drawScale, 0, Math.PI*2); window.ctx.fill(); 
        ...
        window.ctx.globalCompositeOperation = 'lighter';
        window.spaceRipples.forEach(r => { let pr = project3D(r); window.ctx.save(); window.ctx.translate(pr.drawX, pr.drawY); window.ctx.beginPath(); window.ctx.arc(0, 0, r.r * pr.drawScale, 0, Math.PI*2); window.ctx.lineWidth = 20 * (r.life / r.maxLife) * pr.drawScale; window.ctx.strokeStyle = r.color; window.ctx.shadowBlur = 40; window.ctx.shadowColor = r.color; window.ctx.globalAlpha = Math.max(0, r.life / r.maxLife); window.ctx.stroke(); window.ctx.restore(); });
        
        window.impactSparks.forEach(isp => { let pr = project3D(isp); window.ctx.save(); window.ctx.translate(pr.drawX, pr.drawY); window.ctx.globalAlpha = Math.max(0, Math.min(1, isp.life / isp.maxLife)); window.ctx.fillStyle = isp.color; window.ctx.shadowBlur = 15; window.ctx.shadowColor = isp.color; window.ctx.beginPath(); let len = Math.sqrt(isp.vx*isp.vx + isp.vy*isp.vy) * 2.5 * pr.drawScale; let ang = Math.atan2(isp.vy, isp.vx); window.ctx.rotate(ang); window.ctx.ellipse(0, 0, len, 3*pr.drawScale, 0, 0, Math.PI*2); window.ctx.fill(); window.ctx.restore(); });
        ...
        if (window.dimensionCracks && window.dimensionCracks.length > 0) {
            ...
            window.dimensionCracks.forEach(c => {
                ...
                const drawCrackPath = (path, width) => { if (path.length < 2) return; window.ctx.beginPath(); let p0 = project3D(path[0]); window.ctx.moveTo(p0.drawX, p0.drawY); for (let i = 1; i < path.length; i++) { let pi = project3D(path[i]); window.ctx.lineTo(pi.drawX, pi.drawY); } window.ctx.lineWidth = width * alpha * p0.drawScale; window.ctx.stroke(); };
                window.ctx.lineCap = "round"; window.ctx.lineJoin = "miter"; window.ctx.shadowBlur = 20; window.ctx.shadowColor = "#ffffff"; window.ctx.strokeStyle = "rgba(220, 255, 255, 0.5)"; drawCrackPath(c.main, 12); c.branches.forEach(b => drawCrackPath(b, 5));
                window.ctx.shadowBlur = 0; window.ctx.strokeStyle = "#ffffff"; drawCrackPath(c.main, 3); c.branches.forEach(b => drawCrackPath(b, 1.5));
            }); window.ctx.restore();
        }

        if (window.mangaSfx) {
            window.mangaSfx.forEach(sfx => { let pr = project3D(sfx); window.ctx.save(); window.ctx.translate(pr.drawX, pr.drawY); window.ctx.rotate(sfx.ang); window.ctx.scale(pr.drawScale, pr.drawScale); let alpha = Math.min(1, sfx.life / 10); window.ctx.globalAlpha = alpha; window.ctx.font = `900 italic ${sfx.size}px Impact, Arial Black, sans-serif`; window.ctx.lineWidth = 4; window.ctx.strokeStyle = "#000"; window.ctx.strokeText(sfx.text, 0, 0); window.ctx.fillStyle = sfx.isCrit ? "#ff003c" : "#ffffff"; window.ctx.fillText(sfx.text, 0, 0); window.ctx.restore(); });
        }

        if (window.foregroundDebris && window.foregroundDebris.length > 0) {
            window.ctx.save(); window.ctx.filter = 'blur(4px)'; 
            window.foregroundDebris.forEach(d => { window.ctx.save(); window.ctx.translate(d.x, d.y); window.ctx.rotate(d.rot); window.ctx.scale(d.scale, d.scale); window.ctx.globalAlpha = Math.max(0, d.life / d.maxLife); window.ctx.fillStyle = "#333"; window.ctx.fillRect(-15, -15, 30, 30); window.ctx.restore(); }); window.ctx.restore();
        }
        ...
        if (window.speedLinesAlpha > 0) {
            ...
            window.ctx.globalAlpha = Math.min(1, window.speedLinesAlpha);
            window.ctx.lineWidth = 2;
            for (let i = 0; i < window.speedLineAngles.length; i++) {
                let ang = window.speedLineAngles[i];
                let length = 500 + Math.random() * 800;
                let startR = 150 + Math.random() * 100;
                window.ctx.strokeStyle = Math.random() > 0.5 ? "rgba(255,255,255,0.8)" : "rgba(100, 200, 255, 0.4)";
                window.ctx.beginPath();
                window.ctx.moveTo(Math.cos(ang) * startR, Math.sin(ang) * startR);
                window.ctx.lineTo(Math.cos(ang) * length, Math.sin(ang) * length);
                window.ctx.stroke();
                window.speedLineAngles[i] += 0.01;
            }
            window.ctx.restore();
        }
        ...
        for(let b=0; b<6; b++) { let bx = ((b * 500 + timeB * 400 - window.camX * 2.5) % 3000) - 500; let by = window.canvas.height/2 + Math.sin(b + timeB)*300; window.ctx.beginPath(); window.ctx.arc(bx, by, 80 + b*20, 0, Math.PI*2); window.ctx.fill(); } window.ctx.restore();
        ...
        if (window.bokehs.length > 0) {
            ...
            window.bokehs.forEach(b => { window.ctx.save(); let prX = window.canvas.width/2 + (b.x - window.camX) * b.z; let prY = window.canvas.height/2 + (b.y - window.camY) * b.z; window.ctx.translate(prX, prY); window.ctx.globalAlpha = Math.min(1, b.life / 50) * 0.4; window.ctx.fillStyle = b.color; window.ctx.filter = 'blur(10px)'; window.ctx.beginPath(); window.ctx.arc(0, 0, b.size * b.z, 0, Math.PI*2); window.ctx.fill(); window.ctx.restore(); }); window.ctx.restore();
        }

        window.ctx.save(); window.ctx.globalCompositeOperation = 'multiply';
        let vignetteRadius = window.isLowHpPulsing > 0 ? (window.canvas.height*0.3 - Math.sin(window.isLowHpPulsing)*50) : window.canvas.height*0.3;
        let vignette = window.ctx.createRadialGradient(window.canvas.width/2, window.canvas.height/2, vignetteRadius, window.canvas.width/2, window.canvas.height/2, window.canvas.width*0.8);
        vignette.addColorStop(0, "rgba(255, 255, 255, 1)"); vignette.addColorStop(1, window.isLowHpPulsing > 0 ? "rgba(100, 0, 0, 1)" : "rgba(100, 100, 100, 1)"); 
        window.ctx.fillStyle = vignette; window.ctx.fillRect(0, 0, window.canvas.width, window.canvas.height); window.ctx.restore();

        window.ctx.save(); window.ctx.globalCompositeOperation = 'multiply';
        window.screenInks.forEach(ink => { window.ctx.globalAlpha = Math.max(0, ink.life / ink.maxLife) * 0.8; window.ctx.fillStyle = ink.color; window.ctx.beginPath(); window.ctx.arc(ink.x, ink.y + ink.slideY, 30 * ink.scale, 0, Math.PI*2); window.ctx.arc(ink.x - 20, ink.y - 40 + ink.slideY, 15 * ink.scale, 0, Math.PI*2); window.ctx.arc(ink.x + 30, ink.y - 20 + ink.slideY, 10 * ink.scale, 0, Math.PI*2); window.ctx.fill(); window.ctx.fillRect(ink.x - 5*ink.scale, ink.y + ink.slideY, 10*ink.scale, ink.slideY * 2); }); window.ctx.restore();

        if (window.screenFlash > 0) { window.ctx.globalCompositeOperation = 'screen'; window.ctx.fillStyle = `rgba(255, 255, 255, ${Math.min(window.screenFlash, 0.8)})`; window.ctx.fillRect(0, 0, window.canvas.width, window.canvas.height); window.ctx.globalCompositeOperation = 'source-over'; }

        if (window.cinemaBarsHeight > 1) { window.ctx.fillStyle = "#000000"; window.ctx.fillRect(0, 0, window.canvas.width, window.cinemaBarsHeight); window.ctx.fillRect(0, window.canvas.height - window.cinemaBarsHeight, window.canvas.width, window.cinemaBarsHeight); }
