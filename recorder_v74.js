// ==========================================
// RECORDER.JS - V67.2 BUTTER SMOOTH MAX (FULL VERSION - KHÔNG CẮT BỚT)
// Tích hợp: Intro Manga, HUD, Binge-Watch Grid, UI List
// Nâng cấp: Object Pooling, Text Memoization, Path2D Caching, Fixed 60FPS
// ==========================================
...
// [V67.2] Biến Cache tối ưu hóa bộ nhớ
...
// [V67.2] Object Pooling: Khởi tạo sẵn mảng cố định để không xả rác RAM
...
window.startRecording = function() {
    ...
    // [V67.2] Reset Object Pooling Particles thay vì gán mảng rỗng []
...
// [V67.2] Text Memoization (Cache dòng chữ để tránh CPU bị quá tải vì split và measureText)
...
window.captureFrames = function() {
    ...
    // [V67.2] Hard FPS Cap (16ms = ~60FPS) bảo vệ máy tính hoàn toàn khỏi drop khung hình
    let now = performance.now();
    if (now - window._lastFrameTime < 16) return; 
    ...
    // [V67.2] Không chạy glitch trên video màn hình ngang để giảm 50% gánh nặng Card đồ họa
    if (renderNormalH) {
    ...
    // ==========================================
    // 3. BACKGROUND LƯỚI KHÔNG GIAN 3D (TỐI ƯU CỰC MẠNH VỚI PATH2D)
    // ==========================================
    ...
    // [V67.2] Tạo Cache cho các đường lưới dọc cố định
    ...
    // Dùng Stroke Path2D giải phóng CPU thay vì tính lineTo mỗi frame
    ctxV.stroke(window._gridVerticalPath);
    
    // Đường ngang hiệu ứng trôi
    ctxV.beginPath(); 
    ...
    // ==========================================
    // [V67.2] HẠT BỤI KHÔNG GIAN (OBJECT POOLING)
    // ==========================================
    if (Math.random() < 0.3) {
        // Tìm 1 hạt đang tắt để tái chế thay vì Push hạt mới
    ...
    for (let i = 0; i < window.retentionParticles.length; i++) {
        ...
        ctxV.fillRect(p.x | 0, p.y | 0, p.s, p.s); 
        
        // Trôi quá màn hình thì tắt đi để dùng lại
    ...
    // ==========================================
    // [V67.2] EMOJIS BAY LÊN (OBJECT POOLING)
    // ==========================================
