...
// [BẢN HOÀN CHỈNH ULTIMATE: PENDING HIT + HIT-STOP + JUGGLE + CHỮ SÁT THƯƠNG RƠI XUỐNG ĐẤT]
// ==========================================
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce, attackerDirRight = true) {
    ...
    let staminaBefore = target.stamina;
    ...
    if (finalDmg > 0) {
        ...
        if (isCrit) {
            ...
            if (target.hp > 0) window.hitStopFrames = 12;
        ...
        if (target.hp <= 0) {
            ...
            if (isNearWall && isCrit && typeof window.triggerStageTransition === 'function') { window.triggerStageTransition(target); } 
                ...
                if (isCrit) {
                    ...
                    window.isSpinningCam = true; window.orbitFocusX = target.x; window.targetTimeScale = 0.1; 
                    setTimeout(()=>{ window.targetTimeScale = 1.0; window.cameraTilt = 0; window.isSpinningCam = false; window.targetCamOrbitAngle = 0; }, 2500); 

                } else { window.shakeScreen(20, 15); window.targetZoom = 1.4; window.actionCamOffsetY = -30; window.targetTimeScale = 0.25; window.impactAberration = 15; setTimeout(()=>{window.targetTimeScale = 1.0; }, 900); }
