...
// Tối ưu Layout: Clean Game -> Smart Centered Alerts -> Multi-line Live Chat -> Green Screen Cat 
// ==========================================
...
window.captureFrames = function() {
    ...
    if (!isOutroActive) {
        ...
        if (window.p1) {
            ...
            let hudBaseY = splitGameHeight + 90; // ~697
        ...
        if (chatNow - window._lastChatUpdate > window._nextChatDelay) {
            ...
            window._liveChats.push({ name: randomCeleb.name, color: randomCeleb.color, msg: randomMsg });
            if (window._liveChats.length > 8) window._liveChats.shift(); // Giữ 8 bình luận để cuộn
            window._fakeViewers += Math.floor(Math.random() * 3000) - 1000; 
        }

        // --- SMART LIVE ALERTS (HIỂN THỊ CHÍNH GIỮA, NẰM TRÊN KHUNG CHAT) ---
        ...
        // Vị trí nằm trọn vẹn ở giữa 2 thanh máu Boss và Player
        let alertStartY = splitGameHeight + 160; 
        ...
        // --- KHUNG LIVE CHAT (HỖ TRỢ XUỐNG DÒNG - WORD WRAP) ---
        ctxV.save();
        let boxWidth = 960;
        let boxHeight = 440;
        let boxX = 540; 
        ...
        // Vẽ dòng chữ cuộn lên (Hỗ trợ bẻ dòng)
        ...
        for (let i = window._liveChats.length - 1; i >= 0; i--) { // Vẽ từ dưới lên
            ...
            currentY -= 15; // Khoảng cách giữa 2 người khác nhau
        }
