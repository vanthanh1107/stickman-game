// ==========================================
// RECORDER.JS - V70.3 DEEP NAME EXTRACTION (THE DIRECTOR'S CUT)
// ĐÃ FIX LỖI TÊN: Bắt trực tiếp tên gốc từ Game liên tục theo thời gian thực (Deep Scan).
// CẬP NHẬT: Thanh Win Rate chỉ có Xanh/Đỏ, loại bỏ hoàn toàn chữ thừa.
// CẢM XÚC: Cinematic, Sakuga Anime Combat, Tối giản và Đẳng cấp.
...
window.getRealCharName = function(obj, fallback) {
    ...
    let n = null;
    // Quét vào bảng chỉ số Class nếu game dùng ID
    ...
    // Quét trực tiếp trên Object
    if (!n) n = obj.className || obj.name || obj.type || obj.id;
    if (!n || n === "undefined" || n === "null") return fallback;
...
window.drawProceduralIntro = function(ctx, w, h, progress) {
    ...
    // LẤY TÊN THỰC TẾ TRONG REAL-TIME ĐỂ KHÔNG BAO GIỜ TRƯỢT
    ...
    const drawFighterArtistic = (charObj, cx, cy, forcedState, isFacingRight, theme, fx, charName) => {
        ...
        if (charName && fx !== 'silhouette') {
            ...
            ctx.textAlign = "center"; ctx.textBaseline = "middle";
            ctx.font = "900 32px 'Arial Black', sans-serif"; // Nhỏ gọn tinh tế
    ...
    // --- RENDER DỰA TRÊN KỊCH BẢN (TRUYỀN TÊN THẬT VÀO) ---
    if (cut.focus === 'p1') {
        drawFighterArtistic(window.p1, 0, 100, cut.state, true, prm.themeP1, cut.fx, realP1Name);
    } 
    else if (cut.focus === 'p2') {
        drawFighterArtistic(e1, 0, 100, cut.state, false, prm.themeP2, cut.fx, realP2Name);
    } 
    else { // Clash
...
window.startRecording = function() {
    ...
    // LẤY TÊN GAME DÙNG DEEP SCANNER CHO MẢNG METADATA (VIDEO LIST UI)
    ...
    if (renderNormalV) {
        ...
        if (!isOutroActive) {
            ...
            if (window.p1) {
                ...
                // LẤY TÊN LIÊN TỤC TRONG LÚC VẼ HUD (DEEP SCAN)
            ...
            // THANH WIN RATE TỐI GIẢN (LOẠI BỎ TÊN CHỮ)
