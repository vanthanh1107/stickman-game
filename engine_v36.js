// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 29.0 [THE 360 MATRIX ORBIT]
// [BASE] Giữ nguyên 100% mã nguồn V28.0 đã chạy hoàn hảo.
// [NEW] 360-DEGREE ORBIT CAMERA: Camera lướt vòng quanh nhân vật (Góc quay đa chiều giả lập 3D).
// [NEW] TRIGGER CHUYÊN NGHIỆP: Góc quay 360 tự động kích hoạt khi xả Ultimate, Đụng chiêu (Clash) và Kết liễu (Lethal).
// ==========================================
...
// [VIRAL 29.0] 360 MATRIX ORBIT VARS
window.orbitActive = 0; 
window.orbitFocusX = 0; 
window.orbitFocusY = 0;
        ...
        if (target.hp <= 0) {
            ...
            if (isNearWall && isCrit && typeof window.triggerStageTransition === 'function') { window.triggerStageTransition(target); } 
                ...
                if (isCrit) {
                    ...
                    window.invertFrames = 6; window.spawnLensFlare(target.x, target.y - 40, "#ff003c", 5.0);
                    ...
                    // [VIRAL 29.0] 360 MATRIX ORBIT ON K.O
                    window.orbitActive = Math.PI * 2; 
                    window.orbitFocusX = target.x;
                    window.orbitFocusY = target.y;
                    
                    window.targetTimeScale = 1.0;
                    setTimeout(()=>{ 
                        window.targetTimeScale = 0.02; // Bullet time
                        setTimeout(()=>{ 
                            window.targetTimeScale = 1.0; 
                            window.cameraTilt = 0; 
                        }, 2500); 
...
window.update = function() {
            ...
            for (let j = i + 1; j < allFighters.length; j++) { 
                ...
                if (dist < 80 && f1.attackTimer > 5 && f1.attackTimer < 20 && f2.attackTimer > 5 && f2.attackTimer < 20) {
                    if (!f1.clashCooldown && !f2.clashCooldown && f1.isFacingRight !== f2.isFacingRight) {
                        ...
                        // [VIRAL 29.0] 360 MATRIX ORBIT ON CLASH
                        window.orbitActive = Math.PI * 1.5; 
                        window.orbitFocusX = midX;
                        window.orbitFocusY = midY;
    ...
    // [VIRAL 29.0] Ghi đè hệ thống Camera để quay 360 độ (Matrix Bullet-Time)
    if (window.orbitActive > 0) {
        window.orbitActive -= 0.06; // Tốc độ quay
        if (window.orbitActive < 0) window.orbitActive = 0;
        let angle = (Math.PI * 2) - window.orbitActive; 
        
        // Quét góc quay trái/phải cực gắt
        window.actionCamOffsetX = Math.sin(angle) * 550; 
        // Bóp méo trục Z (Skew) tạo cảm giác 3 chiều
        window.actionCamSkew = Math.cos(angle) * 0.25;  
        // Nhịp thở của ống kính (Zoom nhồi)
        window.targetZoom = 1.6 + Math.cos(angle * 2) * 0.2; 
        // Nghiêng máy bay lượn
        window.cameraTilt = Math.sin(angle) * 0.12; 
        
        // Khóa ống kính tuyệt đối vào tâm xoay
        window.targetCamX = ((window.canvas.width / 2) - window.orbitFocusX) + window.actionCamOffsetX; 
        window.targetCamY = Math.max(0, (window.GROUND_Y - window.orbitFocusY) * 0.5 + 40) + window.actionCamOffsetY; 
        
        // Luôn bật tia tốc độ khi đang bay vòng quanh
        window.speedLinesAlpha = 1.0;
    }
    else if (window.p1 && window.introTimer === 0) {
...
window.draw = function() {
    ...
    if (window.isLoading) {
        ...
        window.ctx.fillStyle = "#00f3ff"; window.ctx.font = "italic 900 40px Arial"; window.ctx.textAlign = "center"; window.ctx.shadowBlur = 15; window.ctx.shadowColor = "#00f3ff"; window.ctx.fillText("360 MATRIX ORBIT INITIALIZING...", cx, cy - 50);
