// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 23.0 [THE MATRIX CAMERAMAN]
// [NEW] MULTI-ANGLE ACTION CAM: Camera tự động lia theo đòn đánh (Lên, Xuống, Xuyên thấu).
// [NEW] INSTANT CLOSE-UP: Cắt cảnh cận mặt bạo lực khi có đòn chí mạng.
// [NEW] RADIAL ACTION BLUR: Làm mờ hậu cảnh theo tốc độ di chuyển chuẩn Anime.
// ==========================================
...
// [VIRAL 23.0] HỆ THỐNG MÁY QUAY MA TRẬN
...
window.actionCamOffsetX = 0; // Bù trừ lia máy trục X
window.actionCamOffsetY = 0; // Bù trừ lia máy trục Y
window.dynamicBlur = 0; // Độ mờ Radial quang học
...
window.initGameEngine = function() {
    ...
    window.viralColorGrade = {
        ...
        brightness: 85 + Math.random() * 20,    
        saturation: 90 + Math.random() * 40,    
        hue: Math.floor((Math.random() - 0.5) * 40) 
    };

    const lightingThemes = [
        { mix: 'overlay', color1: 'rgba(255, 150, 50, 0.25)', color2: 'rgba(20, 0, 0, 0.4)' },
        { mix: 'hard-light', color1: 'rgba(0, 200, 255, 0.15)', color2: 'rgba(0, 10, 40, 0.6)' },
        { mix: 'color-burn', color1: 'rgba(255, 50, 50, 0.15)', color2: 'rgba(0, 0, 0, 0.5)' },
        { mix: 'screen', color1: 'rgba(255, 0, 255, 0.1)', color2: 'rgba(0, 255, 255, 0.05)' },
        { mix: 'multiply', color1: 'rgba(255, 255, 255, 0.0)', color2: 'rgba(30, 40, 50, 0.8)' }
    ];
    ...
    for (let i = 0; i < imgData.data.length; i += 4) {
        ...
        imgData.data[i+3] = Math.random() * 22; 
    }
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce) {
    ...
    if (target.state === 'block') { 
        if (target.stamina < 15) {
            ...
            // [VIRAL 23.0] ACTION CAM: Vỡ khiên thì camera giật sát vào
    ...
    if (finalDmg > 0) {
        ...
        if (isCrit) {
            ...
            // [VIRAL 23.0] ĐẠO DIỄN LIA MÁY CẬN CẢNH KHI CÓ CHÍ MẠNG
            window.targetZoom = 1.35; 
            window.dynamicBlur = 12; // Mờ xung quanh
            window.actionCamOffsetX = (target.isFacingRight ? 80 : -80); // Snap máy quay về hướng kẻ bị đánh văng
        ...
        if (target.hp <= 0) {
            ...
            if (isNearWall && isCrit && typeof window.triggerStageTransition === 'function') { window.triggerStageTransition(target); } 
                ...
                if (isCrit) {
                    ...
                    // [VIRAL 23.0] GÓC MÁY TỬ THẦN ĐIỆN ẢNH KHI KẾT LIỄU
                    ...
                    window.hitStopFrames = 25; window.shakeScreen(60, 35);
                    window.vhsGlitchTimer = 35; window.triggerDimensionShatter(); 
                    window.targetTimeScale = 0.05; setTimeout(()=>{ window.targetTimeScale = 1.0; window.cameraTilt = 0; }, 2000); 
                } else { window.hitStopFrames = 12; window.shakeScreen(30, 20); window.targetZoom = 1.5; window.actionCamOffsetY = -40; window.targetTimeScale = 0.2; setTimeout(()=>{window.targetTimeScale = 1.0; }, 1000); }
...
window.attack = function(attacker, targetGroup) {
    ...
    if (MathDist > reach && !isFinisher) { 
        ...
        // [VIRAL 23.0] LIA MÁY THEO CÚ DASH TỚI
    ...
    // [VIRAL 23.0] ĐẠO DIỄN QUAY PHIM THEO LOẠI ĐÒN ĐÁNH
    if (['uppercut', 'dragon_uppercut', 'high_kick'].includes(selectedMove)) {
        slashAngle = -Math.PI / 5;
        window.actionCamOffsetY = 80; // Hất máy quay lên trời
    } else if (['axe_kick', 'elbow_strike'].includes(selectedMove)) {
        slashAngle = Math.PI / 5;
        window.actionCamOffsetY = -40; // Máy quay ép xuống đất
    ...
    if (isFinisher) {
        ...
        window.actionCamOffsetX = attacker.isFacingRight ? 100 : -100; // Snap camera mạnh theo hướng tung đòn kết liễu
    } else {
...
window.update = function() {
    ...
    // [VIRAL 23.0] Tự động hồi vị trí quay phim quang học
    ...
    if (window.p1 && window.introTimer === 0) {
        ...
        // [VIRAL 23.0] MULTI-ANGLE ACTION CAM TRACKING
...
window.draw = function() {
    ...
    if (window.isLoading) {
        ...
        window.ctx.fillStyle = "#00f3ff"; window.ctx.font = "italic 900 40px Arial"; window.ctx.textAlign = "center"; window.ctx.shadowBlur = 15; window.ctx.shadowColor = "#00f3ff"; window.ctx.fillText("MATRIX CAMERAMAN INITIALIZING...", cx, cy - 50);
        ...
        window.ctx.fillStyle = "#fff"; window.ctx.font = "bold 16px monospace"; window.ctx.fillText(`RENDERING CINEMATOGRAPHY... ${Math.floor(window.loadProgress)}%`, cx, cy + 60);
        return; 
    }
    ...
    try {
        ...
        let camBreathX = Math.sin(Date.now() / 1500) * 4 + Math.cos(Date.now() / 900) * 2 + window.camDriftX;
        let camBreathY = Math.cos(Date.now() / 1200) * 4 + Math.sin(Date.now() / 800) * 2 + window.camDriftY;
        ...
        else if (window.viralHueShift) { window.ctx.filter = `contrast(${window.viralColorGrade.contrast}%) brightness(${window.viralColorGrade.brightness}%) saturate(${window.viralColorGrade.saturation}%) hue-rotate(${window.viralColorGrade.hue}deg)`; }
        ...
        // [VIRAL 23.0] DEEP Z-AXIS PARALLAX SHIFT
...
// ==========================================
// 7. [VIRAL 23.0] AFTER EFFECTS TIME-RAMPING & UNLOCKED 120Hz LOOP
// ==========================================
