...
// [ĐÃ FIX: XÓA HIỆU ỨNG CHỚP NHÁY TRẮNG ĐEN GÂY ĐAU MẮT KHI K.O VÀ SẤM SÉT]
// ==========================================
...
window.update = function() {
    ...
    if (!isTimeStopped) {
        ...
        window.weatherParticles.forEach(w => { 
            // Nhóm 1: Các hạt bay ngược lên trời (Khói độc, Tro bụi, Đom đóm)
            if (window.currentWeather === 'toxic' || window.currentWeather === 'ash' || window.currentWeather === 'fireflies') { 
                w.y -= w.speed * 0.5; 
                w.x += Math.sin(w.y/30)*2 + window.globalWind; 
                if(w.y < -20) { w.y = window.canvas.height + 20; w.x = Math.random() * 1200 - 300; } 
            } 
            // Nhóm 2: Mưa mã code Ma Trận (Rơi thẳng đứng cực nhanh)
            ...
            // Nhóm 3: Mưa sao băng (Bay chéo góc siêu tốc)
            ...
            // Nhóm 4: Rơi bình thường (Mưa, Tuyết, Cánh hoa, Mưa máu)
            else { 
                w.y += w.speed; 
                w.x += ((window.currentWeather === 'rain' || window.currentWeather === 'blood_rain') ? -3 : Math.sin(w.y/50)*2) + window.globalWind; 
    ...
    if (window.p1 && window.introTimer === 0) {
        // 🌟 K.O ZOOM CAMERA (Bám sát nhân vật)
        ...
        if (window.gameOver && closest) {
            ...
            let maxPanX = window.canvas.width * 0.85; // Mở rộng giới hạn góc quay
            ...
            window.targetCamY = Math.max(0, Math.min(280, jumpFollowY)); 
            ...
            window.targetCamY = Math.max(0, Math.min(280, (window.GROUND_Y - focusY) * 0.5)); 
...
window.draw = function() {
    ...
    try {
            ...
            window.envDamage.forEach(dmg => {
                ...
                dmg.cracks.forEach(path => {
                    if (Array.isArray(path) && path.length > 0) {
                        ...
                        path.forEach((pt, idx) => { if (idx === 0) window.ctx.moveTo(pt.x, pt.y); else window.ctx.lineTo(pt.x, pt.y); });
                        window.ctx.stroke();
        ...
        window.weatherParticles.forEach(w => { 
            ...
            // 🆕 HIỆU ỨNG MỚI: Đom đóm (Lấp lánh vàng)
            else if (window.currentWeather === 'fireflies') {
                let glow = 0.3 + Math.abs(Math.sin(Date.now()/300 + w.x)) * 0.7; // Chớp tắt liên tục
                window.ctx.fillStyle = `rgba(241, 196, 15, ${glow})`;
                window.ctx.shadowBlur = 10; window.ctx.shadowColor = "#f1c40f";
                window.ctx.beginPath(); window.ctx.arc(w.x, w.y, w.size * 1.5, 0, Math.PI*2); window.ctx.fill();
                window.ctx.shadowBlur = 0;
            }
            // 🆕 HIỆU ỨNG MỚI: Mưa code Ma trận (Rơi chữ 0 1)
            else if (window.currentWeather === 'matrix_rain') {
                window.ctx.fillStyle = `rgba(0, 255, 0, ${0.4 + Math.random()*0.4})`;
                window.ctx.font = "bold 16px monospace";
                window.ctx.fillText(w.char || (Math.random() > 0.5 ? "1" : "0"), w.x, w.y);
            }
            // 🆕 HIỆU ỨNG MỚI: Mưa Máu (Tia dài màu đỏ rực)
            else if (window.currentWeather === 'blood_rain') {
                window.ctx.strokeStyle = "rgba(220, 20, 60, 0.8)";
                window.ctx.lineWidth = 2.5;
                window.ctx.beginPath(); window.ctx.moveTo(w.x, w.y); window.ctx.lineTo(w.x - 4, w.y + 25); window.ctx.stroke();
            }
            // 🆕 HIỆU ỨNG MỚI: Mưa Sao băng (Tia đuôi dài vệt trắng/xanh)
            else if (window.currentWeather === 'shooting_stars') {
                if (Math.random() > 0.95) return; // Làm thưa bớt sao băng
        ...
        if (window.introTimer > 0 && !window.gameOver && window.p1) {
            ...
            if (window.introTimer > 60) {
                ...
                // 🥊 CHỮ FIGHT VÀ GĂNG TAY HOÀNH TRÁNG TỪ BẢN 9.0
