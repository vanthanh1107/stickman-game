// ==========================================
// RECORDER.JS - V70.5 THE CINEMA EXPERIENCE (HOLLYWOOD LORE)
// ĐÃ FIX: Intro 4 Giây chuẩn Cấu trúc 3 Hồi (3-Act Structure) của Hollywood.
// CẬP NHẬT: Màn hình Chapter, Bụi điện ảnh (Dust), Ánh sáng God Rays, Camera Panning.
// CẢM XÚC: Hồi hộp, có cốt truyện sâu sắc, bùng nổ tột độ.
...
// QUẢN LÝ INTRO ĐIỆN ẢNH (4 GIÂY)
window.introStartTime = 0; 
window.introDuration = 4000; 
window._sfxCuts = [false, false, false, false, false, false]; 
...
// ====================================================================
// PROCEDURAL ENGINE (ĐẠO DIỄN KỊCH BẢN PHIM)
// ====================================================================
const THEMES = [
...
const LORES = [
    { chapter: "ACT I: THE BETRAYAL", p2Sub: "I taught you everything you know...", p1Sub: "But not everything I can do." },
    { chapter: "ACT III: LAST STAND", p2Sub: "Look around. You have nothing left.", p1Sub: "I still have my anger." },
    { chapter: "EPISODE 4: THE REMATCH", p2Sub: "You came back just to die again?", p1Sub: "I came back to bury you." },
    { chapter: "FINAL CHAPTER: ENDGAME", p2Sub: "Know your place in this world.", p1Sub: "I'm here to tear your world apart." },
    { chapter: "ACT II: THE ANOMALY", p2Sub: "What kind of monster are you?!", p1Sub: "I am the glitch in your system." },
    { chapter: "EPILOGUE: VENGEANCE", p2Sub: "I don't even remember your name.", p1Sub: "You will scream it today." }
];
...
window.generateIntroParams = function() {
    ...
    let lore = r(LORES);
    return { themeP1: t1, themeP2: t2, lore: lore };
};
...
window.generateLiveChatEvent = function() {
    ...
    const tmpls = [() => `THE LORE IS INSANE 😭`, () => `Bro has anime protagonist aura ☢️`, () => `Absolute cinema 🍿🎬`, () => `Nah this edit is too clean 🔥`, () => `RIP BOZO 📉`];
...
window.bakeThumbnailsForVideo = function(titleText) {
    ...
    try {
        ...
        ctxV.translate(1080*0.7, 1920*0.6); ctxV.rotate((Math.random()-0.5)*0.5); ctxV.font = "italic 900 80px Impact"; ctxV.textAlign = "center"; ctxV.lineWidth = 18; ctxV.strokeStyle = "#000"; ctxV.strokeText(prm.lore.chapter, 0,0);
...
// ====================================================================
// ĐẠO DIỄN: THE CINEMA EXPERIENCE (CẤU TRÚC 3 HỒI HOLLYWOOD)
// ====================================================================
window.drawProceduralIntro = function(ctx, w, h, progress) {
    ...
    // PHÂN CHIA TIMELINE 4 GIÂY THÀNH 5 ACTS
    // progress từ 0.0 đến 1.0 (tương đương 0s -> 4s)
    let act = 0; let localProg = 0;
    if (progress < 0.12) { act = 0; localProg = progress / 0.12; } // 0 -> 480ms: Màn hình đen Chapter
    else if (progress < 0.45) { act = 1; localProg = (progress - 0.12) / 0.33; } // 480ms -> 1800ms: Boss Lore
    else if (progress < 0.78) { act = 2; localProg = (progress - 0.45) / 0.33; } // 1800ms -> 3120ms: Player Lore
    else if (progress < 0.85) { act = 3; localProg = (progress - 0.78) / 0.07; } // 3120ms -> 3400ms: Blackout Tension
    else { act = 4; localProg = (progress - 0.85) / 0.15; } // 3400ms -> 4000ms: The Clash Explosion

    // HỆ THỐNG ÂM THANH CINEMATIC
    if (!window._sfxCuts[act]) {
        if (act === 0 && typeof window.playSound === 'function') window.playSound(80, 'sine', 0.5, 1.0); // Deep Bass Drop
        if (act === 1 && typeof window.playSound === 'function') window.playSound(200, 'triangle', 0.1, 1.0); // Whoosh vào cảnh
        if (act === 2 && typeof window.playSound === 'function') window.playSound(300, 'triangle', 0.1, 1.0); // Whoosh đổi cảnh
        if (act === 3 && typeof window.playSound === 'function') window.playSound(50, 'square', 0.3, 0.2); // Tiếng nhịp tim / Căng thẳng
        if (act === 4 && typeof window.playSound === 'function') window.playSound(100, 'sawtooth', 0.8, 1.5, true); // VỤ NỔ
        window._sfxCuts[act] = true;
    }

    // --- ACT 0: THE CHAPTER CARD ---
    if (act === 0) {
        ctx.fillStyle = "#000000"; ctx.fillRect(0, 0, w, h);
        ctx.textAlign = "center"; ctx.textBaseline = "middle";
        // Chữ từ từ hiện lên
        ctx.globalAlpha = Math.min(1, localProg * 2);
        ctx.fillStyle = prm.themeP1.aura; ctx.font = "italic 900 60px 'Arial Black', sans-serif"; ctx.letterSpacing = "5px";
        ctx.fillText(prm.lore.chapter, w/2, h/2);
        
        ctx.globalAlpha = Math.min(1, Math.max(0, (localProg - 0.5) * 2));
        ctx.fillStyle = "#fff"; ctx.font = "bold 30px Arial"; ctx.letterSpacing = "2px";
        ctx.fillText("A CINEMATIC EXPERIENCE", w/2, h/2 + 70);
        ctx.restore(); window.ctx = originalCtx; return;
    }

    // --- ACT 3: THE BLACKOUT TENSION (SỰ TĨNH LẶNG TRƯỚC CƠN BÃO) ---
    if (act === 3) {
        // Màn hình chớp đỏ 1 frame rồi đen kịt
        if (localProg < 0.1) {
            ctx.fillStyle = prm.themeP2.aura; ctx.fillRect(0,0,w,h);
        } else {
            ctx.fillStyle = "#020005"; ctx.fillRect(0,0,w,h);
            // Có viền đen chớp nháy tạo căng thẳng tột độ
            if (Math.floor(localProg * 30) % 2 === 0) {
                ctx.strokeStyle = prm.themeP1.aura; ctx.lineWidth = 10; ctx.strokeRect(0,0,w,h);
            }
        }
        ctx.restore(); window.ctx = originalCtx; return;
    }

    // CÁC HÀM TIỆN ÍCH DÙNG CHO ACT 1, 2, 4
    const drawCinematicDust = (theme, opacity) => {
        ctx.save(); ctx.fillStyle = theme.aura; ctx.globalAlpha = opacity;
        for(let i=0; i<50; i++) {
            let px = (Math.sin(i*12 + Date.now()*0.0005)*w) - w/2;
            let py = (Math.cos(i*24 + Date.now()*0.001)*h) - h/2;
            ctx.beginPath(); ctx.arc(px, py, 1+Math.random()*4, 0, Math.PI*2); ctx.fill();
        }
        ctx.restore();
    };

    const drawGodRays = (theme) => {
        ctx.save(); ctx.globalAlpha = 0.15; ctx.globalCompositeOperation = "screen";
        let rayGrad = ctx.createLinearGradient(-w/2, -h/2, w/2, h/2);
        rayGrad.addColorStop(0, theme.aura); rayGrad.addColorStop(0.5, "rgba(255,255,255,0)");
        ctx.fillStyle = rayGrad; ctx.fillRect(-w, -h, w*2, h*2);
        ctx.restore();
    };

    // --- SETUP CAMERA CHO ACT 1, 2, 4 ---
    ctx.save();
    let cutShake = 0; if (act === 4 && localProg < 0.3) cutShake = (Math.random() - 0.5) * 80; 

    // Cinematic Pan (Lướt camera chậm từ trái sang phải hoặc ngược lại)
    let panDirection = act === 1 ? 1 : -1;
    let panX = (localProg * 80) * panDirection - (40 * panDirection); 
    if (act === 4) panX = 0; // Đứng yên lúc va chạm
    
    // Zoom chậm rãi vào nhân vật
    let currentZoom = act < 4 ? (4.0 + localProg * 0.5) : (3.0 + localProg * 3); 
    
    ctx.translate(w/2 + cutShake + panX, h/2 + cutShake);
    ctx.scale(currentZoom, currentZoom);
    
    // Tiêu điểm nhân vật
    if (act === 1) ctx.translate(0, 100); // Focus Boss
    if (act === 2) ctx.translate(0, 100); // Focus Player
    if (act === 4) ctx.translate(0, 100); // Focus Clash

    // VẼ NỀN 
    let bgTheme = act === 1 ? prm.themeP2 : prm.themeP1;
    ...
    drawGodRays(bgTheme);
    drawCinematicDust(bgTheme, 0.4);

    // HÀM VẼ NHÂN VẬT THỰC TẾ
    const drawFighterArtistic = (charObj, cx, cy, forcedState, isFacingRight, theme, charName) => {
        if(!charObj) return; 
        ctx.save(); ctx.translate(cx, cy); 
        
        // VẼ TÊN CHUẨN IN-GAME LÚC GIỚI THIỆU (Chỉ hiện trong Act 1, 2)
        if (charName && act < 3) {
            ctx.save();
            ctx.translate(0, -280); 
            ctx.scale(0.8, 0.8); // Thu nhỏ lại chút cho tinh tế
            ctx.textAlign = "center"; ctx.textBaseline = "middle";
            ctx.font = "900 32px 'Arial Black', sans-serif"; 
            ctx.shadowColor = theme.aura; ctx.shadowBlur = 15;
            ctx.lineWidth = 6; ctx.strokeStyle = "#000"; 
            ctx.strokeText(charName, 0, 0); ctx.shadowBlur = 0; ctx.fillStyle = "#fff"; ctx.fillText(charName, 0, 0);
            ctx.restore();
        }
        ...
        ctx.filter = "contrast(115%) saturate(110%) drop-shadow(0 20px 20px rgba(0,0,0,0.8))"; // Darker shadow for depth

        // Vẽ Aura sáng rực
        ctx.save(); ctx.globalAlpha = 0.5 + Math.sin(Date.now()*0.01)*0.1; 
        ctx.shadowBlur = 50; ctx.shadowColor = theme.aura; ctx.scale(1.1, 1.1);
    ...
    else if (act === 4) { // THE CLASH
        let easeIn = Math.pow(localProg, 3);
        let pushP1 = -300 + (easeIn * 250); 
        let pushP2 = 300 - (easeIn * 250);
        drawFighterArtistic(e1, pushP2, 150, 'attack', false, prm.themeP2, null);
        drawFighterArtistic(window.p1, pushP1, 150, 'attack', true, prm.themeP1, null);

        // TIA SÁNG XÉ NGANG LÚC VA CHẠM (ANAMORPHIC LENS FLARE)
        let flareWidth = w * 3 * localProg;
        let flareHeight = 25 + Math.random() * 50;
        ctx.globalCompositeOperation = "screen";
        ctx.fillStyle = prm.themeP1.aura; ctx.fillRect(-flareWidth/2, 100 - flareHeight/2, flareWidth, flareHeight);
        ctx.fillStyle = "#ffffff"; ctx.fillRect(-flareWidth/2, 100 - flareHeight/4, flareWidth, flareHeight/2);
        
        // SPARKS VĂNG TUNG TÓE KHI CHẠM
        ctx.translate(0, 150);
        for(let i=0; i<30; i++) {
            let ang = Math.random() * Math.PI * 2;
            let dist = localProg * 800 * Math.random();
            ctx.fillStyle = Math.random() > 0.5 ? "#fff" : prm.themeP1.aura;
            ctx.globalAlpha = 1 - localProg;
            ctx.fillRect(Math.cos(ang)*dist, Math.sin(ang)*dist, 20*(1-localProg), 4);
        }
        ctx.globalAlpha = 1.0;
    ...
    // --- CINEMATIC VIGNETTE (Viền đen mờ siêu tối góc màn hình) ---
    let vigGrad = ctx.createRadialGradient(w/2, h/2, h*0.25, w/2, h/2, h*0.85);
    vigGrad.addColorStop(0, "rgba(0,0,0,0)"); vigGrad.addColorStop(1, "rgba(0,0,0,0.95)");
    ctx.fillStyle = vigGrad; ctx.fillRect(0, 0, w, h);

    // --- CINEMATIC LETTERBOX (VIỀN ĐEN CHIẾU RẠP) ---
    let barHeight = 250; 
    if (act === 4) barHeight = 0; // Biến mất tức khắc lúc nổ
    ...
    // --- CINEMATIC SUBTITLES (KỂ CHUYỆN) ---
    if (act === 1 || act === 2) {
        ctx.save();
        let subText = act === 1 ? prm.lore.p2Sub : prm.lore.p1Sub;
        // Fade in chữ từ từ
        ctx.globalAlpha = Math.min(1, localProg * 4); 
        ctx.textAlign = "center"; ctx.textBaseline = "bottom";
        
        // Font chuẩn phim chiếu rạp
        ctx.font = "italic 40px 'Arial', sans-serif"; ctx.letterSpacing = "1px";
        let subY = h - barHeight + 90; // Kéo đè lên viền đen chút
        
        ctx.lineWidth = 5; ctx.strokeStyle = "#000"; ctx.strokeText(`"${subText}"`, w/2, subY);
        ctx.fillStyle = "#e5e7eb"; // Màu trắng xám chuẩn Netflix sub
        ctx.fillText(`"${subText}"`, w/2, subY);
        ctx.restore();
    }

    // --- BÙM! CHUYỂN CẢNH KẾT THÚC (ACT 4) ---
    if (act === 4) {
        let flashP = localProg;
...
// ====================================================================
// HUD VÀ THANH TỶ LỆ WIN ĐÃ ĐƯỢC LÀM SẠCH (ZERO TEXT HUD)
// ====================================================================
