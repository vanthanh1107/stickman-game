...
// Tối ưu Layout: Clean Game -> HUD -> WHO WILL WIN POLL -> Smart Alerts -> Live Chat -> Green Screen Cat 
// ==========================================
...
window.startRecording = function() {
    ...
    window.videoTrackH = window.videoTrackH || videoStreamH.getVideoTracks()[0];
    window.videoTrackV = window.videoTrackV || videoStreamV.getVideoTracks()[0];
...
window.captureFrames = function() {
    ...
    if (!isOutroActive) {
        ...
        let p1Hp = 0.5, p2Hp = 0.5; // Mặc định 50/50 nếu chưa tính được
        ...
        if (window.p1) {
            ...
            let hudBaseY = splitGameHeight + 90; // ~697px
        ...
        if (window.p1 && repEnemyObj) {
            ctxV.save();
            let pollY = splitGameHeight + 280; // Trọng tâm của Thanh bình chọn (~887)
            let pollWidth = 960;
            let pollX = 540;
            
            ctxV.translate(pollX, pollY);
            
            // Dòng chữ kích thích Comment nhấp nháy
            let pulseText = 1 + Math.sin(Date.now() / 150) * 0.05;
            ctxV.save();
            ctxV.scale(pulseText, pulseText);
            ctxV.fillStyle = "#ffeb3b";
            ctxV.font = "900 28px 'Arial Black'";
            ctxV.textAlign = "center";
            ctxV.textBaseline = "bottom";
            ctxV.shadowColor = "#ffeb3b"; ctxV.shadowBlur = 10;
            ctxV.fillText("👇 COMMENT 'WHO WILL WIN' TO UNLOCK NEW CHARACTERS 👇", 0, -25);
            ctxV.restore();
            ...
            // Vẽ nền thanh (Màu Đỏ cho Boss)
            ctxV.fillStyle = "#ff0055";
            if(ctxV.roundRect) { ctxV.beginPath(); ctxV.roundRect(-pollWidth/2, 0, pollWidth, 40, 20); ctxV.fill(); }
            else { ctxV.fillRect(-pollWidth/2, 0, pollWidth, 40); }
            
            // Vẽ phần Xanh cho Player đè lên
            ctxV.save();
            if(ctxV.roundRect) { ctxV.beginPath(); ctxV.roundRect(-pollWidth/2, 0, pollWidth, 40, 20); ctxV.clip(); }
            ctxV.fillStyle = "#00f3ff";
            ctxV.fillRect(-pollWidth/2, 0, pollWidth * p1Pct, 40);
            ctxV.restore();
            
            // Viền thanh
            ctxV.strokeStyle = "rgba(255,255,255,0.6)";
            ctxV.lineWidth = 4;
            if(ctxV.roundRect) { ctxV.beginPath(); ctxV.roundRect(-pollWidth/2, 0, pollWidth, 40, 20); ctxV.stroke(); }
            else { ctxV.strokeRect(-pollWidth/2, 0, pollWidth, 40); }

            // Logo VS ở giữa
            ctxV.fillStyle = "#1e293b";
            ctxV.beginPath(); ctxV.arc(-pollWidth/2 + pollWidth * p1Pct, 20, 25, 0, Math.PI*2); ctxV.fill();
            ctxV.lineWidth = 3; ctxV.strokeStyle = "#fff"; ctxV.stroke();
            ctxV.fillStyle = "#fff";
            ctxV.font = "900 20px 'Arial Black'"; ctxV.textAlign = "center"; ctxV.textBaseline = "middle";
            ctxV.fillText("VS", -pollWidth/2 + pollWidth * p1Pct, 20);
            
            // Text hiển thị %
            ctxV.fillStyle = "#000";
            ctxV.font = "900 22px 'Arial Black'";
            ctxV.textAlign = "left"; ctxV.fillText(`[1] PLAYER: ${Math.round(p1Pct*100)}%`, -pollWidth/2 + 20, 20);
            ctxV.textAlign = "right"; ctxV.fillText(`${Math.round((1-p1Pct)*100)}% :BOSS [2]`, pollWidth/2 - 20, 20);
        ...
        // --- [SỬA LỖI] THÔNG BÁO LIVE ALERTS (HIỂN THỊ TRỌN VẸN Ở GIỮA HUD & CHAT) ---
        ...
        // Dời Thông Báo trôi lên từ Y = 840 (Ngay trên thanh Who Will Win, không đè máu)
        let alertStartY = splitGameHeight + 200; 
        ...
        let boxHeight = 440;
        let boxX = 540; 
        let boxY = 1180; // Dời bảng chat thấp xuống một chút để chừa chỗ cho Who Will Win Poll
