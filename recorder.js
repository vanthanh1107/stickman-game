...
window.captureFrameTo1080p = function() {
    ...
    ctx.fillStyle = vignette; ctx.fillRect(0, 60, 1920, 960);

    if (window.p1 && !window.gameOver && window.introTimer <= 120) {
        ctx.lineJoin = "round";
        const drawSkewedPath = (x, y, w, h, isLeft) => {
            ctx.beginPath();
            if (isLeft) { ctx.moveTo(x, y); ctx.lineTo(x + w, y); ctx.lineTo(x + w - 25, y + h); ctx.lineTo(x - 25, y + h); } 
            else { ctx.moveTo(x + 25, y); ctx.lineTo(x + w + 25, y); ctx.lineTo(x + w, y + h); ctx.lineTo(x, y + h); }
            ctx.closePath();
        };
        ...
        if (window.enemies && window.enemies.length > 0) {
            ...
            let p2Hp = Math.max(0, eHp / eMax); let isBoss = window.enemies[0].isDragon; let eStam = Math.max(0, window.enemies[0].stamina / 100);
