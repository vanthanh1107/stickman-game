...
// ĐÃ THÊM: HỆ THỐNG LIVE PREVIEW NHÂN VẬT TẠI MENU KÈM 30 SIÊU KỸ NĂNG
// ==========================================
...
window.startPreviewLoop = function(charStats) {
    // Ngừng vòng lặp cũ nếu có để tránh giật lag khi click nhiều tướng liên tục
    if (window.previewAnimId) cancelAnimationFrame(window.previewAnimId);

    window.previewFighter = {
        x: 0, y: 0,
        scale: (charStats.scale || 1) * 1.5, // Phóng to nhân vật để nhìn cho rõ
        color: charStats.color || "#fff",
        state: 'idle',
        attackTimer: 0,
        previewWaitTimer: 30, // Thời gian đứng thở nghỉ ngơi giữa các điệu múa
        isFacingRight: true,
        onGround: true,
        hp: 100, maxHp: 100,
        iFrames: 0,
...
window.previewLoop = function() {
    ...
    // Xóa sạch khung hình cũ trước khi vẽ khung mới
    ctx.clearRect(0, 0, canvas.width, canvas.height);

    let p = window.previewFighter;
    if (!p) return;

    // 🌟 THUẬT TOÁN ĐIỀU PHỐI NHỊP ĐỘ MÚA
    if (p.attackTimer > 0) {
        p.attackTimer--; // Giảm thời gian múa hiện tại

        // Khi vừa múa xong, cho nhân vật đứng thu tay về nghỉ ngơi một chút
        if (p.attackTimer === 0 && p.state !== 'idle' && p.state !== 'walk') {
            p.state = 'idle';
            p.previewWaitTimer = Math.floor(Math.random() * 50) + 30; // Nghỉ từ 0.5 đến 1.5 giây
        }
    }
    else if (p.previewWaitTimer > 0) {
        // Đang trong nhịp nghỉ, đếm lùi thời gian nghỉ
        p.previewWaitTimer--;
    }
    else {
        // 🌟 BỐC THĂM ĐIỆU MÚA: KHI ĐÃ NGHỈ XONG, GỌI RA 1 TRONG 30 CHIÊU THỨC
        let coolMoves = [
            'idle', 'walk', 'taunt_crane', 'taunt_power', 'taunt_dance', 'taunt_flex',
            // Nhóm Meme & Vũ đạo
            't_pose', 'dab', 'floss', 'siuuuu', 'smooth_criminal', 'salt_bae',
            'gangnam_style', 'robot_dance', 'zombie', 'rickroll', 'breakdance', 'moonwalk',
            // Nhóm Anime
            'super_saiyan', 'jojo_dio', 'jojo_jotaro', 'gear_second', 'goku_teleport',
            'sasageyo', 'yamcha_death', 'kame_charge', 'kame_fire', 'fusion_dance', 'naruto_run',
            // Nhóm Game & Điện ảnh
            'ironman_landing', 'spiderman_shoot', 'wolverine_pose', 'matrix_dodge',
            'hadouken', 'shoryuken', 'tatsumaki', 'levitate', 'praise_the_sun'
        ];

        // Lấy ngẫu nhiên 1 tên trong danh sách trên gán vào trạng thái của nhân vật
        p.state = coolMoves[Math.floor(Math.random() * coolMoves.length)];

        // Đặt thời gian biểu diễn cho từng điệu nhảy
        if (['moonwalk', 'breakdance', 'gangnam_style', 'robot_dance', 'floss', 'naruto_run'].includes(p.state)) {
            p.attackTimer = 160;
        }
        else if (['kame_charge', 'kame_fire', 'super_saiyan', 'jojo_dio', 'jojo_jotaro', 'sasageyo', 'siuuuu', 'fusion_dance'].includes(p.state)) {
            p.attackTimer = 100;
        }
        else if (['yamcha_death', 't_pose', 'ironman_landing', 'wolverine_pose', 'levitate', 'praise_the_sun', 'smooth_criminal'].includes(p.state)) {
            p.attackTimer = 80;
        }
        else {
            p.attackTimer = 50;
    ...
    ctx.save();
    // Đặt trục tọa độ vẽ vào giữa bệ đứng
    ctx.translate(canvas.width / 2, canvas.height - 40);

    // Gọi trạm đồ họa (graphic.js) để vẽ nhân vật
