// ==========================================
// MAIN.JS - BẢN NÂNG CẤP CINEMATIC ULTIMATE 4.0 (ANIME CUT-IN)
// ĐÃ TỐI ƯU HÓA: ĐỒNG BỘ HOÀN TOÀN VỚI ENGINE 9.0 ASYNC/AWAIT
// ĐÃ THÊM: HIỆU ỨNG ANIME CUT-IN, COMBO PREVIEW, DYNAMIC THEMING
// ==========================================
...
window.renderCharacterGrid = function() {
    ...
    for (let id in window.classStats) {
        ...
        card.onclick = async () => { 
            ...
            if(desc) desc.innerHTML = `<span>⏳ Đang tải dữ liệu Tướng...</span>`;
            
            await window.loadCharacterDynamic(id);
            let activeItem = window.classStats[id];
            
            // Đổi màu nền (Glow) của bục Preview theo màu tướng
            let pedestal = document.querySelector('.pedestal-glow');
            if (pedestal && activeItem.color) {
                pedestal.style.background = activeItem.color;
                pedestal.style.boxShadow = `0 0 25px 5px ${activeItem.color}88`;
            }

            if(desc) desc.innerHTML = `
                <div style="display:flex; flex-direction:column; gap:10px; text-align: left;">
                    <span style="font-size:26px; color:${activeItem.color || '#f1c40f'}; font-weight:900; text-transform: uppercase; text-shadow: 0 0 10px ${activeItem.color}66;">${activeItem.className}</span>
                    <span style="color:#fff;">❤️ Sinh Lực: <strong style="color:#ff4757;">${activeItem.hp || 100}</strong></span>
...
window.initBGM = function() {
    ...
    let playPromise1 = window.bgmBase.play();
    let playPromise2 = window.bgmClimax.play();
    
    // Xử lý lỗi trình duyệt chặn autoplay nhạc
    if (playPromise1 !== undefined) playPromise1.catch(e => console.warn("Trình duyệt chặn BGM Base."));
    if (playPromise2 !== undefined) playPromise2.catch(e => console.warn("Trình duyệt chặn BGM Climax."));
}
...
window.matchStart = async function() {
    try {
        ...
        window.p1 = { 
            ...
            introState: tauntList[Math.floor(Math.random() * tauntList.length)], isCinematic: false
        };
        ...
        // 🌟 BẢN VÁ QUAN TRỌNG: Không bọc `update` bằng DOM manipulation nữa. 
        // Bỏ logic `transform: scale(1.7)` cồng kềnh. Mọi thứ giao cho Engine 9.0 xử lý mượt mà.
        if (!window.updateHooked && typeof window.update === 'function') {
            window.originalEngineUpdate = window.update;
            window.update = function() {
                // Nếu Time Stop hoặc Kịch bản đang chạy, không làm gì cả, cứ gọi hàm gốc (Engine tự lo)
...
window.resetMatchVariables = function() { 
    ...
    window.asyncTasks = []; // Xóa rác kịch bản tồn đọng
    window.resetCamera();   // Đặt lại Camera từ Engine 9.0
    
    // Gỡ các DOM cặn của Anime Cut-in nếu còn sót
    let oldCutIn = document.getElementById("anime-cut-in");
...
window.bindAttackEvent = function() { 
    if (!window.attackBound) { 
        ...
        let triggerAttack = function(e) { 
            ...
            e.preventDefault(); 
            // Nếu người chơi đang Cinematic (isCinematic) -> cấm đánh
            if (!window.gameOver && window.p1 && window.introTimer <= 0 && window.p1.attackTimer === 0 && window.p1.hitStun === 0 && window.p1.stunTimer === 0 && !window.p1.isCinematic) { 
...
window.checkGameOver = function() {
    ...
    if (window.p1 && (window.p1.hp <= 0 || allDead)) {
        ...
        let winnerText = (window.p1.hp > 0) ? "🏆 K.O FATALITY!" : "💀 GAME OVER!"; 
        let winnerColor = (window.p1.hp > 0) ? "#f1c40f" : "#ff4757";
        ...
        window.floatingTexts.push({ 
            ...
            text: winnerText, 
            color: winnerColor, 
            alpha: 1, vx: 0, vy: -0.5, font: "900 70px 'Teko', sans-serif", life: 180 
        });
        
        window.applyFilter(window.p1.hp > 0 ? 'dark' : 'blood', 60);
...
window.updateHPUIs = function() {
    ...
    // Nếu UI Bar máu bị đánh tụt, giật nhẹ thanh máu
    if (window.p1.hp < window.p1.maxHp && window.p1.hitStun > 0) window.uiShakeP1 = 5;
    if (currentEnemyHp < window.totalEnemyMaxHp && window.enemies[0] && window.enemies[0].hitStun > 0) window.uiShakeP2 = 5;
...
window.playerBlock = function() {
    if (!window.p1 || window.p1.hp <= 0 || window.gameOver || window.introTimer > 0 || window.p1.isCinematic) return;
...
// 🌟 HIỆU ỨNG ANIME CUT-IN
window.showAnimeCutIn = function(name, color) {
    let gScreen = document.getElementById("game-screen");
    if (!gScreen) return;

    let cutInBox = document.createElement("div");
    cutInBox.id = "anime-cut-in";
    cutInBox.style.cssText = `
        position: absolute; top: 35%; left: -100%; width: 200%; height: 120px;
    ...
    textNode.style.cssText = `
        font-family: 'Teko', sans-serif; font-size: 80px; font-weight: 900; color: #fff;
        text-shadow: 0 0 20px ${color}, 2px 2px 0 #000; font-style: italic; letter-spacing: 5px;
    `;
    
    cutInBox.appendChild(textNode);
    gScreen.appendChild(cutInBox);

    // Chớp màn hình trắng mỏng
    window.applyFilter('invert', 10);
    
    // Lướt vào giữa
    setTimeout(() => { cutInBox.style.left = "-50%"; }, 10);
    
    // Lướt ra và xóa
    setTimeout(() => {
        cutInBox.style.left = "100%";
        setTimeout(() => { if(cutInBox.parentNode) cutInBox.remove(); }, 400);
    }, 800);
};

// 🌟 XỬ LÝ CHIÊU CUỐI BẰNG LÕI ENGINE 9.0 KẾT HỢP CUT-IN
window.useUltimate = function(caster, target) {
    if (!caster || caster.hp <= 0 || window.gameOver || caster.isCinematic) return;
    if (caster.hitStun > 0 || caster.stunTimer > 0 || !target || target.hp <= 0) return;

    // Reset Stamina và Cấm hành động
    caster.stamina = 0; 
    ...
    // Gọi UI Anime Cut-in (Chỉ cho Player để tránh rối màn hình)
    if (caster.isPlayer) {
        window.showAnimeCutIn(caster.className || "ULTIMATE", caster.color || "#ff003c");
    }

    // Bàn giao kịch bản cho Engine 9.0 thực thi (Data-driven)
    if (charDef && typeof charDef.executeUltimate === 'function') {
        (async () => {
            try {
                // Trả quyền kiểm soát kịch bản hoàn toàn cho file Tướng
                await charDef.executeUltimate(caster, target, baseDmg);
            } catch(e) { 
                console.error("Lỗi kịch bản Ultimate:", e); 
            } finally {
                // Đảm bảo xả khóa sau khi Skill kết thúc
                window.isCinematicActive = false;
        ...
        // Fallback: Nếu tướng chưa có kịch bản riêng, đánh cú đấm thường siêu mạnh
        caster.state = 'dragon_uppercut'; 
        caster.attackTimer = 35;
        caster.vx = caster.isFacingRight ? 15 : -15;
        window.isCinematicActive = false;
    }
}
...
// ==========================================
// HỆ THỐNG LIVE PREVIEW (SHADOWBOXING COMBO) TẠI MENU
// ==========================================
...
window.startPreviewLoop = function(charStats) {
    ...
    window.previewFighter = {
        ...
        comboQueue: [] // Mảng chứa chuỗi đòn
    };
...
window.previewLoop = function() {
    ...
    // AI SHADOWBOXING (Đánh Combo)
    if (p.attackTimer > 0) {
        p.attackTimer--;
    } else {
        // Nếu đang có hàng đợi combo, lấy đòn tiếp theo ra đánh
        if (p.comboQueue && p.comboQueue.length > 0) {
            let nextMove = p.comboQueue.shift();
            p.state = nextMove;
            p.attackTimer = 18; // Thời gian frame đòn đánh
        } else {
            // Ngẫu nhiên nạp 1 chuỗi combo mới
            let rand = Math.random();
            if (rand < 0.2) p.comboQueue = ['jab', 'cross', 'uppercut'];
            else if (rand < 0.4) p.comboQueue = ['low_kick', 'high_kick', 'spinning_heel'];
            else if (rand < 0.6) p.comboQueue = ['taunt_power'];
            else if (rand < 0.8) p.comboQueue = ['cast', 'dash_back'];
            else p.comboQueue = ['idle'];
            
            p.state = 'idle';
            p.attackTimer = 60 + Math.random() * 40; // Đứng nghỉ lấy hơi
        }
    }
    ...
    // Gọi engine vẽ (Trỏ tới Engine Graphic 3.0)
