// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 39.0 [AAA CINEMATIC BEAUTY]
// [RESTORED] Giữ nguyên 100% logic V38.0 (AI, Weather, 360 Matrix, Black Hole...).
// [NEW] Supernova Flash - Vụ nổ ánh sáng siêu tân tinh quét ngang không gian.
// [NEW] Camera Lens Dirt - Hiệu ứng bụi bám trên lăng kính camera khi ánh sáng bùng nổ.
// [NEW] 3D Glowing Floor Craters - Sàn nhà bị nứt 3D với dung nham phát sáng bên dưới.
// [ENHANCED] Hyper-Ghost Trails - Tàn ảnh tốc độ cao phát sáng rực rỡ.
// ==========================================
...
// [EPIC UPGRADE 37.0 - 39.0] NEW VFX ARRAYS
window.lightningArcs = []; window.energyPillars = []; window.blackHoles = [];
window.bokehs = []; window.spaceRipples = []; window.lensDirt = []; window.supernova = null;
...
// [AAA UPGRADE 39.0] Camera Lens Dirt
window.spawnLensDirt = function() {
    for(let i=0; i<15; i++) { window.lensDirt.push({ x: Math.random() * window.canvas.width, y: Math.random() * window.canvas.height, r: Math.random() * 40 + 10, alpha: Math.random() * 0.5 + 0.3 }); }
}
...
window.spawnSlash = function(x, y, isRight, color, isCrit, scale, rotation = 0) { 
    ...
    for(let i=0; i< (isCrit ? 15 : 8); i++) { window.impactSparks.push({ x: x, y: y, vx: (isRight ? 1 : -1)*(Math.random()*15+5), vy: (Math.random()-0.5)*15, life: 20 + Math.random()*15, maxLife: 35, color: color, scale: Math.random() * 1.5 }); }
}

window.spawnParticles = function(x, y, color, isCrit = false) { if (window.particles.length > MAX_PARTICLES) return; let count = isCrit ? 35 : 15; for(let i=0; i<count; i++) { let angle = Math.random() * Math.PI * 2; let speed = Math.random() * (isCrit?25:12) + 2; window.particles.push({ x: x, y: y - 30, vx: Math.cos(angle)*speed, vy: Math.sin(angle)*speed, life: 35, maxLife: 35, color: color, size: Math.random() * 6 + 2 }); } }
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce, attackerDirRight = true) {
    ...
    if (finalDmg > 0) {
        ...
        if (isCrit) {
            window.screenFlash = 0.5; window.spawnLensDirt(); 
            window.impactFrameCount = 2; 
            ...
            // [AAA UPGRADE 39.0] Nổ Supernova nhỏ
            window.supernova = { r: 0, a: 0.8, color: color || "#f1c40f" };
        ...
        if (target.hp <= 0) {
            ...
            if (isNearWall && isCrit && typeof window.triggerStageTransition === 'function') { window.triggerStageTransition(target); } 
                ...
                if (isCrit) {
                    window.screenFlash = 0.6; window.spawnLensDirt();
                    ...
                    window.lethalVoid = 100; window.impactAberration = 50; window.spawnForegroundDebris(target.x, target.y, attackerDirRight); window.spawnForegroundDebris(target.x, target.y, attackerDirRight); 
                    ...
                    // [AAA UPGRADE 39.0] Nổ Supernova TOÀN BẢN ĐỒ
                    window.supernova = { r: 0, a: 1.5, color: "#ff003c" };
...
window.update = function() {
    ...
    // [EPIC UPGRADE 37.0 - 39.0] Cập nhật mảng VFX mới
    ...
    // Animate Lens Dirt
    window.lensDirt.forEach(d => d.alpha -= 0.015); window.lensDirt = window.lensDirt.filter(d => d.alpha > 0);
    // Animate Supernova
    if (window.supernova) { window.supernova.r += 120; window.supernova.a -= 0.025; if (window.supernova.a <= 0) window.supernova = null; }
    ...
    if (Math.random() < 0.15 && window.bokehs.length < 35 && window.lethalVoid <= 0) {
        window.bokehs.push({
            ...
            life: 250, maxLife: 250, size: Math.random()*20+15, z: Math.random()*2 + 1.5, color: Math.random() > 0.6 ? "#f1c40f" : "#00f3ff"
        });
    }
    ...
    if (!isTimeStopped) {
        ...
        for (let i = window.envHazards.length - 1; i >= 0; i--) {
            ...
            if (haz.type === 'lightning') {
                if (haz.state === 'warning' && haz.timer <= 0) { haz.state = 'striking'; haz.timer = 12; window.playSound(300, 'sawtooth', 0.8, 0.8, true); window.screenFlash = 0.8; window.spawnLensDirt(); window.shakeScreen(20, 15); window.spawnEnvDamage(haz.x, window.GROUND_Y, 'crater', 1.2, false); 
            ...
            if (f.hitStun <= 0 && f.stunTimer <= 0 && f.state !== 'dash_back' && f.state !== 'block') {
                ...
                if (isCloseEnough) {
                    if (window.classStats && window.classStats[type] && typeof window.classStats[type].executeUltimate === 'function') {
                        ...
                        // [AAA UPGRADE 39.0] Nổ Supernova khi dùng Ulti
                        window.supernova = { r: 0, a: 0.8, color: f.isPlayer ? "#ff4757" : "#9b59b6" }; window.spawnLensDirt();
            ...
            for (let j = i + 1; j < allFighters.length; j++) { 
                ...
                if (dist < 80 && f1.attackTimer > 5 && f1.attackTimer < 20 && f2.attackTimer > 5 && f2.attackTimer < 20) {
                    if (!f1.clashCooldown && !f2.clashCooldown && f1.isFacingRight !== f2.isFacingRight) {
                        ...
                        window.targetZoom = 1.6; window.screenFlash = 0.5; window.spawnLensDirt();
...
window.draw = function() {
    ...
    if (window.isLoading) {
        window.ctx.fillStyle = "#050505"; window.ctx.fillRect(0, 0, window.canvas.width, window.canvas.height); window.ctx.strokeStyle = "rgba(0, 243, 255, 0.05)"; window.ctx.lineWidth = 1;
        ...
        window.ctx.fillStyle = "#fff"; window.ctx.font = "bold 16px monospace"; window.ctx.fillText(`RENDERING PERFECT FRAME... ${Math.floor(window.loadProgress)}%`, cx, cy + 60); return; 
    }
    ...
    try {
        ...
        // [AAA UPGRADE 39.0] VẼ VẾT NỨT SÀN 3D RỰC LỬA (GLOWING CRATERS)
        if (window.envDamage && window.envDamage.length > 0) {
            window.envDamage.forEach(dmg => {
                let pr = project3D(dmg);
                window.ctx.save(); 
                window.ctx.translate(pr.drawX, window.GROUND_Y); // Chốt cứng dưới sàn
                window.ctx.scale(1, 0.3); // Kéo bẹp tạo góc nhìn sàn 3D
                window.ctx.scale(pr.drawScale, pr.drawScale);
                
                window.ctx.lineWidth = 3; window.ctx.strokeStyle = "#000"; window.ctx.beginPath();
                dmg.cracks.forEach(crack => { window.ctx.moveTo(crack[0].x, crack[0].y); for(let i=1; i<crack.length; i++) window.ctx.lineTo(crack[i].x, crack[i].y); });
                window.ctx.stroke();
                
                if (dmg.isBurning) {
                    window.ctx.lineWidth = 2; window.ctx.strokeStyle = "#ff4757";
        ...
        if (window.p1) {
            // [AAA UPGRADE 39.0] HYPER GHOST TRAILS (Tàn ảnh tỏa sáng lõi plasma)
            ...
            sortedFighters.forEach(p => { 
                if (p && p.hp > 0 && p.trailArr) { 
                    p.trailArr.forEach(t => { 
                        ...
                        window.ctx.filter = p.isPlayer ? 'sepia(1) hue-rotate(180deg) saturate(400%) contrast(300%) blur(2px)' : 'sepia(1) hue-rotate(300deg) saturate(400%) contrast(300%) blur(2px)';
                        ...
                        // Vẽ lõi sáng rực rỡ bên trong bóng mờ
                        window.ctx.shadowBlur = 15; window.ctx.shadowColor = p.isPlayer ? "#ff4757" : "#9b59b6";
        ...
        // [AAA UPGRADE 39.0] VẼ VỤ NỔ SUPERNOVA SIÊU TÂN TINH (Tâm khung hình)
        if (window.supernova) {
            window.ctx.save();
            let pr = project3D({x: window.orbitFocusX, y: window.orbitFocusY});
            window.ctx.translate(pr.drawX, pr.drawY);
            window.ctx.globalCompositeOperation = 'screen';
            let snGrad = window.ctx.createRadialGradient(0,0,0, 0,0, window.supernova.r * pr.drawScale);
            snGrad.addColorStop(0, `rgba(255,255,255,${window.supernova.a})`);
            snGrad.addColorStop(0.2, window.supernova.color);
            snGrad.addColorStop(1, 'rgba(0,0,0,0)');
            window.ctx.fillStyle = snGrad;
            window.ctx.beginPath(); window.ctx.arc(0,0, window.supernova.r * pr.drawScale, 0, Math.PI*2); window.ctx.fill();
            window.ctx.restore();
        }

        window.ctx.restore(); // END MAIN TRANSFORM CAMERA 3D

        // VẼ BOKEH LỚP TIỀN CẢNH SIÊU MỜ
        if (window.bokehs.length > 0) {
            window.ctx.save(); window.ctx.globalCompositeOperation = 'screen';
            window.bokehs.forEach(b => {
                window.ctx.save();
                let prX = window.canvas.width/2 + (b.x - window.camX) * b.z; let prY = window.canvas.height/2 + (b.y - window.camY) * b.z;
                window.ctx.translate(prX, prY); window.ctx.globalAlpha = Math.min(1, b.life / 50) * 0.4; 
                window.ctx.fillStyle = b.color; window.ctx.filter = 'blur(10px)'; 
        ...
        // VIGNETTE FOCUS (Làm tối góc viền)
        window.ctx.save();
        ...
        vignette.addColorStop(0, "rgba(255, 255, 255, 1)"); vignette.addColorStop(1, "rgba(100, 100, 100, 1)"); 
        window.ctx.fillStyle = vignette; window.ctx.fillRect(0, 0, window.canvas.width, window.canvas.height);
        window.ctx.restore();

        // CHỚP MÀN HÌNH ĐƯỢC VẼ SAU CAMERA
        ...
        // [AAA UPGRADE 39.0] CAMERA LENS DIRT (Bụi quang học dính sát kính camera)
        if (window.lensDirt.length > 0) {
            window.ctx.save(); window.ctx.globalCompositeOperation = 'screen';
            window.lensDirt.forEach(d => {
                window.ctx.fillStyle = `rgba(255, 255, 255, ${d.alpha * 0.15})`; window.ctx.filter = 'blur(4px)';
                window.ctx.beginPath(); window.ctx.arc(d.x, d.y, d.r, 0, Math.PI*2); window.ctx.fill();
            });
            window.ctx.restore();
        }
