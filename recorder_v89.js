...
// Kèm theo SCROLLING CELEBRITY LIVE CHAT (1-3s/Comment - Tùy chỉnh tự do)
// ==========================================
...
// ====================================================================
// 🎯 KHU VỰC TÙY CHỈNH: NGƯỜI NỔI TIẾNG & BÌNH LUẬN (BẠN CÓ THỂ THÊM VÀO ĐÂY)
// ====================================================================

// 1. Danh sách tên người nổi tiếng và màu sắc của họ
window.CELEB_LIST = [
    ...
    { name: "PewDiePie 👊", color: "#ff0000" },
    { name: "Ninja 🥷", color: "#00a8ff" },
    { name: "Độ Mixi 🎧", color: "#ffb8b8" },
    { name: "Thầy Giáo Ba 👨‍🏫", color: "#f368e0" }
];

// 2. Danh sách hàng tá các bình luận (cứ thêm thoải mái)
window.TOXIC_COMMENTS = [
    ...
    "Imagine sweating this hard and still being trash 💦",
...
window.captureFrames = function() {
    ...
    // CHÍNH GIỮA: VTuber Commentator 
    if (!window.gameOver) { 
        drawVTuberCommentator(ctxV, 800, splitGameHeight + 200, audioPeak, window.StoryModeAI.isTyping); 
    }
    ...
    // ==========================================
    // 7. TOXIC SCROLLING CHAT BOX (HIỆN NGAY LẬP TỨC LÊN MÀN HÌNH)
    // ==========================================
    if (!window._chatSystemInit) {
        ...
        // Đổ sẵn 7 bình luận vào khung để lúc mới vô video không bị trống trải
        for(let i = 0; i < 7; i++) {
    ...
    // Luôn luôn chạy xuyên suốt video (kể cả lúc đang Intro hay Game Over)
    let chatNow = Date.now();
    
    // Cập nhật Comment mới
    if (chatNow - window._lastChatUpdate > window._nextChatDelay) {
        window._lastChatUpdate = chatNow;
        // Random ra một mốc thời gian từ 1 đến 3 giây cho comment tiếp theo
        ...
        // Chỉ giữ tối đa 7 dòng comment trên màn hình
        if (window._liveChats.length > 7) {
            window._liveChats.shift();
        }
    }

    // Vẽ khung Chat lên màn hình dọc (TikTok)
    ctxV.save();
    let boxWidth = 960;
    let boxHeight = 550;
    let boxX = 540; // Nằm chính giữa
    let boxY = splitGameHeight + 600; // Nằm ở phần nửa dưới màn hình

    ctxV.translate(boxX, boxY);
    
    // Nền kính mờ của Chat
    ctxV.fillStyle = "rgba(10, 15, 30, 0.8)";
    ...
    // Cắt (clip) để chữ không bị tràn ra ngoài khung
    ctxV.beginPath(); 
    if(ctxV.roundRect) ctxV.roundRect(-boxWidth/2, -boxHeight/2, boxWidth, boxHeight, 25); 
    ctxV.clip();
    ...
    ctxV.fillRect(-boxWidth/2, -boxHeight/2, boxWidth, 70);
    ctxV.fillStyle = "#fff"; 
    ...
    ctxV.fillText("👥 1.2M VIEWERS", boxWidth/2 - 30, -boxHeight/2 + 35);

    // Vẽ dòng chữ từ dưới lên trên
    let startTextY = boxHeight/2 - 30; // Điểm bắt đầu vẽ là sát dưới đáy khung
    let lineHeight = 65; 
    
    ctxV.textAlign = "left";
    ctxV.textBaseline = "bottom";
    
    for (let i = 0; i < window._liveChats.length; i++) {
        let chat = window._liveChats[i];
        let yPos = startTextY - ((window._liveChats.length - 1 - i) * lineHeight); 
        
        ctxV.font = "bold 35px Arial";
        
        // Vẽ Tên Celeb
        ctxV.fillStyle = chat.color;
        ctxV.fillText(chat.name + ":", -boxWidth/2 + 30, yPos);
        
        // Vẽ Nội Dung Toxic
        let nameWidth = ctxV.measureText(chat.name + ":").width;
        ctxV.fillStyle = "#ffffff";
        ctxV.fillText(" " + chat.msg, -boxWidth/2 + 30 + nameWidth, yPos);
    }
    
    // Viền chớp chớp khi căng thẳng
    if (audioPeak > 0.6) {
        ctxV.strokeStyle = "rgba(255, 0, 85, 0.6)"; 
        ctxV.lineWidth = 12;
        ctxV.beginPath(); 
        if(ctxV.roundRect) ctxV.roundRect(-boxWidth/2, -boxHeight/2, boxWidth, boxHeight, 25); 
        ctxV.stroke();
