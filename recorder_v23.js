// ==========================================
// RECORDER.JS - BẢN HOÀN HẢO CHO CONTENT CREATOR (V54.2 - EMBEDDED THUMBNAIL)
// [NEW] AUTO-EMBED THUMBNAIL: Tấm Poster ngẫu nhiên tự động trở thành 1.5s đầu tiên của Video.
// [OPTIMIZED] YouTube/TikTok tự động lấy ảnh này làm cover, loại bỏ hoàn toàn việc phải tải ảnh thủ công.
// [RETAINED] Cinematic Bloom, Color Grading, Storytelling TTS AI.
// ==========================================
...
// ==========================================
// THUMBNAIL GENERATOR (TẠO 2 POSTER NGANG VÀ DỌC CHUẨN YOUTUBE/TIKTOK)
// ==========================================
window.bakeThumbnailsForVideo = function(titleText) {
    if (!window.p1) return;
    
    // 1. Tạo Canvas Ngang 16:9
    ...
    const drawBackground = (ctx, w, h, isVertical) => {
        if (isVertical) {
            ctx.fillStyle = `hsl(${hue1}, 80%, 15%)`; ctx.fillRect(0, 0, w, h/2);
            ctx.fillStyle = `hsl(${hue2}, 80%, 15%)`; ctx.fillRect(0, h/2, w, h/2);
        } else {
            ctx.fillStyle = `hsl(${hue1}, 80%, 15%)`; ctx.fillRect(0, 0, w/2, h);
            ctx.fillStyle = `hsl(${hue2}, 80%, 15%)`; ctx.fillRect(w/2, 0, w/2, h);
        }
        
        ctx.save(); ctx.translate(w/2, h/2);
        for(let i=0; i<36; i++) {
            ctx.rotate(Math.PI / 18); ctx.fillStyle = (i % 2 === 0) ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.1)";
            ctx.beginPath(); ctx.moveTo(0, 0); ctx.lineTo(Math.max(w,h), 80); ctx.lineTo(Math.max(w,h), -80); ctx.fill();
        } ctx.restore();

        ctx.globalCompositeOperation = 'overlay'; ctx.fillStyle = "rgba(0, 0, 0, 0.4)";
        for(let x=0; x<w; x+=15) { for(let y=0; y<h; y+=15) { if((x+y)%30===0) { ctx.beginPath(); ctx.arc(x, y, 3, 0, Math.PI*2); ctx.fill(); } } }
    ...
    ctxH.save(); ctxH.shadowBlur = 60; ctxH.shadowColor = `hsl(${hue1}, 100%, 50%)`; 
    if(p1CloneH.isDragon && typeof window.drawDragon === 'function') window.drawDragon(ctxH, p1CloneH); else if(typeof window.drawStickman === 'function') window.drawStickman(ctxH, p1CloneH);
    ctxH.restore();

    let p2CloneH = Object.assign({}, e1, { x: 1470, y: 850, scale: (e1.scale || 1) * 4.2, state: sP2, isFacingRight: false });
    ctxH.save(); ctxH.shadowBlur = 60; ctxH.shadowColor = `hsl(${hue2}, 100%, 50%)`; 
    if(p2CloneH.isDragon && typeof window.drawDragon === 'function') window.drawDragon(ctxH, p2CloneH); else if(typeof window.drawStickman === 'function') window.drawStickman(ctxH, p2CloneH);
    ctxH.restore();

    // --- VẼ CHO BẢN DỌC (TIKTOK) ---
    window.ctx = ctxV;
    let p1CloneV = Object.assign({}, window.p1, { x: 540, y: 1550, scale: (window.p1.scale || 1) * 4.5, state: sP1, isFacingRight: true });
    ctxV.save(); ctxV.shadowBlur = 60; ctxV.shadowColor = `hsl(${hue1}, 100%, 50%)`; 
    if(p1CloneV.isDragon && typeof window.drawDragon === 'function') window.drawDragon(ctxV, p1CloneV); else if(typeof window.drawStickman === 'function') window.drawStickman(ctxV, p1CloneV);
    ctxV.restore();

    let p2CloneV = Object.assign({}, e1, { x: 540, y: 700, scale: (e1.scale || 1) * 4.5, state: sP2, isFacingRight: false });
    ctxV.save(); ctxV.shadowBlur = 60; ctxV.shadowColor = `hsl(${hue2}, 100%, 50%)`; 
    if(p2CloneV.isDragon && typeof window.drawDragon === 'function') window.drawDragon(ctxV, p2CloneV); else if(typeof window.drawStickman === 'function') window.drawStickman(ctxV, p2CloneV);
    ctxV.restore();
    window.ctx = originalCtx;

    // --- VẼ CHỮ "VS" VÀ TIÊU ĐỀ ---
...
window.startRecording = function() {
    ...
    let videoStreamH = window.recordCanvasH.captureStream(60); // 60 FPS
    ...
    // 🔥 TẠO THUMBNAIL VÀ ĐẶT THỜI GIAN HIỂN THỊ (90 frames = 1.5s)
        ...
        if (stoppedCount === 2) { 
            setTimeout(() => {
                ...
                window.savedVideos.unshift({ 
                    ...
                    previewThumb: window.bakedThumbH.toDataURL("image/jpeg", 0.5) // Chỉ lưu bản xem trước nhỏ nhẹ cho UI
                });
...
window.captureFrames = function() {
    ...
    // 🔥 XỬ LÝ NHÚNG THUMBNAIL TRỰC TIẾP VÀO VIDEO (1.5 GIÂY ĐẦU)
    if (window.thumbnailHoldFrames > 0) {
        ...
        // Hiệu ứng Fade out (Mờ dần nháy sang game)
        ...
        return; // Dừng lại ở đây, không capture game trong 1.5s này
    }
    ...
    // =====================================
    // CÁC HIỆU ỨNG ĐIỆN ẢNH VÀ QUANG HỌC
    // =====================================
...
window.updateVideoListUI = function() {
    ...
    let html = `<h3 style="margin: 0 0 20px 0; color: #00f3ff; text-align: center; font-family: 'Teko', sans-serif; letter-spacing: 2px; font-size: 32px;">🎬 STUDIO ARCHIVE (${window.savedVideos.length} VIDEOS)</h3>
        ...
        html += `<div style="display: flex; gap: 20px; background: #1e293b; padding: 15px; border-radius: 10px; border: 1px solid #334155; box-shadow: 0 4px 6px rgba(0,0,0,0.3); transition: transform 0.2s;">
                    ...
                    <div style="flex: 1; display: flex; flex-direction: column; justify-content: space-between;">
                        ...
                        <div style="display: flex; flex-wrap: wrap; gap: 10px; margin-top: 15px;">
                            <button onclick="window.copyToClipboard('${vid.viralTitle.replace(/'/g, "\\'")}')" style="background: #3b82f6; color: white; border: none; padding: 8px 12px; border-radius: 5px; font-weight: bold; cursor: pointer; font-size: 13px; transition: 0.2s;">📋 Copy Title</button>
