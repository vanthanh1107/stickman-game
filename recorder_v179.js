// ==========================================
// RECORDER.JS - V76.8 ANIME IMPACT VFX EDITION
// ĐÃ FIX & GIỮ NGUYÊN: Meme Story Mode, Khung Chat Kính Mờ, Tải Tức Thì.
// NÂNG CẤP V76.8 (DYNAMIC COMBAT): 
// 1. DYNAMIC CAMERA: Auto Zoom & Lắc màn hình cực mạnh khi trúng đòn.
// 2. SCREEN FLASH: Chớp sáng màn hình (Trắng/Đỏ) tạo lực đánh (Impact).
// 3. SPEED LINES: Hiệu ứng sọc tốc độ Anime viền màn hình khi giao tranh.
// ==========================================
...
// BIẾN TRACKING CHO HIỆU ỨNG ĐÁNH NHAU SINH ĐỘNG (IMPACT VFX)
...
window.captureFrames = function() {
    ...
    if (renderNormalV) {
        ...
        // ==========================================
        // IMPACT VFX (XỬ LÝ ĐÁNH NHAU SINH ĐỘNG NHƯ ANIME)
        // ==========================================
        if (!isOutroActive && !window.gameOver && window._gamePhaseStarted) {
            ...
            // Nếu có sát thương lớn hơn 2% máu -> Kích hoạt Impact
            if (p1Dmg > 0.02 || p2Dmg > 0.02) {
                window._impactFrames = 12; // Số frame duy trì hiệu ứng
                window._impactIntensity = Math.min((p1Dmg + p2Dmg) * 300, 40); // Độ mạnh của cú giật
                window._impactColor = (p1Dmg > p2Dmg) ? "#ff003c" : "#ffffff"; // Đỏ nếu bị đấm, trắng nếu đấm boss
            }
        }
        ...
        // Xử lý Kill Cam và Camera Impact
        if (isKillCamActive) {
            ...
            // Dynamic Zoom + Extra Shake khi Impact
        ...
        // Screen Flash (Chớp sáng màn hình theo sát thương)
        ...
        ctxV.restore(); // Phục hồi Canvas khỏi Camera Zoom/Shake để không ảnh hưởng HUD bên dưới
