// ==========================================
// RECORDER.JS - BẢN HOÀN HẢO CHO CONTENT CREATOR (V66.4 - ULTIMATE RETENTION HACK)
// [NEW] Thêm dải Marquee chạy chữ "WAIT FOR THE END" thôi miên ánh nhìn.
// [NEW] Hiệu ứng Satisfying: Hạt lửa (Embers) bay lên liên tục ở background.
// [NEW] Hệ thống Emoji Reactions (🔥💀🤯): Tự động nảy lên khi có đòn đánh mạnh (dựa vào Audio Peak) y hệt Livestream.
// [RETAINED] Bố cục Split-Screen nhảy thẳng vào game cho dọc, Manga Intro cho ngang.
// ==========================================
...
window.startRecording = function() {
    ...
    window.retentionParticles = []; window.retentionEmojis = []; // Reset effects
    ...
    window.currentVideoExt = "mp4";
    if (MediaRecorder.isTypeSupported('video/mp4; codecs="avc1.42E01E, mp4a.40.2"')) { options = { mimeType: 'video/mp4; codecs="avc1.42E01E, mp4a.40.2"', videoBitsPerSecond: 15000000 }; } 
    else if (MediaRecorder.isTypeSupported('video/mp4; codecs="avc1"')) { options = { mimeType: 'video/mp4; codecs="avc1"', videoBitsPerSecond: 15000000 }; }
    else if (MediaRecorder.isTypeSupported('video/webm; codecs="h264"')) { options = { mimeType: 'video/webm; codecs="h264"', videoBitsPerSecond: 15000000 }; window.currentVideoExt = "webm"; }
    else if (MediaRecorder.isTypeSupported('video/webm; codecs="vp9"')) { options = { mimeType: 'video/webm; codecs="vp9"', videoBitsPerSecond: 15000000 }; window.currentVideoExt = "webm"; }
    else { options = { mimeType: 'video/webm; codecs="vp8"', videoBitsPerSecond: 15000000 }; window.currentVideoExt = "webm"; }
...
function renderMobileSafeCombo(ctx, fighter, xPos, yPos, align, isMobile) {
    if (fighter && fighter.comboHits >= 2) {
        ...
        ctx.fillText(`🔥 ${hits} HITS`, Math.floor(xPos), Math.floor(yPos));
        ...
        ctx.fillText(`${rank} - ${rankText}`, Math.floor(xPos), Math.floor(yPos + (42 * scale))); ctx.restore();
    }
}
...
function drawAudioVisualizer(ctx, x, y, width, height) {
    ...
    for (let i = 0; i < numBars / 2; i++) {
        ...
        ctx.fillRect(Math.floor(center + offsetX), Math.floor(y - barHeight), Math.floor(barWidth), Math.floor(barHeight)); 
        ctx.fillRect(Math.floor(center - offsetX - barWidth), Math.floor(y - barHeight), Math.floor(barWidth), Math.floor(barHeight));
    }
    ctx.restore();
}
...
window.captureFrames = function() {
    ...
    if (renderNormalH) {
        ...
        ctxH.drawImage(window.canvas, 0, 0, window.canvas.width, window.canvas.height, Math.floor(shakeX), Math.floor(shakeY), 2560, 1440); 
        ctxH.filter = 'none';
        
        if (audioPeak > 0.1) {
            ctxH.globalCompositeOperation = 'screen'; ctxH.globalAlpha = audioPeak * 0.2; 
            ctxH.drawImage(window.canvas, 0, 0, window.canvas.width, window.canvas.height, Math.floor(shakeX - 15), Math.floor(shakeY - 15), 2590, 1470);
    ...
    ctxV.drawImage(window.canvas, 0, 0, window.canvas.width, window.canvas.height, Math.floor(shakeX), Math.floor(shakeY), 1440, splitGameHeight); 
    ctxV.filter = 'none';

    if (audioPeak > 0.1) {
        ctxV.globalCompositeOperation = 'screen'; ctxV.globalAlpha = audioPeak * 0.2; 
        ctxV.drawImage(window.canvas, 0, 0, window.canvas.width, window.canvas.height, Math.floor(-10 + shakeX), Math.floor(-10 + shakeY), 1460, splitGameHeight + 20);
        ctxV.globalAlpha = 1.0; ctxV.globalCompositeOperation = 'source-over';
    }

    // ==========================================
    // 3. BACKGROUND CHO RETENTION ZONE (TỐI ƯU CỰC MẠNH)
    // ==========================================
    ...
    gradBg.addColorStop(0, "#05050a"); gradBg.addColorStop(1, `rgba(0, ${Math.floor(20 + Math.sin(bgPulse)*20)}, 40, 1)`);
    ctxV.fillStyle = gradBg; 
    ctxV.fillRect(0, retainY, 1440, retainHeight); 
    
    // Lưới nền mờ đi một chút để nhường chỗ cho hiệu ứng mới
    ctxV.strokeStyle = "rgba(0, 243, 255, 0.03)"; ctxV.lineWidth = 2;
    ctxV.beginPath(); 
    let gridOffset = Math.floor((Date.now() / 20) % 50);
    for(let i = 0; i < 1440; i+= 50) { ctxV.moveTo(i, retainY); ctxV.lineTo(i, 2560); }
    for(let j = retainY; j < 2560; j+= 50) { ctxV.moveTo(0, j + gridOffset); ctxV.lineTo(1440, j + gridOffset); }
    ctxV.stroke(); 

    // --- HIỆU ỨNG 1: HẠT NĂNG LƯỢNG BAY LÊN (SPARKS/EMBERS) ---
    if (!window.retentionParticles) window.retentionParticles = [];
    if (Math.random() < 0.4) {
        window.retentionParticles.push({
            x: Math.random() * 1440, y: 2560 + 50,
            s: Math.random() * 15 + 5, v: Math.random() * 10 + 5,
            h: Math.random() > 0.5 ? 0 : 190 // 0 = Đỏ lửa, 190 = Xanh lam
        });
    }
    ...
    for (let i = window.retentionParticles.length - 1; i >= 0; i--) {
        ...
        p.y -= p.v;
        ctxV.fillStyle = `hsla(${p.h}, 100%, 60%, 0.8)`;
        ctxV.shadowBlur = 15; ctxV.shadowColor = ctxV.fillStyle;
        ctxV.beginPath(); ctxV.arc(Math.floor(p.x), Math.floor(p.y), p.s, 0, Math.PI*2); ctxV.fill();
        ctxV.shadowBlur = 0;
        if (p.y < retainY) window.retentionParticles.splice(i, 1); // Xóa khi bay qua Retention Zone
    }
    ctxV.globalCompositeOperation = "source-over";

    // --- HIỆU ỨNG 2: THANH MARQUEE CẢNH BÁO BÊN DƯỚI VIDEO (HAZARD TAPE) ---
    ...
    let offsetBanner = Math.floor((Date.now() / 8) % 1000);
    for(let i = -1; i < 5; i++) {
        ctxV.fillText("🚨 WAIT FOR THE END 🚨 DO NOT BLINK", i*850 - offsetBanner, bannerY);
    }

    // --- HIỆU ỨNG 3: EMOJIS LIVESTREAM REACTION (NẢY LÊN KHI CÓ ÂM THANH MẠNH) ---
    ...
    for (let i = window.retentionEmojis.length - 1; i >= 0; i--) {
        ...
        re.y -= re.v;
        ctxV.save();
        ctxV.translate(Math.floor(re.x), Math.floor(re.y));
        ctxV.rotate(re.r + Math.sin(Date.now()/200)*0.2); // Xoay lả lướt
        ctxV.font = "90px Arial";
        ctxV.globalAlpha = Math.max(0, Math.min(1, (re.y - retainY - 100) / 400)); // Mờ dần khi lên cao
    ...
    if (window.p1 && !window.gameOver && window.introTimer <= 120) {
        ...
        // --- HUD DỌC 2K (ĐÃ DỊCH XUỐNG DƯỚI DẢI MARQUEE) ---
        ...
        if (p1Hp > 0) { let hpGradV = ctxV.createLinearGradient(187, 0, 693, 0); hpGradV.addColorStop(0, "#ff4757"); hpGradV.addColorStop(1, "#ff7f50"); drawSkewedPath(ctxV, 187, hudBaseY + 73, Math.floor(507 * p1Hp), 53, true); ctxV.fillStyle = hpGradV; ctxV.fill(); }
        ctxV.fillStyle = "rgba(0,0,0,0.8)"; ctxV.fillRect(187, hudBaseY + 140, 400, 20); ctxV.fillStyle = "#f1c40f"; ctxV.fillRect(187, hudBaseY + 140, Math.floor(400 * p1Stam), 20);
        ...
        if (repEnemyObj) {
            ...
            if (p2Hp > 0) { let hpGradV2 = ctxV.createLinearGradient(747, 0, 1253, 0); hpGradV2.addColorStop(0, "#c0392b"); hpGradV2.addColorStop(1, "#e74c3c"); let eHpWidth = Math.floor(507 * p2Hp); drawSkewedPath(ctxV, 747 + (507 - eHpWidth), hudBaseY + 73, eHpWidth, 53, false); ctxV.fillStyle = hpGradV2; ctxV.fill(); }
            ctxV.fillStyle = "rgba(0,0,0,0.8)"; ctxV.fillRect(853, hudBaseY + 140, 400, 20); ctxV.fillStyle = "#f1c40f"; ctxV.fillRect(853 + (400 - Math.floor(400 * eStam)), hudBaseY + 140, Math.floor(400 * eStam), 20);
        }
    }
    ...
    if (!window.gameOver) {
        ...
        let subY = (typeof splitGameHeight !== 'undefined') ? splitGameHeight + 580 : 1400; 
        ctxV.save(); ctxV.translate(720, subY); let subScale = 1 + (audioPeak * 0.12); ctxV.scale(subScale, subScale);
        ctxV.fillStyle = "#ff0000"; ctxV.shadowColor = "rgba(255,0,0,0.8)"; ctxV.shadowBlur = 10;
    ...
    if (window.gameOver && window.matchEndTimer > 90) { 
        ...
        let drawOutroCTA = (ctx, w, h, isMobile) => {
            ...
            let cx = Math.floor(w / 2); let cy = Math.floor(h / 2);
            ctx.textAlign = "center"; ctx.textBaseline = "middle"; ctx.fillStyle = "#00f3ff"; ctx.shadowColor = "#00f3ff"; ctx.shadowBlur = 15;
            let floatY = Math.floor(Math.sin(window.matchEndTimer * 0.05) * 10);
            ...
            ctx.fillStyle = btnGrad; ctx.shadowColor = "#ff003c"; ctx.shadowBlur = 20 + Math.floor(Math.sin(window.matchEndTimer * 0.1) * 10);
