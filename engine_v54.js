// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 44.0 [THE ULTIMATE ANIME APEX]
// [RESTORED] KHÔNG LỖI ANIMATION: Khớp tay, chân cử động mượt 100%, Zoom rộng bao quát toàn thân như V42.0.
// [INTEGRATED] DBZ TELEPORT CLASH: 2 nhân vật tự bay vút lên không trung chém liên hoàn khi Clash.
// [INTEGRATED] DYNAMIC SPEED LINES: Sọc tốc độ Anime xé rách màn hình khi Dash, Né, K.O.
// [INTEGRATED] ACTION DUTCH ANGLES & OVERKILL SHAKE: Camera nghiêng chéo theo lực đấm, rung chấn K.O buff mạnh 1.5 lần.
// [BASE] Matrix Orbit 360, Tàng hình Phantom Dash, Hất tung Juggle, Nảy sàn, Mực văng Camera, RTX Floor.
// ==========================================
...
// [ANIME APEX] Mảng chứa các đường Speed Lines
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce, attackerDirRight = true) {
    ...
    if (finalDmg > 0) {
        ...
        if (isCrit) {
            ...
            window.targetZoom = 1.15; setTimeout(() => { window.targetZoom = 1.0; }, 100); 
            ...
            // [ANIME APEX] ACTION DUTCH ANGLE - Ép máy quay nghiêng xéo mạnh tạo lực ép vật lý
            window.targetTilt = attackerDirRight ? 0.08 : -0.08; setTimeout(() => { window.targetTilt = 0; }, 500);
        ...
        if (wallBounce) { target.vx = target.isFacingRight ? -8 : 8; target.state = 'hurt'; } 
        ...
        if (target.hp <= 0) {
            ...
            if (isNearWall && isCrit && typeof window.triggerStageTransition === 'function') { window.triggerStageTransition(target); } 
                ...
                if (isCrit) {
                    ...
                    // [ANIME APEX] OVERKILL DUTCH ANGLE
                    ...
                    // [ANIME APEX] BUFF SHAKE LÊN MỨC ĐỘ TÀN BẠO
...
window.attack = function(attacker, targetGroup) {
    ...
    if (MathDist > reach && !isFinisher) { 
        ...
        window.speedLinesAlpha = 1.0; // [ANIME APEX] MỞ SPEED LINES
    ...
    if (selectedMove === 'uppercut' || selectedMove === 'dragon_uppercut') { target.vy = -16; target.onGround = false; target.state = 'juggle'; target.hitStun = 40; }
    ...
    if (isFinisher) {
        isCrit = true; finalDmg = baseDmg * 3.5; window.shakeScreen(15, 12); target.vx = (attacker.isFacingRight ? 6 : -6); 
        ...
        window.speedLinesAlpha = 1.0; window.actionCamOffsetX = attacker.isFacingRight ? 80 : -80; window.targetCamOrbitAngle = attacker.isFacingRight ? 0.3 : -0.3;
        
        // [ANIME APEX] DUTCH ANGLE LÚC KẾT LIỄU
        window.targetTilt = attacker.isFacingRight ? 0.08 : -0.08; setTimeout(() => { window.targetTilt = 0; }, 400);
            ...
            for (let j = i + 1; j < allFighters.length; j++) { 
                ...
                if (dist < 80 && f1.attackTimer > 5 && f1.attackTimer < 20 && f2.attackTimer > 5 && f2.attackTimer < 20) {
                    if (!f1.clashCooldown && !f2.clashCooldown && f1.isFacingRight !== f2.isFacingRight) {
                        ...
                        window.shakeScreen(30, 30); window.playSound(500, 'square', 0.2, 0.8, true);
                        window.targetZoom = 1.8; window.screenFlash = 0.5; window.impactAberration = 35;
                        
                        let midX = (f1.x + f2.x)/2; let midY = (f1.y + f2.y)/2 - 80; 
                        ...
                        for(let k=0; k<8; k++){
                            setTimeout(() => {
                                if(typeof window.spawnSlash === 'function') window.spawnSlash(midX + (Math.random()-0.5)*100, midY + (Math.random()-0.5)*100, Math.random() > 0.5, ["#ff003c", "#00f3ff", "#f1c40f", "#ffffff"][Math.floor(Math.random()*4)], true, 1.5 + Math.random()*2, Math.random()*Math.PI*2);
                                if(typeof window.shakeScreen === 'function') window.shakeScreen(5, 10);
                            }, k * 50);
                        }

                        window.invertFrames = 3; window.isSpinningCam = true; 
                        window.orbitFocusX = midX; window.targetTimeScale = 0.05;
...
window.draw = function() {
    ...
    if (window.isLoading) {
        ...
        window.ctx.fillStyle = "#fff"; window.ctx.font = "bold 20px monospace"; window.ctx.fillText(`ANIME DIRECTOR LOADING... ${Math.floor(window.loadProgress)}%`, cx, window.canvas.height - 20); 
        return; 
    }
    ...
    try {
        ...
        // [ANIME APEX] VẼ ĐƯỜNG NÉT TỐC ĐỘ ANIME (SPEED LINES)
