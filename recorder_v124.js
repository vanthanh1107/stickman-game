// ==========================================
// RECORDER.JS - V70.1 PERFECT UI SCALE (DIRECTOR'S CUT)
// ĐÃ FIX: Tên nhân vật trên đầu nhỏ lại, bay sát đầu, lấy chuẩn tên trong game.
// CẬP NHẬT MỚI: Tối ưu nét chữ, Lens Flare ánh sáng xé ngang, Sương mù khí chất.
// CẢM XÚC: Trailer Game Hạng AAA, Nghệ thuật thị giác đỉnh cao.
...
window.drawProceduralIntro = function(ctx, w, h, progress) {
    ...
    const drawFighterArtistic = (charObj, cx, cy, forcedState, isFacingRight, theme, fx, charName, titleStr) => {
        ...
        // VẼ TÊN NHÂN VẬT (Bám sát thiết kế in-game)
        if (charName && fx !== 'silhouette') {
            ctx.save();
            ctx.translate(0, -320); // Hạ thấp độ cao bay trên đầu
            let nameScale = 1 + Math.sin(Date.now() * 0.005) * 0.02;
            ctx.scale(nameScale, nameScale);
            
            ctx.textAlign = "center"; ctx.textBaseline = "middle";
            
            // Tên Nhân vật (Nhỏ hơn, chuẩn game UI)
            ...
            ctx.lineWidth = 6; ctx.strokeStyle = "#000"; // Viền chữ mỏng lại
            
            ctx.strokeText(charName, 0, 0); 
            ctx.shadowBlur = 0; // Tắt blur
            ctx.fillStyle = "#fff"; ctx.fillText(charName, 0, 0);
            
            // Danh xưng (Title) nhỏ bên dưới
            ctx.font = "bold 16px 'Montserrat', Arial"; ctx.letterSpacing = "2px";
            ctx.fillStyle = theme.aura;
            ctx.fillText(titleStr, 0, 35); // Kéo lại gần tên hơn
    ...
    if (cut.focus === 'p1') {
        drawFighterArtistic(window.p1, 0, 100, cut.state, true, prm.themeP1, cut.fx, prm.p1Name.toUpperCase(), "✦ AWAKENED ✦");
    } 
    else if (cut.focus === 'p2') {
        drawFighterArtistic(e1, 0, 100, cut.state, false, prm.themeP2, cut.fx, prm.p2Name.toUpperCase(), "✦ THE BOSS ✦");
    } 
    else { // Clash
...
window.startRecording = function() {
    ...
    // Bắt thẳng tên thật của game đẩy vào IntroParams
