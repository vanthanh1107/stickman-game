    ...
    for (let id in statsObj) {
        ...
        if (type === 'dausi') {
            statsObj[id].drawMethod = function(ctx, p, bounce, ext, pext, isTrail) {
                ...
                ctx.beginPath(); ctx.arc(handL.x, handL.y, 8, 0, Math.PI*2); ctx.fill(); ctx.beginPath(); ctx.arc(handR.x, handR.y, 8, 0, Math.PI*2); ctx.fill();
...
window.initGame = async function() {
    try {
        ...
        // Hỗ trợ cả 2 trường hợp: Có 5 hay 6 cột đều đọc được
        ...
        window.classStats = { 
            "dausi": { className: "Đấu Sĩ (Lỗi Mạng)", hp: 1500, speed: 6, dmgMod: 1.5, color: "#ff4757", avatarUrl: "https://api.dicebear.com/7.x/adventurer/png?seed=mma&backgroundColor=ffdfbf" },
            "satthu": { className: "Sát Thủ (Lỗi Mạng)", hp: 800, speed: 8, dmgMod: 2.0, color: "#9b59b6", avatarUrl: "https://api.dicebear.com/7.x/adventurer/png?seed=ninja&backgroundColor=ffdfbf" }
        };
        assignDrawMethods(window.classStats);
        window.renderCharacterGrid();
    }
...
window.startGame = function() { 
    ...
    // Tự động load trận đấu và gọi hàm gameLoop
...
window.matchStart = function() {
    try {
        ...
        // KẾT NỐI BẤM MÀN HÌNH CHUẨN XÁC
        if (!window.attackBound) {
            ...
            let triggerAttack = function(e) { 
                ...
                e.preventDefault(); 
                if (!gameOver && window.p1 && introTimer <= 0 && window.p1.attackTimer === 0 && window.p1.hitStun === 0 && window.p1.stunTimer === 0) {
...
window.checkGameOver = function() {
    if (matchResolved) return; let allDead = window.enemies.length === 0 || window.enemies.every(e => e.hp <= 0);
...
window.updateHPUIs = function() {
    if (!window.p1) return; let p1Pct = (window.p1.hp / window.p1.maxHp * 100) + "%"; let currentEnemyHp = 0; window.enemies.forEach(e => currentEnemyHp += e.hp); let p2Pct = totalEnemyMaxHp > 0 ? (currentEnemyHp / totalEnemyMaxHp * 100) + "%" : "0%";
