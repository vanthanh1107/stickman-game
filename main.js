    ...
    try {
        ...
        for (let i = 1; i < rows.length; i++) {
            ...
            if (cols.length >= 6) {
                ...
                // KIỂM TRA ID ĐỂ GÁN HÀM VẼ (DRAW METHOD) RIÊNG BIỆT
                if (id === 'dausi') {
                    window.classStats[id].drawMethod = function(ctx, p, bounce, ext, pext, isTrail) {
        ...
        window.classStats = { "dausi": { className: "Đấu Sĩ MMA", hp: 1500, speed: 6, dmgMod: 1.5, color: "#ff4757", avatarUrl: "https://api.dicebear.com/7.x/adventurer/png?seed=mma&backgroundColor=ffdfbf" } };
        window.renderCharacterGrid();
    }
}
        ...
        if (!window.attackBound) {
            ...
            let triggerAttack = function(e) { 
                ...
                if (!gameOver && p1 && introTimer <= 0 && p1.attackTimer === 0 && p1.hitStun === 0 && p1.stunTimer === 0) {
                    ...
                    p1.comboTimeout = 60; 
                    if(typeof window.attack === 'function') window.attack(p1, enemies); 
                }
            };
...
// Bắt đầu tải dữ liệu Google Sheets NGAY LẬP TỨC
setTimeout(() => { window.loadStatsFromGoogleSheets(); }, 100);
