...
// TÂM LÝ HỌC (NEW): Intro Gọn Gàng (Mưa Icon -> Khung Chat Nhỏ -> Bong Bóng Chat -> VS).
// ==========================================
...
window.drawProceduralIntro = function(ctx, w, h, progress) {
    ...
    // Tinh chỉnh kịch bản:
    // Act 0 (0-0.25): P1 + Mưa Icon Hài hước
    // Act 1 (0.25-0.55): Zoom P1 + Khung Chat Đáy màn hình (Nhỏ gọn, dễ đọc)
    // Act 2 (0.55-0.80): P2 + Bong bóng chat bay
    // Act 3 (0.80-1.0): VS + Lao vào nhau + Vỡ kính
    ...
    if (act === 0) {
        // ACT 0: HIỂN THỊ NHÂN VẬT 1 VỚI MỚ ICON
        drawFighterArtistic(window.p1, 0, 100, 'idle', true, prm.themeP1, realP1Name);
        ctx.restore(); // Thoát camera transform để vẽ icon chuẩn màn hình
        ctx.save();
        ...
        // ACT 1: GỌN GÀNG - HIỂN THỊ CHAT TOXIC DƯỚI ĐÁY MÀN HÌNH
        drawFighterArtistic(window.p1, 0, 100, 'cast', true, prm.themeP1, "");
        ctx.restore(); 
        ctx.save();
        
        let chatW = 850; let chatH = 260;
        ctx.translate(w/2, h - chatH/2 - 200); // Khung chat nằm dưới cùng màn hình
        ...
        let cY = -chatH/2 + 80;
        let numChatsToShow = Math.floor(localProg * 5); // Xuất hiện lần lượt
        ...
        // ACT 2: P2 + BONG BÓNG CHAT BÊN CẠNH
        ...
        // ACT 3: CHỮ VS TO KHỔNG LỒ + CLASH MÀU ÂM BẢN
    ...
    // Không dùng viền đen ở các Act để màn hình rộng nhất có thể
    drawCRTScanlines();
...
window.captureFrames = function() {
    ...
    if (renderNormalV) {
        ...
        if (!isOutroActive) {
            // ENGAGEMENT PARTICLES (CẤP SỐ NHÂN)
            ...
            // HUD CỐ ĐỊNH CHUẨN MỰC BÊN DƯỚI (GAME UI)
