// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 9.0 (DIRECTOR'S CUT)
...
// [NEW 9.0]: CẤY LÕI CINEMATIC DIRECTOR (TWEENING, CAMERA CONTROL, CINEMATIC LOCK)
// THỎA SỨC SÁNG TẠO MỌI KỊCH BẢN SKILL ĐIÊN RỒ NHẤT!
// ==========================================
...
window.cineCamera = null; // [NEW 9.0] Lưu trữ camera tùy chỉnh
...
// [NEW 8.5/9.0] Hàng đợi xử lý Kịch bản kỹ năng & Tween chuyển động
window.asyncTasks = [];
...
// ==========================================
// [NEW 9.0] ĐẠO DIỄN CINEMATIC (DIRECTOR API)
// ==========================================
window.sleep = (ms) => new Promise(resolve => setTimeout(resolve, ms));

window.waitFrames = function(frames, owner = null) {
    return new Promise(resolve => {
        window.asyncTasks.push({ timer: frames, maxTimer: frames, isTween: false, resolve: resolve, owner: owner });
    });
};

// Hàm Tween: Giúp lướt múa mượt mà theo frame (easing: linear, easeOut, easeInOut)
window.doTween = function(target, props, frames, easing = 'easeOut', owner = null) {
    return new Promise(resolve => {
        let startProps = {};
        for(let k in props) startProps[k] = target[k] || 0;
        window.asyncTasks.push({
            timer: frames, maxTimer: frames, isTween: true, 
            target: target, startProps: startProps, endProps: props, easing: easing,
            resolve: resolve, owner: owner
        });
    });
};

// Hàm điều khiển Camera thủ công
window.setCamera = function(x, y, zoom) {
    if(!window.canvas) return;
    window.cineCamera = { 
        x: (window.canvas.width / 2) - x, 
        y: Math.max(0, (window.GROUND_Y - y) * 0.5 + 40), 
        zoom: zoom 
    };
}
window.resetCamera = function() { window.cineCamera = null; }
...
window.update = function() {
    ...
    // [NEW 9.0] XỬ LÝ HÀNG ĐỢI KỊCH BẢN & TWEENING (SKILLS & AI)
    for (let i = window.asyncTasks.length - 1; i >= 0; i--) {
        let task = window.asyncTasks[i];
        if (isTimeStopped && task.owner !== window.timeStopCaster) continue; // Bị ngừng đọng thời gian
        
        if (task.isTween) {
            let p = 1 - (task.timer / task.maxTimer);
            let e = p; // linear default
            if(task.easing === 'easeOut') e = 1 - Math.pow(1 - p, 3);
            else if(task.easing === 'easeInOut') e = p < 0.5 ? 2 * p * p : 1 - Math.pow(-2 * p + 2, 2) / 2;
            
            for(let k in task.endProps) {
                task.target[k] = task.startProps[k] + (task.endProps[k] - task.startProps[k]) * e;
            }
        }
        
        task.timer--;
        if (task.timer <= 0) {
            if(task.isTween) { for(let k in task.endProps) task.target[k] = task.endProps[k]; } // Ensure exact final value
            task.resolve();
    ...
    allFighters.forEach(f => {
        ...
        // [NEW 9.0] MIỄN NHIỄM VẬT LÝ VÀ AI KHI ĐANG MÚA SKILL
        if (f.isCinematic) {
            if (f.attackTimer > 0) f.attackTimer--;
            if (f.comboDisplayTimer > 0) { f.comboDisplayTimer--; f.comboAlpha = 1; } else if (f.comboHits > 0) { f.comboAlpha -= 0.02; if (f.comboAlpha <= 0) { f.comboAlpha = 0; f.comboHits = 0; } }
            // Lưu lại vệt bóng (Trail) nếu đang di chuyển nhanh
            if (!f.trailArr) f.trailArr = [];
            if ((Math.abs(f.vx) > 1 || Math.abs(f.vy) > 1) || (f.attackTimer > 0 && f.attackTimer % 2 === 0)) { f.trailArr.push({x: f.x, y: f.y, state: f.state, isFacingRight: f.isFacingRight, color: f.color, alpha: 0.5, scale: f.scale, isDragon: f.isDragon}); }
            for (let i = f.trailArr.length - 1; i >= 0; i--) { f.trailArr[i].alpha -= 0.05; if (f.trailArr[i].alpha <= 0) f.trailArr.splice(i, 1); }
            return; // CHẶN TOÀN BỘ AI VÀ TRỌNG LỰC
        }
        ...
        else if (f.comboHits > 0) { f.comboAlpha -= 0.02; if (f.comboAlpha <= 0) { f.comboAlpha = 0; f.comboHits = 0; } }
            ...
            if (f.hitStun <= 0 && f.stunTimer <= 0 && f.state !== 'dash_back' && f.state !== 'block') {
                ...
                if (isCloseEnough) {
                    if (window.classStats && window.classStats[type] && typeof window.classStats[type].executeUltimate === 'function') {
                        ...
                        f.isFacingRight = distToTarget > 0; f.vx = 0;
                        (async () => {
                            try { await window.classStats[type].executeUltimate(f, closestTarget, baseDmg); } catch(e) { console.error("Lỗi Ultimate:", e); }
                        })();
                ...
                if (targetFighter && targetFighter.hp > 0) {
                    ...
                    if (f.aiDelay <= 0) {
                        ...
                        if (randAction < 0.45) { 
                            ...
                            (async () => {
                                let maxFire = f.isEvolved ? 8 : 6;
                                for(let i=0; i<maxFire; i++) {
                                    if (window.gameOver || f.hp <= 0 || !window.p1) break;
                                    ...
                                    await window.waitFrames(f.isEvolved ? 4 : 5, f);
                                }
                            })();
                        } 
                            ...
                            (async () => {
                                for(let i=0; i<5; i++) {
                                    ...
                                    await window.waitFrames(f.isEvolved ? 3 : 4, f);
                                }
                            })();
                        } 
                    ...
                    if (f.aiDelay <= 0) {
                        ...
                        if (absDist > 250 && Math.random() < 0.6) { f.state = 'cast'; f.attackTimer = 25; window.playSound(300, 'sine', 0.3, 0.6); window.spawnSlash(f.x + (f.isFacingRight? 50:-50), f.y - 40, f.isFacingRight, "#fff", true, 2.0, Math.PI/2); window.spawnProjectile(f.x, f.y - 40, f.isFacingRight ? 12 : -12, 0, 15, "#fff", Math.floor(25 * f.dmgMod), window.p1); window.floatingTexts.push({ x: f.x, y: f.y - 80, text: "🗡️", color: "#fff", alpha: 1, vx: 0, vy: -2, font: "900 40px Arial", life: 40 }); } 
                            ...
                            (async () => {
                                ...
                                window.floatingTexts.push({ x: f.x, y: f.y - 80, text: "⚡", color: "#e74c3c", alpha: 1, vx: 0, vy: -2, font: "900 40px Arial", life: 40 }); 
                            })();
                        }
    ...
    if (!isTimeStopped) {
        for (let i = 0; i < allFighters.length; i++) { 
            for (let j = i + 1; j < allFighters.length; j++) { 
                let f1 = allFighters[i], f2 = allFighters[j]; if (!f1 || !f2 || f1.hp <= 0 || f2.hp <= 0 || f1.isCinematic || f2.isCinematic) continue;
    ...
    allFighters.forEach(f => {
        if (!f || f.hp <= 0 || f.isCinematic) return;
    ...
    // [NEW 9.0] CẬP NHẬT CAMERA ĐIỆN ẢNH
    if (window.p1 && window.introTimer === 0) {
        if (window.cineCamera) {
            window.targetCamX = window.cineCamera.x;
            window.targetCamY = window.cineCamera.y;
            window.targetZoom = window.cineCamera.zoom;
            window.targetTilt = 0;
        } else {
