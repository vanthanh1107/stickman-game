    ...
    if (window.p1 && window.introTimer === 0) {
        ...
        if (window.gameOver && closest) {
            ...
            if (window.matchEndTimer < 130) {
                ...
                // Sử dụng nội suy toán học (Easing) để camera trôi ra cực kỳ mượt mà
                
                // Từ từ trượt vị trí camera về lại chính giữa sàn đấu (0, 0)
                window.targetCamX += (0 - window.targetCamX) * 0.02; 
                window.targetCamY += (0 - window.targetCamY) * 0.02; 
                
                // Từ từ nhả Zoom về lại góc nhìn 1.0 (hoặc 0.95 nếu muốn rộng hơn nữa)
