// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 8.5
// [PHIÊN BẢN ULTIMATE WEATHER - KHÔNG GỘP DÒNG - FULL TÍNH NĂNG & VFX]
// [ĐÃ NÂNG CẤP: PROCEDURAL LIGHTNING, CINEMATIC PAN OUT & SKILL INTERPRETER]
// ==========================================
...
// ========================================================
// 🌟 TÁCH BIỆT LOGIC SKILL: TRAO QUYỀN CHO FILE CHARACTER
// ========================================================
window.attack = function(attacker, targetGroup) {
    ...
    let gameContext = {
        target: target, distance: MathDist,
        spawnTrap: window.spawnTrap, spawnParticles: window.spawnParticles, spawnProjectile: window.spawnProjectile, spawnSlash: window.spawnSlash,
        spawnLaser: window.spawnLaser, spawnAura: window.spawnAura, spawnCustomObj: window.spawnCustomObj, spawnEnvDamage: window.spawnEnvDamage, 
        playSound: window.playSound, shakeScreen: window.shakeScreen, takeDamage: window.takeDamage, triggerTimeStop: window.triggerTimeStop, applyFilter: window.applyFilter,         
        teleport: (dx, dy) => { 
            window.spawnParticles(attacker.x, attacker.y, "#8e44ad"); 
            attacker.x = dx; attacker.y = dy || window.GROUND_Y; 
            window.spawnParticles(attacker.x, attacker.y, "#8e44ad"); 
        },
        heal: (amount) => { 
            attacker.hp = Math.min(attacker.maxHp, attacker.hp + amount); 
            window.floatingTexts.push({ x: attacker.x, y: attacker.y - 80, text: `+${amount} 💚`, color: "#2ed573", alpha: 1, vx: 0, vy: -3, font: "900 30px Arial", life: 40 }); 
            window.spawnParticles(attacker.x, attacker.y, "#2ed573");
        },
        dash: (speed) => { attacker.vx = attacker.isFacingRight ? speed : -speed; window.spawnDust(attacker.x, window.GROUND_Y); },
        setAnimation: (animName, frames) => { attacker.state = animName; attacker.attackTimer = frames; }
    };

    let charData = window.classStats ? window.classStats[attacker.classId] : null;
    
    if (charData && typeof charData.performCustomSkill === 'function') {
        let skillWasFired = charData.performCustomSkill(attacker, target, gameContext);
        if (skillWasFired) return; 
    }
...
window.update = function() {
    ...
    if (window.p1 && window.introTimer === 0) {
        ...
        // 🌟 GIAI ĐOẠN 2: CINEMATIC PAN OUT (Từ từ lùi góc máy ra xa)
        if (window.gameOver && closest) {
            ...
            if (window.matchEndTimer < 130) {
                // 🎬 GIAI ĐOẠN 1: K.O ZOOM CẬN CẢNH (Kéo dài khoảng 2 giây)
                window.targetCamX = (window.canvas.width / 2) - midX; 
                window.targetCamY = Math.max(0, (window.GROUND_Y - midY) * 0.5 + 40); 
                
                let maxZoomForDistance = Math.max(1.2, 1.6 - (distance / 800) * 0.5); 
                let dynamicZoom = 1.25 - (distance / 600) * 0.35;
                window.targetZoom = Math.min(maxZoomForDistance, dynamicZoom + (window.matchEndTimer * 0.003)); 
                window.targetTilt = 0; 
            } else {
                window.targetCamX += (0 - window.targetCamX) * 0.02; 
                window.targetCamY += (0 - window.targetCamY) * 0.02; 
                window.targetZoom += (1.0 - window.targetZoom) * 0.015;
                window.targetTilt = 0;
            }
        } 
