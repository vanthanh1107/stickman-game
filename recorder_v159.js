// ==========================================
// RECORDER.JS - V75.8 THE ALGORITHM BREAKER (ANTI-LAG & GPU OPTIMIZED)
...
// NÂNG CẤP MỚI NHẤT (V75.8): Thuật toán ép xung GPU, Xóa triệt để giật lag lúc Combat, Cache Video Dimension!
// ==========================================
...
window.FACECAM_VIDEO_URLS = [
    ...
    "https://files.catbox.moe/r2wq16.mp4", // Video 6
    "https://files.catbox.moe/yhoaz3.mp4", // Video 7
    "https://files.catbox.moe/yg0ftp.mp4", // Video 8
    "https://files.catbox.moe/58d0mj.mp4", // Video 9
    "https://files.catbox.moe/ku9sdi.mp4", // Video 10
    "https://files.catbox.moe/ceg81a.mp4", // Video 11
    "https://files.catbox.moe/7ho7yx.mp4", // Video 12
    "https://files.catbox.moe/uap62b.mp4", // Video 13
    "https://files.catbox.moe/4n1o24.mp4", // Video 14
    "https://files.catbox.moe/995cuf.mp4"  // Video 15
];
...
window.startRecording = function() {
    ...
    if (window._facecamVideo) {
        ...
        if (window._facecamVideo.parentNode) {
            window._facecamVideo.parentNode.removeChild(window._facecamVideo);
        }
...
window.captureFrames = function() {
    ...
    if (renderNormalV) {
        ...
        // VẼ MAIN CANVAS LUÔN LUÔN CHẠY
        ctxV.drawImage(window.canvas, 0, 0, window.canvas.width, window.canvas.height, shakeX | 0, shakeY | 0, 1080, splitGameHeight); 

        // TỐI ƯU HÓA: GLITCH EFFECT LÚC COMBAT
        // Thay vì vẽ Canvas 3 lần tốn CPU, giờ ta chỉ vẽ dặm thêm 1 bóng mờ khi có chấn động lớn
        if (shouldGlitch) {
            let glitchStr = ((audioPeak - 0.75) * 30) | 0; 
            ctxV.globalAlpha = 0.5; 
            ctxV.drawImage(window.canvas, 0, 0, window.canvas.width, window.canvas.height, shakeX - glitchStr, shakeY | 0, 1080 + (glitchStr*2), splitGameHeight);
        ...
        if (!isOutroActive) {
            ...
            // 3. FACECAM GÁI XINH (VIDEO DYNAMIC KÍCH THƯỚC + ÉP XUNG GPU)
            if (!window._facecamVideo) {
                ...
                // TỐI ƯU HÓA GPU: Ép trình duyệt đưa thẻ video này vào quy trình xử lý của Card Đồ Họa
                window._facecamVideo.style.cssText = "position:absolute; width:1px; height:1px; opacity:0.01; pointer-events:none; z-index:-9999; transform: translateZ(0);";
                document.body.appendChild(window._facecamVideo);
            ...
            if (window._facecamVideo && window._facecamVideo.readyState >= 2) { 
                ...
                // TỐI ƯU HÓA DOM: Lưu lại cache kích thước video, không gọi liên tục gây tốn CPU
                if (!window._facecamVideo._vW) {
                    window._facecamVideo._vW = window._facecamVideo.videoWidth;
                    window._facecamVideo._vH = window._facecamVideo.videoHeight;
                }
                
                let vW = window._facecamVideo._vW;
                let vH = window._facecamVideo._vH;
