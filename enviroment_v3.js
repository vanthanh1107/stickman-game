// ==========================================
// ENVIRONMENT.JS - TRẠM KẾT XUẤT HIỆU ỨNG & ĐA VŨ TRỤ (V2.1 - FIXED PHYSICS)
// [ĐÃ SỬA LỖI] Khôi phục toàn bộ vật lý rơi của mưa, tuyết, sương và hoa
// ==========================================

// ==========================================
// 1. BỘ KHỞI TẠO MÔI TRƯỜNG & THỜI TIẾT
// ==========================================
...
// ==========================================
// 2. [MỚI THÊM LẠI] HỆ THỐNG VẬT LÝ THỜI TIẾT (UPDATE PHYSICS)
// ==========================================
window.updateWeatherPhysics = function() {
    if (!window.weatherParticles || window.weatherParticles.length === 0 || window.timeStopTimer > 0) return;
    ...
    window.weatherParticles.forEach(w => { 
        ...
        // Bụi tinh vân
            ...
            w.y += Math.sin(Date.now()/1000 + w.x)*0.3; 
            w.x += Math.cos(Date.now()/1000 + w.y)*0.3;
        } 
        // Mưa sao băng
        ...
        // Mưa, tuyết, cánh hoa rơi bình thường
        else { 
            w.y += w.speed; 
...
// ==========================================
// 3. HỆ THỐNG VẼ HIỆU ỨNG (RENDERERS)
// ==========================================
    ...
    window.envDamage.forEach(dmg => {
        ...
        if (dmg.type === 'crater') {
            ...
            if (dmg.isBurning) {
                ...
                ctx.beginPath(); ctx.arc(0, 0, (dmg.radius || 40) * 1.5, 0, Math.PI * 2); ctx.fill();
            } else {
                ctx.fillStyle = `rgba(15, 15, 15, ${0.8 * alpha})`;
                ctx.beginPath(); ctx.arc(0, 0, (dmg.radius || 40), 0, Math.PI * 2); ctx.fill();
            }
            ctx.restore();
        }
        ...
        dmg.cracks.forEach(path => {
            if (Array.isArray(path) && path.length > 1) {
                if (dmg.isBurning) {
                    ctx.strokeStyle = `rgba(255, 50, 0, ${0.8 * alpha})`; ctx.lineWidth = baseLw + 2; ctx.shadowBlur = 15; ctx.shadowColor = "#ff4757";
                } else {
                    ctx.strokeStyle = `rgba(10, 10, 10, ${0.5 * alpha})`; ctx.lineWidth = baseLw + 2; ctx.shadowBlur = 5; ctx.shadowColor = "#000";
                }
    ...
    if (['snow', 'ash', 'toxic'].includes(wType)) {
        ...
        window.weatherParticles.forEach(w => { 
            if (wType === 'petals') { 
                ...
                ctx.beginPath(); ctx.ellipse(w.x, w.y, w.size, w.size*0.5, w.ang + (w.y/50), 0, Math.PI*2); ctx.fill(); 
            }
            else if (wType === 'fireflies') {
                let glow = 0.3 + Math.abs(Math.sin(Date.now()/350 + w.x)) * 0.7; 
                ctx.fillStyle = `rgba(241, 196, 15, ${glow})`; ctx.shadowBlur = 8; ctx.shadowColor = "#f1c40f"; 
                ctx.beginPath(); ctx.arc(w.x, w.y, w.size * 1.4, 0, Math.PI*2); ctx.fill(); ctx.shadowBlur = 0;
            } 
            else if (wType === 'matrix_rain') {
                ctx.fillStyle = `rgba(0, 255, 68, ${0.4 + Math.random()*0.5})`; ctx.font = "bold 15px monospace";
...
// ==========================================
// 4. HOOK VÀO ENGINE CŨ MỘT CÁCH AN TOÀN
// ==========================================

// Hook vào hàm Reset
if (!window._hookedEnvSetup) {
    window._hookedEnvSetup = true;
    if (typeof window.resetMatchVariables === 'function') {
        const originalReset = window.resetMatchVariables;
        window.resetMatchVariables = function() {
            originalReset.apply(this, arguments);
            if (typeof window.setupEnvironment === 'function') window.setupEnvironment();
        };
    }
}

// Hook vào hàm Update (MỚI THÊM ĐỂ KÍCH HOẠT VẬT LÝ THỜI TIẾT)
if (!window._hookedEnvironmentUpdate) {
    window._hookedEnvironmentUpdate = true;
    const originalUpdateEngine = window.update;
    
    window.update = function() {
        // Cập nhật vị trí hạt thời tiết trước
        if (typeof window.updateWeatherPhysics === 'function') window.updateWeatherPhysics();
        
        // Sau đó chạy logic update gốc của game
        if (originalUpdateEngine) originalUpdateEngine.apply(this, arguments);
    };
}
