// ==========================================
// RECORDER.JS - BẢN HOÀN HẢO CHO CONTENT CREATOR (V57.2 - AUTO-TITLE 3D EMBED)
// [NEW] SMART 3D TEXT: Tự động lấy Tiêu đề Viral của AI dập thành chữ 3D siêu to trên Thumbnail.
// [NEW] AUTO WORD-WRAP: Tự động xuống dòng chữ 3D để vừa vặn với bản Ngang (PC) và bản Dọc (TikTok).
// [RETAINED] Giữ nguyên toàn bộ logic chống lỗi 0-byte, UI inline và Snap-Cut 1.5s đầu.
// ==========================================
...
window.bakeThumbnailsForVideo = function(titleText) {
    ...
    // 1. NỀN MANGA/SUNBURST ĐỘNG LỰC HỌC
    ...
    // 2. VẼ NHÂN VẬT (KÈM HÀO QUANG VÀ MẮT LAZE ĐỎ)
    ...
    // 3. VẼ ĐỒ NGHỀ CLICKBAIT (VÒNG ĐỎ, MŨI TÊN, TEXT ẢO)
    ...
    // 4. TIÊU ĐỀ AI ĐÙN KHỐI 3D CHUẨN YOUTUBER (TỰ ĐỘNG XUỐNG DÒNG)
    const draw3DTitleWrap = (ctx, w, h, text, isVertical) => {
        ctx.save();
        // Lớp mờ nền đen ở giữa để nổi bật chữ
        ...
        ctx.rotate(-0.06); // Nghiêng chữ cho ngầu
        ctx.textAlign = "center"; ctx.textBaseline = "middle";
        
        let fontSize = isVertical ? 100 : 130;
        ctx.font = `italic 900 ${fontSize}px 'Arial Black', Impact`;
        
        // Thuật toán Tự động xuống dòng (Word Wrap) thông minh
        let words = text.split(" ");
        let lines = [];
        let currentLine = words[0];
        let maxLineWidth = isVertical ? 1000 : 1800; // Chiều rộng tối đa cho phép của mỗi dòng
        ...
        lines.push(currentLine); // Đẩy dòng cuối cùng vào

        let depth = Math.floor(fontSize * 0.15); // Độ dày 3D
        let colors = [["#f1c40f", "#d35400"], ["#00f3ff", "#0055ff"], ["#ff4757", "#8b0000"]];
        let clr = colors[Math.floor(Math.random() * colors.length)];

        // Vẽ từng dòng 
        let startY = -(lines.length * (fontSize + 10)) / 2 + (fontSize/2); // Canh giữa theo cụm dòng
...
window.startRecording = function() {
    ...
    // START(1000) ĐỂ TRÁNH LỖI FILE RỖNG KHÔNG TẢI ĐƯỢC
...
window.captureFrames = function() {
    ...
    // 🔥 TỰ ĐỘNG CẮT VIDEO SAU K.O KHOẢNG 2 GIÂY
    ...
    // 🔥 XỬ LÝ NHÚNG THUMBNAIL TRỰC TIẾP VÀO VIDEO (1.5 GIÂY ĐẦU TIÊN)
    if (window.thumbnailHoldFrames > 0) {
        ...
        return; // Ép dừng capture khung hình game, hiển thị ảnh tĩnh trong 1.5s
    }
