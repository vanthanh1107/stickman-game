// ==========================================
// RECORDER.JS - BẢN HOÀN HẢO CHO CONTENT CREATOR (V66.8 - THE PROFILE MAGNET)
// [NEW] "Binge-Watch Grid" ở Outro: Giả lập trang cá nhân triệu view đẩy FOMO cực hạn.
// [NEW] "Follow Bouncer": Mũi tên Neon nảy liên tục trỏ thẳng vào nút (+) của TikTok.
// [NEW] Watermark "Part 2 On Profile" chớp nháy kích thích tò mò.
// [RETAINED] 3D Perspective Grid, CapCut AI Highlight, RGB Glitch, Smooth 60FPS.
// ==========================================
...
window.captureFrames = function() {
    ...
    // 1. Watermark chớp nháy kích thích "Part 2 On Profile"
    ...
    // 2. Mũi tên "Thôi miên" trỏ thẳng vào nút (+) của TikTok (Trôi giữa màn hình bên phải)
    if (!window.gameOver) {
        ctxV.save();
        let bouncerY = 1050 + Math.sin(Date.now() / 150) * 30; // Nảy lên nảy xuống
    ...
    // ==========================================
    // 4. BINGE-WATCH GRID (HIỆU ỨNG GIẢ LẬP TRANG CÁ NHÂN TIKTOK Ở CUỐI VIDEO)
    // ==========================================
    if (window.gameOver && window.matchEndTimer > 90) { 
        ...
        let drawOutroCTA = (ctx, w, h, isMobile) => {
            ...
            if (isMobile) {
                // TIKTOK PROFILE FOMO (Dành riêng cho màn hình dọc)
                ...
                // Vẽ 3 ô vuông giả lập Video Profile TikTok
                ...
                // Outro bản Ngang PC (Giữ nguyên phong cách cũ)
