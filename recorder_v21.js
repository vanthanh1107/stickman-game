...
window.startRecording = function() {
    ...
    const finalizeRecordings = () => {
        stoppedCount++;
        if (stoppedCount === 2) { // Rút gọn lại chỉ lưu Horizontal và Vertical
            setTimeout(() => {
...
window.captureFrames = function() {
    ...
    let drawCinematicEnhancements = (ctx, w, h) => {
        ...
        // 1. ANAMORPHIC LENS FLARES (Sự kiện bùng nổ ánh sáng ngang khi có âm thanh mạnh/Clash)
        if (audioPeak > 0.7) {
            ctx.globalCompositeOperation = 'screen';
            let flareIntensity = (audioPeak - 0.7) * 3.3; // Tính toán cường độ xẹt ngang
            let flareGrad = ctx.createLinearGradient(0, h/2, w, h/2);
            flareGrad.addColorStop(0, "rgba(0, 200, 255, 0)"); // Màu Teal / Cam viễn tưởng
            flareGrad.addColorStop(0.5, `rgba(0, 240, 255, ${flareIntensity})`);
            flareGrad.addColorStop(1, "rgba(0, 200, 255, 0)");
            ctx.fillStyle = flareGrad;
            
            // Vẽ 2 dải sáng mỏng kéo giãn chuẩn điện ảnh
            ctx.fillRect(0, h/2 - 15 + shakeY, w, 30);
            ctx.fillRect(0, h/2 - 3 + shakeY, w, 6);
            ctx.globalCompositeOperation = 'source-over';
        }

        // 2. TỐI GÓC MÀN HÌNH (VIGNETTE VÀ COLOR BURN TẠO ĐỘ SÂU)
        ...
        // 3. BỤI QUANG HỌC CỦA ỐNG KÍNH (LENS DUST)
        ...
        // 4. VẾT XƯỚC PHIM NGẪU NHIÊN (CHỐNG AI)
        if (Math.random() > 0.8) {
        ...
        // 5. FILM GRAIN MỀM MẠI LÊN TOÀN BỘ KHUNG HÌNH
