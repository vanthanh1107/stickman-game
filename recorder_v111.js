// ==========================================
// RECORDER.JS - V68.5 PERFORMANCE EDITION (ANTI-LAG ZERO DROP)
// Tối ưu Layout: Clean Game -> HUD -> Short Yellow Text -> Poll -> Smart Alerts -> Chat -> Cat 
// ĐÃ FIX: TRÁNH GIẬT LAG BẰNG ASYNC HOOK & THROTTLING (Game chạy 60FPS, Record 30FPS)
// ĐÃ TỐI ƯU: LAZY CHROMA-KEY GREEN SCREEN (Tiết kiệm 70% CPU)
...
// Biến Cache & Hiệu ứng (THÊM BIẾN TỐI ƯU HIỆU NĂNG)
...
window._lastCaptureTime = 0; // Dùng để giới hạn FPS quay video (Anti-lag)
window._lastCatRenderTime = 0; // Dùng để tối ưu con mèo nền xanh
...
window.StoryModeAI = {
    ...
    generateViralStickmanTitle: function(hero, enemy) {
        ...
        const r = (arr) => arr[Math.floor(Math.random() * arr.length)];
        let rawTitle = r(templates)
            .replace(/\[ADJ\]/g, () => r(adjectives))
            .replace(/\[ACTION\]/g, () => r(actions))
            .replace(/\[SUBJECT\]/g, () => r(subjects))
            .replace(/\[REACTION\]/g, () => r(reactions));

        const allTags = [
            "#schizomovement", "#psychologicalwarfare", "#villainarc", "#gaming", 
            "#brainrot", "#tas", "#movementplayer", "#lobotomy", "#disrespect", 
            "#clutch", "#iq999", "#ragequit", "#forbidden"
        ];
        ...
        if (this._usedTitles.has(finalTitle) && this._usedTitles.size < 500) {
            return this.generateViralStickmanTitle(hero, enemy); 
        }
        this._usedTitles.add(finalTitle);

        return finalTitle;
    },

    generateScript: function(hero, enemy) { 
        let generatedTitle = this.generateViralStickmanTitle(hero, enemy);
        return { title: generatedTitle, lines: [] }; 
    },
    ...
    init: function(hero, enemy) {
        ...
        this.currentLineIndex = 0; this.fullText = ""; this.displayedText = ""; 
        this.charIndex = 0; this.isTyping = false; this.scriptProgress = 0;
    },
...
function drawVTuberCommentator(ctx, x, y, audioPeak) {
    ...
    if (window.catVideoObj.readyState >= 2) { 
        ...
        // CÁCH TỐI ƯU 1: LÀM BIẾNG TÍNH TOÁN (LAZY CHROMA-KEY) - GIẢM LAG CPU CỰC TỐT
        // Chỉ vẽ và xóa phông mỗi 50ms (~20 FPS) thay vì 60 FPS
        if (t - window._lastCatRenderTime > 50) {
            window._lastCatRenderTime = t;
        ...
        // Lúc nào cũng vẽ bản đã cache (siêu nhẹ)
        ...
        if (stoppedCount === 2) { 
            setTimeout(() => {
                if (window.recordedChunksH.length === 0 || window.recordedChunksV.length === 0) {
                    alert("⚠️ Video extraction failed! Please refresh the page (F5), wait 3 seconds, and try again.");
                    return; 
                }
...
// CÁCH TỐI ƯU 2: TÁCH LUỒNG RENDER QUAY VIDEO ĐỂ KHÔNG GIẬT GAME (ASYNC HOOK)
if (!window._hookedDrawForRecorder) {
    ...
    window.draw = function() {
        if (oldDraw) oldDraw.apply(this, arguments); // Game vẽ bình thường trước
        
        if (window.isRecording && !window._isCapturingNow) {
            window._isCapturingNow = true;
            // Đẩy việc quay video ra khỏi luồng vẽ của game để tránh kẹt CPU
            setTimeout(() => {
...
window.captureFrames = function() {
    ...
    // CÁCH TỐI ƯU 3: GIỚI HẠN RECORDER Ở 30 FPS (Game vẫn 60FPS nhưng video chỉ 30FPS)
    // 33ms tương đương ~30 Khung hình/giây
    let now = Date.now();
    ...
    if (!isOutroActive) {
        ...
        if (window.p1) {
            ...
            let hudBaseY = splitGameHeight + 90; 
        ...
        if (chatNow - window._lastChatUpdate > window._nextChatDelay) {
            ...
            window._liveChats.push(window.generateLiveChatEvent());
        ...
        if (Date.now() - window._lastAlertTime > 2500 + Math.random() * 3000) {
            ...
            let alertTypes = [
                `❤️ ${randomUser} liked the LIVE!`,
                `👍 ${randomUser} liked the stream!`,
                `👤 ${randomUser} started following you!`,
                `🎁 ${randomUser} sent a Rose!`,
                `🔥 ${randomUser} joined the LIVE!`
            ];
        ...
        ctxV.save();
        let boxWidth = 960; let boxHeight = 440; let boxX = 540; let boxY = 1250; 
