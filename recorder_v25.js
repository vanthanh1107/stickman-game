// ==========================================
// RECORDER.JS - BẢN HOÀN HẢO CHO CONTENT CREATOR (V55.1 - HOTFIX)
// [FIXED] Sửa lỗi không hiện bảng tải video (Auto-Cut mượt mà ở frame 120).
// [FIXED] Sửa lỗi kẹt game cũ khi Replay (Tự động Force Stop video cũ khi game mới bắt đầu).
// [NEW] MEME THUMBNAIL GENERATOR: Tự động chế ảnh bìa clickbait (Mắt laze, mũi tên đỏ, vòng đỏ, caro).
// [NEW] AUTO-EMBED: Tấm Poster Meme tự động trở thành 1.5s đầu tiên của Video.
// ==========================================
...
window.StoryModeAI = {
    ...
    generateScript: function(hero, enemy) {
        ...
        const narratives = [
            { title: `BRO THINKS HE IS HIM 💀🤡`, lines: ["This might be the most ridiculous sequence ever captured in this game.", "Most players would button mash, but this is pure comedy.", "One missed frame and this entire run is completely dead."] },
            { title: `who let ${h} COOK?! 🗣️🔥`, lines: ["Never disrespect a boss like this unless you are ready for the consequences.", "Watch what happens when you push the game's mechanics to the absolute limit.", "Notice the exact frame the dodge happens. That is pure muscle memory."] },
            { title: `BRO IS LITERALLY HACKING 🤯🚫`, lines: ["I cannot believe they haven't patched this broken interaction yet.", "By canceling your attack at the exact moment, you break the game.", "Look at how fast the health bar just completely melts away."] },
            { title: `maximum disrespect against ${e} 📉💀`, lines: ["This has to be the most disrespectful combo ever pulled off.", "Look at how the hitboxes interact during this specific animation.", "You literally cannot make a single mistake or your health bar is gone."] },
            { title: `POV: average boss experience 😭🚩`, lines: ["We have all been stuck on this exact part of the game for way too long.", "You memorize the patterns, you upgrade your gear, but nothing works.", "Until you finally reach that flow state where everything just clicks."] },
            { title: `THE MOST ILLEGAL TRICK 🤫💻`, lines: ["Here is a secret trick pro gamers use to completely dominate this fight.", "But if you force the parry window, you reset their entire attack pattern.", `Watch how ${h} uses it here to completely humiliate them.`] },
            { title: `1 HP CLUTCH (I ALMOST DIED) 🚨📈`, lines: ["I want you to honestly ask yourself: would you have survived this situation?", "When your HP gets this low, the adrenaline usually makes you spam buttons.", "But the discipline to hold back and wait for the perfect parry window is insane."] }
        ];
...
// ==========================================
// THUMBNAIL MEME GENERATOR (ĐỘC NHẤT VÔ NHỊ MỖI TRẬN)
// ==========================================
window.bakeThumbnailsForVideo = function(titleText) {
    ...
    let hue1 = Math.floor(Math.random() * 360); let hue2 = (hue1 + 180 + (Math.random()*60 - 30)) % 360; 
    let bgStyle = Math.random() > 0.5 ? 'sunburst' : 'checkerboard'; 

    const drawMemeBackground = (ctx, w, h, isVertical) => {
        ctx.fillStyle = `hsl(${hue1}, 90%, 15%)`; ctx.fillRect(0, 0, w, h);
        
        ctx.save();
        if (bgStyle === 'sunburst') {
            ctx.translate(w/2, h/2);
            for(let i=0; i<40; i++) {
                ctx.rotate(Math.PI / 20); ctx.fillStyle = `hsl(${hue2}, 100%, 30%)`;
                ctx.beginPath(); ctx.moveTo(0, 0); ctx.lineTo(Math.max(w,h)*1.5, 100); ctx.lineTo(Math.max(w,h)*1.5, -100); ctx.fill();
            }
        } else {
            ctx.fillStyle = `hsl(${hue2}, 100%, 25%)`;
            let size = isVertical ? w/8 : w/15;
        ...
        // Rải Emoji Random chìm đằng sau
        let bgEmojis = ["🤡", "😭", "💀", "🔥", "💯", "⁉️", "🗿"];
        let e = bgEmojis[Math.floor(Math.random()*bgEmojis.length)];
        ctx.font = `${isVertical ? 250 : 350}px Arial`; ctx.globalAlpha = 0.2;
        ctx.fillText(e, w*0.15, h*0.2); ctx.fillText(e, w*0.85, h*0.8);
        ctx.globalAlpha = 1.0;
    };

    drawMemeBackground(ctxH, 1920, 1080, false);
    drawMemeBackground(ctxV, 1080, 1920, true);
    ...
    const drawMemeOverlays = (ctx, cx, cy, scale, isFacingRight) => {
        // 1. Mắt laze đỏ (NANI / Thug Life)
        if (Math.random() > 0.3) {
            ctx.save(); ctx.translate(cx + (isFacingRight ? 15 : -15)*scale, cy - 75*scale);
            let grad = ctx.createRadialGradient(0,0,0, 0,0, 45*scale);
            grad.addColorStop(0, "white"); grad.addColorStop(0.3, "red"); grad.addColorStop(1, "transparent");
            ctx.fillStyle = grad; ctx.beginPath(); ctx.arc(0,0, 45*scale, 0, Math.PI*2); ctx.fill();
            ctx.fillStyle = "rgba(255, 0, 0, 0.8)";
            ctx.beginPath(); ctx.ellipse(0, 0, 180*scale, 6*scale, (Math.random()-0.5)*0.5, 0, Math.PI*2); ctx.fill();
            ctx.restore();
        }
        // 2. Vòng tròn đỏ Clickbait & Mũi tên đỏ
        if (Math.random() > 0.5) {
            ctx.save(); ctx.translate(cx, cy);
            ctx.strokeStyle = "#ff0000"; ctx.lineWidth = 15;
            ctx.beginPath(); ctx.ellipse((Math.random()-0.5)*80, (Math.random()-0.5)*80, 100*scale, 150*scale, Math.random(), 0, Math.PI*2); ctx.stroke();
            ctx.font = `${80*scale}px Arial`; 
            let arrow = isFacingRight ? "⬅️" : "➡️";
            ctx.fillText(arrow, (isFacingRight ? 150 : -150)*scale, -50*scale);
            ctx.restore();
        }
        // 3. Sweat drops (Mồ hôi sợ hãi)
        if (Math.random() > 0.6) {
            ctx.font = `${60*scale}px Arial`; 
            ctx.fillText("💦", cx + (isFacingRight ? -60 : 60)*scale, cy - 90*scale);
        }
    };

    // --- VẼ BẢN NGANG ---
    window.ctx = ctxH; ctxH.filter = 'contrast(1.5) saturate(1.8)'; 
    let p1CloneH = Object.assign({}, window.p1, { x: 450, y: 850, scale: (window.p1.scale || 1) * 4.2, state: sP1, isFacingRight: true });
    ctxH.save(); ctxH.shadowBlur = 40; ctxH.shadowColor = `hsl(${hue1}, 100%, 80%)`; 
    if(typeof window.drawStickman === 'function') window.drawStickman(ctxH, p1CloneH); ctxH.restore();
    drawMemeOverlays(ctxH, 450, 850, 4.2, true);

    let p2CloneH = Object.assign({}, e1, { x: 1470, y: 850, scale: (e1.scale || 1) * 4.2, state: sP2, isFacingRight: false });
    ctxH.save(); ctxH.shadowBlur = 40; ctxH.shadowColor = `hsl(${hue2}, 100%, 80%)`; 
    if(typeof window.drawStickman === 'function') window.drawStickman(ctxH, p2CloneH); ctxH.restore();
    drawMemeOverlays(ctxH, 1470, 850, 4.2, false);
    ctxH.filter = 'none';

    // --- VẼ BẢN DỌC ---
    window.ctx = ctxV; ctxV.filter = 'contrast(1.5) saturate(1.8)';
    let p1CloneV = Object.assign({}, window.p1, { x: 540, y: 1550, scale: (window.p1.scale || 1) * 4.5, state: sP1, isFacingRight: true });
    ctxV.save(); ctxV.shadowBlur = 40; ctxV.shadowColor = `hsl(${hue1}, 100%, 80%)`; 
    if(typeof window.drawStickman === 'function') window.drawStickman(ctxV, p1CloneV); ctxV.restore();
    drawMemeOverlays(ctxV, 540, 1550, 4.5, true);

    let p2CloneV = Object.assign({}, e1, { x: 540, y: 700, scale: (e1.scale || 1) * 4.5, state: sP2, isFacingRight: false });
    ctxV.save(); ctxV.shadowBlur = 40; ctxV.shadowColor = `hsl(${hue2}, 100%, 80%)`; 
    if(typeof window.drawStickman === 'function') window.drawStickman(ctxV, p2CloneV); ctxV.restore();
    drawMemeOverlays(ctxV, 540, 700, 4.5, false);
    ctxV.filter = 'none';

    window.ctx = originalCtx;

    // --- TIÊU ĐỀ GIẬT GÂN ---
    const drawTextOverlays = (ctx, w, h) => {
        ctx.save(); ctx.translate(w/2, h/2); ctx.rotate(-0.1);
        ctx.font = `italic 900 ${h*0.25}px 'Arial Black', Impact`; ctx.textAlign = "center"; ctx.textBaseline = "middle";
        ctx.lineWidth = 35; ctx.strokeStyle = "#000"; ctx.strokeText("VS", 0, 0); ctx.fillStyle = "#fff"; ctx.fillText("VS", 0, 0); ctx.fillStyle = "#ff4757"; ctx.fillText("VS", -10, -10); 
        ctx.restore();

        ctx.font = `900 ${h*0.08}px 'Arial Black', Impact`; ctx.textAlign = "center";
        ctx.lineWidth = 25; ctx.strokeStyle = "#000"; ctx.strokeText(tTitle, w/2, h * 0.15);
        ctx.fillStyle = "#f1c40f"; ctx.fillText(tTitle, w/2, h * 0.15);

        let grad = ctx.createRadialGradient(w/2, h/2, h*0.4, w/2, h/2, Math.max(w,h));
        grad.addColorStop(0, "rgba(0,0,0,0)"); grad.addColorStop(1, "rgba(0,0,0,0.8)");
        ctx.fillStyle = grad; ctx.fillRect(0, 0, w, h);
    };

    drawTextOverlays(ctxH, 1920, 1080);
    drawTextOverlays(ctxV, 1080, 1920);
};
...
window.startRecording = function() {
    // [FIX] Nếu có video cũ đang quay dở (ví dụ bấm Replay liên tục), ép nó lưu lại trước!
    if (window.isRecording) {
        console.warn("Đang ép lưu Video cũ trước khi bắt đầu trận mới...");
    ...
    // ĐÓNG BĂNG 1.5 GIÂY ĐẦU ĐỂ NHÚNG TẤM MEME THUMBNAIL
    ...
    const finalizeRecordings = () => {
        stoppedCount++;
        // Đảm bảo phải gom đủ cả 2 video Ngang và Dọc mới xuất UI
        if (stoppedCount >= 2) { 
            setTimeout(() => {
                if (window.recordedChunksH.length === 0 || window.recordedChunksV.length === 0) {
                    console.log("Cảnh báo: Video rỗng. Vẫn hiển thị UI.");
                ...
                window.savedVideos.unshift({ 
                    ...
                    previewThumb: window.bakedThumbH.toDataURL("image/jpeg", 0.5) 
                });
                console.log("✅ Đã lưu xong Video. Đang cập nhật UI...");
                if (typeof window.updateVideoListUI === 'function') window.updateVideoListUI();
            }, 500); // Kéo dài thêm 1 tí cho máy tính xử lý Blob an toàn
        }
    };
...
window.stopRecording = function() { 
    ...
    window.isRecording = false; // Ngăn chặn vòng lặp gọi hàm liên tục
    console.log("🛑 Bắt đầu quá trình Stop Recording và xuất Video...");
    
    try { window.mediaRecorderH.requestData(); window.mediaRecorderV.requestData(); } catch(e){} 
    try { window.mediaRecorderH.stop(); } catch(e){}
    try { window.mediaRecorderV.stop(); } catch(e){}
...
window.captureFrames = function() {
    ...
    // 🔥 [AUTO CUT] TỰ ĐỘNG TẮT RECORD KHI TRẬN ĐẤU KẾT THÚC CHUẨN 150 FRAMES (Khoảng 2.5 giây sau K.O)
    if (window.gameOver && window.matchEndTimer > 150) {
        window.stopRecording();
        return;
    }
...
// ==========================================
// BẢNG ĐIỀU KHIỂN (POPUP LÊN CHÍNH GIỮA MÀN HÌNH SAU KHI QUAY)
// ==========================================
window.updateVideoListUI = function() {
    ...
    if (!container) { 
        ...
        // CSS ĐỂ BẢNG UI POPUP RA GIỮA MÀN HÌNH, NẰM TRÊN CÙNG
        container.style.cssText = "position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); z-index: 999999; padding: 25px; background: #0f172a; border-radius: 12px; border: 2px solid #00f3ff; width: 90%; max-width: 900px; color: #fff; font-family: 'Rajdhani', Arial, sans-serif; box-shadow: 0 10px 50px rgba(0,0,0,0.9);"; 
        document.body.appendChild(container); 
    }
    
    // Nút tắt để quay lại game
    let html = `<div style="position: absolute; top: 15px; right: 20px; font-size: 24px; cursor: pointer; color: #ff4757; font-weight: bold;" onclick="document.getElementById('video-list-container').style.display='none'">✖</div>`;
    html += `<h3 style="margin: 0 0 20px 0; color: #00f3ff; text-align: center; font-family: 'Teko', sans-serif; letter-spacing: 2px; font-size: 35px;">🎬 STUDIO ARCHIVE (${window.savedVideos.length} VIDEOS)</h3>
