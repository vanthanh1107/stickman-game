...
window.CREATOR_HANDLE = "Sticklom"; // Thay bằng "VIETNAM SPACEX" nếu bạn muốn
...
window.CELEB_LIST = [
    ...
    // Thêm User thường cho chân thực
...
const TOXIC_MSGS = [
    // English (Brainrot / Twitch slang)
    "Skill issue tbh 💀", "Bro is playing on a microwave 🍞", "AIN'T NO WAY HE MISSED THAT 😭",
    "Uninstall the game bro", "RIP BOZO 📉", "Absolute cinema 🍿🎬", "Bro thinks he is him 😭",
    "My grandma plays better 👵", "L L L L L L", "W W W W W W", "Bro's ping is higher than his IQ 💀",
    "Bro got lobotomized 🧠📉", "He needs some milk 🥛", "Caught in 4K 📸", "THE LORE IS CRAZY 😭",
    "Bro has anime protagonist aura ☢️", "Wait what language is this? 💀", "NAHHH jit trippin 💀",
    
    // Tiếng Việt (Toxic mặn mòi)
    "Đánh như cái máy khâu 🐔", "Xóa game đi bạn êi 🤡", "Thằng này bị ảo game à =)))", 
    "Quả xử lý cồng kềnh vãi chưởng 💀", "Out trình cmnr, nghỉ khỏe", "Mua acc à bạn?", 
    "Tuyển bạn này vào đánh giải... làng 🤣", "Đánh mù mẹ mắt tao rồi 👁️👄👁️", "Khóc đi 😭",
    "Gà 🐔🐔🐔", "Lùa gà à 🐧", "Phản xạ người già à bro?",
    
    // Korean
    "ㅋㅋ 봇인줄 (Tưởng là bot ㅋㅋ)", "이걸 지네 ㅉㅉ (Thế mà cũng thua)", "실화냐? 💀", 
    "피지컬 보소 ㄷㄷ (Nhìn kỹ năng kìa ㄷㄷ)", "삭제해라 게임 (Xóa game đi)", "ㅋㅋㅋㅋㅋㅋ",
    
    // Japanese
    "草 (Cười ỉa)", "何これｗｗｗ (Cái quái gì thế này www)", "弱すぎだろ (Yếu vãi)", "やばい💀",
    "wwwwwwww", "神プレイ (God play)",
    
    // Chinese
    "太菜了吧 😂 (Gà quá đi)", "主播别丢人了 (Đừng làm mất mặt nữa)", "666666", 
    "回家种田吧 (Về quê chăn bò đi)", "牛逼！(Trâu bò!)"
];

window._recentChatsMemory = []; // Bộ nhớ chống lặp comment

window.generateLiveChatEvent = function() {
    const r = (arr) => arr[Math.floor(Math.random() * arr.length)];
    let celeb = r(window.CELEB_LIST);
    
    // Bốc random 1 câu chat, nếu trùng với 12 câu gần nhất thì bốc lại (tối đa thử 10 lần)
    let msg = r(TOXIC_MSGS);
    let attempts = 0;
    while (window._recentChatsMemory.includes(msg) && attempts < 10) {
        msg = r(TOXIC_MSGS);
        attempts++;
    }
...
window.captureFrames = function() {
    ...
    if (renderNormalV) {
        ...
        ctxV.save();
        // Căn ra chính giữa video (chiều ngang 540, chiều dọc 960)
        ctxV.translate(540, 960); 
        ctxV.rotate(-0.15); // Nghiêng nhẹ một chút cho tự nhiên
        
        ctxV.font = "900 70px 'Arial Black', sans-serif";
        ctxV.textAlign = "center";
        ctxV.textBaseline = "middle";
        
        // 1. Viền đen rất mờ (giúp chữ không bị chìm lỉm nếu nền phía sau quá sáng)
        ctxV.lineWidth = 4;
        ctxV.strokeStyle = "rgba(0, 0, 0, 0.15)";
