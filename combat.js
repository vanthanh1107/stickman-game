...
function update() {
    ...
    allFighters.forEach(f => {
        ...
        // BẬT NÃO CHO NHÂN VẬT CHÍNH: TỰ ĐỘNG ĐÁNH VÀ XẢ COMBO Y HỆT ĐỐI THỦ
            ...
            if (closest && closest.hp > 0) {
                ...
                if (absDist > reach) {
                    ...
                    if (f.aiDelay <= 0) {
                        ...
                        // Tự động dùng kỹ năng nếu đủ thể lực
                        if (f.skill) {
...
function drawDragon(ctx, p, isTrail = false) {
    ...
    else { ctx.lineTo(head.x + 10, head.y + 5); ctx.lineTo(neck.x, neck.y); } ctx.stroke();
