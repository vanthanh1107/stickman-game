window.takeDamage = function(target, amount, color, isCrit = false, isWallBounce = false) {
    ...
    if (!target.isGuardBroken && !isWallBounce) {
        ...
        if (target.shieldBreak <= 0) {
            target.shieldBreak = 0; target.isGuardBroken = true; target.hitStun = 30; target.state = 'hurt';
            ...
            window.shockwaves.push({x: target.x, y: target.y - 30, r: 10, maxR: 120, color: "#ff4757", alpha: 1, speed: 10}); window.shakeScreen(20, 10);
        } else {
...
window.attack = function(attacker, potentialTargets) {
    ...
    if (hitTargets.length > 0) {
        ...
        hitTargets.forEach(defender => {
            ...
            let mod = attacker.dmgMod || 1;
            let baseDmg = 6 * mod * dmgMult * (1 + (attacker.comboHits * 0.05));
...
window.playerUseSkill = function(skillType) {
    ...
    if (skillType === 1 && window.p1.stamina >= 25) { 
        ...
        if (window.p1.skill && typeof window.p1.skill.actionCode1 === 'function') { window.p1.skill.actionCode1(window.p1, closestEnemy, gameContext); } 
            ...
            if (closestEnemy && Math.abs(closestEnemy.x - window.p1.x) < 120) { window.takeDamage(closestEnemy, 35 * (window.p1.dmgMod || 1), "#f1c40f", true); closestEnemy.vx = window.p1.isFacingRight?15:-15; }
        }
    }
    if (skillType === 2 && window.p1.stamina >= 50) { 
        ...
        if (window.p1.skill && typeof window.p1.skill.actionCode2 === 'function') { window.p1.skill.actionCode2(window.p1, closestEnemy, gameContext); } 
            ...
            if (closestEnemy && Math.abs(closestEnemy.x - window.p1.x) < 120) { closestEnemy.vy = -5; closestEnemy.onGround = false; window.takeDamage(closestEnemy, 40 * (window.p1.dmgMod || 1), "#1abc9c", true); }
        }
    }
    if (skillType === 3 && window.p1.stamina >= 100) { 
        ...
        window.triggerCinematic(window.p1, () => { 
            if (window.p1.skill && typeof window.p1.skill.actionCode3 === 'function') { window.p1.skill.actionCode3(window.p1, closestEnemy, gameContext); } 
                ...
                window.enemies.forEach(e => { if(Math.abs(e.x - window.p1.x) < 200) window.takeDamage(e, 100 * (window.p1.dmgMod || 1), "#f1c40f", true); }); 
            }
        });
    }
}
