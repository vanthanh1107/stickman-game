// ==========================================
// RECORDER.JS - V76.0 THE RETENTION GOD (ULTIMATE EDITION)
// ĐÃ FIX: Intro, Vỡ kính, Kill-Cam, Bảng Chat đụng Win Rate, Thông báo Follow ngang LIVE.
// UPDATE FACECAM: RAGE MODE (No Cam fallback), Thanh Goal 10K Likes bò dần, Vương miện Top 1.
// TÂM LÝ HỌC (NEW): Nhảy số COMBO khi chém nhau, Chớp viền máu (Bloody Vignette), Double Tap Bait.
// ==========================================
...
// ẢNH FACECAM STREAMER CÂU VIEW (Đổi thành link hỏng hoặc để nguyên để test chức năng RAGE MODE)
window.FACECAM_IMAGE_URL = "https://images.unsplash.com/photo-1611162617213-7d7a39e9b1d7?w=400&q=80";
...
// QUẢN LÝ SẠN & COMBO IN-GAME
window._baitTriggered = false; window._baitType = -1; window._baitStartTime = 0;
window._fakeCombo = 0; window._lastComboTime = 0; // HỆ THỐNG COMBO MỚI
...
window.startRecording = function() {
    ...
    window._fakeCombo = 0; window._lastComboTime = Date.now();
...
window.captureFrames = function() {
    ...
    if (renderNormalV) {
        ...
        // HIỆU ỨNG TÂM LÝ 1: BLOODY VIGNETTE (CHỚP ĐỎ VIỀN MÀN HÌNH KHI DÍNH ĐÒN MẠNH)
        if (audioPeak > 0.65 && !isKillCamActive) {
            ctxV.save();
            let bloodAlpha = (audioPeak - 0.65) * 2;
            let bloodGrad = ctxV.createRadialGradient(540, splitGameHeight/2, splitGameHeight*0.2, 540, splitGameHeight/2, splitGameHeight*0.7);
            bloodGrad.addColorStop(0, "rgba(255,0,0,0)");
            bloodGrad.addColorStop(1, `rgba(255,0,0,${bloodAlpha})`);
            ctxV.fillStyle = bloodGrad;
            ctxV.fillRect(0, 0, 1080, splitGameHeight);
            ctxV.restore();
        }

        // TÍNH TOÁN FAKE COMBO THEO ÂM THANH
        if (audioPeak > 0.5) { window._fakeCombo++; window._lastComboTime = Date.now(); }
        if (Date.now() - window._lastComboTime > 1500) window._fakeCombo = 0; // Reset combo nếu nghỉ quá 1.5s

        // VẼ HIỆU ỨNG COMBO
        if (window._fakeCombo > 3 && !isKillCamActive) {
            ctxV.save();
            let comboScale = 1.0 + (audioPeak * 0.5) + (Math.sin(Date.now()/50) * 0.1);
            let comboStr = `🔥 X${window._fakeCombo} COMBO!`;
            ctxV.translate(150, splitGameHeight/2);
            ctxV.rotate(-0.1);
            ctxV.scale(comboScale, comboScale);
            ctxV.textAlign = "left"; ctxV.textBaseline = "middle";
            ctxV.font = "italic 900 45px 'Arial Black', sans-serif";
            ctxV.lineWidth = 8; ctxV.strokeStyle = "#000"; ctxV.strokeText(comboStr, 0, 0);
            ctxV.fillStyle = (window._fakeCombo > 10) ? "#ff0050" : "#ffeb3b"; 
            ctxV.fillText(comboStr, 0, 0);
            ctxV.restore();
        }

        // HIỆU ỨNG TÂM LÝ 2: CHIM MỒI DOUBLE TAP Ở GÓC TRÁI DƯỚI
        if (!isKillCamActive && elapsed > 3000) {
            ctxV.save();
            let baitScale = 1 + Math.sin(Date.now() / 150) * 0.15;
            ctxV.translate(220, splitGameHeight - 80);
            ctxV.scale(baitScale, baitScale);
            ctxV.font = "bold 24px Arial";
            ctxV.textAlign = "center"; ctxV.textBaseline = "middle";
            ctxV.fillStyle = "rgba(0,0,0,0.5)";
            if(ctxV.roundRect) { ctxV.beginPath(); ctxV.roundRect(-120, -20, 240, 40, 20); ctxV.fill(); }
            ctxV.fillStyle = "#fff";
            ctxV.fillText("🤍 DOUBLE TAP!", 0, 2);
            ctxV.restore();
        }
        ...
        if (!isOutroActive) {
            ...
            // =========================================================
            // CHAT BOX KHUNG CAO HƠN & TÍCH HỢP WATERMARK / FACECAM CAO CẤP
            // =========================================================
            ...
            // 3. FACECAM GÁI XINH & KHU VỰC TƯƠNG TÁC ĐỈNH CAO
            if (!window._facecamImg) {
                window._facecamImg = new Image(); window._facecamImg.crossOrigin = "Anonymous";
                window._facecamImg.src = window.FACECAM_IMAGE_URL || "https://images.unsplash.com/photo-1611162617213-7d7a39e9b1d7?w=400&q=80"; 
            }
            
            ctxV.save();
            let camW = 200; let camH = 280;
            let camX = boxWidth/2 - camW/2 - 20; 
            let camY = -boxHeight/2 + camH/2 + 75; 
            
            if (audioPeak > 0.5) { camX += (Math.random() - 0.5) * 12; camY += (Math.random() - 0.5) * 12; }
            ctxV.translate(camX, camY);

            // A. HIỆU ỨNG SÓNG ÂM THANH TOẢ RA TỪ FACECAM KHI COMBAT
            if (audioPeak > 0.4) {
                ctxV.save();
                let pulseSize = 1 + (audioPeak * 0.5);
                ctxV.scale(pulseSize, pulseSize);
                ctxV.beginPath();
                if(ctxV.roundRect) ctxV.roundRect(-camW/2, -camH/2, camW, camH, 15); else ctxV.rect(-camW/2, -camH/2, camW, camH);
                ctxV.fillStyle = "rgba(0, 243, 255, 0.3)";
                ctxV.fill();
                ctxV.lineWidth = 4;
                ctxV.strokeStyle = "rgba(0, 243, 255, 0.6)";
                ctxV.stroke();
                ctxV.restore();
            }

            // B. MẶT NẠ FACECAM (Nếu ảnh lỗi, nó vẫn sẽ hiện nền đen ngầu)
            ctxV.beginPath();
            if(ctxV.roundRect) ctxV.roundRect(-camW/2, -camH/2, camW, camH, 15); else ctxV.rect(-camW/2, -camH/2, camW, camH);
            ctxV.clip();
            ctxV.fillStyle = "#111"; // Nền đen dự phòng
            ctxV.fill();

            // Nếu ảnh load thành công thì vẽ ảnh
            if (window._facecamImg && window._facecamImg.complete && window._facecamImg.naturalWidth > 0) {
                let zoom = 1.0 + (audioPeak * 0.25);
                ctxV.save(); ctxV.scale(zoom, zoom);
                ctxV.drawImage(window._facecamImg, -camW/2, -camH/2, camW, camH);
                ctxV.restore();
            } else {
                // NẾU ẢNH LỖI -> HIỂN THỊ CHẾ ĐỘ "NO CAM" TRY-HARD
                ctxV.fillStyle = "#ff0050";
                ctxV.font = "900 30px 'Arial Black'"; ctxV.textAlign = "center"; ctxV.textBaseline = "middle";
                ctxV.fillText("📷", 0, -20);
                ctxV.fillStyle = "#fff";
                ctxV.font = "bold 20px Arial";
                ctxV.fillText("RAGE MODE", 0, 20);
            }
            
            // C. VIỀN FACECAM NHẤP NHÁY
            ctxV.lineWidth = 5;
            ctxV.strokeStyle = (Math.floor(Date.now()/200)%2===0) ? "#ff0055" : "#00f3ff";
            ctxV.stroke();

            // D. THÊM THANH "LIKE GOAL" CÂU TƯƠNG TÁC (NẰM DƯỚI ĐÁY FACECAM)
            let goalH = 35;
            let goalY = camH/2 - goalH;
            ctxV.fillStyle = "rgba(0,0,0,0.85)";
            ctxV.fillRect(-camW/2, goalY, camW, goalH); 
            
            let fakeProgress = (Date.now() % 60000) / 60000; 
            ctxV.fillStyle = "#ffeb3b";
            ctxV.fillRect(-camW/2, goalY, camW * fakeProgress, goalH);
            
            ctxV.fillStyle = (fakeProgress > 0.5) ? "#000" : "#fff"; 
            ctxV.font = "900 14px 'Arial Black'"; ctxV.textAlign = "center";
            ctxV.fillText("🔥 GOAL: 10K LIKES", 0, goalY + goalH/2 + 1);

            // E. CÚ CỘP MÁC "TOP 1 DONATE" (NẰM TRÊN ĐỈNH FACECAM)
            ctxV.fillStyle = "rgba(10, 15, 30, 0.9)";
            if(ctxV.roundRect) { ctxV.beginPath(); ctxV.roundRect(-camW/2 + 5, -camH/2 + 5, camW - 10, 30, 15); ctxV.fill(); }
            else { ctxV.fillRect(-camW/2 + 5, -camH/2 + 5, camW - 10, 30); }
            
            ctxV.fillStyle = "#ffeb3b"; ctxV.font = "bold 13px Arial"; ctxV.textAlign = "center";
            ctxV.fillText("👑 Top 1: Bảnh", 0, -camH/2 + 20);

            ctxV.restore(); 

            // 4. CHAT TEXT TEXT (ĐÃ TRỪ HAO CHIỀU RỘNG ĐỂ KHÔNG ĐÈ LÊN FACECAM)
