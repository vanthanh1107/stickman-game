// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 28.0 [THE GOD OF IMPACT]
// [NEW] KINETIC TYPOGRAPHY: Số sát thương văng vật lý, bật nảy 3D cực đã mắt.
// [NEW] SUBWOOFER BASS-DROP: Nhòe màn hình 1 frame tạo ảo giác sóng âm chấn động.
// [NEW] CINEMATIC SLAM: Viền đen sập sầm xuống khóa chặt thị giác người xem.
// [NEW] SCREEN-TEARING: Xé rách không gian ngang màn hình khi có đòn K.O.
// ==========================================
...
window.actionCamOffsetY = 0; 
window.dynamicBlur = 0; 
...
// Biến Đạo diễn / Visuals 28.0
window.mangaSfx = []; 
window.dimensionCracks = []; 
window.inkSplatters = []; 
window.invertFrames = 0; 
window.noiseCanvas = null; 
window.timeScale = 1.0; 
window.targetTimeScale = 1.0; 
window.camVelocityX = 0; 
window.camDriftX = 0; 
window.camDriftY = 0;

// [VIRAL 28.0] BIẾN THẦN LỰC THỊ GIÁC
window.foregroundDebris = []; 
window.lethalVoid = 0; 
window.impactAberration = 0; 
window.bassDropFrames = 0; // Nhòe rung sóng âm
window.screenTearing = 0; // Xé rách màn hình
...
window.initGameEngine = function() {
    ...
    for (let i = 0; i < imgData.data.length; i += 4) {
        ...
        imgData.data[i+3] = Math.random() * 25; 
    }
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce, attackerDirRight = true) {
    ...
    if (target.state === 'dash' || target.state === 'dash_back') {
        if (target.dashTimer > 2 && target.dashTimer < 14) {
            ...
            // [VIRAL 28.0] Kinetic Text cho Dodge
    ...
    if (finalDmg > 0) {
        ...
        // [VIRAL 28.0] KINETIC TYPOGRAPHY (Số máu văng vật lý, co giãn 3D)
        ...
        let sparkCount = isCrit ? 15 : 6;
        let baseDirX = attackerDirRight ? 1 : -1;
        for(let i=0; i<sparkCount; i++) { 
            let vx = baseDirX * (Math.random() * 20 + 5) + (Math.random()-0.5)*5; 
            let vy = (Math.random()-0.5)*15 - 5;
            window.impactSparks.push({ x: target.x, y: target.y - 40, vx: vx, vy: vy, life: 25 + Math.random()*15, maxLife: 40, color: color || "#f1c40f" }); 
        }
        ...
        if (isCrit) {
            ...
            setTimeout(() => { window.targetZoom = 1.1; }, 100); 
            
            window.impactAberration = 15;
            window.spawnForegroundDebris(target.x, target.y, attackerDirRight);

            // [VIRAL 28.0] BASS DROP CỤC BỘ KHI CHÍ MẠNG
        ...
        if (target.hp <= 0) {
            ...
            if (isNearWall && isCrit && typeof window.triggerStageTransition === 'function') { window.triggerStageTransition(target); } 
                ...
                if (isCrit) {
                    ...
                    window.lethalVoid = 80;
                    window.impactAberration = 40; 
                    
                    // [VIRAL 28.0] BASS DROP, SCREEN TEAR VÀ CINEMATIC SLAM KHI K.O
                    window.bassDropFrames = 6; // Nhòe rung cực lâu
                    window.screenTearing = 15; // Xé hình chớp giật
                    window.cinemaBarsHeight = 250; // Sập viền đen tức thời khóa khung nhìn

                    window.spawnForegroundDebris(target.x, target.y, attackerDirRight);
                    window.spawnForegroundDebris(target.x, target.y, attackerDirRight); 

                    window.hitStopFrames = 30; 
                    window.shakeScreen(60, 45); 
                    window.vhsGlitchTimer = 45; window.triggerDimensionShatter(); 
                    window.targetTimeScale = 0.02; setTimeout(()=>{ window.targetTimeScale = 1.0; window.cameraTilt = 0; window.cinemaBarsHeight = 0; }, 2500); 
                } else { window.hitStopFrames = 15; window.shakeScreen(30, 25); window.targetZoom = 1.5; window.actionCamOffsetY = -40; window.targetTimeScale = 0.2; window.impactAberration = 20; window.cinemaBarsHeight = 150; setTimeout(()=>{window.targetTimeScale = 1.0; window.cinemaBarsHeight = 0; }, 1000); }
...
window.attack = function(attacker, targetGroup) {
    ...
    if (MathDist > reach && !isFinisher) { 
        ...
        // [VIRAL 28.0] Hơi co viền khi lướt tạo cảm giác xé gió
    ...
    if (isFinisher) {
        ...
        if (!window.gameOver) window.cinemaBarsHeight = 120; // Sập bẫy viền phụ
    } else {
...
window.update = function() {
    ...
    // Nhả viền đen từ từ nếu không phải gameOver
    ...
    // [VIRAL 28.0] Cập nhật Kinetic Typography Animation
    for (let i = window.floatingTexts.length - 1; i >= 0; i--) { 
        ...
        if (t.life !== undefined) { 
            ...
            if (t.scale !== undefined && t.targetScale !== undefined) {
                t.scale += (t.targetScale - t.scale) * 0.2; // Overshoot giật nảy
            }
...
window.draw = function() {
    ...
    // [VIRAL 28.0] SUBWOOFER BASS-DROP FILTER
    ...
    // [VIRAL 28.0] SCREEN TEARING GLITCH KHI K.O (Xé rách hình ảnh)
    window.ctx.save();
    ...
    try {
        ...
        } else if (!window.bassDropFrames) { // Không đè filter nếu đang có bass drop
        ...
        // [VIRAL 28.0] VẼ KINETIC TYPOGRAPHY (CHỮ 3D VĂNG QUÁN TÍNH)
        window.floatingTexts.forEach(t => { 
            window.ctx.save();
            window.ctx.translate(t.x, t.y);
            
            // Xoay nhẹ theo quán tính văng
            if (t.rot) window.ctx.rotate(t.rot);
            
            // Áp dụng scale động để bóp méo 3D
            let s = t.scale || 1.0;
            ...
            // Mẹo vẽ text đậm viền
            window.ctx.lineWidth = 3; window.ctx.strokeStyle = "#000";
            window.ctx.strokeText(t.text, 0, 0);
            window.ctx.fillText(t.text, 0, 0); 
        ...
        // [VIRAL 28.0] CINEMATIC SLAM LÚC BỊ ĐÁNH & DASH
...
// ==========================================
// 7. [VIRAL 28.0] AFTER EFFECTS TIME-RAMPING & UNLOCKED 120Hz LOOP
// ==========================================
