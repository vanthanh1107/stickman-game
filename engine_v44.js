// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 38.0 [THE PERFECT FRAMING V2]
// [FIXED] Lỗi mất chân nhân vật khi Combo: Tự động chúc ống kính xuống (Y-Compensation) khi Zoom cận.
// [FIXED] Giảm nhẹ độ Zoom tối đa của Combo Escalation để giữ không gian khoáng đạt, không bị ngộp.
// [BASE] Đầy đủ 360 Orbit Camera, Kinetic Text, Lò xo 3D, Lethal Void, Aberration.
// ==========================================
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce, attackerDirRight = true) {
    ...
    if (target.state === 'block') { 
        if (target.stamina < 15) {
            ...
            window.targetZoom = 1.35; window.dynamicBlur = 10; window.hitStopFrames = 20; window.impactAberration = 25; window.chromaTimer = 20; window.vhsGlitchTimer = 20; window.spawnForegroundDebris(target.x, target.y, attackerDirRight); 
        } 
    ...
    if (finalDmg > 0) {
        ...
        window.impactLights.push({x: target.x, y: target.y - 40, color: color || (isCrit ? "#ff4757" : "#f1c40f"), life: isCrit ? 25 : 12, maxLife: isCrit ? 25 : 12, isCrit: isCrit});

        if (isCrit) {
            window.screenFlash = 0.5; window.impactFrameCount = 2; 
            // [VIRAL 38.0] Giảm nhẹ độ giật Zoom khi đánh thường để tránh nhức mắt
            window.targetZoom = 1.3; setTimeout(() => { window.targetZoom = 1.05; }, 100); 
        ...
        } else if (Math.random() > 0.5) { window.spawnMangaSFX(target.x, target.y - 50, false); window.targetZoom = 1.1; setTimeout(() => { window.targetZoom = 1.05; }, 50); window.impactAberration = 4; }
        ...
        if (target.hp <= 0) {
            ...
            if (isNearWall && isCrit && typeof window.triggerStageTransition === 'function') { window.triggerStageTransition(target); } 
                ...
                if (isCrit) {
                    window.screenFlash = 0.6; window.invertFrames = 3; window.speedLinesAlpha = 1.0; window.spawnLensFlare(target.x, target.y - 40, "#ff003c", 5.0);
                    ...
                    window.targetZoom = 1.8; window.actionCamOffsetX = (target.isFacingRight ? 120 : -120); window.actionCamOffsetY = -60; window.dynamicBlur = 30; window.cameraTilt = (Math.random() > 0.5 ? 1 : -1) * 0.18; 
                    ...
                    window.isSpinningCam = true; window.orbitFocusX = (target.x + window.p1.x) / 2; window.targetTimeScale = 0.01; 
                    setTimeout(()=>{ window.targetTimeScale = 1.0; window.cameraTilt = 0; window.isSpinningCam = false; window.targetCamOrbitAngle = 0; }, 3500); 
                } else { window.hitStopFrames = 15; window.shakeScreen(30, 25); window.targetZoom = 1.4; window.actionCamOffsetY = -40; window.targetTimeScale = 0.2; window.impactAberration = 20; setTimeout(()=>{window.targetTimeScale = 1.0; }, 1000); }
...
window.attack = function(attacker, targetGroup) {
    ...
    let extraLag = Math.min(10, attacker.comboHits || 0);
    window.hitStopFrames = isCrit ? (10 + extraLag) : (2 + Math.floor(extraLag/2));
...
window.update = function() {
    ...
    if (window.impactLights) { window.impactLights.forEach(l => l.life--); window.impactLights = window.impactLights.filter(l => l.life > 0); }
    ...
    // [VIRAL 38.0] COMBO ESCALATION CAMERA + Y-COMPENSATION
    let maxComboHits = Math.max(window.p1 ? window.p1.comboHits || 0 : 0, closest ? closest.comboHits || 0 : 0);
    let comboZoomBonus = Math.min(0.15, maxComboHits * 0.015); // Đã giảm bonus zoom để không bị quá cận
    let comboTiltBonus = maxComboHits > 4 ? Math.sin(window.matchTimer * 0.3) * (Math.min(0.05, maxComboHits * 0.005)) : 0;

    window.targetCamX = (window.canvas.width / 2) - window.orbitFocusX + window.actionCamOffsetX + (actionPan * 0.6); // Mượt lia ngang
    ...
    if (window.gameOver && closest) {
        ...
        let dynamicZoom = 1.25 - (distance / 800) * 0.35; 
        ...
        window.targetTilt = 0.025 * Math.sin(window.slowMoTimer * 0.1) + comboTiltBonus;
    }

    // [VIRAL 38.0] TỰ ĐỘNG CHÚC MÁY QUAY XUỐNG KHI ZOOM GẦN (Giữ nguyên gót chân)
...
window.draw = function() {
    ...
    if (window.isLoading) {
        ...
        window.ctx.fillStyle = "#00f3ff"; window.ctx.font = "italic 900 40px Arial"; window.ctx.textAlign = "center"; window.ctx.shadowBlur = 15; window.ctx.shadowColor = "#00f3ff"; window.ctx.fillText("PERFECT FRAMING INITIALIZING...", cx, cy - 50);
        ...
        window.ctx.fillStyle = "#fff"; window.ctx.font = "bold 16px monospace"; window.ctx.fillText(`Y-COMPENSATION SYNCING... ${Math.floor(window.loadProgress)}%`, cx, cy + 60);
        return; 
    }
    ...
    try {
        ...
        if (window.shakeTime > 0) {
            ...
            window.ctx.rotate((Math.random() - 0.5) * 0.02 * window.shakeMag * (Math.PI / 180));
        }
        ...
        if (window.impactLights && window.impactLights.length > 0) {
            window.ctx.save(); window.ctx.globalCompositeOperation = 'screen';
            window.impactLights.forEach(light => {
                let pr = project3D(light); let prog = light.life / light.maxLife; window.ctx.globalAlpha = prog * (light.isCrit ? 0.9 : 0.6);
                let r = (light.isCrit ? 600 : 300) * Math.sin(prog * Math.PI) * pr.drawScale; 
                if (r > 0) { let grad = window.ctx.createRadialGradient(pr.drawX, pr.drawY, 0, pr.drawX, pr.drawY, r); grad.addColorStop(0, light.color); grad.addColorStop(1, 'rgba(0,0,0,0)'); window.ctx.fillStyle = grad; window.ctx.beginPath(); window.ctx.arc(pr.drawX, pr.drawY, r, 0, Math.PI*2); window.ctx.fill(); }
            });
            window.ctx.restore();
        }
        ...
        // [VIRAL 38.0] Vẽ Flash chớp tắt nhưng trong suốt mờ để ko che không gian
        if (window.screenFlash > 0) { 
            window.ctx.globalCompositeOperation = 'screen'; 
            window.ctx.fillStyle = `rgba(255, 255, 255, ${Math.min(window.screenFlash, 0.6)})`; 
            window.ctx.fillRect(0, 0, window.canvas.width, window.canvas.height); 
            window.ctx.globalCompositeOperation = 'source-over'; 
        }
