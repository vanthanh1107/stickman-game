// ==========================================
// RECORDER.JS - V71.1 MOBILE 60FPS OPTIMIZED (DIRECTOR'S CUT)
// ĐÃ FIX: Giật lag trên điện thoại. Tối ưu cực độ CPU/GPU.
// CẬP NHẬT: Loại bỏ lệnh filter nặng, Cache Pattern CRT, Tối ưu hóa Afterimages.
// CẢM XÚC: Vẫn giữ nguyên độ Epic nhưng mượt mà tuyệt đối kể cả trên máy yếu.
...
// ====================================================================
// PROCEDURAL ENGINE (ĐẠO DIỄN KỊCH BẢN ĐA NGÔN NGỮ)
// ====================================================================
const THEMES = [
...
const LORES = [
    ...
    { chapter: "最终章：末路", p2Sub: "认清你自己的位置，小子。", p1Sub: "你的时代今晚终结。" },
    { chapter: "CHƯƠNG 1: KẺ PHẢN BỘI", p2Sub: "Ta đã dạy ngươi mọi thứ...", p1Sub: "Và giờ ta sẽ kết liễu ông." },
    { chapter: "CHƯƠNG CUỐI: TÀN CUỘC", p2Sub: "Bỏ cuộc đi. Ngươi chẳng còn gì cả.", p1Sub: "Ta vẫn còn sự phẫn nộ." }
];
...
// ====================================================================
// ĐẠO DIỄN: THE DEFINITIVE MASTERPIECE (MOBILE OPTIMIZED)
// ====================================================================
window.drawProceduralIntro = function(ctx, w, h, progress) {
    ...
    const drawCinematicDust = (theme, opacity) => {
        ...
        for(let i=0; i<15; i++) { // Tối ưu: Giảm từ 40 xuống 15
    ...
    // TỐI ƯU 1: CACHE LẠI MẪU CRT ĐỂ KHÔNG VẼ LOOP GÂY LAG
    const drawCRTScanlines = () => {
        if (!window._cachedGradients.crt) {
            let c = document.createElement('canvas'); c.width = 4; c.height = 8; let cx = c.getContext('2d');
            cx.fillStyle = 'rgba(0,0,0,0.15)'; cx.fillRect(0,0,4,4);
            window._cachedGradients.crt = ctx.createPattern(c, 'repeat');
        }
        ctx.save(); ctx.fillStyle = window._cachedGradients.crt; ctx.fillRect(-w, -h, w*3, h*3); ctx.restore();
    };

    // TỐI ƯU 2: HÀM VẼ NHÂN VẬT THUẦN (XÓA CHỮ VÀ BỎ FILTER NẶNG MÁY)
    const drawFighterPure = (charObj, cx, cy, forcedState, isFacingRight, theme, isTrail = false) => {
        ...
        // Vẽ Aura nhẹ hơn, bỏ filter css
        if (!isTrail) {
            ctx.save(); ctx.globalAlpha = 0.3 + Math.sin(Date.now()*0.02)*0.1; 
            ctx.shadowBlur = 20; ctx.shadowColor = theme.aura; ctx.scale(1.1, 1.1);
    ...
    // --- ACT 2: STROBE FLASH (TỐI ƯU RGB GLITCH BẰNG ĐỔ BÓNG VẬT LÝ, BỎ FILTER) ---
    if (act === 2) {
        ...
        // Vẽ 3 lớp mô phỏng RGB tách màu mượt nhẹ
        ctx.save(); ctx.globalAlpha = 0.5;
        ctx.translate(-15, 0); drawFighterPure(targetChar, 0, 0, 'dash', isFacing, tTheme, true); ctx.restore();
        ctx.save(); ctx.globalAlpha = 0.5;
        ctx.translate(15, 0); drawFighterPure(targetChar, 0, 0, 'dash', isFacing, tTheme, true); ctx.restore();
        drawFighterPure(targetChar, 0, 0, 'dash', isFacing, tTheme, false);
    ...
    if (act === 0) {
        ...
        if (localProg > 0.8) cutShake = (Math.random() - 0.5) * 60; 
    }
    ...
    // TỐI ƯU CACHE BACKGROUND
    let bgKey = "bg_" + (act === 0 ? prm.themeP2.id : prm.themeP1.id);
    if (!window._cachedGradients[bgKey]) {
        let tTheme = act === 0 ? prm.themeP2 : prm.themeP1;
        let bgGrad = ctx.createRadialGradient(0, 0, 0, 0, 0, w);
        bgGrad.addColorStop(0, tTheme.c1); bgGrad.addColorStop(1, tTheme.c2);
        window._cachedGradients[bgKey] = bgGrad;
    }
    ctx.fillStyle = window._cachedGradients[bgKey]; ctx.fillRect(-w, -h, w*2, h*2);
    
    if (act === 3) {
        ctx.save();
        ctx.strokeStyle = prm.themeP1.aura; ctx.globalAlpha = localProg * 0.5; ctx.lineWidth = 3;
        for(let i=0; i<20; i++) { // Giảm số tia focus
    ...
    let activeTheme = act === 0 ? prm.themeP2 : prm.themeP1;
    drawCinematicDust(activeTheme, 0.4);

    if (act === 0) {
        drawFighterPure(e1, 0, 100, 'idle', false, prm.themeP2);
    } 
    else if (act === 1) {
        drawFighterPure(window.p1, 0, 100, 'cast', true, prm.themeP1);
    } 
    else if (act === 3) {
        ...
        // TỐI ƯU 3: RÚT GỌN AFTERIMAGE CÒN 2 (Mượt hơn gấp đôi)
        for(let t=1; t<=2; t++) {
            let prevProg = Math.max(0, localProg - t*0.06);
            let prevEaseIn = Math.pow(prevProg, 4);
            let trP1 = -400 + (prevEaseIn * 280); 
            let trP2 = 400 - (prevEaseIn * 280);
            ctx.globalAlpha = 0.4 / t; 
            drawFighterPure(e1, trP2, 100, 'attack', false, prm.themeP2, true);
            drawFighterPure(window.p1, trP1, 100, 'attack', true, prm.themeP1, true);
        }
        ctx.globalAlpha = 1.0;

        drawFighterPure(e1, pushP2, 100, 'attack', false, prm.themeP2);
        drawFighterPure(window.p1, pushP1, 100, 'attack', true, prm.themeP1);
        ...
        // VÒNG SÓNG XUNG KÍCH (SHOCKWAVE) GIẢM TẢI
        ctx.translate(0, 50);
        for(let rIdx = 1; rIdx <= 2; rIdx++) {
            let ringProg = Math.max(0, localProg - rIdx * 0.1);
            if (ringProg > 0) {
                ctx.beginPath(); ctx.arc(0, 0, ringProg * 700, 0, Math.PI * 2);
                ctx.lineWidth = 10 * (1 - ringProg); ctx.strokeStyle = prm.themeP1.aura;
                ctx.globalAlpha = (1 - ringProg) * 0.8; ctx.stroke();
            }
        }
        
        // TIA LỬA BỎ BLUR ĐỂ TRÁNH LAG MÁY YẾU
        for(let i=0; i<15; i++) {
            let ang = Math.random() * Math.PI * 2; let speed = Math.random(); 
            let dist = localProg * (400 + speed * 1000); let size = (1 - localProg) * (5 + speed * 25); 
    ...
    // =================================================================
    // LỚP OVERLAY AN TOÀN TUYỆT ĐỐI (KHÔNG BAO GIỜ BỊ TAY CHÂN CHE MẤT)
    // =================================================================

    if(!window._cachedGradients.vig) {
        ...
        window._cachedGradients.vig = vigGrad;
    }
    ctx.fillStyle = window._cachedGradients.vig; ctx.fillRect(0, 0, w, h);
    ...
    // --- TÊN NHÂN VẬT (NỔI Ở LỚP TRÊN CÙNG) ---
    if (act === 0 || act === 1) {
        ctx.save();
        let cName = act === 0 ? realP2Name : realP1Name;
        let cTheme = act === 0 ? prm.themeP2 : prm.themeP1;
        
        ctx.globalAlpha = Math.min(1, localProg * 5); 
        ctx.textAlign = "center"; ctx.textBaseline = "middle";
        ctx.font = "900 42px 'Arial Black', sans-serif"; 
        
        let overlayNameY = barHeight + 50; 
        
        ctx.shadowColor = cTheme.aura; ctx.shadowBlur = 15; // Giảm blur
        ctx.lineWidth = 7; ctx.strokeStyle = "#000"; 
        ctx.strokeText(cName, w/2, overlayNameY); 
        ctx.shadowBlur = 0; ctx.fillStyle = "#fff"; ctx.fillText(cName, w/2, overlayNameY);
        
        ctx.fillStyle = cTheme.aura;
        ctx.fillRect(w/2 - 50, overlayNameY + 30, 100, 4);
        ctx.restore();
    }
    ...
    // --- BÙM! ---
    if (act === 3 && localProg > 0.8) {
        let flashP = (localProg - 0.8) * 5; 
        ctx.fillStyle = `rgba(255, 255, 255, ${1 - flashP})`; ctx.fillRect(0,0,w,h);
        ctx.save(); ctx.translate(w/2, h/2); let pop = 1 + (flashP * 5); ctx.scale(pop, pop);
