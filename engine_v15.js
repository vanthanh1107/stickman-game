// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 14.0 [ULTRA PURE-CORE]
// [FIXED 100%: LỖI DÍNH VỆT MÀU (GHOSTING) DO LỖI "SATURATION" CỦA TRÌNH DUYỆT]
// [TÍNH NĂNG MỚI: TRUE SIN CITY FINISHER, HARD-RESET VRAM, NEON PARTICLES]
// ==========================================
...
window.spawnParticles = function(x, y, color, isCrit = false) { 
    ...
    for(let i=0; i<count; i++) { let angle = Math.random() * Math.PI * 2; let speed = Math.random() * (isCrit?12:6) + 2; window.particles.push({ x: x, y: y - 30, vx: Math.cos(angle)*speed, vy: Math.sin(angle)*speed, life: 20, maxLife: 20, color: color, size: Math.random() * 4 + 2 }); } 
}
...
window.triggerTimeStop = function(frames, caster) { 
    window.timeStopTimer = frames; window.timeStopCaster = caster; window.applyFilter('invert', frames); window.playSound(100, 'square', 1.0, 1.0, true); window.shakeScreen(20, 15); 
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce) {
    ...
    if (target.state === 'block') { 
        if (target.stamina < 15) {
            ...
            window.hitStopFrames = 15; window.chromaTimer = 15; window.targetZoom = 1.3; window.vhsGlitchTimer = 15;
        } 
    ...
    if (finalDmg > 0) {
        ...
        if (isCrit) {
            for(let i=0; i<8; i++) {
                window.impactSparks.push({ x: target.x, y: target.y - 40, vx: (Math.random()-0.5)*18, vy: (Math.random()-0.5)*18, life: 20 + Math.random()*15, maxLife: 35, color: color || "#f1c40f" });
            }
        }
...
window.draw = function() {
    ...
    // ---------------------------------------------------
    // [FIX TỐI THƯỢNG 14.0]: HARD-RESET 100% MA TRẬN KẾT XUẤT 
    // Trị tận gốc các lỗi bóng mờ do rác VRAM trên 120Hz
    // ---------------------------------------------------
    ...
    if (window.isLoading) {
        ...
        window.ctx.fillStyle = "#00f3ff"; window.ctx.font = "italic 900 40px Arial"; window.ctx.textAlign = "center"; window.ctx.shadowBlur = 15; window.ctx.shadowColor = "#00f3ff"; window.ctx.fillText("INITIALIZING PURE-CORE 14.0...", cx, cy - 50);
        ...
        window.ctx.fillStyle = "#fff"; window.ctx.font = "bold 16px monospace"; window.ctx.fillText(`COMPILING SHADERS & VRAM... ${Math.floor(window.loadProgress)}%`, cx, cy + 60);
        return; 
    }
    ...
    try {
        ...
        // =====================================
        // TRUE SIN CITY FINISHER (AN TOÀN TUYỆT ĐỐI)
        // Chỉ áp dụng filter xám đen cho Background và Character
        // =====================================
       ...
       if (window.envHazards && window.envHazards.length > 0) {
        ...
        // =====================================
        // TẮT TẤT CẢ BỘ LỌC, TIẾN HÀNH VẼ CHƯỚNG NGẠI VÀ VFX RỰC RỠ 
        // =====================================
        window.ctx.filter = 'none';
        ...
        }); window.ctx.globalAlpha = 1.0; window.ctx.shadowBlur = 0; // Fix bóng thừa
        ...
        // =====================================
        // SAFE CHROMA ABERRATION (KHÔNG DÙNG OFFCANVAS NỮA ĐỂ CHỐNG TRÀN VÀ LAG)
        // =====================================
        if (window.chromaTimer > 0) {
            let shift = window.chromaTimer * 1.5; 
            window.ctx.setTransform(1, 0, 0, 1, 0, 0);
            window.ctx.globalCompositeOperation = 'screen';
            window.ctx.fillStyle = `rgba(255, 0, 0, 0.1)`;
            window.ctx.fillRect(-shift, 0, window.canvas.width, window.canvas.height);
            window.ctx.fillStyle = `rgba(0, 255, 255, 0.1)`;
            window.ctx.fillRect(shift, 0, window.canvas.width, window.canvas.height);
            window.ctx.globalCompositeOperation = 'source-over'; 
        }
