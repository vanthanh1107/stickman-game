// ==========================================
// RECORDER.JS - V67.4 FINAL PERFECTION (BUTTER SMOOTH MAX)
// Tích hợp: Intro Manga, VTuber, HUD Đẹp/Sửa đè tên, Binge-Watch Grid
// Nâng cấp: Frame Pacing 16.66ms, Alpha Blending Glitch, Text MaxWidth
// ==========================================
...
window._lastFrameTime = performance.now(); // Chuẩn hóa thời gian ban đầu
...
window.startRecording = function() {
    ...
    window._lastFrameTime = performance.now(); // Reset timing start
...
window.captureFrames = function() {
    ...
    // [V67.4 ĐỒNG BỘ FPS HOÀN HẢO] Tính toán chuẩn xác 16.66ms (60Hz) chống trôi frame
    let now = performance.now();
    let elapsed = now - window._lastFrameTime;
    if (elapsed < 15.5) return; // Cho phép chênh lệch nhẹ để không bị miss frame
    window._lastFrameTime = now - (elapsed % 16.66); // Bù trừ phần nghìn giây tránh tích lũy độ trễ
    ...
    // [V67.4] Chuyển Glitch sang Alpha Blending (Siêu nhẹ thay vì dùng Composite Screen)
    if (shouldGlitch) {
        let glitchStr = ((audioPeak - 0.75) * 30) | 0;
        ctxV.globalAlpha = 0.4; // Dùng Alpha cực kỳ tối ưu
    ...
    // ==========================================
    // 5. HUD SYSTEM (MÁU, STAMINA, AVATAR) [V67.4 FIX TÊN NHÂN VẬT ĐẸP MẮT]
    // ==========================================
    ...
    if (window.p1 && !window.gameOver && window.introTimer <= 120) {
        ...
        // Giới hạn độ rộng tối đa của Tên nhân vật để không đè vào nhau
    ...
    // [V67.4 FIX ĐỒNG BỘ RECORDER] Ép MediaRecorder ghi đúng khung hình này, không bỏ lỡ 1 frame nào!
