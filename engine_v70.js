// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 39.0 [THE 4TH WALL BREAKING EDITION]
// [RESTORED] Giữ nguyên UI Emoji, Text Manga, Hiệu ứng không gian Râu Trắng, Giảm chói mắt (Anti-glare).
// [NEW META] Hiệu ứng vỡ văng thanh máu DOM HTML, Rơi Răng/Dép khi ăn đòn, Vết nứt màn hình Camera vĩnh viễn, Trừng phạt Khinh bỉ (Taunt), và Sự kiện Giờ thực (3 AM Event).
// ==========================================
...
window.bokehs = []; window.spaceRipples = []; window.permanentCracks = []; // [NEW] Lưu vết nứt camera
...
window.initGameEngine = function() {
    ...
    // Giảm sáng (Brightness) xuống một chút để chống lóa mắt toàn cục
    ...
    // [NEW] UPGRADE 5: REAL-WORLD TIME EVENT (SỰ KIỆN THEO GIỜ THỰC TẾ)
    let currentHour = new Date().getHours();
    if (currentHour >= 0 && currentHour <= 3) {
        // 3 Giờ sáng: Creepypasta vibe (Đỏ máu mờ ảo)
        window.viralColorGrade = { contrast: 150, brightness: 60, saturation: 50, hue: 0 };
        window.currentWeather = 'blood_rain';
        window.thumbnailConfig.title = "DO NOT PLAY AT 3 AM";
        window.thumbnailConfig.emoji = "👁️";
        console.log("%c Cẩn thận sau lưng bạn đấy...", "color: red; font-size: 20px; font-weight: bold;");
    } else if (currentHour === 12) {
        // Giữa trưa: Nắng gắt High Noon
        window.currentWeather = 'ash';
        window.viralColorGrade.contrast = 120;
        window.viralColorGrade.brightness = 110;
        window.thumbnailConfig.title = "HIGH NOON BRAWL";
        window.thumbnailConfig.emoji = "🥵";
    }
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce, attackerDirRight = true) {
    ...
    // [NEW] UPGRADE 4: CƠ CHẾ KHINH BỈ (PUNISH TAUNT)
    if (target.state === 'taunt') {
        finalDmg *= 3.0; // Ăn phạt 300% sát thương vì tội láo
        window.screenTearing = 15;
        window.floatingTexts.push({ x: target.x, y: target.y - 120, text: "💥 PUNISHED!", color: "#ff003c", alpha: 1.5, vx: 0, vy: -5, font: "italic 900 50px 'Arial Black', Impact", life: 80, scale: 3.0, targetScale: 1.0, scaleVel: 0, rot: (Math.random()-0.5)*0.2 });
        window.playSound(200, 'square', 1.0, 1.0, true);
    }
    ...
    if (finalDmg > 0) {
        ...
        // [NEW] UPGRADE 2: RĂNG VÀ DÉP VĂNG TUNG TÓE (HYPER-SPECIFIC DETAIL)
        if (finalDmg > 30 && Math.random() > 0.4) {
            let sillyItems = ["🦷", "🦷", "🩴", "🕶️", "🧢"];
            let itemToDrop = sillyItems[Math.floor(Math.random() * sillyItems.length)];
            window.customObjs.push({
                x: target.x, y: target.y - 60,
                vx: (attackerDirRight ? 1 : -1) * (8 + Math.random() * 8),
                vy: -10 - Math.random() * 10,
                text: itemToDrop, color: "#fff", font: "900 30px Arial",
                life: 150, maxLife: 150, spin: true, ang: 0
            });
        }
        ...
        if (wallBounce || (isNearWall && isCrit && target.hp > 0)) { 
            target.vx = target.isFacingRight ? -6 : 6; 
            // [NEW] UPGRADE 3: VẾT NỨT MÀN HÌNH VĨNH VIỄN
            if (Math.random() > 0.6 && window.permanentCracks.length < 5) {
                window.playSound(900, 'sawtooth', 0.2, 0.8, true);
                window.permanentCracks.push({
                    x: target.x < 200 ? Math.random()*250 : window.canvas.width - Math.random()*250,
                    y: target.y - 50 + (Math.random()-0.5)*100,
        ...
        if (target.hp <= 0) {
            if (isNearWall && isCrit && typeof window.triggerStageTransition === 'function') { window.triggerStageTransition(target); } 
                ...
                if (isCrit) {
                    ...
                    // [NEW] UPGRADE 1: META UI BREAK (VỠ CẢ GIAO DIỆN)
                    let hpBar = target.isPlayer ? document.getElementById("hp-wrapper-1") : document.getElementById("hp-wrapper-2");
                    if (hpBar) {
                        hpBar.style.transition = "all 1.5s cubic-bezier(0.175, 0.885, 0.32, 1.275)";
                        hpBar.style.transform = `rotate(${(Math.random() - 0.5) * 60}deg) translateY(400px) scale(0.3)`;
                        hpBar.style.opacity = "0";
                        hpBar.style.pointerEvents = "none";
                        window.playSound(600, 'sawtooth', 0.5, 1.0, true);
                    }
                    if (window.canvas) { // Rung toàn bộ trình duyệt Browser
                        window.canvas.style.transition = "transform 0.1s";
                        window.canvas.style.transform = `translate(${(Math.random()-0.5)*40}px, ${(Math.random()-0.5)*40}px) scale(1.05)`;
                        setTimeout(() => { window.canvas.style.transform = "none"; }, 200);
                    }
...
window.update = function() {
    ...
    if (window.uiShakeP1 > 0) { window.uiShakeP1--; let w1 = document.getElementById("hp-wrapper-1"); if (w1) w1.style.transform = `translate(${(Math.random()*6-3)}px, ${(Math.random()*6-3)}px)`; } else { let w1 = document.getElementById("hp-wrapper-1"); if (w1 && w1.style.pointerEvents !== "none") w1.style.transform = "none"; }
    if (window.uiShakeP2 > 0) { window.uiShakeP2--; let w2 = document.getElementById("hp-wrapper-2"); if (w2) w2.style.transform = `translate(${(Math.random()*6-3)}px, ${(Math.random()*6-3)}px)`; } else { let w2 = document.getElementById("hp-wrapper-2"); if (w2 && w2.style.pointerEvents !== "none") w2.style.transform = "none"; }
...
window.draw = function() {
    ...
    try {
        ...
        // [NEW] UPGRADE 3: VẼ VẾT NỨT CAMERA (DRAW THỨ 4TH WALL) - Lớp cao nhất
        if (window.permanentCracks && window.permanentCracks.length > 0) {
            window.ctx.save();
            window.ctx.setTransform(1, 0, 0, 1, 0, 0); // Đưa về tọa độ màn hình tĩnh
            window.permanentCracks.forEach(c => {
                window.ctx.translate(c.x, c.y);
                window.ctx.scale(c.scale, c.scale);
                window.ctx.beginPath();
                window.ctx.moveTo(0,0); window.ctx.lineTo(-20, -30); window.ctx.moveTo(0,0); window.ctx.lineTo(30, -10);
                window.ctx.moveTo(0,0); window.ctx.lineTo(15, 40); window.ctx.moveTo(0,0); window.ctx.lineTo(-30, 20);
                window.ctx.strokeStyle = "rgba(255, 255, 255, 0.9)";
                window.ctx.lineWidth = 1.5; window.ctx.stroke();
                // Khúc xạ kính
                window.ctx.strokeStyle = "rgba(0, 0, 0, 0.5)";
                window.ctx.lineWidth = 3; window.ctx.stroke();
                window.ctx.scale(1/c.scale, 1/c.scale);
                window.ctx.translate(-c.x, -c.y);
            });
            window.ctx.restore();
        }
