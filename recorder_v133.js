// ==========================================
// RECORDER.JS - V70.9 THE ULTIMATE SAKUGA VISUALS (DIRECTOR'S CUT)
// ĐÃ FIX: Khung hình Camera hoàn hảo (Zoom 1.8x-2.2x) thấy rõ 100% nhân vật và Tên.
// ĐÃ FIX: Phân cảnh Clash mở góc siêu rộng (Wide Shot), thấy rõ 2 người lao vào nhau.
// CẬP NHẬT: Tia tốc độ hội tụ (Manga Focus Lines), Name Tag có viền sáng nghệ thuật.
...
// QUẢN LÝ INTRO ĐIỆN ẢNH (3.2 GIÂY DỒN DẬP)
...
// ====================================================================
// ĐẠO DIỄN: THE ULTIMATE SAKUGA VISUALS (PERFECT FRAMING)
// ====================================================================
window.drawProceduralIntro = function(ctx, w, h, progress) {
    ...
    if (progress < 0.25) { act = 0; localProg = progress / 0.25; } // Boss Intro
    else if (progress < 0.50) { act = 1; localProg = (progress - 0.25) / 0.25; } // Player Intro
    else if (progress < 0.65) { act = 2; localProg = (progress - 0.50) / 0.15; } // Strobe Flash / Tension
    else { act = 3; localProg = (progress - 0.65) / 0.35; } // The Epic Clash
    ...
    // HÀM VẼ NHÂN VẬT (BẢO ĐẢM TÊN ĐƯỢC CĂN CHỈNH HOÀN HẢO THEO CAMERA MỚI)
    const drawFighterArtistic = (charObj, cx, cy, forcedState, isFacingRight, theme, charName, isTrail = false) => {
        ...
        // VẼ TÊN NHÂN VẬT GỌN GÀNG SÁT ĐẦU
        if (charName && act < 2 && !isTrail) {
            ctx.save(); 
            ctx.translate(0, -200); // Hạ cực thấp để bám sát đầu nhân vật
            ctx.scale(0.8, 0.8); // Scale gọn chữ lại
            ...
            // Đường line decor dưới tên cực ngầu
            ctx.fillStyle = theme.aura;
            ctx.fillRect(-40, 25, 80, 4);
            ctx.restore();
        }

        if(!isFacingRight) ctx.scale(-1, 1);
        // Scale chuẩn 3.2 để luôn thấy rõ hình thể, vũ khí
    ...
    // --- SETUP CAMERA HOÀN HẢO (ACT 0, 1, 3) ---
    ctx.save();
    let cutShake = 0; 
    let panX = 0;
    let currentZoom = 2.0; // Zoom tiêu chuẩn vừa vặn màn hình

    if (act === 0) {
        currentZoom = 1.8 + localProg * 0.2; // Zoom in nhẹ nhàng
        panX = (1 - localProg) * 60; // Trượt ngang
    } else if (act === 1) {
        currentZoom = 1.8 + localProg * 0.2;
        panX = -(1 - localProg) * 60;
    } else if (act === 3) {
        // GÓC SIÊU RỘNG CHUẨN BỊ LAO VÀO
        currentZoom = 1.2 + Math.pow(localProg, 3) * 1.6; // Từ 1.2x vút lên 2.8x
        if (localProg > 0.8) cutShake = (Math.random() - 0.5) * 80; 
    }
    
    ctx.translate(w/2 + cutShake + panX, h/2 + cutShake);
    ctx.scale(currentZoom, currentZoom);
    ctx.translate(0, 100); // Kéo nhân vật xuống cho cân đối
    ...
    // TIA TẬP TRUNG (MANGA FOCUS LINES) TRONG LÚC ĐÁNH NHAU
    if (act === 3) {
        ctx.save();
        ...
        for(let i=0; i<40; i++) {
            ...
            ctx.beginPath();
            ctx.moveTo(400 - localProg * 300, 0); // Tia chớp lao thẳng vào trung tâm
            ctx.lineTo(1000, 0); ctx.stroke();
        }
        ctx.restore();
    }
    ...
    if (act === 0) {
        ...
        let easeIn = Math.pow(localProg, 4); // Chuyển động cực gắt
        // Start xa tít màn hình (Tạo không gian rõ ràng) -> Lao cực mạnh vào giữa
        let pushP1 = -400 + (easeIn * 280); 
        let pushP2 = 400 - (easeIn * 280);
        
        // VẼ AFTERIMAGES
        for(let t=1; t<=4; t++) {
            let prevProg = Math.max(0, localProg - t*0.04);
        ...
        for(let i=0; i<40; i++) {
            ...
            let dist = localProg * (400 + speed * 1500); let size = (1 - localProg) * (5 + speed * 35); 
            ...
            if (speed > 0.8) ctx.filter = "blur(5px)"; 
