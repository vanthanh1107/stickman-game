// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 34.0 [THE 3D MATRIX REVOLUTION]
// [FIXED] PERFECT Y-FRAMING: Camera căn giữa tuyệt đối, không bao giờ bị cắt chân/mất bóng.
// [NEW] 3D TRUE PROJECTION: Nhân vật to nhỏ theo trục Z-Axis, tự động quay đầu khi camera quay ra sau lưng.
// [NEW] MATRIX BULLET-TIME: Máy quay 360 độ lướt siêu mượt khi có Clash hoặc Kết liễu K.O.
// [BASE] Đầy đủ Kinetic Text, Lethal Void, RTX Floor, Tách màu quang học (Aberration).
// ==========================================
...
// [VIRAL 34.0] 3D MATRIX CAMERA VARS
window.camOrbitAngle = 0; 
window.targetCamOrbitAngle = 0; 
window.isSpinningCam = false; 
window.orbitFocusX = 0;
window.orbitFocusY = window.GROUND_Y - 60; // Trọng tâm chuẩn ở giữa ngực
...
window.spawnForegroundDebris = function(x, y, isRight) {
    ...
    for(let i=0; i<count; i++) {
        ...
        window.foregroundDebris.push({ x: x, y: y, vx: vx, vy: vy, life: 40, maxLife: 40, scale: 1.0, scaleSpeed: scaleSpeed, rot: Math.random() * Math.PI, rotSpeed: (Math.random() - 0.5) * 0.4 });
    }
}
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce, attackerDirRight = true) {
    ...
    if (target.state === 'block') { 
        if (target.stamina < 15) {
            ...
            // [VIRAL 34.0] Xoay góc máy khi khiên vỡ
            window.targetCamOrbitAngle = attackerDirRight ? 0.5 : -0.5; window.orbitFocusX = target.x; setTimeout(() => { window.targetCamOrbitAngle = 0; }, 800);
    ...
    if (finalDmg > 0) {
        ...
        if (isCrit) {
            ...
            // [VIRAL 34.0] VĂNG CAMERA CHÉO KHI CHÍ MẠNG
        ...
        if (target.hp <= 0) {
            ...
            if (isNearWall && isCrit && typeof window.triggerStageTransition === 'function') { window.triggerStageTransition(target); } 
                ...
                if (isCrit) {
                    ...
                    // [VIRAL 34.0] MATRIX 360 LETHAL FINISHER
...
window.update = function() {
    ...
    // [VIRAL 34.0] CĂN GIỮA TUYỆT ĐỐI THEO TRỤC Y KHUNG HÌNH MÁY QUAY
    window.targetCamX = (window.canvas.width / 2) - window.orbitFocusX + window.actionCamOffsetX + actionPan;
    window.targetCamY = (window.canvas.height * 0.65) - window.orbitFocusY + window.actionCamOffsetY;
...
window.draw = function() {
    ...
    // [VIRAL 34.0] THUẬT TOÁN TÍNH TỌA ĐỘ 3D VÀ PHỐI CẢNH Z-AXIS CHÂN THỰC
    ...
    let project3D = (obj) => {
        ...
        let pZ = dx * sinA;
        let pY = obj.y; // CHÍNH XÁC LÀ CHÂN ĐẠP TRÊN SÀN (Không bù trừ linh tinh nữa)
        
        // Càng gần camera càng to, càng xa càng nhỏ (Tỉ lệ vàng)
        let bScale = obj.scale || 1.0;
        let pScale = bScale * (1 + pZ * 0.002);
        if (pScale < 0.1) pScale = 0.1;
        
        let pFacing = obj.isFacingRight;
        if (typeof pFacing !== 'undefined') {
            // Khi xoay camera ra đằng sau gáy, lật mặt nhân vật lại để tránh đánh lộn chiều
    ...
    // Sắp xếp vẽ theo chiều sâu Z, thằng nào xa ống kính (drawZ nhỏ) thì vẽ trước
    let sortedFighters = allFighters.map(f => {
        let proj = project3D(f); f.drawX = proj.drawX; f.drawY = proj.drawY; f.drawZ = proj.drawZ; f.drawScale = proj.drawScale; f.drawFacingRight = proj.drawFacingRight;
        return f;
    ...
    if (window.isLoading) {
        ...
        window.ctx.fillStyle = "#00f3ff"; window.ctx.font = "italic 900 40px Arial"; window.ctx.textAlign = "center"; window.ctx.shadowBlur = 15; window.ctx.shadowColor = "#00f3ff"; window.ctx.fillText("3D MATRIX REVOLUTION INITIALIZING...", cx, cy - 50);
        ...
        window.ctx.fillStyle = "#fff"; window.ctx.font = "bold 16px monospace"; window.ctx.fillText(`BUILDING 3D PROJECTION MATH... ${Math.floor(window.loadProgress)}%`, cx, cy + 60);
        return; 
    }
    ...
    try {
        ...
        window.ctx.save();
        window.ctx.translate((window.camX + bgPanOffset) * 0.95, 0); 
        ...
        if (t2 === "flowing_water" || t2 === "flowing_lava") {
            ...
            const drawWaveLayer = (color, amplitude, frequency, phase, offsetY) => { window.ctx.fillStyle = color; window.ctx.beginPath(); let startX = -4500; let endX = window.canvas.width + 4500; window.ctx.moveTo(startX, window.canvas.height + 400); window.ctx.lineTo(startX, waterBaseY + offsetY); for (let x = startX; x <= endX; x += 40) { let y = waterBaseY + offsetY + Math.sin((x * frequency) + waveSpeed + phase) * amplitude; window.ctx.lineTo(x, y); } window.ctx.lineTo(endX, window.canvas.height + 400); window.ctx.fill(); };
        ...
        // [VIRAL 34.0] REFLECTION MẶT SÀN KHỚP VỚI 3D PROJECTION
        window.ctx.save();
        ...
        // KINETIC TYPOGRAPHY RENDER (Nảy lò xo)
        ...
        // LAYER 2D NGOÀI CÙNG (UI, BARS, FLASH)
