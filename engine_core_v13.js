...
// [BẢN HOÀN CHỈNH ULTIMATE: SIÊU MƯỢT (NO HIT-STOP) + CHỮ SÁT THƯƠNG RƠI NẢY TƯNG TƯNG]
// ==========================================
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce, attackerDirRight = true) {
    ...
    // 🟢 Xóa Slow Motion khi Evade, chữ nhỏ đi, bay lùi
    if (target.state === 'dash' || target.state === 'dash_back') {
        if (target.dashTimer > 2 && target.dashTimer < 14) {
            ...
            window.floatingTexts.push({ x: target.x, y: target.y - 80, text: "⚡ EVADED!", color: "#00f3ff", alpha: 1.5, vx: (target.isFacingRight ? -4 : 4), vy: -6, font: "italic 900 25px 'Arial Black', Impact", life: 50, scale: 1.2, targetScale: 1.0, scaleVel: 0, rot: (Math.random()-0.5)*0.3 });
    ...
    if (target.state === 'block') { 
        if (target.stamina < 15) {
            ...
            window.floatingTexts.push({ x: target.x, y: target.y - 90, text: "💔 CRUSH!", color: "#ff003c", alpha: 1.5, vx: (attackerDirRight ? 4 : -4), vy: -6, font: "italic 900 30px 'Arial Black', Impact", life: 60, scale: 1.5, targetScale: 1.0, scaleVel: 0, rot: (Math.random()-0.5)*0.2 });
            ...
            window.floatingTexts.push({ x: target.x, y: target.y - 80, text: "🛡️ PARRY!", color: "#00ffff", alpha: 1.5, vx: (attackerDirRight ? -3 : 3), vy: -5, font: "italic 900 25px 'Arial Black', Impact", life: 50, scale: 1.2, targetScale: 1.0, scaleVel: 0, rot: (Math.random()-0.5)*0.2 });
    ...
    if (finalDmg > 0) {
        ...
        // 🟢 SỐ SÁT THƯƠNG NHỎ, BAY LÙI THEO CÚ ĐẤM
        ...
        if (isCrit) {
            ...
            // XÓA SLOW-MOTION CHÍ MẠNG & KHÔNG CÓ HIT-STOP NỮA
        ...
        if (target.hp <= 0) {
            ...
            if (isNearWall && isCrit && typeof window.triggerStageTransition === 'function') { window.triggerStageTransition(target); } 
                ...
                if (isCrit) {
                    ...
                    window.floatingTexts.push({ x: target.x, y: target.y - 120, text: "💀 K.O!", color: "#ff003c", alpha: 1.5, vx: (attackerDirRight ? 5 : -5), vy: -8, font: "italic 900 45px 'Arial Black', Impact", life: 100, scale: 1.8, targetScale: 1.0, scaleVel: 0, rot: (Math.random()-0.5)*0.1 });
...
window.update = function() {
    ...
    allFighters.forEach(f => {
            ...
            if (f.hitStun <= 0 && f.stunTimer <= 0 && f.state !== 'dash_back' && f.state !== 'block') {
                ...
                if (isCloseEnough) {
                    if (window.classStats && window.classStats[type] && typeof window.classStats[type].executeUltimate === 'function') {
                        f.stamina = 0; let baseDmg = 50 * (f.currentDmgMod || 1); if (!f.isPlayer) baseDmg = 35 * (f.currentDmgMod || 1); window.playSound(400, 'sine', 0.5, 0.6); window.shakeScreen(15, 10); window.spawnParticles(f.x, f.y, "#f1c40f", true); window.floatingTexts.push({ x: f.x, y: f.y - 100, text: f.isPlayer ? "🔥 ULTIMATE!" : "⚠️ DANGER!", color: f.isPlayer ? "#ff4757" : "#ff0000", alpha: 1.5, vx: 0, vy: -6, font: "italic 900 25px 'Arial Black', Impact", life: 50, scale: 1.5, targetScale: 1.0, scaleVel: 0, rot: 0 }); f.isFacingRight = distToTarget > 0; window.classStats[type].executeUltimate(f, closestTarget, baseDmg); f.vx = 0; window.speedLinesAlpha = 1.0; 
        ...
        if (!launchedUltimate && f.attackTimer <= 0 && f.hitStun <= 0 && f.dashTimer <= 0 && f.stunTimer <= 0 && !window.gameOver && f.hp > 0) {
            if (f.isDragon) {
                if (f.hp > 0 && f.hp <= f.maxHp * 0.3 && !f.isEvolved) { f.isEvolved = true; window.screenFlash = 0; window.shakeScreen(50, 15); window.playSound(50, 'sawtooth', 2.0, 1.0, true); f.color = "#8e44ad"; f.scale *= 1.25; window.floatingTexts.push({ x: f.x, y: f.y - 150, text: "🐉🔥", color: "#8e44ad", alpha: 1.5, vx: 0, vy: -5, font: "italic 900 60px 'Arial Black', Impact", life: 100, scale: 1.5, targetScale: 1.0, scaleVel: 0, rot: 0 }); window.shockwaves.push({x: f.x, y: window.GROUND_Y, r: 10, maxR: 500, color: "#8e44ad", alpha: 1, speed: 25}); window.targetCamOrbitAngle = 0.5; }
                ...
                if (targetFighter && targetFighter.hp > 0) {
                    ...
                    if (f.aiDelay <= 0) {
                        ...
                        if (randAction < 0.4) { f.state = 'cast'; f.attackTimer = 20; window.playSound(400, 'sine', 0.2, 0.4); window.spawnProjectile(f.x, f.y - 50, f.isFacingRight ? 15 : -15, 0, 8, "#9b59b6", Math.floor(15 * f.dmgMod), window.p1); if(Math.random() < 0.5) window.spawnProjectile(f.x, f.y - 60, f.isFacingRight ? 14 : -14, -3, 8, "#9b59b6", Math.floor(15 * f.dmgMod), window.p1); window.floatingTexts.push({ x: f.x, y: f.y - 80, text: "🥷", color: "#9b59b6", alpha: 1.5, vx: 0, vy: -4, font: "900 25px Arial", life: 30, scale: 1.2, targetScale: 1.0, scaleVel: 0, rot: 0 }); window.targetCamOrbitAngle = f.isFacingRight ? -0.1 : 0.1; } 
                        else if (randAction < 0.7) { window.spawnParticles(f.x, f.y, "#2c3e50"); f.x = window.p1.x + (window.p1.isFacingRight ? -80 : 80); f.y = window.p1.y; f.isFacingRight = window.p1.x > f.x; window.spawnParticles(f.x, f.y, "#9b59b6"); f.state = 'spinning_backfist'; f.attackTimer = 15; window.playSound(200, 'square', 0.2, 0.6); if(typeof window.takeDamage === 'function') window.takeDamage(window.p1, Math.floor(20 * f.dmgMod), "#9b59b6", false, false); window.floatingTexts.push({ x: f.x, y: f.y - 80, text: "💨", color: "#8e44ad", alpha: 1.5, vx: 0, vy: -4, font: "900 25px Arial", life: 30, scale: 1.2, targetScale: 1.0, scaleVel: 0, rot: 0 }); window.targetCamOrbitAngle = f.isFacingRight ? 0.2 : -0.2; } 
    ...
    for (let i = 0; i < allFighters.length; i++) { 
        for (let j = i + 1; j < allFighters.length; j++) { 
            ...
            if (dist < 80 && f1.attackTimer > 5 && f1.attackTimer < 20 && f2.attackTimer > 5 && f2.attackTimer < 20) {
                ...
                // 🟢 XÓA KHỰNG TRUYỀN THỐNG KHI CHẠM GĂNG (BRAWL CLASH)
                if (!f1.clashCooldown && !f2.clashCooldown && f1.isFacingRight !== f2.isFacingRight) {
                    ...
                    window.floatingTexts.push({ x: midX, y: midY - 60, text: "⚔️ BRAWL!", color: "#f1c40f", alpha: 1.5, vx: (Math.random()-0.5)*8, vy: -7, font: "italic 900 25px 'Arial Black', Impact", life: 60, scale: 1.2, targetScale: 1.0, scaleVel: 0, rot: (Math.random()-0.5)*0.1 });
                }
            }
            ...
            if (hit.delay <= 0) {
                ...
                if (target && target.hp > 0 && Math.abs(f.x - target.x) <= reach + 35) {
                    ...
                    if (hit.isFinisher) {
                        ...
                        window.floatingTexts.push({ x: target.x, y: target.y - 80, text: "💥", color: "#ff4757", alpha: 1.5, vx: (f.isFacingRight ? 5 : -5), vy: -6, font: "900 45px Arial", life: 50, scale: 1.5, targetScale: 1.0, scaleVel: 0, rot: (Math.random()-0.5)*0.3 });
                        ...
                        if (Math.random() < f.critChance) { 
                            ...
                            window.floatingTexts.push({ x: target.x + (Math.random()*40-20), y: target.y - 60, text: "💢", color: "#f1c40f", alpha: 1.5, vx: (f.isFacingRight ? 4 : -4), vy: -5, font: "italic 900 25px 'Arial Black', Impact", life: 60, scale: 1.0, targetScale: 1.0, scaleVel: 0, rot: 0 }); 
                        } else { window.playSound(350, 'sine', 0.1, 0.1, false); }
                        if ((f.state === 'low_kick' || f.state === 'teep_kick') && Math.random() < 0.4) { target.stunTimer = 35; target.state = 'stunned'; window.floatingTexts.push({ x: target.x, y: target.y - 50, text: "🦵", color: "#e67e22", alpha: 1.5, vx: (f.isFacingRight ? 3 : -3), vy: -4, font: "900 25px Arial", life: 60, scale: 1.0, targetScale: 1.0, scaleVel: 0, rot: 0 }); }
                        if ((f.state === 'uppercut' || f.state === 'elbow_strike') && Math.random() < 0.3) { finalDmg *= 1.5; window.spawnParticles(target.x, target.y - 60, "#c0392b", true); window.floatingTexts.push({ x: target.x, y: target.y - 50, text: "🩸", color: "#c0392b", alpha: 1.5, vx: (f.isFacingRight ? 3 : -3), vy: -4, font: "900 25px Arial", life: 60, scale: 1.0, targetScale: 1.0, scaleVel: 0, rot: 0 }); }
    ...
    // 🟢 NÂNG CẤP VẬT LÝ CHO DÒNG TEXT (Rớt xuống mặt đất và nảy)
    for (let i = window.floatingTexts.length - 1; i >= 0; i--) { 
        let t = window.floatingTexts[i]; 
        
        t.vy += window.GRAVITY * 0.4; // Trọng lực kéo chữ rơi xuống
        t.x += t.vx; 
        t.y += t.vy; 
        ...
        if (t.y >= window.GROUND_Y - 5) {
            ...
            t.vy *= -0.5; // Nảy lên nhẹ
            t.vx *= 0.6;  // Trượt chậm lại
        }
        ...
        if (t.life !== undefined) { 
            ...
            // Mờ dần trong 20 khung hình cuối
