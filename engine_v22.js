// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 20.0 [DIRECTOR'S CUT]
// [HUMANIZED ALGORITHM: CHỐNG NHẬN DIỆN VIDEO AI BẰNG VẬT LÝ QUANG HỌC]
// [TÍNH NĂNG: SPEED RAMPING (VELOCITY EDIT), HANDHELD CAMERA, MOTION BLUR]
// ==========================================
...
// [VIRAL 20.0] HỆ THỐNG ĐẠO DIỄN (DIRECTOR'S CUT)
window.mangaSfx = []; 
window.dimensionCracks = []; 
window.inkSplatters = []; 
window.invertFrames = 0; 
window.noiseCanvas = null; 
window.timeScale = 1.0; // Tốc độ thời gian hiện tại
window.targetTimeScale = 1.0; // Tốc độ thời gian mục tiêu (Dùng cho Speed Ramping)
window.camVelocityX = 0; // Tính toán RGB quang học
...
// ==========================================
// 1. HỆ THỐNG ÂM THANH & MUTATION (ANTI-AI GENERATOR)
// ==========================================
...
// ==========================================
// 2. HỆ THỐNG SPAWN VFX (ĐÃ NÂNG CẤP DYNAMICS)
// ==========================================
...
window.triggerTimeStop = function(frames, caster) { 
    window.targetTimeScale = 0.05; // [VIRAL 20.0] SPEED RAMPING thay vì dừng hẳn
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce) {
    ...
    if (target.state === 'dash' || target.state === 'dash_back') {
        if (target.dashTimer > 2 && target.dashTimer < 14) {
            ...
            // [VIRAL 20.0] SPEED RAMPING KHI DODGE
    ...
    if (target.state === 'block') { 
        if (target.stamina < 15) {
            ...
            window.hitStopFrames = 20; window.chromaTimer = 20; window.targetZoom = 1.4; window.vhsGlitchTimer = 20;
        } 
    ...
    if (finalDmg > 0) {
        ...
        if (isCrit) {
            ...
            // [VIRAL 20.0] CHẠM CHÍ MẠNG LÀ SPEED RAMP (Vuốt tốc độ chuẩn Editor AE)
        ...
        if (target.hp <= 0) {
            ...
            if (isNearWall && isCrit && typeof window.triggerStageTransition === 'function') { window.triggerStageTransition(target); } 
                ...
                if (isCrit) {
                    ...
                    window.targetZoom = 1.6; window.hitStopFrames = 20; window.shakeScreen(50, 30);
                    window.vhsGlitchTimer = 30; window.triggerDimensionShatter(); 
                    window.targetTimeScale = 0.05; setTimeout(()=>{ window.targetTimeScale = 1.0; }, 1500); // Slowmo kết liễu
                } else { window.hitStopFrames = 10; window.shakeScreen(30, 20); window.targetZoom = 1.4; window.targetTimeScale = 0.3; setTimeout(()=>{window.targetTimeScale = 1.0;}, 800); }
...
window.update = function() {
    ...
    if (window.cinematicTimer > 0) { window.cinematicTimer--; if (window.cinematicTimer === 0 && window.cinematicCallback) { try { window.cinematicCallback(); } catch(e) {} window.cinematicCallback = null; } return; }
    if (window.hitStopFrames > 0) { window.hitStopFrames--; return; } 
...
window.draw = function() {
    ...
    // [VIRAL 20.0] CINEMATIC MOTION BLUR & CAM VELOCITY TRACKING
    ...
    // Thay vì xóa trắng (clearRect), ta tô một lớp bán trong suốt để tạo bóng mờ Motion Blur
    ...
    // [VIRAL 20.0] JJK NEGATIVE IMPACT (ĐẢO MÀU LÚC CHẾT)
    ...
    try {
        ...
        // [VIRAL 20.0] HANDHELD CAMERA BREATHING
        ...
        window.ctx.fillStyle = skyGrad; window.ctx.fillRect(-3000, -3000, window.canvas.width + 6000, window.canvas.height + 6000);
        ...
        // [VIRAL 20.0] VELOCITY CHROMATIC ABERRATION
        ...
        // [VIRAL 20.0] NOISE CHỐNG BOT QUÉT VIDEO
...
// ==========================================
// 7. [VIRAL 20.0] AFTER EFFECTS TIME-RAMPING & UNLOCKED 120Hz LOOP
// ==========================================
...
window.gameLoop = function(timestamp) { 
    ...
    // [VIRAL 20.0] SPEED RAMPING SMOOTH LERP (VUỐT TỐC ĐỘ)
