// ==========================================
// RECORDER.JS - BẢN HOÀN HẢO CHO CONTENT CREATOR (V47.1 - MEGA UPDATE)
// QUY HOẠCH LẠI LAYOUT: TÁCH RIÊNG SUB & SÓNG ÂM RA KHỎI GAME (NẰM Ở BLACK BAR)
// TÍCH HỢP AI MEGA-MATRIX (9 CHỦ ĐỀ), ANTI-UNKNOWN TÊN NHÂN VẬT
// NÂNG CẤP: SÓNG ÂM ĐỐI XỨNG TỪ TÂM, SUBTITLE NEON GLOW CHUẨN TIKTOK
// ==========================================
...
window.StoryModeAI = {
    ...
    generateScript: function(hero, enemy) {
        ...
        const narratives = [
            // CHỦ ĐỀ 1: KẾN THỨC TÂM LÝ
            {
                title: `🤯 PSYCHOLOGY TRICK! ${h} vs ${e}!`,
                lines: [
                    ...
                    `Just like how ${e} is constantly staring at ${h}'s movements right now. Watch this finish!`
                ]
            },
            // CHỦ ĐỀ 2: KIẾN THỨC VẬT LÝ KHÔNG GIAN
            {
                title: `🌌 MIND BLOWING FACT! ${h} vs ${e}!`,
                lines: [
                    "Did you know that time actually moves slower at the Pyramids of Giza?",
                    "This is a real physics phenomenon called gravitational time dilation.",
                    "Super heavy objects literally warp the fabric of space and time.",
                    "So if you stand next to a pyramid, you are technically aging slower.",
                    `Speaking of slowing down time, watch ${h}'s perfect reaction timing against ${e} here!`
                ]
            },
            // CHỦ ĐỀ 3: CƠ THỂ CON NGƯỜI
            {
                title: `👁️ YOU CAN'T UNSEE THIS! ${h} vs ${e}!`,
                lines: [
                    ...
                    `Selective attention is exactly what ${h} needs to dodge ${e}'s ultimate attack!`
                ]
            },
            // CHỦ ĐỀ 4: CÁCH ĐỌC VỊ NGƯỜI NÓI DỐI
            {
                title: `🕵️ FBI SECRET! ${h} vs ${e}!`,
                lines: [
                    ...
                    `But ${e} cannot lie to ${h}. Every move is being read perfectly. Wait for it!`
                ]
            },
            // CHỦ ĐỀ 5: HƯỚNG DẪN CHIẾN THUẬT (PRO TIPS)
            {
                title: `🧠 PRO TIP: How ${h} counters ${e}'s mechanics! 💡`,
                lines: [
                    `If you are struggling against ${e}, watch this gameplay closely.`,
                    `Notice how ${h} manages their stamina instead of spamming attacks.`,
                    `${e} relies on heavy burst damage, so you must bait their combo first.`,
                    `The secret is to punish them during their cooldown window.`,
                    `Master this exact timing, and you will easily win this matchup.`
                ]
            },
            // CHỦ ĐỀ 6: PHÂN TÍCH CHỈ SỐ (STAT CHECK)
            {
                title: `📊 STAT CHECK: Can ${h} melt ${e}?! 🛡️`,
                lines: [
                    `Let's break down the stats for this insane boss fight.`,
                    `${e} has one of the highest base health and defense pools in the game.`,
                    `But ${h} is bringing a high DPS pressure build to counter it.`,
                    `It is a classic battle of an unstoppable force versus an immovable object.`,
                    `Watch how the armor penetration completely changes the outcome of this fight.`
                ]
            },
            // CHỦ ĐỀ 7: CỐT TRUYỆN / LORE (STORY)
            {
                title: `📜 HIDDEN LORE: Why ${h} must destroy ${e}... ⚔️`,
                lines: [
                    `Most players don't know the dark, hidden lore behind this specific encounter.`,
                    `Long ago, ${e} was responsible for wiping out ${h}'s entire faction.`,
                    `This is not just a random boss fight. This is pure, unadulterated revenge.`,
                    `Every single strike from ${h} carries the weight of a fallen kingdom.`,
                    `Witness the epic conclusion to the greatest rivalry in gaming history.`
                ]
            },
            // CHỦ ĐỀ 8: KỸ NĂNG CAO / HYPE (ABSOLUTE CINEMA)
            {
                title: `🔥 ABSOLUTE CINEMA: ${h} styles on ${e}! 😱`,
                lines: [
                    `Welcome to the highest level of gameplay you will ever see today.`,
                    `The movement, the spacing, the pure execution by ${h} is flawless.`,
                    `${e} is an absolute monster, but the skill gap here is just massive.`,
                    `One single mistake could end the run, but they are playing perfectly.`,
                    `Wait for the final combo, because this clip is definitely going viral!`
                ]
            },
            // CHỦ ĐỀ 9: BÍ MẬT / CƠ CHẾ ẨN
            {
                title: `🚨 99% PLAYERS MISS THIS: ${h} vs ${e} Trick! 🤯`,
                lines: [
                    `There is a hidden mechanic in this game that nobody talks about.`,
                    `Watch ${h}'s positioning when ${e} prepares their ultimate attack.`,
                    `By staying at this exact angle, you can completely avoid the damage hitbox.`,
                    `Developers actually tried to patch this, but it still works perfectly.`,
                    `Use this trick right now before it gets completely removed from the game.`
                ]
            }
        ];
...
window.startRecording = function() {
    ...
    if (window.p1) {
        ...
        if (window.classStats && window.classStats[window.p1.classId]) {
            charName = window.classStats[window.p1.classId].className || charName;
            charAvatar = window.classStats[window.p1.classId].avatarUrl || charAvatar;
        }
    }
...
// NÂNG CẤP: SUBTITLE RENDERER CHUẨN TIKTOK GLOW
...
// NÂNG CẤP: VISUALIZER ĐỐI XỨNG TỪ TÂM (PODCAST STYLE)
function drawAudioVisualizer(ctx, x, y, width, height) {
    ...
    for (let i = 0; i < numBars / 2; i++) {
        ...
        // Cột bên phải
        if (ctx.roundRect) { ctx.beginPath(); ctx.roundRect(center + offsetX, y - barHeight, barWidth, barHeight, 5); ctx.fill(); } 
        else { ctx.fillRect(center + offsetX, y - barHeight, barWidth, barHeight); }
        
        // Cột bên trái (đối xứng)
        if (ctx.roundRect) { ctx.beginPath(); ctx.roundRect(center - offsetX - barWidth, y - barHeight, barWidth, barHeight, 5); ctx.fill(); } 
        else { ctx.fillRect(center - offsetX - barWidth, y - barHeight, barWidth, barHeight); }
    }
    ctx.restore();
}
...
window.captureFrames = function() {
    ...
    if (window.p1 && !window.gameOver && window.introTimer <= 120) {
        ...
        // ĐỒNG BỘ TÊN VỚI ANTI-UNKNOWN LOGIC
        let p1Name = "PLAYER", p1Url = "https://i.imgur.com/q3813rX.png";
        if (window.p1) {
            p1Name = (window.p1.className || window.p1.name || "PLAYER").toUpperCase();
            if (window.classStats && window.classStats[window.p1.classId]) {
                p1Name = (window.classStats[window.p1.classId].className || p1Name).toUpperCase();
        ...
        if (window.enemies && window.enemies.length > 0) {
            ...
            if (e0.isDragon) eName = "DRAGON";
            if (e0.isBruceLee) eName = "BRUCE LEE";
            if (window.classStats && window.classStats[e0.classId]) {
                eName = (window.classStats[e0.classId].className || eName).toUpperCase();
                p2Url = window.classStats[e0.classId].avatarUrl || p2Url;
            }
    ...
    if (window.StoryModeAI.displayedText.length > 0) {
        // AUDIO VISUALIZER (SÓNG ÂM ĐỐI XỨNG) 
