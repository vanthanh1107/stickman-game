// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 38.2 [FIXED CAMERA SPIN]
// [RESTORED] Giữ nguyên UI Emoji, Text Manga.
// [OPTIMIZED] Tinh chỉnh VFX để mượt mà, không nhiễu loạn thị giác khi combo.
// [NEW] Hiệu ứng vỡ nứt không gian 3D sắc nét lan tỏa từ tâm chấn (Râu Trắng).
// [FIXED] Khóa tâm Camera tuyệt đối ở nhân vật khi xảy ra Lethal Finish, không bị trôi đi nơi khác.
// ==========================================
        ...
        if (target.hp <= 0) {
            ...
            if (isNearWall && isCrit && typeof window.triggerStageTransition === 'function') { window.triggerStageTransition(target); } 
                ...
                if (isCrit) {
                    ...
                    // LETHAL FINISH CAMERA SETTINGS
                    window.targetZoom = 1.8; 
                    window.actionCamOffsetX = 0; // [FIX] Zero offset to stay strictly centered
                    window.actionCamOffsetY = -20; 
                    window.dynamicBlur = 25; 
                    window.cameraTilt = (Math.random() > 0.5 ? 1 : -1) * 0.15; 
                    
                    window.lethalVoid = 100; window.impactAberration = 40; window.spawnForegroundDebris(target.x, target.y, attackerDirRight); window.spawnForegroundDebris(target.x, target.y, attackerDirRight); 
                    window.hitStopFrames = 30; window.shakeScreen(50, 35); window.vhsGlitchTimer = 45; 
                    
                    // GỌI HIỆU ỨNG NỨT KHÔNG GIAN TẠI TÂM KẺ ĐỊCH
                    ...
                    // [FIX] Khóa Tâm Camera tuyệt đối ở mục tiêu bị diệt
...
window.update = function() {
            ...
            for (let j = i + 1; j < allFighters.length; j++) { 
                ...
                if (dist < 80 && f1.attackTimer > 5 && f1.attackTimer < 20 && f2.attackTimer > 5 && f2.attackTimer < 20) {
                    if (!f1.clashCooldown && !f2.clashCooldown && f1.isFacingRight !== f2.isFacingRight) {
                        ...
                        // GỌI HIỆU ỨNG VỠ KHÔNG GIAN KHI CLASH
    ...
    // [FIX] CHỈ CẬP NHẬT TRƯỢT TÂM KHI KHÔNG QUAY CAMERA LETHAL FINISH
