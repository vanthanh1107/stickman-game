...
// [BẢN CẬP NHẬT CINEMATIC ENDING - NHÂN VẬT ĐỨNG YÊN TUYỆT ĐỐI KHI KẾT THÚC]
// ==========================================
...
window.update = function() {
    ...
    allFighters.forEach(f => {
        ...
        // ===============================================
        // CẬP NHẬT: KHÓA CHẶT NHÂN VẬT KHI KẾT THÚC GAME
        // ===============================================
        if (window.gameOver && f.hp > 0) {
            f.dashTimer = 0; 
            if (f.onGround) { 
                f.vx *= 0.5; // Phanh gấp
                if (Math.abs(f.vx) < 0.5) f.vx = 0; // Đứng im hoàn toàn
            }
            if (f.state === 'walk' || f.state === 'dash' || f.state === 'dash_back') f.state = 'idle';
        } else if (f.state === 'idle' && !isTimeStopped) { 
            // Chỉ đung đưa qua lại (thở) khi trận đấu chưa kết thúc
