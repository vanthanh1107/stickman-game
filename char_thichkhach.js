window.currentLoadedChar = {
    ...
    // ĐÒN ĐÁNH THƯỜNG: VUNG BÁN NGUYỆT TRẢM (Lưỡi kiếm vòng cung khổng lồ)
    executeBasicAttack: function(caster, enemies) {
        caster.state = 'sword_slash'; caster.attackTimer = 18; 
    ...
    skill: {
        actionCode1: function(caster, target, ctx) {
            caster.state = 'slash_dash'; caster.attackTimer = 15; caster.vx = caster.isFacingRight ? 25 : -25; caster.iFrames = 15;
        ...
        actionCode2: function(caster, target, ctx) {
            caster.state = 'flurry'; caster.attackTimer = 30;
            let hits = 0;
    ...
    executeUltimate: function(caster, target, baseDmg) {
        caster.state = 'iaijutsu'; caster.attackTimer = 40; 
        setTimeout(() => { 
    ...
    // NÂNG CẤP ĐỒ HỌA: NÓN LÁ RONIN VÀ ÁO KIMONO
    drawMethod: function(ctx, p, bounce, ext, pext, isTrail) {
        ...
        // Vẽ Nón lá Samurai (Conical Hat) che kín mắt
        if (!isTrail) {
            ctx.fillStyle = "#d35400"; 
            ctx.beginPath(); 
            ctx.moveTo(head.x - 22, head.y); // Vành trái
            ctx.lineTo(head.x + 22, head.y); // Vành phải
            ctx.lineTo(head.x, head.y - 18); // Chóp nón
            ctx.closePath(); ctx.fill();
        }

        let swordDir = p.isFacingRight ? 1 : -1;
        let isSlashing = (p.state === 'sword_slash' || p.state === 'slash_dash' || p.state === 'flurry');

        // Kiếm Katana siêu dài có chuôi kiếm
        ...
        if (isSlashing) {
            ...
            // Vẽ nhát chém Bán nguyệt vàng khổng lồ (Crescent Slash)
            if (!isTrail) {
        ...
        if (!isTrail) {
            ...
            ctx.fillRect(handR.x - 2, handR.y - 4, 8, 8);
        }
    }
};
