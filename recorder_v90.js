...
// Kèm theo SCROLLING CELEBRITY LIVE CHAT & SEXY 3D VTUBER AVATAR
// ==========================================
...
// ====================================================================
// 🎯 KHU VỰC TÙY CHỈNH: ẢNH MẪU 3D, NGƯỜI NỔI TIẾNG & BÌNH LUẬN 
// ====================================================================

// Thay link ảnh này bằng link ảnh Model 3D Sexy của bạn (Link Imgur, Discord, Unsplash...)
window.SEXY_3D_MODEL_URL = "https://i.imgur.com/k8L1z8l.jpeg"; // <-- ĐỔI LINK ẢNH TẠI ĐÂY

// Danh sách tên người nổi tiếng và màu sắc của họ
window.CELEB_LIST = [
...
// Danh sách hàng tá các bình luận Toxic/Vui vẻ (cứ thêm thoải mái)
...
window.StoryModeAI = {
    ...
    generateScript: function(hero, enemy) {
        return { title: `BRO THINKS HE IS HIM 💀🤡`, lines: [] }; // Không còn dùng giọng đọc text nữa
    },
...
// ====================================================================
// HỆ THỐNG VẼ MODEL 3D SEXY
// ====================================================================
function drawVTuberCommentator(ctx, x, y, audioPeak) {
    // Tải ảnh vào Cache nếu chưa có
    if (!window.vtuberImg) {
        window.vtuberImg = new Image();
        window.vtuberImg.crossOrigin = "Anonymous";
        window.vtuberImg.src = window.SEXY_3D_MODEL_URL;
    }

    let t = Date.now();
    ctx.save();
    
    // Nhún nhảy và giật theo nhịp combat (audioPeak)
    let pulse = 1 + (audioPeak * 0.12);
    let floatY = Math.sin(t / 250) * 10;
    ctx.translate(x, y + floatY);
    ctx.scale(pulse, pulse);

    // Hiệu ứng viền phát sáng (Glow)
    ctx.shadowColor = audioPeak > 0.6 ? "#ff0055" : "#00f3ff";
    ctx.shadowBlur = 20 + (audioPeak * 40);

    // Kích thước vòng tròn Avatar
    let radius = 175;
    ...
    // Áp dụng cắt tròn cho Ảnh
    ctx.clip();
    
    // Vẽ Ảnh 3D Sexy vào giữa
    if (window.vtuberImg.complete && window.vtuberImg.naturalWidth > 0) {
        // Căn giữa ảnh vào khung tròn
        ctx.drawImage(window.vtuberImg, -radius, -radius, radius * 2, radius * 2);
    } else {
        // Nếu ảnh chưa load kịp thì vẽ nền xám
    ...
    // Huy hiệu "🔴 LIVE"
    ctx.save();
    ctx.translate(x, y + floatY + radius - 10);
    ctx.scale(pulse, pulse);
    ctx.fillStyle = "#ff0000";
    ctx.beginPath(); if(ctx.roundRect) ctx.roundRect(-60, -20, 120, 40, 12); else ctx.fillRect(-60, -20, 120, 40); ctx.fill();
...
window.captureFrames = function() {
    ...
    // Kiểm tra xem đã kết thúc game và vào Outro chưa (Outro bật lên lúc matchEndTimer > 90)
    ...
    // ==========================================
    // KHÔNG HIỂN THỊ CHAT & AVATAR LÚC OUTRO BẬT
    // ==========================================
    if (!isOutroActive) {
        
        // 1. TÍNH NĂNG CHAT TOXIC KÉO TỪ DƯỚI LÊN
        if (!window._chatSystemInit) {
            ...
            // Đổ sẵn 7 bình luận vào khung để không bị trống ngay khi bắt đầu
            for(let i = 0; i < 7; i++) {
        ...
        // Cập nhật Comment mới (1 đến 3 giây rơi ra 1 câu)
        if (chatNow - window._lastChatUpdate > window._nextChatDelay) {
            ...
            window._liveChats.push({ name: randomCeleb.name, color: randomCeleb.color, msg: randomMsg });
            if (window._liveChats.length > 7) window._liveChats.shift();
        }
        ...
        let boxHeight = 550;
        let boxX = 540; 
        let boxY = splitGameHeight + 450; // Kéo Bảng Chat lên trên Avatar
        ...
        // Vẽ dòng chữ cuộn lên
        let startTextY = boxHeight/2 - 30; 
        let lineHeight = 65; 
        
        ctxV.textAlign = "left"; ctxV.textBaseline = "bottom";
        ...
        // 2. VẼ AVATAR 3D SEXY Ở ĐÁY MÀN HÌNH (Dưới bảng chat)
        let vtuberY = splitGameHeight + 950; // Neo thẳng tắp phía dưới bảng chat
