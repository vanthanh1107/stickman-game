// ==========================================
// RECORDER.JS - V70.2 ULTRA-CLEAN UI (THE DIRECTOR'S CUT)
// ĐÃ FIX: Lấy chuẩn 100% tên thật của nhân vật, loại bỏ chữ Player/Boss.
// ĐÃ FIX: Thanh Win Rate tối giản chỉ còn màu Xanh/Đỏ và %.
// CẢM XÚC: Trailer Game Hạng AAA, Gọn gàng, Nghệ thuật thị giác đỉnh cao.
...
window.drawProceduralIntro = function(ctx, w, h, progress) {
    ...
    ctx.filter = "blur(50px)"; ctx.fill(); ctx.filter = "none"; ctx.globalAlpha = 1.0;
    ...
    // HÀM VẼ NHÂN VẬT & NAME TAG THỰC TẾ TRONG GAME
    const drawFighterArtistic = (charObj, cx, cy, forcedState, isFacingRight, theme, fx, charName) => {
        if(!charObj) return; 
        ctx.save(); ctx.translate(cx, cy); 
        
        // VẼ TÊN GỐC CỦA NHÂN VẬT (BỎ TITLE RƯỜM RÀ)
        if (charName && fx !== 'silhouette') {
            ctx.save();
            ctx.translate(0, -300); // Hạ thấp sát đầu nhân vật
            let nameScale = 1 + Math.sin(Date.now() * 0.005) * 0.02;
            ctx.scale(nameScale, nameScale);
            
            ctx.textAlign = "center"; ctx.textBaseline = "middle";
            ctx.font = "900 32px 'Arial Black', sans-serif"; // Nét mảnh, thanh thoát hơn
    ...
    if (cut.focus === 'p1') {
        drawFighterArtistic(window.p1, 0, 100, cut.state, true, prm.themeP1, cut.fx, prm.p1Name);
    } 
    else if (cut.focus === 'p2') {
        drawFighterArtistic(e1, 0, 100, cut.state, false, prm.themeP2, cut.fx, prm.p2Name);
    } 
    else { 
...
window.startRecording = function() {
    ...
    if (window.p1) { 
        charName = window.p1.name || window.p1.className || window.p1.type || window.p1.id || "PLAYER"; 
        if (window.classStats && window.classStats[window.p1.classId]) { charName = window.classStats[window.p1.classId].name || window.classStats[window.p1.classId].className || charName; charAvatar = window.classStats[window.p1.classId].avatarUrl || charAvatar; } 
    }
    
    let enemyName = "BOSS";
    if (window.enemies && window.enemies.length > 0) { 
        let e0 = window.enemies[0]; 
        enemyName = e0.name || e0.className || e0.type || e0.id || "BOSS"; 
        if (window.classStats && window.classStats[e0.classId]) { enemyName = window.classStats[e0.classId].name || window.classStats[e0.classId].className || enemyName; } 
    }

    window.StoryModeAI.init(charName, enemyName);
    
    // Gán tên trực tiếp vào Engine Intro
    window.introParams.p1Name = charName.toUpperCase();
    window.introParams.p2Name = enemyName.toUpperCase();
    ...
    if (renderNormalV) {
        ...
        if (!isOutroActive) {
            ...
            if (window.p1) {
                ...
                // HUD Name Sync
                let p1Name = (window.introParams && window.introParams.p1Name ? window.introParams.p1Name : "PLAYER");
                let eName = (window.introParams && window.introParams.p2Name ? window.introParams.p2Name : "BOSS");
            ...
            // THANH TỶ LỆ WIN (POLL TỐI GIẢN - LOẠI BỎ CHỮ PLAYER/BOSS)
