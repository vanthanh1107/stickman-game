...
// [BẢN CẬP NHẬT CLEAN IMPACT - KHÔNG VẾT NỨT, HẠT ÁNH SÁNG BAY LÊN]
// ==========================================
...
window.spawnEnvDamage = function(x, y, type, scale, isBurning = false) {
    // ĐÃ XÓA TẠO VẾT NỨT MẶT ĐẤT ĐỂ GIỮ CHO SÂN ĐẤU SẠCH SẼ
};
...
// ==========================================
// 4. HỆ THỐNG VẬT LÝ NHẬN SÁT THƯƠNG & ATTACK (KHÔNG HIT STOP GIẬT LAG MÀN HÌNH)
// ==========================================
...
window.update = function() {
        ...
        for (let i = window.projectiles.length - 1; i >= 0; i--) {
            ...
            if (proj.isMeteor) {
                ...
                if (proj.y >= window.GROUND_Y) {
                    ...
                    // -- CẬP NHẬT MỚI: TẠO HẠT ÁNH SÁNG BAY LÊN THAY VÌ VẾT NỨT --
                    if (window.particles.length < MAX_PARTICLES) { 
                        for(let p=0; p<20; p++) { 
                            window.particles.push({ 
                                ...
                                vy: -Math.random()*6 - 4, // Lực đẩy lên mạnh
                                life: 45 + Math.random()*20, 
                                maxLife: 65, 
                                color: Math.random() > 0.5 ? "#f1c40f" : "#ff4757", 
                                size: Math.random()*6 + 4, 
                                isAuraFlame: true // Chống trọng lực, tự động bay lên trên và thu nhỏ dần
                            }); 
                        } 
                    }
        ...
        for (let i = window.envHazards.length - 1; i >= 0; i--) {
            ...
            if (haz.type === 'lightning') {
                if (haz.state === 'warning' && haz.timer <= 0) { 
                    ...
                    // SÉT ĐÁNH CŨNG BAY HẠT ÁNH SÁNG XANH LÊN THAY VÌ VẾT NỨT
                    if (window.particles.length < MAX_PARTICLES) { 
                        for(let p=0; p<15; p++) {
                            window.particles.push({ x: haz.x + (Math.random()-0.5)*40, y: window.GROUND_Y, vx: (Math.random()-0.5)*10, vy: -Math.random()*8 - 5, life: 30, maxLife: 30, color: "#00f3ff", size: Math.random()*5+3, isAuraFlame: true }); 
                        }
                    }
    ...
    if (window.gameOver && window.p1) {
        // NẾU TRẬN ĐẤU KẾT THÚC: Focus vào P1
