// ==========================================
// RECORDER.JS - BẢN HOÀN HẢO CHO CONTENT CREATOR (V51.0 - CINEMATIC CREATOR)
// [NEW] CINEMATIC FILM OVERLAY: Bụi điện ảnh, xước phim và tối góc thẳng vào video xuất ra.
// [NEW] HUMANIZED VIRAL TITLES: Tiêu đề kiểu Streamer/Gen-Z tập trung vào [Hero] vs [Enemy] (Chống AI).
// [NEW] HYPNOTIC OUTRO CTA: Màn hình "Tạo nhân vật" hiện lên, trôi nổi 3D siêu đẹp.
// ==========================================
...
window.savedVideos = [];
window.filmDustY = 0; // Biến chạy cho bụi điện ảnh
...
window.StoryModeAI = {
    ...
    generateScript: function(hero, enemy) {
        ...
        // CÔNG THỨC VIRAL MỚI: Tự nhiên, viết thường, tập trung vào Matchup giữa 2 nhân vật, dùng Icon Gen-Z
        const narratives = [
...
// ==========================================
// RENDER KHUNG HÌNH CHÍNH (TÍCH HỢP CINEMATIC OVERLAY)
// ==========================================
window.captureFrames = function() {
    ...
    // =====================================
    // [VIRAL 51.0] CINEMATIC FILM OVERLAY (CHỐNG AI QUÉT)
    // =====================================
    window.filmDustY += 1.5;
    
    let drawCinematicGrain = (ctx, w, h) => {
        ctx.save();
        // 1. Tối sâu 4 góc màn hình (Vignette)
        ...
        // 3. Vết xước phim ngẫu nhiên (Film Scratches)
        if (Math.random() > 0.85) {
        ...
        if (Math.random() > 0.95) {
            ...
            ctx.fillRect(Math.random() * w, 0, Math.random() * 4 + 1, h);
        }
        ctx.restore();
    };
