...
// ĐÃ FIX: Bố cục không che game, Chat trượt cuộn mượt tốc độ cao, Facecam đặt trên bảng Chat.
// NÂNG CẤP TỐI THƯỢNG: Mưa Tim, Sạn cố ý (Bait), Sai chính tả mồi Comment, Kill-Cam.
// TÂM LÝ HỌC: Super Chat Donation, Clutch Mode 1HP, Cơn bão Toxic Chat đa ngôn ngữ.
// ==========================================
...
window._chatScrollOffset = 0; // Hiệu ứng cuộn chat mượt
...
window.generateIntroParams = function() {
    ...
    let lore = r(LORES); let fakeChapter = makeTypo(lore.chapter);
    return { themeP1: t1, themeP2: t2, lore: { ...lore, chapter: fakeChapter }, badge: r(WAGER_BADGES) };
};

// ====================================================================
// HỆ THỐNG LIVE CHAT TOXIC CỰC GẮT ĐA NGÔN NGỮ
// ====================================================================
window.CELEB_LIST = [
...
window.generateLiveChatEvent = function(isClutch = false) {
    ...
    let msgList = isClutch ? CLUTCH_MSGS : TOXIC_MSGS;
...
window.captureFrames = function() {
    ...
    if (renderNormalV) {
        ...
        // ⌨️ APM TRACKER & ❤️ HEART RATE (TRENCH GAMER HUD)
        if (!window._apm) window._apm = 120;
        let targetApm = 120 + (audioPeak * 350);
        window._apm += (targetApm - window._apm) * 0.15;
        
        ctxV.save(); ctxV.translate(1080 - 130, 60);
        ctxV.fillStyle = "rgba(0, 0, 0, 0.65)"; ctxV.beginPath(); if(ctxV.roundRect) ctxV.roundRect(-110, -25, 220, 50, 12); else ctxV.fillRect(-110, -25, 220, 50); ctxV.fill();
        ctxV.fillStyle = window._apm > 250 ? "#ff0055" : "#ffeb3b"; ctxV.font = "900 22px 'Arial Black'"; ctxV.textAlign = "center"; ctxV.textBaseline = "middle"; ctxV.fillText("⌨️ APM: " + Math.floor(window._apm), 0, 2);
        ctxV.restore();
        ...
        ctxV.fillText(Math.floor(window._bpm) + " BPM", 15, 2); ctxV.restore();
        ...
        // QUY HOẠCH TOÀN BỘ UI STREAMER BÊN DƯỚI (BOTTOM AREA)
        // 1. CHAT BOX (Nằm trọn dưới đáy)
        let boxWidth = 960; let boxHeight = 650; 
        ...
        if (window._facecamImg && window._facecamImg.complete) {
        ...
        // 3. THÔNG BÁO TẶNG QUÀ / THẢ TIM (Bên phải Facecam, nằm phía trên Bảng Chat)
        ...
        for (let i = window._liveAlerts.length - 1; i >= 0; i--) {
            ...
            ctxV.translate(750, alertStartY - al.yOffset); // Dời sang bên phải khung cảnh
        ...
        // 4. BẢNG WIN RATE POLL (Nằm đè trên Game và khung dưới)
        ...
        if (window.p1) {
            ...
            let img1 = getHudImg(p1Url); let img2 = getHudImg(p2Url); let maxNameWidthV = 280; let hudBaseY = splitGameHeight + 30; // HUD đẩy lên ôm sát góc Game
        ...
        if (window.p1 && repEnemyObj) {
            ctxV.save(); let pollWidth = 600; let pollX = 1080 - 60 - pollWidth/2; let pollY = chatTopY - 60; ctxV.translate(pollX, pollY); // Dời Win Poll qua bên phải khung chat
            ...
            ctxV.textAlign = "left"; ctxV.fillText(`${Math.round(p1Pct*100)}%`, -pollWidth/2 + 20, 18); ctxV.textAlign = "right"; ctxV.fillText(`${Math.round((1-p1Pct)*100)}%`, pollWidth/2 - 20, 18); ctxV.restore();
        }

        // HIỂN THỊ SUPER CHAT DONATION
        if (window._donationAlert) {
            ...
            ctxV.translate(750, daY); // Sang góc phải để không đè Facecam
            ctxV.fillStyle = "rgba(255, 215, 0, 0.95)"; ctxV.shadowColor = "#ffb800"; ctxV.shadowBlur = 20;
            ctxV.beginPath(); if(ctxV.roundRect) ctx.roundRect(-300, -45, 600, 90, 45); else ctx.fillRect(-300, -45, 600, 90); ctxV.fill();
        ...
        // ====================================================================
        // ENGINE CHAT SCROLL TỐC ĐỘ CAO (MƯỢT MÀ)
        // ====================================================================
        if (!window._chatSystemInit) { 
            window._chatSystemInit = true; window._liveChats = []; window._lastChatUpdate = Date.now(); window._nextChatDelay = 500; window._chatScrollOffset = 0;
            for(let i = 0; i < 10; i++) { let c = window.generateLiveChatEvent(isClutchMode); window.precalcChatText(c, ctxV); window._liveChats.push(c); } 
        }

        if (!window._chatScrollOffset) window._chatScrollOffset = 0;
        if (window._chatScrollOffset > 0) window._chatScrollOffset -= 5; // Cuộn mượt lên trên
        ...
        if (chatNow - window._lastChatUpdate > window._nextChatDelay) { 
            ...
            window._nextChatDelay = isClutchMode ? 100 + Math.random() * 200 : 250 + Math.random() * 400; // Tăng tốc độ spam chat rất lớn
            ...
            window._chatScrollOffset += 45; // Đẩy khung chat xuống để tạo hiệu ứng cuộn lên
        ...
        // Vẽ Chữ Chat
        ctxV.save(); ctxV.beginPath(); ctxV.rect(-boxWidth/2, -boxHeight/2, boxWidth, boxHeight); ctxV.clip(); // Không bị tràn ra ngoài
        
        let currentY = boxHeight/2 - 20 + window._chatScrollOffset; // Bắt đầu in từ dưới đáy nhích xuống
        let lineHeight = 45; ctxV.textAlign = "left"; ctxV.textBaseline = "bottom"; ctxV.font = "bold 34px Arial"; let headerBottomY = -boxHeight/2; 
