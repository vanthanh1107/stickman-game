...
// Kèm theo RETENTION MULTIPLIER (Toxic Live Chat)
// ==========================================
...
window.captureFrames = function() {
    ...
    // ==========================================
    // 5.5. RETENTION MULTIPLIER: BIG TOXIC CHAT BOX
    // ==========================================
    
    // --- KHỞI TẠO BIẾN CHAT (Chỉ chạy 1 lần và đổ sẵn dữ liệu) ---
    if (!window.retentionToolsInit) {
        window.retentionToolsInit = true;
        
        window.chatNames = ["YasuoMain_99", "NoobMaster69", "SkibidiSigma", "Top1Challenger", "TouchGrass", "AnimePFP_01", "FakerWannabe", "SkillIssue", "AverageOhioPlayer", "GigaChad", "MewingStreak", "GokuSolo"];
        window.chatMsgs = [
            "Bro is playing with his feet 💀", 
        ...
        window.hypeMsgs = ["HE IS HACKING 🚨", "REPORTED!!", "NAHHHH 💀💀💀", "WHAT WAS THAT COMBO", "BRO COOKED 🔥🔥🔥", "W W W W", "WTF IS THIS DAMAGE?!", "AIMBOT DETECTED 🎯", "I STOPPED BREATHING", "HE LOCKED IN 🥶"];
        
        window.fakeChat = [];
        
        // Điền đầy sẵn tin nhắn vào khung ngay từ đầu (tránh cảm giác trống trơn biến mất)
        for(let i = 0; i < 7; i++) {
    ...
    if (!window.gameOver && window.introTimer > 120) {
        ...
        let chatDelay = isHype ? 100 : (700 + Math.random() * 1000); // Đánh căng thì spam liên tục
        ...
        if (nowTime - window.lastChatTime > chatDelay) {
            ...
            if (window.fakeChat.length > 7) window.fakeChat.shift(); // Giữ đúng 7 dòng để lấp đầy khung
        }

        ctxV.save();
        let chatBoxWidth = 850;
        let chatBoxHeight = 450;
        let chatBoxX = 540 - (chatBoxWidth / 2); // Đặt chính giữa màn hình
        let chatBoxY = splitGameHeight + 480; // Nằm ngay dưới VTuber 
        
        ctxV.translate(chatBoxX, chatBoxY); 
        
        // Vẽ background kính mờ to bự
        ...
        ctxV.fillText("👥 15.8K VIEWERS", chatBoxWidth - 25, 30);

        ctxV.font = "bold 32px 'Arial'"; ctxV.textAlign = "left"; ctxV.textBaseline = "top";
        
        // Render từng dòng chat (Cuộn lên êm ái)
