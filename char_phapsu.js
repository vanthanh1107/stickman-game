window.currentLoadedChar = {
    ...
    // ĐÒN ĐÁNH THƯỜNG: BẮN 3 VIÊN ĐẠN TỎA RA (SHOTGUN MAGIC)
    executeBasicAttack: function(caster, enemies) {
        caster.state = 'cast_magic'; caster.attackTimer = 25; 
        caster.vx = caster.isFacingRight ? -3 : 3; // Giật lùi
        
        if (typeof window.spawnProjectile === 'function') {
            let target = enemies[0];
            let dirX = caster.isFacingRight ? 12 : -12;
            let dmg = 8 * caster.dmgMod;
            // Bắn chùm 3 viên đạn tản ra
    ...
    skill: {
        actionCode1: function(caster, target, ctx) {
            caster.state = 'cast'; caster.attackTimer = 25; caster.vx = caster.isFacingRight ? -3 : 3;
        ...
        actionCode2: function(caster, target, ctx) {
            caster.state = 'cast'; caster.attackTimer = 30;
    ...
    executeUltimate: function(caster, target, baseDmg) {
        caster.state = 'cast'; caster.attackTimer = 60;
        if(typeof window.spawnProjectile === 'function') {
    ...
    // NÂNG CẤP ĐỒ HỌA: ÁO CHOÀNG DÀI VÀ TRẬN PHÁP DƯỚI CHÂN
    drawMethod: function(ctx, p, bounce, ext, pext, isTrail) {
        let pts = window.drawBaseLimb(ctx, p, bounce, ext, pext, isTrail);
        let {head, neck, pelvis, footL, kneeL, footR, kneeR, handL, elbowL, handR, elbowR} = pts;
        
        // Vòng tròn ma thuật dưới chân (Magic Circle)
        if (!isTrail) {
            ctx.save(); ctx.translate(p.x, window.GROUND_Y); ctx.scale(1, 0.3); // Tạo độ nghiêng 3D
        ...
        // Vẽ Áo choàng pháp sư (Robe) trùm từ cổ tới chân
        if (!isTrail) {
            ctx.fillStyle = "rgba(108, 92, 231, 0.8)";
            ctx.beginPath(); 
            ctx.moveTo(neck.x, neck.y); 
            ctx.lineTo(pelvis.x - 15, pelvis.y + 25); 
            ctx.lineTo(pelvis.x + 15, pelvis.y + 25); 
            ctx.closePath(); ctx.fill();
        }

        let isCasting = (p.state === 'cast' || p.state === 'cast_magic');
        let staffAngle = isCasting ? (p.isFacingRight ? Math.PI/3 : -Math.PI/3) : 0;
        
        // Trượng phép thuật chi tiết
        ctx.save();
