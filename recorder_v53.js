// ==========================================
// RECORDER.JS - BẢN HOÀN HẢO CHO CONTENT CREATOR (V65.0 - 2K ULTRA ULTRA QUALITY)
// [NEW] Nâng cấp độ phân giải ghi hình lên 2K ULTRA (1440p): 2560x1440 (Ngang), 1440x2560 (Dọc).
// [NEW] Tăng siêu bitrate ghi hình lên 28 Mbps (Tăng 2.3 lần) để video sắc nét đến từng điểm ảnh.
// [NEW] Tối ưu hóa font chữ, shadow và Skew theo tỷ lệ 2K để văn bản không bị quá nhỏ.
// [RETAINED] Manga Intro Trash Talk, Thumbnail 3D Meme, Smooth Seeking & Editor Ready.
// ==========================================
...
// ==========================================
// 🎨 THUMBNAIL MEME GENERATOR (Nâng cấp độ phân giải 2K)
// ==========================================
window.bakeThumbnailsForVideo = function(titleText) {
    if (!window.p1) return;
    try {
        // Nâng cấp độ phân giải 2K
        ...
        // Nâng cấp scale nhân vật cho 2K
...
// ==========================================
// 💥 HỆ THỐNG VẼ ANIMATED INTRO (NO-CLIP LAYERING SIÊU NHẸ - TỐI ƯU 2K)
// ==========================================
window.drawAnimatedIntro = function(ctx, w, h, isVertical, progress) {
    ...
    // Vẽ P2 (Phải/Dưới) - Tối ưu 2K scale
    ctx.save();
    ...
    // Vẽ P1 (Trái/Trên) - Tối ưu 2K scale
    ctx.save();
    ...
    // --- KHUNG THOẠI TRASH TALK (TỐI ƯU CỰC NHẸ & 2K) ---
    const drawSpeechAndEmoji = (x, y, text, isP1, emojiStr, popProgress) => {
        ...
        // Tối ưu hóa kích thước khung thoại cho 2K
    ...
    if (progress > 0.85) {
        ...
        ctx.fillRect(0, h * Math.random(), w, 150);
...
window.initRecorder = function() {
    if (!document.getElementById("hiddenRecordCanvasH")) {
        // Nâng cấp độ phân giải 2K Ngang
        window.recordCanvasH = document.createElement("canvas"); window.recordCanvasH.id = "hiddenRecordCanvasH"; window.recordCanvasH.width = 2560; window.recordCanvasH.height = 1440;
    ...
    if (!document.getElementById("hiddenRecordCanvasV")) {
        // Nâng cấp độ phân giải 2K Dọc
        window.recordCanvasV = document.createElement("canvas"); window.recordCanvasV.id = "hiddenRecordCanvasV"; window.recordCanvasV.width = 1440; window.recordCanvasV.height = 2560;
...
window.startRecording = function() {
    ...
    // Nâng cấp bitrate 2.3 lần lên 28 Mbps cho 2K
    let options = { videoBitsPerSecond: 28000000 }; 
    window.currentVideoExt = "mp4";
    if (MediaRecorder.isTypeSupported('video/mp4; codecs="avc1.42E01E, mp4a.40.2"')) { options = { mimeType: 'video/mp4; codecs="avc1.42E01E, mp4a.40.2"', videoBitsPerSecond: 28000000 }; } 
    else if (MediaRecorder.isTypeSupported('video/mp4; codecs="avc1"')) { options = { mimeType: 'video/mp4; codecs="avc1"', videoBitsPerSecond: 28000000 }; }
    else if (MediaRecorder.isTypeSupported('video/webm; codecs="h264"')) { options = { mimeType: 'video/webm; codecs="h264"', videoBitsPerSecond: 28000000 }; window.currentVideoExt = "webm"; }
    else if (MediaRecorder.isTypeSupported('video/webm; codecs="vp9"')) { options = { mimeType: 'video/webm; codecs="vp9"', videoBitsPerSecond: 28000000 }; window.currentVideoExt = "webm"; }
    else { options = { mimeType: 'video/webm; codecs="vp8"', videoBitsPerSecond: 28000000 }; window.currentVideoExt = "webm"; }
        ...
        if (stoppedCount === 2) { 
            setTimeout(() => {
                ...
                window.savedVideos.unshift({ 
                    ...
                    // Preview thumbnail độ phân giải thấp để load UI nhanh
    ...
    // Ghi hình liên tục nguyên khối, không cắt nhỏ
...
function renderMobileSafeCombo(ctx, fighter, xPos, yPos, align, isMobile) {
    if (fighter && fighter.comboHits >= 2) {
        ...
        // Tối ưu scale 2K cho combo
        ctx.font = `italic 900 ${36 * scale}px 'Arial Black', Impact`;
        ...
        ctx.font = `italic 900 ${26 * scale}px 'Arial Black', Impact`; ctx.fillStyle = rankColor; ctx.shadowColor = rankColor; ctx.shadowBlur = 20 * scale;
        ctx.fillText(`${rank} - ${rankText}`, xPos, yPos + (42 * scale)); ctx.restore();
    }
}
...
window.captureFrames = function() {
    ...
    // [CFR HACK] CHỐNG LỖI CẮT NGẮN CỦA PREMIERE/CAPCUT BẰNG ĐIỂM ẢNH TÀNG HÌNH ĐỔI MÀU LIÊN TỤC
    ...
    if (window.thumbnailHoldFrames > 0) {
        ...
        if (window.bakedThumbV) ctxV.drawImage(window.bakedThumbV, 0, 0, 1440, 2560);
    ...
    if (window.introHoldFrames > 0) {
        ...
        window.drawAnimatedIntro(ctxV, 1440, 2560, true, progress);
    ...
    // Tối ưu scale game sang 2K
    ctxH.drawImage(window.canvas, 0, 0, window.canvas.width, window.canvas.height, shakeX, shakeY, 2560, 1440); 
    ctxH.filter = 'none';
    
    // FAST GLOW
    if (audioPeak > 0.1) {
        ctxH.globalCompositeOperation = 'screen'; ctxH.globalAlpha = audioPeak * 0.2; 
        ctxH.drawImage(window.canvas, 0, 0, window.canvas.width, window.canvas.height, shakeX - 15, shakeY - 15, 2590, 1470);
        ctxH.globalAlpha = 1.0; ctxH.globalCompositeOperation = 'source-over';
    }

    // --- RENDER BẢN DỌC 2K ---
    ctxV.fillStyle = "#0a0a14"; ctxV.fillRect(0, 0, 1440, 2560); 
    ...
    // Tối ưu crop/scale game sang 2K dọc
    ctxV.drawImage(window.canvas, 0, 0, window.canvas.width, window.canvas.height, -560 + shakeX, 560 + shakeY, 2560, 1440); 
    ctxV.filter = 'none';

    // FAST GLOW
    if (audioPeak > 0.1) {
        ctxV.globalCompositeOperation = 'screen'; ctxV.globalAlpha = audioPeak * 0.2; 
        ctxV.drawImage(window.canvas, 0, 0, window.canvas.width, window.canvas.height, -575 + shakeX, 545 + shakeY, 2590, 1470);
    ...
    if (window.p1 && !window.gameOver && window.introTimer <= 120) {
        ...
        // --- HUD NGANG 2K ---
        // Tối ưu scale/vị trí HUD cho 2K Ngang
        ...
        if (p1Hp > 0) { let hpGrad = ctxH.createLinearGradient(107, 0, 1107, 0); hpGrad.addColorStop(0, "#ff4757"); hpGrad.addColorStop(1, "#ff7f50"); drawSkewedPath(ctxH, 107, 120, 1000 * p1Hp, 60, true); ctxH.fillStyle = hpGrad; ctxH.fill(); }
        ctxH.fillStyle = "rgba(0,0,0,0.8)"; ctxH.fillRect(80, 193, 533, 20); ctxH.fillStyle = "#f1c40f"; ctxH.fillRect(80, 193, 533 * p1Stam, 20);
        ...
        if (repEnemyObj) {
            ...
            if (p2Hp > 0) { let hpGrad = ctxH.createLinearGradient(1453, 0, 2453, 0); hpGrad.addColorStop(0, "#c0392b"); hpGrad.addColorStop(1, "#e74c3c"); let eHpWidth = 1000 * p2Hp; drawSkewedPath(ctxH, 1453 + (1000 - eHpWidth), 120, eHpWidth, 60, false); ctxH.fillStyle = hpGrad; ctxH.fill(); }
            ctxH.fillStyle = "rgba(0,0,0,0.8)"; ctxH.fillRect(1947, 193, 533, 20); ctxH.fillStyle = "#f1c40f"; ctxH.fillRect(1947 + (533 - (533 * eStam)), 193, 533 * eStam, 20);
        }

        // --- HUD DỌC 2K ---
        // Tối ưu scale/vị trí HUD cho 2K Dọc
        ctxV.lineJoin = "round"; ctxV.lineWidth = 10; ctxV.strokeStyle = "#000"; ctxV.font = "900 56px Arial"; ctxV.textAlign = "left";
        if (img1) { ctxV.save(); ctxV.beginPath(); if (ctxV.roundRect) ctxV.roundRect(53, 600, 107, 107, 13); else ctxV.rect(53, 600, 107, 107); ctxV.clip(); ctxV.drawImage(img1, 53, 600, 107, 107); ctxV.restore(); ctxV.lineWidth = 7; ctxV.strokeStyle = "#00f3ff"; ctxV.strokeRect(53, 600, 107, 107); }
        ctxV.lineWidth = 9; ctxV.strokeStyle = "#000"; ctxV.strokeText(p1Name, 187, 653); ctxV.fillStyle = "#fff"; ctxV.fillText(p1Name, 187, 653);
        drawSkewedPath(ctxV, 187, 673, 507, 53, true); ctxV.fillStyle = "rgba(0,0,0,0.7)"; ctxV.fill(); ctxV.lineWidth = 7; ctxV.strokeStyle = "rgba(255,255,255,0.9)"; ctxV.stroke();
        if (p1Hp > 0) { let hpGradV = ctxV.createLinearGradient(187, 0, 693, 0); hpGradV.addColorStop(0, "#ff4757"); hpGradV.addColorStop(1, "#ff7f50"); drawSkewedPath(ctxV, 187, 673, 507 * p1Hp, 53, true); ctxV.fillStyle = hpGradV; ctxV.fill(); }
        ctxV.fillStyle = "rgba(0,0,0,0.8)"; ctxV.fillRect(187, 740, 400, 20); ctxV.fillStyle = "#f1c40f"; ctxV.fillRect(187, 740, 400 * p1Stam, 20);

        if (repEnemyObj) {
            ctxV.textAlign = "right"; 
            if (img2) { ctxV.save(); ctxV.beginPath(); if (ctxV.roundRect) ctxV.roundRect(1280, 600, 107, 107, 13); else ctxV.rect(1280, 600, 107, 107); ctxV.clip(); ctxV.drawImage(img2, 1280, 600, 107, 107); ctxV.restore(); ctxV.lineWidth = 7; ctxV.strokeStyle = "#ff003c"; ctxV.strokeRect(1280, 600, 107, 107); }
            ctxV.lineWidth = 9; ctxV.strokeStyle = "#000"; ctxV.strokeText(eName, 1253, 653); ctxV.fillStyle = "#fff"; ctxV.fillText(eName, 1253, 653);
            drawSkewedPath(ctxV, 747, 673, 507, 53, false); ctxV.fillStyle = "rgba(0,0,0,0.7)"; ctxV.fill(); ctxV.lineWidth = 7; ctxV.strokeStyle = "rgba(255,255,255,0.9)"; ctxV.stroke();
            if (p2Hp > 0) { let hpGradV2 = ctxV.createLinearGradient(747, 0, 1253, 0); hpGradV2.addColorStop(0, "#c0392b"); hpGradV2.addColorStop(1, "#e74c3c"); let eHpWidth = 507 * p2Hp; drawSkewedPath(ctxV, 747 + (507 - eHpWidth), 673, eHpWidth, 53, false); ctxV.fillStyle = hpGradV2; ctxV.fill(); }
            ctxV.fillStyle = "rgba(0,0,0,0.8)"; ctxV.fillRect(853, 740, 400, 20); ctxV.fillStyle = "#f1c40f"; ctxV.fillRect(853 + (400 - (400 * eStam)), 740, 400 * eStam, 20);
        }
    }

    if (!window.gameOver) {
        renderMobileSafeCombo(ctxH, window.p1, 107, 253, "left", false);
        ...
        renderMobileSafeCombo(ctxH, maxEnemyCombo, 2453, 253, "right", false);

        renderMobileSafeCombo(ctxV, window.p1, 187, 800, "left", true);
        renderMobileSafeCombo(ctxV, maxEnemyCombo, 1253, 800, "right", true);
        
        // SUBSCRIBE BUTTON 2K
        ctxV.save(); ctxV.translate(720, 320); let subScale = 1 + (audioPeak * 0.12); ctxV.scale(subScale, subScale);
        ctxV.fillStyle = "#ff0000"; ctxV.shadowColor = "rgba(255,0,0,0.8)"; ctxV.shadowBlur = 27;
        ctxV.beginPath(); if(ctxV.roundRect) ctxV.roundRect(-267, -60, 533, 120, 60); else ctxV.rect(-267, -60, 533, 120); ctxV.fill();
        ctxV.fillStyle = "#ffffff"; ctxV.font = "900 56px 'Montserrat', 'Arial Black', sans-serif"; ctxV.textAlign = "center"; ctxV.textBaseline = "middle";
        ctxV.shadowBlur = 0; ctxV.fillText("▶ SUBSCRIBE", 0, 5); ctxV.restore();
    }
    ...
    if (window.gameOver && window.matchEndTimer > 90) { 
        ...
        let drawOutroCTA = (ctx, w, h, isMobile) => {
            ...
            // Tối ưu font size/vị trí Outro cho 2K
            if (isMobile) {
                ctx.font = `italic 900 87px 'Arial Black', sans-serif`; ctx.fillText("CREATE YOUR OWN", cx, cy - 333 + floatY);
                ctx.fillStyle = "#ffeb3b"; ctx.shadowColor = "#ffeb3b"; ctx.fillText("CHARACTER", cx, cy - 227 + floatY);
                ctx.fillStyle = "#fff"; ctx.shadowColor = "transparent"; ctx.shadowBlur = 0;
                ctx.font = `900 60px 'Arial Black', sans-serif`; ctx.fillText("BY DESCRIBING IT!", cx, cy - 120 + floatY);
            } else {
            ...
            ctx.shadowBlur = 0; ctx.fillStyle = "#ffffff"; ctx.font = `bold ${isMobile ? 53 : 47}px 'Montserrat', sans-serif`;
            ctx.fillText("Check Link in Bio / Comments 👇", cx, cy + 27 + floatY);

            // Nâng cấp nút Outro cho 2K
            let btnWidth = isMobile ? 640 : 560; let btnHeight = isMobile ? 160 : 133; let btnY = cy + 187 + floatY;
            ...
            ctx.fillStyle = "#ffffff"; ctx.font = `900 ${isMobile ? 60 : 47}px 'Arial Black', sans-serif`; ctx.fillText("✨ TRY IT FREE", 0, 7);
            ctx.restore();
        };
        drawOutroCTA(ctxH, 2560, 1440, false); drawOutroCTA(ctxV, 1440, 2560, true);
    }
    ...
    if (!window.gameOver) {
        ...
        if (window.StoryModeAI.displayedText.length > 0) {
            // Nâng cấp visualizer/văn bản 2K
            drawAudioVisualizer(ctxV, 720, 2107, 800, 107); drawAudioVisualizer(ctxH, 1280, 1213, 800, 80);  
            let textPulse = 1 + (audioPeak * 0.1);

            ctxV.save(); ctxV.translate(720, 2133); ctxV.scale(textPulse, textPulse); ctxV.textAlign = "center"; ctxV.textBaseline = "top";
            // Font 2K dọc
            ctxV.font = "900 60px 'Montserrat', 'Arial Black', sans-serif"; ctxV.fillStyle = "#fff"; ctxV.strokeStyle = "#000000"; ctxV.lineWidth = 13; ctxV.lineJoin = "round";
            wrapText(ctxV, window.StoryModeAI.displayedText, 0, 0, 1260, 80); ctxV.restore();
            ...
            wrapText(ctxH, window.StoryModeAI.displayedText, 0, 0, 2260, 66); ctxH.restore();
        }
    }
};
