// ==========================================
// RECORDER.JS - V77 FINAL PERFECT EDITION
// ĐÃ TÍCH HỢP: Make.com Webhook Silent Upload (Chống CORS)
// ĐÃ TỐI ƯU: Ép xung dung lượng dưới 5MB để không bị nghẽn mạng
// NÂNG CẤP: ANIME SLASH PERFECT LOOP & KINETIC SUBTITLES
// ==========================================

// --- CẤU HÌNH WEBHOOK (MAKE.COM) ---
window.WEBHOOK_URL = "https://hook.eu1.make.com/ixuwwn40w36mgmg3gpcyviq2gvwlns7w";

window.sendVideoToWebhook = async function(videoBlob, title, description) {
    window.showRenderToast("☁️ ĐANG GỬI LÊN MÁY CHỦ YOUTUBE... (KHÔNG ĐÓNG TAB)");

    let formData = new FormData();
    formData.append("videoFile", videoBlob, "stickman_combo.mp4");
    ...
    try {
        let response = await fetch(window.WEBHOOK_URL, {
            method: "POST",
            mode: "cors", // FIX LỖI CORS QUAN TRỌNG
            body: formData
        });

        if (response.ok) {
            window.showRenderToast(`✅ ĐÃ TẢI LÊN MÁY CHỦ THÀNH CÔNG!`);
            setTimeout(() => window.hideRenderToast(), 4000);
            return true;
        } else {
            window.showRenderToast("❌ LỖI TẢI LÊN MÁY CHỦ (Có thể File > 5MB)!");
            return false;
        }
    } catch (e) {
        console.error("Lỗi Webhook:", e);
        window.showRenderToast("❌ LỖI MẠNG KHI GỬI VIDEO (Tắt Adblock nếu có)!");
        return false;
    }
};
...
window.startRecording = function() {
    ...
    // TỐI ƯU HÓA BITRATE (2.5 Mbps) ĐỂ ĐẢM BẢO VIDEO < 5MB (TRÁNH LỖI MAKE.COM)
...
window.mergeAndDownloadVideo = async function(vidId) {
    ...
    recorder.onstop = async () => {
        ...
        // GỌI HÀM BẮN LÊN MAKE.COM
        let isSuccess = await window.sendVideoToWebhook(finalBlob, vid.viralTitle, postKitText);
        
        if (isSuccess) {
            if(btn) { 
                btn.innerHTML = `✅ ĐÃ UPLOAD THÀNH CÔNG`; 
