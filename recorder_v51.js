// ==========================================
// RECORDER.JS - BẢN HOÀN HẢO CHO CONTENT CREATOR (V64.0 - ZERO-LAG ESPORTS INTRO)
// [FIXED] Loại bỏ hoàn toàn ctx.clip(), measureText và shadowBlur trong vòng lặp Intro.
// [NEW] Kỹ thuật "No-Clip Layering": 3D Pop-out nhân vật siêu nhẹ, tăng x10 hiệu năng.
// [NEW] Chuyển cảnh Fast RGB Glitch: Xé ngang màn hình siêu mượt không độ trễ.
...
window.totalIntroFrames = 150; // 2.5s Intro
...
// ==========================================
// 🎨 THUMBNAIL MEME GENERATOR (ẢNH TĨNH)
// ==========================================
...
// ==========================================
// 💥 HỆ THỐNG ANIMATED INTRO (NO-CLIP LAYERING SIÊU NHẸ)
// ==========================================
window.drawAnimatedIntro = function(ctx, w, h, isVertical, progress) {
    ...
    // --- BƯỚC 1: LÀM NỀN P2 (ĐỎ) PHỦ KÍN MÀN HÌNH TRƯỚC ---
    ...
    // Marquee Background (Dải cảnh báo)
    ...
    // Vẽ P2 (Phải/Dưới) - Không dùng shadow để cứu FPS
    ctx.save();
    ...
    // --- BƯỚC 2: VẼ ĐÈ POLYGON P1 (XANH) CẮT CHÉO MÀN HÌNH ---
    ctx.fillStyle = "rgba(0, 15, 30, 0.95)"; // Đen xanh
    ctx.beginPath();
    ...
    // Speedlines Xanh cho P1
    ctx.save(); ctx.beginPath();
    if (isVertical) { ctx.moveTo(-w,-h); ctx.lineTo(w*2,-h); ctx.lineTo(w*2, h*0.48); ctx.lineTo(-w, h*0.52); }
    else { ctx.moveTo(-w,-h); ctx.lineTo(w*0.55, -h); ctx.lineTo(w*0.45, h*2); ctx.lineTo(-w, h*2); }
    ctx.clip(); // Dùng clip duy nhất 1 lần ở đây cho speedline xanh
    ...
    // Rung khung hình lúc va chạm
    if (progress > 0.25 && progress < 0.4) {
        let intensity = (0.4 - progress) * 80;
        ctx.translate((Math.random() - 0.5) * intensity, (Math.random() - 0.5) * intensity);
    }

    // --- BƯỚC 3: VẼ KHUNG THOẠI TRASH TALK (TỐI ƯU CỰC NHẸ) ---
    const drawSpeechAndEmoji = (x, y, text, isP1, emojiStr, popProgress) => {
        ...
        // Giả lập bóng đổ bằng hình khối (Nhanh gấp 100 lần shadowBlur)
        let tw = text.length * 20 + 80; let th = 90; // Đo thủ công
        
        ctx.save(); ctx.transform(1, 0, -0.1, 1, 0, 0); 
        ctx.fillStyle = "#000"; ctx.fillRect(-tw/2 + 10, -th/2 + 10, tw, th); // Fake Shadow
        ...
        ctx.font = "italic 900 38px 'Arial Black', Impact"; ctx.textAlign = "center"; ctx.textBaseline = "middle";
        ctx.fillText(text, 0, 0);
        
        // Vẽ Emoji
        let emX = isP1 ? tw/2 + 30 : -tw/2 - 30; let emY = -th/2 - 10;
        let tPulse = Date.now() / 80; 
        ctx.translate(emX, emY + Math.sin(tPulse) * 15); ctx.rotate(Math.sin(tPulse * 0.5) * 0.3);
        ctx.font = "80px Arial"; ctx.fillText(emojiStr, 0, 0);
        ctx.font = "60px Arial"; ctx.fillText(isP1 ? "⁉️" : "🧂", -70, -50); 
    ...
    // --- BƯỚC 4: RGB GLITCH CHUYỂN CẢNH SIÊU MƯỢT ---
    if (progress > 0.85) {
        let flashAlpha = Math.min(1, (progress - 0.85) * 6.6);
        ctx.globalCompositeOperation = 'screen';
        
        // Vẽ vệt nhiễu ngang ngẫu nhiên thay vì copy toàn màn hình
...
window.startRecording = function() {
    ...
    const p1Talks = ["Bro skipped tutorial 🤡", "Your Jordans are fake.", "Touch grass.", "My grandma plays better 💀", "You smell like onions.", "Hold this L bozo."];
    const p2Talks = ["Skill issue detected.", "I'm lagging bro 📶", "Mom said it's my turn 😡", "I'll put dirt in your eye.", "You're getting uninstalled 🗑️", "Look at this clown 🤡"];
    const emos = ["🤬", "🤡", "💀", "😭", "💦", "🔥", "🥶", "📉", "🤫", "🗿", "💩"];
    const banners = ["🚨 ALERT: SWEATY TRYHARD DETECTED 🚨", "🤡 BOZO INCOMING - DO NOT ENGAGE 🤡", "⚠️ ERROR 404: SKILL NOT FOUND ⚠️"];
    const stats = ["IQ: 10", "IQ: 9999", "PING: 999", "SWEAT: 100%", "NOOB: TRUE", "HACKS: ON"];
