// ==========================================
// RECORDER.JS - BẢN HOÀN HẢO CHO CONTENT CREATOR (V66.0 - TIKTOK SPLIT-SCREEN RETENTION META)
// [NEW] Bố cục Split-Screen: Game 16:9 ở trên, Retention Zone Cyberpunk ở dưới.
// [NEW] Dịch chuyển toàn bộ HUD, Combo, AI Text xuống Retention Zone cực rõ nét.
// [NEW] Tối ưu font chữ, shadow và Skew theo tỷ lệ Split-screen.
// [RETAINED] 2K ULTRA (1440p), Siêu bitrate 28 Mbps, Manga Intro, Storytelling AI.
// ==========================================
...
window.captureFrames = function() {
    ...
    ctxV.drawImage(window.canvas, 0, 0, window.canvas.width, window.canvas.height, 0 + shakeX, 0 + shakeY, 1440, splitGameHeight); 
    ctxV.filter = 'none';

    // FAST GLOW CHO GAME
    if (audioPeak > 0.1) {
        ctxV.globalCompositeOperation = 'screen'; ctxV.globalAlpha = audioPeak * 0.2; 
        ctxV.drawImage(window.canvas, 0, 0, window.canvas.width, window.canvas.height, -10 + shakeX, -10 + shakeY, 1460, splitGameHeight + 20);
        ctxV.globalAlpha = 1.0; ctxV.globalCompositeOperation = 'source-over';
    }

    // 3. BACKGROUND CHO RETENTION ZONE (Phần dưới video)
    let retainY = splitGameHeight; 
    let retainHeight = 2560 - retainY;
    
    // Tạo lưới Cyberpunk chuyển động nhẹ ở background dưới
    ctxV.save();
    ctxV.beginPath(); ctxV.rect(0, retainY, 1440, retainHeight); ctxV.clip();
    let bgPulse = (Date.now() / 2000) % (Math.PI * 2);
    let gradBg = ctxV.createLinearGradient(0, retainY, 0, 2560);
    gradBg.addColorStop(0, "#05050a"); gradBg.addColorStop(1, `rgba(0, ${20 + Math.sin(bgPulse)*20}, 40, 1)`);
    ctxV.fillStyle = gradBg; ctxV.fillRect(0, retainY, 1440, retainHeight);
    
    ctxV.strokeStyle = "rgba(0, 243, 255, 0.05)"; ctxV.lineWidth = 2;
    let gridOffset = (Date.now() / 20) % 50;
    for(let i = 0; i < 1440; i+= 50) { ctxV.beginPath(); ctxV.moveTo(i, retainY); ctxV.lineTo(i, 2560); ctxV.stroke(); }
    for(let j = retainY; j < 2560; j+= 50) { ctxV.beginPath(); ctxV.moveTo(0, j + gridOffset); ctxV.lineTo(1440, j + gridOffset); ctxV.stroke(); }
    
    // Chữ "WAIT FOR IT..." nhấp nháy để giữ chân
    ctxV.textAlign = "center"; ctxV.textBaseline = "middle";
    ctxV.font = "900 45px 'Arial Black'";
    ctxV.fillStyle = `rgba(255, 235, 59, ${0.5 + Math.sin(Date.now()/150)*0.5})`;
    ctxV.fillText("👇 WAIT FOR IT 👇", 720, retainY + 40);
    ctxV.restore();
    ...
    if (window.p1 && !window.gameOver && window.introTimer <= 120) {
        ...
        // --- HUD DỌC 2K (DỊCH CHUYỂN XUỐNG DƯỚI VIDEO BẰNG SPLIT-SCREEN) ---
        let hudBaseY = splitGameHeight + 120; // Đặt HUD bắt đầu từ ngay dưới video
        ...
        if (p1Hp > 0) { let hpGradV = ctxV.createLinearGradient(187, 0, 693, 0); hpGradV.addColorStop(0, "#ff4757"); hpGradV.addColorStop(1, "#ff7f50"); drawSkewedPath(ctxV, 187, hudBaseY + 73, 507 * p1Hp, 53, true); ctxV.fillStyle = hpGradV; ctxV.fill(); }
        ctxV.fillStyle = "rgba(0,0,0,0.8)"; ctxV.fillRect(187, hudBaseY + 140, 400, 20); ctxV.fillStyle = "#f1c40f"; ctxV.fillRect(187, hudBaseY + 140, 400 * p1Stam, 20);
        ...
        if (repEnemyObj) {
            ...
            if (p2Hp > 0) { let hpGradV2 = ctxV.createLinearGradient(747, 0, 1253, 0); hpGradV2.addColorStop(0, "#c0392b"); hpGradV2.addColorStop(1, "#e74c3c"); let eHpWidth = 507 * p2Hp; drawSkewedPath(ctxV, 747 + (507 - eHpWidth), hudBaseY + 73, eHpWidth, 53, false); ctxV.fillStyle = hpGradV2; ctxV.fill(); }
            ctxV.fillStyle = "rgba(0,0,0,0.8)"; ctxV.fillRect(853, hudBaseY + 140, 400, 20); ctxV.fillStyle = "#f1c40f"; ctxV.fillRect(853 + (400 - (400 * eStam)), hudBaseY + 140, 400 * eStam, 20);
        }
    }
    ...
    if (!window.gameOver) {
        ...
        // Dịch chuyển thông báo Combo bản dọc xuống Retention Zone
        let comboY = (typeof splitGameHeight !== 'undefined') ? splitGameHeight + 320 : 800;
        renderMobileSafeCombo(ctxV, window.p1, 187, comboY, "left", true);
        renderMobileSafeCombo(ctxV, maxEnemyCombo, 1253, comboY, "right", true);
    ...
    if (!window.gameOver) {
        ...
        if (window.StoryModeAI.displayedText.length > 0) {
            // Đặt visualizer làm bệ đỡ cho Text AI ở không gian trống phía dưới
            let visY = 2050; // Vị trí mới cho bản dọc Split-Screen
            drawAudioVisualizer(ctxV, 720, visY, 1000, 150); 
            drawAudioVisualizer(ctxH, 1280, 1213, 800, 80);  
            
            let textPulse = 1 + (audioPeak * 0.1);

            // Tối ưu hóa Text AI cho Retention Zone (Bản dọc)
            ctxV.save(); ctxV.translate(720, visY + 30); ctxV.scale(textPulse, textPulse); ctxV.textAlign = "center"; ctxV.textBaseline = "top";
            // Làm Font to và béo hơn để dễ đọc trên điện thoại
            ...
            wrapText(ctxV, window.StoryModeAI.displayedText, 0, 0, 1300, 100); 
            ctxV.restore();

            // Giữ nguyên Text AI cho bản ngang
            ...
            wrapText(ctxH, window.StoryModeAI.displayedText, 0, 0, 2260, 66); 
            ctxH.restore();
        }
    }
};
