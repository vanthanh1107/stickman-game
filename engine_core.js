    ...
    if (!isTimeStopped) {
        for (let i = window.projectiles.length - 1; i >= 0; i--) {
            ...
            if (proj.isMeteor) {
                ...
                if (Math.sqrt(dx*dx + dy*dy) < proj.radius + 20) { if(proj.onHit) proj.onHit(); if(typeof window.takeDamage === 'function') window.takeDamage(proj.target, proj.dmg, proj.color || "#e74c3c", false, false, proj.vx > 0); window.shakeScreen(8, 4); window.projectprojectiles.splice(i, 1); }
