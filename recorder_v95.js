...
// Tối ưu Layout: Clean Game -> Banner -> HUD -> Smart Live Alerts -> Live Chat -> Green Screen Cat 
// ==========================================
    ...
    if (!isOutroActive) {
        ...
        if (window.p1) {
            ...
            if (repEnemyObj) {
                ...
                // --- [HOOK 3: FAKE TIKTOK SEEKBAR] ---
                let vidProgress = Math.max(0, 1 - p2Hp); 
                ctxV.save();
                ctxV.fillStyle = "rgba(255, 255, 255, 0.2)"; 
                ctxV.fillRect(0, 1910, 1080, 10); 
                ctxV.fillStyle = "#ffffff";
                ctxV.shadowColor = "#ffffff"; ctxV.shadowBlur = 10;
                ctxV.fillRect(0, 1910, 1080 * vidProgress, 10); 
                ctxV.beginPath(); 
        ...
        // --- [HOOK 2: SMART LIVE ALERTS] THÔNG BÁO FOLLOW/QUÀ ẢO CHỐNG CẮT LỀ ---
        // Nằm ở khu vực an toàn (Y = ~890, X = 40)
        if (Date.now() - window._lastAlertTime > 2500 + Math.random() * 3000) {
            window._lastAlertTime = Date.now();
            
            // Lấy tên người ngẫu nhiên từ bảng chat (Bỏ đi icon để trông giống Tên hiển thị thật)
        ...
        let alertStartY = splitGameHeight + 290; // Y ~ 897 (An toàn, không bị che bởi HUD)
        for (let i = window._liveAlerts.length - 1; i >= 0; i--) {
            let al = window._liveAlerts[i];
            al.life -= 0.015; // Mờ dần
            al.yOffset += 1.2; // Nổi dần lên trên
            
            ctxV.save();
            ctxV.globalAlpha = Math.max(0, Math.min(1, al.life * 1.5));
            ctxV.translate(40, alertStartY - al.yOffset); // Dịch vào 40px tránh bị TikTok crop
            
            ctxV.fillStyle = "rgba(0, 0, 0, 0.5)"; // Nền kính mờ
            ...
            if(ctxV.roundRect) { 
                ctxV.beginPath(); ctxV.roundRect(0, 0, textW + 40, 46, 23); 
                ctxV.fill(); ctxV.stroke(); 
            } else {
                ctxV.fillRect(0, 0, textW + 40, 46);
            }
            
            ctxV.fillStyle = "#fff";
            ctxV.fillText(al.text, 20, 32);
            ctxV.restore();
