// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 29.0 [THE SEAMLESS FLOW]
// [NEW] FLUID TIME-RAMPING: Loại bỏ hoàn toàn sự giật cục/đơ/khựng frame (Hit-stop). 
// [NEW] SEAMLESS COMBAT: Mọi đòn đánh, so găng đều chuyển qua Slow-motion mượt mà, giúp animation liên tục 100%.
// [BASE] Giữ nguyên Camera 2.5D, Kinetic Typography và các tính năng AAA.
// ==========================================
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce, attackerDirRight = true) {
    ...
    if (target.state === 'block') { 
        if (target.stamina < 15) {
            ...
            // [LIÊN HOÀN] Thay thế Freeze bằng Time Ramping
            ...
            target.stamina = Math.min(100, target.stamina + 30); window.shakeScreen(15, 10); 
            // [LIÊN HOÀN]
            window.targetTimeScale = 0.5; setTimeout(()=>{window.targetTimeScale=1.0;}, 200);
            
            window.chromaTimer = 10; window.impactAberration = 15;
    ...
    if (finalDmg > 0) {
        ...
        if (isCrit) {
            ...
            // [LIÊN HOÀN] Time-Ramping thay vì đóng băng
            window.targetTimeScale = 0.2; 
            setTimeout(()=>{ window.targetTimeScale = 1.0; }, 250); 
        ...
        if (target.hp <= 0) {
            ...
            if (isNearWall && isCrit && typeof window.triggerStageTransition === 'function') { window.triggerStageTransition(target); } 
                ...
                if (isCrit) {
                    ...
                    // [LIÊN HOÀN] Chuyển mượt vào slow-mo siêu chậm (không đóng băng)
                    window.targetTimeScale = 0.05; 
                    setTimeout(()=>{ 
                        window.targetTimeScale = 1.0; 
                        window.cameraTilt = 0; 
                    ...
                    window.shakeScreen(30, 25); window.targetZoom = 1.5; window.actionCamOffsetY = -40; 
                    window.impactAberration = 20; 
                    window.actionCamSkew = attackerDirRight ? 0.05 : -0.05;
                    // [LIÊN HOÀN]
                    window.targetTimeScale = 0.2; setTimeout(()=>{window.targetTimeScale = 1.0; }, 1000); 
                }
            ...
            window.impactFrameTimer = 3; window.shakeScreen(15, 12); window.chromaTimer = 15; window.playSound(180, 'square', 0.3, 0.6, true);
            // [LIÊN HOÀN]
            window.targetTimeScale = 0.3; setTimeout(()=>{ window.targetTimeScale = 1.0; }, 200);
        } else { 
            window.shakeScreen(5, 4); window.playSound(250, 'sine', 0.15, 0.3, true); 
        }
    }
};
...
window.update = function() {
    ...
    // [LIÊN HOÀN] Không Return ở đây nữa, giữ dòng chảy mượt mà
            ...
            if (f.hitStun <= 0 && f.stunTimer <= 0 && f.state !== 'dash_back' && f.state !== 'block') {
                ...
                if (isCloseEnough) {
                    if (window.classStats && window.classStats[type] && typeof window.classStats[type].executeUltimate === 'function') {
                        ...
                        window.actionCamSkew = f.isFacingRight ? 0.1 : -0.1; 
            ...
            for (let j = i + 1; j < allFighters.length; j++) { 
                ...
                if (dist < 80 && f1.attackTimer > 5 && f1.attackTimer < 20 && f2.attackTimer > 5 && f2.attackTimer < 20) {
                    if (!f1.clashCooldown && !f2.clashCooldown && f1.isFacingRight !== f2.isFacingRight) {
                        ...
                        window.actionCamSkew = f1.isFacingRight ? 0.2 : -0.2; 
                        
                        // [LIÊN HOÀN] Time-Ramping thay cho Freeze
                        window.targetTimeScale = 0.1; 
                        setTimeout(() => { window.targetTimeScale = 1.0; }, 350);
                    }
                }
...
window.draw = function() {
    ...
    try {
        ...
        let vanishingPointX = window.canvas.width / 2 + window.camX * 0.3; 
...
window.gameLoop = function(timestamp) { 
    ...
    // [LIÊN HOÀN] Chạy physics kể cả khi khựng (Bản V27.1 đã xóa đóng băng)
