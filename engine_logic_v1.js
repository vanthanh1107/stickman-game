...
// NÂNG CẤP: HỆ THỐNG COMBO, JUGGLING, WALL BOUNCE, GROUND BOUNCE
// ==========================================
...
// ==========================================
// THƯ VIỆN COMBO & CHIÊU THỨC (NÂNG CẤP)
// ==========================================
window.MOVE_DATABASE = {
    // Đòn nhẹ (Light - Nhanh, stun ngắn)
    'jab': { dmgMod: 0.5, stun: 12, pushX: 1.5, pushY: 0, fx: 'light', type: 'light' },
    'low_kick': { dmgMod: 0.6, stun: 14, pushX: 2, pushY: 0, fx: 'light', type: 'light' },
    
    // Đòn trung (Medium - Nối combo)
    'cross': { dmgMod: 0.9, stun: 18, pushX: 3, pushY: 0, fx: 'medium', type: 'mid' },
    'spinning_kick': { dmgMod: 1.0, stun: 20, pushX: 4, pushY: 0, fx: 'medium', type: 'mid' },
    
    // Đòn hất tung (Launcher - Bắt đầu Juggling / Không chiến)
    'uppercut': { dmgMod: 1.2, stun: 30, pushX: 1.5, pushY: -14, fx: 'heavy', type: 'launcher' },
    'shoryuken': { dmgMod: 1.5, stun: 35, pushX: 2, pushY: -16, fx: 'critical', type: 'launcher' },

    // Đòn dậm đất (Ground Bounce - Dập kẻ địch từ trên không xuống)
    'axe_kick': { dmgMod: 1.4, stun: 25, pushX: 0, pushY: 15, fx: 'heavy', type: 'smash' },
    
    // Đòn liên hoàn (Multi-hit)
    'ora_ora': { dmgMod: 0.3, stun: 10, pushX: 0.5, pushY: -1, fx: 'multi', hits: 5, type: 'flurry' },
    'chun_li_kicks': { dmgMod: 0.25, stun: 12, pushX: 0.5, pushY: -0.5, fx: 'multi', hits: 7, type: 'flurry' },

    // Đòn kết liễu (Finisher - Tương tác Camera mạnh, đẩy lùi xa, dính tường)
    'dragon_punch': { dmgMod: 3.5, stun: 50, pushX: 18, pushY: -5, fx: 'finisher', type: 'finisher' },
    'asura_strike': { dmgMod: 4.0, stun: 60, pushX: 25, pushY: -2, fx: 'finisher', type: 'finisher' }
};
...
// ==========================================
window.takeDamage = function(target, amount, color, isCrit, wallBounce, attackerDirRight = true, moveData = null) {
    ...
    if (finalDmg > 0) {
        ...
        // VẬT LÝ NHẬN ĐÒN & BOUNCE
        if (target.superArmor <= 0 && target.state !== 'stunned') { 
            target.state = 'hurt'; 
            target.hitStun = moveData ? moveData.stun : (isCrit ? 22 : 12); 
            target.attackTimer = 0; 
            target.comboStep = 0; 
            
            if (moveData) {
                let pushX = moveData.pushX || 2;
                let pushY = moveData.pushY || 0;
                target.vx = (attackerDirRight ? 1 : -1) * pushX;
                
                // Hất tung (Juggle)
                if (pushY < 0) {
                    target.vy = pushY;
                    target.onGround = false;
                    window.spawnDust(target.x, target.y);
                    window.playSound(400, 'sine', 0.2, 0.7);
                } 
                // Đập đất (Ground Bounce / Smash)
                else if (pushY > 0 && !target.onGround) {
                    target.vy = pushY;
                }

                // Dội tường (Wall Bounce)
                let isNearWall = (target.x < 60) || (target.x > window.canvas.width - 60);
                if (isNearWall && pushX > 5) {
                    target.vx = (attackerDirRight ? -1 : 1) * (pushX * 0.8);
                    target.vy = -6; 
                    target.onGround = false;
                    window.shakeScreen(15, 10);
                    window.spawnGlassShatter(target.x, target.y - 40);
                    window.floatingTexts.push({ x: target.x, y: target.y - 80, text: "💥 WALL BOUNCE!", color: "#f1c40f", alpha: 1.5, vx: 0, vy: -5, font: "italic 900 30px Impact", life: 50, scale: 2.0, targetScale: 1.0, scaleVel: 0, rot: 0 });
                }
            } else if (wallBounce) {
...
window.attack = function(attacker, targetGroup) {
    ...
    let dist = target.x - attacker.x;
    let mathDist = Math.abs(dist); 
    let reach = 90 * (attacker.scale || 1); 
    attacker.isFacingRight = dist > 0;

    // Lao vào nếu ở quá xa
    if (mathDist > reach && attacker.comboStep === 0) { 
        attacker.vx = (attacker.isFacingRight ? 1 : -1) * attacker.currentSpeed * 4; 
        attacker.state = 'dash'; attacker.attackTimer = 12; window.spawnDust(attacker.x, attacker.y); 
        window.speedLinesAlpha = 0.5; return; 
    }

    // AI CHỌN COMBO THÔNG MINH
    let moveName = 'jab';
    let isFinisher = false;

    // Trạng thái kẻ địch đang bị hất tung (Juggling)
    if (!target.onGround && target.y < window.GROUND_Y - 50) {
        let airMoves = ['cross', 'spinning_kick', 'ora_ora', 'axe_kick'];
        moveName = airMoves[Math.floor(Math.random() * airMoves.length)];
        if (attacker.comboStep >= 3) moveName = 'axe_kick'; // Kết liễu combo trên không
    } 
    // Combo dưới đất
    else {
        if (attacker.comboStep === 0) {
            moveName = Math.random() > 0.3 ? 'jab' : 'low_kick'; 
        } else if (attacker.comboStep === 1) {
            moveName = Math.random() > 0.4 ? 'cross' : 'uppercut'; 
        } else if (attacker.comboStep === 2) {
            moveName = Math.random() > 0.5 ? 'ora_ora' : 'chun_li_kicks'; 
        } else {
            let finishers = ['dragon_punch', 'asura_strike', 'shoryuken'];
            moveName = finishers[Math.floor(Math.random() * finishers.length)];
            isFinisher = true;
            attacker.comboStep = -1; 
        }
    }

    let moveData = window.MOVE_DATABASE[moveName] || window.MOVE_DATABASE['jab'];
    attacker.state = moveName;
    attacker.attackTimer = isFinisher ? 35 : (moveData.type === 'flurry' ? 45 : 18);
    attacker.vx = (attacker.isFacingRight ? 1 : -1) * (isFinisher ? 8 : 2);

    let baseDmg = 12 * attacker.currentDmgMod;

    // HIỆU ỨNG THỊ GIÁC ĐI KÈM
    if ((moveName === 'low_kick') && Math.random() < 0.4) { target.stunTimer = 35; target.state = 'stunned'; window.floatingTexts.push({ x: target.x, y: target.y - 50, text: "🦵", color: "#e67e22", alpha: 1.5, vx: 0, vy: -3, font: "900 35px Arial", life: 40, scale: 1.8, targetScale: 1.0, scaleVel: 0, rot: 0 }); }
    if ((moveName === 'uppercut') && Math.random() < 0.3) { baseDmg *= 1.5; window.spawnParticles(target.x, target.y - 60, "#c0392b", true); window.floatingTexts.push({ x: target.x, y: target.y - 50, text: "🩸", color: "#c0392b", alpha: 1.5, vx: 0, vy: -3, font: "900 35px Arial", life: 40, scale: 1.8, targetScale: 1.0, scaleVel: 0, rot: 0 }); }

    // XỬ LÝ MULTI-HIT (Đánh liên hoàn ORA ORA)
    if (moveData.type === 'flurry') {
        let hitsCount = 0;
        let flurryInterval = setInterval(() => {
            if (window.gameOver || attacker.hp <= 0 || target.hp <= 0 || attacker.state !== moveName) {
                clearInterval(flurryInterval); return;
            }
            window.takeDamage(target, baseDmg * moveData.dmgMod, "#f1c40f", false, false, attacker.isFacingRight, moveData);
            attacker.vx = (attacker.isFacingRight ? 1 : -1) * 3; 
            window.shakeScreen(3, 2);
            window.spawnSlash(target.x, target.y - 40, attacker.isFacingRight, "#fff", false, 1.2, (Math.random()-0.5));
            
            hitsCount++;
            if (hitsCount >= moveData.hits) clearInterval(flurryInterval);
        }, 80); 
        attacker.comboStep++;
        return; 
    }

    // XỬ LÝ ĐÒN ĐÁNH ĐƠN (Single Hit)
    let isCrit = isFinisher || Math.random() < attacker.critChance;
    let finalDmg = baseDmg * moveData.dmgMod * (isCrit ? attacker.critMult : 1);

    if (isFinisher) {
        finalDmg *= 2.5;
        window.speedLinesAlpha = 1.0;
        window.targetCamOrbitAngle = attacker.isFacingRight ? 0.3 : -0.3;
        window.actionCamOffsetX = attacker.isFacingRight ? 80 : -80;
    ...
    window.takeDamage(target, Math.floor(finalDmg), isCrit ? "#ff4757" : "#fff", isCrit, false, attacker.isFacingRight, moveData);
    
    attacker.comboStep++;
    attacker.comboHits = (attacker.comboHits || 0) + 1; 
    attacker.comboDisplayTimer = 90; 
    attacker.comboAlpha = 1; 
    attacker.stamina = Math.min(100, attacker.stamina + (isCrit ? 8.0 : 3.0));

    let slashAngle = (moveData.type === 'launcher') ? -Math.PI/4 : (moveData.type === 'smash' ? Math.PI/3 : (Math.random()-0.5)*0.2);
    window.spawnSlash(target.x, target.y - 35, attacker.isFacingRight, isCrit ? "#ff4757" : "#00f3ff", isCrit, isFinisher ? 3.0 : 1.8, slashAngle);
};
...
window.update = function() {
    ...
    allFighters.forEach(f => {
        ...
        // GROUND BOUNCE & WALL SPLAT LOGIC
        if (f.state === 'wall_splat') { 
            ...
            if (f.vy > 0 && f.y + f.vy >= window.GROUND_Y && !f.onGround) { 
                ...
                // BOUNCE EFFECTS
                if (f.vy > 12 && f.hp > 0 && f.hitStun > 0) { 
                    f.vy = -(f.vy * 0.4); 
                    f.hitStun += 10;
                    window.shakeScreen(8, 6); 
                    window.spawnEnvDamage(f.x, window.GROUND_Y, 'crater', f.scale || 1, false);
                    window.playSound(200, 'square', 0.3, 0.6, true);
                    window.floatingTexts.push({ x: f.x, y: f.y - 60, text: "〽️ BOUNCE!", color: "#e67e22", alpha: 1.5, vx: (Math.random()-0.5)*2, vy: -4, font: "italic 900 25px Impact", life: 40, scale: 1.5, targetScale: 1.0, scaleVel: 0, rot: 0 });
                } else {
                    if (f.vy > 8) window.shakeScreen(5, 3); 
                }
            } 
        }
        ...
        if (f.y >= window.GROUND_Y) { 
            f.y = window.GROUND_Y; 
            if (f.vy > -2 && f.vy < 2) f.vy = 0; // Tránh nảy vi chạm vô tận
            f.onGround = true; 
        } else { 
            f.onGround = false; 
        }
