// ==========================================
// RECORDER.JS - BẢN HOÀN HẢO CHO CONTENT CREATOR (V56.0 - ULTIMATE YOUTUBE THUMBNAIL EDITION)
// [NEW] Cỗ máy sinh Thumbnail siêu cấp: Text 3D, Hào quang, Vòng đỏ clickbait, Nhãn dán bựa, Nền Comic/Manga.
...
window.bakeThumbnailsForVideo = function(titleText) {
    ...
    // Tự động xáo trộn phong cách mỗi trận
    ...
    const drawBackground = (ctx, w, h, isVertical) => {
        ...
        if (selectedTheme === 'anime_burst') {
            ...
            ctx.fillStyle = `hsl(${hue1}, 90%, 35%)`;
            ctx.beginPath(); ctx.moveTo(0,0); ctx.lineTo(w,0); ctx.lineTo(0,h); ctx.fill();
            ctx.fillStyle = `hsl(${hue2}, 90%, 25%)`;
            ctx.beginPath(); ctx.moveTo(w,h); ctx.lineTo(w,0); ctx.lineTo(0,h); ctx.fill();
            
            // Vẽ viền sọc chéo trắng ở giữa
            ctx.lineWidth = 20; ctx.strokeStyle = "#fff";
            ctx.beginPath(); ctx.moveTo(-100, h+100); ctx.lineTo(w+100, -100); ctx.stroke();
        } else {
    ...
    // 3. VẼ NHÂN VẬT VỚI HÀO QUANG CỰC MẠNH
    ...
    const drawCharWithAura = (ctx, charObj, colorGlow) => {
        ctx.save();
        // Lớp 1: Hào quang phát sáng siêu bự
        ctx.shadowColor = colorGlow; ctx.shadowBlur = 80;
        for(let i=0; i<3; i++) {
            if(charObj.isDragon && typeof window.drawDragon === 'function') window.drawDragon(ctx, charObj); 
            else if(typeof window.drawStickman === 'function') window.drawStickman(ctx, charObj);
        }
        ctx.restore();
        // Lớp 2: Bản thân nhân vật
        ctx.save();
        if(charObj.isDragon && typeof window.drawDragon === 'function') window.drawDragon(ctx, charObj); 
        else if(typeof window.drawStickman === 'function') window.drawStickman(ctx, charObj);
        ctx.restore();
    ...
    // --- RENDER BẢN NGANG ---
    window.ctx = ctxH;
    let p1CloneH = Object.assign({}, window.p1, { x: 450, y: 880, scale: (window.p1.scale || 1) * 4.8, state: sP1, isFacingRight: true });
    let p2CloneH = Object.assign({}, e1, { x: 1470, y: 880, scale: (e1.scale || 1) * 4.8, state: sP2, isFacingRight: false });
    drawCharWithAura(ctxH, p1CloneH, `hsl(${hue1}, 100%, 60%)`);
    drawCharWithAura(ctxH, p2CloneH, `hsl(${hue2}, 100%, 60%)`);

    // --- RENDER BẢN DỌC ---
    window.ctx = ctxV;
    let p1CloneV = Object.assign({}, window.p1, { x: 540, y: 1650, scale: (window.p1.scale || 1) * 5.0, state: sP1, isFacingRight: true });
    let p2CloneV = Object.assign({}, e1, { x: 540, y: 650, scale: (e1.scale || 1) * 5.0, state: sP2, isFacingRight: false });
    drawCharWithAura(ctxV, p1CloneV, `hsl(${hue1}, 100%, 60%)`);
    drawCharWithAura(ctxV, p2CloneV, `hsl(${hue2}, 100%, 60%)`);

    window.ctx = originalCtx; // Trả lại context gốc

    // 4. CHÈN PROP MEME VÀ STICKER CLICKBAIT VÀO KHOẢNG TRỐNG
    const drawMemeProps = (ctx, w, h, isVertical) => {
        ctx.save();
        // Chèn Vòng tròn đỏ và mũi tên
        if (Math.random() > 0.3) {
            let cx = isVertical ? (Math.random() > 0.5 ? w*0.2 : w*0.8) : w/2 + (Math.random() > 0.5 ? 300 : -300);
            let cy = isVertical ? h*0.4 + Math.random()*200 : h/2 + (Math.random()*200 - 100);
            
            ctx.strokeStyle = "#ff0000"; ctx.lineWidth = 25; ctx.shadowColor = "#000"; ctx.shadowBlur = 10;
            ctx.beginPath(); ctx.ellipse(cx, cy, 150 + Math.random()*50, 100 + Math.random()*100, (Math.random()-0.5), 0, Math.PI*2); ctx.stroke();
            
            ctx.font = "120px Arial"; ctx.fillText("⬅️", cx + 150, cy - 50);
        }
        ...
        // Dán bừa Emoji Há mồm / Sợ hãi vào góc
    ...
    const draw3DText = (ctx, text, x, y, fontSize) => {
        ...
        // Điểm nhấn trắng bóng 
        ctx.fillStyle = "#ffffff"; ctx.fillText(text, x - 5, y - 5);
        ctx.restore();
    };

    // Áp dụng Text 3D vào Video
    let shortTitle = (titleText || "EPIC BRAWL").replace(/#.*/g, '').trim(); // Lọc bỏ hashtag
    ...
    draw3DText(ctxH, shortTitle, 0, -350, 120);
    ctxH.restore();
    ...
    // Tách dòng cho bản dọc dễ đọc
    let words = shortTitle.split(" ");
    let line1 = words.slice(0, Math.ceil(words.length/2)).join(" ");
    let line2 = words.slice(Math.ceil(words.length/2)).join(" ");
    draw3DText(ctxV, line1, 0, -450, 110);
    draw3DText(ctxV, line2, 0, -320, 110);
    ctxV.restore();
};
...
window.startRecording = function() {
    // Ép dừng và lưu Video cũ trước khi quay trận mới
