// ==========================================
// ENGINE.JS - THE ABSOLUTE ULTIMATE MASTERPIECE 29.0 [THE IMMERSIVE REALITY]
// [FIXED] LỖI ĐƠ MÀN HÌNH CUỐI GAME (Tách biệt Hit-stop và Time-scale)
// [FIXED] LỖI MẤT CHỮ FIGHT (Sắp xếp lại Z-Index)
// [NEW] VOLUMETRIC GOD RAYS: Tia sáng thể tích chiếu xuyên sương mù.
// [NEW] SAMURAI-CUT INTRO: Màn dạo đầu "READY -> FIGHT" chém rách màn hình.
// [NEW] 3D PARALLAX SPARKS: Tia lửa văng 3 chiều.
// ==========================================
...
window.globalIllumination = null; 
...
window.initGameEngine = function() {
    ...
    window.viralColorGrade = {
        ...
        brightness: 85 + Math.random() * 10,
        saturation: 95 + Math.random() * 30,    
    ...
    window.noiseCanvas.width = 300; window.noiseCanvas.height = 300;
    let nCtx = window.noiseCanvas.getContext('2d');
    let imgData = nCtx.createImageData(300, 300);
    for (let i = 0; i < imgData.data.length; i += 4) {
        let val = Math.random() * 255;
        imgData.data[i] = val; imgData.data[i+1] = val; imgData.data[i+2] = val;
        imgData.data[i+3] = Math.random() * 20; 
    }
...
window.playSound = function(freq, type, duration, vol, isImpact = false) { 
    ...
    try {
        ...
        if (isImpact) { 
            ...
            let snap = window.audioCtx.createOscillator(); let snapGain = window.audioCtx.createGain(); snap.type = 'square'; snap.frequency.setValueAtTime(freq * 3, t); snap.frequency.exponentialRampToValueAtTime(30, t + 0.05); snapGain.gain.setValueAtTime(safeVol * 0.4, t); snapGain.gain.exponentialRampToValueAtTime(0.01, t + 0.05);
            snap.connect(snapGain); snapGain.connect(window.audioCtx.destination); snap.start(t); snap.stop(t + 0.05);
        } else { 
...
// ==========================================
// 2. HỆ THỐNG SPAWN VFX CAO CẤP (3D)
// ==========================================
...
window.spawnParticles = function(x, y, color, isCrit = false) { 
    ...
    for(let i=0; i<count; i++) { let angle = Math.random() * Math.PI * 2; let speed = Math.random() * (isCrit?18:8) + 2; window.particles.push({ x: x, y: y - 30, vx: Math.cos(angle)*speed, vy: Math.sin(angle)*speed, life: 25, maxLife: 25, color: color, size: Math.random() * 5 + 2 }); } 
}

window.spawnDust = function(x, y) { 
    if (window.particles.length > MAX_PARTICLES) return;
    for(let i=0; i<8; i++) { window.particles.push({ x: x + (Math.random()*40-20), y: y, vx: (Math.random()-0.5)*8, vy: -Math.random()*2 - 0.2, life: 30, maxLife: 30, color: "rgba(189, 195, 199, 0.4)", size: Math.random() * 12 + 6, isGroundDust: true }); } 
}

window.spawnInk = function(x, y, color, isRight) {
    window.inkSplatters.push({ x: x, y: y, color: color, life: 35, maxLife: 35, scale: Math.random() * 0.8 + 0.6, ang: (Math.random() - 0.5)*0.5 + (isRight ? 0 : Math.PI) });
}

window.spawnForegroundDebris = function(x, y, isRight) {
    let count = 6 + Math.floor(Math.random() * 4);
    for(let i=0; i<count; i++) {
        let vx = (isRight ? 1 : -1) * (15 + Math.random() * 20); 
        let vy = -10 - Math.random() * 15;
        let scaleSpeed = 1.05 + Math.random() * 0.05; 
        window.foregroundDebris.push({
            x: x, y: y, vx: vx, vy: vy, life: 40, maxLife: 40, 
            scale: 1.0, scaleSpeed: scaleSpeed, rot: Math.random() * Math.PI, rotSpeed: (Math.random() - 0.5) * 0.4
        });
    }
}
...
window.triggerTimeStop = function(frames, caster) { 
    ...
    setTimeout(() => { window.targetTimeScale = 1.5; setTimeout(() => { window.targetTimeScale = 1.0; }, 300); }, frames * 16);
...
window.takeDamage = function(target, amount, color, isCrit, wallBounce, attackerDirRight = true) {
    ...
    if (target.state === 'dash' || target.state === 'dash_back') {
        if (target.dashTimer > 2 && target.dashTimer < 14) {
            ...
            window.floatingTexts.push({ x: target.x, y: target.y - 80, text: "⚡ FLASH STEP!", color: "#00f3ff", alpha: 1, vx: 0, vy: -4, font: "italic 900 40px 'Arial Black', Impact", life: 50, scale: 2.0, targetScale: 1.0, rot: (Math.random()-0.5)*0.2 });
    ...
    if (target.state === 'block') { 
        if (target.stamina < 15) {
            ...
            window.floatingTexts.push({ x: target.x, y: target.y - 90, text: "💔 GUARD CRUSH!", color: "#ff003c", alpha: 1, vx: 0, vy: -3, font: "italic 900 45px 'Arial Black', Impact", life: 60, scale: 2.5, targetScale: 1.0, rot: (Math.random()-0.5)*0.3 });
            window.hitStopFrames = 20; window.chromaTimer = 20; window.targetZoom = 1.45; window.vhsGlitchTimer = 20;
            window.actionCamOffsetX = 0; window.actionCamOffsetY = -30;
            window.dynamicBlur = 10;
            ...
            window.floatingTexts.push({ x: target.x, y: target.y - 80, text: "🛡️ PERFECT PARRY!", color: "#00ffff", alpha: 1, vx: 0, vy: -3, font: "italic 900 40px 'Arial Black', Impact", life: 50, scale: 2.0, targetScale: 1.0, rot: (Math.random()-0.5)*0.2 });
    ...
    if (finalDmg > 0) {
        ...
        let textVx = (attackerDirRight ? 1 : -1) * (Math.random() * 4 + 2);
        let textVy = -6 - Math.random() * 6;
        let textScale = isCrit ? 3.5 : 1.8;
        window.floatingTexts.push({ 
            x: target.x, y: target.y - 50, text: dmgText, color: color || (isCrit ? "#ff4757" : "#fff"), 
            alpha: 1, vx: textVx, vy: textVy, font: "italic 900 35px 'Arial Black', Impact", life: 45, 
            scale: textScale, targetScale: isCrit ? 1.2 : 0.8, rot: (Math.random()-0.5)*0.5 
        });

        window.spawnParticles(target.x, target.y - 40, color || "#fff", isCrit);
        
        // [VIRAL 29.0] 3D PARALLAX SPARKS (Tia lửa phóng to thu nhỏ theo Z-Axis)
        let sparkCount = isCrit ? 18 : 8;
        let baseDirX = attackerDirRight ? 1 : -1;
        for(let i=0; i<sparkCount; i++) { 
            let vx = baseDirX * (Math.random() * 25 + 5) + (Math.random()-0.5)*5; 
            let vy = (Math.random()-0.5)*20 - 5;
            let sparkZScale = Math.random() > 0.5 ? 1.05 : 0.95; // Phóng to ra trước hoặc lùi về sau
            window.impactSparks.push({ x: target.x, y: target.y - 40, vx: vx, vy: vy, life: 25 + Math.random()*15, maxLife: 40, color: color || "#f1c40f", scale: 1.0, zSpeed: sparkZScale }); 
        }

        if (isCrit) {
            window.screenFlash = 1.0; 
            window.impactFrameCount = 4; 
            window.targetZoom = 1.45; setTimeout(() => { window.targetZoom = 1.1; }, 100); 
            window.impactAberration = 15;
            window.spawnForegroundDebris(target.x, target.y, attackerDirRight);
            window.bassDropFrames = 3;

            window.spawnMangaSFX(target.x, target.y - 60, true);
            window.spawnInk(target.x, target.y - 40, color || "#ff003c", target.isFacingRight); 
            
            // Fixed HitStop & TimeScale overlap
            window.hitStopFrames = 10; // Đóng băng game hoàn toàn
            window.targetTimeScale = 1.0; // Giữ speed bình thường trong lúc hitstop
            setTimeout(()=>{ window.targetTimeScale = 0.15; setTimeout(()=>{ window.targetTimeScale = 1.0; }, 300); }, 10 * 16); // Xả slow-mo SAU khi hit-stop kết thúc
            
        } else if (Math.random() > 0.5) {
            window.spawnMangaSFX(target.x, target.y - 50, false);
            window.targetZoom = 1.15; setTimeout(() => { window.targetZoom = 1.1; }, 50);
            window.impactAberration = 4; 
        }
        ...
        if (target.hp <= 0) {
            ...
            if (isNearWall && isCrit && typeof window.triggerStageTransition === 'function') { window.triggerStageTransition(target); } 
                ...
                if (isCrit) {
                    window.screenFlash = 1.5; 
                    window.invertFrames = 6; window.speedLinesAlpha = 1.0; window.spawnLensFlare(target.x, target.y - 40, "#ff003c", 5.0);
                    window.floatingTexts.push({ x: target.x, y: target.y - 120, text: "💀 LETHAL FINISH!", color: "#ff003c", alpha: 1, vx: 0, vy: -1, font: "italic 900 85px 'Arial Black', Impact", life: 90, scale: 3.0, targetScale: 1.0, rot: 0 });
                    
                    window.targetZoom = 2.0; 
                    window.actionCamOffsetX = (target.isFacingRight ? 120 : -120);
                    window.actionCamOffsetY = -60; 
                    window.dynamicBlur = 30;
                    window.cameraTilt = (Math.random() > 0.5 ? 1 : -1) * 0.18; 
                    
                    window.lethalVoid = 120;
                    window.impactAberration = 40; 
                    window.bassDropFrames = 6; 
                    window.screenTearing = 15; 
                    window.cinemaBarsHeight = 250; 

                    window.spawnForegroundDebris(target.x, target.y, attackerDirRight);
                    window.spawnForegroundDebris(target.x, target.y, attackerDirRight); 

                    // Perfect Timing Fix
                    window.hitStopFrames = 25; // 1. Freeze
                    window.shakeScreen(60, 45); window.vhsGlitchTimer = 45; window.triggerDimensionShatter(); 
                    window.targetTimeScale = 1.0;
                    setTimeout(()=>{ 
                        window.targetTimeScale = 0.05; // 2. Super Slowmo AFTER freeze
                        setTimeout(()=>{ 
                            window.targetTimeScale = 1.0; // 3. Back to normal
                            window.cameraTilt = 0; 
                            window.cinemaBarsHeight = 0; 
                        }, 2500); 
                    }, 25 * 16); 
                } else { 
                    window.hitStopFrames = 10; window.shakeScreen(30, 25); window.targetZoom = 1.5; window.actionCamOffsetY = -40; 
                    window.impactAberration = 20; window.cinemaBarsHeight = 150; 
                    window.targetTimeScale = 1.0;
                    setTimeout(()=>{ window.targetTimeScale = 0.2; setTimeout(()=>{window.targetTimeScale = 1.0; window.cinemaBarsHeight = 0; }, 1000); }, 10 * 16);
                }
                
                window.impactFrameTimer = 0; window.chromaTimer = 40;
...
window.attack = function(attacker, targetGroup) {
    ...
    if (MathDist > reach && !isFinisher) { 
        ...
        window.actionCamOffsetX = attacker.isFacingRight ? 60 : -60;
        if (!window.gameOver) window.cinemaBarsHeight = 80;
        return; 
    }
    ...
    if (['uppercut', 'dragon_uppercut', 'high_kick'].includes(selectedMove)) {
        slashAngle = -Math.PI / 5; window.actionCamOffsetY = 60; 
    } else if (['axe_kick', 'elbow_strike'].includes(selectedMove)) {
        slashAngle = Math.PI / 5; window.actionCamOffsetY = -30; 
    ...
    if (isFinisher) {
        ...
        window.floatingTexts.push({ x: target.x, y: target.y - 80, text: "💥", color: "#ff4757", alpha: 1, vx: (Math.random()-0.5)*2, vy: -4, font: "900 65px Arial", life: 50, scale: 2.0, targetScale: 1.0, rot: (Math.random()-0.5)*0.3 });
        window.speedLinesAlpha = 1.0; window.actionCamOffsetX = attacker.isFacingRight ? 100 : -100; 
        if (!window.gameOver) window.cinemaBarsHeight = 120; 
    } else {
        if (Math.random() < attacker.critChance) { isCrit = true; finalDmg = baseDmg * attacker.critMult; window.floatingTexts.push({ x: target.x + (Math.random()*40-20), y: target.y - 60, text: "💢", color: "#f1c40f", alpha: 1, vx: 0, vy: -2, font: "italic 900 35px Arial", life: 30, scale: 1.5, targetScale: 1.0, rot: 0 }); } else { window.playSound(350, 'sine', 0.1, 0.1, false); }
        if ((selectedMove === 'low_kick' || selectedMove === 'teep_kick') && Math.random() < 0.4) { target.stunTimer = 35; target.state = 'stunned'; window.floatingTexts.push({ x: target.x, y: target.y - 50, text: "🦵", color: "#e67e22", alpha: 1, vx: 0, vy: -1, font: "900 35px Arial", life: 40, scale: 1.5, targetScale: 1.0, rot: 0 }); }
        if ((selectedMove === 'uppercut' || selectedMove === 'elbow_strike') && Math.random() < 0.3) { finalDmg *= 1.5; window.spawnParticles(target.x, target.y - 60, "#c0392b", true); window.floatingTexts.push({ x: target.x, y: target.y - 50, text: "🩸", color: "#c0392b", alpha: 1, vx: 0, vy: -1, font: "900 35px Arial", life: 40, scale: 1.5, targetScale: 1.0, rot: 0 }); }
...
window.update = function() {
    ...
    // Decay variables
    window.actionCamOffsetX *= 0.85; 
    window.actionCamOffsetY *= 0.85;
    window.dynamicBlur *= 0.9;
    if (window.impactAberration > 0) window.impactAberration -= 1.5;
    if (window.lethalVoid > 0) window.lethalVoid--;
    if (window.bassDropFrames > 0) window.bassDropFrames--;
    if (window.screenTearing > 0) window.screenTearing--;
    
    if (!window.gameOver && window.cinemaBarsHeight > 0) {
        window.cinemaBarsHeight -= 5;
        if(window.cinemaBarsHeight < 0) window.cinemaBarsHeight = 0;
    }

    if (window.invertFrames > 0) window.invertFrames--;
    ...
    if (window.vhsGlitchTimer > 0) window.vhsGlitchTimer--;
    if (window.speedLinesAlpha > 0) window.speedLinesAlpha -= 0.04;
    ...
    if (window.foregroundDebris) {
        window.foregroundDebris.forEach(d => {
            d.x += d.vx; d.y += d.vy; d.vy += window.GRAVITY;
            d.scale *= d.scaleSpeed; d.rot += d.rotSpeed; d.life--;
        });
        window.foregroundDebris = window.foregroundDebris.filter(d => d.life > 0 && d.scale < 10);
    }
    ...
    // [VIRAL 29.0] Cập nhật Intro Timer không ngắt vật lý
    ...
    // Đã chuyển HitStop ra GameLoop, không return ở đây nữa!
    ...
    allFighters.forEach(f => {
        ...
        if (f.state === 'idle' && !isTimeStopped) {
            f.x += Math.sin(Date.now() / 80 + f.x) * (f.isRage ? 1.5 : 0.8);
        }
            ...
            if (f.hitStun <= 0 && f.stunTimer <= 0 && f.state !== 'dash_back' && f.state !== 'block') {
                ...
                if (isCloseEnough) {
                    if (window.classStats && window.classStats[type] && typeof window.classStats[type].executeUltimate === 'function') {
                        f.stamina = 0; let baseDmg = 50 * (f.currentDmgMod || 1); if (!f.isPlayer) baseDmg = 35 * (f.currentDmgMod || 1); window.playSound(400, 'sine', 0.5, 0.6); window.shakeScreen(15, 10); window.spawnParticles(f.x, f.y, "#f1c40f", true); window.floatingTexts.push({ x: f.x, y: f.y - 100, text: f.isPlayer ? "🔥 ULTIMATE!" : "⚠️ DANGER!", color: f.isPlayer ? "#ff4757" : "#ff0000", alpha: 1, vx: 0, vy: -3, font: "900 35px Arial", life: 50 }); f.isFacingRight = distToTarget > 0; window.classStats[type].executeUltimate(f, closestTarget, baseDmg); f.vx = 0; window.speedLinesAlpha = 1.0; 
                    } else if (typeof window.useUltimate === 'function') { window.useUltimate(f, closestTarget); f.vx = 0; window.speedLinesAlpha = 1.0; }
        ...
        if (!launchedUltimate && f.attackTimer <= 0 && f.hitStun <= 0 && f.dashTimer <= 0 && f.stunTimer <= 0 && !window.gameOver && f.hp > 0) {
            if (f.isDragon) {
                if (f.hp > 0 && f.hp <= f.maxHp * 0.3 && !f.isEvolved) { f.isEvolved = true; window.slowMoTimer = 60; window.screenFlash = 0; window.shakeScreen(50, 15); window.playSound(50, 'sawtooth', 2.0, 1.0, true); f.color = "#8e44ad"; f.scale *= 1.25; window.floatingTexts.push({ x: f.x, y: f.y - 150, text: "🐉🔥", color: "#8e44ad", alpha: 1, vx: 0, vy: -3, font: "italic 900 60px Arial", life: 100, scale: 2.0, targetScale: 1.0, rot: 0 }); window.shockwaves.push({x: f.x, y: window.GROUND_Y, r: 10, maxR: 500, color: "#8e44ad", alpha: 1, speed: 25}); }
                ...
                if (targetFighter && targetFighter.hp > 0) {
                    ...
                    if (f.aiDelay <= 0) {
                        ...
                        if (randAction < 0.4) { f.state = 'cast'; f.attackTimer = 20; window.playSound(400, 'sine', 0.2, 0.4); window.spawnProjectile(f.x, f.y - 50, f.isFacingRight ? 15 : -15, 0, 8, "#9b59b6", Math.floor(15 * f.dmgMod), window.p1); if(Math.random() < 0.5) window.spawnProjectile(f.x, f.y - 60, f.isFacingRight ? 14 : -14, -3, 8, "#9b59b6", Math.floor(15 * f.dmgMod), window.p1); window.floatingTexts.push({ x: f.x, y: f.y - 80, text: "🥷", color: "#9b59b6", alpha: 1, vx: 0, vy: -2, font: "900 40px Arial", life: 30, scale: 2.0, targetScale: 1.0, rot: 0 }); } 
                        else if (randAction < 0.7) { window.spawnParticles(f.x, f.y, "#2c3e50"); f.x = window.p1.x + (window.p1.isFacingRight ? -80 : 80); f.y = window.p1.y; f.isFacingRight = window.p1.x > f.x; window.spawnParticles(f.x, f.y, "#9b59b6"); f.state = 'spinning_backfist'; f.attackTimer = 15; window.playSound(200, 'square', 0.2, 0.6); if(typeof window.takeDamage === 'function') window.takeDamage(window.p1, Math.floor(20 * f.dmgMod), "#9b59b6", false, false); window.floatingTexts.push({ x: f.x, y: f.y - 80, text: "💨", color: "#8e44ad", alpha: 1, vx: 0, vy: -2, font: "900 40px Arial", life: 30, scale: 2.0, targetScale: 1.0, rot: 0 }); } 
            ...
            for (let j = i + 1; j < allFighters.length; j++) { 
                ...
                if (dist < 80 && f1.attackTimer > 5 && f1.attackTimer < 20 && f2.attackTimer > 5 && f2.attackTimer < 20) {
                    if (!f1.clashCooldown && !f2.clashCooldown && f1.isFacingRight !== f2.isFacingRight) {
                        ...
                        window.floatingTexts.push({ x: midX, y: midY - 60, text: "⚔️ CLASH!", color: "#f1c40f", alpha: 1, vx: 0, vy: -3, font: "italic 900 60px 'Arial Black', Impact", life: 50, scale: 2.5, targetScale: 1.0, rot: (Math.random()-0.5)*0.2 });
                        window.spawnLensFlare(midX, midY, "#00f3ff", 4.0); window.speedLinesAlpha = 1.0;
                        window.impactAberration = 25; 
                        window.bassDropFrames = 4;
    ...
    allFighters.forEach(f => {
        ...
        if (f.x < wallBound) { 
            ...
            if (f.hitStun > 0 && f.vx < -8 && !f.wallBounced) {
                ...
                window.floatingTexts.push({ x: f.x, y: f.y - 80, text: "💥 WALL SPLAT!", color: "#f1c40f", alpha: 1, vx: 2, vy: -2, font: "italic 900 30px 'Arial Black', Impact", life: 40, scale: 2.0, targetScale: 1.0, rot: 0 });
        ...
        if (((f.state === 'dash' || f.state === 'dash_back' || f.isRage) && Math.abs(f.vx) > 1) || (isAttacking && f.attackTimer % 2 === 0)) { 
            f.trailArr.push({x: f.x, y: f.y, state: f.state, isFacingRight: f.isFacingRight, color: f.color, alpha: 0.6, scale: f.scale, isDragon: f.isDragon, timer: 0}); 
        }
        for (let i = f.trailArr.length - 1; i >= 0; i--) { 
            f.trailArr[i].timer++;
            f.trailArr[i].alpha -= 0.04; 
            if (f.trailArr[i].alpha <= 0) f.trailArr.splice(i, 1); 
        }
    });
    ...
    for (let i = window.floatingTexts.length - 1; i >= 0; i--) { 
        ...
        if (t.life !== undefined) { 
            t.vy += window.GRAVITY * 0.3; t.x += t.vx; t.y += t.vy; t.life--; 
            if (t.scale !== undefined && t.targetScale !== undefined) {
                t.scale += (t.targetScale - t.scale) * 0.2; 
            }
            if (t.life <= 0) t.alpha -= 0.05; 
        } else { 
            t.x += t.vx; t.y += t.vy; t.vy += 0.15; t.alpha -= 0.02; 
        } 
    ...
    if (window.p1 && window.introTimer === 0) {
        ...
        if (window.gameOver && closest) {
            ...
            window.targetCamX = ((window.canvas.width / 2) - midX) + window.actionCamOffsetX; 
            window.targetCamY = Math.max(0, (window.GROUND_Y - midY) * 0.5 + 40) + window.actionCamOffsetY; 
        } 
        else if (closest && !window.gameOver && window.slowMoTimer <= 0) {
            let midX = (window.p1.x + closest.x) / 2; let midY = (window.p1.y + closest.y) / 2; let maxPanX = window.canvas.width * 0.85; 
            let actionPan = (window.p1.vx + (closest.vx || 0)) * 8; 
            let desiredCamX = (window.canvas.width / 2) - midX; 
            window.targetCamX = Math.max(-maxPanX, Math.min(maxPanX, desiredCamX + actionPan)) + window.actionCamOffsetX; 
            window.targetCamY = Math.max(0, Math.min(280, (window.GROUND_Y - midY) * 0.5)) + window.actionCamOffsetY; 
            
            let distance = Math.abs(window.p1.x - closest.x); let dynamicZoom = 1.25 - (distance / 600) * 0.35; 
            ...
            window.targetCamX = ((window.canvas.width / 2) - focusX) + window.actionCamOffsetX; 
            window.targetCamY = Math.max(0, Math.min(280, (window.GROUND_Y - focusY) * 0.5)) + window.actionCamOffsetY; 
...
window.draw = function() {
    ...
    if (window.invertFrames > 0) {
        ...
        allFighters.forEach(p => { 
            if (p && p.hp > 0 && typeof window.drawStickman === 'function') { 
                ...
                let clone = Object.assign({}, p, { x: 0, y: 0 }); 
    ...
    if (window.impactFrameCount > 0) {
        ...
        allFighters.forEach(p => { 
            if (p && p.hp > 0) { 
                ...
                window.ctx.translate(p.x, p.y); if (!p.isFacingRight) window.ctx.scale(-1, 1);
    ...
    if (window.isLoading) {
        ...
        window.ctx.fillStyle = "#00f3ff"; window.ctx.font = "italic 900 40px Arial"; window.ctx.textAlign = "center"; window.ctx.shadowBlur = 15; window.ctx.shadowColor = "#00f3ff"; window.ctx.fillText("GOD OF IMPACT INITIALIZING...", cx, cy - 50);
        ...
        window.ctx.fillStyle = "#fff"; window.ctx.font = "bold 16px monospace"; window.ctx.fillText(`RENDERING KINETIC TYPOGRAPHY... ${Math.floor(window.loadProgress)}%`, cx, cy + 60);
        return; 
    }

    window.ctx.save();
    if (window.screenTearing > 0) {
        let tearY = Math.random() * window.canvas.height;
        let tearHeight = Math.random() * 150 + 50;
        let tearShift = (Math.random() > 0.5 ? 1 : -1) * (Math.random() * 50 + 20);
        
        window.ctx.beginPath();
        window.ctx.rect(0, tearY, window.canvas.width, tearHeight);
        window.ctx.clip();
        window.ctx.translate(tearShift, 0);
        window.ctx.filter = `hue-rotate(${Math.random()*360}deg) saturate(300%)`;
    }
    ...
    try {
        ...
        window.camDriftX = (window.camDriftX || 0) + (Math.random() - 0.5) * 0.5;
        window.camDriftY = (window.camDriftY || 0) + (Math.random() - 0.5) * 0.5;
        ...
        window.ctx.translate(window.canvas.width / 2, window.canvas.height / 2); window.ctx.scale(actualZoom, actualZoom); 
        ...
        if (window.screenFilter === 'grayscale') { 
            window.ctx.filter = 'grayscale(100%) contrast(120%) brightness(60%)'; 
        } else if (!window.bassDropFrames) { 
            window.ctx.filter = `contrast(${window.viralColorGrade.contrast}%) brightness(${window.viralColorGrade.brightness}%) saturate(${window.viralColorGrade.saturation}%) hue-rotate(${window.viralColorGrade.hue}deg)`;
        }
        ...
        window.ctx.save();
        window.ctx.translate(window.camX * 0.95, window.camY * 0.8); 
        window.ctx.fillRect(-3000, -3000, window.canvas.width + 6000, window.canvas.height + 6000);
        window.ctx.restore();

        // [VIRAL 29.0] VOLUMETRIC GOD RAYS (Tia sáng chiếu không gian)
        window.ctx.save(); window.ctx.globalCompositeOperation = "screen"; let rayShift = Date.now() / 3000;
        window.ctx.translate(window.camX * 0.85, window.camY * 0.7);
        for(let r=0; r<12; r++) { 
            let rayAlpha = 0.05 + Math.abs(Math.sin(rayShift + r)) * 0.08; 
            let rayGrad = window.ctx.createLinearGradient(0, -800, 0, window.GROUND_Y);
            rayGrad.addColorStop(0, `rgba(255, 255, 255, ${rayAlpha})`);
            rayGrad.addColorStop(1, `rgba(255, 255, 255, 0)`);
            window.ctx.fillStyle = rayGrad; 
            window.ctx.beginPath(); 
            window.ctx.moveTo(-500 + r*200 + Math.sin(rayShift)*150, -800); 
            window.ctx.lineTo(-300 + r*200 + Math.sin(rayShift)*150, -800); 
            window.ctx.lineTo(-100 + r*250, window.GROUND_Y); 
            window.ctx.lineTo(-400 + r*250, window.GROUND_Y); 
            window.ctx.fill(); 
        }
        window.ctx.restore();

        window.ctx.save(); window.ctx.translate(-window.camX * 0.6, -window.camY * 0.4); 
        ...
        window.ctx.save(); window.ctx.translate(-window.camX * 0.2, -window.camY * 0.1); 
        window.ctx.fillStyle = cmap.bg1;
        for(var i = -1500; i < window.canvas.width + 1500; i += 120) {
        ...
        for(let i = -1500; i < window.canvas.width + 1500; i += 250) {
        ...
        reflectionGrad.addColorStop(1, "rgba(0, 0, 0, 0.8)"); 
        
        window.ctx.globalAlpha = 0.35; 
        window.ctx.translate(0, window.GROUND_Y); 
        window.ctx.scale(1, -0.6); 
        let distY = window.shakeTime > 0 ? Math.sin(Date.now() / 50) * 10 : 0;
        window.ctx.translate(0, -window.GROUND_Y + distY);
        ...
        window.ctx.restore();
        window.ctx.save();
        window.ctx.fillStyle = reflectionGrad;
        window.ctx.globalCompositeOperation = "overlay";
        window.ctx.fillRect(-3000, window.GROUND_Y, window.canvas.width + 6000, 300);
        window.ctx.restore();

        if (window.globalIllumination && window.lethalVoid <= 0) {
            window.ctx.save();
            window.ctx.globalCompositeOperation = window.globalIllumination.mix;
            let cx = window.canvas.width/2 - window.camX;
            let cy = window.GROUND_Y - 300 - window.camY;
            let dynGlow = window.ctx.createRadialGradient(cx, cy, 0, cx, cy, 2500);
            dynGlow.addColorStop(0, window.globalIllumination.color1);
            dynGlow.addColorStop(1, window.globalIllumination.color2);
            window.ctx.fillStyle = dynGlow;
            window.ctx.fillRect(-3000, -3000, window.canvas.width + 6000, window.canvas.height + 6000);
            window.ctx.restore();
        }
        ...
        if (window.inkSplatters) {
            window.inkSplatters.forEach(ink => {
                window.ctx.save(); window.ctx.translate(ink.x, ink.y); window.ctx.rotate(ink.ang); window.ctx.scale(ink.scale, ink.scale);
        ...
        if (Math.abs(window.camVelocityX) > 5) {
            let caShift = window.camVelocityX * 0.5;
            window.ctx.save();
            window.ctx.globalCompositeOperation = 'screen';
            window.ctx.translate(caShift, 0);
            window.ctx.fillStyle = "rgba(255, 0, 0, 0.1)"; window.ctx.fillRect(-2000, -2000, 5000, 5000);
            window.ctx.translate(-caShift*2, 0);
            window.ctx.fillStyle = "rgba(0, 255, 255, 0.1)"; window.ctx.fillRect(-2000, -2000, 5000, 5000);
            window.ctx.restore();
        }
            ...
            allFighters.forEach(p => { 
                if (p && p.hp > 0 && p.trailArr) { 
                    p.trailArr.forEach(t => { 
                        ...
                        let trailP = Object.assign({}, p, {x: t.x, y: p.y, state: t.state, isFacingRight: t.isFacingRight, color: t.color, alpha: t.alpha, scale: t.scale * scaleDown}); 
        ...
        window.particles.forEach(pt => { 
            ...
            if (pt.isGroundDust) { window.ctx.beginPath(); window.ctx.ellipse(pt.x, window.GROUND_Y - pt.size/2, pt.size * 1.5, pt.size * 0.4, 0, 0, Math.PI*2); window.ctx.fill(); }
            else if (pt.isRubble) { window.ctx.save(); window.ctx.translate(pt.x, pt.y); window.ctx.rotate(pt.life * 0.1); window.ctx.fillRect(-pt.size/2, -pt.size/2, pt.size, pt.size); window.ctx.restore(); } 
            else if (pt.isGlass) { window.ctx.save(); window.ctx.translate(pt.x, pt.y); window.ctx.rotate(pt.life * 0.2); window.ctx.beginPath(); window.ctx.moveTo(0, -pt.size); window.ctx.lineTo(pt.size, pt.size); window.ctx.lineTo(-pt.size, pt.size); window.ctx.fill(); window.ctx.restore(); } 
            else { window.ctx.shadowBlur = 10; window.ctx.shadowColor = pt.color; window.ctx.beginPath(); window.ctx.arc(pt.x, pt.y, pt.size, 0, Math.PI*2); window.ctx.fill(); window.ctx.shadowBlur = 0; if (pt.isCoin) { window.ctx.strokeStyle = "#d35400"; window.ctx.lineWidth = 1; window.ctx.stroke(); } }
        }); window.ctx.globalAlpha = 1.0; window.ctx.shadowBlur = 0;
        
        window.ctx.globalCompositeOperation = 'lighter';
        window.lensFlares.forEach(lf => { let lProg = lf.life / lf.maxLife; window.ctx.globalAlpha = Math.pow(lProg, 2); window.ctx.save(); window.ctx.translate(lf.x, lf.y); let lfGrad = window.ctx.createRadialGradient(0, 0, 0, 0, 0, 300 * lf.scale); lfGrad.addColorStop(0, "#fff"); lfGrad.addColorStop(0.1, lf.color); lfGrad.addColorStop(1, "rgba(0,0,0,0)"); window.ctx.fillStyle = lfGrad; window.ctx.fillRect(-400 * lf.scale, -4 * lf.scale, 800 * lf.scale, 8 * lf.scale); window.ctx.fillRect(-8 * lf.scale, -100 * lf.scale, 16 * lf.scale, 200 * lf.scale); window.ctx.beginPath(); window.ctx.arc(0, 0, 40 * lf.scale, 0, Math.PI*2); window.ctx.fill(); window.ctx.restore(); });
        window.ctx.globalAlpha = 1.0;

        window.shockwaves.forEach(sw => { window.ctx.beginPath(); window.ctx.arc(sw.x, sw.y, sw.r, 0, Math.PI*2); window.ctx.lineWidth = 5; window.ctx.strokeStyle = sw.color; window.ctx.globalAlpha = Math.max(0, Math.min(1, sw.alpha)); window.ctx.shadowBlur = 15; window.ctx.shadowColor = sw.color; window.ctx.stroke(); window.ctx.shadowBlur = 0; });
        
        window.ctx.globalCompositeOperation = 'lighter';
        window.impactSparks.forEach(isp => { 
            window.ctx.save(); window.ctx.translate(isp.x, isp.y); 
            window.ctx.globalAlpha = Math.max(0, Math.min(1, isp.life / isp.maxLife)); 
            window.ctx.fillStyle = isp.color; 
            window.ctx.shadowBlur = 15; window.ctx.shadowColor = isp.color; 
            window.ctx.beginPath(); 
            let len = Math.sqrt(isp.vx*isp.vx + isp.vy*isp.vy) * 2.5; 
            let ang = Math.atan2(isp.vy, isp.vx); 
            window.ctx.rotate(ang); 
            window.ctx.ellipse(0, 0, len, 3, 0, 0, Math.PI*2); 
            window.ctx.fill(); 
            window.ctx.restore(); 
        });
        ...
        if (window.foregroundDebris && window.foregroundDebris.length > 0) {
            window.ctx.save();
            window.ctx.filter = 'blur(4px)'; 
            window.foregroundDebris.forEach(d => {
                window.ctx.save();
                window.ctx.translate(d.x, d.y);
                window.ctx.rotate(d.rot);
                window.ctx.scale(d.scale, d.scale);
                window.ctx.globalAlpha = Math.max(0, d.life / d.maxLife);
                window.ctx.fillStyle = "#333";
                window.ctx.fillRect(-15, -15, 30, 30);
                window.ctx.restore();
            });
            window.ctx.restore();
        }
        ...
        window.floatingTexts.forEach(t => { 
            window.ctx.save();
            window.ctx.translate(t.x, t.y);
            if (t.rot) window.ctx.rotate(t.rot);
            let s = t.scale || 1.0;
            window.ctx.scale(s, s);
            window.ctx.font = t.font || "900 22px Arial"; 
            window.ctx.fillStyle = t.color; 
            window.ctx.shadowBlur = 10; window.ctx.shadowColor = t.color; 
            window.ctx.globalAlpha = Math.max(0, Math.min(1, t.alpha)); 
            window.ctx.lineWidth = 3; window.ctx.strokeStyle = "#000";
            window.ctx.strokeText(t.text, 0, 0); window.ctx.fillText(t.text, 0, 0); 
            window.ctx.restore();
        }); 
        ...
        if (window.speedLinesAlpha > 0) {
            ...
            for(let sl=0; sl<80; sl++) { 
                let ang = (sl / 80) * Math.PI * 2 + (timeOffset % 1); 
                let r1 = 150 + Math.random() * 200; 
                let r2 = window.canvas.width; 
                window.ctx.beginPath(); 
                window.ctx.moveTo(Math.cos(ang)*r1, Math.sin(ang)*r1); 
                window.ctx.lineTo(Math.cos(ang)*r2, Math.sin(ang)*r2); 
                window.ctx.strokeStyle = `rgba(255, 255, 255, ${Math.random()})`; 
        ...
        if (impactShift !== 0) {
            ...
            window.ctx.translate(impactShift * 2, 0);
            window.ctx.fillStyle = "rgba(255, 0, 0, 0.4)"; window.ctx.fillRect(0, 0, window.canvas.width, window.canvas.height);
            window.ctx.translate(-impactShift * 4, 0);
            window.ctx.fillStyle = "rgba(0, 255, 255, 0.4)"; window.ctx.fillRect(0, 0, window.canvas.width, window.canvas.height);
            window.ctx.restore();
        }

        if (window.dynamicBlur > 0.5) {
            let cx = window.canvas.width/2; let cy = window.canvas.height/2;
            let blurGrad = window.ctx.createRadialGradient(cx, cy, window.canvas.height * 0.2, cx, cy, window.canvas.width * 0.8);
            blurGrad.addColorStop(0, "rgba(0,0,0,0)");
            blurGrad.addColorStop(1, `rgba(0,0,0,${Math.min(0.8, window.dynamicBlur/20)})`);
            window.ctx.fillStyle = blurGrad; window.ctx.fillRect(0,0, window.canvas.width, window.canvas.height);
        }
        ...
        let renderComboRank = function(fighter, xPos, align) {
            if (fighter && fighter.comboHits >= 2) {
                ...
                let rank = "D"; let rankColor = "#bdc3c7"; let rankSize = 40; let rankGlow = 10;
                if (hits >= 4) { rank = "C"; rankColor = "#2ecc71"; rankSize = 45; }
                if (hits >= 6) { rank = "B"; rankColor = "#3498db"; rankSize = 50; }
                if (hits >= 8) { rank = "A"; rankColor = "#9b59b6"; rankSize = 55; rankGlow = 15; }
                if (hits >= 12) { rank = "S"; rankColor = "#f1c40f"; rankSize = 65; rankGlow = 20; }
                if (hits >= 16) { rank = "SS"; rankColor = "#e67e22"; rankSize = 75; rankGlow = 25; }
                if (hits >= 20) { rank = "SSS"; rankColor = "#ff003c"; rankSize = 90 + Math.sin(Date.now()/50)*10; rankGlow = 35; }
                
                window.ctx.globalAlpha = alpha; window.ctx.textAlign = align;
                let bounceY = Math.sin(Date.now() / 100) * 5;
                let baseY = 80; 
                ...
                window.ctx.fillText(rank, xPos, baseY + bounceY);
                
                window.ctx.font = "italic 900 20px Arial"; window.ctx.fillStyle = "#fff"; window.ctx.shadowBlur = 5; window.ctx.shadowColor = "#000";
                window.ctx.fillText(`🔥 ${hits} HITS`, xPos, baseY + rankSize * 0.5 + 15 + bounceY);
                window.ctx.shadowBlur = 0;
            }
        };

        renderComboRank(window.p1, 40, "left");
        ...
        renderComboRank(maxEnemyCombo, window.canvas.width - 40, "right");

        if (window.gameOver && window.endIconType) {
            if (window.dimensionCracks && window.dimensionCracks.length > 0) {
                window.ctx.save();
                window.ctx.strokeStyle = "rgba(255, 255, 255, 0.8)"; window.ctx.lineWidth = 3;
                window.dimensionCracks.forEach(c => {
                    window.ctx.beginPath(); window.ctx.moveTo(c.points[0].x, c.points[0].y);
                    for(let i=1; i<c.points.length; i++) { window.ctx.lineTo(c.points[i].x + (Math.random()-0.5)*10, c.points[i].y + (Math.random()-0.5)*10); }
                    window.ctx.stroke();
                });
                window.ctx.restore();
            }
        ...
        if (window.screenFlash > 0) { 
            window.ctx.globalCompositeOperation = 'screen';
            window.ctx.fillStyle = `rgba(255, 255, 255, ${window.screenFlash})`; 
            window.ctx.fillRect(0, 0, window.canvas.width, window.canvas.height); 
            window.ctx.globalCompositeOperation = 'source-over';
        }
        ...
        // [VIRAL 29.0] CHỮ "FIGHT" NỔ TUNG MÀN HÌNH SAU KHI CẮT KIẾM
        if (window.introTimer > 0 && !window.gameOver && window.p1) {
            ...
            if (window.introTimer > 60) {
                ...
                if (window.introTimer > 100) {
                    window.ctx.fillStyle = "#fff"; window.ctx.font = "italic 900 120px 'Arial Black', Impact"; window.ctx.textAlign = "center"; window.ctx.shadowColor = "#00f3ff"; window.ctx.shadowBlur = 30; window.ctx.fillText("READY?", window.canvas.width/2, window.canvas.height/2);
                }
            } else { 
                let fadeAlpha = window.introTimer / 60; window.ctx.fillStyle = `rgba(0, 0, 0, ${fadeAlpha})`; window.ctx.fillRect(0, 0, window.canvas.width, window.canvas.height); let scale = 1 + ((60 - window.introTimer) / 60) * 0.4; window.ctx.save(); window.ctx.translate(window.canvas.width/2, window.canvas.height/2); window.ctx.scale(scale, scale); window.ctx.font = "italic 900 150px 'Arial Black', Impact"; window.ctx.fillStyle = "#ff4757"; window.ctx.strokeStyle = "#fff"; window.ctx.lineWidth = 5; window.ctx.shadowBlur = 45; window.ctx.shadowColor = "#ff4757"; window.ctx.textAlign = "center"; window.ctx.textBaseline = "middle"; window.ctx.strokeText("🔥 FIGHT! 🔥", 0, 0); window.ctx.fillText("🔥 FIGHT! 🔥", 0, 0); window.ctx.restore(); 
            }
            window.ctx.restore();
        }

        // [VIRAL 29.0] NOISE CHỐNG BOT QUÉT LÀ LỚP CUỐI CÙNG
        if (window.noiseCanvas) {
            ...
            let ptrn = window.ctx.createPattern(window.noiseCanvas, 'repeat');
            window.ctx.fillStyle = ptrn;
            window.ctx.translate(-offsetX, -offsetY);
            window.ctx.fillRect(0, 0, window.canvas.width + 100, window.canvas.height + 100);
            window.ctx.restore();
        }
...
// ==========================================
// 7. [VIRAL 29.0] TIME-ENGINE VÀ PHYSICS LOOP CHUẨN XÁC
// ==========================================
...
window.gameLoop = function(timestamp) { 
    ...
    // [VIRAL 29.0] FIX ĐƠ MÀN HÌNH: Tách Hit-Stop ra khỏi Time-Scale
    if (window.hitStopFrames > 0) {
        window.hitStopFrames--;
        // Camera vẫn được cập nhật mượt mà dù game bị đóng băng
        ...
        return; // Dừng hàm tại đây, KHÔNG CHẠY PHYSICS CỦA GAME (Tạo hiệu ứng Hit-stop chuẩn)
    }

    // Nếu không bị Hit-stop, game chạy theo Time Scale (Slow-mo / Fast-forward)
