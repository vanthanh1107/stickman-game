// ==========================================
// RECORDER.JS - V67.6 FINAL FIX (TIMELINE & CAPCUT 0-SECOND BUG)
// Tích hợp: Tự động bơm Metadata, Ép chuẩn WebM chống phân mảnh
// ==========================================

// [V67.6] Tự động tải thư viện sửa lỗi Timeline và Duration cho Video
if (typeof window.ysFixWebm === 'undefined' && !window._webmFixerLoading) {
    window._webmFixerLoading = true;
    window._webmFixerFailed = false;
    let script = document.createElement('script');
    script.src = 'https://cdn.jsdelivr.net/npm/fix-webm-duration@1.0.6/fix-webm-duration.js';
    script.onerror = () => { window._webmFixerFailed = true; console.warn("Lỗi tải thư viện fix-webm-duration"); };
...
// Biến tính toán thời lượng siêu chuẩn cho CapCut
window._recordingStartTime = 0; 
window._recordingEndTime = 0; 
...
window.startRecording = function() {
    ...
    window.currentVideoExt = "webm"; // [V67.6] ÉP CHUẨN WEBM - TUYỆT ĐỐI KHÔNG DÙNG MP4 PHÂN MẢNH
    options.mimeType = MediaRecorder.isTypeSupported('video/webm; codecs="vp8"') ? 'video/webm; codecs="vp8"' : 'video/webm';
    ...
    // [V67.6] BẮT ĐẦU BẤM GIỜ TẠI ĐÂY ĐỂ FIX LỖI CAPCUT
    ...
    const finalizeRecordings = () => {
        ...
        if (stoppedCount === 2) { 
            setTimeout(() => {
                ...
                // [V67.6] TÍNH TOÁN DURATION VÀ BƠM METADATA BẰNG ysFixWebm
                let safeFileName = window.sanitizeFileName(window.StoryModeAI.viralTitle);
                let durationMs = window._recordingEndTime - window._recordingStartTime;
                
                let processVideo = (chunks, mimeType) => {
                    return new Promise((resolve) => {
                        let blob = new Blob(chunks, { type: mimeType });
                        
                        let checkAndFix = () => {
                            if (typeof window.ysFixWebm === 'function') {
                                // Gọi thư viện bơm Metadata và Cue Points
                                window.ysFixWebm(blob, durationMs, function(fixedBlob) { 
                                    resolve(fixedBlob); 
                                });
                            } else if (window._webmFixerFailed) {
                                // Nếu mạng lác không tải được thư viện, trả về bản gốc
                                console.warn("Không thể bơm Metadata, trả về file gốc.");
                                resolve(blob); 
                            } else {
                                // Đợi thư viện tải xong (kiểm tra mỗi 500ms)
                ...
                // Xử lý song song 2 video Ngang và Dọc
                Promise.all([
                    processVideo(window.recordedChunksH, window.mediaRecorderH.mimeType),
                    processVideo(window.recordedChunksV, window.mediaRecorderV.mimeType)
                ]).then(([fixedBlobH, fixedBlobV]) => {
                    window.savedVideos.unshift({ 
                        id: Date.now(), 
                        urlH: URL.createObjectURL(fixedBlobH), 
                        urlV: URL.createObjectURL(fixedBlobV), 
...
window.stopRecording = function() { 
    ...
    // [V67.6] Chốt thời gian kết thúc siêu chuẩn
    window._recordingEndTime = Date.now();
