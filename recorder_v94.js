...
// Tối ưu Layout: Clean Game + Cinematic Mode + Live Alerts + Fake Seekbar + Green Screen Cat
// ==========================================
...
window._cinematicT = 0; // Chế độ điện ảnh
window._liveAlerts = []; // Thông báo Follow ảo
...
// Danh sách tên ảo cho thông báo Follow/Tặng quà
window.ALERT_NAMES = ["YasuoMain99", "ToxicBoy", "Skibidi", "AnimeLover", "ProGamerVN", "HackerLmao", "ChillGuy"];
...
window.captureFrames = function() {
    ...
    // --- [HOOK 1: CINEMATIC LETTERBOX] CHẾ ĐỘ ĐIỆN ẢNH KHI COMBAT ---
    if (audioPeak > 0.65) window._cinematicT += 0.1;
    else window._cinematicT -= 0.05;
    window._cinematicT = Math.max(0, Math.min(1, window._cinematicT));
    
    if (window._cinematicT > 0 && !isOutroActive) {
        let barHeight = 70 * window._cinematicT; // Viền đen 70px
        ctxV.fillStyle = "#000000";
        ctxV.fillRect(0, 0, 1080, barHeight); 
        ctxV.fillRect(0, splitGameHeight - barHeight, 1080, barHeight); 
    }
    ...
    if (!isOutroActive) {
        ...
        // Dải màu đỏ cảnh báo (Red Banner)
        let bannerY = retainY; 
        ...
        if (window.p1) {
            ...
            if (repEnemyObj) {
                ...
                // --- [HOOK 3: FAKE TIKTOK SEEKBAR] THANH TIẾN TRÌNH ÁO KÍCH THÍCH XEM HẾT ---
                let vidProgress = Math.max(0, 1 - p2Hp); // Tiến trình video tương đương máu boss đã mất
                ctxV.save();
                ctxV.fillStyle = "rgba(255, 255, 255, 0.2)"; 
                ctxV.fillRect(0, 1910, 1080, 10); // Thanh xám nhạt dưới đáy
                
                ctxV.fillStyle = "#ffffff";
                ctxV.shadowColor = "#ffffff"; ctxV.shadowBlur = 10;
                ctxV.fillRect(0, 1910, 1080 * vidProgress, 10); // Thanh trắng chạy
                
                // Chấm tròn mốc video
                ctxV.beginPath(); 
                ctxV.arc(1080 * vidProgress, 1915, 8, 0, Math.PI*2); 
                ctxV.fill();
                ctxV.restore();
            }
        }

        // --- [HOOK 2: TIKTOK LIVE ALERTS] THÔNG BÁO FOLLOW/QUÀ ẢO ---
        if (Date.now() - window._lastAlertTime > 4000 + Math.random() * 3000) {
            window._lastAlertTime = Date.now();
            let randomUser = window.ALERT_NAMES[Math.floor(Math.random() * window.ALERT_NAMES.length)];
            let alertTypes = [
                `👤 ${randomUser} vừa Follow bạn!`,
                `🎁 Faker_T1 vừa tặng 1 Hoa Hồng`,
                `🔥 Hàng ngàn người đang ấn Thích!`,
                `🚀 Video này đang lên xu hướng!`
            ];
            window._liveAlerts.push({ text: alertTypes[Math.floor(Math.random() * alertTypes.length)], life: 1.0, yOffset: 0 });
        }
        
        let alertStartY = splitGameHeight + 270; // Hiển thị phía dưới HUD nhân vật
        for (let i = window._liveAlerts.length - 1; i >= 0; i--) {
            let al = window._liveAlerts[i];
            al.life -= 0.01;
            al.yOffset += 1.5; // Trôi nhẹ lên trên
            
            ctxV.save();
            ctxV.globalAlpha = Math.max(0, al.life);
            ctxV.translate(20, alertStartY - al.yOffset);
            
            ctxV.fillStyle = "rgba(0, 0, 0, 0.7)";
            ctxV.strokeStyle = "#00f3ff";
            ctxV.lineWidth = 2;
            if(ctxV.roundRect) { ctxV.beginPath(); ctxV.roundRect(0, 0, 420, 50, 25); ctxV.fill(); ctxV.stroke(); }
            
            ctxV.fillStyle = "#fff";
            ctxV.font = "bold 22px Arial";
            ctxV.fillText(al.text, 20, 33);
            ctxV.restore();
        ...
        // --- AVATAR MÈO GREEN SCREEN / MẪU 3D ĐÁY MÀN HÌNH ---
        let vtuberY = 1620; 
