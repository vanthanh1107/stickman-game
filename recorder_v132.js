// ==========================================
// RECORDER.JS - V70.8 FINAL MASTERPIECE (GLOBAL CINEMA + VISUAL OVERLOAD)
// ĐÃ FIX: Tên nhân vật nổi 3D (Deep Scanner) & Đa ngôn ngữ (Multi-Lingual Lore).
// NÂNG CẤP KỸ XẢO: Viền đen động cơ học (Kinetic Letterbox), Tàn ảnh (Afterimage Trails), 
// Tia lửa 3D (Parallax Sparks), Nhiễu màu quang học (RGB Split).
// CẢM XÚC: Cinematic AAA Trailer, Action dồn dập 3.2s, giữ chân tuyệt đối.
...
// KHO KỊCH BẢN ĐA QUỐC GIA (GLOBAL LORES)
const LORES = [
    ...
    { chapter: "ACTO I: LA TRAICIÓN", p2Sub: "Te enseñé todo lo que sabes...", p1Sub: "Y ahora te mostraré mi ira." },
    { chapter: "АКТ I: ПРЕДАТЕЛЬСТВО", p2Sub: "Я научил тебя всему...", p1Sub: "Теперь я покажу тебе свой гнев." }
];
...
// ====================================================================
// ĐẠO DIỄN: THE MULTI-LINGUAL SAKUGA + VISUAL OVERLOAD (3.2s)
// ====================================================================
window.drawProceduralIntro = function(ctx, w, h, progress) {
    ...
    // TIMELINE: 3.2s Dồn dập
    let act = 0; let localProg = 0;
    if (progress < 0.3) { act = 0; localProg = progress / 0.3; } // Boss vỗ mặt
    else if (progress < 0.6) { act = 1; localProg = (progress - 0.3) / 0.3; } // Player đáp
    else if (progress < 0.75) { act = 2; localProg = (progress - 0.6) / 0.15; } // STROBE FLASH Tension
    else { act = 3; localProg = (progress - 0.75) / 0.25; } // Clash nổ tung
    ...
    // HÀM VẼ NHÂN VẬT & NAME TAG THỰC TẾ
    // Bổ sung cờ isTrail để không vẽ tên và bóng đổ cho các tàn ảnh
    const drawFighterArtistic = (charObj, cx, cy, forcedState, isFacingRight, theme, charName, isTrail = false) => {
        ...
        // CHỈ VẼ TÊN NẾU KHÔNG PHẢI TÀN ẢNH VÀ ĐANG Ở ACT 0 HOẶC 1
        ...
        let clone = Object.assign({}, charObj, {x:0, y:0, scale: 3.8, isFacingRight: true, state: forcedState});
    ...
    // --- HIỆU ỨNG 4: RGB CHROMATIC SPLIT (ACT 2 TENSION) ---
    if (act === 2) {
        ...
        ctx.translate(w/2, h/2 + 100); ctx.scale(5,5); 
        
        ctx.save();
        // Áp dụng filter nhiễu quang học phân tách Xanh / Đỏ
    ...
    // --- SETUP CAMERA (ACT 0, 1, 3) ---
    ctx.save();
    let cutShake = 0; 
    if (act === 3 && localProg < 0.3) cutShake = (Math.random() - 0.5) * 120; 
    ...
    if (act === 0) {
        ...
        // HIỆU ỨNG 2: AFTERIMAGE TRAILS (TÀN ẢNH TỐC ĐỘ CAO)
        for(let t=1; t<=3; t++) {
            let prevProg = Math.max(0, localProg - t*0.06);
            let prevEaseIn = Math.pow(prevProg, 3);
            let trP1 = -300 + (prevEaseIn * 250); 
            let trP2 = 300 - (prevEaseIn * 250);
            ctx.globalAlpha = 0.4 / t; // Bóng mờ nhạt dần
            drawFighterArtistic(e1, trP2, 150, 'attack', false, prm.themeP2, null, true);
            drawFighterArtistic(window.p1, trP1, 150, 'attack', true, prm.themeP1, null, true);
        }
        ctx.globalAlpha = 1.0;

        // VẼ NHÂN VẬT THỰC (Bản thể thật)
        ...
        // HIỆU ỨNG 3: PARALLAX 3D SPARKS (Tia lửa đa chiều Bokeh Blur)
        ctx.translate(0, 150);
        for(let i=0; i<60; i++) {
            let ang = Math.random() * Math.PI * 2; 
            let speed = Math.random(); 
            let dist = localProg * (500 + speed * 1200); 
            let size = (1 - localProg) * (5 + speed * 35); 
            ...
            ctx.rotate(ang);
            if (speed > 0.8) ctx.filter = "blur(5px)"; // Hạt gần bị Bokeh làm mờ
            ctx.fillRect(0, 0, size * 5, size);
            ctx.restore();
        }
    ...
    // =================================================================
    // CÁC LỚP OVERLAY BÊN NGOÀI ĐẢM BẢO KHÔNG BAO GIỜ BỊ CẮT XÉN
    // =================================================================
    ...
    // --- HIỆU ỨNG 1: KINETIC LETTERBOX (VIỀN ĐEN ĐỘNG CƠ HỌC) ---
    let barHeight = 0;
    if (act === 0 || act === 1) {
        barHeight = 180 + (localProg * 40); // Ép dần xuống từ 180 -> 220 tạo ngột ngạt
    } else if (act === 2) {
        barHeight = 220 + (localProg * 80); // Strobe Flash: Ép gắt xuống 300
    } else if (act === 3) {
        barHeight = 300 - (localProg * 1200); // Va chạm: Bung tỏa ra ngoài vũ trụ (0) ngay tức khắc
    }
    barHeight = Math.max(0, barHeight);

    if (barHeight > 0) {
        ctx.fillStyle = "#000000"; 
        ctx.fillRect(0, 0, w, barHeight); // Viền trên
        ctx.fillRect(0, h - barHeight, w, barHeight); // Viền dưới
    }

    // --- TÊN CHAPTER TRƯỢT NGANG ---
    ...
    // --- BÙM! MÀN HÌNH CHỚP LÓA LÚC VA CHẠM KẾT THÚC ---
    if (act === 3) {
