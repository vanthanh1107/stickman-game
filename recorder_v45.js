// ==========================================
// RECORDER.JS - BẢN HOÀN HẢO CHO CONTENT CREATOR (V60.0 - BRAINROT & MEME OVERLOAD)
// [NEW] KHO TRASH TALK SIÊU BỰA: Cà khịa chuẩn Gen-Z Gamer.
// [NEW] DẢI BĂNG CẢNH BÁO (Marquee): Chữ chạy "Bozo incoming", "Skill Issue"... phía sau nền.
// [NEW] HIỆU ỨNG ĐẬP CHỮ "VS" VÀ MƯA EMOJI: Rung màn hình, rắc muối, rớt dấu chấm hỏi.
// [RETAINED] Thumbnail Embed 0.5s, Auto-Cut 120 Frames, UI Inline, Anti-Empty Video.
// ==========================================
...
window.drawAnimatedIntro = function(ctx, w, h, isVertical, progress) {
    ...
    // [3] DẢI BĂNG CẢNH BÁO (MARQUEE WARNING) CHẠY NGANG SAU LƯNG NHÂN VẬT
    ...
    drawMarquee(h*0.2, window.bannerText1, 1.5, "#ffcc00", "#000"); // Dải trên
    drawMarquee(h*0.8, window.bannerText2, -2.0, "#ff003c", "#fff"); // Dải dưới

    let e1 = window.enemies && window.enemies.length > 0 ? window.enemies[0] : window.p1;
    let easeOut = 1 - Math.pow(1 - Math.min(1, progress * 4.5), 3); // Slide vào cực gắt
    ...
    // [4] VẼ NHÂN VẬT P1 & P2
    ctx.save(); ctx.beginPath(); // P1 Cắt khung
    ...
    ctx.fillStyle = "rgba(0, 150, 255, 0.25)"; ctx.fill(); 
    ...
    ctx.scale(-1, 1); // Đảo ngược lại để viết chữ k bị ngược
    ctx.fillStyle = "#ff4757"; ctx.font = "900 35px Arial Black"; ctx.fillText(window.statBadgeP2, -80, 50);
    ctx.restore();

    // Khâu lại nét chẻ đôi
    ctx.beginPath();
    ...
    // [5] VẼ KHUNG THOẠI BỰA (TRASH TALK) & MƯA EMOJI NẢY TƯNG TƯNG
    const drawSpeechAndEmoji = (x, y, text, isP1, emojiStr, popProgress) => {
        ...
        // Vẽ thêm dấu ? hoặc ! lơ lửng cho lố bịch
        if(Math.random() > 0.5) {
            ctx.fillStyle = "#f1c40f"; ctx.font = "900 70px Impact";
            ctx.fillText("⁉️", -100, -80);
        } else {
            ctx.font = "70px Arial"; ctx.fillText("🧂", -100, -80); // Lọ muối
        }
        ctx.restore();
    ...
    // Nảy Khung 2 (P2 Đáp trả cực gắt)
    ...
    // [6] RGB GLITCH SNAP-CUT TRƯỚC KHI VÀO GAME
    if (progress > 0.85) {
...
window.startRecording = function() {
    ...
    // TẠO DỮ LIỆU RÁC CHO INTRO (MEME, TRASH TALK BẨN)
    const p1Talks = ["Bro skipped the tutorial 🤡", "Your Jordans are fake.", "Touch grass immediately.", "I'm boutta end your career.", "My grandma plays better 💀", "You smell like onions.", "Hold this L bozo.", "I'm reporting you for griefing."];
