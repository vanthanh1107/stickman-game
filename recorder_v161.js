// ==========================================
// RECORDER.JS - V76.0 THE ALGORITHM BREAKER (POST-PROCESSING RENDERER)
// ĐÃ FIX & GIỮ NGUYÊN: Intro God-Tier, Vỡ kính 3D, Kanji Manga, Kill-Cam, Viral Post Kit.
// NÂNG CẤP TỐI THƯỢNG: Facecam Render Hậu Kỳ. 
// Tối Ưu: LÚC CHƠI GAME KHÔNG LOAD FACECAM ĐỂ CHỐNG LAG 100%. CHỈ GHÉP FACECAM VÀO KHI BẤM NÚT DOWNLOAD!
// ==========================================
...
// QUẢN LÝ VIDEO FACECAM (Chỉ gọi ra khi bấm Tải Xuống)
...
// HÀM HIỂN THỊ THÔNG BÁO RENDERING CHUNG
...
window.startRecording = function() {
    ...
    // TỐI ƯU HÓA: Random sẵn URL Facecam để lát nữa xài lúc tải về
    ...
    window.mediaRecorderV.onstop = () => {
        setTimeout(() => {
            ...
            window.savedVideos.unshift({ 
                ...
                facecamUrl: window._chosenFacecamUrl // Lưu lại URL facecam để lát ghép
            });
...
window.captureFrames = function() {
    ...
    if (renderNormalV) {
        ...
        if (!isOutroActive) {
            ...
            // ========================================================
            // 3. FACECAM GÁI XINH (MÀN HÌNH CHỜ TRONG LÚC CHƠI GAME ĐỂ TRÁNH LAG)
            // Lát nữa lúc Tải Xuống, thuật toán Hậu Kỳ sẽ tự động đắp video thật lên đúng cái viền neon này!
            // ========================================================
            ctxV.save();
            ...
            ctxV.fillStyle = "rgba(0,0,0,0.8)"; // Nền đen chờ ghép video
            ctxV.fill();
...
window.mergeAndDownloadVideo = function(vidId) {
    ...
    if(btn) { btn.innerText = "⏳ ĐANG GHÉP VIDEO... ĐỢI TÍ"; btn.disabled = true; btn.style.background = "#555"; }
    
    window.showRenderToast("⏳ HỆ THỐNG ĐANG GHÉP FACECAM VÀO CLIP (Tuyệt đối KHÔNG ĐÓNG TAB) Mất tầm 30s-1p...");
    ...
    rawVid.onloadeddata = () => {
        // Lấy luồng hình ảnh từ canvas và âm thanh từ video game
        ...
        recorder.onstop = () => {
            let finalBlob = new Blob(chunks, { type: recorder.mimeType });
        ...
        function drawLoop() {
            if (rawVid.paused || rawVid.ended) return;
            
            // Vẽ Game gốc
            ctx.drawImage(rawVid, 0, 0, 1080, 1920);

            // Tự động Crop Facecam
            ...
            if (vW > 0 && vH > 0) {
                ...
                // Vị trí Facecam đắp chính xác lên ô màu đen đã chừa sẵn
        ...
        html += `<div style="display: flex; gap: 20px; background: #1e293b; padding: 15px; border-radius: 10px; border: 1px solid #334155; box-shadow: 0 4px 6px rgba(0,0,0,0.3); transition: transform 0.2s;">
                    ...
                    <div style="flex: 1; display: flex; flex-direction: column; justify-content: space-between;">
                        ...
                        <div style="display: flex; flex-wrap: wrap; gap: 10px; margin-top: 15px;">
                            ...
                            <!-- NÚT TẢI XUỐNG ĐÃ ĐƯỢC CHUYỂN SANG HÀM MERGE FACECAM -->
