// ==========================================
// RECORDER.JS - V71.0 THE DEFINITIVE MASTERPIECE (DIRECTOR'S CUT)
// ĐÃ FIX: Tên nhân vật nổi 3D ở lớp trên cùng, không bao giờ bị tay chân nhân vật che khuất.
// NÂNG CẤP KỸ XẢO: Vòng sóng xung kích không gian (Shockwave Rings), Màu phim Teal & Orange.
// CẢM XÚC: Trailer phim bom tấn, Nghệ thuật đỉnh cao, Giữ chân 100% người xem.
...
// ====================================================================
// ĐẠO DIỄN: THE DEFINITIVE MASTERPIECE (PERFECT LAYERING & SHOCKWAVE)
// ====================================================================
window.drawProceduralIntro = function(ctx, w, h, progress) {
    ...
    // HÀM VẼ NHÂN VẬT THUẦN TÚY (ĐÃ TÁCH TÊN RA NGOÀI ĐỂ KHÔNG BAO GIỜ BỊ CHE)
    ...
    if (act === 2) {
        ...
        drawFighterPure(targetChar, 0, 0, 'dash', isFacing, tTheme);
        ctx.restore();
    ...
    // --- TÊN NHÂN VẬT ĐƯỢC VẼ TẠI ĐÂY (LUÔN NẰM Ở LỚP TRÊN CÙNG) ---
    if (act === 0 || act === 1) {
        ctx.save();
        ...
        // Đặt vị trí tên cố định ngay phía dưới viền đen trên cùng (an toàn 100%)
        let overlayNameY = barHeight + 50; 
        
        ctx.shadowColor = cTheme.aura; ctx.shadowBlur = 25;
